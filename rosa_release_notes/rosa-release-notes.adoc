...
As part of the {product-title} move to OVN-Kubernetes as the only supported network plugin, starting with {product-title}  4.16, if your cluster uses the OpenShift SDN network plugin, you cannot upgrade to future major versions of {product-title}  without migrating to OVN-Kubernetes. For more information about migrating to OVN-Kubernetes, see Migrating from the OpenShift SDN network plugin.
