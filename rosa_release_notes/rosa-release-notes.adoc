...
{rosa-short} clusters now have a new deletion policy. This policy is based on a set time period of customer non-response to service notifications. For more information, see xref:../rosa_architecture/rosa_policy_service_definition/rosa-hcp-life-cycle.adoc#rosa-delete-policy_rosa-hcp-life-cycle[Deletion policy].
