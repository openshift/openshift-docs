...
* **ROSA with Hosted Control Plane (HCP) cluster node limit update.** ROSA with HCP clusters can now scale to 500 worker nodes. This is an increase from the previously recommended limit of 180 nodes. For more information, see xref:../rosa_planning/rosa-limits-scalability.html[Limits and scalability].
