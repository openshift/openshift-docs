...
ifdef::openshift-rosa-hcp[]
* **{hcp-title} region added.** {product-title} is now available in the following regions:
+
...
* **{hcp-title} regions added.** {product-title} is now available in the following regions:
+
** Zurich (`eu-central-2`)
** Hong Kong (`ap-east-1`)
...
* **{product-title}.** {hcp-title} is now generally available. For more information, see xref:../rosa_hcp/rosa-hcp-sts-creating-a-cluster-quickly.adoc#rosa-hcp-sts-creating-a-cluster-quickly[Creating ROSA with HCP clusters using the default options].
