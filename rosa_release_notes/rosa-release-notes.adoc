...
* Deprecated `--private-link` flags for {product-title} clusters* With the completion of architectural changes with ROSA CLI 1.2.55, networking has been made more flexible with HCP clusters. This replaces the `--private-link` flag when creating an HCP cluster with the `--private` and `--default-ingress-private` flags. Now, users can choose to have a combination of a public or private API with a private or public ingress. Another change was that the default ingress listening method available for day 1 operations instead of the default as a day 2 operation.
