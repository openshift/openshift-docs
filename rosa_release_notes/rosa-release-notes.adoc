...
* **AWS Windows Licence Included now available for {product-title}.** You can now add a Windows Licence Included enabled machine pool to a {hcp-title} cluster. For more information, see xref:../rosa_cluster_admin/rosa_nodes/rosa-managing-worker-nodes.adoc#creating_machine_pools_cli[Managing compute nodes].
