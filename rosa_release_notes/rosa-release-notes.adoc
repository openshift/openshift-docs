...
A new deletion policy for {hcp-title} clusters based on a set period of customer non-response to service notifications has been implemented. For more information, see xref:../rosa_architecture/rosa_policy_service_definition/rosa-hcp-life-cycle.html#rosa-delete-policy_rosa-hcp-life-cycle[Deletion policy].
