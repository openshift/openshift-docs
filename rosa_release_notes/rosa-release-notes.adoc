...
* **Configurable process identifier (PID) limits update.** You can now configure the maximum number of PIDs in a cluster using {cluster-manager}. For more information, see xref:../rosa_cluster_admin/rosa-configuring-pid-limits.adoc#setting-higher-pid-limit-on-existing-cluster-ocm_rosa-configuring-pid-limits[Setting the PIDs limit using {cluster-manager}].
