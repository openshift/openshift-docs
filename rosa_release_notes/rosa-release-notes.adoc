...
* **AWS Trainium and Inferentia instance types now supported.** You can now use {AWS} Trainium and Inferentia instance types for your {product-title} clusters. For more information, see xref:../rosa_architecture/rosa_policy_service_definition/rosa-hcp-instance-types.adoc#[Red Hat OpenShift Service on AWS (ROSA) with hosted control planes (HCP) instance types].
