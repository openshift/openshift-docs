...
toc::[]
You can migrate virtual machine (VM) storage either _offline_ (when the VM is turned off) or _online_ (when the VM is running). For KubeVirt VMs, the main reason for migrating storage is to move from one storage class to another, perhaps to rebalance between storage providers or to use a new storage class better suited for the workload.
...
Before migrating VM storage, you must meet the following prerequisites:

* The xref:../virt/install/installing-virt.adoc#installing-virt-operator_installing-virt[[{VirtProductName} Operator] must be installed.
...
* You need OpenShift Virtualization version 4.17 or later to support live storage migration. Earlier versions of {VirtProductName} do not support live storage migration.

* The {mtc-short} Custom Resource Definitions (CRDs) must be present, and you need at least two storage classes.

* For online migration, the VM must be running.
