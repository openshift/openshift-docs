...
[NOTE]
====
If you are looking for a quick start guide for {product-title}, see the {product-title} quick start guide.
====

The procedures in this document use the `auto` modes in the {rosa-cli} and {cluster-manager} to immediately create the required IAM resources using the current AWS account. The required resources include the account-wide IAM roles and policies, cluster-specific Operator roles and policies, and OpenID Connect (OIDC) identity provider.
