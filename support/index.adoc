...
// TODO: Include this in ROSA HCP when the Logging book is migrated.
...
** xref:../observability/logging/logging_release_notes/cluster-logging-support.adoc#cluster-logging-must-gather-collecting_cluster-logging-support[Collecting information about your logging environment by using the `oc adm must-gather` command]
