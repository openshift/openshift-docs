...
Insights repeatedly analyzes the data Insights Operator sends, which includes workload recommendations from Deployment Validation Operator (DVO). Users of OpenShift Container Platform can display the results in the {insights-advisor-url}. Users of {product-title} can display the results in the {insights-advisor-url} service on {hybrid-console}.
