...
[[about_oce_features]]
== {oce} features
While {oce} is based on {product-title}, the two products are different in
several ways. The following sections describe the key features and functions of
{oce} and several important ways in which it differs from {product-title}.
...
Helm, and {project-title} templates. {oce} users can use both the `oc` and
...
You can access all maintained and curated content that the {project-title}
...
You can use {oce} in multi-cluster deployment configuration to the same extent
as {product-title}. Consolidated views of clusters and the use of Kubernetes
technologies to offer an agnostic layer across public and on-premises clouds
is allowed with {oce}.
...
{oce} subscription. {project-title}’s Kubernetes CNI plug-in for automation of
multi-tenant network segmentation between {project-title} projects is not
...
found in {project-title}. That ingress solution is not supported in {oce}.
...
With {oce}, the following {project-title} capabilities are not supported:

* The developer experience utilities and tools.
* {project-title}’s pipeline feature that integrates a streamlined,
...
* The developer persona in the {project-title} web console.
