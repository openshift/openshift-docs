...
ifndef::openshift-dedicated,openshift-rosa[]
OpenShift Dedicated::
A managed {op-system-base} {product-title} offering on Amazon Web Services (AWS) and {gcp-full}. OpenShift Dedicated focuses on building and scaling applications.
endif::openshift-dedicated,openshift-rosa[]
