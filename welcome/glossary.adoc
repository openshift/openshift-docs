...
Service layers facilitate connectivity between application components. For example, a front-end web service can communicate with a database instance through its service. Services also enable simple internal load balancing across multiple pod instances. {product-title} automatically injects service details into running containers to streamline service discovery.
