...
The product version is: {product-version}.

[id="network-flow-matrix-control2_{context}"]
.Control plane node base flows
[%header,format=csv,subs="attributes+"]
|===
pass:a[include::https://raw.githubusercontent.com/openshift-kni/commatrix/{product-version}/docs/stable/unique/common-master.csv[]]
|===
