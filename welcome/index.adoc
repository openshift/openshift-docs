...
ifdef::openshift-aro[]
{product-title} is a fully managed Red Hat OpenShift service that is jointly engineered and supported by Microsoft and Red Hat. For information on the following topics, check out the {product-title} on the link:https://docs.microsoft.com/en-us/azure/openshift/[Microsoft Azure documentation site].
* Installing a {product-title}.
* Upgrading a {product-title} cluster.
* Getting support for your {product-title} cluster.
endif::[]
