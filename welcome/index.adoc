...
- *xref:../installing/acs-installation-platforms.adoc#installation-platforms[Installation methods for different platforms]*: Determine the best installation method for your platform.
- *xref:../installing/install-ocp-operator.adoc#installing-using-an-operator[Installing by using an Operator]*: Learn how to install {product-title} on {ocp} or a Kubernetes cluster by using the {product-title} Operator. This is the recommended installation method for most platforms.
- *xref:../installing/installing_helm/install-helm-quick.adoc#acs-install-helm-quick[Installing using Helm charts]*: Learn how to install {product-title} by using Helm charts. This is the recommended installation method for Amazon Elastic Kubernetes Service (Amazon EKS), Google Kubernetes Engine (Google GKE), and Microsoft Azure Kubernetes Service (Microsoft AKS).
- *xref:../installing/install-quick-roxctl.adoc#install-quick-roxctl[Installing by using the `roxctl` CLI]*: Learn how to install {product-title} by using the `roxctl` command-line interface (CLI). `roxctl` is a CLI for running commands on {product-title}.
