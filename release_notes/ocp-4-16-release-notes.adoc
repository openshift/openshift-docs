...
////
// Bug marked as internal. Confirming with the SME.
[id="ocp-4-16-36-enhancements_{context}"]
==== Enhancements

[id="ocp-4-16-36-irqbalance-aarch64_{context}"]
===== Added the `irqbalance` package to `AArch64` architecture builds

{op-system-first} adds the `irqbalance` package to `AArch64` architecture builds. This package distributes hardware interrupts across processors on a multiprocessor system, so that performance increases occur on low latency applications. (link:https://issues.redhat.com/browse/OCPBUGS-47787[*OCPBUGS-47787*])
////
