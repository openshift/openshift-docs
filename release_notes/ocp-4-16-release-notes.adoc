...
[id="ocp-4-16-networking-openshift-sdn-upgrade-block"]
==== OpenShift SDN network plugin blocks future major upgrades

As part of the {product-title} transition to OVN-Kubernetes as the only supported network plugin, if your cluster uses the OpenShift SDN network plugin, starting with {product-title} 4.16, you cannot upgrade to future major versions of {product-title} without migrating to OVN-Kubernetes.
