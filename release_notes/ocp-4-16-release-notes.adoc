...
* Previously, the `openshift-install` CLI failed to connect to the bootstrap node when collecting bootstrap gather logs. The installation program reported an error message such as `The bootstrap machine did not execute the release-image.service systemd unit`. With this release, 


What: when the openshift-install failed to connect to the bootstrap node to collect bootstrap gather logs, the installer program would report the error as "The bootstrap machine did not execute the release-image.service systemd unit"
Fix: adjusted the message to now say "Invalid log bundle or the bootstrap machine could not be reached and bootstrap logs were not collected"
