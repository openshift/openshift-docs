...
* Previously, the `nodeip-configuration` service did not send logs to the serial console, which made it difficult to debug problems when networking is not available and there is no access to the node. With this release, the `nodeip-configuration` service logs output to the console for easier debugging when there is no network access to the node. (link:https://issues.redhat.com/browse/OCPBUGS-19628[*OCPBUGS-19628*])
