...
If you use the OVN-Kubernetes network plugin, you can configure the transit and join subnets. The transit, join, and masquerade subnets can be configured either during cluster installation or after. For the masquerade subnet, it is only configurable as a Day 0 operation. The subnet defaults are:
