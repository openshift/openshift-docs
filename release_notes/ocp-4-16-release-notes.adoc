...
* Previously ClusterTasks are listed on the *Pipelines builder* page and on the ClusterTask list page in the *Tasks* navigation menu. Currently, the ClusterTasks are deprecated from Pipelines 1.17 and the ClusterTask dependency is removed from static plugin. On the  Pipelines builder page, you will only see the task that is present in the namespace and community tasks. (link:https://issues.redhat.com/browse/OCPBUGS-45015[*OCPBUGS-45015*])
