...
Although the expected wake up latency is under 20μs, latencies exceeding this can occasionally be seen when running the `cyclictest` tool for long durations (24 hours or more). 
Testing has shown that wake up latencies are under 20μs for over 99.999999% of the samples.
(link:https://issues.redhat.com/browse/RHEL-16385[*RHEL-16385*])
