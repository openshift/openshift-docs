...
[id="ocp-4-16-ipv6-default-ipvlan"]
==== IPv6 unsolicited neighbor advertisements on IPVLAN CNI plugin (General Availability)

Pods created by using the `ipvlan` CNI plugin, where the IP address management CNI plugin has assigned IPs, now send IPv6 unsolicited neighbor advertisements by default onto the network.
