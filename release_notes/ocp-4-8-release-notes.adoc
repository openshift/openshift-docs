...
* On an {product-title} Single Node Openshift (SNO) configuration, pod creation times are over two times slower when using the realtime (RT) kernel than when using the non-RT kernel. The slower creation times impact the maximum number of supported pods when using the RT kernel, because recovery time is consequently impacted after a node reboots. Booting with the `rcupdate.rcu_normal_after_boot=1` kernel command line argument improves recovery time, which requires an RT kernel version `kernel-rt-4.18.0-305.16.1.rt7.88.el8_4` or later. (link::https://bugzilla.redhat.com/show_bug.cgi?id=1975356[*BZ#1975356*], link::https://bugzilla.redhat.com/show_bug.cgi?id=1995431[*BZ#1995431*])
