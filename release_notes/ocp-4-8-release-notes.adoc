...
You can now create a performance profile using the Performance Profile Creator (PPC) tool. The tool consumes `must-gather` data from the cluster, several user-supplied profile arguments, and generates a performance profile that is appropriate for your hardware and topology.

//For information about running the tool with `podman` or with a supplied wrapper script see add link to creating a performance profile (depends on https://github.com/openshift/openshift-docs/pull/30639).
