...
* Previously, the *Topology* view showed an error or a blank page while loading because the OpenShift Samples Operator was not installed. So there were no templates in the OpenShift namespace. The current release fixes this issue. The *Topology* view now loads as expected. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1949810[BZ#1949810])
...
* Previously, routes tied to the *Developer* perspective were not rendered. This produced 404 error messages when you refreshed the *Developer* perspective. The current release fixes this issue by loading all the inactive routes. Now, the routes switch to the correct perspective before loading. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1929769[BZ#1929769])
...
* Previously, if the API server encountered a conflict while updating a resource quota, it sometimes failed to create a resource and returned a 409 status code. In that case, the client had to retry the API request. With this update, when the API server encounters this type of conflict, it retries creating the resource up to three times. This is often sufficient to create the resource. Otherwise, the API server displays an error in the console. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1920699[BZ#1920699])
...
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1932472[BZ#1932472])

* Previously, the topology URLs created for deployments using Bitbucket repository in the {product-title} web console did not work if they included a branch name that contained a slash character. This was due to an issue with the Bitbucket API (link:https://jira.atlassian.com/browse/BCLOUD-9969[BCLOUD-9969]). The current release mitigates this issue: if a branch name contains a slash, the topology URLs point to the default branch page for the repository. This issue will be fixed in a future release of {product-title}.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1969535[BZ#1969535])

* Previously, if you created a pipeline with parameters that had empty strings (`”`) as the default value, the fields in the {product-title} web console did not accept the empty strings. The current release fixes this issue. Now, `”` is supported as a valid default property within the parameters section.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1951043[BZ#1951043])

* Previously, when an error occurred due to a user not having the required access for a namespace, the *Workload* drop-down menu in the *Monitoring* dashboard page continuously displayed a loading-in-progress icon. The current release fixes this issue. Now, the *Monitoring* dashboard page displays an error message indicating that a *Forbidden* error has occurred.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1930546[BZ#1930546])
