...
[id="ocp-4-8-adjust-net-queues"]
==== Modifying the net queues using the Performance Addon Operator
Configuring the performance profile associated with the Performance Addon Operator allows you to adjust the queue count for each network device. Device network queues allow packets to be distributed among different physical queues and each queue gets a separate thread for packet processing.
For DPDK based workloads it is important to reduce the net queues to just the number of reserved/housekeeping CPUs to ensure the desired low latency is achieved.

Refer to section in the docs for more information.
