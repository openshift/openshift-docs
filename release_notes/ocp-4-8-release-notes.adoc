...
[id="ocp-4-8-nodes-descheduler-profile"]
==== New descheduler profile available (Developer Preview)

The `DevPreviewLongLifecycle` descheduler profile is now available as a Developer Preview. This profile provides cluster balancing similar to the `LifecycleAndUtilization` profile, but does not evict pods older than 24 hours.

[IMPORTANT]
====
The `DevPreviewLongLifecycle` profile is for Developer Preview purposes and is not supported by Red Hat at this time.
====
