...
[id="ocp-4-8-35-bug-fixes"]
==== Bug fixes
* Previously, ExternalIPs were managed by Kubernetes. With this update, administrators will now have to direct traffic from externalIPs to the cluster by assigning a destination IP via a static route, some route protocol or by adding an IP to a node. For guidance see (link:https://access.redhat.com/solutions/6954851[KCS*])(link:https://bugzilla.redhat.com/show_bug.cgi?id=2076662[*BZ#2076662*])
