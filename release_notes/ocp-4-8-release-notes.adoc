...
The egress IP feature of OpenShift SDN now balances network traffic roughly equally across nodes for a given namespace, if that namespace is manually assigned multiple egress IP addresses. Each IP address must reside on a different node.
For more information, refer to xref:../networking/openshift_sdn/assigning-egress-ips.adoc#assigning-egress-ips[Considerations when using manually assigned egress IP addresses] for OpenShift SDN.
