...
For more information, see xref:../updating/updating-restricted-network-cluster/restricted-network-update-OSUS.adoc#update-service-overview_updating-restricted-network-cluster-OSUS[Understanding the OpenShift Update Service].
...
[id="ocp-4-8-web-console-developer-perspective"]
...
* Use the options in the *Add* page to create applications and associated services and deploy these applications and services on {product-title}. The *Add* page options include: *Getting started resources*, *Creating applications using samples*, *Build with guided documentation*, and *Explore new developer features*.
...
* Create multiple event source types in {product-title} and connect these source types to sinks. You can take functions deployed as a Knative service on a {product-title} cluster and connect them to a sink.
* Use the `finally` task in the pipeline to execute commands in parallel.
* Use code assistance in the *Add Task* form to access the task parameter values. To view the pipeline parameter, and the correct syntax for referencing that particular pipeline parameter, go to the respective text fields.
* Run a `Task` only after certain conditions are met. Use the `when` field to configure the executions of your task, and to list a series of references to `when` expressions.
...
==== The oc-compliance plugin is now available
...
For more information, see xref:../security/compliance_operator/oc-compliance-plug-in-using.adoc#using-oc-compliance-plug-in[Using the `oc-compliance` plugin]
...
Starting with {product-title} 4.8, automatic migration for the following in-tree volume plugins to their equivalent CSI drivers is available as a Technology Preview feature:
...
|GA
|GA
|xref:../release_notes/ocp-4-8-release-notes.adoc#ocp-4-8-34-removed-feature-azure-mint-mode[REM]
...
[id="ocp-4-8-removed-feature-azure-mint-mode"]
==== Support for minting credentials for Microsoft Azure removed

Starting with {product-title} 4.8.34, support for using the Cloud Credential Operator (CCO) in mint mode on Microsoft Azure clusters has been removed from {product-title} 4.8. This change is due to the planned link:https://azure.microsoft.com/en-us/updates/update-your-apps-to-use-microsoft-graph-before-30-june-2022/[retirement of the Azure AD Graph API by Microsoft on 30 June 2022] and is being backported to all supported versions of {product-title} in z-stream updates.
...
* Previously, the Container Network Interface (CNI) plugin for Multus did not understand IPv6 addresses that started with any number of zeros. With this update, the CNI plugin works with IPv6 that start with values greater than zero. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1919048[*BZ#1919048*])
...
* Previously, node reboots using a `CNI` plugin would not complete successfully.  CRI-O was modified to call `CNI DEL` on all containers that were running before reboot. This update cleans up `CNI` resources and allows a successful reboot. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1948137[*BZ#1948137*])
...
* A minimum help message was being printed for the Operator SDK because the default plugin was not invoked when `--help` was passed on the command line interface (CLI). This fix invokes the default plugin and prints a more useful help message when a user runs the `operator-sdk init --help` command. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1866222[*BZ#166222*])
...
[id="ocp-4-8-20"]
=== RHBA-2021:4574 - {product-title} 4.8.20 bug fix update
...
* The current opt-in obfuscation will not work on clusters with OVN because the `hostsubnets.network.openshift.io` is not currently on OVN clusters. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2009322[*BZ#2009322*])


[id="ocp-4-8-20-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster using the CLI] for instructions.

[id="ocp-4-8-21"]
=== RHBA-2021:4716 - {product-title} 4.8.21 bug fix update
...
[id="ocp-kubernetes-1-21-5-updates"]
===== Updates from Kubernetes 1.21.5

This update contains changes from Kubernetes 1.21.5. More information can be found in the following changelog: link:https://github.com/kubernetes/kubernetes/blob/release-1.21/CHANGELOG/CHANGELOG-1.21.md#v1215[1.21.5].


[id="ocp-4-8-21-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster using the CLI] for instructions.

[id="ocp-4-8-22"]
=== RHBA-2021:4830 - {product-title} 4.8.22 bug fix and security update
...
[id="ocp-kubernetes-1-21-6-updates"]
===== Updates from Kubernetes 1.21.6

This update contains changes from Kubernetes 1.21.6. More information can be found in the following changelog: link:https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md#v1216[1.21.6].


[id="ocp-4-8-22-updating"]
==== Updating
...
[id="ocp-4-8-24"]
=== RHBA-2021:4999 - {product-title} 4.8.24 bug fix and security update

Issued: 2021-12-14

{product-title} release 4.8.24, which includes security updates, is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2021:4999[RHBA-2021:4999] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2021:4998[RHBA-2021:4998] advisory.
...
[id="ocp-4-8-29-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster using the CLI] for instructions.

[id="ocp-4-8-31"]
=== RHBA-2022:0484 - {product-title} 4.8.31 bug fix and security update
...
[id="ocp-4-8-31-features-ip-reconciler"]
===== IP reconciliation for Whereabouts CNI plugin

A new enhancement to the Whereabouts CNI plugin adds an IP reconciliation job, `ip-reconciler`, which runs as a Kubernetes cronjob.
...
(https://bugzilla.redhat.com/show_bug.cgi?id=2028966[*BZ#2028966*])

[id="ocp-4-8-31-bug-fixes"]
==== Bug fixes
...
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2039377[*BZ#2039377*])

* Previously, the `standard-csi` storage class did not include a value for the `reclaimPolicy` field.
As a result, the OpenStack Cinder CSI Driver Operator continuously printed `StorageClassUpdated` events in the logs.
This update provides a default value to that `reclaimPolicy` field.
The Operator no longer prints excessive update events in the logs.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2049088[*BZ#2049088])

* Before this update, several subcommands of the `oc set` command did not correctly work with the `--dry-run` option.
Commands using the `dry-run=server` option performed updates to resources.
This update fixes the `--dry-run` option so that the `oc set` subcommands work as expected.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2038931[*BZ#2038931*])

* Previously, the web console did not read the `resource` property for `ServiceBinding`-type resources.
As a result, the topology view did not display service binding connectors.
This update fixes the problem. The web console reads the `resource` property and correctly displays service binding connectors.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2019301[*BZ#2019301*])

[id="ocp-4-8-31-updating"]
==== Updating
...
[id="ocp-4-8-34"]
=== RHBA-2022:0795 - {product-title} 4.8.34 bug fix update
...
[id="ocp-kubernetes-1-21-8-updates"]
===== Updates from Kubernetes 1.21.8

This update contains changes from Kubernetes 1.21.6 up to 1.21.8. More information can be found in the following changelogs: link:https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md#changelog-since-v1217[1.21.7] and link:https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.21.md#changelog-since-v1218[1.21.8].

[id="ocp-4-8-34-removed-feature-azure-mint-mode"]
==== Removed features

Starting with {product-title} 4.8.34, support for using the Cloud Credential Operator (CCO) in mint mode on Microsoft Azure clusters has been removed from {product-title} 4.8. This change is due to the planned link:https://azure.microsoft.com/en-us/updates/update-your-apps-to-use-microsoft-graph-before-30-june-2022[retirement of the Azure AD Graph API by Microsoft on 30 June 2022] and is being backported to all supported versions of {product-title} in z-stream updates. For more information, see xref:../release_notes/ocp-4-8-release-notes.adoc#ocp-4-8-removed-feature-azure-mint-mode[Support for minting credentials for Microsoft Azure removed].

[id="ocp-4-8-34-known-issues"]
==== Known issues

* Currently, neither {op-system-first} nor the Machine Config Operator images change in a version bump, for example, when upgrading from {product-title} 4.8.20 to 4.8.21. Consequently, the upgrade is marked as `complete` when control plane nodes are still in an `updating` state. When marked as `upgrading`, users cannot perform other procedures because it can break the update. As a temporary workaround, users should wait on control plane nodes to complete updating before performing other procedures. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2025396[*BZ#2025396*])

[id="ocp-4-8-34-bug-fixes"]
==== Bug fixes

* Previously, the `SystemMemoryExceedsReservation` alert using Prometheus QL was using `hugepages` memory consumption. Consequently, the alert was triggered unnecessarily on the cluster for {product-title} 4.8. With this update, Linux `hugepages` have been removed from the system memory circulation, and the alert no longer unnecessarily triggers. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2028854[*BZ#2028854*])

* Previously, {product-title}, with OVN-Kubernetes, managed ingress access to services via ExternalIP. When upgrading from 4.8.33 to 4.8.34, access `ExternalIP` stops work with issues like "No Route to Host". With this update, administrators will now have to direct traffic from externalIPs to the cluster. For guidance, see (link:https://access.redhat.com/solutions/6954851[KCS*]) and (link:https://kubernetes.io/docs/concepts/services-networking/service/#external-ips[*Kubernetes External IPs*]) (link:https://bugzilla.redhat.com/show_bug.cgi?id=2076662[*BZ#2076662*])

[id="ocp-4-8-34-updating"]
==== Updating
...
[id="ocp-4-8-37"]
=== RHBA-2022:1369 - {product-title} 4.8.37 bug fix update
...
==== Bug fixes
* Previously, when deleting a node, the Local Storage Operator issued a request to delete the persistent volume (PV), leaving the PV in the terminating state while connected to a pod. This update removes the owner reference from the PV, preventing it from going into the terminating state when deleting a node. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2072573#[*BZ#2072573*])

[id="ocp-4-8-37-updating"]
==== Upgdating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster within a minor version by using the CLI] for instructions.

[id="ocp-4-8-39"]
=== RHBA-2022:1427 - {product-title} 4.8.39 bug fix update
...
* Currently, upgrading from 4.8 to 4.9 is not available. {product-title} users are advised to upgrade to the next version when it is available in the appropriate release channel. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2068601[*BZ#2068601*])

[id="ocp-4-8-39-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster within a minor version by using the CLI] for instructions.

[id="ocp-4-8-41"]
=== RHSA-2022:2272 - {product-title} 4.8.41 bug fix and security update
...
[id="ocp-kubernetes-1-21-11-updates"]
===== Updates from Kubernetes 1.21.11

This update contains changes from Kubernetes 1.21.9 up to 1.21.11. More information can be found in the following changelogs: link:https://github.com/kubernetes/kubernetes/blob/release-1.21/CHANGELOG/CHANGELOG-1.21.md#downloads-for-v1219[1.21.9], link:https://github.com/kubernetes/kubernetes/blob/release-1.21/CHANGELOG/CHANGELOG-1.21.md#downloads-for-v12110[1.21.10], and link:https://github.com/kubernetes/kubernetes/blob/release-1.21/CHANGELOG/CHANGELOG-1.21.md#downloads-for-v12111[1.21.11].

[id="ocp-4-8-41-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster using the CLI] for instructions.

[id="ocp-4-8-42"]
=== RHBA-2022:4737 - {product-title} 4.8.42 bug fix update
...
[id="ocp-4-8-43"]
=== RHBA-2022:4952 - {product-title} 4.8.43 bug fix and security update
...
[id="ocp-4-8-45"]
=== RHBA-2022:5167 - {product-title} 4.8.45 bug fix update
...
==== Bug fixes
* Previously, `NetworkManager-wait-online.service` timed out preventing a connection to `coreos-installer` before it fetched the Ignition config files. With this update, `NetworkManager-wait-online.service` has more time to load allowing `coreos-installer` to fetch Ignition config files. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1983773#[*BZ#1983773*])

[id="ocp-4-8-45-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster within a minor version by using the CLI] for instructions.

[id="ocp-4-8-46"]
=== RHBA-2022:5424 - {product-title} 4.8.46 bug fix update
...
[id="ocp-4-8-47"]
=== RHBA-2022:5889 - {product-title} 4.8.47 bug fix update
...
[id="ocp-4-8-48"]
=== RHBA-2022:6099 - {product-title} 4.8.48 bug fix update

Issued: 2022-08-25

{product-title} release 4.8.48 is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2022:6099[RHBA-2022:6099] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2022:6098[RHBA-2022:6098] advisory.
...
[id="ocp-4-8-48-bug-fixes"]
==== Bug fixes
* Previously, the installation program used the first hosts listed in the `platform.baremetal.hosts` section of `install-config.yaml` as control plane machines, regardless of their roles. With this update, the role is honored if present. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2025901#[*BZ#2025901*])
...
[id="ocp-4-8-49"]
=== RHSA-2022:6308 - {product-title} 4.8.49 bug fix and security update

Issued: 2022-09-14

{product-title} release 4.8.49, which includes security updates, is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2022:6308[RHSA-2022:6308] advisory. There are no RPM packages with this update.
...
[id="ocp-4-8-50-bug-fixes"]
==== Bug fixes
* Previously, operators had to delete and recreate the IngressController to enable PROXY protocol for clusters updated to 4.8. With this update, setting the IngressController API status fields `spec.endpointPublishingStrategy.hostNetwork.protocol` or `spec.endpointPublishingStrategy.nodePort.protocol` to `PROXY` updates the fields on upgraded clusters. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2084337#[*BZ#2084337*])

[id="ocp-4-8-50-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster using the CLI] for instructions.

[id="ocp-4-8-51"]
=== RHSA-2022:6801 - {product-title} 4.8.51 bug fix and security update

Issued: 2022-10-13

{product-title} release 4.8.51, which includes security updates, is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2022:6801[RHSA-2022:6801] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2022:6800[RHBA-2022:6800] advisory.
...
[id="ocp-4-8-51-updating"]
==== Updating

To update an existing {product-title} 4.8 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc[Updating a cluster using the CLI] for instructions.

[id="ocp-4-8-52"]
=== RHBA-2022:7034 - {product-title} 4.8.52 bug fix update

Issued: 2022-10-26

{product-title} release 4.8.52 is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2022:7034[RHBA-2022:7034] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2022:7032[RHBA-2022:7032] advisory.
...
[id="ocp-4-8-52-bug-fixes"]
==== Bug fixes
* Previously, the liveness probe would timeout after one second, causing the kuryr-controller to restart. This fix increases the default time limit, resulting in the kuryr-controller running for a longer period. (link:https://issues.redhat.com/browse/OCPBUGS-472?jql=key%20in%20(%27OCPBUGS-472%27%2C%27OCPBUGS-1977%27%2C%27OCPBUGS-1023%27)[*OCPBUGS-216*])
...
[id="ocp-4-8-53"]
=== RHSA-2022:7874 - {product-title} 4.8.53 bug fix and security update

Issued: 2022-11-18

{product-title} release 4.8.53, which includes security updates, is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2022:7874[RHSA-2022:7874] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2022:7873[RHBA-2022:7873] advisory.
...
[id="ocp-4-8-54"]
=== RHBA-2022:8619 - {product-title} 4.8.54 bug fix update

Issued: 2022-11-30

{product-title} release 4.8.54 is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2022:8619[RHBA-2022:8619] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2022:8618[RHBA-2022:8618] advisory.
