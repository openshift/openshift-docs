...
* Previously, for clusters that were configured to have a separate `/var` partition, querying the filesystem returned the size only of disks mounted in the root directory, excluding the size of the `/var` partition. This fix changes how that query runs, and users can now determine the total size of the filesystem on a cluster. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1960612[*BZ#1960612*]))
