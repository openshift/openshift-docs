...
==== NTP servers deployed on control plane nodes for disconnected operation

In {product-title} {product-version}, installer-provisioned clusters deployed without connecting to a routable network can deploy NTP servers on control plane nodes and configure worker node NTP clients to retrieve time from NTP servers on the control plane nodes.
