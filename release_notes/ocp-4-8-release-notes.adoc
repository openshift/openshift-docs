...
[id="ocp-4-8-preparing-upgrade-next-release"]
==== Preparing to upgrade to the next {product-title} release

{product-title} 4.8.10 introduced a check that will impact all upgrades to the next {product-title} release, currently planned as {product-title} 4.9. This is because Kubernetes 1.22 removed a significant number of deprecated `v1beta1` APIs. This check is to try to prevent issues after upgrading where the removed APIs are still in use.

All clusters will require a manual acknowledgment by an administrator that the cluster was evaluated for removed APIs and instances were migrated to an appropriate `v1` API version. You cannot upgrade to {product-title} 4.9 until this acknowledgment is complete.

For more information on the list of removed APIs, tips for how to evaluate your cluster for removed APIs, and how to provide the acknowledgment, see link:todo-kbase-link[Preparing to upgrade to OpenShift Container Platform 4.9].
