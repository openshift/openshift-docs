...
* Previously, while creating a sample application the Developer Console creates multiple resources. These resources depend on each other and should be created in a specific order. This issue has now been fixed by creating the resources step by step leading to a more stable application. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1933664[*BZ#1933664*])

* Previously, the history listener used to fire telemetry events of the URL changes. The history listener is added when the console UI is rendered but is not removed when the console UI is unmounted. This issue has now been fixed by removing the history listener when the console UI is unmounted. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1961897[*BZ#1961897*])
