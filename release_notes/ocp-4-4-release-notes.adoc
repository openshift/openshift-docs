...
* Previously, when a user with limited cluster-wide permissions created an application in the *Add* page, by using the *Container Image* and *Image name from the internal registry* options, no imagestreams were detected in the project, though they existed. Now, the imagestream fetching logic has been moved from the cluster level to the namespace level, which enables a user with permission to a namespace to see imagestreams in that namespace.(link:https://bugzilla.redhat.com/show_bug.cgi?id=1784264[*BZ#1784264*])
...
* In the *Developer* perspective, when invalid characters such as a period (.) are entered in the serverless traffic distribution tag, the traffic distribution feature stops working but displays no error messages to prevent the user from entering invalid characters in the tag. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1822344[*BZ#1822344*])
