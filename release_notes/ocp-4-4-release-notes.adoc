...
[id="cp-4-4-notable-technical-changes"]
== Notable technical changes

{product-title} 4.4 introduces the following notable technical changes.

[discrete]
[id="ocp-4-4-bundle-registry-dev-preview"]
=== New Operator bundle format and `opm` CLI (Developer Preview)

[IMPORTANT]
====
These features are currently in Developer Preview and are not intended for
production use. Developer Feature features provide early access to upcoming
product innovations, enabling you to test functionality and provide feedback
during the development process. These features are not currently supported and
limited documentation is available.
====

Operator Lifecycle Manager (OLM) in {product-title} 4.4 introduces a new
_Operator bundle_ format as a Developer Preview feature. A bundle is a container image
that stores Kubernetes manifests and metadata associated with an Operator and is
meant to present a specific version of an Operator. The new format requires two
directories named `manifests` and `metadata`:
...
the new Operator bundle format, also as a Developer Preview feature. You can use
the `opm` CLI tool to:
...
documentation on these Developer Preview features:
