...
was unable to initialize behind ab MITM proxy. Now, the certificates are
correctly propagated to the kube-controller-manager, and the cloud provider
works as expected with an MITM proxy.
...
* Because HAProxy timeout values can be sensitive to some applications, such as
...
approve InstallPlans (for example, a link showing that _1 requires approval_)
...
makes the the CNO fatal, which is a violation of RFC 3986. This meant that the
CNO would not work at all and because it was cluster-critical, it had a major
impact. This bug fix parses it with golangs `url.ParseRequestURI`, which
implemented RFC 3986 correctly and a few more RFCs. As a result, capital case is
now allowed in httpProxy and httpsProxy.
...
changed its path causing SDN to have a nil deference trying to parse the empty
...
* Previously, local ref settings on imagestreams could be ignored if the
...
complete. This results in accurate local ref imagestream policies even
