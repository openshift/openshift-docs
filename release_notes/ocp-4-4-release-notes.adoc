...
* Earlier, when two consecutive rollouts failed, the *Topology* view showed failed pods instead of displaying the last active revision. With this bug fix, when a rollout fails the last active revision is displayed. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1760828[*BZ#1760828*])

* Previously, when a user with limited cluster-wide permissions created an application using the *Container Image* option in the *Add* page, and chose the *Image name from the internal registry* option, no imagestreams were detected in the project, though an imagestream existed. Now, the imagestream fetching logic has been moved from the cluster level to the namespace level enabling an user with permission to a namespace to see imagestreams in that namespace.(link:https://bugzilla.redhat.com/show_bug.cgi?id=1784264[*BZ#1784264*])
...
* When a user selects a node or a group of nodes in the *Topology* view on the Firefox browser, the backgrounds of all the associated labels and nodes become transparent. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1822337[*BZ#1822337*])

* In  the *Topology view*, when a user selects a node or workload and then clicks the *View monitoring dashboard* link in the *Monitoring* tab on the side panel, the user sees the monitoring dashboard for that specific workload. This filtered workload dashboard view is not clearly named, causing confusion with the generic dashboard that displays metrics for all the workloads. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1822331[*BZ#1822331*])

* In the *Developer* perspective, when invalid characters such as a period (.) are entered in the serverless traffic distribution tag, it stops working, but displays no error messages to prevent invalid characters from being entered in the tag. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1822344[*BZ#1822344*])
