...
[id="ocp-4-4-telemetry"]
=== Telemetry

[id="ocp-4-4-insights-operator-anonymized-csr"]
==== Insights Operator now collects anonymized CSRs

With this enhancement, Insights Operator periodically collects anonymized
certificate signing requests (CSR) to identify CSRs that are not verified in
Kubernetes or have not been approved. Additionally, Insights Operator collects
data if certificates are valid. As a result, this helps to improve the OpenShift
customer support experience.
// Red Hat Jira Ticket: CCXDEV-227
