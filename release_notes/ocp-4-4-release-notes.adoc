...
* After upgrading an {product-title} 4.1 or 4.2 cluster to version 4.4, the
MachineSets cannot scale up because they still reference the previous versioned
cluster's boot image. As a workaround, complete the following steps:
+
. Successfully upgrade to {product-title} 4.4 from a cluster that was originally
installed as version 4.1 or 4.2.
. Identify and copy the {op-system} 4.4 image/AMI.
. Update each MachineSet with the newer {op-system} image reference.
+
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1829642)[*BZ#1829642*])
