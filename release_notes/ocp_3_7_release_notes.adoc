...
start back up. The following Ansible variables must now be set in your inventory
file before upgrading or installing:
+
--
...
--
+
These variables should match what is currently used in the cluster. If you do
not remember what these values are, you can find them in
*_/etc/origin/master/master-config.yaml_* on a master with the names
`clusterNetworkCIDR` (for `osm_cluster_network_cidr`), `hostSubnetLength` (for
`osm_host_subnet_length`), and `serviceNetworkCIDR` (for
`openshift_portal_net`). These variables are automatically set for masters when
using the scale-up playbook. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1451023[*BZ#1451023*])
...
* The `OPTIONS` value in the *_/etc/sysconfig/atomic-openshift-node_* fo;e now
