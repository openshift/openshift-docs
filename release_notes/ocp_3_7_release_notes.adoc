...
{product-title} 3.7 as a set of containers.

[IMPORTANT]
====
CloudForms (CFME) 4.6 is not yet released. Until it is available, this role is
limited to installing ManageIQ (MIQ), the open source project that CFME is based
on.
====

 CFME is an available API endpoint on
...
See xref:../install_config/cfme/index.adoc#install-config-cfme-intro[Deploying
Red Hat CloudForms on OpenShift Container Platform] for more information.
