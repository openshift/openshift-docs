...
Network Policy is a specification of how groups of pods are allowed to communicate with each other and other network endpoints. It
...
For example, the *allow-to-red* policy specifies all red pods in the same namespace as the NetworkPolicy object allow
traffic from any pods in any namespace.
...
See
xref:../admin_guide/managing_networking.adoc#admin-guide-networking-networkpolicy[Managing
...
* Limit NFS provisioning of the Quick Installer to hosts that are part of the
cluster. If a host is outside of the cluster, where Ansible will not run, use
