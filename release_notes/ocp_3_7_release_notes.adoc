...
[[ocp-37-changing-admin-role-permissions]]
==== Changing admin role permissions

The following are changes to the default `admin` role from version 3.6. These changes are relevant to any custom roles created from the `admin` role, which might need to be readjusted after an upgrade.

* The `jobs` resource is no longer part of the `[extensions]` API group and can no longer be used with the `create`, `delete`, `deletecollection`, `get`, `list`, `patch`, `update`, and `watch` verbs.
* The `[rbac.authorization.k8s.io]` API group has been added, as well as the `rolebindings`, and `roles` resources, which can be used with the `create`, `delete`, `deletecollection`, `get`, `list`, `patch`, `update`, and `watch` verbs.
* The `policies` and `policybindings` resources have been removed from the `[authorization.openshift.io]` API group and can no longer be used with the `get`, `list`, `patch`, and `watch` verbs.
* The `generatedeploymentconfigs` resource has been removed from the `[build.openshift.io]` API group and can no longer be used with the `create`, `delete`, `deletecollection`, `get`, `list`, `patch`, `update`, and `watch` verbs.
* The `resourceaccessreviews` and `subjectaccessreviews` resources have been added to the `[authorization.openshift.io]` API group and can be used with the `create` verb.
* The `servicebindings` and `serviceinstances` resources have been added to the `[servicecatalog.k8s.io]` API group and can be used with the `create`, `delete`, `get`, `list`, `patch`, `update`, and `watch` verbs.
* The `podpresets` resource has been added to the `[settings.k8s.io]` API group and can be used with the `create`, `delete`, `get`, `list`, `update`, and `watch` verbs.
...
[[ocp-37-reencrypt-routes-expose-registry]]
==== Re-encrypt Routes are Now Supported for Exposing the {product-title} Registry

You can gain external access to the {product-title} registry 
xref:../install_config/registry/securing_and_exposing_registry.adoc#exposing-the-registry[using a re-encrypt route]. 
This allows you to log in to the registry from outside the cluster using the route address and to tag and push images using the route host.
