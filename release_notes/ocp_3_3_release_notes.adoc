...
[IMPORTANT]
====
The `*net.ipv4.tcp_syncookiesnet.ipv4.tcp_syncookies*` sysctl has been
whitelisted upstream because it has been namespaced in kernels >= 4.6. However,
it is not yet supported in {product-title} 3.3 as it is not yet namespaced in
RHEL 7 kernels. See
