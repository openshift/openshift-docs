...
Central fails to start when using {product-title-short} with Federal Information Processing Standards (FIPS)-enabled clusters. Do not upgrade to release 4.8.3 if you have FIPS-enabled clusters. You must upgrade to release 4.8.4 or later.
...
//ROX-30837
* In this update, users previously experienced issues starting Central in {product-title-short} 4.8.3 on FIPS-enabled clusters due to non-FIPS compliant `roxctl` binaries. With this release, the `roxctl` binaries in the {product-title-short} main image are no longer built with `strictfipsruntime` GOTAGS, fixing a startup error. As a result, Central now operates in {product-title-short} with FIPS-enabled {ocp} clusters.
