...
=== Metrics
OpenShift Dedicated now includes integrated Hawkular metrics as part of the Base Cluster. This integration allows visibility into memory, CPU, and network usage for each pod.

This metrics integration includes:

.  xref:osd-usability-project-overview[Project Overview] Integration
.  Metrics tab for specific Pods/Deployments (Applications > Pods > specific pod > Metrics)
...
=== Logging
OpenShift Dedicated now includes integrated logging, using an EFK stack, as part of the Base Cluster.
