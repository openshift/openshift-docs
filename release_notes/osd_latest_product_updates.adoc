...
{product-title} now includes integrated Hawkular metrics as part of the Base
Cluster. This integration allows visibility into memory, CPU, and network usage
for each pod.

In the web console, this metrics integration includes:

* xref:osd-usability-project-overview[Project Overview] integration
* Metrics tab for specific pods or deployments
** Available via *Applications* -> *Pods* -> specific pod -> *Metrics*
...
Having metrics installed on the cluster also enables the
xref:osd-web-console-ab-routing[A/B Routing] feature.

[[osd-integrated-logging]]
=== Logging

{product-title} now includes integrated logging, using an EFK stack, as part of
the Base Cluster.

In the web console, this logging integration includes:

* Logging tab for specific pods
** Available via *Applications* -> *Pods* -> specific pod -> *Logs*
* xref:ocp33-web-console-monitoring[Monitoring Tab] integration
* Access to aggregate or historical data using a Kibana URL specific to each
cluster
** Also available via *View archive* link on *Logging* tab
