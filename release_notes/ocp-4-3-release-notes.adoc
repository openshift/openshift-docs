...
* When running a third-party network plugin that uses a standalone kube-proxy
started by the Cluster Network Operator (CNO), the kube-proxy Pods crash. This
issue was introduced in {product-title} 4.3.8.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1820778[*BZ#1820778*])

// BZ#1820778 is set to be fixed in 4.3.11. Will follow BZ for updates.
