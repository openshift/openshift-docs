...
[id="ocp-4-3-ibm-z-restrictions"]
=== IBM Z and LinuxONE Restrictions

Note the following restrictions for {product-title} on IBM Z and LinuxONE:

* {product-title} for IBM Z does not include the Technology Preview for:
...
** HyperPAV enabled on IBM System Z or the virtual machine for FICON-attached ECKD storage.
