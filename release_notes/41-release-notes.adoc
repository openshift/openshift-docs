...
- xref:../release_notes/41-release-notes.adoc#launching-vulnerability-management-20-technology-preview_release-notes-41[Launching Vulnerability Management 2.0 (Technology Preview)]

- xref:../release_notes/41-release-notes.adoc#proxy-configuration-for-secured-cluster-services-on-rhacs-cloud-service_release-notes-41[Proxy configuration for secured cluster services on {product-title-managed-short}]
...
- xref:../release_notes/41-release-notes.adoc#sso-configuration-using-roxctl-cli_release-notes-41[SSO configuration using `roxctl` CLI]
...
[id="launching-vulnerability-management-20-technology-preview_{context}"]
=== Launching Vulnerability Management 2.0 (Technology Preview)

In {product-title-short} 4.1, you can experience significant improvements in Vulnerability Management with the introduction of Workload CVEs (Technology Preview). This feature is the first step towards Vulnerability Management 2.0 and provides you with improved workflows that are purposeful and effortless to navigate.

These improved workflows provide actionable data with just a few clicks to help you address vulnerabilities. It is anticipated that Vulnerability Management 1.0 functionality will gradually transition to Vulnerability Management 2.0 in upcoming releases.

This release provides a new navigation menu item, Vulnerability Management 2.0, which currently provides a view of vulnerabilities in applications running on clusters in your system. It is anticipated that in upcoming releases, this menu item will also provide a view of vulnerabilities present in platform and nodes.
...
In this technology preview, you can explore the read-only *Workload CVEs* page. However, upcoming releases are expected to include additional functionality such as Reporting, Risk Acceptance, and Manage Watched Images functionality. For more information, see xref:../operating/manage-vulnerabilities/vulnerability-management.adoc#vulnerability-management-process_acs-operating-manage-vulnerabilities[Vulnerability Management process].

:FeatureName: Vulnerability Management 2.0
include::snippets/technology-preview.adoc[]


[id="proxy-configuration-for-secured-cluster-services-on-rhacs-cloud-service_{context}"]
=== Proxy configuration for secured cluster services on {product-title-managed-short}

With this release, you can establish a reliable connection between the secured cluster and the designated proxy server, ensuring seamless data collection and transmission.
This improves the overall security and performance of your {rh-rhacscs-first} environment and enables effective monitoring and protection of your cluster resources.

With this feature, you can set the `HTTP_PROXY` and `HTTPS_PROXY` variables to define the proxy server addresses for `HTTP` and `HTTPS` connections, respectively.
You can also use the `NO_PROXY` variable to specify hostnames or IP addresses that should not be accessed through the proxy server. For more information, see xref:../cloud_service/installing_cloud_ocp/configuring-the-proxy-for-secured-cluster-services-in-rhacs-cloud-service.adoc#configuring-the-proxy-for-secured-cluster-services-in-rhacs-cloud-service[Configuring the proxy for secured cluster services in {product-title-managed-short}].
...
You can configure the settings required to delegate scanning through the {product-title-short} APIs. It is anticipated that in an upcoming release, you will also be able to do this configuration through {product-title-short} UI.
...
You can now simplify and focus the Compliance Dashboard results by selecting only the built-in default standards you want to see in your results. You can now configure the dashboard to display default compliance standards, such as CIS Docker v1.2.0 and HIPAA 164.

You can click the *Manage standards* button in the Compliance Dashboard to remove the standards that you do not want to display in the dashboard. For more information, see xref:../operating/manage-compliance.adoc#run-compliance-scan_manage-compliance[Running a compliance scan].
...
You can create declarative configurations for authentication and authorization resources such as authentication providers, roles, permission sets, and access scopes. Configurations are stored in YAML files and mounted during Central installation. For more information, see xref:../operating/manage-user-access/manage-role-based-access-control-3630.adoc#declarative-configuration-auth-resources_manage-role-based-access-control[Declarative configuration for authentication and authorization resources].

[id="sso-configuration-using-roxctl-cli_{context}"]
=== SSO configuration using `roxctl` CLI

In {product-title-short} 4.1, you can enhance Central's security and login capabilities with the new feature of configuring an authentication provider with `roxctl` CLI.
This feature enables users to use the `roxctl` CLI and integrate with authentication systems such as OpenID Connect (OIDC) for a seamless single sign-on (SSO) experience.

Previously, the `roxctl` CLI only supported token and password-based workflows. For more information, see xref:../cli/getting-started-cli.adoc#configuring-an-authentication-provider-in-central_cli-getting-started[Configuring an authentication provider in Central].
...
* The ability to secure clusters built on a host OS that is not currently part of the Collector support package.
* Uptime is improved and common problems related to non-existent kernel drivers are reduced.
* Offline customers can update the kernel of the host OS without having to upload a new support package to Central.
...
The Network graph (2.0 preview) is now generally available (GA). The earlier *Network graph (1.0)* page has been removed. Improvements and bug fixes have been made to the network graph page, including the following changes:
...
* Network Policy Simulation feature includes additional verbal clarification in the UI regarding how the network policies are generated.

The option to apply recommended network policies directly from the UI has been discontinued as this is not a recommended best practice. For network graph documentation, see xref:../operating/manage-network-policies.adoc#manage-network-policies[Managing network policies].

[id="policy-management-simplification_{context}"]
=== Policy Management simplification

{product-title-short} 4.1 introduces improvements to policy creation and editing at the UI to provide a more intuitive experience and reduce the potential for errors: Some criteria field names have been revised to be more descriptive. When you drag a criteria field into place, a clear sentence in natural language now indicates the condition that triggers that criteria

Further, the UI now only displays the criteria that are relevant to the policy in question and hides all unnecessary options. In addition, the documentation has been adapted to the layout of the UI to ensure consistency and make it easier for you to find the help and guidance you need.
...
The following new permission sets have been added:
...
* Vulnerability Management Admin: Provides administrative access to analyze vulnerabilities, generate reports, and manage the risk acceptance process.
* Vulnerability Management Consumer: Provides read-only access to analyze vulnerabilities and initiate the risk acceptance process.
...
* The Central persistent volume claim (PVC) `stackrox-db` is now considered deprecated and is no longer needed after this upgrade.
...
* You can now configure the time interval used to determine the frequency of scanning Orchestrator-level components (Kubernetes, OpenShift, Istio) with the `ROX _ORCHESTRATOR_ VULN _SCAN_ INTERVAL` environment variable.
...
| `/v1/imagecves/suppress` and `/v1/imagecves/unsuppress`
| GA
| GA
| DEP
...
Network graph 1.0 has been removed after the depreciation notice in {product-title-short} 4.0. Network graph 2.0 performs the same functionality and is generally available.
...
* Previously, there used to be crash loop when reinstalling Central with Postgres enabled. This issue occurred due to a mismatch between the life cycle of the `central-db-password` secret and the `central-db` PVC. The operator intentionally did not delete the PVCs when deleting Central to avoid data loss, but deleted the secret that contained the password.
+
...
+
This can lead to inaccuracies or inconsistencies in the reported GID. This issue does not impact the {product-title-short} UI or policy criteria. Also, when using ePBF on {ibm-z} and {ibm-linuxone}, the GID process might be invalid (`-1`) for processes that terminate shortly after execution. There is currently no workaround for this issue. (ROX-17459)
