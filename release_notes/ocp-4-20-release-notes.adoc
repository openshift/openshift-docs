...
Additionally, the volume populators feature, which was introduced in (product-title) 4.12 as a Technology Preview feature, is now supported as generally available.

Volume population is enabled by default. However, {product-title} does not ship with any volume populators.

// For more information about volume populators, see ???
...
//For more information about performance plus, see ???

[id="ocp-release-notes-storage-change-block-tracking_{context}"]
==== Change block tracking (Developer Preview)
Changed block tracking enables efficient and incremental backups and disaster recovery for persistent volumes (PVs) managed by Container Storage Interface (CSI) drivers. 
...
:FeatureName: Change block tracking
...
For more information about One Zone, see xref:../storage/container_storage_interface/persistent-storage-csi-aws-efs.adoc#efs-one-zone_persistent-storage-csi-aws-efs[Support for One Zone].

[id="ocp-release-notes-storage-storage-security-policy_{context}"]
==== Configuring fsGroupChangePolicy and selinuxChangePolicy ast namespace and pod level is generally available
Certain characteristics of a volume can cause pod startup delays, which might cause pod timeouts.

For volumes with a lot of files, pod startup timeouts can occur because, by default, {product-title} recursively changes ownership and permissions for the contents of each volume to match the `fsGroup` specified in a pod’s `securityContext` when that volume is mounted. This can be time consuming, slowing pod startup. You can use the `fsGroupChangePolicy` parameter inside a `securityContext` to control the way that {product-title} checks and manages ownership and permissions for a volume. 

Changing this parameter at the pod level was introduced in {product-title} 4.10. In 4.20, you can set this parameter at the namespace level as well as pod level as a generally available feature.

SELinux (Security-Enhanced Linux) is a security mechanism that assigns security labels (contexts) to all objects (files, processes, network ports, etc.) on a system. These labels determine what a process can access. When a pod starts, the container runtime recursively relabels all files on a volume to match a pod’s SELinux context. For large volumes, this can significantly increase pod startup times. 

{product-title} 4.20 introduces the ability to set the parameter `selinuxChangePolicy` at both the namespace and pod level as a  generally available feature.

//For more information about Configuring fsGroupChangePolicy and selinuxChangepolicy per namespace , see ???use same link as above RWX/RWO SELinux Mount???

[id="ocp-release-notes-storage-rwx-rwo-selinux-mount_{context}"]
==== RWX/RWO SELinux Mount (Technology Preview)
Pods might take a significant amount of time to start when the volume contains a large number of files. To avoid SELinux labeling issues while keeping SELinux confining, you can enable the ReadWriteMany/ReadWriteOnce (RWX/RWO) SELinux Mount feature. With this feature, the persistent volume (PV) file system is mounted with the right SELinux context, thus avoiding relabeling every file. 

This feature was introduced in {product-title} 4.17 as a Developer Preview feature, but is now supported in 4.20 as a Technology Preview feature.

//For more information about RWX/RWO SELinux Mount, see ???
...
By default, {product-title} creates Manila storage classes that provide access to all IPv4 clients. In {product-title} 4.20, you can limit client access by defining custom storage classes that use specific client IP addresses or subnets by using the `nfs-ShareClient` parameter.
...
|Configuring fsGroupChangePolicy/selinuxChangePolicy per namespace
|Not Available
|Not Available
|General Availability
