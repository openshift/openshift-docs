...
* The SuperMicro ARS-111GL-NHR server is unable to access virtual media during boot when the virtual media image is served through an IPv6 address. As a consequence, users cannot use virtual media functionality on the SuperMicro ARS-111GL-NHR server model with an IPv6 network configuration. (link:https://issues.redhat.com/browse/OCPBUGS-60070[*OCPBUGS-60070*])
