...
[id="ocp-4-19-installation-and-update-vsphere-multiple-nics_{context}"]
==== Installing a cluster on {vmw-full} with multiple network interface controllers (Generally Available)

With this release, the option to install a {vmw-full} cluster with multiple network interface controllers (NICs) for a node is Generally Available.
