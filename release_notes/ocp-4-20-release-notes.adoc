...
The `oc desccribe mcp` output has a new `ImageBuildDegraded` status field. If an {image-mode-os-on-lower} build succeeded, but build pod dependencies failed, the build pod and MOSB were incorrectly reported as completed. Now, the status correctly reports as `ImageBuildDegraded`.
...
Support for deploying pods and containers into Linux user namespaces is now generally available and enabled by default. Running pods and containers in individual user namespaces can mitigate several vulnerabilities that a compromised container can pose to other pods and the node itself. For more information, see _Running pods in Linux user namespaces_.
