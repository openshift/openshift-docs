...
* When updating a Dell server BMC firmware, the Redfish API is temporarily disrupted. This may cause connection failures and trigger Ironic to mark the update as fail. To work around this issue, manually update the BMC firmware outside Ironic to avoid service disruption. (link:https://issues.redhat.com/browse/OCPBUGS-61871[*OCPBUGS-61871*])
