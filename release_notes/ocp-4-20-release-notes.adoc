...
[id="ocp-release-notes-installer_{context}"]
=== Installer

[id="ocp-release-notes-using-UUIDs-for-Transit-Gateway-and-VPC-installer_{context}"]
...
* Before this update, when you installed a Konflux-built cluster on {ibm-power-server-title}, the installation could fail due to errors in semantic versioning (SemVer) parsing. With this release, the parsing issue is resolved so that the installation can continue successfully. (link:https://issues.redhat.com/browse/OCPBUGS-61120[OCPBUGS-61120])
