...
* Firmware updates on Hewlett Packard Enterprise (HPE) DL110G11 servers and similar models may fail due to a  bug that is specific to this hardware, caused by the way the 'NetworkAdapters' resource is implemented. It may become unavailable during the update. To work around this issue, manually update the Baseboard Management Controller (BMC) firmware outside Ironic to avoid service disruption. (link:https://issues.redhat.com/browse/OCPBUGS-60708[*OCPBUGS-60708*])
