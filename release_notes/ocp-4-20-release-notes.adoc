...
==== Support for worker nodes with Intel Sierra Forest CPUs (Technology Preview)

With this release, you can use the PerformanceProfile custom resource (CR) to configure worker nodes on machines equipped with Intel Sierra Forest CPUs. These CPUs are fully supported when configured with a single NUMA domain (NPS=1).

[id="ocp-release-notes-amd-turin-support_{context}"]
==== Support for worker nodes with AMD Turin CPUs

With this release, you can use the PerformanceProfile custom resource (CR) to configure worker nodes on machines equipped with AMD Turin CPUs. These CPUs are fully supported when configured with a single NUMA domain (NPS=1).
