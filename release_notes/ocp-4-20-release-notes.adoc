...
With this release, the `HostFirmwareComponents` resource for bare metal describes network interface controllers (NICs) and you can update the host firmware components of NICs on the supported servers, Intel Ethernet 800 Series (`ice` driver) and the NVIDIA Mellanox ConnectX-6 (CX6) and ConnectX-7 (CX7) (`mlx_5` driver).
