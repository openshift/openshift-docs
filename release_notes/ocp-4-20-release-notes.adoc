...
Updated boot images is now supported as a Technology Preview feature for {vmw-first} clusters. This feature allows you configure your cluster to update the node boot image whenever you update your cluster. By default, the boot image in your cluster is not updated along with your cluster. For more information, see _Updated boot images_.
...
For more information, see _On-cluster image mode known limitations_.

[id="ocp-release-notes-machine-config-operator-boot-image_{context}"]
==== {image-mode-os-on-caps} status reporting improvements

The `oc describe mcp` output has a new `ImageBuildDegraded` status field. If an {image-mode-os-on-lower} build succeeded, but build pod dependencies failed, the build pod and MOSB were incorrectly reported as completed. Now, the status correctly reports as `ImageBuildDegraded`.
...
You can now use the `kernelType` parameter in a `MachineConfig` object on nodes with on-cluster custom layered images to install a realtime kernel on the node.
Previously, on nodes with on-cluster custom layered images the `kernelType` parameter was ignored.  
...
Support for sigstore `ClusterImagePolicy` and `ImagePolicy` objects is now generally available. The API version is now `config.openshift.io/v1`. 

[id="ocp-release-notes-openshift-cli_{context}"]
=== Support for sigstore bring your own PKI (BYOPKI) image validation

You can now use sigstore `ClusterImagePolicy` and `ImagePolicy` objects to generate BYOPKI config to the `policy.json` file, enabling you to verify image signatures with link:https://docs.sigstore.dev/cosign/verifying/verify/cosign[BYOPKI].

[id="ocp-release-notes-machine-config-operator-namespace_{context}"]
==== Linux user namespace support is now generally available

Support for deploying pods and containers into Linux user namespaces is now generally available and enabled by default. Running pods and containers in individual user namespaces can mitigate several vulnerabilities that a compromised container can pose to other pods and the node itself. This change also include two new security context contstraints that are specifically designed for use with user namespaces: `restricted-v3` and `nested-container`. You can also configure the `/proc` file system in pods as `unmasked`. For more information, see _Running pods in Linux user namespaces_.

[id="ocp-release-notes-machine-config-operator-in-place_{context}"]
==== In-place pod resizing

In-place pod resizing allows you to change the CPU and memory resources for containers within a running pod without application disruption. For more information, see _Manually adjust pod resource levels_.
