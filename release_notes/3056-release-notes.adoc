...
= Red Hat Advanced Cluster Security for Kubernetes version 3.0.56
...
[role="_abstract"]
{product-title} version 3.0.56 includes feature enhancements, bug fixes, scale improvements, and other changes.
...
* Red Hat plans to deprecate the `--json` option for the `roxctl image check` command in {product-title} version 3.0.59; use the `--json-fail-on-policy-violations` option instead.
