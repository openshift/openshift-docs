...
[discrete]
[id="ocp-4-11-stalld-service-now-delivered-with-OS"]
==== The stalld service is now delivered with the operating system

The `stalld` service now ships with the operating system instead of the Node Tuning Operator (NTO). With this change, any {product-title} 4.10 cluster that uses the `stalld` service will incur an additional reboot during the 4.10 to 4.11 upgrade. This is one-time issue, and will not occur in any future cluster upgrades.
