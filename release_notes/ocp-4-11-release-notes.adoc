...
This release updates the minimum system requirements for installing OpenShift on a single node. Users should configure a minimum of 16 GB of RAM. Specific workload requirements may require additional RAM. The complete list of supported platforms has been updated to include bare metal, vSphere, Red Hat OpenStack, and Red Hat Virtualization platforms. In all cases, the user must specify the `platform.none: {}` parameter in the `install-config.yaml` configuration file.
