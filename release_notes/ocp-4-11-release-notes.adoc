...
* A three-node cluster bootstrap fails due to missing `/etc/resolve.conf` file. The workaround is to restart the `NetworkManager` which creates the `resolve.conf` file and the installation can continue. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGS-70[*OCPBUGS-70*])

* After deploying a {sno} with the RAN DU profile, the `openvswitch: cpu_id mismatch with handler threads` Open vSwitch kernel logs are continuously generating. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-46165[*OCPBUGSM-46165*])

* If secure boot is currently disabled and you attempt to enable it through ZTP, the installation will not begin the first time ZTP is started. When secure boot is enabled through ZTP, the boot options are configured before virtual CD is attached. Therefore, the first boot is booting into the existing HD with secure boot on. The installation then gets stuck because booting from CD is never triggered. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-45085[*OCPBUGSM-45085*])

* If an invalid subscription channel is specified in the subscription policy used by the update, the {cgu-operator-full} indicates a successful update right after the policy is enforced because the state does not change (`AtLatestKnown`). There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-43618[*OCPBUGSM-43618*])

* When installing {product-title} 4.10 in a hybrid cluster with two bare metal worker nodes configured for dual-stack with `dnsmasq` server for static IPv6 address allocation, both worker nodes receive dual-stack IP addresses but only one worker includes the IPv6 address in the `InternalIP` list. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-43122[*OCPBUGSM-43122*])

* If SRIOV-FEC policies are added separately, the {cgu-operator-full} adds the policies to the top of the policy list. The original content in the `source-crs` overrides the contents updated by the enforce policy, which causes the original inform policy to never become `Compliant`. The workaround is to add the `CatalogSource` change in the `common-config-policy`.(link:https://issues.redhat.com/browse/OCPBUGSM-39859[*OCPBUGSM-39859*])

* The `SiteConfig` disk partition definition fails when applied to multiple nodes in a cluster. When `SiteConfig` is used to provision a compact cluster, creating a valid `diskPartition` config on multiple nodes fails with a Kustomize plugin error. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-44403[*OCPBUGSM-44403*])

* Images for `ztp-site-generate` are assigned an incorrect ArgoCD patch. When you patch the ArgoCD `Resource`, the patch points to a different `ztp-site-generate` image. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-44261[*OCPBUGSM-44261*])

* Installing {product-title} 4.10.13 on HPE nodes is unsuccessful. Redfish hardware cannot be inspected due to a Redfish set boot device failure. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-44151[*OCPBUGSM-44151*])

* Applying `BMCEventSubscription` fails to create a Redfish subscription. After the subscription YAML file is created and applied, no active Redfish subscription is visible. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-43707[*OCPBUGSM-43707*])

* Using the ZTP process, configuration sets up two `CatalogSources` on a {sno} in a restricted environment for operators. One of the `CatalogSources` is deleted after multiple reboots. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-46245[*OCPBUGSM-46245*])

* A number of clusters fail to update during scale testing. After starting {product-title} 4.9.26 update with Telecom vDU configuration applied, a cluster update to 4.10.13 initiated with the `ClusterVersion` CR fails to complete. There is currently no workaround for this issue. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-44655[*OCPBUGSM-44655*])

* Spoke {sno} deployment on real bare metal (Dell PowerEdge R640) is blocked when `virtualmedia` does not disconnect the ISO after writing the image to the disk. The workaround is to manually disconnect the ISO through the virtual media tab in the `idrac` management UI. (link:https://issues.redhat.com/browse/OCPBUGSM-45884[*OCPBUGSM-45884*])

* Spoke bare metal host deployment using the converged workflow is blocked at inspection. An {product-title} 4.11 hub using the upstream assisted-service gets stuck in `inspecting` state, and eventually fails with an `Introspection` timeout. There is currently no workaround for this issue.(link:https://issues.redhat.com/browse/OCPBUGSM-45428[*OCPBUGSM-45428*])

* On a {op-system-base-full} 9 system that is booted with workload partitioning to isolate CPUs, when you run a container with `podman`, the CPU affinity masks for all the `usr` processes on the system get overwritten to use every CPU on the system, including isolated CPUs. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/RHELPLAN-126093[*RHELPLAN-126093*])

* On a dual-stack environment, devices and connections get stuck on `ip-check` state with a default `dhcp6` profile generated with the `nm-initrd-generator`. The workaround is to restart the `NetworkManager` which creates the `resolve.conf` file and the installation can continue. (link:https://issues.redhat.com/browse/RHELPLAN-127788[*RHELPLAN-127788*], link:https://issues.redhat.com/browse/OCPBUGS-70[*OCPBUGS-70*])

* On NVIDIA-branded NICs, incoming packets that are larger than the pre-set F5 application receive buffer (currently set to 8K) arrive with the incorrect VLAN tag. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/RHELPLAN-123058[*RHELPLAN-123058*])

* An Operator Lifecycle Manager registry-server container fails to reach the `READY` state. When creating subscriptions with a new `CatalogSource`, the `CatalogSource` remains in a `TRANSIENT_FAILURE` state. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-44041[*OCPBUGSM-44041*])

* A {sno} node configured with static IPv6 and deployed using ZTP becomes unreachable during Day 2 operator configuration. The {product-title} cluster installation completes successfully, and the cluster is healthy. After reboot, the node becomes unreachable with the interface down. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-46688[*OCPBUGSM-46688*])

* During ZTP deployment, the bare metal host image provisioning failed with the following error: `The ETag supplied did not match the ETag required to change this resource`. The workaround is to manually attach an ISO image using the Baseboard Management Controller GUI. (link:https://issues.redhat.com/browse/OCPBUGSM-46305[*OCPBUGSM-46305*])

* During a master node replacement on bare metal setup, the machine is stuck in `Provisioning` state, but the node is reported as `Ready`. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-45772[*OCPBUGSM-45772*])

* In runtime, some vDU pods use the reserved cores for the platform, and {product-title} `etcd` uses the isolated application cores. There is currently no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-45078[*OCPBUGSM-45078*])
