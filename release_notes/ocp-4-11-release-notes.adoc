...
== Known issues
//Placeholder for TELCODOCS-751 PR
* If you change the operator scope from namespace to cluster, you cannot migrate the operator to a different version. To upgrade, you must remove the previous version of the operator, without deleting the namespace nor the instance of the MetalLB CR. Deploy the new operator to keep MetalLB running with the current configuration.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2100180[*BZ#2100180*])
