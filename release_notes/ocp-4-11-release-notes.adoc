...
* During ZTP cluster deployments on certain Lenovo models, for example the SE450, bare-metal host image provisioning might fail with HTTP code 400. The error might reference the following URI: `/redfish/v1/Systems/1/Pending` and might include a message indicating that the request attempted to change a read only property. Currently, there is no workaround for this issue. (link:https://issues.redhat.com/browse/OCPBUGSM-46305[*OCPBUGSM-46305*])
