...
[id="ocp-4-11-enhancement-scale-up-operations-etcd-clusters"]
==== Enhancement to scale-up operations for etcd clusters
Raft algorithm allows for scaling of etcd members using a new learner state. As a result, maintaining quorum, adding and removing new members, and promoting learners occur without disrupting cluster operation.
