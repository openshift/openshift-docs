...
* The log collector pod has higher CPU usage than the same pod in previous versions when you deploy a {sno} with the vRAN DU profile. (link:https://issues.redhat.com/browse/OCPBUGSM-46986[*OCPBUGSM-46986*])

* When you apply a tuned override to a pod and delete the tuned pod to force a restart, the pod should restart and the system should run normally. Instead a `systemd` hang occurs and causes the system to stop responding. (link:https://issues.redhat.com/browse/RHELPLAN-131021[*RHELPLAN-131021*])
