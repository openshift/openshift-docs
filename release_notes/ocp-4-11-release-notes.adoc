...
* During ZTP cluster deployments on certain Lenovo models, for example the SE450, bare-metal host image provisioning might fail with HTTP error code. The error might reference the following URI: `/redfish/v1/Systems/1/Pending` and might include a message indicating that the request attempted to change a read only property. For example, the error message can be the folowing:
+
[source,terminal]
----
