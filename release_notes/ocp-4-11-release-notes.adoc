...
* The `dnsmasq` service causes high CPU usage during 4.11 deployment or after a 4.10 to 4.11 upgrade on an SNO node. As a workaround, you can SSH into the node and restart the `dnsmasq` service by running the following command:
+
[source,terminal]
----
...
* When booting ZT system machines from a live ISO with static IPv6 address configuration, `NetworkManager` exits successfully before the interface link becomes ready, which leaves the network interface without configuration. As a workaround, edit the RHCOS ISO referenced in the `AgentServiceConfig` CR to append `rd.net.timeout.carrier` to the kernel parameters in the `grub.cfg` file:
...
* Installing {product-title} 4.10.13 on Hewlett Packard Enterprise (HPE) nodes fails. Redfish hardware cannot be inspected due to a Redfish set boot device failure. As a workaround, update the firmware to version 2.63 or later. (link:https://issues.redhat.com/browse/OCPBUGSM-44151[*OCPBUGSM-44151*])
...
//[id="ocp-4-11-ran-fixed-issues"]
//== {product-title} 4.11 RAN fixed issues
