...
* A number of clusters fail to update during scale testing. After starting {product-title} 4.9.26 update with the Telecom vDU configuration applied, a cluster update to 4.10.13 initiated with the `ClusterVersion` CR fails at 15% and waits for cluster Operators updating to the target version. (link:https://issues.redhat.com/browse/OCPBUGSM-44655[*OCPBUGSM-44655*])
...
* After removing the `SriovNetworkNodePolicy` policy from the Git repository, the `SriovNetworkNodePolicy` CR remains on the spoke cluster. (link:https://issues.redhat.com/browse/OCPBUGSM-34614[*OCPBUGSM-34614*])
