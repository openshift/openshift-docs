...
* When booting ZT Systems machines from a live ISO with static IPv6 address configuration, `NetworkManager` exits successfully before the interface link becomes ready, which leaves the network interface without configuration. As a workaround, edit the RHCOS ISO referenced in the `AgentServiceConfig` CR to append `rd.net.timeout.carrier` to the kernel parameters in the `grub.cfg` file:
...
* When you update configuration files that have the `musthave` compliance type, a new entry is created in the CR instead of overwriting the existing file. As a workaround, you can set the `complianceType` to `mustonlyhave` in the `PolicyGenTemplate` CR:
+
[source,yaml]
----
...
----
+
(link:https://issues.redhat.com/browse/OCPBUGSM-41001[*OCPBUGSM-41001*])
