...
* After deploying a {sno} with the RAN DU profile, an error `openvswitch: cpu_id mismatch with handler threads` is continuously generating in the Open vSwitch kernel logs. (link:https://issues.redhat.com/browse/OCPBUGSM-46165[*OCPBUGSM-46165*])

* If secure boot is currently disabled and you attempt to enable it through ZTP, the cluster installation does not start. When secure boot is enabled through ZTP, the boot options are configured before the virtual CD is attached. Therefore, the first boot is from the existing hard disk with secure boot turned on. The cluster installation gets stuck because the system never boots from the CD. (link:https://issues.redhat.com/browse/OCPBUGSM-45085[*OCPBUGSM-45085*])

* If an invalid subscription channel is specified in the subscription policy that is used to perform a cluster upgrade, the {cgu-operator-full} indicates a successful upgrade right after the policy is enforced because the state does not change (`AtLatestKnown`). (link:https://issues.redhat.com/browse/OCPBUGSM-43618[*OCPBUGSM-43618*])

* If SRIOV-FEC policies are added separately, the {cgu-operator} adds the policies to the top of the policy list. The original content in the `/source-crs` folder overrides the contents updated by the enforce policy, which causes the original inform policy to never become `Compliant`. The workaround is to add the `CatalogSource` change in the `common-config-policy`. (link:https://issues.redhat.com/browse/OCPBUGSM-39859[*OCPBUGSM-39859*])
...
* When you pull the `ztp-site-generate:v4.10.0-53` GitOps ZTP container and attempt to patch the ArgoCD resource, the patch points to a different ztp-site-generate image (`registry.redhat.io/openshift4/ztp-site-generate-rhel8:v4.10.0`). (link:https://issues.redhat.com/browse/OCPBUGSM-44261[*OCPBUGSM-44261*])

* Installing {product-title} 4.10.13 on Hewlett Packard Enterprise (HPE) nodes fails. Redfish hardware cannot be inspected due to a Redfish set boot device failure. (link:https://issues.redhat.com/browse/OCPBUGSM-44151[*OCPBUGSM-44151*])

*  Applying a `BMCEventSubscription` CR fails to create a Redfish events subscription for ZT systems servers. After the subscription YAML file is created and applied, no active Redfish subscription is visible. To workaround this issue, run the following command substituting appropriate BMC parameters to call the API directly and create the subscription:
+
[source,terminal]
----
...
* Using the GitOps ZTP pipeline to install a {sno} cluter in a disconnected environment, there should be two `CatalogSource` CRs applied in the cluster. One of the `CatalogSource` CRs gets deleted following multiple node reboots. As a workaround, you can change the default names, such as `certified-operators` and `redhat-operators`, of the catalog sources. (link:https://issues.redhat.com/browse/OCPBUGSM-46245[*OCPBUGSM-46245*])

* A number of clusters fail to update during scale testing. After starting {product-title} 4.9.26 update with the Telecom vDU configuration applied, a cluster update to 4.10.13 initiated with the `ClusterVersion` CR fails to complete. (link:https://issues.redhat.com/browse/OCPBUGSM-44655[*OCPBUGSM-44655*])

* Using {rh-rhacm-first}, spoke cluster deployments on Dell PowerEdge R640 servers are blocked when the virtual media does not disconnect the ISO in the iDRAC console after writing the image to the disk. To workaround this issue, disconnect the ISO manually through the Virtual Media tab in the iDRAC console. (link:https://issues.redhat.com/browse/OCPBUGSM-45884[*OCPBUGSM-45884*])

* Spoke cluster deployment on bare-metal hosts using the converged workflow is blocked at inspection. An {rh-rhacm} 4.11 hub cluster using the `assisted-service` API gets stuck in an `inspecting` state, and eventually fails with an `Introspection` timeout. (link:https://issues.redhat.com/browse/OCPBUGSM-45428[*OCPBUGSM-45428*])

* On a dual-stack networking environment, devices and connections get stuck in an `ip-check` state with a default `dhcp6` profile generated by the `nm-initrd-generator` utility. The `/etc/resolve.conf` file is not generated. To workaround this issue, restart the `NetworkManager` service. This generates the missing `/etc/resolve.conf` file and the installation can continue. (link:https://issues.redhat.com/browse/RHELPLAN-127788[*RHELPLAN-127788*], link:https://issues.redhat.com/browse/OCPBUGS-70[*OCPBUGS-70*])

* Using NVIDIA-branded Mellanox NICs on Dell hardware, incoming packets that are larger than the preset F5 application receive buffer (currently set to 8K) arrive with an incorrect VLAN tag. This results in sporadic truncated packets being delivered by the NIC. (link:https://issues.redhat.com/browse/RHELPLAN-123058[*RHELPLAN-123058*])
...
* Sometimes, a {sno} node configured with static IPv6 and deployed using the GitOps ZTP pipeline becomes unreachable during Day 2 operator configuration. The {product-title} cluster installation completes successfully, and the cluster is healthy. After reboot, the node becomes unreachable with the network interface down. (link:https://issues.redhat.com/browse/OCPBUGSM-46688[*OCPBUGSM-46688*])

* During ZTP cluster deployments, the bare-metal host image provisioning fails with the following error: `The ETag supplied did not match the ETag required to change this resource`. To workaround this issue, attach the ISO image to the host using the Baseboard Management Controller GUI. (link:https://issues.redhat.com/browse/OCPBUGSM-46305[*OCPBUGSM-46305*])

* During a master node replacement in a bare-metal cluster, the new master host gets stuck in a `Provisioning` state, but the node reports in to the cluster as `Ready`. (link:https://issues.redhat.com/browse/OCPBUGSM-45772[*OCPBUGSM-45772*])

* At runtime, when host CPU cores are reserved separately for the cluster platform and deployed applications, some vDU pods use the reserved platform cores, and the {product-title} `etcd` datastore service uses isolated application cores. (link:https://issues.redhat.com/browse/OCPBUGSM-45078[*OCPBUGSM-45078*])
...
* The `dnsmasq` service causes high CPU usage after a 4.10.21 to 4.11.0-rc.1 upgrade on an SNO node. As a workaround, you can SSH into the node and restart the `dnsmasq` service by running the following command:
+
[source,terminal]
----
...
----
+
(link:https://issues.redhat.com/browse/OCPBUGSM-46705[*OCPBUGSM-46705*])

* When booting from a live ISO with static IPv6 address configuration, `NetworkManager` exits successfully before the interface link becomes ready, which leaves the network interface without configuration. As a workaround, edit the RHCOS ISO referenced in the `AgentServiceConfig` CR to append `rd.net.timeout.carrier` to the kernel parameters in the `grub.cfg` file:

. Pull the `rhsoc-live` ISO image for the release being installed. The URL can be retrieved from the `AgentServiceConfig` CR on the hub cluster by running the following command:
+
[source,terminal]
----
...
.. Append the `rd.net.timeout.carrier=20` string to the file.
...
* A kernel crash occurs after rebooting a bare-metal SNO with DU profile and workload test application. As a workaround, you can add an additional kernel parameter to the performance profile:
+
[source,yaml]
----
