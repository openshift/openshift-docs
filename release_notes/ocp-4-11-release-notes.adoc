...
//Update with the relevant information closer to release.
//{product-title} (link:https://access.redhat.com/errata/RHSA-2022:0056[RHSA-2022:0056]) is now available. This release uses link:https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md[Kubernetes 1.23] with CRI-O runtime. New features, changes, and known issues that pertain to {product-title} {product-version} are included in this topic.Red Hat did not publicly release {product-title} 4.10.0 as the GA version and, instead, is releasing {product-title} 4.10.3 as the GA version.
...
With this enhancement, if a machine config pool (MCP) is paused when when the CA certificate rotation happens, you now receive alerts in the Alerting UI of the {product-title} web console to warn you of the condition.  If an MCP is paused when the certificates are rotated, the MCO cannot push the newly rotated certificates to those nodes. This causes the cluster to become degraded and causes failure in multiple `oc` commands. For more information on pausing MCPs, see xref:../support/troubleshooting/troubleshooting-operator-issues.adoc#troubleshooting-disabling-autoreboot-mco_troubleshooting-operator-issues[Disabling the Machine Config Operator from automatically rebooting].

[id="ocp-4-11-mco-update-zone-age"]
==== Update nodes by zone and age

With this enhancement, after a machine config change, the Machine Config Operator (MCO) updates the affected nodes alphabetically by zone, based on the `topology.kubernetes.io/zone` label. If a zone has more than one node, the oldest nodes are updated first. For more information, see xref:../post_installation_configuration/machine-configuration-tasks.adoc#machine-config-overview-post-install-machine-configuration-tasks[Machine config overview].
