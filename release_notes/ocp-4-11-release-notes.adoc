...
* xref:../authentication/understanding-and-managing-pod-security-admission.html#understanding-and-managing-pod-security-admission[Understanding and managing pod security admission]

* xref:../operators/understanding/olm-packaging-format.adoc#olm-file-based-catalogs_olm-packaging-format[Operator Framework packaging formats]
...
* Previously, when you tried to destroy multiple clusters in parallel, the destroy process failed because of a bug in the `vmware/govmomi` library. The bug caused one of the cluster's tags to be deleted, which resulted in a `404` error when the destroy process could not find the tag. This update ignores tags that are not found and continues the destroy process so that it finishes without error. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2021041[*BZ#2021041*])
...
* Previously, a pod disruption budget was used to create a OVN raft quorum on {SNO}. This raised an unhelpful `PodDisruptionBudgetAtLimit` alert on {SNO} clusters. With this update, the `PodDisruptionBudgetAtLimit` alert is no longer raised on these clusters. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2037721[*BZ#2037721*])
