...
* In {product-title} {product-version}, the `restricted` pod security admission profile is enabled for all namespaces by default. As a result, an error dispalys if you install a catalog source that was built with an `opm` version earlier than 1.23.0.
+
.Example error
[source,terminal]
----
...
----
+
Workaround: Cluster administrators can allow the creation of catalog sources built with earlier `opm` versions by setting the pod security admission labels in the namespace metadata files. Apply the `privileged` label to the `warn` and `audit` modes, as shown in the following example:
+
.Example `manifests/<target_namespace>.yaml` file
+
[source,yaml]
----
apiVersion: v1
...
...
  labels:
    ...
    pod-security.kubernetes.io/audit: privileged <1>
    pod-security.kubernetes.io/warn: privileged <2>
  name: "openshift-marketplace"
----
<1> Set the `audit` mode label to `privileged`.
<2> Set the `warn` mode label to `privileged`.
+
For more information, see Pod security admission release notes and Understanding pod security admission. (xrefs will be added when the other PRs merge.) (link:https://bugzilla.redhat.com/show_bug.cgi?id=2100323[*BZ#2100323*])
