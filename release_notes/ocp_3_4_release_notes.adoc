...
xref:ocp-3-4-1-10-bug-fixes[Bug Fixes]). The script is installed on all master
hosts that use the RPM installation method.

[NOTE]
====
If all of your masters are using the containerized installation method, see the
following Knowledgebase Solution which has the script attached, and download it
to a system where you can run `oc` commands:

link:https://access.redhat.com/[Knowledgebase Solution]
====
...
will attempt to determine it), and the `-s` and `-c <cacert>` options if the
registry is secured.
