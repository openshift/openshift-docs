...
When installing RPM packages, {product-title} now accepts both three-part and two-part version schemes. Packages that use either scheme can be installed. See link:https://bugzilla.redhat.com/show_bug.cgi?id=1826213[*BZ#1826213*] for more information.
...
{product-title} 4.5 enables transparent end-to-end HTTP/2 support in HAProxy for
application owners to make use of its protocol capabilities, including single
connection, header compression, binary streams, and more.

To enable HTTP/2 for the connection from the client to HAProxy, a route must
specify a custom certificate. A route that uses the default certificate cannot
use HTTP/2. This restriction is necessary to avoid problems from connection
coalescing, like the client re-using a connection for different routes that use
...
with the backend. The implication is that end-to-end HTTP/2 is possible with
...
* Previously, the VM wizard did not generate the cloud-init configuration in correct format, and as a result it was not applied on the VM. With this release, the format generated by the wizard has been corrected and the cloud-init configuration tha is provided in the VM wizard is applyed on the VM. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1821024[*BZ#1821024*])
...
* When deleting a non-empty image registry with GCP storage, the image regustry hostname was not being removed from the image configuration file. This prevented you from creating a new image registry. The code has been changed to remove the image registry hostname from the image configuration file when you delete an image registry. As a result, you can delete and create image registries as epected.
...
* Because the default Image Registry Operator was configrued with 0 replicas, problems could result unless the value is manually changed. The operator was updated to install with 1. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1811846[*BZ#1811846*]) 
...
* Because a {rh-openstack} user-provisioned installation script was not compatible with some Ansible versions, installations could fail. The script was updated to assure broad compability. Now, installations succeed regardless of the your Ansible version. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1810916[*BZ#1810916*])
...
* The Ingress Virtual IP management configuration was using a fixed string for its password. If two VRRP keepalived instances in separate clusters had the same Virtual Router ID, they would have the same password and potentially join the clusters having the Virtual IP fall where it should not. This bug fix makes the password change depending on cluster configuration. As a result, different cluster Ingress Virtual IPs now have a different password. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1803232[*BZ#1803232*])
...
* The installed Operators list was only sortable by the `Name` column, limiting sorting options for users. Now, users can sort the list by more than just the name columnn. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1797931[*BZ#1797931*])
...
* Not blocking having several listeners, on different protocols, on the same port for `ovn-octavia` driver. This was not supported previously on the `ovn-octavia` driver, but now it is supported and there is no need to block it. Several listeners on different protocols can be exposed in the same port. This means that it's possible to have, for instance, the DNS service to expose port 53 in both TCP and UDP protocols when using `ovn-octavia`. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1846452[*BZ#1846452*])
