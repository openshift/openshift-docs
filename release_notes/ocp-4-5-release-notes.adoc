...
====  New Capability and Subscription Required filters for Operators in OperatorHub

OLM provides a list of attributes that Operator developers can tag their
Operators with, that describe the Operator's functionality or attributes, such
as `disconnected`, `license required`, `FIPS mode`, and so on. You can now
filter these options by *Capability* and *Subscription Required* in OperatorHub.
