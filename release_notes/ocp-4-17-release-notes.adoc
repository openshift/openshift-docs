...
{product-title} release {product-version}.31 is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2025:8552[RHSA-2025:8552] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2025:8553[RHBA-2025:8553] advisory.
...
* There is currently a known issue where a Technology Preview-enabled cluster has Sigstore verification for payload images in `policy.json`, but the Podman version in the base image does not support Sigstore configuration, so the new node is not available.
+
Workaround: The node will start running when the Podman version in the base image does not support Sigstore. Use the default `policy.json` that does not have Sigstore verification if the base image is 4.11 or earlier. (link:https://issues.redhat.com/browse/OCPBUGS-52313[OCPBUGS-52313])
...
* Previously, there was no communication between a `localnet` pod and a pod in the default network, when both pods are on the same node. With this release, an update fixes this problem when the endpoints were on a different node. (link:https://issues.redhat.com/browse/OCPBUGS-56244[OCPBUGS-56244])
...
// Sec level internal
* Previously, the Cloud Credential Operator degraded a {gcp-first} cluster that was installed with a `mitm` proxy. As a result, you could not create credentials in the {gcp-short} cluster. With this release, the pod configuration for the Operator has been updated to include a trusted CA bundle, so that the Operator no longer fail whenc creating credentials in a {gcp-short} cluster that is configured with a `mitm` proxy. (link:https://issues.redhat.com/browse/OCPBUGS-54828[OCPBUGS-54828])

// OCPBUGS-35944 should not be attached to the Images advisory. Marked up for 4.20 but still persists on the Images advisory. 
