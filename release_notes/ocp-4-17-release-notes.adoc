...
{product-title} no longer supports the following Transport Layer Security (TLS) 1.2 cipher suites that formed part of the custom TLS profile:
...
If you attempt to use any of these cipher suites, you might not get the expected behavior.

If you must use TLS v1.2, use only v1.2 cipher suites from the Intermediate TLS profile type, such as any of the following cipher suites:
...
[source,terminal]
----
$ oc adm release info 4.17.39 --pullspecsgit c
----
