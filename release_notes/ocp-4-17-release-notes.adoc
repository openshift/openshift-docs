...
[id="ocp-4-17-additional-networks-support-for-dummy-cni-plugin_{context}"]
==== Additional networks support for dummy CNI plugin

The Multus `dummy` CNI plugin is now available for use as an additional network. This CNI plugin is used in conjunction with other CNI plugins to provide an internal non-loopback address for applications to use. For more information, see
// xref
