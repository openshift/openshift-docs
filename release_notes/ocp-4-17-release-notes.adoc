...
[id="ocp-4-17-ovnobservability-tp"]
==== New `OVNObservability` view in web console (Technology Preview)
Users can now view network traffic flows using a new feature in the web console. The following types of traffic and resources are viewable: `AdminNetworkPolicy`, `BaselineAdminNetworkPolicy`, `NetworkPolicy`, egress firewall, user-defined network isolation, and multicast access control lists (ACL)s. For more information, see xref:../
...
|`OVNObservability` network traffic sampling in web console
| Not Available
| Not Available
| Technology Preview
