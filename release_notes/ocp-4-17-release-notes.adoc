...
* Previously, there was no communication between a `localnet` pod and a pod in the default network, when both pods were on the same node. With this release, an update fixes this problem when the endpoints were on a different node. (link:https://issues.redhat.com/browse/OCPBUGS-56244[OCPBUGS-56244])
