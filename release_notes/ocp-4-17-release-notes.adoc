...
[id="ocp-4-17-egressip-single-pod-egress-multiple-networks_{context}"]
==== EgressIP support for single pod egressing to multiple networks

This release adds support for pod traffic egress through a specific interface, depending on the traffic destination address. This behavior is backwards compatible with the existing EgressIP behavior. When configured, you can use a different, consistent source IP address for each network interface that you configure. Both shared and local gateway modes are supported.
