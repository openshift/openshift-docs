...
// SME
* Previously, when the OVN-Kubernetes network plugin and theKubernetes-NMState Operator interacted with each other, unexpected connection profiles persisted on disk storage. These connection profiles sometimes caused the `ovs-configuration` service to fail when rebooted. With this release, the unncessary conenction profiles are now cleanred before the `ovs-configuration` starts so that this issue no longer occurs. (link:https://issues.redhat.com/browse/OCPBUGS-52257[*OCPBUGS-52257*])
...
* Previously, adding external labels to alerts on *Alert rules* page of the *Developer* perspective of the web console caused invalid links. This happened because the external labels could not link an alert rule with the alert. With this release, you cannot add external labels to alerts on the *Alert rules* page so that alert links now link to alerts as expected. (link:https://issues.redhat.com/browse/OCPBUGS-51126[*OCPBUGS-51126*])

* Previously, users could not create `NodePools` resources with ARM64 architecture on non-AWS or {azure-short} platforms. This bug resulted in validation errors that prevented the addition of bare-metal compute nodes and caused CEL validation blocks when creating a `NodePool` resource. The fix modifies the `NodePool` spec validation rules to allow ARM64 architecture on non-AWS or {azure-short} or Agent platforms by adding "self.platform.type == 'None'", so users can now create `NodePools` with ARM64 architecture specifications on non-AWS or {azure-short} bare-metal platforms, expanding functionality for end users. (link:https://issues.redhat.com/browse/OCPBUGS-46440[*OCPBUGS-46440*])

// SME
* Previously, for a `kubevirt-csi` pod that ran on a node in a guest cluster, the persistent volume claim (PVC) from the guest cluster was removed from the VM after the VM was rebooted. However, the `VolumeAttachment` resource was not removed and this caused issues for the cluster as it expected the PVC to be attached to the VM. With this release, after a VM is rebooted, the `VolumeAttachment` resource is removed so that the cluster issues no longer occur. (link:https://issues.redhat.com/browse/OCPBUGS-44623[*OCPBUGS-44623*])
