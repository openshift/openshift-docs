...
[id="ocp-4-17-support-for-bgp-routing-protocol_{context}"]
==== Support for the BGP routing protocol

The OVN-Kubernetes network plugin supports the BGP routing protocol, allowing the importation and exportation of routes to the underlying provider network. BGP support offers the following benefits:

- Learn routes to pods dynamically
- Advertise routes dynamically
- Reduce network overhead by eliminating Geneve encapsulation
- Use a pure routing implementation for EgressIPs instead of layer 2 segmentation
