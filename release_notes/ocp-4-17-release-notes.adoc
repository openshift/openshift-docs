...
* Previously, when you ran the `openshift-install agent create pxe-files` command, the command created temporary directories in `/tmp/agent` and the command did not remove these directories upon command completion. With this release, the command now removes the directories when the command nears completion, so no manual deletion of directories is necessary. (link:https://issues.redhat.com/browse/OCPBUGS-52961[*OCPBUGS-52961*])

// SME review needed
* Previously, a code migration operation caused external labels to not be processed correctly on the *Alert detail* page on the *Administrator perspective* of the web console. These external labels are required to prevent silenced alert notifications from getting added to notification bell icon. Because the *Alert detail* page did not handle external labels correctly, the notification bell provided links to these *Alert detail* pages that generated a `no matching alerts found` message when you clicked on a link. With this release, the *Alert detail* page accepts external labels so clicking on an alert in the notification bell links to the correct *Alert detail* page. (link:https://issues.redhat.com/browse/OCPBUGS-51117[*OCPBUGS-51117*])

// Revise
* Previously, the physical function (PF) interface for an Single Root I/O Virtualization (SR-IOV) daemon did not get unbinded during a node reboot operation. Instead, the virtual function (VF) was unbinded because of an InfiniBand (IB) link type, and this caused node reboots to take up to 24 hours to complete. With this release, a VF interface gets a valid Globally Unique Identifier (GUID) so node reboots happen much more quickly. (link:https://issues.redhat.com/browse/OCPBUGS-53254[*OCPBUGS-53254*])
