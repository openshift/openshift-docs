...
[id="known-issues-369"]
== Known issues
* *ROX-9750*: The `FROM` instruction in the `DISALLOWED DOCKERFILE LINE` policy field is not recognized by {product-title-short}. For example, creating a policy that disallows `FROM:unwanted.example.com` in the Dockerfile does not generate a policy violation.
