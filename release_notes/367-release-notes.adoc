...
{product-title} 3.67 is thoroughly tested and supported on OpenShift Dedicated on Amazon Web Services and Google Cloud.
...
Users can now use ACS to define the container runtime configuration that may be used to run a pods containers using the *Runtime Class* policy criteria.
...
* *ROX-7254*: Previously, the *Alpine Linux package manager (APK) in Image* policy looked for the presence of `apk` package in the image rather than the `apk-tools` package. We’ve fixed this issue.
...
[NOTE]
====
{product-title} ignores K8s and Istio vulnerability definitions when you manually upload the vulnerability definitions in online mode.
====
