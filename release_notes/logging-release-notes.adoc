:_mod-docs-content-type: ASSEMBLY
[id="logging-release-notes"]
= Logging 6.5 release notes

include::_attributes/common-attributes.adoc[]
:context: logging-release-notes

toc::[]

[role="_abstract"]
This release of {LoggingProductName} is supported on {ocp-product-title} 4.19 and later.

include::modules/logging-release-notes-6-5-3.adoc[leveloffset=+1]

include::modules/logging-release-notes-6-5-3-fixed-issues.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-3-cves.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-2.adoc[leveloffset=+1]

include::modules/logging-release-notes-6-5-2-deprecated-features.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-2-fixed-issues.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-2-cves.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-1.adoc[leveloffset=+1]

include::modules/logging-release-notes-6-5-1-fixed-issues.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-1-cves.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-0.adoc[leveloffset=+1]

include::modules/logging-release-notes-6-5-0-enhancement.adoc[leveloffset=+2]

include::modules/logging-release-notes-6-5-0-fixed-issues.adoc[leveloffset=+2]
