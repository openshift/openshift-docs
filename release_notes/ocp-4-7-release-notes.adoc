...
[id="ocp-4-7-monitoring-etcd-alerts"]
==== New etcd alerts

New etcd alerts are now available:

* A critical alert when the etcd database quota is 95% full
* A warning alert when there is a sudden surge in etcd writes, leading to an increase in the etcd database quota size
* A critical alert when the 99th percentile of the etcd members fsync duration is greater than 1 second

See link:https://bugzilla.redhat.com/show_bug.cgi?id=1890808[*BZ#1890808*] for more information.
