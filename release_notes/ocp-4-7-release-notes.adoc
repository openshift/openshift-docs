...
==== CIS Kubernetes Benchmark profile for Compliance Operator

You can now use the Compliance Operator to perform Center for Internet Security (CIS) Kubernetes benchmark checks. The CIS profile performs both Platform and Node scans against CIS Kubernetes benchmark standards and can apply remediations. 
