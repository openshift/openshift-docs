...
{product-title} 4.7 provides post-installation state-driven network configuration on the secondary network interfaces of cluster nodes using `kubernetes-nmstate` as a technology preview. For more information, see xref:../virt/k8s_nmstate/virt-k8s-nmstate-using-kubernetes-nmstate.adoc[Using Kubernetes NMState (Technology Preview)].
