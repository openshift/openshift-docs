...
[id="ocp-4-7-machine-api-avoid-reboots"]
==== Changes to registries no longer trigger a reboot

The Machine Config Operator (MCO) watches your cluster for configuration changes to nodes. When it detects these changes, it drains and reboots the nodes. The MCO now no longer reboots nodes after these specific changes:
...
When the MCO detects these changes, it drains the corresponding nodes, applies the changes, and uncordons the nodes, but does not reboot the nodes.
...
For more information, see xref:../nodes/pods/nodes-pods-priority.adoc#non-preempting-priority-class_nodes-pods-priority[Non-preempting priority classes].

[id="ocp-4-7-nodes-using-image-short-names-containerRuntimeSearchRegistries"]
==== Enabling image registries that allow image short names

You can now add registries that allow searching for an image by using an image short name. An image short name enables you to search for images without including the fully qualified domain name in the pull spec. Using image short names with public registries is strongly discouraged. You should use image short names with only internal or private registries.

For more information, see xref:../openshift_images/image-configuration.adoc#images-configuration-shortname_image-configuration[Adding registries that allow image short names].
