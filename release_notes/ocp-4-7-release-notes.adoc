...
* If you are running a cluster on VMware using the `x86_64` architecture configured with the `platform: none` field in the `install-config.yaml` file, an upgrade from {product-title} cluster version 4.6 to version 4.7 might fail. This upgrade failure happens when a cluster uses virtual machines (VMs) configured with virtual hardware version 14 or greater. As a workaround, you can configure VMs with virtual hardware version 13. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1941714[*BZ#1941714*])
