...
* The 9100 port is blocked on all nodes by default. Prometheus cannot scrape the
`node_exporter` service running on the other nodes, which listens on port 9100.
This bug fix modifies the firewall configuration to allow incoming TCP traffic
for the 9000 - 1000 port range. As a result, Prometheus can now scrape the
...
OpenShift Extended Comformance Tests and the Node Vertical Test we run, dnsmasq
was hanging and new pods were not being created. A change to the code increases
the maximim number of open file descriptors so the node can pass the tests.
...
associated end-user connections.  A change has been made so that a value of 0
