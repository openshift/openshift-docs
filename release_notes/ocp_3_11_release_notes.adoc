...
|xref:../install_config/router/f5_router.adoc#install-config-router-f5[F5 router plug-in]
|The original {product-title}-supported integration of the F5 router plug-in is
replaced with the F5 BIG-IP® Controller for {product-title}.
...
See a link:https://www.youtube.com/watch?v=nGM2s4-Qr74[video walkthrough].
...
[[ocp-311-helm-operator]]
==== Helm Operator with Existing Charts

Use existing Helm charts as an Operator without writing code.

* There is no tiller running.
* The Operator calls Helm internal code a library.
* Builds on an existing cluster RBAC.
* The chart is reapplied when an object is modified.
* Built with the Operator SDK.

Read the blog post
link:https://blog.openshift.com/make-a-kubernetes-operator-in-15-minutes-with-helm[Make
a Kubernetes Operator in 15 minutes with Helm] for more information.

[[ocp-311-Go-Operator]]
==== Go Operators

Golang SDK is available for building advanced Operators. Use the full power of
the tools curated by the Kubernetes upstream development community.
Functionality is tailored toward ISVs and development teams, versus ops teams.
The most advanced Operators that exist are using Golang.
...
or scoped to a specific namespace. When installing the broker,  you can set
`broker_kind argument` as `ServiceBroker` (namespace specific) or
...
The Ansible configuration parameter `log_path` is now defined. Users must me in
