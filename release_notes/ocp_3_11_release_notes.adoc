...
The Operator SDK provides tools to get started quickly embedding application
business logic into an Operator. It saves you from doing the work to set up
scaffolding, helps run end-to-end tests of your logic on a local or remote
cluster, and it is used by Couchbase, MongoDB, Redis, and more.
...
. The tenant made a PVC and owns a PV from one of the supported storage solutions.
The administrator must create a new `StorageClass` in the cluster with:
+
----
kind: StorageClass
...
* Receive notifications from pre-packaged alerts, enabling owners to take correctiveactions and start troubleshooting problems.
* View pre-packaged Grafana dashboards for etcd, cluster state, and many other aspectsof cluster health.

See xref:../install_config/prometheus_cluster_monitoring.adoc[Configuring Prometheus Cluster Monitoring] for more information.
