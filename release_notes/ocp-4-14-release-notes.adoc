...
* Previously, after a compute node was removed from the cluster, provisioning of a `PersistentVolume` (PV) resource failed and a message was outputted. The message stated "The object <virtual machine ID> has already been deleted or has not been completely created". With this release, a fix ensures that when a compute node is removed, this removal does not impact provisioning of a PV resource. (link:https://issues.redhat.com/browse/OCPBUGS-51045[*OCPBUGS-51045*])

* Previously, if you tried to create a {hcp} cluster in {aws-first} GovCloud region, the cluster stalled and the installation failed. This issue occurred because {aws-short} clients could not access the {aws-short} GovCloud resources. With this release, a fix to credentials used by Control Plane Operator (CPO) means that these credentials support the {aws-short} GovCloud region. This means that you can now successfully create a {hcp} cluster in {aws-short} GovCloud region. (link:https://issues.redhat.com/browse/OCPBUGS-50700[*OCPBUGS-50700*])

* Previously, requests to the `deploymentconfig/scale` subresource would fail when there was an admission webhook matching the request. With this release, the issue is resolved and requests to the deploymentconfig/scale` subresource will succeed. (link:https://issues.redhat.com/browse/OCPBUGS-50477[*OCPBUGS-50477*]) 
