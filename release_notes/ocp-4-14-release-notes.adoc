...
* On clusters with the SR-IOV Network Operator installed and configured, pods with a secondary interface of SR-IOV VF create a pod fail with the error message: `SRIOV-CNI failed to configure VF "failed t │
│ o set vf 0 vlan configuration - id 0, qos 0 and proto 802.1q: invalid argument"`. To resolve this issue, upgrade the SR-IOV Network Operator before upgrading {product-title}. This will ensure the fix to this issue is included in the SR-IOV Network Operator. (link:https://issues.redhat.com/browse/OCPBUGS-38091[*OCPBUGS-38091*])
