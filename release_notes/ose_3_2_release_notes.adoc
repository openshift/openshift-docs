...
- Registry can now serve images of
link:https://github.com/docker/distribution/blob/master/docs/spec/manifest-v2-2.md[manifest V2 schema 2].
- It can also accept the schema if
xref:../install_config/install/docker_registry.adoc#docker-registry-configuration-reference-middleware[enabled
in configuration or with an environment variable].
...
===== Support for importing images of manifest V2 schema 2
- You can now import images of
link:https://github.com/docker/distribution/blob/master/docs/spec/manifest-v2-2.md[manifest
V2 schema 2] from external registries. All the images pushed by Docker
daemon 1.10 or newer to a registry will be of this schema if the registry
supports it. Previously, an image of such schema was converted to
link:https://github.com/docker/distribution/blob/master/docs/spec/manifest-v2-1.md[schema
1] having a different digest than requested one, which caused build and
deployment failures.

- You can determine the schema version of particular image by running one of
  the following commands:
+
----
$ # image of schema 1 run by user with permission to get cluster images:
$ oc get -o jsonpath='{.image.dockerImageManifestMediaType}' \
    image/sha256:5e5a9e67506d4d41c0be0c2ebfa5007fcdb27a64c955bd1d119526affc70aefb
application/vnd.docker.distribution.manifest.v1+json
$
$ # busybox image of schema 2 tagged in user's image stream under 'latest' tag:
$ oc get -o jsonpath='{.image.dockerImageManifestMediaType}' \
    istag/busybox:latest
application/vnd.docker.distribution.manifest.v2+json
$
$ # hello-openshift image of schema 1 tagged in user's image stream:
$ oc get -o jsonpath='{.image.dockerImageManifestMediaType}' \
     isimage/hello-openshift@sha256:e44d78ac0b70255d8a84a7707c2001d115d4db9354591b4098a718c8ccb693f7
application/vnd.docker.distribution.manifest.v1+json
----
