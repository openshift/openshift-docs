...
* Before this update, when a Bare Metal Host (BMH) was marked as `Provisioned` or `ExternallyProvisioned`, the system would try to deprovision it or power it off first and the `DataImage` attached to the BMH would also prevent deletion. This issue blocked or slowed down host removal, creating operational inefficiencies. With this release, if the BMH has the ‘detached annotation’ and deletion is requested, the BMH transitions to the deleting state, allowing for direct deletion. (link:https://issues.redhat.com/browse/OCPBUGS-59133[OCPBUGS-59133])
