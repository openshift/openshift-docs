...
* Before this update, when a Bare Metal Host (BMH) was marked as `Provisioned` or `ExternallyProvisioned`, the system would try to deprovision it or power it off first. Also, the `DataImage` attached to the BMH would also prevent deletion. This blocked or slowed down host removal, creating operational inefficiencies. With this release, if the BMH has the `detached annotation` label and deletion is requested, the BMH transitions to the deleting state, allowing for direct deletion. (link:https://issues.redhat.com/browse/OCPBUGS-59133[OCPBUGS-59133])
...
* Before this update, the `console.flag/model` extension point did not work in production, preventing flags from being properly set when their associated model was provided. With this release, the `console.flag/model` works as expected in production and properly sets a flag when the associated model is provided. (link:https://issues.redhat.com/browse/OCPBUGS-59513[OCPBUGS-59513])
