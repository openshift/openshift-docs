...
As part of aligning with Image Mode for {op-system-base}, {op-system} is now built as a layer on top of a shared {op-system-base} base image. The most noticeable change for users is around versioning. For example, `VERSION_ID` in `/etc/os-release` now reflects the version of {op-system-base}, such as {op-system-base} 9.6, rather than the version of {product-title}, such as {product-title} {product-version}. This version change might show up in other places, such as in the output of the command `rpm-ostree status`, or in boot loader entries.
