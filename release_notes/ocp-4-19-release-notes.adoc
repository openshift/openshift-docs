...
* Before this update, calls to retrieve an image manifest and metadata by using a tagged image name did not cache the result of the lookup. As a result, memory usage the HyperShift Operator quickly grew, which created performance issues. With this release, images in the HyperShift Operator using a named tag or canonical name are cached for 12 hours optimizing memory usage. (link:https://issues.redhat.com/browse/OCPBUGS-59933[OCPBUGS-59933])
...
* Before this update, duplicate quick start buttons were created due to using the  `nextQuickStart` spec to link to other quick starts. This resulted in confusion for users with duplicate quick start buttons on the quick start page. With this release, there is only one quick start button which improves navigation consistency. (link:https://issues.redhat.com/browse/OCPBUGS-60420[OCPBUGS-60420])
