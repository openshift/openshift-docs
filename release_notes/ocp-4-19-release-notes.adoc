...
* Added the `PrometheusPossibleNarrowSelectors` alert to warn users when {cmo-first} detects that Prometheus query selectors might not take into account that values on the `le` label of classic histograms or the `quantile` label of summaries could also be floats.

[id="ocp-4-19-monitoring-prometheus-version-3"]
==== Prometheus version 3 

TBD

[id="ocp-4-19-monitoring-metrics-collection-profiles-ga"]
==== Metrics collection profiles (General Availability)
