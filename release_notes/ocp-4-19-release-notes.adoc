...
In the 4.19 release, a bug is fixed in which the kubelet incorrectly calculated `readinessProbe` periods. Previously, the kubelet wouldn't account for probes run in the `syncPod` method, which periodically checks the state of a pod and does a readiness probe outside of the normal probe period. With this release, pod readiness latency when configured with readiness probes may increase, but is more accurate to the configured probe. For more information, see (link:https://issues.redhat.com/browse/OCPBUGS-50522[OCPBUGS-50522])
