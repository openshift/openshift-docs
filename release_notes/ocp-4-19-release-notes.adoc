...
// xref will be added after the PR is merged.

[id="ocp-4-19-edge-computing-coordinating-reboots_{context}"]
==== Coordinating reboots for configuration changes

You can now use {cgu-operator-full} (TALM) to coordinate reboots across a fleet of spoke clusters when configuration changes require a reboot, such as deferred tuning changes. {cgu-operator} reboots all nodes in the targeted `MachineConfigPool` object on the selected clusters when the reboot policy is applied.
