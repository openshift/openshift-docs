...
On 30 September 2025, the default outbound access connectivity for all new virtual machines (VMs) in {azure-first} will be retired. To enhance security, {azure-short} is moving towards a secure-by-default model where default outbound access to the internet will be turned off.

As a result, after 30 September 2025 nodes that you deploy in {product-title} using {azure-short} will not provide for outbound access connectivity unless explicitly configured.

Existing VMs and existing and new installer-provisioned infrastructure installations are not affected by the retirement. However, for user-provisioned infrastructure installations on {azure-short} you must assign a public IP address for load balancers.
