...
The OpenShift installer now automatically detects and removes leftover persistent storage volumes on {vmw-full} when you delete a cluster. This prevents orphaned volumes from consuming disk space and creating unnecessary alerts in vCenter.
