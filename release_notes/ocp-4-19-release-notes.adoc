...
[id="ocp-release-notes-scalability-and-performance-nrop-hcp_{context}"]
==== NUMA-Aware Scheduling supported on {hcp} (Technology Preview)

With this release, you can install the NUMA Resources Operator on {hcp}, enabling NUMA-aware scheduling support. This feature is available as a Technology Preview. For more information, see xref:../scalability_and_performance/cnf-numa-aware-scheduling.adoc#cnf-creating-nrop-cr-hosted-control-plane_numa-aware[Creating the NUMAResourcesOperator custom resource for hosted control planes].
