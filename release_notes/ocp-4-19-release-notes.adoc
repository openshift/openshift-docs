...
* Before this update, when a bare-metal host was marked as `Provisioned` or `ExternallyProvisioned`, the system would try to remove it or power it off first. Also, the `DataImage` attached to the bare-metal host would also prevent deletion. This blocked or slowed down host removal, creating operational inefficiencies. With this release, if the bare-metal host has the `detached annotation` label and deletion is requested, the bare-metal host transitions to the deleting state, allowing for direct deletion. (link:https://issues.redhat.com/browse/OCPBUGS-59133[OCPBUGS-59133])
