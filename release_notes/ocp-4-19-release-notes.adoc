...
[id="ocp-release-notes-machine-config-operator-ocl-naming_{context}"]
==== New feature names

Two features have been renamed in order to better represent those features.

* The on-cluster layering feature is now called _Image mode for OpenShift_.
* The updated boot images is now called _boot image management_.

[id="ocp-release-notes-machine-config-operator-ocl-ga_{context}"]
==== Image mode for OpenShift is now generally available

The image mode for OpenShift feature, formerly called the on-cluster layering feature, is promoted to General Availability.

With the promotion to GA, note the following changes:
...
==== Boot image management now default on GCP and AWS clusters

The boot image management feature, previously called updated boot images, is now active by default on GCP and AWS clusters. When enabled, the Machine Config Operator magages and updates the boot images associate with the machine sets in your cluster. The MCO uses these boot images when creating new nodes. You can opt-out of the feature, as necessary. For more information, see xref:../machine_configuration/mco-update-boot-images.adoc#mco-update-boot-images[Boot image management].
