...
* Previously, the Samples Operator established a watch for all cluster Operators, which caused its sync loop to run when any Operator changed. With this release, the Samples Operator watches only the cluster Operators it needs to monitor. (link:https://issues.redhat.com/browse/OCPBUGS-54589[OCPBUGS-54589])
