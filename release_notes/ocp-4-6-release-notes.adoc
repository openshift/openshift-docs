...
* You can install {product-title} on {rh-virtualization} with user-provisioned infrastructure, such as an external load balancer. This process uses a series of Ansible playbooks to enable a more flexible installation.
...
* Due to a current limitation of the Cloud-native Network Functions (CNF) tests image, the image parameters must be specified as follows:
+
[source,bash]
----
-e CNF_TESTS_IMAGE=cnf-tests-rhel8:4.6 -e DPDK_TESTS_IMAGE=dpdk-base-rhel8:4.6
----
+
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1892294[*BZ#1892294*])
