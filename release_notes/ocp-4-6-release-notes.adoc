...
* When powering on a virtual machine on vSphere with user-provisioned infrastructure, the process of scaling up a node might not work as expected with the error:
+
[source,terminal]
----
Invalid memory setting: memory reservation (sched.mem.min) should be equal to memsize(xxxx) <1>
----
<1> `xxxx` is the memory size in MB.
+
Machines are created within the hypervisor but are not powered on. Manual intervention is necessary to set the memory reservation amount to its configured memory size. For more information, see the Red Hat Knowledgebase solution link:https://access.redhat.com/solutions/5785341[UPI vSphere node scale up doesn't work as expected]. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1918383[*BZ#1918383*])

* In {product-title} 4.6.8, a bug fix was introduced that changed how certificates were regenerated by the Cluster Logging Operator (CLO). This fix causes an issue where the CLO could possibly regenerate certificates while the Elasticsearch Operator (EO) is trying to restart the cluster. This results in communication issues between the EO and the cluster, causing EO nodes to have mismatched certificates.
+
