...
[id="ocp-4-6-installation-osp-bare-metal-machines"]
==== Bare metal machines for clusters on {rh-openstack}

Clusters on {rh-openstack-first} can now run compute and control plane machines on bare metal. 
...
You can now select {rh-openstack} Compute (Nova) availability zones while installing a cluster on {rh-openstack}.
...
// [id="ocp-4-6-installation-kuryr-ipv6"]
// ==== Clusters that use Kuryr on {rh-openstack} now support IPv6
