...
//For more information, see xref:../installing/installing_bare_metal_ipi/ipi-install-overview.adoc[Installing a cluster on bare metal]
...
* Previously, the `NET_RAW` capability was granted to pods by CRI-O. This allowed pods to craft custom network packets, which included changing any header field like low ports, source IP, and source MAC. With these privileges, ARP spoofing, DNS hijacking, and DoS attacks were possible. This capability has
been removed for pods by default. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1874671[*BZ#1874671*])
