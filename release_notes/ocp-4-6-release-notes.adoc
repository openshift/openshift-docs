...
Because of the changes to the Ignition specification, if you want to add more nodes to your {product-title} cluster by using the latest version of {op-system} boot media, you might need to manually modify your Ignition configuration as part of the process.
//forthcoming KCS links
...
For more information, see xref:../installing/installing_aws/installing-aws-government-region.adoc#installing-aws-government-region
...
For more information, see xref:../installing/installing_azure/installing-azure-government-region.adoc#installing-azure-government-region
...
For more information, see xref:../installing/installing_azure/installing-azure-private.adoc#installation-azure-user-defined-routing_installing-azure-private
...
==== Minimum disk side for control plane nodes has increased for Azure installations
...
[id="ocp-4-6-web-console-insights-plugin"]
==== Insights plug-in

The Insights plug-in is now integrated into the {product-title} web console. Insights provides cluster health data, such as the number of total issues and total risks of the issues. Risks are labeled as *Critical*, *Important*, *Moderate*, or *Low* and you can quickly navigate to the OpenShift Cluster Manager to learn more about the issues and associated risks.
...
[id="ocp-4-6-noe-maintenance-operator-validation"]
...
for removal in {product-title} 4.9.
...
* The Cluster Version Operator used to load trusted CAs from the ConfigMap referenced by the Proxy configuration's `trustedCA` property. The referenced ConfigMag is user-maintained, and a user setting would interrupt the Operator's access to the Proxy. Now, the Operator loads the trustedCAs from the `openshift-config-managed/trusted-ca-bundle`, which the Network Operator populated when the Proxy configuration's referenced trustedCA ConfigMap is valid. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1897123[*BZ#1897123*])
...
* Previously, when deleting network policies on a cluster that is using the OVN-Kubernetes network provider, a race condition prevented network policies from being reliably deleted when deleting the associated namespace. Now network policy objects are always correctly deleted. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1859682[*BZ#1859682*])
...
* The GPU Operator and Node Feature Discovery (NFD) Operator are not available on a fresh installation of {product-title} 4.6.1. To use NFD in version 4.6.1, you must install {product-title} 4.5 and upgrade the cluster to version 4.6.1.
+
Another workaround on a fresh installation of {product-title} 4.6.1 is to install the NFD Operator from its associated GitHub repository using the CLI.
+
. Clone the NFD Operator GitHub repository:
+
[source,terminal]
----
$ git clone --branch release-4.5 https://github.com/openshift/cluster-nfd-operator.git
----
+
. Navigate into the new NFD Operator repository folder:
+
[source,terminal]
----
...
----
+
. Set the pull policy for the repository to always deploy the NFD Operator, so it is deployed to your running cluster:
+
[source,terminal]
----
...
----
+
The Operators will be available for fresh cluster installations in a future 4.6.z release. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1890673[*BZ#1890673*])
...
* Currently, when scaling to greater than 75 nodes in a cluster, the OVN-Kubernetes pod network provider database might become corrupt, leaving the cluster in an unusable state. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1887585[*BZ#1887585*])
...
* Currently, when scaling up a worker node that is running on {op-system-base-full} 7.8, the OVN-Kubernetes pod network provider fails to initialize on the new node. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1884323[*BZ#1884323*])
...
* When third party Helm chart repositories are added to a cluster, the charts from these repositories are not available in the *Developer Catalog*. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1872369[*BZ#1872369*])

* The availability of Kiali features in the *Developer* perspective depends on the availability of the Elastic Search Operator. Currently, this Operator is not available on a fresh installation of {product-title} {product-version}, hence the Kiali features do not work in the *Developer* perspective. However, these features are available when you upgrade {product-title} from a previous version to the latest. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1889374[*BZ#1889374*])

* The availability of OpenShift Pipelines features in the *Developer* perspective are dependent on the availability of the OpenShift Pipelines Operator. Currently, this Operator is not available on a fresh installation of {product-title} {product-version}, hence the OpenShift Pipelines features do not work in the *Developer* perspective. However, these features are available when you upgrade {product-title} from a previous version to the latest. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1889380[*BZ#1889380*])
...
link:https://access.redhat.com/errata/RHBA-2020:4196[RHBA-2020:4196] advisory.
The RPM packages included in the update are provided by the
link:https://access.redhat.com/errata/RHBA-2020:4197[RHBA-2020:4197] advisory.
