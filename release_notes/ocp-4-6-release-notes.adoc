...
You can now override the default Dynamic Host Configuration Protocol (DHCP)
networking by setting the relevant `guestinfo` properties before booting a VM
from an OVA in vSphere. This lowers the barrier for automatic {op-system-first}
deployment in environments with the following:

* No configured DHCP.
* An Ignition config that requires networking.

This enhancement allows for higher-level automation to provision an {op-system}
OVA in environments with static networking.
