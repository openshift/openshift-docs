...
==== Change the image pruner's `logLevel`

Cluster administrators can now configure `logLevel` in the Pruning Custom Resource, so you can debug logs.
...
You can now set up and configure the Image Registry for Azure Government.
...
==== Change the Image Registry Operator's `logLevel`

Cluster administrators can now configure `logLevel` in the Image Registry Operator, so you can debug logs.
...
* The availability of CodeReady Workspace features in the *Developer* perspective are dependent on the availability of the CodeReady Workspace Operator. Currently, this Operator is not available on a fresh installation of {product-title} {product-version}, hence the CodeReady Workspace features do not work in the *Developer* perspective. However, these features are available when you upgrade {product-title} from a previous version to the latest. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1889382[*BZ#1889382*])
