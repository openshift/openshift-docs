...
* *2x* speed improvement in hardware restart.
...
|In older versions, the command was `roxctl generate netpol`.
...
For more details, see xref:../operating/manage-network-policies.adoc#comparing-kubernetes-manifests-using-the-network-policy-connectivity-difference-command_manage-network-policies[Comparing Kubernetes manifests using the network policy connectivity difference command].
