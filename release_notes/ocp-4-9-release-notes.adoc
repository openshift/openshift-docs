...
* For clusters installed on vSphere infrastructure you cannot upgrade to {product-title} 4.9 if the network provider used is OVN-Kubernetes and xref:../installing/install_config/installing-customizing.html#installation-special-config-encrypt-disk_installing-customizing[Tang disk encryption] is enabled. If you attempt to upgrade the cluster, nodes become stuck in the `NotReady` state. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2006756[*BZ#2006756*])
