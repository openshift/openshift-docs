...
Resource Manager (ARM) endpoint. This endpoint needed to be provided through Azure Stack Hub configuration. Consequently, the ingress operator failed to configure a wildcard DNS record for ingress on Azure Stack Hub. With this update, the ingress operator can check the cluster infrastructure configuration object for the ARM endpoint provided by the installation program. As a result, the ingress operator uses the ARM endpoint to configure the wildcard DNS record on the Azure Stack Hub. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2032677[*BZ#2032677*])
