...
Logging in to the web console with the `cluster-admin` role now enables you to create new network policies in any namespace in the cluster from a form in the console as well as directly in YAML.

For more information, see xref:..https://docs.openshift.com/container-platform/4.9/networking/network_policy/creating-network-policy.html[Creating a network policy]
