...
* Cluster administrators can specify a custom HTTP error code response page for either 503, 404, or both error pages. If you do not specify the correct format for the custom error code response page, the router pods go down and do not come up again. The router does not reload to reflect custom error code page updates. As a workaround, rsh in the router pods and run `reload-haproxy` in all the router pods that serve the custom http error code pages:
+
[source,terminal]
----
