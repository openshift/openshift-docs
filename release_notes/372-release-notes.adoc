...
* Before this improvement, the legend in the Network Graph required hovering over the symbols representing namespaces, deployments, and connections. The updated legend shows the symbols and explanatory text together.
* Sometimes, the graph view edges did not connect the edges of nodes and overlapped on namespaces. This issue is fixed.
* Before this improvement, there were differences in the YAML file viewer instances. With this update, {product-title-short} shows the same formatting and interface when you view YAML files.
...
|====
|Feature | {product-title-short} 3.70 |{product-title-short} 3.71 | {product-title-short} 3.72

|`RenamePolicyCategory` and `DeletePolicyCategory` {product-title-short} Application Programming Interface (API) endpoints
|DEP
|DEP
|DEP
...
| `ids` field in the `/v1/cves/suppress` and `/v1/cves/unsuppress` {product-title-short} API payload
| GA
| DEP
| DEP
...
** Retrieval by using the `props` fields will be removed in version a future release.
