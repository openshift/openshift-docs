:_mod-docs-content-type: ASSEMBLY
//OpenShift Pipelines Release Notes
include::_attributes/common-attributes.adoc[]
[id="op-release-notes-1-21"]
= {pipelines-title} release notes
:context: op-release-notes-1-21

toc::[]

[NOTE]
====
For additional information about the {pipelines-shortname} lifecycle and supported platforms, refer to the link:https://access.redhat.com/support/policy/updates/openshift_operators[OpenShift Operator Life Cycles] and link:https://access.redhat.com/support/policy/updates/openshift[Red{nbsp}Hat {OCP} Life Cycle Policy].
====

Release notes contain information about new and deprecated features, breaking changes, and known issues. The following release notes apply for the most recent {pipelines-shortname} releases on {OCP}.

{pipelines-title} is a cloud-native CI/CD experience based on the Tekton project which provides:

* Standard Kubernetes-native pipeline definitions (CRDs).
* Serverless pipelines with no CI server management overhead.
* Extensibility to build images using any Kubernetes tool, such as S2I, Buildah, JIB, and Kaniko.
* Portability across any Kubernetes distribution.
* Powerful CLI for interacting with pipelines.
* Integrated user experience with the {OCP} web console, up to {OCP} version 4.20.

For an overview of {pipelines-title}, see xref:../about/understanding-openshift-pipelines.adoc#understanding-openshift-pipelines[Understanding {pipelines-shortname}].

// Compatibility and support matrix 1.21
include::modules/op-tkn-pipelines-compatibility-support-matrix.adoc[leveloffset=+1]

// Release notes for Red Hat OpenShift Pipelines 1.21.0
include::modules/op-release-notes-1-21-0.adoc[leveloffset=+1]

//Additional resources 1.21
//.Additional resources
//* xref:../modules/op-event-pruner-configuration.adoc#event-pruner-configuration_customizing-configurations-in-the-tektonconfig-cr[Enabling the event-driven pruner]
//* xref:../modules/op-resolver-cluster-tasks-ref.adoc#op-opc-task_remote-pipelines-tasks-resolvers[opc task]
//* xref:modules/op-behavior-of-approvaltask-with-groups-and-users.adoc#op-behavior-of-approvaltask-with-groups-and-users_using-manual-approval[Behavior of ApprovalTask with groups and users]
//* xref:../modules/op-pipelines-as-code-command-reference.adoc#pipelines-as-code-command-reference_pac-command-reference[{pac} command reference]
//* xref:../modules/op-customizing-pipelines-as-code-configuration.adoc#customizing-pipelines-as-code-configuration_install-config-pipelines-as-code[Customizing {pac} configuration]
