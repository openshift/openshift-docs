...
+
The application template for AMQ deployments uses suffix for every transport type to distinguish. Hence there should be a configurable parameter for setting service name as envvironmental parameter *`A_MQ_SERVICE_NAME*`. 
