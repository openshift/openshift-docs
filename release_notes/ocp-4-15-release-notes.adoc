...
{product-title} release 4.15.50, which includes security updates, is now available. The list of bug fixes that are included in this update is documented in the link:https://access.redhat.com/errata/RHSA-2025:7698[RHSA-2025:7698] advisory. The RPM packages that are included in this update are provided by the link:https://access.redhat.com/errata/RHBA-2025:7700[RHBA-2025:7700] advisory.
...
* The grandmaster clock (T-GM) transitions to the Locked state too soon. This is a known issue that happens before the Digital Phase-Locked Loop (DPLL) completes its change to the Locked-HO-Acquired state, and after the Global Navigation Satellite Systems (GNSS) time source is restored. (link:https://issues.redhat.com/browse/OCPBUGS-55466[*OCPBUGS-55466*]).

[id="ocp-4-15-51-bug-fixes_{context}"]
==== Bug fixes

* Previously, when viewing the list of installed Operators, an Operator appeared twice in the list. This duplication occurred when the currently selected project matched an Operator's default namespace while copied cluster service versions (CSVs) were disabled in the Operator Lifecycle Manager (OLM). With this release, the Operator appears once. (link:https://issues.redhat.com/browse/OCPBUGS-55733[OCPBUGS-55733]). 

* Previously, when a validating webhook was created for a user or group on a resource managed by the {product-title} oauth API server, the validating webhook did not get executed. With this release, the addition of a konnectivity proxy sidebar fixed the communication between openshift oauth API server and data plane by adding a konnectivity proxy sidecar. The webhooks on users and groups are now validated as expected.
