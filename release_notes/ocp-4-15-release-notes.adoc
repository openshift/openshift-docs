...
* Previously, the `platformFaultDomainCount` parameter was configured to use the maximum available value instead of a fixed value of `2`. After a cluster upgrade operation, this parameter configuration caused the machine set controller to attempt to modify existing availability sets during machine set scaling operations. but the modification operation failed because availability sets are immutable. With this release, availability sets are no longer modified after a clusteer upgrade operation so that machine sets can scale successfully. (link:https://issues.redhat.com/browse/OCPBUGS-56657[OCPBUGS-56657]). 
