...
==== Removal of the NetworkPolicyStatus property

{product-title} {product-version} has removed support for the `NetworkPolicyStatus` property of the `NetworkPolicy` object. The property enabled the `status` subresource for the `NetworkPolicy` object. If you need to check the status information of network policies, you can use the observeability function of your container network interface (CNI) plug-in or monitor logs and events from CNI controller pods.
