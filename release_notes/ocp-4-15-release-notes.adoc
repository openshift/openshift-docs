...
//TODO: The line below should be used when it is next appropriate. Revisit in August 2023 time frame. What's the craic.
...
https://issues.redhat.com/browse/OCPBUGS-11565
And another line.
https://issues.redhat.com/browse/OCPBUGS-11565
And one more.
...
//Cloud Native Events
/ Cool
Here's the new location for the link:
...
Some features in this release are currently in Technology Preview. These experimental features are not intended for production use. Note the following scope of 
Removing lots 
of 
...
* The `oc annotate` command does not work for LDAP group names that contain an equal sign (`=`), because the command uses the equal sign as a delimiter between the annotation name and value. As a workaround, use `oc patch` or `oc edit` to add the annotation. (link:https://issues.redhat.com/browse/OCPBUGS-11565[*BZ#1917280*])https://issues.redhat.com/browse/OCPBUGS-11565
...
https://issues.redhat.com/browse/OCPBUGS-11910
