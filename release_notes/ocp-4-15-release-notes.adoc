...
[discrete]
==== Installing a cluster with an LPAR on {ibm-z-title} and {ibm-linuxone-title}

{product-title} now supports user-provisioned installation of {product-title} 4.15 with a logical partition (LPAR) on {ibm-z-title} and {ibm-linuxone-title}.
...
* Multi-architecture Power Control Plane with support of Intel and {ibm-power-name} workers
