...
* Before this update, a network timeout occurred while installing an Operator on a cluster. This event resulted in a failed Operator installation. With this release, the Operator installation timeout issue is resolved by increasing the catalog cache refresh interval to 30 minutes. As a result, the Operator installation is successful. (link:https://issues.redhat.com/browse/OCPBUGS-57430[OCPBUGS-57430])

* Before this update, a Machine Config Operator (MCO) deployed `MoneyChainNet` (MCN) custom resource definitions (CRD) in default clusters, which caused upgrade conflicts due to a {product-title} 4.15 feature. As a consequence, upgrade failures in {product-title} 4.15 caused instability during cluster updates. With this release, the MCN CRD deployment in default clusters is removed to avoid upgrade conflicts. As a result, the upgrade process does not fail because of MCN CRD conflicts. (link:https://issues.redhat.com/browse/OCPBUGS-59737[OCPBUGS-59737])

* Before this update, navigating between tabs on the *Storage class* page with a multipath URL resulted in an error. As a consequence, blank pages appeared when switching tabs after loading the plugin on a *Storage class* page. With this release, there are no blank tabs on the *Storage class* page after switching from the *Portworx* tab because the invalid `href` is corrected. As a result, there are no blank pages when switching tabs in the console plugin. (link:https://issues.redhat.com/browse/OCPBUGS-60029[OCPBUGS-60029])

* Before this update, creating a cluster with an internal Image Registry on {azure-first} using a reused storage account with a private endpoint was not thoroughly checked. This issue led to mount permission issues when creating a pod and persistent volume claim (PVC) with the `azurefile-csi` storage class. As a consequence, users encountered mount permission issues with private storage accounts, which resulted in pod failures. With this release, the `azurefile-csi` storage class is updated to support private endpoints, and resolves mount permission issues with pre-existing private storage accounts. As a result, pods run successfully using the predefined storage class. (link:https://issues.redhat.com/browse/OCPBUGS-60544[OCPBUGS-60544])
