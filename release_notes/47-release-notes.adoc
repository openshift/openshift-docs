...
[id="about-release-477_{context}"]
== About release version 4.7.7

*Release date*: {ga-date-477}

//ROX-30924
Starting with {product-title-short} 4.7.7, you might notice changes to container image metadata, such as container labels or Software Bill of Materials (SBOM) contents and location, as the product images are now built and released by using different technologies. These changes do not affect product functionality, but they might impact your third-party integrations.

This release addresses the following issue:

//ROX-30447, ROX-30498
* Before this update, you might have experienced a consistently growing Central database that caused constant resizing of the persistent volume claim (PVC) or service interruptions from a `no space left on device` error. With this release, if you want to prevent the hashes table from growing, set the environment variable `ROX_HASH_FLUSH_INTERVAL` to `0`.

This release addresses the following security vulnerability:

* Flaw in the form-data dependency (link:https://access.redhat.com/security/cve/CVE-2025-7783[CVE-2025-7783])

[id="about-release-478_{context}"]
== About release version 4.7.8

*Release date*: {ga-date-478}

This release addresses the following security vulnerability:

//ROX-30909
* Lack of a data size check in the Axios dependency (link:https://access.redhat.com/security/cve/CVE-2025-58754[CVE-2025-58754])

[id="about-release-479_{context}"]
== About release version 4.7.9

*Release date*: {ga-date-479}
...
* Incorrect default permissions assigned to critical directories in containerd (link:https://access.redhat.com/security/cve/cve-2024-25621[CVE-2024-25621])

* `libexpat` excessive memory allocation (link:https://access.redhat.com/security/cve/cve-2025-59375[CVE-2025-59375])

* SQLite query causes memory corruption (link:https://access.redhat.com/security/cve/cve-2025-6965[CVE-2025-6965])
