...
For more information, see xref:../operating/manage-user-access/configure-short-lived-access.adoc#configure-short-lived-access[Configuring short-lived access].
...
In addition to the severity and Common Vulnerability Scoring System (CVSS) score, an EPSS probability score from 0%-100% is now displayed for each detected Common Vulnerabilities and Exposures (CVEs). 
...
This feature was a technology preview in {product-title-short} 4.6.0 and is now generally available in {product-title-short} 4.7.0.
...
{product-title-short} 4.7 introduces improved options for the `roxctl netpol generate` command. You can now specify DNS ports by using names as well as numbers. If you do not specify a port, port `53` is used automatically. You can change the port value by using the `--dnsport` flag. 

For example, you can now specify the port when a service has a defined name by using the `--dnsport dns` flag. 
If you are using {osp} with the default DNS setting, you can change the default port by using the `--dnsport` flag, as the DNS pod of {osp} listens on port `5353`.
...
With {product-title-short} 4.7, you can now generate a Software Bill of Materials (SBOM) from the scanned container images. These SBOMs give you a detailed overview of all software components, dependencies, and libraries within your application.
...
////
For information about how to use {product-title-short} to generate SBOMs, see xref:../operating/manage-vulnerabilities/scanner-generate-sbom.adoc#scanner-generate-sbom[Generating SBOMs from scanned images].
////
...
{product-title-short} 4.7 introduces the automatic rotation of certificates.
{product-title-short} secured cluster components use these certificates to securely communicate with the Central service.
...
For more information, see xref:../configuration/reissue-internal-certificates.adoc#reissuing-internal-certificates-for-secured-clusters-by-using-automatic-certificate-renewal_reissue-internal-certificates[Reissuing internal certificates for secured clusters by using automatic certificate renewal].
...
+
To enable scanning of Scanner V4 nodes while scanning of Scanner V2 nodes continues in parallel, you must change the `ROX_NODE_INDEX_ENABLED` variable from a BooleanSetting to a feature flag so that it is accessible to the user interface (UI) through the Central API. Central with Scanner V4 automatically prefers Scanner V4 scans, while those with StackRox Scanner V2 continue to work without changes.

* `stackRox.io` Content Delivery Network (CDN) has been moved from CloudFlare to Akamai. When configuring firewall rules, you must use the hostname instead of the IP addresses. If you previously allowed the IP ranges from `stackrox.io`, you must update these rules. 
+
...
+
This caused issues in {product-title-short} 4.6.0 and 4.6.1 when testing the technical preview feature of {op-system} node scanning with Scanner V4, as identical IndexReport messages sent at different times are treated as duplicates, even though they might return different sets of vulnerabilities.
...
//ROX-27829
* Before to this update, the {product-title-short} UI incorrectly validated Slack webhook URLs and blocked the Mattermost integration due to strict regex rules. With this update, the regex check has been removed to allow for more flexible URL formats.
