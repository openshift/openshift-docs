...
The dynamic configuration manager is enabled by default in technology preview. You can reduce your memory footprint by using the dynamic configuration manager on ingress controllers to propogate endpoint changes through a dynamic API that enables the underlying routers to adapt to changes (scale ups and scale downs) without reloads. 

You can use the dynamic configuration manager by enabling the `TechPreviewNoUpgrade` feature set.
