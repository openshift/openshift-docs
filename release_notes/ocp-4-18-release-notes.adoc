...
[id="ocp-4-18-aws-custom-dns_{context}"]
==== Installing a cluster on {aws-first} with customer managed DNS (Tech Preview)

You can now install a cluster on {aws-short} with a custom DNS solution that you manage. If your organization's security policies do not allow the use of public DNS services, you can use your own DNS service with this feature. If you provide your own DNS solution, it must include entries for the `api.<cluster_name>.<base_domain>` and `*.apps.<cluster_name>.<base_domain>` records.

For more information, see xref:../installing/installing_aws/installation-config-parameters-aws.adoc#installation-configuration-parameters-optional-aws_installation-config-parameters-aws[Installation configuration parameters].
