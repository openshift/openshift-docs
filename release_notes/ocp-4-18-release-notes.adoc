...
[id="ocp-4.18-support-for-plugins_{context}"]
==== Configuring node affinity scoring for architectures

This feature introduces a new field, `plugins`, in the `ClusterPodPlacementConfig` object for architecture-aware workload scheduling. You can configure the `plugins.nodeAffinityScoring` field to set architecture preferences for pod placement. If you enable the `nodeAffinityScoring` plugin, the scheduler prioritizes nodes for pod placement based on the scores configured for architectures in the `nodeAffinityScoring.platforms` field.

For more information about the `nodeAffinityScoring` plugin, see TBD.
//xref link will be added after merging the PR for OSDOCS-13057
