...
[discrete]
[id="ocp-release-note-edge-computing-bug-fixes_{context}"]
==== Edge computing

* Previously, disk encryption with PCR 1 and 7 protection failed on systems that automatically created extra EFI entries during boot. The extra entry changed the EFI variables, which caused server attestation with PCR 1 to fail. With this release, disk encryption with PCR 1 and 7 protection works correctly, even if the system creates extra EFI entries during boot.
