...
{product-title} release {product-version}.1, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHBA-2025:1904[RHBA-2025:1904] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2025:1908[RHSA-2025:1908] advisory.
...
// SME
* Previously, the Operator Controller would not accept live updates to registries with a proxy configuration. This issue caused OLM v1 to read the wrong image URL and this prevented controller pods from restarting. With this release, a fix to the Operator Controller means that it accepts live updates to registries with a proxy configuration so that the controller pod restart issue no longer persists. (link:https://issues.redhat.com/browse/OCPBUGS-51141[*OCPBUGS-51141*])
...
// SME review
* Previously, when the `ClusterNetwork` classless inter-domain routing (CIDR) mask value is greater than the `hostPrefix` value in the `install-config.yaml` file, the installation program failed a validation check. With this release, a fix means that this situation does not cause the installation program to fail a validation check so that the cluster installation can continue as expected. (link:https://issues.redhat.com/browse/OCPBUGS-49864[*OCPBUGS-49864*])
...
// SME needed
* Previously, {gcp-first} removed API calls that did not include a `wait` operation to designate an API call as having ended. This issue caused the installation program to not delete backend services. With this release, {gcp-short} adds a `wait` operation to an API call before removing them so that the installation program can delete backend services. (link:https://issues.redhat.com/browse/OCPBUGS-49320[*OCPBUGS-49320*])

* Previously, when the cluster certificate authority (CA) bundle was updated with a custom CA bundle, a delay would occur for the change to reflect in the `insights-runtime-extractor` container. This issue occurred if the Insights Operator gathered data after the CA bundle is updated. The container might not have been restarted and the workload runtime data would not be gathered. With this release, a fix removes the delay so that this issue no longer occurs.(link:https://issues.redhat.com/browse/OCPBUGS-48790[*OCPBUGS-48790*])
...
* Previously, bundle properties occasionally did not get propagated to the annotations on Helm charts that were created during an Operator installation. With this release, properties are now taken from both the CSV of the bundle and `metadata.yaml` file or `properties.yaml` file so that this issue no longer exists. (link:https://issues.redhat.com/browse/OCPBUGS-45114[*OCPBUGS-45114*])
...
// https://issues.redhat.com/browse/OCPBUGS-32158 RN needed?
