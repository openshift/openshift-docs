...
* In a future release, the following endpoints will require authentication:
** The `/v1/availableAuthProviders` endpoint will require authentication and a permission level of at least READ on the `Access` resource. Ensure that any flow interacting with this endpoint is authenticated and has the proper permission.
** The `/v1/tls-challenge` endpoint will require authentication. Ensure that all interactions with this endpoint includes proper authentication.
...
[id="resolved-in-version-445_{context}"]
=== Resolved in version 4.4.5

*Release date*: 29 August 2024

This release of {product-title-short} fixes the following bugs:

* Fixed an issue where Sensor could not identify pull secrets because the annotation was changed from `kubernetes.io/service-account.name` to `openshift.io/internal-registry-auth-token.service-account` in {ocp}.
...
* link:https://access.redhat.com/security/cve/CVE-2024-3727[CVE-2024-3727]: Fixed a vulnerability related to Scanner where the `containers/image` digest type did not guarantee a valid type.

* link:https://access.redhat.com/security/cve/CVE-2024-37298[CVE-2024-37298]: Fixed a vulnerability in `gorilla/schema` that allowed a potential memory exhaustion attack due to sparse slice deserialization.

* link:https://access.redhat.com/security/cve/CVE-2024-6104[CVE-2024-6104]: Fixed a vulnerability in `go-retryablehttp` where URLs might inadvertently write sensitive information to log files.

[id="resolved-in-version-446_{context}"]
=== Resolved in version 4.4.6

*Release date*: 13 November 2024

This release of {product-title-short} fixes the following bug:

* Fixed an issue where you could not view detailed scan results for certain images in {product-title-short} because the page was redirected to an error after a brief display.

This release of {product-title-short} fixes the following security vulnerabilities:

* link:https://access.redhat.com/security/cve/CVE-2024-34156[CVE-2024-34156]: Fixed a vulnerability in the `encoding/gob` package that could cause a Denial of Service (DoS) through stack exhaustion when you decode deeply nested structures.

* link:https://access.redhat.com/security/cve/CVE-2024-45590[CVE-2024-45590]: Fixed a vulnerability in the body-parser that could cause a DoS through specially crafted payloads when you have enabled URL encoding.

* link:https://access.redhat.com/security/cve/CVE-2024-48910[CVE-2024-48910]: Fixed a vulnerability in `DOMPurify` that allowed remote attackers to modify object prototypes, potentially injecting malicious attributes or causing application crashes.

* link:https://access.redhat.com/security/cve/CVE-2024-24789[CVE-2024-24789]: Fixed a vulnerability in the `archive/zip` library that allowed malicious compressed files to access hidden information or files through mishandling corrupted central directory records.

* link:https://access.redhat.com/security/cve/CVE-2024-24790[CVE-2024-24790]: Fixed a vulnerability in the `net/netip` library where `Is*()` methods did not properly handle IPv6 addresses mapped to IPv4 addresses, that impacted the resource access control.

[id="resolved-in-version-447_{context}"]
=== Resolved in version 4.4.7
