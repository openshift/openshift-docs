...
* *Consistent and accurate scanning*: Reliable vulnerability scan results across the entire Red Hat product ecosystem, Red Hat {product-title-short}, and Red Hat Quay.
...
* *Comprehensive vulnerability database source*: Adoption of link:https://osv.dev/[`OSV.dev`] as the vulnerability database source for all supported programming language packages.
...
For more information about the OpenShift Operator maintenance lifecycles, see the Red Hat Knowledgebase solution link:https://access.redhat.com/support/policy/updates/openshift_operators[OpenShift Operator Life Cycles].

For more information about compatibility and supportability with {osp} releases, see the Red Hat Knowledgebase solution link:https://access.redhat.com/node/7045053[{product-title} Support Matrix].
...
In the {product-title-short} portal, navigate to *Platform Configuration -> Integrations*, and then select the integration type, for example, *Amazon ECR*. You can select an existing integration or click *New integration* to create a new one.
...
In the {product-title-short} portal, navigate to *Platform Configuration -> Integrations*, and then select the integration type, for example, *Google Artifact Registry*. You can select an existing integration or click *New integration* to create a new one.
...
In the {product-title-short} portal, navigate to *Platform Configuration -> Clusters -> Discovered clusters* to view the current status of the discovered clusters.
...
In the {product-title-short} portal, navigate to *Platform Configuration -> Integrations -> Machine access configuration* to create a configuration to enable exchanging OIDC identity tokens for short-lived {product-title-short}-issued tokens.
