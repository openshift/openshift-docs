...
* Currently, there is an issue for OpenShift Container Platform 4.12 on the {ibm-power} architecture where the CORE_BPF collection method does not work properly. From {product-title-short} 4.4.1 and later, Collector automatically uses the EBPF collection method in this particular case when CORE_BPF is selected. 
