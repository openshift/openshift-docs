...
* Running the `oc get packagemanifest` or `oc describe packagemanifest` commands can return a random package manifest if the following conditions are met:
+
--
** Multiple catalog sources are installed in the same namespace.
** The catalog sources contain the same Operators or Operators with the same name.
--
+
You can specify the catalog source for the Operator you want to install by running the following command:
+
[source,terminal]
----
