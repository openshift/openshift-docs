...
For more informsation, see xref:../installing/installing_aws/installing-aws-localzone.adoc#installing-aws-localzone[Installing a cluster using AWS Local Zones].
...
[id="ocp-4-13-rhcos-new-regions"]
====  New Alibaba Cloud regions available for deploying {product-title}
{op-system} images are now available for a few new Alibaba Cloud regions. You can now install {product-title} to the following regions:
...
With this release, {ibmzProductName} and {linuxoneProductName} are now compatible with {product-title} {product-version}. The installation can be performed with z/VM or {op-system-base} KVM. For installation instructions, see the following documentation:
...
[discrete]
==== {ibmzProductName} and {linuxoneProductName} supported new features

The following new features are supported on {ibmzProductName} and {linuxoneProductName} with {product-title} {product-version}:
...
[discrete]
==== {ibmzProductName} and {linuxoneProductName} supported features

The following features are also supported on {ibmzProductName} and {linuxoneProductName}:
...
These features are available only for {product-title} on {ibmzProductName} and {linuxoneProductName} for {product-version}:

* HyperPAV enabled on {ibmzProductName} and {linuxoneProductName} for the virtual machines for FICON attached ECKD storage

[discrete]
==== Restrictions on {ibmzProductName} and {linuxoneProductName}

The following restrictions impact {product-title} on {ibmzProductName} and {linuxoneProductName}:
...
The following new features are supported on {ibmpowerProductName} with {product-title} {product-version}:
...
[discrete]
==== {ibmpowerProductName} new supported features

The following features are also supported on {ibmpowerProductName}:
...
[discrete]
==== {ibmpowerProductName} restrictions

The following restrictions impact {product-title} on {ibmpowerProductName}:
...
[discrete]
[id="ocp-4-13-ocm-nodeip-configuration-vsphere"]
...
* Previously, the `ControlPlaneMachineSet` Operator attempted to recreate a control plane machine before the cluster bootstrapping completed. This resulted in the removal of the bootstrap node from the etcd cluster membership that caused etcd quorum loss and the cluster to go offline. With this update, `ControlPlaneMachineSet` Operator only recreates a control plane machine after the etcd Cluster Operator removes the bootstrap node. (link:https://issues.redhat.com/browse/OCPBUGS-10960[*OCPBUGS-10960*])
