...
==== {sno-caps} supported on x86_64 and arm64 architectures

{product-title} {product-version} now supports `x86_64` and `arm64` CPU architectures on {sno-caps}. When running a large fleet of clusters, this enhancement lets you reduce resource utilization during development and testing. For example, Edge solutions where development is in the cloud and deployment is to bare metal {sno-caps}.
