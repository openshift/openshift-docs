...
* Previously, a control plane machine set replica was considered ready when its backing machine entered the `Running` state, even though the linked node must also be ready for the replica to be considered ready. With this release, a node and its machine must be in the `Ready` state for the control plane machine set replica to be considered ready. (link:https://issues.redhat.com/browse/OCPBUGS-8424[*OCPBUGS-8424*])

* The previous resolution of link:https://issues.redhat.com/browse/OCPBUGS-5546[OCPBUGS-5546] removed the `clusterName` assignment of `MachineConfig.Name` in the machine configuration object. As a result, the value of the parameter was an empty string and, when combined with the value of `machineName` to create an IP address name, created an invalid value. This caused machines to fail during provisioning. With this release, the value for `clusterName` is obtained from the infrastructure object so that it creates a valid IP address name.
(link:https://issues.redhat.com/browse/OCPBUGS-7696[*OCPBUGS-7696*])
...
* Previously, the `spec.metadata.annotations` parameter value in an Amazon Web Services compute machine set was not copied from a compute machine to its node. This caused the node be missing the annotations specified in the compute machine set. With this release, annotations are correctly applied to the node.
...
* Previously, the bare metal installer-provisioned infrastrature used port 80 for providing images to the Baseboard Management Controller (BMC) and the deployment agent. Security risks could exist with port 80, because this port is commonly chosen for internet communcations. The bare metal installer-provisioned infrastrature now used port 6180 for serving images that are used by the `metal3` pod on deployed clusters. (link:https://issues.redhat.com/browse/OCPBUGS-8511[*OCPBUGS-8511*])

* Previously, the `aws-sdk-go` library that the installation program uses did not recognize newly added regions. When you issued the `create install-config` command, the program would not display the regions as options. Additionlly, the installation would fail if the installation requires the regions for its configuration. These issues are now resolved, because the `aws-sdk-go` library now supports the `ap-south-2`, `ap-southeast-4`, `eu-central-2`, `eu-south-2`, and `me-central-1` regions. (link:https://issues.redhat.com/browse/OCPBUGS-102131[*OCPBUGS-10213*])
