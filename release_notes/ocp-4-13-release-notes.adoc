...
* Previously, when editing any pipeline in the {product-title} console, the correct data was not rendered in the *Pipeline builder* and *YAML view* configuration options. This issue prevented you from editing the pipeline in the *Pipeline builder*. With this update, data is parsed correctly and you can edit the pipeline by using the builder. (link:https://issues.redhat.com/browse/OCPBUGS-5016[*OCPBUGS-5016*])

* Previously, the {product-title} used API version `v1alpha1` for Knative serving and Eventing but the flag was not set for API version `v1beta1`. With this fix, the {product-title} supports both the API versions. (link:https://issues.redhat.com/browse/OCPBUGS-5164[*OCPBUGS-5164*])
...
* In the {product-title} 4.12 release, the node terminal tab would display an error because of missing annotations that were caused by pod security measures. Without the required annotations, the node debug pod cannot start. With this update, {product-title} adds these annotations, so that the node debug pod can start and the node terminal tab loads without any errors. (link:https://issues.redhat.com/browse/OCPBUGS-4252[*OCPBUGS-4252]) 

*  Previously, when downloading a `kubeconfig` file for a ServiceAccount, an error is displayed and the ServiceAccount token is unable to be reached. This error is due to the removal of autogenerated secrets. With this update, the download `kubeconfig` action has been removed and the error no longer occurs. (link:https://issues.redhat.com/browse/OCPBUGS-7308[*OCPBUGS-7308*])

* Previously, the cluster dropdown was positioned so it is not always visible making it unclear which cluster you are viewing. With this update, the cluster dropdown is now in the masthead so the cluster dropdown is always visible, and you can always see which cluster you are viewing. (link:https://issues.redhat.com/browse/OCPBUGS-7089[*OCPBUGS-7089*])
