...
[id="ocp-4-12-cli-manager"]
==== OpenShift CLI Manager (Technology Preview)

You can now use the OpenShift CLI Manager for easier management of plug-ins for the OpenShift CLI (`oc`). The OpenShift CLI Manager is offered as a link:https://access.redhat.com/support/offerings/techpreview[Technology Preview].

Cluster administrators decide which CLI plug-ins to publish using the OpenShift CLI Manager. Cluster users can then list the available CLI plug-ins and install them locally.

For more information, see xref:../cli_reference/openshift_cli/cli-manager.adoc#cli-manager[Managing CLI plug-ins with the OpenShift CLI Manager].
...
In {product-title} 4.12, the Cluster Ingress Operator exports a new metric named `route_metrics_controller_routes_per_shard`. The `name` label of the metric specifies the name of the shards. This metric gives the total number of routes that are admitted by each shard.
