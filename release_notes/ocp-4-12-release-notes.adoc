...
==== OVN-Kubernetes is now the default networking plug-in

When installing a new cluster the OVN-Kubernetes network plug-in is the default networking plug-in. For all prior versions of {product-title}, OpenShift SDN remains the default networking plug-in.

The OVN-Kubernetes network plug-in includes a wider array of features than OpenShift SDN, including:
...
If you are using the OpenShift SDN network plug-in, note that:
...
For more information about OVN-Kubernetes, including a feature comparison matrix with OpenShift SDN, see xref:../networking/ovn_kubernetes_network_provider/about-ovn-kubernetes.adoc#about-ovn-kubernetes[About the OVN-Kubernetes network plug-in].
