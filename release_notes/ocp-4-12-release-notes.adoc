...
[id="ocp-4-12-ironic-container-image-rhel9"]
==== Ironic container images use RHEL 9 base image
In earlier versions of {product-title}, Ironic container images used Red Hat Enterprise Linux (RHEL) 8 as the base image. In {product-title} 4.12 and later, Ironic container images use RHEL 9 as the base image. The RHEL 9 base image adds support for CentOS Stream 9, Python 3.8, and Python 3.9 in Ironic components.

For more information about the Ironic provisioning service, see the xref:../installing/installing_bare_metal_ipi/ipi-install-overview.adoc[deployment of installer-provisioned clusters on bare metal].
...
[id="ocp-4-12-nw-ipxe-ztp-network-booting"]
...
[id="ocp-4-12-ipv6-spoke-cluster-support"]
