...
// TODO: Include an update like this in whichever 4.12 z-stream this gets backported to:
[id="ocp-4-12-x-features"]
==== Features

[id="ocp-4-12-x-psa-tp-feature-set"]
===== Pod security admission restricted enforcement (Technology Preview)

With this update, pod security admission restricted _enforcement_ is available as a Technology Preview feature by enabling the `TechPreviewNoUpgrade` feature set. Now, if you enable the `TechPreviewNoUpgrade` feature set, pods are rejected if they violate pod security standards, instead of only logging a warning.
