...
[id="ocp-4-12-10-features"]
==== Features

[id="ocp-4-12-10-oc-mirror-max-nested-paths"]
===== New flag for the oc-mirror plugin: --max-nested-paths

With this update, you can now use the `--max-nested-paths` flag for the oc-mirror plugin to specify the maximum number of nested paths for destination registries that limit nested paths.
