...
[id="ocp-4-12-oc-commands-flags-tokens-deprecated"]
...
[id="ocp-4-12-deprecated-features-vsphere-7.0update1"]
==== Support for vSphere 7.0 Update 1 or earlier is deprecated
In {product-title} {product-version}, support for VMware vSphere 7.0 Update 1 or earlier is deprecated. While vSphere 7.0 Update 1 or earlier remains fully supported, Red Hat recommends that you use vSphere 7.0 Update 2 or later.

[id="ocp-4-12-deprecated-features-esxi7.0update1"]
...
[id="ocp-4-12-removed-features-vsphere-6.7update2"]
...
[id="ocp-4-12-removed-features-esxi6.7update2"]
...
==== Networking
* Previously, administrators had to manually create allow rules in egress firewalls or a generic CIDR rule that encompasses the host networks to by pass them. With this update, `NodeSelector` has been added to the egress firewall destination spec in OVN-Kubernetes cluster network provider. Users can now specify a label applied to one or more nodes and the OVN-Kubernetes cluster network provider node IPs will be included in that rule. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1993841#[*BZ#1993841*])
