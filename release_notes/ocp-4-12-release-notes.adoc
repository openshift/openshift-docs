...
* Previously, attempting to install an {sno} cluster with two Day 2 workers failed when using `ignitionEndpoint`. After the initial spoke cluster installation, the `AgentClusterInstall` never reached the `adding-hosts` state and remained in the `installed` state. Additionally, the admin `kubeconfig` secret was not being generated on the hub cluster, so the `kubeapi` of the spoke cluster could not be accessed. With this update, installing an {sno} cluster with two Day 2 workers succeeds when using `ignitionEndpoint`. The `AgentClusterInstall` now reaches the `adding-hosts` state and the admin `kubeconfig` secret is generated on the hub cluster, allowing the `kubeapi` of the spoke cluster to be accessible.

* Previously, when deploying a spoke cluster via Assisted Service (Infra Operator) in a disconnected environment and referencing a public ocp image to install, the assisted service pod would not recognize the ImageContentSourcePo (ICSP) object and fail to pull the image. Setting the ICSP in the `AgentClusterInstall` as an `install-config` override and relying on the mirror configmap also resulted in the same error. With this update, the assisted service pod recognizes the ICSP and pulls the image. There is no longer an error when setting the ICSP in the `AgentClusterInstall` as an `install-config` override when using the mirror configmap.
...
* Previously, when adding Day 2 workers to an {sno} cluster, certificate signing requests (CSR) were not auto-approved and did not automatically join the cluster. With this update, CSRs are auto-approved and nodes join the cluster automatically.

* Previously, when adding a Day 2 worker with an illegal role (not master, worker, auto-assign), the agent resource reported `specsynced` as successful instead of reporting an "Illegal role" error, and the `agentclusterinstall` displayed a backend error for `specsynced`. With this update, the agent resource reports the backend error, and agent validations fail when `specsynced` reports "Illegal role defined".
