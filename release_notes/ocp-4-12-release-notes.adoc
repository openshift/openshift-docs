...
Beginning with OpenShift Container Platform 4.12, you can specify a persistent load balancer type in AWS during installation. If an ingress controller is deleted, the load balancer type no longer reverts to the default (Classic ELB), but persists as configured.

For more information, see xref:../installing/installing-aws.adoc#installation-network-customizations_installing-aws[Installing a cluster on AWS with network customizations].
