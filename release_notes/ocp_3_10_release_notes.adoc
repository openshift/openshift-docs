...
* In the xref:../upgrading/downgrade.adoc#install-config-downgrade[Downgrading
OpenShift] documentation, an issue was found with the steps for restoring etcd.
A warning admonition has been included at the beginning of the topic stating
that the downgrade steps should not be followed until this issue has been
resolved. Further testing is ongoing and the documentation will be updated
shortly with improved procedures.
