...
[[ocp-310-operator-SDK]]
==== Operator SDK

This feature helps you build and publish Kubernetes applications that run like
cloud services anywhere {product-title} runs. Operator SDK includes all
scaffolding code. You only need to build the logic specific to the application.

See link:https://coreos.com/blog/introducing-operator-framework[Introducing the
Operator Framework: Building Apps on Kubernetes] for more information.
