...
See xref:../admin_guide/managing_networking#admin-guide-deploying-an-egress-router-dns-proxy-pod[Deploying an Egress Router DNS Proxy Pod]
...
See xref:../install_config/configuring_sdn#growing-the-service-network[Growing the Service Network]
for more information.
...
The Node Problem Detector is in xref:ocp-310-technology-preview[Technology
Preview] and is not for production workloads. The Node Problem Detector monitors
the health of your nodes by finding certain problems and reporting these
problems to the API server, where external controllers could take action.
...
{product-title} is no longer supported. The 3.10 upgrade will require you to
move to RPMs or the kubelet system containers (by setting
`openshift_use_system_containers=true`), to start or run the platform.

* `openshift_node_bootstrap` is now removed. All nodes must be bootstrapped.
...
[[ocp-310-container-native-virtualization]]
==== Container Native Virtualization (Technology Preview)

Container Native Virtualization is currently in
xref:ocp-310-technology-preview[Technology Preview] and not for production
workloads.

link:https://access.redhat.com/articles/3500741[Container Native Virtualization]
is an add-on to {product-title} that allows virtual machine workloads to run and
be managed alongside container workloads in OpenShift. Existing virtual machines
can be imported into OpenShift, created from images stored in the container
registry, or created from scratch within OpenShift.

Container Native Virtualization introduces two new resources to OpenShift:

* Virtual Machine: The virtual machine image in OpenShift Virtual Machine
* Instance: A running instance of the virtual machine

With the Container Native Virtualizaiton add-on, virtual machines run in
OpenShift pods, and have the same network and storage capabilities, and
deployment strategies, as standard pods.

Existing virtual machine disks can be imported into OpenShift persistent volumes
(PVs), and made accessible to CNV virtual machines using persistent volume
claims (PVCs). In OpenShift, the virtual machine object can be modified or
replaced as needed, without affecting the persistent data stored on the PV.
...
See xref:../install_config/registry/accessing_registry#accessing-registry-metrics[Accessing Registry Metrics]
for more information.
...
[[ocp-310-web-console-miscelaneous-changes]]
==== Miscellaneous Changes
* You can create generic secrets in the web console (secrets with any key / value pairs).
* There is now an improved search algorithm for the service catalog UI.
...
==== Control Sharing the PID Namespace Between Containers
...
.If `--follow` fails, user is presented with the message, *Failed to stream the build logs - to view the logs, run oc logs build/<build-name>*.
...
* By default, no cap is set on the default logging performed by `docker`. The host
disk can easily fill completely with logs. Set
`openshift_docker_log_options='max-size=50m'` in the inventory file, or on the
Ansible command-line (`-e` option argument).  As an alternative, set
`openshift_docker_log_driver=journald`. The maximum size of logs will be
limited, or managed by journald.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1594678[*BZ#1594678*])
...
* Because of an issue with the Ansible installation playbooks, the playbooks
created directories for which Ansible did not have ownership. This could cause
the installer to fail if running the playbooks with a non-standard unmask
configuration. The playbooks have been changed to own all directories they
create.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1433022[*BZ#1433022*])
...
to `removedFromBrokerCatalog: false`, and was preventing removed classes from
being used again later. This issue is now resolved.
...
|xref:ocp-310-container-native-virtualization[Container Native Virtualization]
|-
|-
|TP
