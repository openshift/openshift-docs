...
now run as static pods by the kubelet on master hosts.
...
.Node bootstrapping overview
image::node_bootstrapping.png["Node bootstrapping overview"]
...
When configuring the Red Hat OpenStack cloud provider, the node's hostname must
