...
for
xref:../install_config/upgrading/manual_upgrades.adoc#manual-step-ocp-3-5-5-2[manually
...
See
xref:../architecture/additional_concepts/authorization.adoc#authorization-determining-what-you-can-do-as-an-authenticated-user[Authorization]
...
See
xref:../install_config/persistent_storage/dynamically_provisioning_pvs.adoc#azure-disk[Dynamic
...
xref:../install_config/upgrading/automated_upgrades.adocupgrade-hooks[custom
tasks], such as cycling hosts in and out of load balancers during the upgrade
process.
* Open vSwitch (OVS) and etcd version increases.
* Firewalld now triggers iptables updates. Firewalld alerts the kubelet of
iptables changes.
* Rolling updates of certificates.
* Increased use of Ansible modules, rather than freeform YAML files.
...
In {product-title} 3.5, there is now a "Run on OpenShift" experience that allows you to
provide external links in the web console to deploy templates.

Use the URL pattern to select a template or image. You can customize to have it
come from separate project. The end-user is prompted for the project.
...
cluster. See
xref:../install_config/upgrading/manual_upgrades.adoc#manual-step-ocp-3-5-0-0[Manual
...
There are no known issues for the {product-title} 3.5 initial GA release.
...
Space precluded documenting all of the bug fixes amd enhancements for this
...
[[oocp-3-5-rhba-2017-0903-upgrading]]
==== Upgrading
