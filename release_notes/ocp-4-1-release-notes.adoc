...
[[ocp-4-1-release-acknowledgements]]
=== Acknowledgments
...
[[ocp-4-1-new-features-and-enhancements]]
...
[[ocp-4-1-operator-lifecycle-manager]]
...
[[ocp-4-1-installation-and-upgrade]]
...
[[ocp-4-1-cluster-monitoring]]
=== Cluster monitoring

[[ocp-4-1-autoscale-pods-horizontally-based-on-custom-metrics-api]]
...
[[ocp-4-1-cluster-monitoring-alerting-UI]]
...
[[ocp-4-1-cluster-monitoring-telemeter]]
==== Telemeter
...
[[ocp-4-1-cluster-monitoring-autoscale]]
...
through the Kubernetes resource metrics API. There is no need to install a
separate metrics server anymore.

[[ocp-4-1-developer-experience]]
=== Developer experience

[[ocp-4-1-code-ready-containers]]
...
[[ocp-4-1-registry-managed-by-operator]]
...
[[ocp-4-1-new-management-screens]]
...
[[ocp-4-1-notable-technical-changes]]
...
[discrete]
[[ocp-4-1-service-broker-service-catalog-deprecation]]
...
[discrete]
[[ocp-4-1-service-catalog-no-longer-installed-by-default]]
...
[discrete]
[[ocp-4-1-template-service-broker-no-longer-installed-by-default]]
...
[discrete]
[[ocp-4-1-configurability-of-imagepolicyadmission]]
...
|Multi-stage buids in Dockerfiles
|
|TP
|GA
...
[[ocp-4-1-known-issues]]
== Known issues

* Unsafe sysctl cannot be used in {product-title} 4.1.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1690754[*BZ#1690754*])

* There is an issue in adding finalizers of machine-controller, which can cause
instances to be leaked during `delete()` operations.  If a call to the cloud
provider/actuator `Delete()`` function returns an error, since finalizers might
not be applied, the machine-object will be deleted. Since the `Delete()` error
could result in not being able to delete the actual instance from the cloud
provider (such as api-quota limits), the instance will survive the
machine-object deletion, and there will no longer be accounting of the instance.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1712056[*BZ#1712056*])
...
[[ocp-4-1-asynchronous-errata-updates]]
