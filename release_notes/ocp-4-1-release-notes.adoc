...
* When using service brokers, deleting projects can get stuck in a "Terminating"
state. This is caused by API servers not being deleting properly by service
catalog, and the deletion hangs. Service catalog leaves `serviceinstance`
objects that have a `kubernetes-incubator/service-catalog` finalizer and does
not do its job of deleting them. As a result, the namespace controller cannot
finish the namespace (project) deletion.
+
A change was made to service catalog to prevent the hangs by not deleting
itself. If you have any projects stuck in a "Terminating" state, you must remove
the finalizers from the `servicebindings` and `serviceinstances` objects, remove
the projects manually, then remove the `apiservice` object for the service
catalog. After implementing this workaround, you can continue deleting projects
without the service catalog causing an issue. See the Bugzilla report for a
script to automate the workaround procedure. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1746174[*BZ#1746174*])
