...
[id="security-update"]
== Security update
In previous versions of {product-title}, the `write` permission for the `APIToken` resource allowed users to create API tokens for any role, including the `admin` role. This security vulnerability has been fixed.

[id="important-system-changes"]
== Important system changes

//TODO: * Active and inactive components in Vuln management.
...
| registry.redhat.io/rh-acs/main:3.68.0

| Scanner
| Scans images and nodes.
| registry.redhat.io/rh-acs/scanner:3.68.0

| Scanner DB
| Stores image scan results and vulnerability definitions.
| registry.redhat.io/rh-acs/scanner-db:3.68.0

| Collector
| Collects runtime activity in Kubernetes or {ocp} clusters.
| registry.redhat.io/rh-acs/collector:3.68.0-latest
|===
