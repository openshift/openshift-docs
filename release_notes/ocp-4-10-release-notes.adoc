...
* Currently, for `ClusterGroupUpgrade` CR state changes, there is only a single condition type available - `Ready`. The `Ready` condition can have a status of `True` or `False` only. This does not refect the range of states the `ClusterGroupUpgrade` CR can be in.
