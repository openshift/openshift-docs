...
You can now install a cluster on {rh-openstack-first} for which compute machines run on Open vSwitch with the Data Plane Development Kit (OVS-DPDK) networks and have their workloads benefit from it. 
...
For clusters that run on {rh-openstack}, Kuryr now only creates Neutron networks and subnets for namespaces that have at least one pod on the pods network. Additionally, pools in a namespace are only populated after at least one pod on the pods network is created in the namespace.

[id="ocp-4-10-openstack-dcn-for-enterprise"]
==== Support for {rh-openstack} DCN (Technology Preview)

You can now deploy a cluster on a {rh-openstack-first} deployment that uses a link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/16.1/html/distributed_compute_node_and_storage_deployment/index[distributed compute node (DCN)] configuration. There are a number of limitations:
...
Clusters that run on {rh-openstack} now support external cloud providers. This capability is available as part of the `TechPreviewNoUpgrade` feature set.
...
* For clusters that run on {rh-openstack}, a bug in ovn-octavia-provider can cause load balancer listeners to be stuck in a `PENDING_UPDATE` state while the load balancer that they are attached to remains in an `ACTIVE` state. As a result, kuryr-controller can crash. To resolve this problem, update {rh-openstack} to a version that includes a fix as soon as possible. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2019980[*BZ#20004542*])
* For clusters that run on {rh-openstack} and use Kuryr, a bug in ovn-octavia-provider can cause load balancer listeners to be stuck in a `PENDING_UPDATE` state while the load balancer that they are attached to remains in an `ACTIVE` state. As a result, kuryr-controller can crash. To resolve this problem, update {rh-openstack} to a version that includes a fix as soon as possible. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2019980[*BZ#20004542*])
