...
In a bare metal cloud environment, applications must act on hardware changes and failures quickly with high reliability. OpenShift Container Platform now provides a hardware event proxy that enables such applications to respond to Redfish hardware events with low latency.

The hardware event proxy supports a publish-and-subscribe service through which hardware events detected by Redfish can be consumed by relevant applications. The proxy must be running on hardware that supports Redfish v1.8 and higher.
...
Note: This feature is only supported in this release in the context of Single Node OpenShift.
...
|Low-latency Redfish hardware event delivery
|-
|-
|TP
