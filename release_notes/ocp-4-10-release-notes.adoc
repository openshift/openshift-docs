...
For clusters that run on {rh-openstack}, Kuryr now only creates Neutron networks and subnets for namespaces that have at least one pod. Additionally, pools in a namespace are only populated after at least one pod is created in the namespace.
