...
* Previously, Ironic failed to attach virtual media images on SuperMicro X11/X12 servers due to overly restrictive URl validations on the BMCs of these machines. With this update, the `filename` parameter has now been removed from the URL if the virtual media image is backed by a local file. As a result, the parameter is still passed if the image is backed by an object store. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2011626[*BZ#2011626*])

* Previously, users had to enter a long form of an IPv6 address in the installation configuration file, for example `2001:0db8:85a3:0000:0000:8a2e:0370:7334`. Ironic could not find an interface matching this IP address. Consequently, the installation failed. With this update, the IPv6 address supplied by the user is converted to the short form, for example `2001:db8:85a3::8a2e:370:7334`. As a result, installation is now successful. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2010698[*BZ#2010698*])
...
* Previously, only some drivers supported `UEFISecureBoot` mode. With this update, validation has been added to the installation process to ensure that `UEFISecureBoot` mode is not used with bare metal drivers that do not support changing the boot status. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2011893[*BZ#2011893*])
