...
The tutorials walk new users through the web console and the {oc-first} interfaces. New users can accomplish the following tasks through the Getting Started:
...
However, {oc-first} 4.10 or later can be used to display additional upgrade paths for {product-title} 4.10 clusters.
...
This release introduces the oc-mirror {oc-first} plugin as a Technology Preview. You can use the oc-mirror plugin to mirror images in a disconnected environment.
...
==== Dynamic plugin (Technology Preview)
Starting with {product-title} {product-version}, the ability to create OpenShift console dynamic plugins is now available as a Technology Preview feature. You can use this feature to customize your interface at runtime in many ways, including:

* Adding custom pages
* Adding perspectives and updating navigation items
* Adding tabs and actions to resource pages

For more information about the dynamic plugin, see xref:../web_console/dynamic-plugins.adoc#dynamic-plugins_dynamic-plugins[Adding a dynamic plugin to the {product-title} web console].
...
[id="ocp-410-nw-network-observability-updates"]
...
==== Console Storage Plugin enhancement
...
Starting with {product-title} 4.8, automatic migration for in-tree volume plugins to their equivalent Container Storage Interface (CSI) drivers became available as a Technology Preview feature. This feature now supports automatic migration for the Azure File in-tree plugin to the Azure File CSI driver.
...
Starting with {product-title} 4.8, automatic migration for in-tree volume plugins to their equivalent Container Storage Interface (CSI) drivers became available as a Technology Preview feature. This feature now supports automatic migration for the vSphere in-tree plugin to the vSphere CSI driver.
...
Support for using {op-system-base-full} 7 with the {oc-first} is deprecated and will be removed in a future {product-title} release.
...
+
This update corrects the accepted label specification so that the plugin imports pod templates from `ConfigMap` and `ImageStream` objects that have the `jenkins-agent` label. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2034839[2034839])
...
* Previously, the image admission plugin did not check annotations inside deployment configuration templates. As a result, annotations inside deployment configuration templates could not be handled in replica controllers, and they were ignored. Now, the image admission plugin analyzes the template of deployment configurations. With this fix, the image admission plugin recognizes the annotations on the deployment configurations and on their templates. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2032589[*BZ#2032589*])
...
{product-title} release 4.10.50, which includes security updates, is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2023:0241[RHSA-2023:0241] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2023:0240[RHBA-2023:0240] advisory.
...
[id="ocp-4-10-50-bug-fixes"]
==== Bug fixes

* Previously, when rotating {rh-openstack-first} credentials, the Cinder Container Storage Interface driver would continue to use old credentials. Using any old credentials that were invalid would cause all volume operations to fail. With this update, the Cinder Container Storage Interface driver is updated automatically when the {rh-openstack-first} credentials are updated. (link:https://issues.redhat.com/browse/OCPBUGS-4717[*OCPBUGS-4717*])

* * Previously, pod failures were artificially extending the validity period of certificates causing them to incorrectly rotate. With this update, the certificate validity period is accurately determined, which helps certificates to rotate correctly. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2020484[*BZ#2020484*])
...
[id="ocp-4-10-51"]
=== RHSA-2023:0561 - {product-title} 4.10.51 bug fix and security update

Issued: 2023-02-08

{product-title} release 4.10.51, which includes security updates, is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2023:0561[RHSA-2023:0561] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2023:0560[RHSA-2023:0560] advisory.
...
[id="ocp-4-10-53"]
=== RHSA-2023:0698 - {product-title} 4.10.53 bug fix and security update

Issued: 2023-03-01

{product-title} release 4.10.53, which includes security updates, is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2023:0899[RHSA-2023:0899] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2023:0898[RHBA-2023:0898] advisory.
...
[id="ocp-4-10-53-bug-fixes"]
==== Bug fixes

* In order to be compatible with OpenStack clouds that do not have Swift installed, Cluster Image Registry Operator (CIRO) has a mechanism for automatically choosing the storage back-end during the first boot. If Swift is available, Swift is used. Otherwise, a persistent volume claim (PVC) is issued and block storage is used. Previously, the CIRO would fall back to using a PVC when it failed to reach Swift. In particular, a lack of connectivity during the first boot would make CIRO fall back to using a PVC. With this change, a failure to reach the OpenStack API, or other incidental failures, cause CIRO to retry the probe. The fallback to PVC occurs only if the OpenStack catalog is correctly found, and it does not contain object storage, or if the current user does not have permission to list containers. (link:https://issues.redhat.com/browse/OCPBUGS-5974[*OCPBUGS-5974*])

* Previously, the User Provisioned Infrastructure (UPI) did not create a server group for compute machines. {product-title} {product-version} updates the UPI script, so that the script creates a server group for compute machines. The UPI script installation method now aligns with the installer-provisioned installation (IPI) method.
 (link:https://issues.redhat.com/browse/OCPBUGS-2731[*OCPBUGS-2731*])

[id="ocp-4-10-53-upgrading"]
==== Updating

To update an existing {product-title} 4.10 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc#updating-cluster-cli[Updating a cluster within a minor version by using the CLI] for instructions.

[id="ocp-4-10-54"]
=== RHSA-2023:1154 - {product-title} 4.10.54 bug fix and security update

Issued: 2023-03-15

{product-title} release 4.10.54, which includes security updates, is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2023:1154[RHSA-2023:1154] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2023:1153[RHBA-2023:1153] advisory.
...
[id="ocp-4-10-54-bug-fixes"]
==== Bug fixes

* Previously, when editing any pipeline in the {product-title} console, the correct data was not rendered in the *Pipeline builder* and *YAML view* configuration options, which prevented you from editing the pipeline in the *Pipeline builder*. With this update, data is parsed correctly and you can edit the pipeline using the builder. (link:https://issues.redhat.com/browse/OCPBUGS-7657[*OCPBUGS-7657*])

[id="ocp-4-10-54-upgrading"]
==== Updating

To update an existing {product-title} 4.10 cluster to this latest release, see xref:../updating/updating-cluster-cli.adoc#updating-cluster-cli[Updating a cluster within a minor version by using the CLI] for instructions.

[id="ocp-4-10-55"]
=== RHSA-2023:1392 - {product-title} 4.10.55 bug fix and security update

Issued: 2023-03-29

{product-title} release 4.10.55, which includes security updates, is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2023:1392[RHSA-2023:1392] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2023:1391[RHBA-2023:1391] advisory.
...
{product-title} release 4.10.58 is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2023:1867[RHBA-2023:1867] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2023:1866[RHSA-2023:1866] advisory.
...
[id="ocp-4-10-63"]
=== RHSA-2023:3911 - {product-title} 4.10.63 bug fix and security update

Issued: 2023-07-06

{product-title} release 4.10.63, which includes security updates, is now available. This update includes a Red Hat security bulletin for customers who run {product-title} in FIPS mode. For more information, see link:https://access.redhat.com/security/vulnerabilities/RHSB-2023-001[RHSB-2023:001].

Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHSA-2023:3911[RHSA-2023:3911] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2023:3910[RHSA-2023:3910] advisory.
...
[id="ocp-4-10-65"]
=== RHBA-2023:4445 - {product-title} 4.10.65 bug fix update

Issued: 2023-08-09

{product-title} release 4.10.65 is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2023:4445[RHBA-2023:4445] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2023:4447[RHBA-2023:4447] advisory.
...
[id="ocp-4-10-67"]
=== RHBA-2023:4896 - {product-title} 4.10.67 bug fix and security update

Issued: 2023-09-06

{product-title} release 4.10.67, which includes security updates, is now available. Bug fixes included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2023:4896[RHBA-2023:4896] advisory. RPM packages included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2023:4898[RHSA-2023:4898] advisory.
