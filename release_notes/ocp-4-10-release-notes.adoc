...
[id="ocp-4-10-44"]
=== RHBA-2022:8702 - {product-title} 4.10.44 bug fix update

Issued: 2022-12-06

{product-title} release 4.10.44 is now available. The bug fixes that are included in the update are listed in the link:https://access.redhat.com/errata/RHBA-2022:8702[RHBA-2022:8702] advisory. There are no RPM packages for this release.

You can view the container images in this release by running the following command:

[source,terminal]
----
$ oc adm release info 4.10.44 --pullspecs
----

[id="ocp-4-10-44-enhancements"]
==== Enhancements

* IPv6 unsolicited neighbor advertisements and IPv4 gratuitous address resolution protocol are now default on the SR-IOV CNI plug-in. Pods created with the Single Root I/O Virtualization (SR-IOV) CNI plug-in, where the IP address management CNI plug-in has assigned IP addresses, now send IPv6 unsolicited neighbor advertisements, IPv4 gratuitous address resolution protocol, or both by default onto the network. This enhancement notifies hosts of the new pod's MAC address for a particular IP address to refresh ARP and NDP caches with the correct information. For more information, see xref:../networking/hardware_networks/about-sriov.adoc#supported-devices_about-sriov[Supported devices].

[id="ocp-4-10-44-bug-fixes"]
==== Bug fixes

* Previously, object storage instances responded with `204 No Content` when there was no content to display. The {rh-openstack-first} SDK used in {product-title} did not handle 204 messages correctly. With this update, the installation program works around the issue when there are zero items to list. (link:https://issues.redhat.com/browse/OCPBUGS-4160[OCPBUGS-4160])

[id="ocp-4-10-44-upgrading"]
==== Updating
