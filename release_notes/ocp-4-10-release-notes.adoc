...
It is now possible to synchronize user groups from OIDC identity providers. To configure group synchronization, set the `oauth.spec.identityProviders.openID.claims.groups` field in
your identity providers' configuration in the `oauth.config.openshift.io/cluster` configuration object. The groups for a specific user get synchronized upon the user's login.
