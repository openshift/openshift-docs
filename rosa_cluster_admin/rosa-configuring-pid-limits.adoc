...
[role="_abstract"]
A process identifier (PID) is a unique identifier assigned by the Linux kernel to each process or thread running on a system. You can configure PID limits for your {product-title} cluster to control how many processes can run simultaneously.
