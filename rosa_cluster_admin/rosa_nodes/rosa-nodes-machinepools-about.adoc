...
In {product-title} clusters, the hosted control plane spans multiple availability zones (AZ) in the installed cloud region. Each machine pool in a {product-title} cluster deploys in a single subnet within a single AZ. ???Each of these AZs can have only one machine pool.??
