...
{product-title-managed-short} is hosted on Amazon Web Services (AWS) over two regions (eu-west-1, us-east-1) and uses the network access points provided by the cloud provider.
Each tenant from the Cloud Service uses highly-available egress proxies and is spread over 3 availability zones. For more information about {product-title-managed-short} system architecture and components, see xref:../architecture/acscs-architecture.adoc#acscs-architecture[{rh-rhacscs-first} architecture].
...
{product-title-managed-short} uses certificate-based architecture (mTLS) for both authentication and end-to-end encryption of all inflight traffic between the customer's site and Red Hat. It does not require a VPN. IP allowlists are not supported. Data transfer is encrypted using mTLS. File transfer, including Secure FTP, is not supported.
...
* link:https://access.redhat.com/security_testing_sdl_practice[Red Hat Software Development Life Cycle practices]
...
The default identity provider in {product-title-managed-short} is Red Hat Single Sign-On (SSO). Authorization rules are set up to provide administrator access to the user who created the {product-title-managed-short} and to users who are marked as organization administrators in Red Hat SSO. The `admin` login is disabled for {product-title-managed-short} by default and can only be enabled temporarily by SREs. For more information about authentication using Red Hat SSO, see xref:../cloud_service/getting-started-rhacs-cloud-ocp.adoc#default-access-acs-console[Default access to ACS Console].

[id="passwords_{context}"]
=== Password management

Red Hat's password policy requires the use of a complex password.
...
Red Hat follows best password practices according to link:https://pages.nist.gov/800-63-3/sp800-63b.html[NIST guidelines].
...
To prevent information leakage, Logs are shipped to SRE through our SIEM, Splunk.
...
include::modules/cloud-getting-support.adoc[leveloffset=+2]

include::modules/cloud-service-removal.adoc[leveloffset=+2]
...
//privacy policy located on bottom of every page on the web site, no need to reiterate here