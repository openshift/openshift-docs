...
* **Workload Identity Federation (WIF) authentication type is now available.** {product-title} on {gcp-full} customers can now use WIF as an authentication type when creating a cluster. WIF is a GCP Identity and Access Management (IAM) feature that provides third parties a secure method to access resources on a customer's cloud account.
