...
(See  https://docs.openshift.com/container-platform/4.4/registry/configuring-registry-operator.html[Image Registry Operator in OpenShift Container Platform].)

* SR-IOV Operator
(See https://docs.openshift.com/container-platform/4.4/networking/hardware_networks/about-sriov.html[About SR-IOV hardware on OpenShift Container Platform].)

* Performance AddOn Operator
(See https://docs.openshift.com/container-platform-ocp/4.4/scalability_and_performance/cnf-performance-addon-operator-for-low-latency-nodes.html[About CNF Performance addon operator].)
...
We spawn the `testpmd` interactive terminal then start a test workload and close it after 20 seconds.
...
and starts a build.
(Internal registry must be configured in the cluster).
...
You should create a performance profile
(See [https://docs.openshift.com/container-platform-ocp/4.4/scalability_and_performance/cnf-performance-addon-operator-for-low-latency-nodes.html#cnf-creating-the-performance-profile-object_cnf-master]) To allocate Hugepages and isolated CPUs.

Then you should create both SR-IOV network policy and SR-IOV network attachment
(See https://docs.openshift.com/container-platform-ocp/4.4/networking/hardware_networks/using-dpdk-and-rdma.html[using-dpdk-and-rdma]) based on your network card type.

Using a deployment config resource instead of a regular deployment will allow automatic redeploy the workload whenever a new image is built.
You will need to create a special `SecurityContextConstraints` resource that will allow the `deployer` service account to create the
...
<3> The requested resource. The limit and request should be the same so the QOS will be guaranteed and the CPUs will be pinned.
