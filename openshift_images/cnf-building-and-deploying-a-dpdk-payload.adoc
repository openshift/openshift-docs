...
For more information, see https://doc.dpdk.org/guides/testpmd_app_ug/[Testpmd Application User Guide].
...
. Create a performance profile to allocate Hugepages and isolated CPUs. For more information,
see xref:../modules/cnf-tuning-nodes-for-low-latency-via-performanceprofile.adoc[Tuning nodes for low latency via PerformanceProfile].
