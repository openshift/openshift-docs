[id="overview-of-images"]
= Overview of images
include::modules/common-attributes.adoc[]
:context: overview-of-images

toc::[]

To know all about xref:../openshift_images/images-understand.adoc#understanding-images[containerized software], you must know the various terminologies:

** Containers
** Images
** Image streams

[discrete]
== How you can use the Cluster Samplers Operator

During the initial start-up, the operator creates the default samples resource to initiate the creation of the image streams and templates.
The Cluster Samples Operator manages the sample image streams and templates stored in the `openshift` namespace.

As a cluster administrator, you can use the Cluster Samplers Operator to:

** xref:../openshift_images/configuring-samples-operator.adoc#configuring-samples-operator[Configure the operator].
** xref:../openshift_images/samples-operator-alt-registry.adoc#samples-operator-alt-registry[Use the operator with an alternate registry].

[discrete]
== What you can do with images

A container image is the binary data that encapsulates an application and all its software dependencies.
A container image is an executable software bundle that runs standalone.
You can xref:../openshift_images/create-images.adoc#creating-images[create], xref:../openshift_images/managing_images/managing-images-overview.adoc#managing-images-overview[manage], and xref:../openshift_images/using_images/using-images-overview.adoc#using-images-overview[use] container images.

[discrete]
== What you can do with images streams

An image stream is a single pointer to a set of related images.
A single image stream can contain many different tags such as `latest`, `develop`, and more.
Each tag points to an image in a registry.

You can use image streams to:

** xref:../openshift_images/image-streams-manage.adoc#managing-image-streams[Manage].
** xref:../openshift_images/using-imagestreams-with-kube-resources.adoc#using-imagestreams-with-kube-resources[use with Kubernetes resources].
** xref:../openshift_images/triggering-updates-on-imagestream-changes.adoc#triggering-updates-on-imagestream-changes[Trigger updates on image stream updates].

[discrete]
== How you can use templates

A template is a definition of an object to be replicated.
You can use xref:../openshift_images/using-templates.adoc#using-templates[templates] to build and deploy configurations.

[discrete]
== How you can use Ruby on Rails

As a developer, you can use xref:../openshift_images/templates-using-ruby-on-rails.adoc#templates-using-ruby-on-rails[Ruby on Rails] to:

** Write your application.
** Set up a database.
** Create a welcome page.
** Configure your application for {product-title}.
** Store your application in Git.
** Deploy your application in {product-title}.
