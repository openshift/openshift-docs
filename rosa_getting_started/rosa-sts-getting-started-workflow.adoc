...
//
. xref:../rosa_planning/rosa-hcp-prereqs.adoc#rosa-hcp-prereqs[Complete the AWS prerequisites for ROSA clusters] or  xref:../rosa_planning/rosa-classic-prereqs.adoc#rosa-classic-prereqs[Complete the AWS prerequisites for ROSA (classic architecture) clusters]. To deploy a ROSA cluster with STS, your AWS account must meet the prerequisite requirements.
