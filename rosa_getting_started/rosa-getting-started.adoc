...
Choose from one of the following methods to deploy a {product-title} (ROSA) cluster that uses the AWS Security Token Service (STS). In both scenarios, you can deploy your cluster through the ROSA CLI (`rosa`) or {cluster-manager-first}:

* *xref:../rosa_install_access_delete_clusters/rosa-sts-creating-a-cluster-quickly.adoc#rosa-sts-creating-a-cluster-quickly[Creating a ROSA cluster with STS quickly]*: You can create a ROSA cluster with STS quickly by using the default options and automatic STS resource creation.
