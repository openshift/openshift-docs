:_mod-docs-content-type: ASSEMBLY
include::_attributes/common-attributes.adoc[]
[id="using-argo-rollouts-for-progressive-deployment-delivery"]
= Using Argo Rollouts for progressive deployment delivery
:context: using-argo-rollouts-for-progressive-deployment-delivery

toc::[]

To use Argo Rollouts and manage progressive delivery, after you install the {gitops-titel} Operator on the cluster, you can create and configure a `RolloutManager` custom resource (CR) instance in the namespace of your choice. You can scope the `RolloutManager` CR for single or multiple namespaces. 

[id="prerequisites_using-argo-rollouts-for-progressive-deployment-delivery_{context}"]
== Prerequisites
* You have access to the cluster with `cluster-admin` privileges.
* You have access to the {OCP} web console.
* {gitops-title} 1.9.0 or a newer version is installed on your cluster.

// Creating a RolloutManager custom resource
include::modules/gitops-creating-rolloutmanager-custom-resource.adoc[leveloffset=+1]

// Deleting a RolloutManager custom resource
include::modules/gitops-deleting-rolloutmanager-custom-resource.adoc[leveloffset=+1]

// Installing Argo Rollouts CLI on Linux
include::modules/gitops-installing-argo-rollouts-cli-on-linux.adoc[leveloffset=+1]

// Installing Argo Rollouts CLI on Mac OS
include::modules/gitops-installing-argo-rollouts-cli-on-mac-os.adoc[leveloffset=+1]

[role="_additional-resources"]
[id="additional-resources_using-argo-rollouts-for-progressive-deployment-delivery_{context}"]
== Additional resources
* xref:../argo_rollouts/getting-started-with-argo-rollouts.adoc#getting-started-with-argo-rollouts[Getting started with Argo Rollouts]
* xref:../installing_gitops/installing-openshift-gitops.adoc#installing-openshift-gitops[Installing {gitops-title}]
* xref:../removing_gitops/uninstalling-openshift-gitops.adoc#uninstalling-openshift-gitops[Uninstalling {gitops-title}]
* link:https://argo-rollouts-manager.readthedocs.io/en/latest/crd_reference/[`RolloutManager` Custom Resource specification]
* link:https://cloud.redhat.com/blog/trying-out-argo-rollouts-in-openshift-gitops-1.9/[Argo Rollouts tech preview limitations]