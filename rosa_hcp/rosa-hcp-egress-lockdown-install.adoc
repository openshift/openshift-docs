...
Zero-egress clusters use an Amazon ECR to create a highly secure network architecture that blocks outbound traffic from the cluster to the public internet. Because the necessary cluster communication occurs over private network channels, Amazon ECR serves as the cornerstone by storing and gathering the necessary images. For more on Amazon ECR, see link:https://aws.amazon.com/ecr/[Amazon Elastic Container Registry].
