...
Customers can use their own Container Network Interface (CNI) plugin when creating a {hcp-title-first} cluster.
You can create a {hcp-title} cluster without a CNI to bring your own CNI plugin and install it after cluster creation.

[NOTE]
====
For clusters in this configuration, the responsibility of CNI plugin support belongs to the customer in coordination with their chosen CNI vendor.
====
