...
ifdef::openshift-rosa[]
xref:../cli_reference/rosa_cli/rosa-cli-permission-examples.adoc#rosa-cli-hcp-examples_rosa-cli-permission-examples[Least privilege permissions for common ROSA CLI commands]
endif::openshift-rosa[]
ifdef::openshift-rosa-hcp[]
endif::openshift-rosa-hcp[]
...
. Install the CLI from xref:../rosa_install_access_delete_clusters/rosa_getting_started_iam/rosa-installing-rosa.html[Installing the Red{nbsp}Hat OpenShift Service on AWS (ROSA) CLI, rosa] or from the OpenShift link:https://console.redhat.com/openshift/downloads#tool-rosa[console].
...
* Ensure that your AWS account has sufficient quota to deploy a cluster.
+
[source,terminal]
----
$ rosa verify quota
----
+
This command only checks the total quota allocated to your account; it does not reflect the amount of quota already consumed from that quota.
