...
* Ensure that your AWS account has sufficient permissions to use the ROSA CLI: link:https://docs.openshift.com/rosa/cli_reference/rosa_cli/rosa-cli-permission-examples.html#rosa-cli-permission-examples[Least privilege permissions for common ROSA CLI commands]
...
. Download and install`oc` from the {cluster-manager} link:https://console.redhat.com/openshift/downloads#tool-oc[Command-line interface (CLI) tools] page, or follow the instructions in link:https://docs.openshift.com/rosa/cli_reference/openshift_cli/getting-started-cli.html#cli-getting-started[Getting started with the OpenShift CLI].
