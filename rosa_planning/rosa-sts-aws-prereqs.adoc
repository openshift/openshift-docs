...
The following prerequisites must be complete before you deploy a 
ifdef::openshift-rosa[]
{product-title} (ROSA) cluster that uses the AWS Security Token Service (STS).
endif::openshift-rosa[]
ifdef::openshift-rosa-hcp[]
{hcp-title} cluster.
endif::openshift-rosa-hcp[]
...
//TODO OSDOCS-11789: Nothing in the following module is actually a requirement, it's purely informative/recommended
