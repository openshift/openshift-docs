...
<<<<<<< HEAD
* Ensure that you address all `Upgradeable=False` conditions so the cluster allows an upgrade to the next minor version. You can run the `oc adm upgrade` command for an output of all `Upgradeable=False` conditions and the condition reasoning to help you prepare for a minor version update.
=======
...
>>>>>>> 7ddcd75138 (BZ1978420: Add a PodDisruptionBudget related bullet in prerequisites)
