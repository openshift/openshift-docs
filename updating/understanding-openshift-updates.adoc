...
With {product-title} 4, you can update a {product-title} cluster with a single operation by using the web console or the {oc-first}. Platform administrators are automatically notified when an update is available for their cluster.
