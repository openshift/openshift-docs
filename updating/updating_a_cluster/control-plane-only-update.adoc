...
However, administrators who want to update between two even-numbered minor versions or three consecutive versions can do so incurring only a single reboot of non-control plane hosts.

[IMPORTANT]
====
This update was previously known as an *EUS-to-EUS* update and is now referred to as a *Control Plane Only* update. <4.y+2> updates are only viable between *even-numbered minor versions* of {product-title}.
====
...
* Until the machine config pools are unpaused and the update is complete, some features and bugs fixes in <4.y+1>, <4.y+2>, and <4.y+3> of {product-title} are not available.
