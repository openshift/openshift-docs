...
However, beginning with the update from {product-title} 4.8 to 4.9 to 4.10, administrators who wish to update between two Extended Update Support (EUS) versions can do so incurring only a single reboot of non-master hosts. For more information, see link:https://access.redhat.com/labs/ocpupgradegraph/update_path[Red Hat OpenShift Container Platform Update Graph].
...
* EUS-to-EUS updates are not applicable to compact three-node clusters and Single Node Openshift (SNO) topologies because all of the hosts in these configurations are control plane hosts.
