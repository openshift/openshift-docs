:_mod-docs-content-type: ASSEMBLY
[id="updating-cluster-prepare"]
= Preparing to update to {product-title} {product-version}
include::_attributes/common-attributes.adoc[]
:context: updating-cluster-prepare

toc::[]

////
WARNING: This assembly has been moved into a subdirectory for 4.14+. Changes to this assembly for earlier versions should be done in separate PRs based off of their respective version branches. Otherwise, your cherry picks may fail.

To do: Remove this comment once 4.13 docs are EOL.
////

[role="_abstract"]
Before you update your {product-title} cluster, complete the required administrative tasks and review best practices to minimize disruption and avoid update failures.

[id="kube-api-removals_{context}"]
== Kubernetes API removals

There are no Kubernetes API removals in this release.

ifndef::openshift-rosa,openshift-dedicated,openshift-rosa-hcp,openshift-origin[]
include::modules/about-self-service-tsr.adoc[leveloffset=+1]
endif::openshift-rosa,openshift-dedicated,openshift-rosa-hcp,openshift-origin[]

include::modules/update-assessment-ai.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources

* link:https://docs.redhat.com/en/documentation/red_hat_openshift_lightspeed/1.0[{ols-official} documentation]

// Assessing the risk of conditional updates
include::modules/update-preparing-conditional.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources
* xref:../../updating/understanding_updates/how-updates-work.adoc#update-evaluate-availability_how-updates-work[Evaluation of update availability]

// etcd backups before cluster updates
include::modules/update-etcd-backup.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources

* xref:../../backup_and_restore/control_plane_backup_and_restore/backing-up-etcd.adoc#backup-etcd[Backing up etcd]

* xref:../../backup_and_restore/control_plane_backup_and_restore/disaster_recovery/scenario-2-restoring-cluster-state.adoc#dr-restoring-cluster-state[Restoring to an earlier cluster state]

include::modules/oc-adm-upgrade-recommend.adoc[leveloffset=+1]

include::modules/oc-adm-upgrade-recommend-custom-alert.adoc[leveloffset=+2]

include::modules/oc-adm-upgrade-recommend-accept.adoc[leveloffset=+2]

// Best practices for cluster updates
include::modules/update-best-practices.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources
* xref:../../updating/understanding_updates/intro-to-updates.adoc#understanding_clusteroperator_conditiontypes_understanding-openshift-updates[Understanding cluster Operator condition types]

// HAProxy version pinning before cluster updates
include::modules/update-preparing-haproxy-version.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources
* xref:../../networking/networking_operators/ingress-operator.adoc#nw-about-haproxy-versions_configuring-ingress[About HAProxy versions for Ingress Controllers]
* xref:../../networking/networking_operators/ingress-operator.adoc#nw-selecting-haproxy-version_configuring-ingress[Selecting an HAProxy version]
* xref:../../networking/networking_operators/ingress-operator.adoc#nw-verifying-haproxy-version_configuring-ingress[Verifying the active HAProxy version]
* xref:../../networking/networking_operators/ingress-operator.adoc#nw-ingress-controller-configuration-parameters_configuring-ingress[Ingress Controller configuration parameters]

// Minimizing worker node deployment time
include::modules/minimizing-worker-node-deployment-time.adoc[leveloffset=+1]
