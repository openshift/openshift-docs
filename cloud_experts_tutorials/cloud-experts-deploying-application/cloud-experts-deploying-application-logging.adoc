...
There are various methods to view your logs in {product-rosa} (ROSA). Use the following procedures to forward the logs to AWS CloudWatch, view the logs directly through the pod by using `oc logs`, and use LokiStack to search the logs.
...
The cluster logging components are based on Fluentd and Elasticsearch (ES). Fluentd is deployed to each node in the cluster. It collects application logs and either writes them to ES or forwards them to CloudWatch.
