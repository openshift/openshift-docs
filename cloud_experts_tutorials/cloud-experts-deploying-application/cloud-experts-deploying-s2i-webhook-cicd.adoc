...
. In your terminal, run the following command to get the GitHub webhook trigger secret:
+
[source,terminal]
----
...
. Run the following command to get the GitHub webhook trigger URL from the OSToy's buildconfig:
+
[source,terminal]
----
...
. Now, make a change in the source code. Any changes automatically trigger a build and deployment. In this example, the colors that denote the status of your OSToy app are selected randomly. Change the box to only display grayscale.
+
