...
Although the OSToy application functions independently, many real-world applications require external services like databases, object stores, or messaging services.

.Objectives

* Learn how to integrate the OSToy application with other Amazon Web Services (AWS) services, specifically AWS S3 Storage. By the end of this section, the application will securely create and read objects from S3.
...
The link:https://aws-controllers-k8s.github.io/community/docs/community/overview/[Amazon Controller for Kubernetes](ACK) allows you to create and use AWS services directly from
Kubernetes. You can deploy your applications, including any required AWS services, directly in the Kubernetes framework using a familiar structure to declaratively define and create AWS services such as S3 buckets or RDS databases.

With ACK, you can create an S3 bucket, integrate it with OSToy, upload a file to it, and view the file in your application.

== IAM roles for service accounts

IAM roles for service accounts is an AWS feature that allows you to assign IAM roles directly to Kubernetes service accounts. You can use it to grant the ACK controller credentials to deploy services in your AWS account. Use link:https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[IAM roles for service accounts] to automate the management and rotation of temporary credentials.

Pods receive a valid OpenID Connect (OIDC) JSON web token (JWT) and pass it to the AWS STS `AssumeRoleWithWebIdentity` API operation in order to receive IAM temporary role credentials. The process relies on the EKS pod identity mutating webhook which modifies pods that require AWS IAM access.
...
. Select *Update approval* under "Manual".
+
[IMPORTANT]
====
...
** Use the link:resources/setup-s3-ack-controller.sh[setup-s3-ack-controller.sh] script, which automates the process for you.
...
----
+
. Confirm successful setup of the ACK controller in the {hybrid-console-second} by clicking *Operators* and then *Installed operators*.
+
...
----
+
. Confirm that the previous step was successful by running the following command:
+
[source,terminal]
----
...
. Create an S3 bucket using a manifest file by using one of the following options:
...
----
+
** Download the link:yaml/s3-bucket.yaml[command], then run it.
+
[IMPORTANT]
====
...
When AWS clients detect `AssumeRoleWithWebIdentity` security tokens to assume the IAM role to connect to the AWS APIs. See the link:https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRoleWithWebIdentity.html[AWS AssumeRoleWithWebIdentity] documentation for more details.
...
. Make sure that the page that lists the contents of the bucket is empty.
+
...
+
. Scroll to the top section for existing files and confirm that the file you just create is there.
. Click the file name to view the file.
+
image:cloud-experts-deploying-integrating-ack-viewobj.png[]
+
. Confirm with the AWS CLI by run the following command to list the contents of your bucket:
+
[source,terminal]
----
