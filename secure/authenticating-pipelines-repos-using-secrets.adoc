...
You can configure a named workspace in a task, specifying a path where the workspace is mounted. When you run the task, use the secret as the workspace with this name. When {pipelines-shortname} executes the task, the information in the secret is available to the task.
