...
In most cases, creating a custom task for building the image and configuring user namespaces in this task enables running Buildah without root privileges.

If your image does not build successfully using this configuration, you can define custom service account (SA) and security context constraint (SCC); however, in this option, you must enable the Buildah step to raise its privileges (`allowPrivilegeEscalation: true`).
