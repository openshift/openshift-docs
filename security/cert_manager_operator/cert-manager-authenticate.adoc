...
[role="_abstract"]
To enable the {cert-manager-operator} to manage components on your cloud provider, authenticate the Operator by configuring cloud credentials. You can grant the Operator access to external services required for certificate issuance, such as DNS providers.
