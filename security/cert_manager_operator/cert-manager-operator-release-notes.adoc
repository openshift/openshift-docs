...
Version `1.15.0` of the {cert-manager-operator} is based on the upstream cert-manager version `v1.14.5`. For more information, see the link:https://cert-manager.io/docs/releases/release-notes/release-notes-1.15/[cert-manager project release notes for v1.15].

*Scheduling Overrides for {cert-manager-operator}*

With this release, you can configure scheduling overrides for  {cert-manager-operator}, including the controller, webhook, and CA injector.
...
*Integrating Istio-CSR with {cert-manager-operator} [Technology Preview]*

With this release, the {cert-manager-operator} introduces support for integrating `Istio-CSR`, enabling seamless certificate provisioning and lifecycle management for `Istio-csr` service mesh environments. You can now configure {cert-manager-operator} as the certificate authority for `Istio-csr`.
