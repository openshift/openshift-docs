...
The following advisory is available for the {cert-manager-operator} 1.14:

* link:https://errata.engineering.redhat.com/advisory/128547[RHEA-2024:128547-02]


Version `1.14.0` of the {cert-manager-operator} is based on the upstream cert-manager version `v1.14.5`. For more information, see the link:https://cert-manager.io/docs/releases/release-notes/release-notes-1.14[cert-manager project release notes for v1.14].
...
The {cert-manager-operator} now supports the NCM issuer. The `ncm-issuer` is a cert-manager External Issuer that integrates with the Nokia NetGuard Certificate Manager (NCM) PKI system using a Kubernetes controller to sign certificate requests. This integration streamlines the process of obtaining non-self-signed certificates for applications, ensuring their validity and keeping them up-to-date.
