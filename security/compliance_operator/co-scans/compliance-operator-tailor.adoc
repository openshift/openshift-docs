:_mod-docs-content-type: ASSEMBLY
[id="compliance-operator-tailor"]
= Tailoring the Compliance Operator
include::_attributes/common-attributes.adoc[]
:context: compliance-tailor

toc::[]

[role="_abstract"]
While the Compliance Operator includes ready-to-use profiles, they must be modified to fit the organization needs and requirements. The process of modifying a profile is called _tailoring_.

The Compliance Operator provides the `TailoredProfile` object to help tailor profiles.

include::modules/compliance-new-tailored-profiles.adoc[leveloffset=+1]

include::modules/compliance-tailored-profiles.adoc[leveloffset=+1]
