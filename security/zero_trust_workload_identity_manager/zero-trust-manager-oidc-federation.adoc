...
[role="_abstract"]
Configure {zero-trust-full} to act as an OpenID Connect (OIDC) provider through the SPIRE server. This integration allows workloads to receive verifiable JSON Web Tokens (JWT-SVIDs) and allows external systems, such as cloud providers, to retrieve public keys from the discovery endpoint.
