...
The MCS CA and signing key appear in the cluster as a config map in the `openshift-machine-config-operator` namespace called `machine-config-server-ca`. The config map contains the `ca.crt` key. The private key is not stored in the cluster and is discarded after the installation completes. The config map is used in the following ways:
...
The MCS cert appears in the cluster as a secret in the `openshift-machine-config-operator` namespace called `machine-config-server-tls`. The secret contains the `tls.crt` and `tls.key` keys.
...
== Expiration
The MCS CA and MSC cert are valid for 10 years and are automatically rotated by the MCO at 8 years.
