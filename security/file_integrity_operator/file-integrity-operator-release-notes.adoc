...
* link:https://access.redhat.com/errata/RH??-2025:????[RH??-2025:???? OpenShift File Integrity Operator Bug Fix Update]

[id="file-integrity-operator-1-3-6-bug-fixes"]
=== Bug fixes

* Previously, FIO would re-initialize on failure for all nodes in the cluster. With this release, FIO has been updated to re-initialize the database on failure nodes only.  (link:https://issues.redhat.com/browse/OCPBUGS-18933[*OCPBUGS-18933*])

* Previously, after a reset of FIO, the metric file_integrity_operator_node_failed would be reset, which would cause an existing alert `NodeHasIntegrityFailure` to disappear. With this release, `NodeHasIntegrityFailure` is preserved after an FIO restart. (link:https://issues.redhat.com/browse/OCPBUGS-42807[*OCPBUGS-42807*])

* Previously, when a new node is added to a cluster by scaling up the `machineset`, FIO would mark the newly added node as `Failed`. With this release FIO marks the newly added node as `Succeeded`. (link:https://issues.redhat.com/browse/OCPBUGS-36483[*OCPBUGS-36483*])

* Previously, the `aide-openshift-file-integrity-daemon` pod would constantly force-initialize the AIDE database. With this release, FIO initializes the AIDE database only once. (link:https://issues.redhat.com/browse/OCPBUGS-37300[*OCPBUGS-37300*])
