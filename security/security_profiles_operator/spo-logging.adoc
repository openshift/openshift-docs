...
[NOTE]
====
The Security Profiles Operator supports only {op-system-first} (RHCOS) 10 worker nodes.
...
For node debugging sessions:
	...
	* The supported Linux used with the Advanced Audit Logging Framework is {op-system-first} 10 running in a container in {product-title} 4.20 or later.
...
include::modules/spo-logging-enrich.adoc[leveloffset=+1]

include::modules/spo-logging-enabling.adoc[leveloffset=+1]

include::modules/spo-logging-debugging.adoc[leveloffset=+1]

include::modules/spo-logging-correlate.adoc[leveloffset=+2]

include::modules/spo-logging-disable.adoc[leveloffset=+3]
