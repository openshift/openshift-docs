...
[IMPORTANT]
====
{product-title} defaults to using an in-tree, or non-CSI, driver to provision AWS EBS storage. For {product-title}, automatic migration from AWS EBS in-tree to the CSI driver is available as a Technology Preview (TP) feature.
With migration enabled, volumes provisioned using the existing in-tree driver are automatically migrated to use the AWS EBS CSI driver. For more information, see xref:persistent-storage-csi-migration.adoc#persistent-storage-csi-migration[CSI automatic migration feature].
====
