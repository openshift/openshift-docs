...
`PodDisruptionBudget` is an object that specifies the minimum percentage of
...
but it should not be the same number as the `ReplicationController` otherwise
the node cannot be evacuated normally and the evacuation needs to be forced.

[NOTE]
====
Depending upon your xref:../../admin_guide/scheduling/priority_preemption.adoc#admin-guide-priority-preemption[pod priority and preemption settings], lower-priority pods 
might be removed despite their pod disruption budget requirements.
====
