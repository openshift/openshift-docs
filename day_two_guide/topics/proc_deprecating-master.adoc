...
[IMPORTANT]
====
In {product-title} {product-version} clusters running multiple masters, one of
the master nodes includes additional CA certificates in `/etc/origin/master`,
`/etc/etcd/ca`, and `/etc/etcd/generated_certs`. These are required for
application node and etcd node scale-up operations and must be restored on
another master node if the CA host master is being deprecated.
====
