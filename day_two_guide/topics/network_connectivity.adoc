...
*Determining internal and external master URLs*

The following commands can be used to determine the URLs that are used by the internal cluster and external clients. Example URLs are found in the previous example.

To determine the internal cluster URL:
----
$ grep masterURL /etc/origin/master/master-config.yaml
----

To determine the external client URL:
----
