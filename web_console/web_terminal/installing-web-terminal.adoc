...
[NOTE]
====
If the {web-terminal-op} is being installed manually by creating a `Subscription` resource, the `Subscription` must be named `web-terminal` for the terminal icon to appear in the masthead of the console.
====
