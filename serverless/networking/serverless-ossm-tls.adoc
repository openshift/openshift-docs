...
The following example shows how you can create a TLS key and certificates for a domain and subdomain.
...
The example certificate for this domain is used as a certificate authority (CA) that signs the sub-domain certificate.

To complete and verify this procedure in your deployment, you will need either a certificate signed by a widely trusted public CA, or a CA provided by your organization.

Example commands should be adjusted according to your domain, sub-domain and CA.
====
...
$ curl --cacert example.com.crt \
  --connect-to "custom-ksvc-domain.example.com:443:${INGRESS_ROUTER}:443" \
  https://custom-ksvc-domain.example.com
----
