...
{ServerlessProductName} is based on the open source link:https://knative.dev/[Knative] project, which provides portability and consistency across hybrid and multi-cloud environments by enabling an enterprise-grade serverless platform.
...
This improves developer productivity by removing the need for complex infrastructure set up before apps can be deployed.

Operational costs can be reduced by implementing out of the box {ServerlessProductName} solutions for automating operational tasks, such as scaling apps, by installing the {ServerlessProductName} Knative Serving and Eventing components.
