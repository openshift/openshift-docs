...
= Using kn to complete Knative Serving tasks
:context: serving-kn-commands
include::modules/common-attributes.adoc[]

toc::[]

The Knative `kn` CLI extends the functionality of the `oc` or `kubectl` CLI tools to integrate interaction with Knative components on {product-title}. `kn` allows developers to deploy and manage applications without editing YAML files directly.
