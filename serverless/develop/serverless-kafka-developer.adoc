...
[id="serverless-kafka-developer-channel"]
== Kafka channel

You can use Knative Kafka to create `KafkaChannel` objects, which are xref:../../serverless/discover/serverless-channels.adoc#serverless-channels[channels] that are backed by Apache Kafka topics.
