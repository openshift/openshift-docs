...
= Using an API server source
include::modules/common-attributes.adoc[]
:context: serverless-apiserversource

toc::[]

API server source is an event source that can be used to connect an event sink, such as a Knative service, to the Kubernetes API server.
An API server source watches for Kubernetes events and forwards them to the Knative Eventing broker.
