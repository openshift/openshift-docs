...
* For instructions on using the Kafka source `kn` plug-in, see xref:../event_sources/serverless-kafka-source.adoc#serverless-kafka-source[Creating a Kafka event source using the `kn` CLI].
