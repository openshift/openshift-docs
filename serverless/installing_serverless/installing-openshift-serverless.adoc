...
HA is enabled for some Knative Serving components by default. You can disable HA by following the documentation on xref:../../serverless/admin/serverless-HA.adoc#serverless-HA[Configuring high availability replicas on {ServerlessProductName}].
