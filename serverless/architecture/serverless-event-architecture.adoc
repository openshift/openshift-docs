...
Publish an event without creating a consumer:: You can send events to a Broker as an HTTP POST, and use a sink binding to decouple the destination configuration from your application that is producing events.
Consume an event without creating a publisher:: You can use a trigger to consume events from a broker based on event attributes. Your application receives events as an HTTP POST.
...
Addressable objects:: Able to receive and acknowledge an event delivered over HTTP to an address defined in the event `status.address.url` field. The Kubernetes Service object also satisfies the addressable interface.
Callable objects:: Able to receive an event delivered over HTTP and transform it, returning 0 or 1 new events in the HTTP response payload. These returned events can be further processed in the same way that events from an external event source are processed.
