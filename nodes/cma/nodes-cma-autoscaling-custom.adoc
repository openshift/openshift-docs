...
The Custom Metrics Autoscaler Operator generates and rotates self-signed certificates that encrypt the traffic between the custom metrics autoscaler components with mutual TLS. You can optionally use your own self-signed certificates by using the `caConfigMaps` parameter in the `KedaController` custom resource, as described in "Installing the custom metrics autoscaler".

The parameter points to a config map that contains PEM-encoded trusted certificate authorities (CAs). The files from the config map are loaded by the Custom Metrics Autoscaler Operator during start-up and is used by scalers to authenticate TLS-enabled metrics data sources.
