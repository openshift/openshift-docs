...
The Custom Metrics Autoscaler Operator generates and rotates self-signed certificates that encrypt the traffic between the custom metrics autoscaler components with mutual TLS. You can optionally use your own self-signed certificates by placing the certificates into one or more directories. Then, configure the `KedaController` custom resource to point to the directories, as described in "Installing the custom metrics autoscaler".
