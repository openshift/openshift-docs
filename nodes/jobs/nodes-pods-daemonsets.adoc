...
A daemon set ensures that all eligible nodes run a copy of a pod. Normally, the node that a pod runs on is selected by the Kubernetes scheduler. However, previously `DaemonSet` pods are created and scheduled by the daemon set controller. That introduces the following issues:
