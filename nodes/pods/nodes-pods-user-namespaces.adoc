...
By default, a container runs in the host system's root user namespace. Running a container in the root user namespace can be useful when the container needs a feature that is available only in the root namespace. However, running pods in the the root namespace introduces security concerns, such as the possibility of container breakouts, in which a process inside another container breaks out onto the host where the process can access or modify files on the host or in your containers. 
