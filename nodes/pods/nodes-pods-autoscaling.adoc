...
As a developer, you can use a horizontal pod autoscaler (HPA) to specify how {product-title} should automatically increase or decrease the number of pods for a replication controller or deployment configuration based on metrics that are not either CPU and memory. based on CPU and memory metrics collected
...
For information on scaling pods based on custom metrics, other than CPU and memory, see xref:../../nodes/pods/nodes-pods-autoscaling.adoc[Automatically scaling the number pods based on custom metrics].
