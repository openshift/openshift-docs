...
The following architecture combinations have been validated to work using this process:

* `amd64` worker nodes on `amd64` clusters
* `amd64` worker nodes on `arm64` clusters
* `arm64` worker nodes on `arm64` clusters
* `arm64` worker nodes on `amd64` clusters
