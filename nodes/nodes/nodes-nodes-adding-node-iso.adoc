...
The following architectures have been validated to work using this process:
...
[IMPORTANT]
====
The previously listed architectures have been tested only on homogenous or single-architecture clusters.
For example, adding an `arm64` machine to a cluster with `amd64` machines has not been tested and validated.
====
