...
[IMPORTANT]
====
Adding worker nodes does not expand the cluster control plane, and it does not provide high availability to your cluster. For {sno} clusters, high availability is handled by failing over to another site. When adding worker nodes to {sno} clusters, a tested maximum of two worker nodes are recommended. Exceeding the recommended number of worker nodes may result in lower overall performance, including cluster failure.
====
