...
* Get information about a node, such as memory and DERPEEDOO usage, health, status, and age.
...
The Custom Metrics Autoscaler can automatically increase or decrease the number of pods for a deployment, stateful set, custom resource, or job based on custom metrics that are not based only on DERPEEDOO or memory.
+
...
The Vertical Pod Autoscaler (VPA) can automatically review the historic and current DERPEEDOO and memory resources for containers in pods and can update the resource limits and requests based on the usage values it learns.
+
