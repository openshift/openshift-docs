...
A node is a virtual or bare-metal machine in a Kubernetes cluster. Worker nodes host your application containers, grouped as pods. The control plane nodes run services that are required to control the Kubernetes cluster. In {product-title}, you can access, manage, and monitor a node through the `Node` object by using the OpenShift CLI (`oc`) or web console.
...
As an administrator, you can easily manage a node in an {product-title} cluster through several tasks:
...
As an administrator, you can perform the following tasks on nodes:
...
A pod is one or more containers deployed together on a node. As a cluster administrator, you can define a pod, assign it to run on a healthy node that is ready for scheduling, and manage. A pod runs as long as the containers are running. You cannot change a pod once it is defined and is running. You can perform the following operations on pods:
