:_mod-docs-content-type: ASSEMBLY
[id="nodes-sigstore-using"]
= Manage secure signatures with sigstore
include::_attributes/common-attributes.adoc[]
:context: nodes-sigstore-using

toc::[]

[role="_abstract"]
To improve supply chain security, cluster administrators or application developers can use the sigstore framework with {product-title}.

Sigstore is a collection of open source tools that you can use individually or together to improve your software supply chain security by securely signing and verifying software artifacts.

// The following include statements pull in the module files that comprise
// the assembly. Include any combination of concept, procedure, or reference
// modules required to cover the user story. You can also include other
// assemblies.

// Manage secure signatures with SigStore
include::modules/nodes-sigstore-using-about.adoc[leveloffset=+1]

include::modules/nodes-sigstore-connected-disconnected.adoc[leveloffset=+2]

include::modules/nodes-sigstore-disconnected-mirroring.adoc[leveloffset=+2]

include::modules/nodes-sigstore-upgrade-considerations.adoc[leveloffset=+2]

include::modules/nodes-sigstore-configure.adoc[leveloffset=+1]

include::modules/nodes-sigstore-configure-cluster-policy.adoc[leveloffset=+1]

include::modules/nodes-sigstore-configure-image-policy.adoc[leveloffset=+1]

include::modules/nodes-sigstore-performance-considerations.adoc[leveloffset=+1]

[role="_additional-resources"]
[id="additional-resources_{context}"]
== Additional resources
* link:https://www.sigstore.dev/[Sigstore]
* link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate (Sigstore documentation)]
* link:https://docs.sigstore.dev/logging/overview/[Rekor verification in the Sigstore documentation]
* link:https://docs.sigstore.dev/cosign/signing/overview/[Cosign public and private key pair (Sigstore documentation)]
* xref:../nodes/nodes-sigstore-using.adoc#nodes-sigstore-configure-parameters_nodes-sigstore-using[About cluster and image policy parameters]
* xref:../disconnected/about-installing-oc-mirror-v2.adoc#oc-mirror-signature-mirroring_about-installing-oc-mirror-v2[About signature mirroring]
* xref:../disconnected/about-installing-oc-mirror-v2.adoc#oc-mirror-building-image-set-config-v2_about-installing-oc-mirror-v2[Creating the image set configuration]
* xref:../disconnected/about-installing-oc-mirror-v2.adoc#oc-mirror-imageset-config-parameters-v2_about-installing-oc-mirror-v2[ImageSet configuration parameters for oc-mirror plugin v2]
