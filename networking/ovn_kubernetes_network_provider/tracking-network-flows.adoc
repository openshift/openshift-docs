...
When you configure the Cluster Network Operator with one or more collector IP addresses and port numbers, the Operator configures Open vSwitch (OVS) on each node to send the network flows records to each collector.
...
[id="tracking-network-flows-additional-resources"]
