...
[IMPORTANT]
====
Egress traffic that uses an egress IP address hosted on the primary network interface is an exception to the preceding description of the `true` value. Static routes that are configured in the main routing table of the host are not used for this traffic, even when you set `routingViaHost` to `true`. Egress traffic from pods that are not selected by any `EgressIP` object is not affected by this limitation.
For more information about the limitations, see "Limitations of egress IP addresses with host static routes".
====

[role="_additional-resources"]
.Additional resources
* xref:../../networking/ovn_kubernetes_network_provider/configuring-egress-ips-ovn.adoc#nw-egress-ips-host-static-routes_configuring-egress-ips-ovn[Limitations of egress IP addresses with host static routes]
