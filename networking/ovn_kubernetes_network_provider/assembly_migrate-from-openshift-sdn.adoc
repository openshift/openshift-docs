...
[role="_abstract"]
As a {product-title} cluster administrator, you can initiate the migration from the OpenShift Software-Defined Networking (SDN) network plugin to the OVN-Kubernetes network plugin and verify the migration status by using the {rosa-cli-first}.
...
include::modules/proc_migrate-sdn-ovn.adoc[leveloffset=+1]
