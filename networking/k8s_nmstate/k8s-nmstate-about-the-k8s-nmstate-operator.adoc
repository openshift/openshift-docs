...
[IMPORTANT]
====
Red Hat supports the Kubernetes NMState Operator in production environments on bare-metal, {iibm-cloud-bm-reg}, {ibm-z-reg}, {ibm-linux-one-reg}, VMware vSphere, and OpenStack installations.
====
