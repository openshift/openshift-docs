...
* For more information on the `NodeNetworkConfigurationPolicy` manifest, see xref:../../networking/k8s_nmstate/k8s-nmstate-updating-node-network-config.adoc#virt-creating-interface-on-nodes_k8s_nmstate-updating-node-network-config[Creating an interface on nodes] in _Observing and updating the node network state and configuration_.
