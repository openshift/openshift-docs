...
= Accelerate packet processing with DPDK and RDMA
...
[role="_abstract"]
Achieve maximum packet processing performance and low latency by using Data Plane Development Kit (DPDK) and Remote Direct Memory Access (RDMA) with Single Root I/O Virtualization (SR-IOV) network devices.

[id="using-dpdk-and-rdma-prerequisites"]
== Prerequisites
...
* xref:../../networking/hardware_networks/sriov-configure-device.adoc#nw-sriov-networknodepolicy-object_configuring-sriov-device[Configure SR-IOV network devices]
