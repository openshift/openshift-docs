...
= Offload packet processing to NIC hardware
...
[role="_abstract"]
Improve packet processing performance and reduce CPU load by offloading network operations to SR-IOV network interface hardware.
...
* You xref:../../networking/networking_operators/sr-iov-operator/installing-sriov-operator.adoc#installing-sriov-operator[installed the SR-IOV Network Operator]. Verify by running:
+
[source,terminal]
----
...
* Your cluster uses the xref:../../networking/ovn_kubernetes_network_provider/about-ovn-kubernetes.adoc#about-ovn-kubernetes[OVN-Kubernetes network plugin]. Verify by running:
+
[source,terminal]
----
...
----
+
The output shows `OVNKubernetes`.
* In your xref:../../networking/networking_operators/cluster-network-operator.adoc#gatewayConfig-object_cluster-network-operator[OVN-Kubernetes network plugin configuration], the `gatewayConfig.routingViaHost` field is set to `false`. Verify by running:
+
[source,terminal]
----
...
----
+
The output shows `false` or is empty, which defaults to `false`.
