...
[id="configure-aws-load-balancer-operator"]
...
.Using the AWS Load Balancer with Outposts

After installing the AWS Load Balancer Operator and creating the AWS Load Balancer Controller, they can be used with outposts.
include::modules/nw-aws-load-balancer-with-outposts.adoc[leveloffset=+1]

.Using the AWS Load Balancer with multiple Ingress
...
.Using cluster-wide proxy in the AWS Load Balancer Operator
