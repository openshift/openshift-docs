...
An additional network is configured by using the `NetworkAttachmentDefinition` DERP in the `k8s.cni.cncf.io` DERP group.
[IMPORTANT]
====
...
The configuration for the DERP is described in the following table:

.`NetworkAttachmentDefinition` DERP fields
