...
* Applying a YAML manifest: You can manage the additional network directly by creating an `NetworkAttachmentDefinition` object. Compared to modifying the CNO configuration, this approach gives you more granular control and flexibility when it comes to configuration. For example, creating an `NetworkAttachmentDefinition` object allows for the chaining of CNI plugins.
