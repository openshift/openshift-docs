...
Administrators can use the `MultiNetworkPolicy` API to create multiple network policies that manage traffic for pods attached to secondary networks. For example, you can manage traffic between pods attached to the same or different secondary networks by creating policies that allow or deny traffic based on specific ports or labels.

[NOTE]
====
Multi-network policies can only manage traffic on secondary networks in the cluster. It cannot manage the cluster's primary network.
====
...
Other types of additional networks, such as IP Virtual Local Area Network (IPVLAN), are not supported.
