...
Multi-network policy is used by administrators to manage secondary networks created using the network attachment definition (NAD) resource. Administrators use the `MultiNetworkPolicy` API to manage traffic for pods that are attached to multiple networks. For example, you can separate the control plane and data plane traffic.
