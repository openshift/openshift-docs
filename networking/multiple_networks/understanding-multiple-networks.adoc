...
* *bond-cni*: xref:../../networking/multiple_networks/secondary_networks/creating-secondary-nwt-other-cni.adoc#nw-multus-bond-cni-object_configuring-additional-network-cni[Configure a bond-cni secondary network] providing a method for aggregating
multiple network interfaces into a single logical _bonded_ interface.
