...
{product-title} uses the Multus CNI plugin to allow chaining of CNI plugins. During cluster installation, you configure your default pod network. The default network handles all ordinary network traffic for the cluster. You can define an additional network based on the available CNI plugins and attach one or more of these networks to your pods. You can define more than one additional network for your cluster, depending on your needs. This gives you flexibility when you configure pods that deliver network functionality, such as switching or routing.

Administrators or users can leverage xref:../../networking/multiple_networks/primary_networks/about-user-defined-networks.adoc#about-user-defined-networks[user-defined networks (UDNs)] or xref:../../networking/multiple_networks/primary_networks/about-primary-nwt-nad.adoc#understanding-multiple-networks[NetworkAttachmentDefinition (NADs)] to create additional networks of the following types:

* *Primary networks*: Act as the primary network for the pod. By default, all traffic passes through the primary network unless a route is configured to send traffic through other networks, or a network policy is configured to restrict traffic.
...
Additional CNI plugins, such as bridge, host-device, ipvlan, vlan, macvlan, TAP, and SR-IOV are also available as secondary networks. For more information, see "Additional networks in {product-title}".
