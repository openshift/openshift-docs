...
{product-title} uses the Multus CNI plug-in to allow chaining of other CNI plug-ins. During cluster installation, you configure your _default_ CNI network provider. The default network provider handles all ordinary network traffic for the cluster.

You can define an _additional network_ based on the available CNI plug-ins and attach one or more of these networks to your Pods. You can define more than one additional network for your cluster, depending on your needs. This gives you flexibility when you configure Pods that deliver network functionality, such as switching or routing.

Additional networks are compatible with both IPv4 and IPv6 networks.
