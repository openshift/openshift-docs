...
User-defined networks offer enhanced customization and isolation by leveraging the `ClusterUserDefinedNetwork` custom resource (CR) or the `UserDefinedNetwork` CR depending on your needs. Unlike NADs, which are only namespaced scope, UDNs offer administrators the ability to create and define additional networks at the cluster level. UDNs also offer both administrators and users the ability to define additional networks at the namespace level.
