...
{product-title} provides the following methods for communicating from outside the cluster with services running in the cluster. A cluster administrator configures the Ingress Controller, and a developer configures Route or Ingress objects.

The methods are recommended, in order or preference:

* If you have HTTP/HTTPS, use an Ingress Controller.
* If you have a TLS-encrypted protocol other than HTTPS, use an Ingress Controller with Route objects.
