...
. Edit the existing `IngressController` to set `spec.endpointPublishingStrategy.loadBalancer.providerParameters.aws.type` to `NLB`. Starting in {product-title} 4.22, the cloud controller does not reprovision the load balancer automatically. The `IngressController` reports a `Progressing` condition that instructs you to delete the router `Service` in the `openshift-ingress` namespace so that a new load balancer can be created. That interruption can change the load balancer hostname and IP addresses. Complete *Updating the subnets on an existing Ingress Controller* for the steps that show reading the `Progressing` condition and deleting the router `Service`.
