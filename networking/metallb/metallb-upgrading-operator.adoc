...
If automatic updates from a much earlier version are not successful, complete the following procedure to update the MetalLB Operator from that version:

A summary of the upgrade procedure for the MetalLB Operator from 4.10 and earlier is as follows:

. Delete the installed MetalLB Operator version that is currently installed. Ensure that the namespace and the metallb custom resource are not removed.

. Using the CLI, install the MetalLB Operator for the latest version in the same namespace where the previous version of the MetalLB Operator was installed.

[NOTE]
====
Only the 4 most recent versions are supported.
====
