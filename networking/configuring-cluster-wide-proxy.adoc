...
ifdef::openshift-rosa[]
//
// Adding links for HCP and Classic until the split out
//
...
* For the installation prerequisites for ROSA clusters that use the AWS Security Token Service (STS), see xref:../rosa_planning/rosa-classic-prereqs.adoc#rosa-classic-prereqs[AWS prerequisites for ROSA (classic architecture)] and xref:../rosa_planning/rosa-hcp-prereqs.adoc#rosa-hcp-prereqs[AWS prerequisites for ROSA].
