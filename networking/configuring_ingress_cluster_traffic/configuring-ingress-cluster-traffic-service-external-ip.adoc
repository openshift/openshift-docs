...
You can use either MetalLB or IP failover to host an external IP address on a service so that the service is available to traffic outside your {product-title} cluster. Consider this configuration for a cluster installed on bare-metal hardware. 
