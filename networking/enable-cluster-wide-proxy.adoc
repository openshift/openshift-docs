...
status:
  ...
  - .svc
  - 127.0.0.1
  - <api_server_internal_url> <1>
# ...
----
<1> You can find the URL of the internal API server by running the `oc get infrastructures.config.openshift.io cluster -o jsonpath='{.status.etcdDiscoveryDomain}'` command. 
<2> Specify IP address address blocks from which pod IP addresses are allocated. The default value is `10.128.0.0/14` with a host prefix of `/23`.
<3> Specify the IP address blocks for machines. The default value is `10.0.0.0/16`.
<4> Specify IP address block for services. The default value is `172.30.0.0/16`.
