...
* If you enabled IPsec for network interfaces that are not attached to OVS, as a cluster administrator you must manually disable ESP hardware offloading on each interface that is not attached to OVS.
