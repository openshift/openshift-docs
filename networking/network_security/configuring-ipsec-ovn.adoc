...
[NOTE]
====
Upgrading your cluster to {product-title} {product-version} when the `libreswan` and `NetworkManager-libreswan` packages differ between {product-title} versions leads to two consecutive compute node reboot operations. For the first reboot, the Cluster Network Operator (CNO) applies the IPsec configuration to compute nodes. For the second reboot, the Machine Config Operator (MCO) applies the latest machine configs to the cluster.

Before upgrading your cluster, set the `paused` parameter to `true` in the `MachineConfigPools` custom resource (CR) that groups compute nodes as demonstrated with the following command:

[source,terminal]
----
$ oc patch mcp/<machineconfigpool_name> --patch '{"spec":{"paused":true}}' --type=merge
----

After you upgrade your cluster, set the parameter to `false`. This configurations workflow combines CNO and MCO updates into a single node reboot.
====
