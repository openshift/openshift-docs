...
[NOTE]
====
Upgrading your cluster to {product-title} {product-version} when the `libreswan` and `NetworkManager-libreswan` packages differ between {product-title} versions leads to two consecutive node reboot operations. The first reboot operation allows the Cluster Network Operator (CNO) to apply the IPsec configuration to compute nodes. The second reboot operation allows the Machine Config Operator (MCO) to apply the latest machine configs to the cluster.

To avoid the two consecutive node reboots, set the `paused` parameter to `true` in the `MachineConfigPools` custom resource (CR) before you upgrade your cluster. After you upgrade your cluster, set the `paused` parameter to `false`. These configurations lead to a single node reboot operation where the CNO and MCO updates happen together.
====
