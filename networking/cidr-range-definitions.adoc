...
[NOTE]
====
When using {hcp-title}, the static IP address `172.20.0.1` is reserved for the internal Kubernetes DERP address. The machine, pod, and service CIDRs ranges must not conflict with this IP address.
====
endif::[]
