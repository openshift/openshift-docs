...
An isolation selector describes the set of pods that a network policy isolates. You can isolate all pods in a namespace or selectively apply a network policy to a subset of pods.

If you specify an `podSelector`, as in the following example, then all pods in a namespace are selected.
...
A network policy must specify one or more policy rules that describe which ingress or egress traffic is permitted for pods that the network policy isolates.
