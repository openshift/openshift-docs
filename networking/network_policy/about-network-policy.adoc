...
A pod that is isolated is one that is selected by at least one network policy isolation selector.
...
[IMPORTANT]
====
To ensure that pods are isolated by default, it is recommended to always create a deny ingress by default network policy that selects all pods in the namespace.
====
