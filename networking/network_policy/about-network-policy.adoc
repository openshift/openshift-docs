...
When using ingress and ingress policy rules, the following guidelines apply:
...
* As a network policy peer selector in an ingress or egress rule to select which traffic from a pod or to a pod to allow
////
...
When you define a network policy, each policy rule describes the network policy peers that the network policy applies to. This peer group is the set of pods to which ingress or egress traffic is restricted. You can also specify network policy peers by IP address block if necessary.
...
* In a specified namespace, match all pods as network policy peers:
+
...
* In any namespace, match all pods with `deployment: blue` as network policy peers:
+
...
* In a specified namespace and with the `deployment: blue` label, select the matching pods as network policy peers:
+
...
When defining network policies, it is recommended to always first define a deny by default policy. Because network policies are additive, any subsequent network policy rules for an isolated pod allows network traffic from only the network policy peers selected by the policy.
...
In the previous example, the `web-deny-all` network policy isolates the `app: web` pods and does not allow any ingress traffic. The `web-allow-http` network policy also isolates the `app: web` pods and allows traffic to them on port 8088 from all pods in the same namespace.

TODO try: The correct description of the example above is that the web-deny-all policies isolates app: web pods, and then does not add any allowed traffic. The web-allow-http policy also isolates app: web pods, and then also allows traffic to them on port 8088 from all pods in the same namespace. In particular, you get exactly the same effect without the web-deny-all policy, because all it was doing is selecting those pods for isolation, but the web-allow-http policy selects the same pods for isolation.
