...
NOTE: When using the OpenShift SDN cluster network provider, the following limitations apply regarding network policies:
...
You can use network policy for only TCP, UDP, and SCTP traffic types.
...
`policy-group.network.openshift.io/ingress: ""`:: Selects all Ingress Controller pods.
`policy-group.network.openshift.io/monitoring`: "":: Selects all monitoring pods.
...
TODO: is `podSelector:` and `podSelector: {}` equivalent?
