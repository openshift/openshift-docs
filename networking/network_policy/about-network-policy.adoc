...
* Network policy applies to pods in the namespace in which the policy is defined.
...
* If only a pod selector is used then a policy rule applies to all pods that the selector matches, regardless of namespace.
