...
As a cluster administrator, you can apply network policies to allow network traffic to some pods and deny network traffic to other pods.
...
As a cluster or project administrator, you can use a network policy to allow and deny network traffic. The following concepts apply to network policy:

* Network policy applies to pods in a specific namespace.
* Network policy defines the following permitted network traffic types:
...
* Network policy supports specifying network ports for supported protocols.

A network policy consists of two parts:

* Isolation selector: Identifies which pods the network policy isolates. A pod selected by an isolation selector is considered to be an _isolated pod_.
...
A pod that is isolated is one that is selected by the isolation selector of at least one network policy.
If a pod is not selected by the isolation selector of any network policy in the namespace that the pod exists, the pod is unaffected by network policy and all ingress and egress traffic is permitted.
...
[id="types-of-network-policy_{context}"]
...
* As an isolation selector to select the pods that the policy isolates
...
An isolation selector identifies the set of pods that a network policy isolates. You can isolate all pods in a namespace or selectively apply a network policy to a subset of pods.
...
If you define a network policy rule with an empty list, the network policy does allow any traffic. Any pods selected by the network policy are completely isolated.
