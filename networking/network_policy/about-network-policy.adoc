...
* Isolation selector: Identifies which pods the network policy isolates
...
A network policy must specify one or more policy rules that identifies which ingress or egress traffic is permitted for pods that the network policy isolates.
...
For example, if your cluster defines the following network policies, ingress traffic is allowed on port `8088` to the pod with the `app=web` label from any other pods in the same namespace.
