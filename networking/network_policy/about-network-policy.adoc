...
In the previous example, the `deny-all` network policy isolates all pods in the namespace and does not allow any ingress traffic. The `web-allow-http` network policy also isolates the `app: web` pods and allows traffic to them on port 8088 from all pods in the same namespace.
