...
* Network policy does not apply to the host network. Pods with host networking enabled, such as in the following example, are unaffected by network policy rules.
+
[source,yaml]
----
apiVersion: v1
...
An ingress policy rule is defined by the following:

An egress policy rule is defined by the following:
