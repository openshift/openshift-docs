...
[id="aws-load-balancer-operator-release-notes-1.3.3"]
== AWS Load Balancer Operator 1.3.3

The following advisory is available for the AWS Load Balancer Operator version 1.3.3:

* link:https://access.redhat.com/errata/RHEA-XXXX:XXXX[RHEA-XXXX:XXXX Release of AWS Load Balancer Operator on OperatorHub Enhancement Advisory Update]

[id="aws-load-balancer-operator-1.3.3-notable-changes"]
=== Notable changes

* This release uses the Kubernetes API version 0.XX.X.

[id="aws-load-balancer-operator-1.3.3-new-features"]
=== New features

* The AWS Load Balancer Operator is now supported on ARM64 architecture.
