...
eBPF CRDs:: Cluster-scoped `ClusterBpfApplication` and namespace-scoped `BpfApplication` custom resources that load one or more eBPF programs from OCI container images. The Operator generates `ClusterBpfApplicationState` and `BpfApplicationState` objects that report the per-node load and attach status.
bpfman-agent:: Runs within a daemon set container, ensuring eBPF programs on each node are in the desired state.
...
* link:https://bpfman.io/v0.6.0[bpfman]
* link:https://bpfman.io/v0.6.0/getting-started/example-bpf-k8s/[Deploying example eBPF programs on Kubernetes]