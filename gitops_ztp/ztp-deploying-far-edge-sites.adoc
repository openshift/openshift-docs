...
[IMPORTANT]
====
The target bare-metal host must meet the networking, firmware, and hardware requirements listed in xref:../gitops_ztp/ztp-reference-cluster-configuration-for-vdu.adoc#sno-configure-for-vdu[Recommended {sno} cluster configuration for vDU application workloads].
====
...
=======
* xref:../gitops_ztp/ztp-deploying-far-edge-sites.adoc#ztp-verifying-ipsec_ztp-deploying-far-edge-sites[Verifying the IPsec encryption]

>>>>>>> 5997cf630b (TELCODOCS#2177: Refactor Edge Computing to improve ToC navigation):gitops_ztp/ztp-deploying-far-edge-sites.adoc
...
* xref:../gitops_ztp/ztp-deploying-far-edge-sites.adoc#ztp-deploying-far-edge-sites[Installing managed clusters with {rh-rhacm} and SiteConfig resources]
...
* xref:../gitops_ztp/ztp-preparing-the-hub-cluster.adoc#ztp-configuring-hub-cluster-with-argocd_ztp-preparing-the-hub-cluster[Configuring the hub cluster with ArgoCD]
...
* xref:../gitops_ztp/ztp-manual-install.adoc#ztp-creating-the-site-secrets_ztp-manual-install[Creating the managed bare-metal host secrets]
