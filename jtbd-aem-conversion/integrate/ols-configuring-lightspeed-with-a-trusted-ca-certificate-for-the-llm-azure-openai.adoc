...
[id="ols-configuring-lightspeed-with-a-trusted-ca-certificate-for-the-llm-azure-openai"]

= Configuring {ols-long} with a trusted CA certificate for {azure-openai}

[role="_abstract"]
Use the {ocp-product-title} web console to configure a trusted CA certificate for secure communication between {ols-long} and {azure-openai}.
...
. Update the `OLSConfig` custom resource (CR) file to include the name of the `ConfigMap` object you just created. The following example uses {azure-openai} as the LLM provider.
+
[source,yaml]
----
