...
[id="ols-configuring-lightspeed-with-a-trusted-ca-certificate-for-the-llm-openshift-ai"]

= Configuring {ols-long} with a trusted CA certificate for {rhoai}

[role="_abstract"]
Use the {ocp-product-title} web console to configure a trusted CA certificate for secure communication between {ols-long} and {rhoai}.
...
. Update the `OLSConfig` custom resource (CR) file to include the name of the `ConfigMap` object you just created. The following example uses {rhoai} as the LLM provider.
+
[source,yaml]
----
