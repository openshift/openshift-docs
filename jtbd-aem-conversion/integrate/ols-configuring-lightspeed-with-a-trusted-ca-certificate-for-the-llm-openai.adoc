...
[id="ols-configuring-lightspeed-with-a-trusted-ca-certificate-for-the-llm-openai"]

= Configuring {ols-long} with a trusted CA certificate for {openai}

[role="_abstract"]
Use the {ocp-product-title} web console to configure a trusted CA certificate for secure communication between {ols-long} and {openai}.
...
. Update the `OLSConfig` custom resource (CR) file to include the name of the `ConfigMap` object you just created. The following example uses {openai} as the LLM provider.
+
[source,yaml]
----
