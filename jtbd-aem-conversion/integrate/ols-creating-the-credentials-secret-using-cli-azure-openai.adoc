...
[id="ols-creating-the-credentials-secret-using-cli-azure-openai"]

= Creating the {azure-openai} credentials secret by using the CLI

[role="_abstract"]
Use the command line interface to store the API token that {ols-long} uses to authenticate with {azure-openai}.

{azure-official} also supports authentication using {entra-id}.
...
. Create a YAML file that has the credential secret for {azure-openai}.
+
[NOTE]
====
