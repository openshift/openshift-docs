...
[id="ols-configuring-lightspeed-with-a-trusted-ca-certificate-for-the-llm-google-vertex-ai"]

= Configuring {ols-long} with a trusted CA certificate for Google Vertex AI

[role="_abstract"]
Use the {ocp-product-title} web console to configure a trusted CA certificate for secure communication between {ols-long} and Google Vertex AI.
...
. Update the `OLSConfig` custom resource (CR) file to include the name of the `ConfigMap` object you just created. The following example uses Google Vertex AI as the LLM provider.
+
[source,yaml]
----
