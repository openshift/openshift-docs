...
. Expose the `lightspeed-app-server` service as a route.
. Obtain an authentication token by using the `oc whoami -t` command.
. Send a request to the `/v1` endpoint by using the route host and your authentication token.
