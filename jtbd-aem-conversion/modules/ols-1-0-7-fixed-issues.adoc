...
[role="_abstract"]
{ols-official} 1.0.7 improves installation stability by adding a health check to the `OLSConfig` custom resource that validates model references. This fix prevents deployment failures due to invalid model settings and provides detailed error information directly in the web console.
