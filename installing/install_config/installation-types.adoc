...
[NOTE]
====
Not all installation types are supported for all platforms, and not all supported installation types are documented.

In the following table, installation types that are supported and documented are marked with a hyperlinked `X` character.

Installation types that are supported, but not documented, have a check mark.
====
...
Other platform and installation methods are possible, but they are not documented and may not be supported.
====
