...
From {product-title} 4.14 and later versions, you can use the {ai-full} to install a cluster on {oci-first}. This method is recommended for most users, and requires an internet connection.

If you want to set up the cluster manually or using other automation tools, or if you are working in a disconnected environment, you can use the Red Hat Agent-based Installer for the installation. For details, see "Installing a cluster on {oci-first-no-rt} by using the Agent-based Installer".
