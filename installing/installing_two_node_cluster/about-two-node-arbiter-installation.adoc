...
After installation, you can add additional worker nodes to a cluster with two control plane nodes and one local arbiter node but not to a standard multi-node cluster. It is also not possible to convert between a two-node {product-title} cluster with a local node arbiter and standard topology.
