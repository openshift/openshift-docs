...
[role="_abstract"]
A two-node {product-title} cluster with fencing (TNF) provides high availability (HA) with a reduced hardware footprint. This configuration is designed for distributed or edge environments where deploying a full three-node control plane cluster is not practical.
...
The two-node {product-title} cluster with fencing requires the following hosts:
...
* xref:../../../installing/installing_bare_metal/upi/installing-bare-metal-network-customizations.adoc#creating-machines-bare-metal_installing-bare-metal-network-customizations[Installing RHCOS and starting the {product-title} bootstrap process]
