...
= Bare-metal cluster postinstallation configuration
...
[role="_abstract"]
Configure bare-metal cluster settings after installation, including `BareMetalHost` resource management with BMC certificates, provisioning networks, and `br-ex` bridge customization using the Bare Metal Operator.
