...
[role="_abstract"]
As a cluster administrator, after deploying a bare-metal cluster, you can use the following procedures to expand the number of worker nodes. These procedures cover Redfish Virtual Media provisioning and BMC configuration. Ensure that each prospective worker node meets the prerequisites.
...
include::modules/ipi-install-preparing-to-deploy-with-virtual-media-on-the-bare-metal-network.adoc[leveloffset=+1]
