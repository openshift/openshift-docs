[id="installing-rhv-verifying-the-rhv-environment"]
= Verifying the {rh-virtualization} environment
include::modules/common-attributes.adoc[]
:context: installing-rhv-verifying-the-rhv-environment
:rhv-version: 4.3.9

To install and run {product-title} on {rh-virtualization-first} without failures, the {rh-virtualization} must satisfy a set of requirements.

Review the following list of requirements. Then inspect the {rh-virtualization} environment to verify that it meets each requirements. Also write down a few pieces of information you will provide to the installer later.
...
== Checking the {rh-virtualization} version is {rhv-version}

. In the {rh-virtualization} Administration Portal, click the “?” help icon in the upper-right corner and select *About*.
. In the dialog that opens, confirm that the {rh-virtualization} Software Version is {rhv-version}.
...
[NOTE]
====
To customize the disk size, see XREF TBD.
====
...
----
curl -k -u <ovirt-engine username>:<password> \ <1>
https://<engine-fqdn>/ovirt-engine/api <2>
----
<1> For <ovirt-engine username> and <password>, specify the username and password of an oVirt user with `SuperUser` or `ClusterAdmin` privileges.
