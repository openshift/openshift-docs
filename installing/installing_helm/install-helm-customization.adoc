...
Before you install the `SecuredCluster` resource on a cluster, you must create an init bundle or CRS. The cluster that has `SecuredCluster` installed and configured then uses the init bundle or the CRS to authenticate with Central.
