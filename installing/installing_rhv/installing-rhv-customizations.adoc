...
include::modules/installation-initializing-new.adoc[leveloffset=+1]
// testing without include::modules/installing-rhv-create-install-config-file.adoc[leveloffset=+1]
include::modules/installing-rhv-inspect-and-modify-params-in-install-config.adoc[leveloffset=+1]
// In 4.4 GA, use the include above because the current RHV installer does not support most configuration parameters. When the installer supports more custom parameters in an upcoming z-stream: 1) remove the include above; 2) uncomment the include below; 3) and update the contents of the included file to display supported parameters.
