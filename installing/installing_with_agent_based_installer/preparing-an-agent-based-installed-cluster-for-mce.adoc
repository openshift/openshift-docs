...
[id="preparing-an-agent-based-installed-cluster-for-multicluster-engine"]
= Prepare an Agent-based installed cluster for the {mce}
:context: preparing-an-agent-based-installed-cluster-for-mce

toc::[]

[role="_abstract"]
You can install the {mce-short} and deploy a hub cluster with the Agent-based {product-title} Installer. This installation is partially automated and requires manual steps after the initial cluster is deployed.
...
* xref:../../storage/persistent_storage_local/persistent-storage-local.adoc#persistent-storage-using-local-volume[The Local Storage Operator]
