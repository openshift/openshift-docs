...
[id="installing-aws-user-infra-overview-steps"]
== Overview steps
The following list outlines the steps you need to complete to install a cluster on user-provisioned infrastructure by using CloudFormation templates:

* Create the installation files.
* Extract the infrastructure name.
* Create a VPC.
* Create network and load balancing components.
* Create security group and roles.
* Access RHCOS AMIs with stream metadata.
* Create  the bootstrap node.
* Create the control plane machines.
* Create the worker nodes.
* Initialize the bootstrap sequence with user-provisioned infrastructure.
* Log into the cluster by using the CLI.
* Approve the certificate signing requests for your machines.
* Initial Operator configuration.
* Delete the bootstrap resources.
* Create the Ingress DNS Records.
* Complete an installation on user-provisioned infrastructure.
* Log into the cluster by using the web console.

[id="installing-aws-user-infra-overview"]
== Steps overview

* For information about generating the files that the installation program needs to deploy your cluster and modify them, see _Creating the installation files for AWS_.

* For information about extracting references that uniquely identify your cluster, see _Extracting the infrastructure name_.

* For information about creating a Virtual Private Cloud (VPC) for your cluster, see _Creating a VPC in AWS_.

* For information about configuring networking and classic or network load balancing that your cluster can use, see _Creating networking and load balancing components in AWS_.

* For information about creating security groups and roles for your cluster to use, see _Creating security group and roles in AWS_.
