...
[role="_abstract"]
In {product-title} version {product-version}, you can install a cluster with a customized network configuration on infrastructure that the installation program provisions on {ibm-cloud-name}.

By customizing your network configuration, your cluster can coexist with existing IP address allocations in your environment and integrate with existing MTU and VXLAN configurations. To customize the installation, you modify parameters in the `install-config.yaml` file before you install the cluster.
...
[id="additional-resources_installing-ibm-cloud-network-customizations"]
.Additional resources
