...
= Installing a cluster with customizations on {ibm-cloud-title}
:context: installing-ibm-cloud-customizations

toc::[]

In {product-title} version {product-version}, you can install a
cluster, with customizations, on infrastructure that the installation program provisions on {ibm-cloud-name}.
