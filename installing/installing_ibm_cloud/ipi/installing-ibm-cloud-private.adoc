...
= Installing a private cluster with an existing VPC network on {ibm-cloud-title}
:context: installing-ibm-cloud-private

toc::[]

In {product-title} version {product-version}, you can install a
private cluster, with an existing Virtual Private Cloud (VPC) network and no external connectivity, on infrastructure that the installation program provisions on {ibm-cloud-name} within the target VPC (optionally including further customization).
