...
= Installing a cluster with network customizations on {ibm-cloud-title}
:context: installing-ibm-cloud-network-customizations

toc::[]

In {product-title} version {product-version}, you can install a
cluster, with Virtual Private Cloud (VPC) network configuration customizations, on infrastructure that the installation program provisions on {ibm-cloud-name}. By customizing your network configuration, your cluster can coexist with existing IP address allocations in your environment and integrate with existing MTU and VXLAN configurations.
