...
= Installing a cluster with an existing VPC network on {ibm-cloud-title}
:context: installing-ibm-cloud-vpc

toc::[]

In {product-title} version {product-version}, you can install a
cluster, with an existing Virtual Private Cloud (VPC) network, on infrastructure that the installation program provisions on {ibm-cloud-name} within the target VPC (optionally including further customization).
