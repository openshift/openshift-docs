...
. Convert the ISO image to an {oci-edge-no-rt} image, upload it to an {oci} Home Region Bucket, and then import the uploaded image to the {oci-edge-no-rt} system (Oracle).
