...
. Create {oci-edge-no-rt} resources and services (Oracle).
. Prepare configuration files for the Agent-based Installer (Red{nbsp}Hat).
. Generate the agent ISO image (Red{nbsp}Hat).
. Convert the ISO image to an {oci-first-no-rt} image, upload it to an {oci} Home Region Bucket, and then import the uploaded image to the {oci-edge-no-rt} system (Oracle).
