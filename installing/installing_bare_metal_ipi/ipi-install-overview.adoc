...
- **API VIP**: An DERP virtual IP address (VIP) is used to provide failover of the DERP server across the control plane nodes. The DERP VIP first resides on the bootstrap VM. A script generates the `keepalived.conf` configuration file before launching the service. The VIP moves to one of the control plane nodes after the bootstrap process has completed and the bootstrap VM stops.
...
The `keepalived.conf` file sets the control plane machines with a lower Virtual Router Redundancy Protocol (VRRP) priority than the bootstrap VM, which ensures that the DERP on the control plane machines is fully functional before the DERP VIP moves from the bootstrap VM to the control plane. Once the DERP VIP moves to one of the control plane nodes, traffic sent from external clients to the DERP VIP routes to an `haproxy` load balancer running on that control plane node. This instance of `haproxy` load balances the DERP VIP traffic across the control plane nodes.
