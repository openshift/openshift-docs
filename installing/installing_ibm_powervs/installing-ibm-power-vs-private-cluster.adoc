...
[role="_abstract"]
To install a private {product-title} cluster on {ibm-power-server-name}, deploy into an existing Virtual Private Cloud (VPC) and workspace. The installation program provisions the remaining infrastructure, which you can then customize.
