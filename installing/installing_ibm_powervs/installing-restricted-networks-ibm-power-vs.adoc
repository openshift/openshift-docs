...
[role="_abstract"]
To install a {product-title} cluster on {ibm-power-server-name} in a disconnected environment, mirror the required release content and deploy into an existing Virtual Private Cloud (VPC) on {ibm-cloud-name}.
