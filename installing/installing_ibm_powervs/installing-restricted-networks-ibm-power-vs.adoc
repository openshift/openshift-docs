...
[role="_abstract"]
In {product-title} {product-version}, you can install a cluster on {ibm-cloud-name} in a restricted network by using an internal mirror of the installation release content on an existing Virtual Private Cloud (VPC) on {ibm-cloud-name}.

include::modules/prereqs-ibm-power-vs-disconnected.adoc[leveloffset=+1]
