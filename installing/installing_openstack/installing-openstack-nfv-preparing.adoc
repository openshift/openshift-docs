...
include::modules/installation-openstack-sr-iov-preparing.adoc[leveloffset=+1]

// TODO: SME/QE check: Confirm that the following SR-IOV network creation workflow is still valid after the RHOSP-to-RHOSO transition and does not require additional data plane prerequisites.
include::modules/installation-osp-configuring-sr-iov.adoc[leveloffset=+2]

include::modules/installation-openstack-ovs-dpdk-preparing.adoc[leveloffset=+1]

include::modules/installation-openstack-nfv-next-steps.adoc[leveloffset=+1]
...
* xref:../../networking/networking_operators/sr-iov-operator/installing-sriov-operator.adoc#install-operator-cli_installing-sriov-operator[Install the SR-IOV Operator]
...
* xref:../../networking/hardware_networks/configuring-sriov-device.adoc#nw-openstack-ovs-sr-iov-testpmd-pod_configuring-sriov-device[A test pod template for clusters that use SR-IOV on OpenStack]
