...
* If you are installing using link:https://cloud.google.com/vpc/docs/private-service-connect[custom service endpoints], ensure that the {product-title} is hosted on a network with access to those endpoints. If the installation program cannot access those endpoints, the installation process fails.
