...
[IMPORTANT]
====
When installing a cluster on a Shared Virtual Private Cloud (VPC) using short-lived credentials, you must grant the `compute.subnetworks.use` permission in the host project to Day 2 Operator service accounts. 
