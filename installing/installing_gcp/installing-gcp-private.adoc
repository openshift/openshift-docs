...
= Installing a private cluster on {gcp-full}
:context: installing-gcp-private

toc::[]

In {product-title} version {product-version}, you can install a private cluster into an existing VPC on {gcp-full}. The installation program provisions the rest of the required infrastructure, which you can further customize. To customize the installation, you modify
...
* To implement short-term credentials that are managed outside the cluster for individual components, follow the procedures in xref:../../installing/installing_gcp/installing-gcp-private.adoc#installing-gcp-with-short-term-creds_installing-gcp-private[Configuring a {gcp-full} cluster to use short-term credentials].
