...
.3+|The FIPS implementations lacks use of single function computing hash and validating keys based on compliant run times and services. This limitation will continue to be evaluated and improved in future {product-title} releases.
...
Although the {product-title} cluster itself uses FIPS validated modules, ensure that the systems that support your {product-name} cluster use FIPS validated modules for cryptography.
...
To ensure that containers know that they are running on a host that has is using FIPS validated cryptography modules, use CRI-O to manage your runtimes. mode. CRI-O supports FIPS-Mode, in that it configures the containers to know that they are running in FIPS mode.
...
* xref:../installing/installing_bare-metal/installing-bare-metal.adoc#installing-bare-metal[Bare metal]
