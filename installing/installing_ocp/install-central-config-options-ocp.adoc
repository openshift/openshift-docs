:_mod-docs-content-type: ASSEMBLY
[id="install-central-config-options-ocp"]
= Configuring Central configuration options for RHACS using the Operator
include::modules/common-attributes.adoc[]
:context: install-central-config-options-ocp

toc::[]

[role="_abstract"]
When installing the Central instance by using the Operator, you can configure optional settings.

// Central configuration options overview
include::modules/central-configuration-options-operator.adoc[leveloffset=+1]

// Central settings parameters
include::modules/central-settings-operator.adoc[leveloffset=+2]

[id="additional-resources-central-settings_{context}"]
[role="_additional-resources"]
.Additional resources
* link:https://www.postgresql.org/docs/15/libpq-connect.html#LIBPQ-CONNSTRING[Connection Strings - PostgreSQL Docs]
* link:https://www.postgresql.org/docs/15/config-setting.html#CONFIG-SETTING-CONFIGURATION-FILE[Parameter Interaction via the Configuration File - PostgreSQL Docs]
* link:https://www.postgresql.org/docs/15/auth-pg-hba-conf.html[The pg_hba.conf File - PostgreSQL Docs]

// StackRox Scanner settings parameters
include::modules/scanner-settings-operator.adoc[leveloffset=+2]

// Scanner V4 settings parameters
include::modules/scannerv4-settings-operator.adoc[leveloffset=+2]

// General and miscellaneous settings parameters
include::modules/general-and-miscellaneous-settings-operator.adoc[leveloffset=+2]

[id="additional-resources-general-settings_{context}"]
[role="_additional-resources"]
.Additional resources
* xref:../../operating/manage-vulnerabilities/vulnerability-reporting.adoc#configure-vulnerability-report-row-limit_vulnerability-reporting[Configuring the vulnerability report row limit]

// Customizing the installation using the Operator with overlays
include::modules/customize-installation-operator-overlays.adoc[leveloffset=+1]

// Overlays overview
include::modules/overlays-overview.adoc[leveloffset=+2]

// Adding an overlay
include::modules/adding-an-overlay.adoc[leveloffset=+3]

// Overlay examples
include::modules/overlay-examples.adoc[leveloffset=+2]

// Specifying an EKS pod role ARN
include::modules/adding-eks-role-arn-annotation.adoc[leveloffset=+3]

// Injecting an environment variable
include::modules/adding-env-variable-to-deployment.adoc[leveloffset=+3]

// Extending network policy with an ingress rule
include::modules/adding-ingress-to-network-policy.adoc[leveloffset=+3]

// Modifying ConfigMap data
include::modules/changing-configmap-data.adoc[leveloffset=+3]

// Adding a container to deployment
include::modules/adding-container-to-deployment.adoc[leveloffset=+3]