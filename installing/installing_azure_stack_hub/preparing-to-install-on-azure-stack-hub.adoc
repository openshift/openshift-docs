...
* Manually manage your cloud credentials when installing a cluster to Azure Stack Hub. Do this by configuring the Cloud Credential Operator (CCO) for manual mode before you install the cluster. For more information, see xref:../../installing/installing_azure/manually-creating-iam-azure.adoc#manually-creating-iam-azure[Manually creating IAM for Azure].
...
* **xref:../../installing/installing_azure_stack_hub/installing-azure-stack-hub-customizations.adoc#installing-azure-stack-hub-customizations[Installing a cluster on Azure Stack Hub with customizations]**: You can install a customized cluster on Azure Stack Hub infrastructure that the installation program provisions. The installation program allows for some customization to be applied at the installation stage. Many other customization options are available xref:../../post_installation_configuration/cluster-tasks.adoc#post-install-cluster-tasks[post-installation].
