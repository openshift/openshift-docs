...
* The Cloud Credential Operator (CCO) only supports your cloud provider in manual mode. As a result, you must xref:../../installing/installing_azure_stack_hub/installing-azure-stack-hub-default.adoc#manually-create-iam_installing-azure-stack-hub-default[manually manage clould credentials] by specifying the identity and access management (IAM) secrets for your cloud provider.
