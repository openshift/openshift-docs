...
Note that certain options, such as *Pipelines*, *Event Source*, and *Import Virtual Machines*, are displayed only when the {pipelines-shortname} Operator, {ServerlessOperatorName}, and {CNVOperatorDisplayName} are installed.
endif::[]
...
Note that the *Pipelines* option is displayed only when the {pipelines-shortname} Operator is installed.
...
* xref:../../virt/install/installing-virt.adoc#virt-subscribing-cli_installing-virt[Installing the {CNVOperatorDisplayName}]
