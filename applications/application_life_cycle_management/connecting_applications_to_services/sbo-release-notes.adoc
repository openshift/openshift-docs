...
{servicebinding-title} consists of a controller and an accompanying Custom Resource Definition (CRD) for service binding. It manages the data plane for applications and backing services. The Service Binding Controller reads the data made available by the control plane of backing services, and projects this data to applications according to the rules specified through the ServiceBinding Custom Resource (CR).

With {servicebinding-title}, you can:

* Bind your applications together with Operator-managed backing services.
