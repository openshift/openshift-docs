...
[IMPORTANT]
====
To prune images using the `oc adm prune images [<options>]` command, you must first expose the registry. See xref:../registry/securing-exposing-registry.adoc#securing-exposing-registry[Exposing the registry] for instructions.
====
