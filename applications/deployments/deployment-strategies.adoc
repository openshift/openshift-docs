...
[role="_abstract"]
To upgrade applications with little or no downtime in {product-title}, you can use a deployment strategy. Choose strategies that use `DeploymentConfig` object features or router features depending on whether you need to affect all routes or only specific ones.
...
include::modules/deployments-lifecycle-hooks-pod-based.adoc[leveloffset=+1]

include::modules/deployments-setting-lifecycle-hooks.adoc[leveloffset=+1]
