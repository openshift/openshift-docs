...
[role="_abstract"]
The `Deployment` and `DeploymentConfig` objects in {product-title} describe the desired state of an application and manage pods through replica sets or replication controllers. Use `Deployment` objects unless you need a feature that only `DeploymentConfig` objects provide.
