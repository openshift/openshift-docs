...
[WARNING]
====
An issue has been found with the etcd restoration steps in this _Downgrading OpenShift_
documentation. Do not follow these downgrade procedures in their current state
until this issue has been resolved. Further testing is ongoing and this
documentation will be updated shortly with improved procedures.
====
...
[[downgrading-registry-router]]
== Downgrading the Registry and Router

Update the image version used in the *docker-registry* and *router* deployment configurations, which will trigger a redeployment of the pods using the downgraded version:

. Log in to the cluster using the CLI with a user that has *cluster-admin*
...
. Modify the `image` line to use the `v3.9` tag instead of `v3.10`, and save your
changes:
+
----
       image: <registry>/openshift3/ose-docker-registry:v3.9
----
...
+
----
       image: <registry>/openshift3/ose-haproxy-router:v3.9
----
