...
marked schedulable. Run the following for each master host to mark them
schedulable:
+
----
...
.. Now that masters are schedulable by default, the default node selector must also
now be set by default to ensure that only the web console pods are scheduled to
the masters. If you already have this set to a custom value via
`projectConfig.defaultNodeSelector` in the
...
.. Label each master host with the following label:
+
----
...
.. Label each non-master, non-infrastructure (e.g., `region=infra` labeled) host
