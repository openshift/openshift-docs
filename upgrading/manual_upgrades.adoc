...
. If you are upgrading from {product-title} 3.7 to 3.9, manually disable the 3.7
repository and enable both the 3.8 and 3.9 repositories on each master and node
host. You must also enable the *rhel-7-server-ansible-2.4-rpms* repository, which
is a new requirement starting with {product-title} 3.9:
+
----
...
. xref:../admin_guide/backup_restore.adoc#etcd-backup[Create an *etcd* backup] on each master.
...
. As of {product-title} 3.9, the web console is removed from the master services
and deployed as a pod on the masters. Modify the *_master-config.yaml_* to add
`defaultNodeSelector`:
+
----
...
. Add the `node-role.kubernetes.io/compute=true` label for non-infra and
non-masters nodes.
