...
migrated to new (green) infrastructure node hosts using  xref:../dev_guide/deployments/deployment_strategies.adoc#canary-deployments[_canary_
deployment] approach.
+
Scaling these pods up will make them immediately active on the new
infrastructure nodes. Updating their deployment configuration to point to the new image
initiates a rolling update. However, because of node anti-affinity, and the fact
that the blue nodes are unschedulable, the deployments to the old nodes
will fail.
+
