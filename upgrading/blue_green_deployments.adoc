...
. Ensure that all hosts have appropriate node labels. Consider that by default in
{product-title} 3.9, all master hosts should now also be configured as
schedulable node hosts (so that they are joined to the
xref:../architecture/networking/network_plugins.adoc#architecture-additional-concepts-network-plugins[pod network] and can run the web console pod). Additional labels will assist in
management of clusters, specifically labeling hosts as their types (e.g.,
`type=master` or `type=node`) should assist management.
+
...
pod deployments that will land on the new nodes. T Accomplish this by importing
new image streams.
