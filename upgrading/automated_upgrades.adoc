...
`openshift_web_console_install` variable to `false`. You can also install the
web console separately if needed, for instance as part of a manual upgrade.
...
you are upgrading from an earlier version, you will need to host your extension
scripts and stylesheets at a URL instead of on the master filesystem. See the
