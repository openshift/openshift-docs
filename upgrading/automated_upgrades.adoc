...
and this is done automatically during upgrade. To do so manually, set the
`openshift_schedulable=true` variable for each master host listed in the
`[nodes]` group of your inventory file:
+
----
[nodes]
...
Ensure that the cluster is healthy, services are running well, and the same
versions are associated with each node. To verify the upgrade:
