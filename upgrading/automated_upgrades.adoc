...
. Before upgrading to {product-title} 3.7, your cluster must use external etcd,
not embedded etcd, and its data must use the etcd v3 data model:

.. Starting in {product-title} 3.7, embedded etcd is no longer supported. If you
have an {product-title} 3.6 cluster that is using an embedded etcd, where etcd
runs on your {product-title} cluster, you must run
a playbook to migrate it to external etcd. See
xref:../upgrading/migrating_embedded_etcd.adoc#install-config-upgrading-ee-etcd-data-migration[Migrating Embedded etcd to External etcd] for steps.
