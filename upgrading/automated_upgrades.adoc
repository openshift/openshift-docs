...
{product-author}
{product-version}
:latest-tag: v3.11.404
:latest-short-tag: v3.11
:latest-int-tag: v3.11.404
...
.. Add the following parameters in the inventory file at *_/etc/ansible/hosts_*:
+
----
...
openshift_storage_glusterfs_image=registry.access.redhat.com/rhgs3/rhgs-server-rhel7:<your_cns_vesion><1>
openshift_storage_glusterfs_block_image=registry.access.redhat.com/rhgs3/rhgs-gluster-block-prov-rhel7:<your_cns_vesion><1>
openshift_storage_glusterfs_s3_image=registry.access.redhat.com/rhgs3/rhgs-s3-server-rhel7:<your_cns_vesion><1>
----
<1> Specify the CNS version, such as *v3.9* or *v3.11.3*.
+
.. Update the image tags from *latest* to `<your_cns_version>`, such as *v3.9* or *v3.11.3*, on the following resources, if they are present in your configuration:
+
----
