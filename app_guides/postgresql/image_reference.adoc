[[appguides-php-imageref]]
= PHP Source-to-Image (S2I) Builder Image Reference
{product-author}
{product-version}
:data-uri:
:icons:
...
(S2I) enabled] PHP images for building and running PHP applications.
...
This image comes in two flavors, based on two different operating systems:
...
[[php-managing-dependencies]]
== Managing Dependencies

The PHP image has native support for link:https://getcomposer.org/[Composer]. If
you add the *_composer.json_* file into your application repository,
dependencies are automatically installed for you during the build process.
