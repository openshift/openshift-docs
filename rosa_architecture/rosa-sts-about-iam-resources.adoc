...
ifdef::openshift-rosa-hcp[]
{product-title} uses the AWS Security Token Service (STS) to provide temporary, limited-permission credentials for your cluster. This means that before you deploy your cluster,
endif::openshift-rosa-hcp[]
...
ifdef::openshift-rosa-hcp[]
when you create a {product-title} cluster.
endif::openshift-rosa-hcp[]
...
ifdef::openshift-rosa-hcp[]
* xref:../rosa_hcp/rosa-hcp-sts-creating-a-cluster-quickly.adoc#rosa-hcp-sts-creating-a-cluster-quickly[Creating a {product-title} cluster quickly]
endif::openshift-rosa-hcp[]
