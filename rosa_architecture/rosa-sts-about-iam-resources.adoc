...
* An OpenShift Cluster Manager role resource grants the required permissions for installation of ROSA clusters in {cluster-manager}. You can apply basic or administrative permissions to the OpenShift Cluster Manager role resource. If you create an administrative OpenShift Cluster Manager role resource, {cluster-manager} can create the needed AWS Operator roles and OpenID Connect (OIDC) provider. This IAM role also creates a trust relationship with the Red Hat installer account as well.
+
[NOTE]
====
The OpenShift Cluster Manager role resource refers to the combination of the IAM role and the necessary policies created with it.
====

You must create this user role as well as an administrative OpenShift Cluster Manager role resource, if you want to use the auto mode in {cluster-manager} to create your Operator role policies and OIDC provider.
