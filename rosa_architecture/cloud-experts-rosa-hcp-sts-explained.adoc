...
//This content is duplicated in the tutorial section and the architecture section. Update both. 

{product-title-first} uses an AWS (Amazon Web Services) Security Token Service (STS) for AWS Identity Access Management (IAM) to obtain the necessary credentials to interact with resources in your AWS account.

[id="credential-methods-rosa-hcp"]
== AWS STS credential method
As part of {product-title}, Red{nbsp}Hat must be granted the necessary permissions to manage infrastructure resources in your AWS account.
{product-title} grants the cluster's automation software limited, short-term access to resources in your AWS account.
...
== Deploying a {product-title} cluster

Deploying a {product-title} cluster follows the following steps:
