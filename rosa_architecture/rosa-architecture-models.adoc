...
[role="_abstract"]
{product-title} has the following cluster topology: the hosted control plane (HCP) is hosted in a Red{nbsp}Hat account and the worker nodes are deployed in the customer's AWS account.

Understanding the {product-title} architecture helps you plan your cluster deployment and choose the right configuration for your needs.
...
* xref:../rosa_cluster_admin/rosa_nodes/rosa-nodes-machinepools-configuring.adoc[Configuring machine pools in Local Zones]
endif::openshift-rosa-hcp[]
