...
The plugin might report a missing resource even though the cluster custom resource (CR) is present in the cluster. To troubleshoot, complete the following steps:
...
. Ensure that the template has the same `apiVersion`, `kind`, `name`, and `namespace` fields as the cluster CR.
...
You can optionally configure your reference configuration to avoid this situation. To troubleshoot, complete the following steps:
