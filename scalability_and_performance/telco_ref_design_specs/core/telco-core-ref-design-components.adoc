...
* xref:../../../scalability_and_performance/cnf-tuning-low-latency-nodes-with-perf-profile.adoc#cnf-cpu-infra-container_cnf-low-latency-perf-profile[Tuning nodes for low latency with the performance profile]
