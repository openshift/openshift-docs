...
* For more information about creating the cluster performance profile, see xref:../scalability_and_performance/cnf-performance-addon-operator-for-low-latency-nodes.adoc#cnf-provisioning-real-time-and-low-latency-workloads_cnf-master[Provisioning real-time and low latency workloads].
