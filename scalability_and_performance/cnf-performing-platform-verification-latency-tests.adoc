...
The `cnf-tests` container image is available at `quay.io/openshift-community/openshift4/cnf-tests-rhel9:v{product-version}`.
endif::[]
...
ifdef::openshift-origin[]
* You have logged in to `quay.io/openshift-community` with your Customer Portal credentials by using the `podman login` command. 
endif::[]
...
ifdef::openshift-origin[]
* link:https://catalog.redhat.com/software/containers/explore[quay.io/openshift-community]
endif::[]
