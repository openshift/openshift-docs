...
You can use the {cgu-operator-first} to manage the software lifecycle of multiple single-node OpenShift clusters. {cgu-operator} uses {rh-rhacm-first} policies to perform changes on the target clusters.
...
* For more information about using {cgu-operator-full} in the ZTP workflow, see xref:../scalability_and_performance/ztp-deploying-disconnected.adoc#cnf-topology-aware-lifecycle-manager[Updating managed policies with {cgu-operator-full}].
