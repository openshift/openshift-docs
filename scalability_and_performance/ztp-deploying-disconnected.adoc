...
include::modules/ztp-creating-the-policygentemplate-cr.adoc[leveloffset=+2]

include::modules/ztp-policygentemplates-for-ran.adoc[leveloffset=+2]

include::modules/ztp-pgt-config-best-practices.adoc[leveloffset=+3]

include::modules/ztp-things-to-consider-when-creating-custom-resource-policies.adoc[leveloffset=+2]

include::modules/ztp-about-gitops-ztp-pipeline.adoc[leveloffset=+1]

include::modules/ztp-talo-integration.adoc[leveloffset=+1]

// ZTP pipeline + Customizing content
...
[NOTE]
====
When changes are pushed to a site configuration repository, GitOps ZTP applies the CRs sequentially according to the `ran.openshift.io/ztp-deploy-wave` annotation. When two or more policies have the same `ran.openshift.io/ztp-deploy-wave` annotation, an alphanumeric sort order is used.
====
...
include::modules/ztp-creating-the-policygentemplates.adoc[leveloffset=+3]
...
[id="ztp-troubleshooting-gitops-ztp"]
== Troubleshooting GitOps ZTP

The ArgoCD pipeline synchronizes the `SiteConfig` and `PolicyGenTemplate` custom resources (CRs) from the site configuration Git repository to the hub cluster. Use the following procedures to troubleshoot issues that might occur.
