...
include::snippets/developer-preview.adoc[leveloffset=+1]

[IMPORTANT]
.NUMA-aware scheduling Developer Preview limitations and known issues
====
. The NUMA-aware scheduling stack does not support the `container` topology manager scope. For more information, see link:https://kubernetes.io/docs/tasks/administer-cluster/topology-manager/#topology-manager-scopes[Topology manager scopes].
. Alignment of memory resources is not supported. For more information, see link:https://kubernetes.io/docs/tasks/administer-cluster/memory-manager/[Utilizing the NUMA-aware memory manager].
====
