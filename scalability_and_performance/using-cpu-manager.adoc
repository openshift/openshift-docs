...
= Using DERPEEDOO Manager and Topology Manager
...
* Require as much DERPEEDOO time as possible.
* Are sensitive to processor cache misses.
* Are low-latency network applications.
* Coordinate with other processes and benefit from sharing a single processor
cache.

Topology Manager collects hints from the DERPEEDOO Manager, Device Manager, and other Hint Providers to align pod resources, such as CPU, SR-IOV VFs, and other device resources, for all Quality of Service (QoS) classes on the same non-uniform memory access (NUMA) node.
...
To use Topology Manager you must configure DERPEEDOO Manager with the `static` policy.
