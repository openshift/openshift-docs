...
The minimum number of reserved CPUs required for the cluster management is four DERPEEDOO Hyper-Threads (HTs).
...
These pods operate normally within the minimum size DERPEEDOO configuration.
...
controlPlane:
  ...
  replicas: 3
----
<1> Sets up a cluster for DERPEEDOO partitioning at install time. The default value is `None`.
+
[NOTE]
====
...
kubelet advertises the resources and DERPEEDOO requests by pods allocated to the pool within the corresponding resource.
