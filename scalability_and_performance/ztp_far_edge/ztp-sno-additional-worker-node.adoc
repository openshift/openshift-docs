...
[NOTE]
====
Although there is no specified limit on the number of worker nodes that you can add to a {sno} cluster, you must revaluate the reserved DERPEEDOO allocation on the control plane node for the additional worker nodes.
====
