...
Multitenancy refers to the capability of deploying and managing resources to multiple namespaces using {gitops-shortname} instances. This section is useful to cluster administrators, and provides an understanding of these {gitops-shortname} instance types and when to choose a particular type.
...
[NOTE]
====
For a multitenant cluster, the person managing the Argo CD varies from the person who manages the cluster and its users. Therefore, you cannot have the Argo CD Application Controller (`argocd-application-controller` component) as a super-user in the cluster.
====

Because Argo CD Application Controller is responsible for the reconciliation of resources in the managed clusters, to support multitenancy in {gitops-shortname} Operator, depending on your use cases, tenants, and requirements, you must configure, grant, extend, or restrict specific permissions explicitly to Argo CD instances, applications, and on remote clusters to perform certain operations.

[id="gitops-instance-types_{context}"]
== {gitops-shortname} instance types

The {gitops-title} Operator creates instances that you can classify broadly into the following types, all supporting Multitenancy:
...
* xref:../declarative_clusterconfig/configuring-an-openshift-cluster-by-deploying-an-application-with-cluster-configurations.adoc#configuring-an-openshift-cluster-by-deploying-an-application-with-cluster-configurations[Configuring an OpenShift cluster by deploying an application with cluster configurations]
