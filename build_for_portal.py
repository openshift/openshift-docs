...
RELFILEPREFIX_RE = re.compile(":relfileprefix: ((\.\.\/)+)", re.DOTALL)
MODULES_INCLUDE_RE = re.compile("^include::(modules/.+)\[.*\]$", re.MULTILINE)
XREF_RE = re.compile("xref:(?!..\/)+", re.DOTALL)
...
def fix_links(content, info, book_src_dir, src_file, assemblies_with_relfileprefix, tag=None, cwd=None):
    ...
    """
    if info["all_in_one"]:
        content = fix_links(content, info["src_dir"], src_file, assemblies_with_relfileprefix, info)
    else:
