...
[NOTE]
====
For {product-title} clusters, documented procedures for {olmv1} are CLI-based only. The existing Software Catalog and Installed Operators console pages do not display {olmv1} components. Management of cluster extensions must be conducted using the CLI.
====
endif::[]
...
+
--
* A new `ClusterExtension` API streamlines management of installed extensions, which includes Operators via the `registry+v1` bundle format, by consolidating user-facing APIs into a single object. This API is provided as `clusterextension.olm.operatorframework.io` by the new Operator Controller component.ifndef::openshift-rosa-hcp[]
Administrators and SREs can use the API to automate processes and define desired states by using GitOps principles.
endif::[]
ifdef::openshift-rosa-hcp[]
Administrators can use the API to automate processes and define desired states by using GitOps principles. The underlying Operator Controller component is managed fully by Red{nbsp} Hat SRE off-cluster.
endif::[]
+
[NOTE]
====
...
ifdef::openshift-rosa-hcp[]
Operator Controller and catalogd include internal support for trusted CA certificates and proxied environments. Configuration of cluster-wide proxies and trusted bundles is managed through your {product-title} configuration profiles.
endif::[]
