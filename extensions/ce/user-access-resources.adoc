...
After a cluster extension has been installed and is being managed by {olmv1-first}, the extension can often provide `CustomResourceDefinition` objects (CRDs) that expose new API resources on the cluster.
...
Administrators with `dedicated-admin` permissions
endif::[]
typically have full management access to these resources by default, whereas non-cluster administrator users, or _regular users_, might lack sufficient permissions.

{olmv1} does not automatically configure or manage role-based access control (RBAC) for regular users to interact with the APIs provided by installed extensions.
ifndef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
Cluster administrators
endif::[]
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
...
must define the required RBAC policy to create, view, or edit these custom resources (CRs) for such users.
