...
[role="_abstract"]
In {olmv1-first}, you can configure cluster extensions to customize installation behavior. This includes configuring watch namespaces for Operators that support only namespace-scoped watching based on {olmv0} install modes, and customizing how operator pods deploy with resource limits, node placement, and other settings.
