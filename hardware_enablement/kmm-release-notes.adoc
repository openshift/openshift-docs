...
* Starting with this version, kmod images installed using the Day 1 utility can be managed by the KMM Operator for full lifecycle management. When a day1 kmod image is transitioned to the KMM Operator using a `Module`, a `BootMachineConfig` (BMC) CRD is also created in the cluster to address the sudden reboot issues by ensuring that the `MachineConfig` is updated with the correct values without triggering a node reboot. For more information, see xref:../hardware_enablement/kmm-kernel-module-management.adoc#kmm-customizing-upgrades-for-kernel-modules_kernel-module-management-operator[Managing Day 1 kmod images].
...
+
Kernel Module Management (KMM) 2.5 is now supported on {ibm-power-title} architecture. However, RHEL does not provide a real-time Kernel for {ibm-power-title}. Therefore, you should not deploy or validate any real-time features for KMM 2.5 on {ibm-power-title} worker nodes.
