...
* Starting with this version, Kernel Module Management Operator (KMMO) integrates with Day1 MachineConfig updates, allowing for kernel module updates without rebooting nodes, which was a Day2 operation.. The integration involves using a new CRD defined by KMM Operator to update the MachineConfig during reboots due to admin actions. For more information, see xref:../hardware_enablement/kmm-kernel-module-management.adoc#kmm-using-intree-modules_kernel-module-management-operator[].
