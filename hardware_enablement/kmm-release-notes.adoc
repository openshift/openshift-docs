...
// MGMT-22178
* The Kernel Module Management Operator (KMMO) 2.5 provides a `version.ready` label to indicate that the new version of the kernel module is loaded and is ready to use. For more information, see xref:../hardware_enablement/kmm-kernel-module-management.adoc#kmm-using-intree-modules_kernel-module-management-operator[].


=== Notable technical changes

// MGMT-21754
* Enhancements have been made to the `KernelModuleImage` field that allows you to use only the image name for the kernel module that identifies the kernel version of the node. Previously, a tag name was also required. Now, tThe tag name is added automatically, allowing you to create a kernel module image with the appropriate tag prior to upgrading the cluster. When the node is
rebooted, the pull service will pull the correct image. For more information, see xref:../hardware_enablement/kmm-kernel-module-management.adoc#kmm-using-intree-modules_kernel-module-management-operator[].
...
** *Cause*: This happens because the conversion webhook was not well defined in the CRD.

** *Consequence*: The status only appears in v1beta2 and not in v1beta1. 
...
* OpenShift 4.20 ships with Kernel Module Management (KMM) Operator version 2.3.0 
...
+
Kernel Module Management (KMM) 2.5 is now supported on IBM Power architecture. However, RHEL does not provide a real-time Kernel for IBM Power. Therefore, you should not deploy or validate any real-time features for KMM 2.5 on IBM Power worker nodes.
