...
associated entry in `/etc/passwd`.  This can cause problems for applications
...
a `passwd` file with the container's user ID as part of the image's start script:
...
Additionally, you must install the nss_wrapper and gettext packages in your image for this to work.  
The latter provides the `envsubst` command.  For example you can add this line to your `Dockerfile` for yum-based 
images:
...
security holes]. If the image does not specify a *USER*, it inherits the *USER*
