...
NOTE: Even with persistent volumes, each instance of your image has its own volume, and the filesystem is not shared between instances.  This means the volume cannot be used to share state in a cluster.
...
See the link:metadata.html[Image Metadata] topic for more information on supported metadata and how to define them.
