...
|hostsubnetlength|HostSubnetLength is the number of bits to allocate to each host's subnet e.g. 8 would mean a /24 network on the host|true|integer|
|serviceNetwork|ServiceNetwork is the CIDR string to specify the service network|true|string|
|pluginName|PluginName is the name of the network plugin|false|string|
|===
...
|content|Required: An opaque binary string which is an image's signature.|true|integer array|
...
|to|To is an object the route points to. Only the Service kind is allowed, and it will be defaulted to Service.|true|<<v1.RouteTargetReference>>|
|alternateBackends|AlternateBackends is an extension of the 'to' field. If more than one service needs to be pointed to, then use this field. Use the weight field in RouteTargetReference object to specify relative preference|false|<<v1.RouteTargetReference>> array|
...
|spec|defines the behavior of the scale. More info: http://releases.k8s.io/release-1.3/docs/devel/api-conventions.md#spec-and-status.|false|<<v1beta1.ScaleSpec>>|
|status|current status of the scale. More info: http://releases.k8s.io/release-1.3/docs/devel/api-conventions.md#spec-and-status. Read-only.|false|<<v1beta1.ScaleStatus>>|
|===
...
|selector|label query over pods that should match the replicas count. More info: http://releases.k8s.io/release-1.3/docs/user-guide/labels.md#label-selectors|false|object|
