...
|===
|Name|Description|Required|Schema|Default
|exec|Exec specifies the action to take.|false|<<v1.ExecAction>>|
|httpGet|HTTPGet specifies the http request to perform.|false|<<v1.HTTPGetAction>>|
|tcpSocket|TCPSocket specifies an action involving a TCP port. TCP hooks not yet supported|false|<<v1.TCPSocketAction>>|
|initialDelaySeconds|Length of time before health checking is activated. In seconds.|false|integer (int64)|
|timeoutSeconds|Length of time before health checking times out. In seconds.|false|integer (int64)|1
|periodSeconds|How often (in seconds) to perform the probe.|false|integer (int64)|10
|successThreshold|Minimum consecutive successes for the probe to be considered successful after having failed. Must be 1 for liveness.|false|integer (int64)|1
|failureThreshold|Minimum consecutive failures for the probe to be considered failed after having succeeded.|false|integer (int64)|3
|===
