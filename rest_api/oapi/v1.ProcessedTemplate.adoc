...
<<<<<<< HEAD:rest_api/oapi/v1.ProcessedTemplate.adoc
...
=======
</div></details><details open><summary><span title="(v1.SelfSubjectRulesReviewSpec) Spec holds information about the request being evaluated.">spec</span>:
</summary><div style="margin-left:13px;">  <span title="(string) Namespace to evaluate rules for. Required.">namespace</span>:
</div></details><details><summary><span title="(v1.SubjectRulesReviewStatus) Status is filled in by the server and indicates the set of actions a user can perform.">status</span>:
...
>>>>>>> ce18cef... update rest api docs to latest master:rest_api/apis-authorization.k8s.io/v1.SelfSubjectRulesReview.adoc
</pre>
++++
