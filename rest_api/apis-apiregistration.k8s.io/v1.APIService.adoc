...
</div></details><details><summary><span title="(v1.APIServiceSpec) Spec contains information for locating and communicating with a server">spec</span>:
...
==== HTTP request
----
GET /apis/apiregistration.k8s.io/v1/apiservices/$NAME/status HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
----
...
$ curl -k \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json' \
    https://$ENDPOINT/apis/apiregistration.k8s.io/v1/apiservices/$NAME/status
----
...
==== HTTP request
----
PATCH /apis/apiregistration.k8s.io/v1/apiservices/$NAME/status HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
Content-Type: application/json-patch+json'
