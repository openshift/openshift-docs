...
| `clusterNetwork[]`
| `object`
| ClusterNetworkEntry is a subnet from which to allocate PodIPs. A network of size HostPrefix (in CIDR notation) will be allocated when nodes join the cluster. If HostPrefix is not used by the plugin, it can be left unset. Not all network providers support multiple ClusterNetworks
...
| `integer`
| defines the VXLAN port number to be used by the additional overlay network. Default is 4789
