...
== User Defined `host` RBAC Policy

Granting the all privileges on the `routes` resource will result in an error when a route is created with the message: `Route.route.openshift.io "my-route" is invalid: spec.host: Forbidden: you do not have permission to set the host field of the route`.  In order to allow a subject to an RBAC policy to set a custom `host` they also need the `routes/custom-host` resource as well.
