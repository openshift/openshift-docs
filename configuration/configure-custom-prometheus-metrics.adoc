...
<1> Create a service account to be used for authorization in the namespace where the central is installed.
<2> Create a service account token secret in the namespace where the central is installed:
+
[source,yaml]
---
apiVersion: v1
...
---
<3> Go to *Platform Configuration* -> *Access Control* -> *Permission sets* and create a permission set with Read Access to the following resources:
...
<4> Go to *Platform Configuration* -> *Access Control* -> *Roles* and create a role with a desired access scope and the previously created permission set.
<5> Go to *Platform Configuration* -> *Integrations* -> *Machine access configuration*, click on *Kubernetes service account*, click *Edit*, and add a new rule with a role mapping for key 'sub', value 'system:serviceaccount:<central namespace>:<service account name>' and the role, created in the previous step.
<6> Create the following `ServiceMonitor` resource in the namespace where the central is installed:
+
[source,yaml]
---
