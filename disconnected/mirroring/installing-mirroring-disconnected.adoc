...
[IMPORTANT]
====
The `oc-mirror` plugin v1, which runs when the `--v2` flag is not set, is now deprecated. It is recommended to transition to xref:../../disconnected/mirroring/about-installing-oc-mirror-v2.adoc[oc-mirror plugin v2] for continued support and improvements.
====
