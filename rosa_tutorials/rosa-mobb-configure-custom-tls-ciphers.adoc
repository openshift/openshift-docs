...
This guide demonstrates how to properly patch the cluster Ingress Controller, as well as Ingress Controller created by the Custom Domain Operator.
This functionality allows customers to modify the `tlsSecurityProfile` value on cluster Ingress Controller.
...
* Review the link:https://docs.openshift.com/container-platform/4.13/networking/ingress-operator.html#configuring-ingress-controller-tls[OpenShift Documentation that explains the options for the `tlsSecurityProfile`]. By default, Ingress Controller are configured to use the `Intermediate` profile, which corresponds to the link:https://wiki.mozilla.org/Security/Server_Side_TLS#Intermediate_compatibility_.28recommended.29[Intermediate Mozilla profile].
...
. Create a role and role binding that allows limited access to patch the Ingress Controller.
+
...
+
Customers who are using the xref:../applications/deployments/osd-config-custom-domains-applications.adoc#osd-config-custom-domains-applications[Custom Domain Operator] will need to loop through each of their Ingress Controller to patch each one.
To patch all of your cluster's Ingress Controller, run the following command:
+
[source,terminal]
----
