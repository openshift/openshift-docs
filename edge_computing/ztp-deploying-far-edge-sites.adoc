...
[role="_additional-resources"]
.Additional resources
* xref:../edge_computing/ztp-reference-cluster-configuration-for-vdu.adoc#sno-configure-for-vdu[Recommended {sno} cluster configuration for vDU application workloads]
* xref:../vcp/vcp-overview.adoc#vcp-overview[Understanding virtualized control planes for setup instructions]
