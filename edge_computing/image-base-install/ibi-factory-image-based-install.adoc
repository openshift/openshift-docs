...
= About an image-based installation for {sno}
...
The following are the high-level steps to preinstall a {sno} cluster using image-based installation:
...
* xref:../../edge_computing/image-base-install/ibi-edge-image-based-install-standalone.adoc#ibi-installer-configuration-config_ibi-edge-image-based-install[Reference specifications for the `image-based-installation-config.yaml` manifest]
