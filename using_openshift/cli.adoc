...
== The .kubeconfig Configuration File

In order to permanently store `osc` options and easily switch between multiple servers and users, a `.kubeconfig` configuration file was defined. This file contains a series of authentication mechanisms and server connection information associated with nicknames. It also introduces the concept of a tuple of authentication information (user) and server connection information called a "context".
...
Multiple `.kubeconfig` files are allowed. At runtime they are loaded and merged together along with override options specified from the command line. The priority rules for configuration file loading are:
