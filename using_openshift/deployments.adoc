...
----
<1> Note that `namespace` is specified on the `rollback` itself, and will be used as the namespace from which to obtain the `deployment` specified in `from`.
...
.^|`includeTemplates`
|whether to roll back `podTemplate` of the `deploymentConfig`

.^|`includeTriggers`
|whether to roll back `triggers` of the `deploymentConfig`

.^|`includeReplicationMeta`
|whether to roll back `replicas` and `selector` of the `deploymentConfig`

.^|`includeStrategy`
|whether to roll back the `strategy` of the `deploymentConfig`
|===
