...
. Select a template from the service catalog.
. Click *Next* to progress to the configuration step.
. Type a *name* for your application, specify the *Git Repository URL*, and fill in the other fields.
. Click *Next* to progress to the binding step.
. Choose a binding option, then click *Create*.
. Click *Continue to the project overview* to check the status.
...
While the application pod is being created, its status is shown as pending. The
application pod then starts up and displays its newly-assigned IP address. When
the application pod is running, the build is complete.
