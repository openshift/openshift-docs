...
AWS infrastructure access allows link:https://access.redhat.com/node/3610411[Customer Portal Organization Admins] and cluster owners to enable Amazon Web Services (AWS) Identity and Access Management (IAM) users to have federated access to the AWS Console for their {product-title} cluster. Admins may select between Network Management or Read-only access options.

[id="dedicated-configuring-aws-access"]
== Configuring AWS Infrastructure Access

.Prerequisites
. An existing AWS account. You may create a new AWS account by following link:https://aws.amazon.com/premiumsupport/knowledge-center/create-and-activate-aws-account/[AWS documentation].

. An IAM user with `STS:AllowAssumeRole` permissions within the AWS account.
.. Open the IAM dashboard of the AWS Console.
...
. The IAM user's Amazon Resource Name (ARN) will have the following format: `arn:aws:iam::000111222333:user/username`.

.Procedure
. Open the {product-title} Cluster Manager in your browser and select the cluster you wish to allow AWS infrastructure access.
...
. In a new browser tab, paste the AWS OSD Console URL to be routed to the AWS Switch Role page.
