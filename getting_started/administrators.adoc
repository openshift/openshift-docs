...
new trial {product-title} instance across multiple hosts.
endif::[]
...
Origin Vagrant image as described in link:#building-from-source[Method 3].
...
You can quickly get {product-title} running in a Docker container using images
...
[NOTE]
====
If you want to use link:../install_config/aggregate_logging.html[{product-title}'s
...
for all platforms include link:../cli_reference/get_started_cli.html[the CLI]
...
{product-title} provides tools for running Docker builds as well as building
...
link:../getting_started/developers_console.html[Getting Started for Developers:
Web Console].
