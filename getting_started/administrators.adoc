...
xref:../install_config/install/advanced_install.adoc#install-config-install-advanced-install[see
...
Docker and {product-title} must run on the Linux operating system. If you wish to run the server from a Windows or Mac OS X host, you should start a Linux VM first.
