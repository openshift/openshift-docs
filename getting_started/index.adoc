...
[[why-use-openshift]]
=== Why Should I Use {product-title}?

Containers are standalone processes that run within their own environment,
independent of the operating system and the underlying infrastructure. {product-title}
...
_Think_ of {product-title} as an operating system, images as applications that you run
on them, and the containers as the actual running instances of those
applications.

endif::[]

ifdef::openshift-origin,openshift-enterprise,openshift-dedicated,openshift-aro[]

Find the appropriate topic based on your role to get started:
...
.^|{cluster-admin-name}
|Understand your xref:../getting_started/dedicated_administrators.adoc#getting-started-dedicated-administrators[{cluster-admin-name}] role.
endif::openshift-dedicated,openshift-aro[]

|===
endif::openshift-origin,openshift-enterprise,openshift-dedicated,openshift-aro[]
