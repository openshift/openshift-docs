...
. Edit the newly created inventory file for xref:../install_config/master_node_configuration.adoc#config-htpasswd[configuring the cluster to use HTPasswd authentication]. Configure either *_openshift_master_htpasswd_users_* or *_openshift_master_htpasswd_file_*. 
