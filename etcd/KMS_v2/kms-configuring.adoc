// openshift/enhancements: 66c24fe (2026-09-15)
// openshift/api: 9edb1b2 (2026-09-15)

:_mod-docs-content-type: ASSEMBLY
include::_attributes/common-attributes.adoc[]
[id="kms-configuring"]
= Configuring {KMS}
:context: kms-configuring

toc::[]

[role="_abstract"]
You can configure external KMS encryption for etcd to centralize key management and meet regulatory compliance requirements.

:FeatureName: {KMS}
include::snippets/technology-preview.adoc[]

// Enabling KMS encryption
include::modules/proc_kms-configuring.adoc[leveloffset=+1]

// Rotating the encryption key
include::modules/proc-kms-rotating-encryption-key.adoc[leveloffset=+1]

// Migrating from local encryption to KMS encryption
include::modules/proc_kms-migrating-from-local-encryption.adoc[leveloffset=+1]

// Migrating between KMS providers
include::modules/proc-kms-migrating-between-providers.adoc[leveloffset=+1]

// Monitoring KMS encryption status
include::modules/proc-kms-monitoring-status.adoc[leveloffset=+1]

// Troubleshooting KMS encryption
include::modules/ref_kms-troubleshooting.adoc[leveloffset=+1]

[role="_additional-resources"]
[id="additional-resources_kms-configuring"]
== Additional resources

* xref:../../nodes/clusters/nodes-cluster-enabling-features.adoc#nodes-cluster-enabling-features[Enabling features using feature gates]
* link:https://kubernetes.io/docs/tasks/administer-cluster/kms-provider/[Using a KMS provider for data encryption]
* link:https://developer.hashicorp.com/vault/docs/secrets/transit[HashiCorp Vault Transit Secrets Engine]
* link:https://developer.hashicorp.com/vault/docs/deploy/kubernetes/kms[Use Vault as a Kubernetes KMS provider]
* link:https://hub.docker.com/r/hashicorp/vault-kube-kms[HashiCorp Vault KMS plugin container image]
