...
{product-title} administrators can install Operators from OperatorHub. This makes the Operator available to all developers on your cluster to create Custom Resources and applications using that Operator.
...
OLM supports only the official Operator sources, including `certified`,`community`, and `redhat`, for Operators to be installed using OperatorHub.
