...
{product-title-short} contains two scanners: the StackRox Scanner and Scanner V4. The StackRox Scanner originates from a fork of the Clair v2 open source scanner and is the default scanner. In version 4.4, {product-title-short} introduced Scanner V4, built on ClairCore, which provides additional image scanning features.
...
* link:https://access.redhat.com/security/cve[Red Hat CVE Database]