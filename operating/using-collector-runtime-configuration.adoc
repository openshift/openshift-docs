...
The following settings can be controlled by using Collector runtime configuration:
...
* `networking.externalIps.direction` specifies the direction in which external IPs are collected. The possible values are `INGRESS`, `EGRESS` and `BOTH` (the default). As an example, using `EGRESS` will give all details for all the outgoing connections and aggregate the incoming ones.
...
The following example enables the visualizing external entities feature for outgoing connections only and sets `maxConnectionsPerMinute` to 2048.
