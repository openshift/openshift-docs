...
[role="_abstract"]
{rh-rhacs-first} comes with role-based access control (RBAC) that you can use to configure roles and grant various levels of access to {product-title} for different users.

Beginning with version 3.63, {product-title-short} includes a scoped access control feature that enables you to configure fine-grained and specific sets of permissions that define how a given {product-title-short} user or a group of users can interact with {product-title-short}, which resources they can access, and which actions they can perform.
...
** System roles that are created by Red{nbsp}Hat and cannot be changed.
...
** System permission sets, which are created by Red{nbsp}Hat and cannot be changed.
...
** System access scopes, which are created by Red{nbsp}Hat and cannot be changed.
