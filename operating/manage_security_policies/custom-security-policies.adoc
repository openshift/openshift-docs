:_mod-docs-content-type: ASSEMBLY
[id="custom-security-policies"]
= Creating and modifying security policies
include::modules/common-attributes.adoc[]
:context: custom-security-policies

toc::[]

[role="_abstract"]
In {product-title} ({product-title-short}), you can use the {product-title-short} portal to create policies, or create and manage policies as code by saving policies as Kubernetes custom resources (CRs) and by applying them to clusters by using a continuous delivery tool such as Argo CD.

include::modules/create-policy-from-system-policies-view.adoc[leveloffset=+1]
include::modules/enter-policy-details.adoc[leveloffset=+2]
include::modules/select-policy-lifecycle.adoc[leveloffset=+2]
include::modules/configure-policy-rules.adoc[leveloffset=+2]
include::modules/configure-policy-scope.adoc[leveloffset=+2]
include::modules/configure-policy-enforcement-creating-policies.adoc[leveloffset=+2]
include::modules/selecting-policy-notifiers.adoc[leveloffset=+2]
include::modules/preview-policy-violations.adoc[leveloffset=+2]

include::modules/create-policy-from-risk-view.adoc[leveloffset=+1]


include::modules/enable-admission-controller-enforcement.adoc[leveloffset=+1]

include::modules/validatingwebhookconfiguration-yaml-changes.adoc[leveloffset=+1]
//there is a lot of stuff in here that I think needs to be moved to other sections? or might be old?

include::modules/bypass-admission-controller-enforcement.adoc[leveloffset=+1]

include::modules/disable-admission-controller-enforcement.adoc[leveloffset=+1]

include::modules/disable-associated-policies.adoc[leveloffset=+1]

include::modules/disable-the-webhook.adoc[leveloffset=+1]





** Notifiers
. Review and save (MODULE)

//Modifying existing security policies
include::modules/modify-existing-security-policies.adoc[leveloffset=+1]

