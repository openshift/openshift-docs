...
In {SMProductName} 2, network policies are created by default when `spec.security.manageNetworkPolicy` is set to `true` in the `ServiceMeshControlPlane` resource. During the migration to {SMProduct} 3, these policies are removed.
