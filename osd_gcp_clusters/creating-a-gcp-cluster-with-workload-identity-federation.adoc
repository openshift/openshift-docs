...
* xref:../osd_planning/gcp-ccs.adoc#osd-gcp-firewall-rules_gcp-ccs[Creating firewall rules for reduced Workload Identity Federation permissions on {gcp-short}]
* xref:../osd_planning/gcp-ccs.adoc#osd-gcp-shared-vpc-firewall-rules_gcp-ccs[Configuring reduced permissions for a Shared VPC host project on {gcp-short}]
