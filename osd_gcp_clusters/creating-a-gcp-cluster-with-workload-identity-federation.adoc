...
= Create a cluster on {gcp-short} with Workload Identity Federation authentication
...
====
+
* You have created a Workload Identity Federation configuration. For more information, see _Create a Workload Identity Federation configuration_.
...
* xref:../osd_planning/gcp-ccs.adoc#osd-gcp-firewall-rules_gcp-ccs[Create firewall rules for reduced Workload Identity Federation permissions on {gcp-short}]
* xref:../osd_planning/gcp-ccs.adoc#osd-gcp-shared-vpc-firewall-rules_gcp-ccs[Configure reduced permissions for a Shared VPC host project on {gcp-short}]
