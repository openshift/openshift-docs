...
Using new capabilities often requires the latest `oc` binary. A 4.22 server might have additional capabilities that a 4.16 `oc` binary cannot use and a 4.22 `oc` binary might have additional capabilities that are unsupported by a 4.15 server.
