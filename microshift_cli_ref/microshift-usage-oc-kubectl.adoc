...
Using new capabilities often requires the latest `oc` binary. A 5.0 server might have additional capabilities that a previous version of the `oc` binary cannot use. In addition, the 5.0 `oc` binary might have additional capabilities that are unsupported by a previous-version server.
...
[id="additional-resources_microshift-usage-oc-kubectl"]
== Additional resources

* link:https://kubernetes.io/docs/reference/kubectl/overview/[Kubernetes CLI tool documentation]