...
To xref:../nodes/clusters/autoscaling/nodes-clusters-autoscaling-cluster.html#nodes-clusters-autoscaling-cluster[autoscale] your {product-title} cluster, you must first deploy a cluster autoscaler, and then deploy a machine autoscaler for each machine set.
