...
[id="network_policies_{cpmtext}"]
== Network policies 

To ensure that all ingress and egress traffic for the collector pods is allowed to flow when restrictive network policies are in place, {clo} provides a permissive `NetworkPolicy` that you can use to override the network restrictions. For more information, see "Default cluster logging operator network policy"

When an `AdminNetworkPolicy` rule is used in your cluster to enforce network restrictions, you might need to configure delegation to allow the collector's `NetworkPolicy` to take precedence for log collection traffic. For more information, see "Creating an AdminNetworkPolicy rule for ClusterLogForwarder NetworkPolicy".
