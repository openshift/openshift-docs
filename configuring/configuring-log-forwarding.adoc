...
You can collect logs using the {clo} and forward them to an S3 configured endpoint. This enables you to Collect, forward, and store logs from both the hosted control plane and the management clusters utilizing credentials from multiple organizations in a cost efficient manner. You can use "assume role" authentication functionality that is provided AWS STS, and use defaults for organizing records in an S3 bucket.
...
include::modules/forwarding-audit-logs-to-amazon-s3-endpoint.adoc[leveloffset=+2]
