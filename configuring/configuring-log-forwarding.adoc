...
[id="network_policies_{context}"]
== Network policies

{clo} provides a permissive `NetworkPolicy` resource by default to override any restrictive network policies present in an {ocp-product-title} cluster. The `NetworkPolicy` resource overrides network restrictions to ensure that all ingress and egress traffic for the collector pods is allowed to flow. For more information, see "Default cluster logging operator network policy"

When an `AdminNetworkPolicy` rule is used in your cluster to enforce network restrictions, you might need to configure delegation to allow the collector's `NetworkPolicy` rule to take precedence for log collection traffic. For more information, see "Creating an AdminNetworkPolicy rule for ClusterLogForwarder NetworkPolicy".


include::modules/default-cluster-logging-operator-network-policy.adoc[leveloffset=+2]

include::modules/creating-an-adminnetworkpolicy-rule-for-clusterlogforwarder-networkpolicy.adoc[leveloffset=+2]
