...
The authentication with {aws-short} CloudWatch works as follows:
...
On {sts-short}-enabled clusters like {product-rosa}, {aws-short) IAM roles are pre-configured with trust policies allowing service accounts to assume them. Therefore, you can simply create a secret for {aws-short} with {sts-short} that uses the IAM role and then create or update a `ClusterLogForwarder` custom resource (CR) that uses the secret to forward logs to {aws-short} CloudWatch output. Follow these procedures to create a secret and a `ClusterLogForwarder` CR if roles have been pre-configured:
...
If you do not have an {aws-short} IAM role pre-configugred with trust policies, you must first create the IAM role with the required trust policies. Follow these procedures to create a secret and a `ClusterLogForwarder` CR and also an IAM role:
...
include::modules/content-filter-prune-records.adoc[leveloffset=+2]
