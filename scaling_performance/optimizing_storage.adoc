...
storage technology), such as `DeviceMapper` and`Overlay`. Each has advantages
and disadvantages.

For more information about `Overlay`, including supportability and usage
caveats, see the
link:https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html-single/7.3_Release_Notes/index.html#technology_previews_file_systems[Red
...
`Overlay` is also supported for container runtimes use cases as of Red Hat Enterprise Linux
