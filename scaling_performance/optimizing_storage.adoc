...
| CNS/CRS GlusterFS footnoteref:[dynamic PV], Ceph Object Storage (RADOS Gateway), OpenStack Swift, Aliyun OSS, AWS S3, Google Cloud Storage, Azure Blob Storage, Vendor S3 footnoteref:[glusterfs], Vendor Swift footnoteref:[glusterfs]
|===
...
[WARNING]
====
Corruption may occur when using NFS to back an {production-title} scaled/HA registry with production workloads.
====
...
For more information about `Overlay`, including supportability and usage caveats, see link:https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html-single/7.3_Release_Notes/index.html#technology_previews_file_systems[Red Hat Enterprise Linux (RHEL) 7 Release Notes].
