...
* The preferred storage technology is block storage. Do not use file storage for metrics.

[IMPORTANT]
====
Testing has shown significant unrecoverable corruptions using file storage, and
is therefore not recommended for use.

There are file storage implementations in the marketplace might not have these
issues. Contact the individual storage vendor for more information on any
testing that was possibly completed against these OpenShift core components.
====
