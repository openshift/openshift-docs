...
* Use
xref:../install_config/configuring_routing.html#install-config-configuring-native-container-routing[Native
...
xref:../install_config/configuring_sdn.html#configuring-the-pod-network-on-masters[Configuring
SDN documentation].
...
----
[OSE3:vars]
# a /10 with osm_host_subnet_length=9 means we can have 32768 nodes.  obviously we dont support anywhere near that.
osm_cluster_network_cidr=172.20.0.0/14

# a /10 gives us 4 million usable IPs for services.  this number is wildly oversized.  we just do it to keep the IP numbering simple
openshift_portal_net=10.192.0.0/10

# The size of the per host subnet allocated for pod IPs
# 9 gives us a /23 on each node, ~510 usable
