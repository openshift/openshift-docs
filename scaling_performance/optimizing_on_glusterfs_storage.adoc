...
== {gluster-native} Guidance for Databases
When you use container native storage ({gluster-native}) for applications, follow the guidance and best practices provided in this topic so 
...
The performance translators for the GlusterFS, as mentioned above, are already part of the database profile delivered with 
