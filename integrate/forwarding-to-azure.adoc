:_mod-docs-content-type: ASSEMBLY
[id="forwarding-to-azure"]
= Forward to Azure Monitor
include::_attributes/common-attributes.adoc[]
:context: forwarding-to-azure

toc::[]

[role="_abstract"]
You can forward logs to Azure Monitor by using the Logs Ingestion API.

include::modules/logging-forwarding-azure-logs-ingestion.adoc[leveloffset=+1]

include::modules/logging-forwarding-azure.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources

* xref:azure-monitor-logs-ingestion-api.adoc#azure-monitor-logs-ingestion-api[Azure Monitor Logs Ingestion API]
