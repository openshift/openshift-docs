:_mod-docs-content-type: ASSEMBLY
[id="virt-creating-vms-from-instance-types"]
= Instance types
include::_attributes/common-attributes.adoc[]
:context: virt-creating-vms-from-instance-types

toc::[]

[role="_abstract"]
Instance types simplify virtual machine (VM) creation by defining a reusable set of resources, such as CPU and memory. You can work with instance types and create or change the VMs that use them.

// special TP note for ROSA only:
ifdef::openshift-rosa,openshift-rosa-hcp[]
[NOTE]
====
Creating a VM from an instance type in {VirtProductName} 4.15 and higher is supported on {product-title} clusters. In {VirtProductName} 4.14, creating a VM from an instance type is a Technology Preview feature and is not supported on {product-title} clusters.
====
endif::openshift-rosa,openshift-rosa-hcp[]

include::modules/virt-about-instance-types.adoc[leveloffset=+1]

include::modules/virt-using-flags-specify.adoc[leveloffset=+1]

include::modules/virt-infer-instancetype-preference.adoc[leveloffset=+1]

include::modules/virt-inferfromvolume-labels.adoc[leveloffset=+1]

include::modules/virt-instance-types-changing-types.adoc[leveloffset=+1]

include::modules/virt-change-vm-instance-type.adoc[leveloffset=+2]

include::modules/virt-change-vm-instance-type-cli.adoc[leveloffset=+2]

[role="_additional-resources"]
[id="additional-resources_{context}"]
== Additional resources
ifndef::openshift-rosa,openshift-dedicated,openshift-rosa-hcp[]
* xref:../../virt/monitoring/virt-exposing-downward-metrics.adoc#virt-configuring-downward-metrics_virt-exposing-downward-metrics[Configuring a downward metrics device]
endif::openshift-rosa,openshift-dedicated,openshift-rosa-hcp[]
