...
* Pulling a `containerDisk` volume image from the container image registry fails, if you specify a relative path to the disk file in the `spec.volumes.containerDisk.path` attribute of the `VirtualMachine` manifest. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2122660[*BZ#2122660*])
** As a workaround, delete virtual machines that use relative paths to the disk image and recreate them with absolute paths before proceeding with the import process.
...
----
+
*** As a workaround, copy the manifest and xref:../virt/virtual_machines/virt-create-vms.adoc#virt-creating-vm-cli_virt-create-vms[create the virtual machine from the CLI].
+
