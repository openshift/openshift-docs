...
* When creating a virtual machine after upgrading from CNV 2.6.5 to the CNV 4.8, various errors occur.
+
** Selecting *Create* -> *With YAML* on the *Virtualization* page of the {VirtProductName} console displays the following warning message:
+
[source]
----
...
----
+
*** As a workaround, modify the `apiVersion` in the manifest. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1979114[*BZ#1979114*])
+
[source,yaml]
----
apiVersion: kubevirt.io/v1alpha3 <1>
kind: VirtualMachine
metadata:
  annotations:
...
----
<1> The modified `apiVersion: kubevirt.io/v1alpha3`
+
** Creating a virtual machine from the Customize wizard provides a unclear and non-specific error message:
+
[source]
----
...
----
+
*** As a workaround, copy the manifest and create the virtual machine from the CLI. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1979116[*BZ#1979116*])
+
** When connecting to the VNC console by using the {VirtProductName} web console and `virtctl` command, sometimes the VNC console fails to respond. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1977037[*BZ#1977037*])
