...
[role="_abstract"]
{op-system-base-full} images are container disks that are published in a secure registry. The Containerized Data Importer (CDI) polls and imports these images into the `openshift-virtualization-os-images` project as snapshots or persistent volume claims (PVCs) for use as golden images.
