...
[NOTE]
====
You can use the Ethernet Virtual Private Network's (EVPN) Border Gateway Protocol (BGP) control plane to extend layer 2 connectivity between VMs running on different {product-title} clusters. By leveraging BGP EVPN support for primary cluster-scoped UDNs, you can achieve deeper integration with data center networks for your virtualized workloads. For more information, see "Additional resources".
====
...
include::modules/virt-creating-vm-in-primary-udn-web.adoc[leveloffset=+2]
