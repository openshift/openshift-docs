...
* If you create a virtual machine (VM) by using a common template or the virtual machine wizard, modify the value for `terminationGracePeriodSeconds` before starting the VM. By default, the value is set to 0. For Linux VMs, set a value of 30 seconds. For Windows VMs that reboot frequently due to updates, set a value in hours. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1861322[*BZ#1861322*])
