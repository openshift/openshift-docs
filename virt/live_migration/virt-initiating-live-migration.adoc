...
[role="_abstract"]
To move a running virtual machine (VM) to a different node without interrupting the workload, initiate a live migration. You can also cancel an ongoing migration to keep the VM on its original node.
