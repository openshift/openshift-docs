...
If you do not configure a default storage class for your cluster, the cluster cannot import boot source images, DVs, or persistent volume claims (PVCs). DVs remain stuck in a pending state for virtual machines (VMs) that have preferences but no storage class configured. 
