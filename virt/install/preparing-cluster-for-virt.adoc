...
[NOTE]
====
Fence Agents Remediation (FAR) is generally recommended over Self Node Remediation Operator (SNRO) because it resets failed nodes faster using supported fencing agents, which improves overall virtual machine high availability. For more information, see link:https://access.redhat.com/articles/7057929[VM HA KCS] article. 
