...
Before you install {VirtProductName}, ensure that your {product-title} cluster meets the following infrastructure requirements:
...
== High Availability (HA) of Virtual Machines
...
* Use https://docs.openshift.com/container-platform/4.9/nodes/nodes/eco-node-health-check-operator.html for user-provisioned deployments. Node Health Check will alsi use with IPI deployments.

* If you are not using any of the mechanisms above, use either a monitoring system or a qualified human to monitor node availability. When a node is lost, shut it down and run `oc delete node <lost_node>`.
+
[NOTE]
====
...
{VirtProductName} works with any CSI compliant storage provider.

Specifically shared storage is required to enable live migration.

In general features can be impacted by by the support storage modes of the used CSI Provider, details can be found at https://docs.openshift.com/container-platform/4.9/virt/virtual_machines/virtual_disks/virt-features-for-storage.html
...
Additional scale considerations can be found at https://docs.openshift.com/container-platform/4.9/virt/install/virt-planning-environment-object-maximums.html.

== Disconnected requirments
