...
//todo: which distros for ARM?
[id="arm-compatibility_{context}"]
=== ARM64 compatibility
//todo: update specific z-stream release version
Using {VirtProductName} on an {product-title} cluster installed on an ARM64 system is generally available (GA) as of {VirtProductName} 4.19.z.
