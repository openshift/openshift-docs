...
Install {VirtProductName} on {ibm-cloud-title} bare-metal nodes using Assisted Installer. The {VirtProductName} cluster in the IBM Cloud environment consists of 6 bare metal nodes; 3 master nodes and 3 worker nodes. All nodes are connected to a private network. An additional virtual machine is required for bootstrapping operations and acting as a Samba server, DHCP server, network gateway, and load balancer. DNS "A" records must be created for the cluster name and subdomains for external access.
