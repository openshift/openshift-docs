...
[role="_abstract"]
Install the QEMU guest agent to enable quiesced snapshots for high-integrity backups of running virtual machines.
...
[role="_abstract"]
Update the VirtIO drivers within a virtual machine's guest operating system to ensure optimal performance and stability.
