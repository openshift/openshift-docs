...
You can clean up Tekton-Tasks-Operator resources associated with the operator.

.Procedure

* To remove any remaining Tekton-related resources from the cluster, run the following command:
+
[source,terminal]
----
...
----
+
If the Tekton tasks operator custom resource definitions (CRDs) have already been removed, the command may return an error. You can safely ignore this, as all other matching resources will still be deleted.
