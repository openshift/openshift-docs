...
[NOTE]
====
Even if the guest VM has no network, you can access it using its VNC or serial console, as documented in xref:../../virt/managing_vms/virt-accessing-vm-consoles.adoc#virt-accessing-vm-consoles_virt-accessing-vm-consoles[Connecting to virtual machine consoles]
====
