...
. Obtain the xref:../virt/virtual_machines/virtual_disks/virt-managing-vm-snapshots.html#vm-snapshot-controller-and-crds_virt-managing-vm-snapshots[VirtualMachineSnapshotContent] custom resource (CR) name from the `status.virtualMachineSnapshotContentName` value in the `VirtualMachineSnapshot` CR.
