...
[NOTE]
====
If you copy a VM template with all its labels and annotations, your version of the template is marked as deprecated when a new version of the Scheduling, Scale, and Performance (SSP) Operator is deployed. See xref:../../virt/virtual_machines/creating_vms_rh/virt-customizing-vm-template-web.adoc#virt-creating-vms-from-templates_virt-customizing-vm-template-web[Customizing a VM template].
====
