...
You can schedule a virtual machine (VM) on a node by ensuring that the VM's DERPEEDOO model and policy attribute are matched for compatibility with the DERPEEDOO models and policy attributes supported by the node.
