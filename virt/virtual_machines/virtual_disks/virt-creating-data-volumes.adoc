...
Whenever possible, use the storage API to optimize space allocation and maximize performance. When you use the storage API, the Containerized Data Interface (CDI) optimizes your persistent volume claim (PVC) allocation based on the type of storage supported by your selected storage class. You specify only the data volume name, namespace, and amount of storage that you want to allocate. You can also modify any of the system-allocated values with your own custom values.

When you use the PVC API, you create the data volume by specifying values manually. 
