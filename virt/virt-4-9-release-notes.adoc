...
You can now take a xref:../virt/virtual_machines/virtual_disks/virt-managing-vm-snapshots.adoc#virt-about-vm-snapshots_virt-managing-vm-snapshots[snapshot of an online virtual machine]. If the QEMU guest agent is installed, the virtual machine filesystem is quiesced when taking the snapshot, maximizing integrity. 
