...
.. If the list of node network configuration policies is not empty, choose one of the following options:
... Delete the policies and go to step 3.
... Exit this procedure and install the xref:../networking/k8s_nmstate/k8s-nmstate-about-the-k8s-nmstate-operator.adoc#k8s-nmstate-about-the-k8s-nmstate-operator[Kubernetes NMState Operator] to preserve and support your existing nmstate configuration.
...
* The restore process of a cloned virtual machine (VM) snapshot fails if the source VM is deleted. This is because the `DataVolumeTemplate` value in the `VirtualMachineSnapshotContent` CR which refers to the source VM's persistent volume claim (PVC), which no longer exists. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2104479[*BZ#2104479*])
** As a workaround, edit the `DataVolumeTemplate` value of the `VirtualMachineSnapshotContent` CR to refer to the existing PVC.
...
* Windows virtual machines using the HyperV reenlightenment fail to migrate. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2100054[*BZ#2100054*])
