...
* As of version {VirtVersion}, the `node-maintenance-operator` (NMO) is no longer shipped with {VirtProductName}. It is now available to deploy as a standalone Operator from the *OperatorHub* in the {product-title} web console.
+
...
* Windows virtual machines that have HyperV enlightenments enabled fail to migrate. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2100054[*BZ#2100054*])
