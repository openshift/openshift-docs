...
* {VirtProductName} now automatically updates `instancetype.kubevirt.io` objects saved in the `ControllerRevisions` settings to the latest version of the object. As a result, you no longer need to manually update `ControllerRevisions` if you require using updated supported versions of `instancetype.kubevirt.io` objects.
