...
* {VirtProductName} now supports `MultiNetworkPolicies` with VMs to `ipBlock` matchers. You can now use `Pod` and `Namespace` selectors in `MultiNetworkPolicies` created for VMs.
