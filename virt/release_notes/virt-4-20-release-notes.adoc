...
// CNV-60596
* The RHEL 8 `kubevirt-virtctl` RPM is deprecated. In 4.20, the RPM remains available but prints a deprecation message. Download `virtctl` from the cluster instead of installing it by using `dnf`. The RPM will be removed in 4.21. See xref:../../virt/getting_started/virt-getting-started.adoc#installing-virtctl_virt-using-the-cli-tools[Installing virtctl].
