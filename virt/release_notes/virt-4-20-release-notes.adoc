...
//CNV-61645
* The procedure for hot-plugging disks now includes a step for selecting a bus type, which allows the user to select the `virtio-blk` type. The `virtio-blk` type is the default.
...
//CNV-61645
* Previously, only the `virtio-scsi` bus type could be used for hot-plugging disks. Now the `virtio-blk` bus type is supported as well.
...
* The use of non-persistent hot-plugged disks is deprecated. They will not be supported in future releases.
