...
* Cross-cluster live migration is xref:../../virt/live_migration/virt-about-cross-cluster-live-migration.adoc[now available]. By enabling cross-cluster live migration, you can move a virtual machine (VM) workload from one {product-title} cluster to another {product-title} cluster without disruption.
+
