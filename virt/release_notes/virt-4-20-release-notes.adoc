...
//CNV-70866
* Attempting a storage live migration from the {VirtProductName} web console might hang and fail to create a destination PersistentVolumeClaim (PVC). (link:https://issues.redhat.com/browse/CNV-70866[*CNV-70866*])
** This issue occurs because the web console does not detect a label that marks source PVCs previously used for migration. When this label is present, the migration cannot proceed successfully.
