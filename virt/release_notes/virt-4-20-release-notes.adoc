...
// CNV-30390
* vNUMA topology for VMs is now generally available (GA) and is tested and supported for production use. This feature allows you to opt-in to an improved NUMA configuration for VMs, with better performance and optimal resource allocation. For more information, see xref:../../virt/managing_vms/advanced_vm_management/virt-numa-topology.adoc#virt-numa-topology[Working with NUMA topology for virtual machines].
...
//CNV63575
* With this update, the `vNUMA` attribute is displayed in the VM details, allowing visibility of NUMA status either next to the *CPU | Memory* section, or in the *InstanceType* section, depending on the VM creation method. You can now easily identify if NUMA is enabled on your virtual machines, enhancing transparency and understanding of your VM's configuration. This simplifies management and troubleshooting, helps you to make informed decisions about your deployments, and allows you to optimize performance based on NUMA settings.
...
//CNV-58005
* {VirtProductName} is now supported on link:https://learn.microsoft.com/en-us/azure/azure-boost/overview[Microsoft Azure Boost].
