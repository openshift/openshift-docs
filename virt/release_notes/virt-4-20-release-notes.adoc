...
// CNV-62399
* The descheduler profile `DevKubeVirtRelieveAndMigrate` has been renamed to `KubeVirtRelieveAndMigrate` and is now generally available. The updated profile improves virtual machine (VM) eviction stability during live migrations by enabling background evictions and reducing oscillatory behavior. For more information, see xref:../../virt/managing_vms/advanced_vm_management/virt-enabling-descheduler-evictions.adoc#virt-enabling-descheduler-evictions_virt-enabling-descheduler-evictions[Enabling descheduler evictions on a virtual machine].
