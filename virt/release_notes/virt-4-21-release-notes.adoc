...
* Red Hat Enterprise Linux CoreOS workers. In the Microsoft SVVP Catalog, they are named __Red{nbsp}Hat {product-title} {product-version}__.
* Intel and AMD CPUs.
...
With this update, cluster administrators can optionally remove the MTC Operator when they upgrade from previous {VirtProductName} versions to version 4.21. MTC is end-of-life and no longer needed in {VirtProductName} version 4.21 to support live storage migration.
+
...
Virtualization settings show observability status::
The Virtualization settings in the web console provides visibility into the installation and configuration status of the {coo-first} and related observability components. This enhancement helps administrators see the status of logging, metrics and monitoring, event correlation, and incident detection, and provides guided links to complete setup.
+
link:https://issues.redhat.com/browse/CNV-66519[CNV-66519]
...
Previously, installation of Migration Toolkit for Containers (MTC) was required before migrating VM disks to different storage classes. This requirement no longer exists, as VM disk storage class migration is now native to {virtproductname}.
+
...
[id="virt-maintenance-releases_{context}"]
== Maintenance releases

include::modules/virt-zstream-4-21-1.adoc[leveloffset=+2]

include::modules/virt-zstream-4-21-2.adoc[leveloffset=+2]