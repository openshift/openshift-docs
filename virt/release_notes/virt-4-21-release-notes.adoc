...
Remove the {mtv-first} Operator during the upgrade process::
With this update, cluster administrators can optionally remove the {mtv-short} Operator when they upgrade from previous {VirtProductName} versions to version 4.21. {mtv-short} is end-of-life and no longer needed in {VirtProductName} version 4.21 to support live storage migration.
+
...
Previously, installation of {mtv-first} was required before migrating VM disks to different storage classes. This requirement no longer exists, as VM disk storage class migration is now native to {virtproductname}.
+
