...
//CNV-77645
Multi-namespace storage migrations fail when the combination of resource name and namespace exceeds 63 characters::
When performing a storage migration, the process fails if the combined length of the resource name and the migrated virtual machine namespace is longer than 63 characters.
+
To work around this issue, use the KubeVirt storage migration APIs to individually migrate the affected virtual machines. For more information, see the link:https://kubevirt.io/user-guide/storage/volume_migration/[KubeVirt Volume Migration user guide].