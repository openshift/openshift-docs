...
Redesigned *Overview* page for multi-cluster support::
With this update, the {VirtProductName} *Overview* page features a redesigned layout to improve management of multi-cluster environments. Key enhancements include:
+
* The *Settings* tab is now a standalone navigation item for easier access.
* Information from the removed *Top consumers* and *Migration* tabs is now integrated into dedicated cards on the main dashboard.
* The *Inventory* section includes new nested cards for better resource tracking, while the *VirtualMachines per resource* card has been removed to reduce visual clutter.
+
This redesign streamlines monitoring workflows and ensures critical cluster data is accessible at a glance.
+
link:https://issues.redhat.com/browse/CNV-69180[CNV-69180]
