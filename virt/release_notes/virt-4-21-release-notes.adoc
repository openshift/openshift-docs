...
// Networking
Custom MAC pool range configuration:: With this update, you can specify a custom MAC pool range for your {VirtProductName} environment, preventing potential MAC address conflicts with other virtualization solutions on the same network. This empowers you to have more control over your network resources, enhancing the overall user experience, and ensuring network stability and resource management efficiency. This enhancement results in a more secure and efficient deployment of virtual machines.
+
...
//CNV-58005
{VirtProductName} support for Azure Boost (Technology Preview)::
{VirtProductName} clusters can run on link:https://learn.microsoft.com/en-us/azure/azure-boost/overview[Azure Boost], which provides dedicated host hardware to improve performance and isolation for Azure virtual machines. This capability is a Technology Preview feature.
+
link:https://issues.redhat.com/browse/CNV-58005[CNV-58005]
...
[id="virt-4-21-ki-ibm-z_{context}"]
Call traces appear for virtio_balloon free page reporting on s390x VMs::
When you create a VM by using {op-system-base-full} container disk images for s390x architecture, call traces that reference `virtio_balloon` free page reporting print to the VM console because of a kernel bug. As a consequence, the VM console output includes repeated call trace messages.
+
...
+
link:https://issues.redhat.com/browse/OCPBUGS-51113[OCPBUGS-51113]

// CNV-56889: Matan indicated this will remain in 4.20 but is targeted for 4.21
Incorrect boot mode options for s390x virtual machines::
...
+
link:https://issues.redhat.com/browse/CNV-56889[CNV-56889]
