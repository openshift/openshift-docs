...
* By using the OpenShift Container Platform web console, you can now xref:/../../virt/managing_vms/virt-migrating-vms-in-single-cluster-to-different-storage-class.adoc#virt-migrating-vms-in-single-cluster-to-different-storage-class[migrate single-cluster VMs in bulk from one storage class to another storage class].
