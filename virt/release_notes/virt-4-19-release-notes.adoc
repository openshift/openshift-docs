...
* The following table identifies the deprecated `HyperConverged` custom resource (CR) feature gate fields. The fields have moved from the `featureGates` object to the `spec` field and been renamed. If used, the deprecated feature gates are ignored.

.`HyperConverged` (CR) feature gates changes
[options="header"]
|====
|New name|Old name

|`spec.deployVmConsoleProxy`
|`spec.featureGates.deployVmConsoleProxy`

|`spec.EnableApplicationAwareQuota`
|`spec.featureGates.EnableApplicationAwareQuota`
