...
* {VirtProductName} now enables users to deploy VMs on a scalable, clustered file system. {IBMFusionSAN} offers access to consolidated, block-level data storage. It presents storage devices, such as disk arrays and tape libraries, to the operating system as if they were direct-attached storage.
+
The {IBMFusionSAN} Operator is available in the {product-title} Operator hub.
+
See xref:../../virt/fusion_access_SAN/virt-about-fusion-access-san.adoc
#about-fusion-access-san[About {IBMFusionSAN}] for more information.
