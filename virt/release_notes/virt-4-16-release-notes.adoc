...
* You can now select `sysprep` options when you create a Windows VM from an instance type. Previously it was necessary to set the `sysprep` options by customizing the VM after its creation.
