...
You can provision {VirtProductName} clusters that run Red Hat Enterprise Linux CoreOS (RHCOS) version 9.8 and version 10.2 in {product-title} 4.22. RHCOS 9.8 is the default operating system. VM live migration from RHCOS10.x to RHCOS9.x and from RHCOS9.x to RHCOS10.x worker nodes is available as a Technology Preview feature in {product-title} 4.22.
+
