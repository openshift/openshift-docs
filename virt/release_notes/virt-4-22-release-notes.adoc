...
Implement next version (v1) of the HyperConverged API::
+
Introducing the new API `v1` version for the `HyperConverged` type. The API version `v1` is not the default version. The current `v1beta1` version is the main API. OpenShift uses API `v1` for non-versioned `HyperConverged` operations, while versioned operations are safe. For example, applying a YAML file is safe, because the `api-version` is part of the file. When you use `oc` to `get`, `edit`, or `patch`, the `HyperConverged` custom resource (CR) must use the fully versioned type name `hyperconvergeds.v1beta1.hco.kubevirt.io`, instead of the non-versioned type name of `hyperconverged` or the short name of `hco`. For example: `oc get hyperconvergeds.v1beta1.hco.kubevirt.io -n openshift-cnf kubevirt-hyperconverged -o yaml`. For the `oc explain` command, use the `--api-version` flag: `oc explain --api-version=hco.kubevirt.io/v1beta1 hco.spec`
+
...
+
Cluster administrators can use the Application-Aware Quota (AAQ) Operator in the OpenShift user interface as the primary mechanism for defining and managing resource limits for virtual machines (VMs) and mixed workloads. You can configure AAQ to track resource utilization for VMs only, for both pods and VMs together, or for VMs and pods separately with dedicated quotas for each.
+
link:https://redhat.atlassian.net/browse/CNV-61720[CNV-61720]

Configure a run strategy for virtual machines in OpenShift Virtualization user interface::
+
