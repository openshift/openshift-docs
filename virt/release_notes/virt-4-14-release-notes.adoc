...
* Using {VirtProductName} on Amazon Web Services (AWS) bare-metal {product-title} clusters was previously Technology Preview and is now generally available. For more information, see xref:../../virt/install/preparing-cluster-for-virt.adoc#virt-aws-bm_preparing-cluster-for-virt[{VirtProductName} on AWS bare metal].
