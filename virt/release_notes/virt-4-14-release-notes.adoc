...
//CNV-19436 Release note: NEW Retrieve a temporary token to access the VNC endpoint of a VM
* You can now xref:../../virt/virtual_machines/virt-accessing-vm-consoles.adoc#virt-temporary-token-VNC_vnc-console[generate a temporary token] to access the VNC of a VM.
...
//CNV-38746
* When you perform a Control Plane Only update from {product-title} 4.12 to an even-numbered minor version, VMs that use the `cnv-bridge` Container Network Interface (CNI) fail to live migrate. (link:https://access.redhat.com/solutions/7069807[])
** As a workaround, change the `spec.config.type` field of your `NetworkAttachmentDefinition` manifest from `cnv-bridge` to `bridge` before performing the update.
