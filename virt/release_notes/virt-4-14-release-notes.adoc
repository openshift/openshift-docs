...
//CNV-29935
* As a VM owner, you can add an xref:../../virt/virtual_machines/virt-accessing-vm-ssh.adoc#virt-adding-key-creating-vm-template_static-key[authorized SSH key] to a project by using the web console. The key is subsequently added to all VMs that you create in the project.
...
* The *DataSources* page and the *Bootable volumes* page have been merged into the xref:../../virt/getting_started/virt-web-console-overview.adoc#bootablevolumes-page_virt-web-console-overview[*Bootable volumes* page] so that you can manage these similar resources in a single location.
...
* You can now install and edit xref:../../virt/virtual_machines/virt-creating-vms.adoc#virt-creating-vm-instancetype_virt-creating-vms[customized instance types] and preferences to create a VM from a volume or PersistentVolumeClaim (PVC).
