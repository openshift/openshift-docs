...
* Red Hat Enterprise Linux CoreOS workers. In the Microsoft SVVP Catalog, they are named __Red{nbsp}Hat {product-title} 4.15__.
* Intel and AMD CPUs.
...
** As a workaround, link:https://docs.redhat.com/en/documentation/monitoring_stack_for_red_hat_openshift/4.15/html/managing_alerts/managing-alerts-as-an-administrator#managing-silences_managing-alerts-as-an-administrator[silence alerts].
...
//CNV-66487
* Some versions of {VirtProductName} 4.15 are incompatible with {pipelines-title} 1.19 due to missing custom resource definitions (CRDs). To avoid issues, use {pipelines-shortname} 1.18. Alternatively, before you update to {pipelines-shortname} 1.19, set the `deployTektonTaskResources` feature gate to `false`. (link:https://issues.redhat.com/browse/CNV-66487[*CNV-66487*])
