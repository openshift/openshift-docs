:_mod-docs-content-type: ASSEMBLY
[id="virt-reviewing-vm-dashboard"]
= Reviewing resource usage by virtual machines
include::_attributes/common-attributes.adoc[]
:context: virt-reviewing-vm-dashboard

toc::[]

Dashboards in the {product-title} web console provide visual representations of cluster metrics to help you to quickly understand the state of your cluster. Dashboards belong to the xref:../../observability/monitoring/monitoring-overview.adoc#monitoring-overview[Monitoring overview] that provides monitoring for core platform components.

The {VirtProductName} dashboard provides data on resource consumption for virtual machines and associated pods. The visualization metrics displayed in the {VirtProductName} dashboard are based on xref:../../virt/logging_events_monitoring/virt-prometheus-queries.adoc#virt-prometheus-queries[Prometheus Query Language (PromQL) queries].

A xref:../../observability/monitoring/enabling-monitoring-for-user-defined-projects.adoc#enabling-monitoring-for-user-defined-projects[monitoring role] is required to monitor user-defined namespaces in the OpenShift Virtualization dashboard.

You can view resource usage for a specific virtual machine on the xref:../../virt/virt-web-console-overview.adoc#ui-virtualmachine-details-metrics_virt-web-console-overview[*VirtualMachine details* page -> *Metrics* tab] in the web console.

include::modules/virt-about-reviewing-top-consumers.adoc[leveloffset=+1]

include::modules/virt-reviewing-top-consumers.adoc[leveloffset=+1]

[id="additional-resources_reviewing-vm-dashboard"]
[role="_additional-resources"]
== Additional resources

* xref:../../observability/monitoring/monitoring-overview.adoc#monitoring-overview[Monitoring overview]

* xref:../../observability/monitoring/reviewing-monitoring-dashboards.adoc#reviewing-monitoring-dashboards[Reviewing monitoring dashboards]
