...
. *Install and Deploy container Registry*: Install the container Registry and then ensure that the container Registry is deployed to locally manage images:
+
----
$ oc adm registry --config=/etc/origin/master/admin.kubeconfig
----
+
For more information, see xref:../../install_config/registry/index.adoc#install-config-registry-overview[Deploying a container Registry]
