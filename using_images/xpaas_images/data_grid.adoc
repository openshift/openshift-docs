...
1.  The *_OPENSHIFT_KUBE_PING_NAMESPACE_* environment variable must be set (as seen in the link:#Configuration_Environment_Variables[Configuration Environment Variables]).
If this variables is not set, then the server will act as if it is a single-node cluster, or a cluster that consists of only one node.
+
2.  The *_OPENSHIFT_KUBE_PING_LABELS_* environment variable must be set (as seen in the link:#Configuration_Environment_Variables[Configuration Environment Variables]).
If this variables is not set, then pods outside the application (but in the same namespace) will attempt to join.
+
...
| *_CONTAINER_SECURITY_ROLES_* | Define role names and assign permisions to them. | Example: *_admin=ALL,reader=READ,writer=WRITE_*
...
| *_HOTROD_ENCRYPTION_* | If defined the hotrod-connectors will be confiugred with encryption in the *_ApplicationRealm_*. | Example: *_true_*
...
| *_<CACHE_NAME>_CACHE_REMOTE_TIMEOUT_* | In *_SYNC_* mode the timeout, in milliseconds, used to wait for an acknolwedgment when making a remote call, after which the call is aborted and an exception is thrown.  Defaults to *_17500_*. | *_25000_*
