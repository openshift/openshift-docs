...
. *Install OpenShift* - Before using the xPaaS JBoss EAP images, you must have an OpenShift environment installed and configured. The
...
. *Create Image Streams* - Image streams are configured during Advanced OpenShift Installation. If required, manually create the image streams for both versions of the xPaaS JBoss EAP image as follows:
+
----
...
. *Create Instant App Templates* - Instant App templates define a full set of objects for running applications and are configured during the Advanced OpenShift Installation. If required, create Instant App templates as follows:
