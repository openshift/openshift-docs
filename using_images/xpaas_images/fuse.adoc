...
* Patching in an OpenShift environment is different from standalone Fuse since each application image is a complete runtime environment.  To apply a patch, the application image is rebuilt and redployed within OpenShift.  Core OpenShift management capabilities allow for rolling upgrades and side-by-side deployment to maintain availability of your application during upgrade.
...
. This will create a maven project with all required dependencies. Maven properties and plugins that are used to create Docker images are added to the `*pom.xml*`.

. Set the following environment variables to communicate with OpenShift and the docker daemon:

+
|===

| DOCKER_HOST | Specifies the connection to the Docker daemon of an OpenShift Node (e.g. the Master) | `tcp://172.28.128.4:2375`
