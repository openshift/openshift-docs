...
JBoss Fuse Integration Services application templates generated from archetypes depend on s2i builder *`ImageStreams`*, which MUST be created ONCE. The OpenShift installer creates them automatically. For users existing OpenShift setups, it can be achieved with the following command:
