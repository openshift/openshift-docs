...
is accessible from within the container for troubleshooting purposes.  First open a
...
In addition to viewing the OpenShift logs, you can troubleshoot a running Decision Image container by viewing the console logs.  These are outputted to the container's console, and are accessible with the following command:
...
[NOTE]
By default, the OpenShift Decision Server xPaaS image does not have a file log handler configured. Logs are only sent to the console.
