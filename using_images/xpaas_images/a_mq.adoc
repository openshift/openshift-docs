...
is designed for use with {product-title}. It allows developers to quickly deploy
an A-MQ message broker in a hybrid cloud environment.
...
There are several major functionality differences in the {product-title} JBoss A-MQ
xPaaS image:
...
link:#configuring-params[Application Template Parameters].
** using the S2I (Source-to-image) tool, as described in
link:#configuring-sti[Configuration Using S2I].
...
`*AMQ_RELEASE*`::
  ...
  available.
`*APPLICATION_NAME*`::
  The name of the application used internally in {product-title}. It is used in names
...
Only SSL connections can connect from outside of the {product-title} instance,
...
inside the {product-title} instance.
...
TrustStore and supply them to the image using the {product-title} secrets
mechanism or S2I.
...
JBoss A-MQ image by viewing the JBoss A-MQ logs that are outputted to the
container's console:
...
[NOTE]
====
By default, the {product-title} JBoss A-MQ xPaaS image does not have a file log
