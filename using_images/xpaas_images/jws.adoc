...
+
The results of a successful Maven build are copied to *_/opt/webserver/webapps_*. By default, this includes:
+
* All WAR files from the *_target_* directory in the source repository.
* All WAR files from the directory within the source repository specified by the `*$ARTIFACT_DIR*` environment variable.
