...
Jenkins slave pods are deleted by default after the build completes or is aborted. 
This behavior can be changed via the Kubernetes plug-in "Pod Retention" setting. 
Pod retention can be set for all Jenkins builds, with overrides for each 
