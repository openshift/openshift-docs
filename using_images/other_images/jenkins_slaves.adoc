...
be overridden via the `OPENSHIFT_JENKINS_JVM_ARCH` environment variable.  The
...
its heap.  This value can be modified via the `CONTAINER_HEAP_PERCENT`
...
xref:../../dev_guide/application_memory_sizing.adoc#sizing-openjdk[Sizing
OpenJDK on {product-title}] via the GRADLE_OPTS, JAVA_OPTS or JAVA_TOOL_OPTIONS environment
variables.
* Set the maximum heap size and/or JVM arguments for any Gradle test JVM via
the maxHeapSize and/or jvmArgs settings in build.gradle, or via the
