[[using-images-other-images-jenkins-agents]]
= Jenkins Agents
{product-author}
{product-version}
:data-uri:
...
* link:https://github.com/openshift/jenkins/tree/master/agent-maven-3.6[Maven v3.6 image]
...
_Maven v3.5 is still available for download but it doesn't appear anymore the image catalog as maven 3.5 has reached end of life._
...
$ docker pull registry.redhat.io/openshift3/jenkins-agent-maven-36-rhel7
...
$ docker pull openshift/jenkins-agent-maven-36-centos7
