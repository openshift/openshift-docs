...
The {product-title} Client Plug-in aims to provide a readable, concise, comprehensive,
and fluent Jenkins Pipeline syntax for rich interactions with OpenShift Container Platform.
The plug-in uses the `oc` binary, which must be available on the nodes executing the script.
