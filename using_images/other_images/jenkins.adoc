...
If you want to customize the official OpenShift Jenkins image, you can use
Docker layering or you can use the image as a Source-To-Image builder.
...
plugins or replace the provided `config.xml` file with your own, custom, configuration.
...
Note that the content of `configuration` folder will be copied into the `/var/lib/jenkins`
folder, so you can also include additional files (like `credentials.xml`, etc.).

For example, you can use following build configuration to customize the Jenkins
image in OpenShift:
...
<1> The source defines the source Git repository with the layout described above
<2> The strategy defines the original Jenkins image to use as a source image for the build.
<3> The output defines the resulting, customized Jenkins image you can use in deployment configuration instead of the official Jenkins image
