...
<<<<<<< HEAD
of slave images for the Kubernetes plug-ins by scanning the project Jenkins is deployed in for
existing image streams with the label *role* set to *jenkins-slave*.

When an image stream with this label is found, corresponding Kubernetes plug-in configuration
is generated so you can assign your Jenkins
=======
