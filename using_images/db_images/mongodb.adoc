...
== Overview
{product-title} provides a Docker image for running MongoDB.  This image can
...
entrypoint to behave xref:mongo-creating-headless-service[slightly differently]
...
The `initiate` argument in the xref:example-service-pod[container specification
...
creating MongoDB users and databases. xref:creating-the-service-pod[The service
...
To xref:../../dev_guide/deployments.adoc#scaling[increase the number of members]
in the cluster:
