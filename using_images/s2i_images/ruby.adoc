...
For Ruby on Rails applicatoin, run the built Rails application with the
...
image with the `*RACK_ENV=development*` environment variable. For example, see
link:../../dev_guide/new_app.html#specifying-environment-variables[the `oc
new-app` command].
