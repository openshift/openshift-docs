...
[NOTE]
====
Storage is only automatically configured when you install an installer-provisioned infrastructure cluster on AWS, Azure, {gcp-full}, {ibm-name}, or OpenStack.

When you install or upgrade an installer-provisioned infrastructure cluster on AWS, Azure, {gcp-full}, {ibm-name}, or OpenStack, the Image Registry Operator sets the `spec.storage.managementState` parameter to `Managed`. If the `spec.storage.managementState` parameter is set to `Unmanaged`, the Image Registry Operator takes no action related to storage.
====
endif::openshift-dedicated,openshift-rosa,openshift-rosa-hcp[]
...
* xref:../registry/configuring_registry_storage/configuring-registry-storage-gcp-user-infrastructure.adoc#configuring-registry-storage-gcp-user-infrastructure[Configuring the registry for {gcp-full} user-provisioned infrastructure]
