...
[NOTE]
====
Storage is only automatically configured when you install an installer-provisioned infrastructure cluster on AWS, Azure, {gcp-short}, IBM, or OpenStack.

When you install or upgrade an installer-provisioned infrastructure cluster on AWS, Azure, {gcp-short}, IBM, or OpenStack, the Image Registry Operator sets the `spec.storage.managementState` parameter to `Managed`. If the `spec.storage.managementState` parameter is set to `Unmanaged`, the Image Registry Operator takes no action related to storage.
====
endif::openshift-dedicated,openshift-rosa[]
