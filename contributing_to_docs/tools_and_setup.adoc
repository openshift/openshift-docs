...
. Change to the `openshift-docs` directory and build every distribution by entering the `run` command for the tool that you are using. The following example uses Docker and should be identical for Podman on Linux while might require some setup on Mac. 
+
----
$ cd openshift-docs
