...
. Change to the `openshift-docs` directory and build every distribution by entering the `run` command for the tool that you are using. The following example uses Docker. If you are using Podman on a Mac, you must perform additional configuration steps provided at the end of this section. 
+
----
$ cd openshift-docs
