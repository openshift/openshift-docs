...
1. Install _docker_ or __podman_ package
2. From the `openshift-docs` directory, run build:
+
----
$ cd openshift-docs
$ docker run --rm -it -u `id -u` -v `pwd`:/docs:Z quay.io/openshift-cs/asciibinder asciibinder build
----
3. Open the generated HTML file in your web browser. This will be located in the
