...
[id="map-file-metadata"]
== Map file metadata
Every map must be placed in the `maps/` folder and should contain the following metadata at the top:

----

:_mod-docs-content-type: MAP           <1>
:context: example-context-id           <2>

----
<1> The content type for the file. For maps, always use `:_mod-docs-content-type: MAP`. Place this attribute before the context attribute for job maps.
<2> Only required for job maps, for example, link:https://github.com/openshift/openshift-docs/blob/main/maps/jobs/capture-ovs-interface-state.adoc?plain=1[MicroShift job]. Context is not required for navigation or category maps.
...
* `:_mod-docs-content-type: MAP` (Only for job maps and JTBD category map files)
...
|`openshift-origin`
|OKD
a|* 4.18, 4.19, 4.20, 4.21, 4.22
* 4 for the `latest/` build from the `main` branch

|`openshift-enterprise`
|OpenShift Container Platform
a|* 3.0, 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7, 3.9, 3.10, 3.11
* 4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 4.8, 4.9, 4.10, 4.11, 4.12, 4.13, 4.14, 4.15, 4.16, 4.17, 4.18, 4.19, 4.20, 4.21, 4.22, 5.0

|`openshift-dedicated`
|OpenShift Dedicated
a|* No value set for the latest `dedicated/` build from the `enterprise-4.22` branch
* 3 for the `dedicated/3` build from the `enterprise-3.11` branch

|`openshift-rosa`
|Red Hat OpenShift Service on AWS (classic architecture)
|No value set for the `rosa/` build from the `enterprise-4.22` branch
...
[IMPORTANT]
====
The following situations have an exception to allow xrefs in modules as long as the module is only ever included once from one assembly:
...
[IMPORTANT]
====
In OpenShift docs, you can normally only use `xref` in assemblies, not in modules. However, the following situations have an exception to allow xrefs in modules as long as the module is only ever included once from one assembly:
