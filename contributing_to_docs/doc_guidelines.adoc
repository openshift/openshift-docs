...
*Problem*
\*\`openshift-*`* is invalid formatting. It is intended to mark up a literal value, `openshift-*`, in bold highlighting. 
Because of the two asterisks, AsciiDoctor does not know which asterisk is the closing tag. Travis reports _Opening and ending tag mismatch:_.

*Solution*
Use an escape character for the second asterisk or use double asterisks; \*\*`openshift-*`**.  
