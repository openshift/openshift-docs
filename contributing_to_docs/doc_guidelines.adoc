...
The main distinction between showng command syntax and example is that a command syntax should just show customers how to use the command, with replaceable parameters of that command. Whereas an example should show the command with actual values and the output of that command.
