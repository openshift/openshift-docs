...
* All links to internal content is created by using and `xref`, and each `xref` **must have an anchor ID**.
* Only use `xref` in assemblies, not in modules.
* All links to external websites are created by using `link`.
...
To maintain consistency across OpenShift Container Platform documentation, ensure that you adhere to the following guidance for lead-in link sentences:

*  Use the following format when pointing to a section in the same document: `For more information, see "<section_name>."`
...
* When pointing to a link in a section that exists in another customer product document, use the `For more information on <topic>, see "<section_name>" in _<document_name>_.` format. Note the quotation marks around `<section_name>` and the italics around `<document_name>`.
* When pointing to a link that exists external to Red Hat's customer portal documentation, use the `link` attribute, place the link in an _Additional resources_ section, and use the `<document_heading_name> (<document_source>)` format. For example, see link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/[AWS Local Zones features] (AWS documentation).
...
* For more information about the differences between the _Additional resources_ and _Next steps_ headings, see xref:additional-resources-sections[Additional resources sections].
* For more information about the `xref` and `link` attributes, see xref:links-conditionals-assemblies[Links, hyperlinks, and cross references].
...
You can add link, hyperlinks, and cross references to either the _Additional resources_ section or the _Next steps_ section, but know the following differences:
...
* Additional resources - assembly level: Requires the `== Additional resources` section heading. If a resource applies to all of the modules in a user story, consider listing the resource in the _Additional resources_ section of the assembly file.
