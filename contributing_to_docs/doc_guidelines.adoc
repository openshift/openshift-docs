...
** <step_description> by running the following command:
** <step_description> by entering the following command:
** <step_description>, run the following command:
** <step_description>, enter the following command:

Do not add redundant text, such as `at the CLI` or `at the command line`.
...
In most cases, describe what action each command accomplishes.
For example:
...
[NOTE]
====
When a step is made up of multiple CLI substeps, stating `by running the following commands:` in the step text obviates the need to repeat `by running the following command:` for each substep.
====

When the user must complete several identical command line steps with different arguments, substeps are not required.
However, each command must be included in an separate code block.
For example:

....
. Configure the following environment variables by running the following commands:
+
[source,terminal]
----
