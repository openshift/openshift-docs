...
To mark up command syntax, use the code block and wrap any user-replaceable values in angle brackets (`<>`) with the required command parameter, using underscores (`_`) between words as necessary for legibility.
...
[[choosing-varnames]]
===== Choosing variable names for replaceable values

When choosing how to present a user-replaceable value, consider the following:

* Check the link:https://github.com/openshift/openshift-docs/blob/main/contributing_to_docs/term_glossary.adoc[OpenShift glossary of terms] for entries that could be related to the variable, and use a suggested variable name if provided.
* If no entry exists in the term glossary:
** Append a suffix of `_name>` when referring to an instance of an API object. For example, `<pod_name>` instead of simply `<pod>`; this is longer, but less amibiguous.
*** A common exception: Use `<namespace>` instead of `<namespace_name>`.
** If "ID" or a suffix of `_id` exists in related descriptions or field names, use a suffix of `_id>` over `_name>`.
*** A common example: Use `<cluster_id>` instead of `<cluster_name>`, unless a field or UI element explicitly mentions "cluster name".
* If you still are not sure, check other instances of variables in nearby content (modules and the assembly) and adhere to the existing style.
