...
Do not include Technology Preview features in the OSD (`openshift-dedicated`) and ROSA (`openshift-rosa`) distributions. If the feature is documented in a module or assembly that is reused in either of these distributions, use a conditional statement so that it is excluded.
...
== Indicating Generally Available features for OSD and ROSA

When a Technology Preview feature becomes generally available, determine if it applies to OSD, ROSA, or both. If the content applies to either of these distributions, include the `snippets/sd-generally-available.adoc` file in the feature's assembly or module. Provide values for the `:FeatureName:` and `:GAVersion:` variables before you include this module.
...
:FeatureName: The XYZ plug-in
:GAVersion: 4.15
\include::snippets/sd-generally-available.adoc[]
endif::openshift-dedicated,openshift-rosa[]
----
