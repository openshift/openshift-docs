...
[[jq-commands]]
==== Commands with jq
Where a command or procedure requires the user to manipulate JSON and the same result cannot be achieved with `oc` and `jsonpath`, you can use `jq` in the procedure.
If a command or procedure includes `jq`, add installing `jq` as a prerequisite for the procedure.

For commands that use `oc`, the same or similar result can be accomplished using `jsonpath` instead of redirecting a command output to `jq`.
