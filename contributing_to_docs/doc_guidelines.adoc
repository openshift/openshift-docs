...
for guidelines for OpenShift / OKD 3.x documentation. These guidelines build on top of the
...
anchor and the section title and the anchor id should match the section title:

----
[id='<section-anchor>-{context}']
=== Section title
----
...
----
[discrete]
[id='managing-authorization-policies-{context}']
...
Links can be used to cross-reference internal content, or send customers to external information resources for further reading.
...
1. Link to another file that exists in the same group, or directory.
2. Link to another file that exists in a separate group, or directory.
...
*Link to content in same group directory*
...
*Link to content in different group directory*
...
supportablity wording consistent across Technology Preview features.
...
== Writing concepts
A _concept_ is an assembly (full .adoc file) or section (individual module within an
assembly) that supports the things that users want to do and should not include
task information like commands or numbered steps. Consider section titles
with a verb like "Understanding <concept>" if it is solely concept-based.

== Writing tasks
A _task_ is an assembly (full .adoc file) or section (individual module within an
assembly) that supports the things that users want to do and includes procedural
