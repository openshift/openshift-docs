...
conditionalize an entire file. If an entire file is specific to only
some OpenShift distributions, specify them in the `&#95;topic&#95;map.yml`
file.
...
For all supported versions, the topic map files are available in the `&#95;topic&#95;maps` folder. Older versions support the `&#95;topic&#95;map.yml` file in the root folder.
...
For supported branches, the `&#95;topic&#95;map.yml` is based in the `&#95;topic&#95;maps` folder in the root directory and is organized (primarily) by distributions.
...
the topic or topic group will appear in all product documentation versions.
