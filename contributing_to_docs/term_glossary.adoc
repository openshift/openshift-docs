...
In Red Hat OpenShift, the Assisted Installer is an installation solution that is offered on the {hybrid-console} to provide Software-as-a-Service functionality for cluster installations. The Assisted Installer supports installing {product-title} on premises in a connected environment. The Assisted Installer is used for connected deployments.
