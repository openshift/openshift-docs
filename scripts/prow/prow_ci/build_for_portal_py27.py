...
# Python 2.7 compatible version of build_for_portal.py for use with quay.io/redhat-docs/openshift-docs-bccutil:latest in the OpenShift Prow CI.
...
def generate_master_entry(
    node, book_dir, distro, include_name=True, all_in_one=False
):
    """
...
def scrub_file(info, book_src_dir, src_file, tag=None, cwd=None):
    ...
    if https_pos >=0:
        ...
        try:
            ...
            list_of_errors.append("An include file wasn't found: {}".format(base_src_file))
            sys.exit(-1)
...
def dir_to_book_name(dir,src_file,info):
    ...
    log.error(
        ...
        src_file,
        dir)
    list_of_errors.append("ERROR ({}): book not found for the directory {}".format(src_file, dir))
    return(dir)
            ...
            if fixed_link_file_abs in file_to_id_map:
                ...
                if full_relative_path[:2]=="..":
                    log.error(
                        ...
                        src_file,
                        link_file)
                    list_of_errors.append("ERROR ({}): link pointing outside source directory? {}".format(src_file, link_file))
                    continue
                ...
                if EXTERNAL_LINK_RE.search(link_file) is not None:
                    ...
                    list_of_errors.append("ERROR ({}): {} appears to try to reference a file not included in the {} distro".format(rel_src_file, link_text_message, info["distro"]))
        else:
            fixed_link = "xref:" + link_anchor.replace("#", "") + link_title
