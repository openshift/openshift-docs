...
. You have forked and cloned the link:https://github.com/openshift/release[OpenShift release repository] to `$HOME/release`:
+
[IMPORTANT]
====
...
. Create a new branch by running the `git checkout <your_new_branch>` command. Replace `<your_new_branch>` with an appropriate branch name.
...
. MacOS only: You have installed the `gettext` package by running the following commands:
+
[source,terminal]
----
...
. Commit your changes and open a pull request (PR) against the `openshift/release` `main` branch. Ensure that all Prow CI tests pass. Add a `/pj-rehearse` comment in the PR to verify the new build.

. Get an `/lgtm` approval from someone in the openshift-docs link:https://github.com/openshift/release/blob/main/ci-operator/config/openshift/openshift-docs/OWNERS[OWNERS] file.
