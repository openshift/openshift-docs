...
FILES=$(git diff --name-only HEAD~1 HEAD --diff-filter=d "*.yml" "*.adoc" ':(exclude)_unused_topics/*')
...
for ASSEMBLY in $ALL_ASSEMBLIES; do
    ...
    if grep -rq "$PAGE" --include "*.yml" _topic_maps ; then
        DISTROS=$(echo "$(grep -rl "$PAGE" --include "*.yml" _topic_maps)")
    fi
done
...
DISTROS="${DISTROS} ${UPDATED_DISTROS}"
# Clean up and remove dupes from the list
