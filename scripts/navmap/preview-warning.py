...
WARNING_STYLE = """<!-- navmap-preview-warning-start -->
<style>
  .navmap-preview-warning {
    background: #fff1f1;
  ...
  .navmap-preview-warning strong { color: #a30000; }
  .navmap-preview-warning details { margin-top: .75em; }
  .navmap-preview-warning pre {
    background: #fff;
...
<!-- navmap-preview-warning-end -->"""


def warning_markup(report: str) -> str:
    escaped = html.escape(report)
    return (
        '<div class="navmap-preview-warning" role="alert">'
        ...
        "from the map and track the content gap separately.</li>"
        "</ol>"
        "<p>After fixing the includes, rebuild the preview with "
        "<code>./preview-map &lt;distro&gt;</code>.</p>"
        "</details></div>"
    )
...
def process(path: Path, report: str) -> bool:
    ...
    if "<!-- navmap-preview-warning-start -->" in source:
        return False
    head = HEAD_CLOSE_RE.search(source)
    body = BODY_RE.search(source)
    if not head or not body:
        return False
    source = source[: head.start()] + WARNING_STYLE + "\n" + source[head.start() :]
    body = BODY_RE.search(source)
...
def main() -> int:
    if len(sys.argv) != 3:
        print(f"Usage: {Path(sys.argv[0]).name} OUTPUT_DIR PREFLIGHT_REPORT", file=sys.stderr)
        return 2
