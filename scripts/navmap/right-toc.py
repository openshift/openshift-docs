...
RIGHT_TOC_RE = re.compile(
    r'\s*<aside id="navmap-right-toc".*?</aside>\s*',
    re.DOTALL,
)
RIGHT_STYLE_RE = re.compile(
    r'\s*<!-- navmap-right-toc-start -->.*?<!-- navmap-right-toc-end -->\s*',
    re.DOTALL,
)
LEFT_NAV_TOGGLE_RE = re.compile(
    r'\s*<!-- navmap-left-nav-toggle-start -->.*?<!-- navmap-left-nav-toggle-end -->\s*',
    re.DOTALL,
)
TAG_RE = re.compile(r'<[^>]+>')
...
RIGHT_TOC_STYLE = """<!-- navmap-right-toc-start -->
<style>
  body.navmap-has-right-toc { padding-right: 18em; }
  #navmap-right-toc {
    position: fixed;
    top: 0;
    right: 0;
  ...
  #navmap-right-toc h2 { margin-top: 0; font-size: 1.1em; }
  #navmap-right-toc ul { list-style: none; margin: 0; padding: 0; }
  #navmap-right-toc li { line-height: 1.35; margin: .4em 0; }
  #navmap-right-toc li.level-3 { padding-left: 1em; }
  #navmap-right-toc li.level-4 { padding-left: 2em; }
  #navmap-right-toc li.level-5,
  #navmap-right-toc li.level-6 { padding-left: 3em; }
  #navmap-right-toc a { text-decoration: none; }
  @media screen and (max-width: 1023px) {
    body.navmap-has-right-toc { padding-right: 0; }
    #navmap-right-toc {
      position: static;
      width: auto;
      height: auto;
...
<!-- navmap-right-toc-end -->"""

LEFT_NAV_TOGGLE_ASSETS = """<!-- navmap-left-nav-toggle-start -->
<style>
  #toc .toc-toggle {
  ...
  #toc .toc-toggle:hover { color: #0066cc; }
  #toc .toc-toggle.open { transform: rotate(90deg); }
  #toc li > ul.sectlevel2,
  #toc li > ul.sectlevel3,
  #toc li > ul.sectlevel4 { display: none; }
  #toc li.toc-expanded > ul.sectlevel2,
  #toc li.toc-expanded > ul.sectlevel3,
  #toc li.toc-expanded > ul.sectlevel4 { display: block; }
</style>
<script>
(function() {
  var previewPath = window.location.pathname.slice(
    0,
    window.location.pathname.lastIndexOf('/')
  ) || '/';
  var STORAGE_KEY = 'navmap-toc-state:' + previewPath;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var buttons = document.querySelectorAll('#toc .toc-toggle');

    buttons.forEach(function(btn) {
      var key = btn.getAttribute('data-toc-key');
      ...
      links.forEach(function(a) {
        ...
        if (href === currentPage) { isCurrentSection = true; }
      });

      if (state[key] === true || isCurrentSection) {
        li.classList.add('toc-expanded');
        btn.classList.add('open');
        if (isCurrentSection) { state[key] = true; }
      }

      btn.addEventListener('click', function(e) {
        e.preventDefault();
        var expanded = li.classList.toggle('toc-expanded');
    ...
    /* --- Scroll current page into view in the TOC --- */
    var toc = document.getElementById('toc');
    if (toc) {
      var cur = toc.querySelector('a[href="' + currentPage + '"]');
      if (!cur) { cur = toc.querySelector('a[href$="/' + currentPage + '"]'); }
      if (cur) {
...
def build_toc(headings: list[tuple[int, str, str]]) -> str:
    ...
    return (
        '<aside id="navmap-right-toc" aria-label="On this page">'
...
def set_right_toc_class(source: str, enabled: bool) -> str:
    ...
    if class_match:
        ...
        if enabled and "navmap-has-right-toc" not in classes:
            classes.append("navmap-has-right-toc")
        if not enabled:
            classes = [item for item in classes if item != "navmap-has-right-toc"]
        if classes:
        ...
        body_attrs = f' class="navmap-has-right-toc"{body_attrs}'
...
def process(
    ...
    source = LEFT_NAV_TOGGLE_RE.sub("", source)
    source = source.replace("<!-- navmap-right-toc-start -->", "")
    source = source.replace("<!-- navmap-left-nav-toggle-start -->", "")

    has_docinfo_toggle = "has-children" in source and "toc-toggle" in source
    if not has_docinfo_toggle:
        source = add_static_toc_toggles(source)
        source = source.replace("</head>", LEFT_NAV_TOGGLE_ASSETS + "\n</head>", 1)
