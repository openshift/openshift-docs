...
function check_if_existing_comment {
    ...
    local vale_review="$1"
    local pull_comments="$2"

    # Iterate through Vale alert JSON
    jq -c '.[]' "$vale_review" | while IFS= read -r json_object; do
    BODY_VALE=$(echo "$json_object" | jq -r '.body')
    FILENAME_VALE=$(echo "$json_object" | jq -r '.path')
    LINE_NUMBER_VALE=$(echo "$json_object" | jq -r '.line')

        # Iterate through pull comment JSON to see if a comment exists
        jq -c '.[]' "$pull_comments" | while IFS= read -r json_object2; do
            BODY_COMMENT=$(echo "$json_object2" | jq -r '.body')
            FILENAME_COMMENT=$(echo "$json_object2" | jq -r '.path')
            LINE_NUMBER_COMMENT=$(echo "$json_object2" | jq -r '.line')
            ...
            if [ "$BODY_VALE" == "$BODY_COMMENT" ] && [ "$FILENAME_VALE" == "$FILENAME_COMMENT" ] && [ "$LINE_NUMBER_VALE" == "$LINE_NUMBER_COMMENT" ]; then
                echo "Found existing comment for $json_object"
                echo "Removing alert because review comment already exists... "
                
                # jq --argjson obj "$json_object" '. | del(.[index($obj)])' "$vale_review" > processing.json
                # jq --argjson obj "$json_object2" '. | del(.[index($obj)])' "$pull_comments" > processing_comments.json
...
function get_pr_comments {
    curl -L -H "Accept: application/vnd.github+json" -H "Authorization: Bearer $GITHUB_AUTH_TOKEN" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/pulls/$PULL_NUMBER/comments
}

# Run vale with the custom template on updated files
for FILE in ${FILES}; do

    vale_review_json=$(vale --minAlertLevel=error --output=.vale/templates/bot-comment-output.tmpl | jq .)

    pull_comments_json=$(get_pr_comments)

    check_if_existing_comment "$vale_review_json" "$pull_comments_json"
    ...
    # Iterate through the vale json review objects
    jq -c '.[]' "$vale_review_json" | while IFS= read -r json_object; do
        BODY=$(echo "$json_object" | jq -r '.body')
        FILENAME=$(echo "$json_object" | jq -r '.path')
        LINE_NUMBER=$(echo "$json_object" | jq -r '.line')
        ...
        while read -r line; do
            ...
            if [[ $line =~ @@ ]]; then
                ...
                if (( LINE_NUMBER >= diff_start_line && LINE_NUMBER <= diff_start_line + added_lines )); then
                    ...
                    send_github_review "$BODY" "$FILENAME" "$LINE_NUMBER"
                    
                    break  # Exit the loop since the alert is within the diff, move on to the next JSON json_object
                fi
            fi
