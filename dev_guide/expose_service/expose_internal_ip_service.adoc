...
If the project and service already exist, go to the next step: xref:exposing-service[Expose the Service to Create a Route].
...
. Log into the xref:create-external-project-service[project] where the service you want to expose is located.
+
----
$ oc project project1
----
