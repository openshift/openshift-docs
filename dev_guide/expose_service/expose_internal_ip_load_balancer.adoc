...
A load balancer service allocates a unique IP from a configured pool. The load balancer a single edge router IP (which can be a xref:create-external-ip-fail[virtual IP (VIP)], but is still a single machine for initial load balancing).
...
include::dev_guide/expose_service/expose_internal_ip_service.adoc[tag=expose-svc-prereqs]

Then, perform the following tasks:

* xref:automatically-assign-create-lb[Create the Load Balancer Service]
* xref:automatically-assign-ips-network[Configure networking]
...
. Load the project where the service you want to expose is located. If the project or service does not exist, see xref:create-external-project-service[Create a Project and Service].
+
----
$ oc project project1
----
...
----
+
The examples in this section use a MySQL service, which requires a client application. If you receive a cURL response similar to the following, you are connecting with the service:
+
----
$ curl 172.29.121.74:3306
5.7.16�Z(7uPs*u���T(YSNF6t.mysql_native_password��Got packets out of order
----
...
. Add a route between the IP address of the exposed service on the master and the IP address of the master host:
+
----
...
If you see the above output, your service is accessible from the node.
...
If you see the above output, your service is accessible outside the cluster.
