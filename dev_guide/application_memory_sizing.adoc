...
= Application memory and ephemeral storage sizing
{product-author}
{product-version}
:data-uri:
:icons:
...
. Determining the memory, ephemeral storage (only if your administrator has enabled the ephemeral-storage tech preview in {product-title 3.10), and risk requirements of a containerized application
...
. Configuring containerized application runtimes (for example, OpenJDK) to adhere
   optimally to the configured container memory and ephemeral storage parameters.

. Diagnosing and resolving memory- and ephemeral storage-related error conditions associated with
...
For the purposes of sizing application memory and ephemeral storage, the key points are:
...
* For each kind of resource (memory, cpu, storage, and ephemeral storage), {product-title} allows
  ...
  pod. For the purposes of this page, we are solely interested in memory and ephemeral storage
...
* *Ephemeral Storage request*
  ...
  - Ephemeral storage requests only apply if your administrator has enabled the ephemeral-storage tech preview in {product-title} 3.10.
...
* *Ephemeral Storage limit*
  ...
  - Ephemeral storage limits only apply if your administrator has enabled the ephemeral-storage tech preview in {product-title} 3.10.
...
* *Administration*
  ...
  - The cluster administrator may assign quota against the memory and/or ephemeral storage request value,
    limit value, both, or neither.

  - The cluster administrator may assign default values for the memory and/or ephemeral storage request
    value, limit value, both, or neither.

  - The cluster administrator may override the memory and/or ephemeral storage request values that a
...
container should request memory and ephemeral storage according to the expected peak usage plus a
percentage safety margin. If the risk appetite is higher, it may be more
appropriate to request memory and ephemeral storage according to the expected mean usage.
...
. *Set container memory and/or ephemeral storage limits, if required*
+
Set container memory and/or ephemeral storage limits, if required. Setting a limit has the effect of
...
On the one hand, it may make unanticipated excess memory or ephemeral storage usage obvious early
