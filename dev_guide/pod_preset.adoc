...
* Enable the pod preset admission controller plugin through the *_/etc/origin/master/master-config.yaml_*;
...
. Make sure the pod preset admission controller plugin is present in the *_/etc/origin/master/master-config.yaml_* file:
+
----
admissionConfig:
  pluginConfig:
...
----
+
If you need to add the plugin, restart the {product-title} services:
+
