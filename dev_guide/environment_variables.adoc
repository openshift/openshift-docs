...
`oc env` command.  This command can also be used on `*BuildConfig*` objects.

== CLI Interface

{product-title} provides the `oc env` command to set or unset environment
...
any object that has a pod template.  This command can also be used on `*BuildConfig*` objects.
