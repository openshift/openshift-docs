...
Before working with {product-title} image streams and their tags, it helpa
...
changes, it does not reflect in the destination tag.

A _tracking_ tag means the destination tag's metadata is imported during
...
must include the `:80` port suffix or the secret is used when attempting
...
An image stream definition specifies information about the imageStream and the specific tags to be imported.
...
required for it to show up in the catalog. Add tags that associate it with one
...
For more information on the fields that can be defined in an image stream, see the xref:../rest_api/apis-image.openshift.io/v1.ImageStream.adoc#object-schema[Imagestream API] and the xref:../rest_api/apis-image.openshift.io/v1.ImageStreamTag.adoc#object-schema[ImagestreamTag API].
