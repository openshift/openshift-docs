...
. Enabling image stream resolution on an image stream. This allowa all
...
When adding an Image Trigger to Deployments, you can also use the `oc set triggers` command.  For example the following command adds an image change trigger to the Deployment named `example` such that when the `example:latest` image stream tag is updated, the `web` container inside the deployment is updated with the new image value:
