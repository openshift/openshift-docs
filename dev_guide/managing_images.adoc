...
[[writing-imagestreams-for-s2i-builders]]
== Writing Image Streams for S2I Builders

Image streams for S2I builders that will be displayed in the management console's catalog page require additional
metadata to provide the best experience for end users.
...
----
<1> A user-friendly name for the whole image stream. This should be short.
<2> The tag will be referred to as the version. Tags will appear in a dropdown for the end user to pick between.
<3> A user-friendly name for this tag within the image stream. This should be short and include version information when appropriate.
<4> A description of this tag, which should contain enough detail so users understand what the image is providing.  It can include
links to additional instructions.  Keep the length of the description limited to a few sentences.
<5> The icon to show for this tag. Pick from our existing https://rawgit.com/openshift/origin-web-console/master/app/styles/fonts/openshift-logos-icon/demo.html[logo icons]
when possible. Icons from http://fontawesome.io/icons/[FontAwesome] and https://www.patternfly.org/styles/icons/[Patternfly] can also be used. Alternatively provide icons through
xref:../install_config/web_console_customization.html#loading-custom-scripts-and-stylesheets[CSS customizations] that can be added to an OpenShift cluster that uses your Image Stream.
You must specify an iconClass that exists or it will prevent falling back to the generic icon.
<6> A URL to a source repository that works with this builder image tag and results in a sample running application.
<7> Categories this image stream tag fits into.  The builder tag is required for it to show up in the catalog. Add tags that will include it into one
of the out of the box catalog categories.  Refer to the `id` and `categoryAliases` in `CATALOG_CATEGORIES` in the console's
https://github.com/openshift/origin-web-console/blob/master/app/scripts/constants.js[constants file]. The categories can also be
xref:../install_config/web_console_customization.html#configuring-catalog-categories[customized] for the whole cluster.
