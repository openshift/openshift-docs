...
Currently, it is also possible to make them work with native Kubernetes resources,
such as xref:../../dev_guide/jobs.adoc#dev-guide-jobs[Jobs],
xref:../architecture/core_concepts/deployments.adoc#replication-controllers[Replication Controllers],
xref:../architecture/core_concepts/deployments.adoc#replication-controllers[Replication Controllers] or
xref:../dev_guide/deployments/kubernetes_deployments.adoc#dev-guide-kubernetes-deployments-support[Kubernetes Deployments].

Which exact resources can be used is xref:../admin_guide/image_policy.adoc#admin-guide-image-policy[Image Policy][configured by cluster administrator].
...
. Setting image stream resolution at a resource level. This will allow only this
  resource to use image stream name in the image field.
...
Both of these operation can be done using `oc set image-lookup`. Example:

----
$ oc set image-lookup mysql
----

will allow all resource to use image stream named mysql to use it.

----
$ oc set image-lookup deploy/mysql
----

will allow Kubernetes Deployment named mysql to use image streams.

You can query image stream and see if they have that option set using:
