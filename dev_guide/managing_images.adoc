...
----
$ oc login -u user
$ curl -k -u unused:$(oc whoami -t) https://<registry_server>:<port>/v2/_catalog?n=100
----

You can use registry certificate to list repositories with secure connection:
----
$ sudo cp /etc/origin/master/registry.crt /etc/pki/ca-trust/source/anchors/ && sudo update-ca-trust
$ curl -u unused:$(oc whoami -t) https://<registry_server_external_route>/v2/_catalog?n=100
----
