...
A _tracking_ tag means the destination tag's metadata is be updated during
...
To move one or more images from one Docker registry to another,
use `oc image mirror` command. The images are streamed from registry to registry
without being stored locally.

For example, to copy image from dockerhub to the integrated registry use the
following command:

[source, bash]
----
$ oc image mirror docker.io/library/busybox:latest 172.30.124.220:5000/myproject/toybox:latest
----

[IMPORTANT]
====
If you use `docker.io` in source or destination you can't omit `docker.io` and
`library` components. You can't omit `latest` if you want to get latest tag.
====

It's possible to copy an image to several locations at once. To do this, you
need to specify several destinations:

[source, bash]
----
$ oc image mirror 172.30.124.220:5000/myproject/busybox:latest docker.io/myrepository/busybox:stable docker.io/myrepository/toybox:dev
----

[NONE]
====
The `oc image mirror` runs locally, not inside the openshift cluster. This means
that this command must have access to the source and destination registry.
====

If the Docker registry requires authentication to pull or push an image, you
you need to log in manually (using the `docker login` command) before executing
the oc image mirror command.
