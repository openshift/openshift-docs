...
To expose a service by giving it an externally-reachable hostname like `www.example.com` use an {product-title} xref:../routes.adoc#dev-guide-routes[route]. In your case you need to expose the frontend service by typing:
