...
With this command, {product-title} fetches the source code, sets up the builder image, xref:../builds/index.adoc#dev-guide-how-builds-work[builds] your application image, and deploys the newly created image together with the specified xref:../../using_images/db_images/postgresql.adoc#postgresql-environment-variables[environment variables]. The application is named `rails-app`.
