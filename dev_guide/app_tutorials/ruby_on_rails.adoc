...
In order to have your application communicating with the PostgreSQL database service that will be running in OpenShift, you will need to edit the `default` section in your `config/database.yml` to use xref:../../using_images/db_images/postgresql.adoc#postgresql-environment-variables[environment variables], which you will define later, upon the database service creation.
...
* Creating a database xref:../../architecture/core_concepts/pods_and_services.adoc#services[service] from OpenShift's xref:../../using_images/db_images/postgresql.adoc#using-images-db-images-postgresql[PostgreSQL image]
* Creating a frontend xref:../../architecture/core_concepts/pods_and_services.adoc#services[service] from OpenShift's xref:../../using_images/s2i_images/ruby.adoc#using-images-s2i-images-ruby[Ruby 2.0 builder image] and your Ruby on Rails source code, which we wire with the database service
...
To bring your application to OpenShift, you need to specify a repository in which your application lives, using once again the xref:../new_app.adoc#dev-guide-new-app[oc new-app] command, in which you will need to specify database related xref:../../using_images/db_images/postgresql.adoc#environment-variables[environment variables] we setup in the xref:./ruby_on_rails.adoc#creating-the-database-service[Creating the Database Service]:
...
With this command, OpenShift fetches the source code, sets up the Builder image, xref:../builds.adoc#dev-guide-builds[builds] your application image, and deploys the newly created image together with the specified xref:../../using_images/db_images/postgresql.adoc#environment-variables[environment variables]. The application is named `rails-app`.
...
Once the xref:../builds.adoc#dev-guide-builds[build] is complete, you can look at the running xref:../../architecture/core_concepts/pods_and_services.adoc#pods[pods] in OpenShift
...
To expose a service by giving it an externally-reachable hostname like `www.example.com` use OpenShift xref:../routes.adoc#dev-guide-routes[route]. In your case you need to expose the frontend service by typing:
