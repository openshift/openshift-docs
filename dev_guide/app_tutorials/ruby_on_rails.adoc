...
In order to have your application communicating with the PostgreSQL database service that will be running in {product-title}, you will need to edit the `default` section in your `config/database.yml` to use xref:../../using_images/db_images/postgresql.adoc#environment-variables[environment variables], which you will define later, upon the database service creation.
...
With this command, {product-title} fetches the source code, sets up the Builder image, xref:../../dev_guide/builds.adoc#dev-guide-builds[builds] your application image, and deploys the newly created image together with the specified xref:../../using_images/db_images/postgresql.adoc#environment-variables[environment variables]. The application is named `rails-app`.
