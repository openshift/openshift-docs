...
[NOTE]
====
For Pipeline strategy builds, given the current restrictions with the Git plug-in for Jenkins, any Git
operations via the Git plug-in will not leverage the HTTP or HTTPS proxy defined in the `BuildConfig`.
The Git plug-in only will use the the proxy configured in the Jenkins UI at the Plugin Manager panel.
This proxy will then be used for all git interactions within Jenkins, across all jobs.
Instruction on how to configure proxies via the Jenkins UI can be found
link:https://wiki.jenkins-ci.org/display/JENKINS/JenkinsBehindProxy[`here.`]
====
