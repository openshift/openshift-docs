...
[NOTE]
====
As noted above, while it is not required, It is still recommended that you specify all possible
environment variables / job parameters in the `BuildConfig`.  Doing so reduces
disk I/O and consequently improves performance during Jenkins processing.
====
