...
provided by the various xref:build_inputs.adoc#dev-guide-build-inputs[input
...
Note the following details on the mappings between the Pipeline strategy `BuildConfig` environment variables and Jenkins job parameters:

* When a Jenkins job is created or updated based on changes to the `BuildConfig`, any environment variables in the `BuildConfig` are mapped to Jenkins job parameters definitions. The default values for the Jenkins job parameters definitions are the current values of the associated environment variables.
* You can add additional parameters (whose names differ from the names of the environment variables in the `BuildConfig`) 
to the Jenkins job from the Jenkins console after the job's initial creation. These additional parameters are honored when builds are started for those Jenkins jobs.


How you start a build for the Jenkins job determines how the parameters are set.

* `oc start-build`: the current values for the environment variables specified in the BuildConfig take precedence over whatever default value the corresponding parameter in the Jenkins job has. 
+
Any changes you make to the default  parameter values from the Jenkins console are ignored. 

* `oc start-build -e`: the values for the environment variables specified with the `-e` option take precedence and override any environment variables of the same name listed in the `BuildConfig`. Any variables specified in the `BuildConfig` that are not referenced with the `-e` options are still taken.
+
[NOTE] 
====
If you specify an environment variable not listed in the `BuildConfig`, that variable will be added as a Jenkins job parameter definition.
====
+
Any changes you make from the Jenkins console to the parameters corresponding to the environment variables are ignored.  

* *Jenkins console*: set the environment variable though the Jenkins console as part of starting the build job.
