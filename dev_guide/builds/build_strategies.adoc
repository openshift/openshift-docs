...
Source to image supports a `.s2iignore` file which contains a list of file patterns that should
be ignored.  Files in the build working directory (as provided by the various xref:build_inputs.adoc#[input sources]) 
which match a pattern found in the `.s2iignore` file will not be made available to the `assemble` script.
