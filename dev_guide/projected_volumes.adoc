...
== Example Scenarios
// https://github.com/kubernetes/community/blob/master/contributors/design-proposals/all-in-one-volume.md
...
The steps can be used to create a user name and password xref:../dev_guide/secrets.adoc#dev-guide-secret[Secrets] from local files. You then create a pod that runs one container, using a projected volume to mount the Secrets into the same shared directory.
