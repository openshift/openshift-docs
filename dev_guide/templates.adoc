...
[NOTE]
====
`Bind` response key/value pairs may be used in other parts of the system as
environment variables. It is therefore recommended that every annotation key
with its prefix removed should be a valid environment variable name, i.e.
...
characters such as `.`, `@`, and so on as metacharacters, regardless of their
