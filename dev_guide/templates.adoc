...
the object in the template. The annotation key following the prefix is passed
through to key the returned field in the `bind` response.
...
The value of the expose annotation is a
...
field whose value should be returned.
...
- kind: Service
  ...
  metadata:
    annotations:
      template.openshift.io/expose-service-ip-port: "{.spec.clusterIP}:{.spec.ports[?(.name==\"web\")].port}"
  spec:
    ports:
    - name: "web"
      port: 8080
...
----
{
  "credentials": {
    "username": "foo",
    "password": "YmFy",
    "service-ip-port": "172.30.12.34:8080",
...
=== Adding NodeSelectors to a Template

You can add xref:../dev_guide/deployments/basic_deployment_operations.adoc#assigning-pods-to-specific-nodes[node selectors] to a new or existing template.

Edit the tempalte configuration file and add the `spec.nodeSelector` parameter to under `template` and add node selector section
...
Also, change the template name and namespace values in the `metadata` section. Save the configuration file with a new name and `.json` extension.
