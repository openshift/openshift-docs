...
[[adding-node-selector-to-deployment-config]]
=== Adding Default NodeSelectors to a DeploymentConfig

Make specific components to deploy on specific nodes by adding xref:../../dev_guide/deployments/basic_deployment_operations.adoc#assigning-pods-to-specific-nodes[node selectors] to a deployment configuration.

----
#oc edit dc <deploymentConfigName> -o json
----

Add the `spec.nodeSelector` parameter to under `template` and add node selector section
