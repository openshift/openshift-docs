...
Learn more about
xref:../../deployments/advanced_deployment_strategies.adoc#dev-guide-advanced-deployment-strategies[advanced deployment strategies].

Another way is to use `customParams` to inject the custom deployment logic into the
existing deployment strategies. This can be done by providing a custom shell script
logic and calling the `openshift-deploy` binary. In that case, users don't have to supply
their custom deployer container image, but the default {product-title} deployer image will
be used instead:
