...
A common example of this is configuring a xref:../../admin_guide/high_availability.adoc#admin-guide-high-availability[highly available] service.
...
xref:../../admin_guide/high_availability.adoc#admin-guide-high-availability[High availability] improves the chances that an IP address will remain active, by assigning a virtual IP address to the host in a configured pool of hosts. If the host goes down, the virtual IP address is automatically transferred to another host in the pool.
