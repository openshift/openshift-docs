...
An administrator can create a wildcard DNS entry, and then set up a router. Afterward, the users can self-service the
...
link:http://kubernetes.io/docs/user-guide/services/#type-loadbalancer[Load balancers] are available on AWS and GCE clouds, and non-cloud options are also available.

The non-cloud load balancer allocates a unique IP from a configured pool. This limits you to a
...
All of the approaches can expose VIPs.
