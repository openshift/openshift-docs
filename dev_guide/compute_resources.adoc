...
these resources in pod specs just like CPU and memory. The scheduler takes
...
implemented. There is no resource quota or limitrange support for these resources,
and they have no impact on QoS.
====
...
The HTTP request below advertises 5 "foo" resources on node `openshift-node-1`.
Please not that `~1` is the encoding for the character `/` in the patch path.
The operation path value in JSON-Patch is interpreted as a JSON-Pointer. For
more details, please refer to xref:https://tools.ietf.org/html/rfc6901#section-3[IETF RFC 6901, section 3].
