...
spec:
  ...
  to:
    ...
    name: frontend
----
<1> For xref:../architecture/core_concepts/routes.adoc#path-based-routes[path-based routing], specify a path component that can be compared against a URL.
endif::[]
...
For information on configuring routes using the CLI, see xref:../architecture/core_concepts/routes.adoc#route-types[Route Types].
