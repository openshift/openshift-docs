...
[[creating-routes-shards]]

== Creating Routes with router shards

In case us use the router sharding possibility 
link: ../install_config/install/deploy_router.adoc#creating-router-shards[Creating Router Shards]
you need to add the labels for the dedicated routes.

----
oc expose svc/low-sla-frontend --hostname=low-sla.example.com --labels='sla=low'
----
