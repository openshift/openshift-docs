...
A common next step for users of OpenShift Pipelines is to launch Jenkins in one 
project and then have the OpenShift Plugin monitor a group of projects that the 
developers work in.

The recommended steps to do this include:

* Disable the auto-provisioning feature 
ifdef::openshift-origin,openshift-enterprise[]
For information on how to disable
Jenkins auto=provisioning, see
xref:../../install_config/configuring_pipeline_execution.adoc#overview[Configuring
Pipeline Execution].
endif::[]

* Enable the Jenkins Service Account to have access to each of the projects that 
will run OpenShift Pipelines
ifdef::openshift-origin,openshift-enterprise[]
For information on how to add access see
xref:../../using_images/other_images/jenkins.adoc#jenkins-cross-project-access[Cross Project
Access].
endif::[]

* And then add the projects to monitor by either
** Logging into the Jenkins console, going to "Manage Jenkins", then "Configure System", and then
update the "Namespace" field under "OpenShift Jenkins Sync"
** Extend the OpenShift Jenkins image using the
