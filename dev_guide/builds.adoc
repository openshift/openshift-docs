...
The set of TLS certificate authorities that are trusted during a `*git clone*` operation are
built into the {product-title} infrastructure images.  If your git server uses a self-signed
certificate or one signed by an authority not trusted by the image, you can disable TLS 
verification by setting the `GIT_SSL_NO_VERIFY` environment variable to `*true*` in the
appropriate strategy section of your build configuration.  You can use the xref:../dev_guide/environment_variables.adoc#overview[oc set env] 
command to manage `*BuildConfig*` environment variables.
