...
[[source-to-image-strategy-specific-options]]

== Source-to-Image strategy specific options

=== Incremental builds
S2I can perform incremental builds, which means it will reuse artifacts from previously-built images. To
...
You can override the scripts (`assemble`, `run`, `save-artifacts`) provided by the builder image in one of two ways: Either provide an `assemble`, `run`, and/or `save-artifacts` script in the `.sti/bin` directory of your application source repository, or provide a url of a directory containing the scripts as part of the strategy definition:
...
[NOTE]
Files located at the `scripts` url take precedence over files located in `.sti/bin` of the source repository.
For information on how S2I scripts are used see the link:../creating_images/s2i.html[creating images guide] and the link:https://github.com/openshift/source-to-image/blob/master/docs/builder_image.md#sti-scripts[S2I] documentation.

[[docker-strategy-specific-options]]

== Docker strategy specific options

=== No cache
Docker builds normally reuse cached layers found on the host performing the build.  The no cache option forces the build to ignore cached layers and rerun all steps of the Dockerfile:
