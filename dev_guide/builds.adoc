...
Similar to the xref:buildconfig-environment[*BuildConfig* environment] variables, the environment variables
defined in the above payload will be made available to your build.  When variable names in the above payload
collide with the names of the xref:buildconfig-environment[*BuildConfig* environment] variables, the variables
...
A build is completed by a pod that consumes resources (memory and CPU) on a
