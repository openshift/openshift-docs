...
<3> You can specify a list of link:#build-triggers[triggers], which cause a new
build to be created.
...
<7> The `*postCommit*` section defines an optional link:#build-hooks[build
hook].
====
...
field, OpenShift will perform a shallow clone (`--depth=1` clone). That means
only the `HEAD` (usually the `master` branch) will be downloaded. This result
into much faster download times of repositories with rich history of commits.

The shallow clone is also used when the `*ref*` field is specified and set to an
existing remote branch name. However, if you specify the `*ref*` field to a commit
SHA256 the system will fallback to a regular git clone operation and then checkout the
commit because using the `--depth=1` option only works with named branch refs.

If you want to perform a full Git clone of the `master` for the specified
repository, you can set the `*ref*` to `master`.
...
Setting the `*runPolicy*` field to `*SerialLatestOnly*` will cause all new
builds created from the `*Build*` configuration to be run sequentially,
same as using the `*Serial*` run policy. The difference is that when a currently
running build completes, the next build that will run is the latest build
created. In other words, you don't wait for the queued builds to run as they
will be skipped. Skipped builds are marked as `*Cancelled*`.
This policy can be used for fast, iterative development. 
...
The `sample-build-2` build will be cancelled (skipped) and the next build that will
run after `sample-build-1` complete will be the `sample-build-3` build:
...
Setting the `*runPolicy*` field to `*Parallel*` will cause all new builds
created from the `*Build*` configuration to be run in parallel.  This can
produce unpredictable results as the first created build can complete last which
and will replace the pushed Docker image produced by the last build which
completed earlier.
Use the parallel run policy in cases where you don't care about the order in which
the builds will complete.
...
|You have exceeded one of the link:../admin_guide/quota.html#usage-limits[image
quotas] set on your project. Consult `oc describe quota` and see the limits
applied and storage in use.
endif::[]
ifdef::openshift-dedicated,openshift-online[]
|You have exceeded one of the image quotas set on your project. Consult
`oc describe quota` and see the limits applied and storage in use.
endif::[]
|===
