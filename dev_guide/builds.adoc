...
<1> In order to allow a generic webhook to pass in environment variables, this
...
Similar to the link:#buildconfig-environment[*BuildConfig* environment] variables, the environment variables
defined in the above payload will be made available to your build.  When variable names in the above payload
collide with the names of the link:#buildconfig-environment[*BuildConfig* environment] variables, the variables
in the payload will take precedence.  By default, environment variables passed via webhook are ignored.
You must set `*allowEnv*` to true on the webhook definition to enable this behavior.
