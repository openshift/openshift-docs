...
<1> Similar to the link:#buildconfig-environment[*BuildConfig* environment]
variables, the environment variables defined here are made available to your
build. If these variables collide with the
link:#buildconfig-environment[*BuildConfig* environment] variables, these
...
To pass this payload using `curl`, define it in a file named *_payload_file.yaml_* and run:
