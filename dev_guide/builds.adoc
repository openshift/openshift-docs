      ...
      "onSuccess": [
        {
          ...
          "build": {
            "kind": "BuildConfig",
             "name": "downstream-buildconfig-1",
             "namespace": "downstream-bc-namespace"
          }
        }
      ],
      "onFailure": [
...
build to be created.
<3> You can specify a new type of post action trigger that executes when the
current build enters a finished state.
...
[build-post-actions]
== Build Post Actions
Trigger downstream builds after build completion.
Triggering any of the above actions in a different namespace is possible as long
as the user is allowed to start builds in the said namespace.

[[on-success]]

=== On Success Post Action
This will run once the build succeeded triggering the build of one or
more specified images.
...
"onSuccess": [
  {
    ...
    "build": {
      "kind": "BuildConfig",
       "name": "downstream-buildconfig-1", <1>
       "namespace": "downstream-bc-namespace" <2>
    }
  }
]
----
<1> Specify the buildconfig that should be triggered when the current image is
successful.
<2> Optional field that specifies the namespace where this build should be
triggered. If we omit this field, the default namespace is the on of upstream
build.

[[onFailure]]
=== On Failure Post Action
This will run when the build fails and trigger notifications.
...
"onFailure": [
  {
    ...
    "build": {
      "kind": "BuildConfig",
      "name": "downstream-buildconfig-3", <1>
      "namespace": "downstream-bc-namespace"
      }
  }
]
