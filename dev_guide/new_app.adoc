...
repository by specifying the image (either an image stream or Docker specification) and the repository, with a `~` as a separator.
...
Only
<<<<<<< HEAD
xref:../architecture/core_concepts/builds_and_image_streams.adoc#referencing-images-in-image-streams[image
...
>>>>>>> f0ec379... edits from minimalism seminar
