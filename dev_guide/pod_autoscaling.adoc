...
* `AbleToScale` indicates whether the horzizontal pod autoscaler is able to fetch and update scales, and
...
* `ScalingActive` indicates whether the horzizontal pod autoscaler is enabled (the replica count of the target is not zero) and
