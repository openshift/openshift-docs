...
during resource spikes, such as during start up. Pods in the `unready` state will 
have `0 CPU` usage when scaling up and the autoscaler will ignore the pods when scaling down.  
Pods without known metrics have a `0% CPU` usage when scaling up and 100% CPU when scaling down.  
This will allow for more stability during the HPA decision. To use this feature, you must configure
