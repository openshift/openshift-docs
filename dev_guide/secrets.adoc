...
original pod and create a new pod (perhaps with an identical PodSpec). When you mount a secret as a volume, your secret automatically gets updated.
...
If your container uses a `secret` as an environment variable, you must restart the container to see the updated `secret`. See xref:secrets-examples[examples] of YAML files with secret data.
