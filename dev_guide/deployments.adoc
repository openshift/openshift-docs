...
== Viewing the logs of a deployment

To view the logs of the latest deployment for a config:

----
$ oc logs dc/<deployment_config> [--follow]
----

Note that if the deployment is successful, there will be no logs to view. Logs 
can be retrieved either when the deployment is running or fails. There is also
the ability to view logs from older deployments:

----
$ oc logs --version=1 dc/<deployment_config> 
----

The above command will return the logs from the first deployment of the provided
deployment config, if and only if that deployment exists (has failed and hasn't 
been manually deleted or pruned).
