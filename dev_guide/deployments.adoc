...
== Deployment Client Tool Operations
Use the 'ocs deploy' command with available options to start, view, cancel, or
retry a deployment.

----
$ oc deploy <deployment_config> [--options]
----

[cols= 2]
|===
|`oc deploy <deployment_config> --latest`
|Starts the specified deployment

|`oc deploy <deployment_config>`
|Dsiplays the specified deployment

|`oc deploy <deployment_config> --retry`
|Attempts to retry to deploy the last deployment. The deployer pod and any hook pods are deleted.

|`oc deploy <deployment_config> --cancel`
|Cancels the deployment that is currently in progress
