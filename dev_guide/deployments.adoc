...
<3> A link:#config-change-trigger[configuration change trigger] causes a new deployment to be created any time the pod template changes.
...
<5> The link:#rolling-strategy[Rolling strategy] is the default way of deploying your pods. May be omitted.
...
<7> Revision history limit is the limit of old deployments you want to keep around for rolling back. May be omitted.
<8> Minimum seconds to wait on top of the readiness checks in the container for a pod to be considered available. May be omitted.
====
...
This will show details about all recent deployments, including any currently running deployment.
You can view details specific to a deployment by using the `--revision` flag:
...
deployment specified in the rollout command, and a new deployment will be
started.
...
----
$ oc rollout enable-triggers dc/<name>
----

To roll back to a specific version:

----
$ oc rollout undo dc/<name> --to-version=1
----

To see what the rollback would look like without performing the rollback:

----
$ oc rollout undo dc/<name> --dry-run
----
...
To stream the logs of the latest deployment for a given deployment configuration:

----
$ oc logs -f dc/<name>
----

If the latest deployment is running or failed, `oc logs` will return the logs of the
...
<2> `*pre*`, `*mid*`, and `*post*` are both link:#lifecycle-hooks[lifecycle hooks].
====
