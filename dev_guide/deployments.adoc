...
Use the 'oc deploy' command with available options to start, view, cancel, or
retry a deployment.
...
|Manually starts the specified deployment

|`oc deploy <deployment_config>`
|Displays information about the specified deployment

|`oc deploy <deployment_config> --retry`
|Retries the last deployment if it failed.
