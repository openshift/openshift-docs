...
<2> An optional parameter, specifying the percentage of a new deployment's replicas to be scaled each interval (rather than the default of 1 replica per interval). For example, an `*updatePercent*` of 10, with a deployment of 50 replicas, will be scaled up and down 5 replicas each interval.
