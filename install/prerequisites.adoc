...
You must configure storage for each system that runs a container daemon. By default, the
...
to update your DNS configuration when new routes are added. If you configure a
wildcard for the router, set the `openshift_master_default_subdomain` parameter
to this value when you configure xref:configuring_inventory_file.adoc#advanced-install-networking-variables-table[the
