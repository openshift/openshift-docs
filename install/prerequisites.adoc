...
link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html-single/installation_guide/index[Red Hat Enterprise Linux (RHEL) 7.4 or 7.5]
...
[[configuring-hypervisor-no-overcommit]]
=== Configuring underlying Hypervisor to guaranteed resources / no overcommit

{product-title} nodes can run virtualized on supported hypervisors. Care must
be taken to configure the underlying hypervisor / virtualization infrastructure
correctly. It is **not recommend to run {product-title} nodes on overcommitted
hypervisors**. You should configure the virtualization infrastructure in a way
that for all {product-title} virtual machines, the workload on one virtual machine
does not affect the processing resources available to the other virtual
machines. Guaranteed resources, i.e. no overcommitment on CPU and RAM
resources is strongly recommended.

Rational: You would end up with two schedulers (the hypervisor and kubernetes)
to fight against each other. This could lead to unstable and erratic behaviour
of the system.

Example: Kubernetes could schedule a new pod onto a node where it _thinks_
there is enough cpu capacity, but _actually_ the node is slow because it does
not get enough cpu resources from the hypervisor (due to cpu overcommitment).
By the time the hypervisor detects this and assigns more cpu resources,
kubernetes could re-schedule the pod to a different node.

Please see with the documentation of the used hypervisor on how to configure
accordingly.
