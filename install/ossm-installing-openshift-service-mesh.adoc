...
For clusters without existing OpenShift Service Mesh instances, ensure proper scope configuration to prevent conflicts between Istio control planes, as Service Mesh is deployed cluster-wide. For clusters with OpenShift Service Mesh 3.0 or later, see xref:../install/ossm-deploying-multiple-service-meshes-on-single-clusterc#ossm-deploying-multiple-service-meshes-on-single-cluster[Deploying multiple service meshes on a single cluster].
====
