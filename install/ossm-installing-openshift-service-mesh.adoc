...
For clusters without existing {SMProduct} instances, ensure proper scope configuration to prevent conflicts between {istio} control planes, as {SMProductShortName} is deployed cluster-wide. For clusters with {SMProduct} 3 or later, see xref:../install/ossm-deploying-multiple-service-meshes-on-single-clusterc#ossm-deploying-multiple-service-meshes-on-single-cluster[Deploying multiple service meshes on a single cluster].
====
