...
[IMPORTANT]
====
Testing has shown issues with using the NFS server on RHEL as storage backend for the container registry. This includes the embedded registry and Quay, Cassandra for metrics storage, and ElasticSearch for logging storage. Therefore, using NFS on RHEL as storage for these framework services is not recommended.

Other NFS implementations on the marketplace may not have these issues. Please contact the individual NFS implementation vendor for more information on any testing they may have performed against these OpenShift framework components.
====
...
The admin console is an additional web interface like the web console, but
focused on admin tasks. The admin console supports many of the same common
...
following variables can be used to customize the admin console.

.Admin Console Customization Variables
[options="header"]
|===
...
|Determines whether to install the admin console. Can be set to `true` or `false`. Defaults to `true`.

|`openshift_console_hostname`
|Sets the hostname of the admin console. Defaults to `console.<openshift_master_default_subdomain>`. If you alter this variable, ensure the hostname is accessible via your router.

|`openshift_console_cert`
|Optional certificate to use for the admin console route. This is only needed if using a custom hostname.

|`openshift_console_key`
|Optional key to use for the admin console route. This is only needed if using a custom hostname.

|`openshift_console_ca`
|Optional CA to use for the admin console route. This is only needed if using a custom hostname.

|`openshift_base_path`
|Optional base path for the admin console. If set, it should begin and end with a slash like `/console/`. Defaults to `/` (no base path).
