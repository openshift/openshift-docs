...
Enterprise Linux (RHEL) 7.5 or later with the latest packages from the Extras
channel or RHEL Atomic Host 7.4.2 or later.
* For cloud-based installations, use a base installation of RHEL 7.5 or later with
...
- https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux_atomic_host/7/html/installation_and_configuration_guide/index[Red Hat Enterprise Linux Atomic Host 7 Installation and Configuration Guide]
...
[NOTE]
====
If you encounter an error while running the `yum install openshift-ansible` command, see link:https://access.redhat.com/solutions/3949501[the resolution to fix that error].
====
+
...
.. In *_/etc/sysconfig/docker-storage-setup_*, set *DEVS* to the path of the block device to use. Set *VG* to the volume group name to create, such as
