...
See the xref:../install_config/install/index.adoc[Administrator Guide] for
...
- Enabling clustering for xref:../using_images/db_images/index.adoc[database
images].
- Using the
xref:../architecture/infrastructure_components/web_console.adoc#jvm-console[JVM
Console].
- Using xref:../dev_guide/persistent_volumes.adoc[persistent volume] plug-ins
other than the supported xref:../admin_guide/persistent_storage_nfs.adoc[NFS]
plug-in, such as
...
xref:../whats_new/ose_3_0_release_notes.adoc[here].
...
Red Hat JBoss EAP is available as a containerized
...
xref:../using_images/xpaas_images/eap.adoc[this documentation].
...
xref:../using_images/s2i_images/index.adoc[S2I section] of the image
documentation.
...
xref:../using_images/xpaas_images/jws.adoc[this documentation].
...
Red Hat JBoss A-MQ is available as a containerized
...
xref:../install_config/upgrades.adoc[upgrading your OpenShift cluster] properly,
...
* To make it easier to xref:../install_config/upgrades.adoc[upgrade your cluster],
the `oadm reconcile-cluster-roles` command has been added to
xref:../install_config/upgrades.adoc#updating-policy-definitions[update your
...
* A new
xref:../admin_guide/configuring_authentication.adoc#LDAPPasswordIdentityProvider[LDAP
...
xref:../admin_guide/service_accounts.adoc#enable-service-account-authentication[set
the `*masterCA*` field] in the
xref:../admin_guide/master_node_configuration.adoc#master-configuration-files[master
configuration file].
...
creating xref:../architecture/additional_concepts/authorization.adoc[role
...
* You can now xref:../admin_guide/web_console_customization.adoc[customize the
