...
[id="gcp-ccs"]
= Customer Cloud Subscriptions on {gcp-full}
:context: gcp-ccs

toc::[]

[role="_abstract"]
{product-title} provides a Customer Cloud Subscription (CCS) model that allows Red Hat to deploy and manage clusters in a customer's existing {gcp-full} account.
...
* For more information about creating an {product-title} cluster with the Workload Identity Federation (WIF) authentication type, see xref:../osd_gcp_clusters/creating-a-gcp-cluster-with-workload-identity-federation.adoc#osd-creating-a-cluster-on-gcp-with-workload-identity-federation[Creating a cluster on {gcp-full} with Workload Identity Federation authentication].
