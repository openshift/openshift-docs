...
+
For example: _+++https://www.example.com/sso-login?then=${url}+++_

* _${query}_ is replaced with the current query string, unescaped.
+
For example: _+++https://www.example.com/auth-proxy/oauth/authorize?${query}+++_
