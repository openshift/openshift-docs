...
(nodes). This is done by specifying
...
. Modify the master configuration file, *_/etc/origin/master/master-config.yaml_*, in two places to add `nodeSelectorLabelBlacklist` to the `admissionConfig` section:
+
====
----
...
