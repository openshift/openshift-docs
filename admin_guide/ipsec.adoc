...
When working with IPsec enabled, you should be aware of the following: 

* IPsec adds a header to outgoing packets. You should decrease the MTU of the NIC on each node accordingly to prevent packets from fragmenting, which leads to performance losses.

* The IPsec encryption and decryption process requires CPU power. High-traffic nodes will see an increase in CPU usage if the CPU does not have encryption offload capability.

* IPsec encrypts traffic at the IP level, before it hits the NIC, and it protects fields that otherwise could be used by NIC hardware offloading. As a result, some NIC acceleration features might not be usable when IPSec is enabled and throughput and CPU usage will increase.
