...
2. Filter out all containers that are running or are not alive longer
...
5. For each pod that contains more containers than
   `*maximum-dead-containers-per-container*`, sort the containers by creation time
6. Start removing containers from the oldest first
   until `*maximum-dead-containers-per-container*` is met
7. If there are still more containers in the list than `*maximum-dead-containers*`,
