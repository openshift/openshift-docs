...
Local roles should only be created if a cluster role does not provide the set
of permissions needed for a particular situation.

Some cluster role names are initially confusing. You can bind cluster role
`cluster-admin` to a user using a local role binding, making it appear
that this user has the privileges of a cluster administrator. This is not the case.
Binding the `cluster-admin` cluster role to a certain project grants the permissions of the cluster role
