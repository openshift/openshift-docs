...
An example configuration for running a link:./examples/geo-cache.json[geo-cache] service is provided along with this document. The docker image referenced in that file `myimages/geo-cache` does not exist and is there purely for reference purposes.
Edit the link:./examples/geo-cache.json[geo-cache.json] configuration to replace `myimages/geo-cache` with your favorite cache software docker image, change the number of replicas to 4 and ensure the label matches the one used in step 1 above `ha-cache=geo`.
...
****
`$ oscadm ipfailover ha-geo-cache --replicas=4 --selector="ha-cache=geo" --virtual-ips=10.245.2.101-104 --watch-port=9736 --credentials="$OPENSHIFTCONFIG" --create`
****
...
****
`$ oscadm ipfailover harreporter --selector="my-ha-service=har-reporter" --virtual-ips="10.245.2.42,10.245.2.100-104,10.245.2.142,10.245.2.242" --watch-port=4242 --replicas=7 --create`
****
