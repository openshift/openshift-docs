...
[IMPORTANT]
====
There is a maximum of 255 VIPs in the cluster.
====
...
The list of command options described here is a subset that is relevant to this document.
...
|80
|The ipfailover pod tries to open a TCP connection to this port on each VIP. If a connection is established, the service is considered to be running. If this port is set to 0, the test always passes.
