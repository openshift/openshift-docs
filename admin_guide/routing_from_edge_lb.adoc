...
Follow instructions in link:./high_availability.html#ip-failover[ipfailover]
documention to create a service account and add it to the *privileged*
scc.
...
you can manually edit the *privileged* scc and add the *f5ipfailover*
service account.
...
Then, configure *ipfailover* using your chosen VIP (`*RAMP_IP*`) and the
*ipfailover* service account , assigning the VIP to your two nodes using
the *f5rampnode* label:
