...
link:../persistent_storage_nfs.html[use persistent storage] using
`*PersistentVolume*` and `*PersistentVolumeClaim*` objects for storage
for the registry. For example, to attach an existing NFS volume to the
registry once it has been defined:
...
mode. This is automatically enabled for you when you specify `--mount-host`.
...
. Create a new link:../service_accounts.html[service account] in the *default*
project for the registry to run as. The following example creates a service
account named *registry*, but it can be named anything:
+
----
$ echo \
...
[NOTE]
====
The listing below is just a sample; what you see will vary based on the contents of your registry.
====
...
$ oadm ca create-server-cert --signer-cert=ca.crt \
    ...
    --cert=/etc/secrets/registry.crt --key=/etc/secrets/registry.key
----
+
