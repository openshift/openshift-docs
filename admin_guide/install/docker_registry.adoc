...
Docker registry] to can be deployed in your OpenShift environment to manage
images "locally".
...
For production use, you should link:../persistent_storage_nfs.html[use
persistent storage] using `*PersistentVolume*` and `*PersistentVolumeClaim*`
objects for storage for the registry.

For non-production use, you can use the `--mount-host` option to specify a
directory for the registry to use for persistent storage:
...
The registry volume is created as a host-mount at the specified `<path>`.
Note the following when using the `--mount-host` option:
...
configuration, it is possible that your registry pods and containers will run on
different nodes. This means that you could end up with two or more registry
containers, each with its own local storage. This will lead to unpredictable
behavior, as subsequent requests to pull the same image repeatedly may succeed
some times and fail others, depending on which container the request ultimately
goes to.

- The `--mount-host` options requires that the registry container run in
privileged mode. This is automatically enabled for you when you specify
`--mount-host`. However, by default, not all pods are allowed to run privileged
containers. If you still want to use this option:
+
. Create a service account for the registry to run as:
+
----
$ echo \
    '{"kind":"ServiceAccount","apiVersion":"v1","metadata":{"name":"registry"}}' \ <1>
    | oc create -f -
----
<1> Here the service is named *registry* but it can be named anything.

. Add the service account to the list of users allowed to run privileged
containers:
+
----
...
To access the registry directly such as to `docker push` or `docker pull`, you
must first log in to the registry using an access token.

. Ensure you are logged in to OpenShift as a real user:
...
You can now perform `docker pull` and `docker push` operations.
...
After you have a registry deployed, you can link:deploy_router.html[deploy a
router], and then continue on to the link:first_steps.html[First Steps] topic to
