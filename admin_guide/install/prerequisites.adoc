...
access to all hosts. For running the installer as a non-root user, passwordless
*sudo* rights must also be configured on each destination host.
...
OpenShift using the link:quick_install.html#installing-openshift[quick
installation] or link:advanced_install.html#installing-ansible[advanced
installation] method.
