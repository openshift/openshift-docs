...
=== *Special Note About Using The Container Network Stack*

The OpenShift router runs inside a docker container and the default
behavior is to use the network stack of the host (node where the router
container runs).  This default behavior has performance benefits as the
network traffic from remote clients no longer needs to go through a double
hop (through user space) to reach the target service/container.
Additionally, it enables the router to get the actual source IP address of
the remote connection rather than get the node's IP address and is useful
for defining ingress rules based on the originating IP, supporting sticky
sessions, monitoring traffic amongst other uses. This behavior is
controlled by the `--host-network` router command line option and the
default behaviour is the equivalent of using `--host-network=true`.

If you wish to run the router with the container network stack (an example
is shown below), use the `--host-network=false` option.
Internally, this means the router container needs to publish the 80 and 443
ports in order for the external network to communicate with the router.
Please note that running with the container network stack means that the
router sees the source IP address of a connection to be the NATed IP
address (of the node) rather than the actual remote IP address.

----
# oadm router --credentials="$KUBECONFIG" --service-account=router --host-network=false
----
