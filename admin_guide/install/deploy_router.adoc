...
ingress point for external all traffic destined for
...
on OpenShift. It currently supports only HTTP(S) traffic and TLS-enabled
traffic via SNI.
...
For example, using the credentials the OpenShift master generates
by default for the router at startup and creating two instances on
different hosts:
...
A TLS-enabled route that does not include a certificate will use the router's
default certificate. In most cases, this certificate should be provided by a
...
application backends. For example, it might be desirable in a highly-available
