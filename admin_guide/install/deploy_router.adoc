...
If the command fails with below error, make sure that  $KUBECONFIG have the correct path
and rerun the command.

----
error: router could not be created; you must specify a .kubeconfig file path
containing credentials for connecting the router to the master with --credentials
----

The default $KUBECONFIG is present in /etc/openshift/master/openshift-router.kubeconfig.

Below is an example of how to set KUBECONFIG path.
----
export KUBECONFIG='/etc/openshift/master/openshift-router.kubeconfig'
----
