...
// tag::node-configmap[]
During installation, {product-title} creates a configuration map in the *openshift-node* project for each type of node group:

* *node-config-master* for xref:../architecture/infrastructure_components/kubernetes_infrastructure.adoc#master[master nodes];
* *node-config-infra* for xref:infrastructure-nodes[infrastructure nodes];
* *node-config-compute* for compute nodes.

In order to make configuration changes to an existing node, edit the appropriate configuration map. A _sync pod_ on each node
watches for changes in the configuration maps. The sync pods are automatically created by the installer using _sync daemonsets_ and
create a *_/etc/origin/node/node-config.yaml_* file on each node, where the node configuration parameters reside. When a sync pod
detects configuration map change, it updates the *_node-config.yaml_* on all nodes in that node group.
// end::node-configmap[]
...
[NOTE]
====
The *_/etc/origin/node/node-config.yaml_* must not be modified directly.
====
...
. Add the `*kubeletArguments*` section and include any desired options:
+
----
kubeletArguments:
  max-pods: <1>
