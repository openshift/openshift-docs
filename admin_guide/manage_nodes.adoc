...
[NOTE]
====
The `$ oc delete node <node>` command does not clean the filesystem of a node. A node that has been deleted can register itself again with {product-title} if you do not also xref:../install/uninstalling_cluster.adoc#uninstalling-nodes[uninstall the node].
====
