...
<8> Identifies the group and resource for image stream resolution. If resource is `*`,
...
[NOTE]
====
If you normally rely on infrastructure images being pulled using a default registry prefix (such as 'docker.io' or registry.access.redhat.com'),
those images will not match to any `matchRegistries` value since they will have no registry prefix.  To ensure infrastructure images
have a registry prefix that can match your image policy, set the xref:../install_config/master_node_configuration.adoc#master-config-image-config[imageConfig.format] value in your master-config.yaml.
====
