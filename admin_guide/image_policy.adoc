...
The xref:../install_config/install/advanced_install.adoc[advanced installation]
does not generate the setting, therefor it whitelists all registries. However,
the OpenShift master API spawned without any configuration file will use the
recommended list of registries.

The full recommended list of registries can be listed by writing out the
configuration file:

----
dir="$(mktemp -d)"
openshift start master --write-config="${dir}" && \
  sed -n '/^\s*allowedRegistriesForImport/,/^\s*[a-z]/p' "${dir}/master-config.yaml" && \
  rm -rf "${dir}"
----
