...
<7> Array of rules allowing the use of image streams in Kubernetes resources. By default,
    this will allow pods, replication controllers, replica sets, deployments, and jobs
    to use image streams.
<8> Identifies the group and resource for image stream resolution. If the resource is `*`,
