...
interchangeably.  _This applies only if you have enabled the Ephemeral Storage tech preview in {product-title} 3.10.  This is disabled by default; to enable this features, please
...
`*requests.memory*`, or `*ephemeral-storage*`, then it requires that
every incoming container make an explicit request for those
resources. If the quota has a value specified for `*limits.cpu*`,
`*limits.memory*`, or `*ephemeral-storage`, then it requires that
every incoming container specify an explicit limit for those
resources.
...
service for either memory, CPU, or ephemeral storage.
