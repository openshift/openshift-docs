...
ifdef::openshift-origin[]
OpenShift Origin 1.0.4 or later, first see the
...
ifdef::openshift-enterprise[]
OpenShift Enterprise 3.0.1.0 or later, first see the
link:#additional-instructions-per-release[Additional Instructions per Release]
...
With the
ifdef::openshift-origin[]
OpenShift Origin 1.0.5 or later
endif::[]
ifdef::openshift-enterprise[]
OpenShift Enterprise 3.0.2.0 or later
endif::[]
release, the router has been updated to use the host networking stack by
default instead of the old behavior of using the container network stack,
which proxied traffic to the router, which in turn proxied the traffic to
the target service/container. This default behavior has performance benefits
where network traffic from remote clients no longer needs to go through a
double hop (through user space) in order to search the target
service/container. Additionally, it enables the router to get the actual
source IP address of the remote connection and is useful for defining
ingress rules based on the originating IP, supporting sticky sessions,
monitoring traffic amongst other uses.

Existing router deployments will however continue to use the container
network stack and need to be modified to switch to using the host network
stack.
