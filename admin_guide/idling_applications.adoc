...
reduce the consumption of resources. This is useful when deployed on a public
cloud where cost is related to resource consumption.

Scalable resources that are not currently in use are discovered and idled by
scaling them to 0 replicas. When network traffic is directed to them they are
unidled, scaled up, and contiue operation.
