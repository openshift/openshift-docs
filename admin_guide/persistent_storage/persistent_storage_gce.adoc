...
[IMPORTANT]
====
Before creating persistent volumes using GCE, OpenShift must first be
properly link:../configuring_gce.html[configured for GCE Persistent Disk].
====
...
link:../../admin_guide/configuring_gce.html[configured for GCE PersistentDisk], all that is
required for Openshift and GCE is an GCE Persistent Disk volume ID and the `*PersistentVolume*` API.
