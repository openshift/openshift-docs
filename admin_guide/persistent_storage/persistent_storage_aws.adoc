...
[IMPORTANT]
====
Before creating persistent volumes using AWS, OpenShift must first be
properly link:../configuring_aws.html[configured for AWS ElasticBlockStore].
====
...
link:../../admin_guide/configuring_aws.html[configured for AWS Elastic Block Store], all that is
required for Openshift and AWS is an AWS EBS volume ID and the `*PersistentVolume*` API.
