...
Diagnostics are intended to assist with troubleshooting by detecting
and explaining many common problems with the OpenShift cluster and
ultimately automating troubleshooting effort. The diagnostic tool is
under development along with the project and is currently considered
experimental; it is included in the `openshift` binary itself so that
you can run it wherever you have a server or client installed.

=== Expected Diagnostics Usage

OpenShift can be deployed in many ways: built from source, included
in a VM image, in a Docker image, or as enterprise RPMs. Each method
implies different configuration and environment. In order to keep
...
Diagnostics look for config files in standard locations:
...
`--config`, `--master-config`, and `--node-config`). If a config file
...
Master and node diagnostics will be most useful in a specific target
environment, which is a deployment of RPMs with Ansible deployment
logic. This provides some diagnostic benefits:
...
locations and use them if found; this should make the Ansible-installed
use case as simple as possible. It's also easy to specify configuration
files that are not in the expected Enterprise locations:

    openshift ex diagnostics --master-config=... --node-config=...

Systemd units and logs entries in journald are necessary for the current
log diagnostic logic. For other deployment types, logs may be going into
files, to stdout, or may combine node and master. Log diagnostics are not
able to work properly and will be skipped in these situations at this time.

==== Client Environment

The user may have access as an ordinary user, and/or as a cluster-admin
user, and/or may be running on a host where OpenShift master or node
servers are operating. The diagnostics will attempt to use as much access
as the user has available.

A client with ordinary access should be able to diagnose its connection
to the master and run a diagnostic pod. If multiple users/masters are
configured, connections will be tested for all, but the diagnostic pod
only runs against the current user/server/project.

A client with cluster-admin access available (for any user but only the
current master) should be able to diagnose the status of infrastructure
such as nodes, registry, and router.

In each case, running:

    openshift ex diagnostics

looks for the client configuration in its standard location and uses it
if available. Note that there is no `oc ex diagnostics`, but if all you
have is the `oc` client, you can copy or symlink it with the name
`openshift` and it can then be used for diagnostics.
