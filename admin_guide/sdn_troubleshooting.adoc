...
* Kube - declare a service as NodePort and it will claim that port on all machines in the cluster (on what interface?) and then route into kube-proxy and then to a backing pod. See http://kubernetes.io/docs/user-guide/services/#type-nodeport (some node must be accessible from outside)
