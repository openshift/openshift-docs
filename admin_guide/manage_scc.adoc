...
pre-allocted UID, without granting everyone access to the privileged SCC, you
...
. Add the service account user name to privileged SCC using:
+
----
$ oc edit scc privileged
----
...
. Set the service account using `--service-account=<name>` in the `oadm
registry` command.
