...
Adding an SCC to a project is a question that is often asked.  SCCs are not granted
to a project.  In this case you will be adding a service account to an SCC and either
specifying a service account name on your pod or, when unspecified, running as the
...
If you are currently in the project that the service account belongs to you may use the `-z` flag:
----
