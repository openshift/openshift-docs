...
is used (see below). For this operation to work properly, ensure your
...
It's important to know that by default the integrated registry caches blobs metadata
to reduce the number of requests to the storage and increase the speed of processing
the request. Pruning doesn't update the integrated registry cache. This means that images
that are pushed after pruning and that contain pruned layers will be broken because
the pruned layers that have metadata in the cache will not be pushed, resulting in those
layers being missing after the push is complete. Therefore it is necessary to clear
the cache after pruning. This can be accomplished by redeploying the registry:
...
xref:../install_config/registry/extended_registry_configuration.adoc#docker-registry-configuration-reference-redis[redis cache]
then you need to clean the database manually.

If redeploying the registry after pruning is not acceptable to you,
then you must
xref:../install_config/registry/extended_registry_configuration.adoc#docker-registry-configuration-reference-cache[permanently disable the cache]
instead.
...
We also recommend to xref:manage_authorization_policy.adoc[restrict the access] to cron jobs
