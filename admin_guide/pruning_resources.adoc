...
[NOTE]
====
Pruning images removes data from the docker registry. For this operation
to work properly make sure your registry is configured with `storage:delete:enabled`
set to `true`. See
xref:../install_config/install/docker_registry.adoc#docker-registry-configuration-reference-storage[registry storage configuration]
for more info.
====
