...
.^|`--exclude-imagestreamtag=<regexp>`
|Exclude an imagestream from pruning you can specify a regular expression which
will be applied to the `<namespace>/<name>:<tag>` string.

.^|`--exclude-imagestreamtag-file=<filename>`
|The filename that contains the regular expressions to exclude an imagestream
from pruning.
