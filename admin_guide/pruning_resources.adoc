...
from the current user's configuration file. If provided, a secure connection
will be initiated.
...
case it fails (the registry cannot be resolved or reached), use this flag to
...
2. The provided `registry-url` is a local-link address or localhost.
3. The configuration of the current user allows for an insecure connection.
...
----
$ image_name="sha:abz"
$ oc get is -n <namespace> -o go-template='{{range $isi, $is := .items}}{{range
...
run a standard prune just before the hard prune.
+
...
.. Get the registry's service account name:
+
----
