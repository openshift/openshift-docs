...
====
+
. Remove the `--insecure-registry` option from docker unit file in
*/etc/sysconfig/docker*, only for this particular registry. Then, reload the
daemon and restart the docker service to reflect this configuration change:
+
====
----
