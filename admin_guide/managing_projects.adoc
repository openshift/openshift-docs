...
[[recovering-deleted-default-projects]]

== Recovering Deleted Default Projects

If you delete any of the default projects like `*default*` or `*openshift*`,
then you will likely experience issues that impact the whole environment. To
recover from this, restart the master service:
...
[WARNING]
====
The default project `*openshift-infra*` is unrecoverable in OpenShift 3.0 and 3.1.
====

The master creates the missing namespaces and bindings on startup.

After restarting, verify the namespaces exist and are not in the *Terminating*
state:

====
----
# oc get namespace/default namespace/openshift namespace/openshift-infra
----
====

Next, you need to manually recreate the content (including templates, image
streams, the registry, the router, and so on) by following the steps in
link:../install_config/install/docker_registry.html[Deploying a Docker
Registry], link:../install_config/install/deploy_router.html[Deploying a
Router], and link:../install_config/imagestreams_templates.html[Loading the
