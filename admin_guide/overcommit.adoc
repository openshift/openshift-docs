...
The xref:../admin_guide/scheduler.adoc#admin-guide-scheduler[scheduler] attempts to improve the
...
It is important to understand that oversubscribing the physical resources on a
...
Disable swap by default on your nodes in order to preserve quality of service
guarantees.
...
[WARNING]
====
Failure to disable swap results in nodes not recognizing that they are
experiencing *MemoryPressure*. Failure to recognize this condition results in
pods not getting the memory they made in their scheduling request. As a result,
additional pods are placed on the node to further increase memory pressure, and
this ultimately increases your risk of experiencing a system out of memory (OOM)
event.

If swap is enabled, any
xref:../admin_guide/out_of_resource_handling.adoc#out-of-resource-eviction-policy[out
of resource handling] eviction thresholds around available memory will not work
as expected. Take advantage of out of resource handling to allow pods to be
evicted from a node when it is under memory pressure, and rescheduled on an
alternative node that has no such pressure.
====
