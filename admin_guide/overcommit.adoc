...
A node is _overcommitted_ when the sum of limits across all pods on that node exceeds available compute resources. 
A node is also considered overcommitted when it has a pod scheduled that makes no request.

Compute resources are considered to be total CPU and memory that can be requested, allocated, and consumed.
...
time. You might want to overcommit, for example, in development environments  where
you need to execute a large number of jobs, where performance is not a concern 
you can endure pods being killed. 

Each node is assigned a specific amount of memory and CPU and each 
container can ask for the memory and CPU it needs. 
The scheduler finds the best fit for each container based on the CPU and memory on the nodes.

Scheduling is based on resources requested, which can be set higher than requested resources. The difference
...
Administrators can overcommit a node using _requests_ and _limits_ to allow and manage the override the limits 
of resources on a node.

Note also that after overrides, the container limits and requests must still
be validated by any xref:../admin_guide/limits.html#admin-guide-limits[`LimitRange` objects] in the project. It is possible,
for example, to specify a limit close to the minimum
limit, and have the request then be overridden below the minimum limit,
causing the pod to be forbidden. 
...
Pod resource capacity is the sum of the resource requests/limits of that type for each Container in the Pod.
...
are set (not equal to 0) for all resources and they are equal.
...
Can be requested as a plain integer or as a fixed-point integer using one of these suffixes: E, P, T, G, M, K. 
...
Can be requested as a whole number of decimal, where one CPU equals one core.
...
The following is an example of a master configured to allow overcommitting.
...
<2> (optional, 1-100) Allows the nodes to override the container memory limit. Here memory can be overridden by 25%.
<3> (optional, 1-100) Allows the nodes to override the CPU request to a percentage of CPU limit.  Here there CPU limit can be overridden by 25%.
...
*Specifying Compute Resources Requests and Limits* - Containers are given resource requests and limits through the xref:../dev_guide/compute_resources.html#dev-compute-resources[pod specifications]. You can use 
the specifications to create containers that can override the node limits.  

include::dev_guide/compute_resources.adoc[tag=node_compute_resources]


[[enforcing-cpu-limits]]
*Enforcing CPU Limits* - By default, nodes enforce specified CPU limits using the CPU CFS quota support in
...
We recommend that you reserve some portion of the node capacity to allow
for the system daemons that are required to run on your node for your cluster to
function (*sshd*, *docker*, etc.). In particular, reserve resources for 
...
. Create xref:../dev_guide/compute_resources.html#dev-compute-resources[pod specifications] that allow for overriding the node limits and create the pods. 
