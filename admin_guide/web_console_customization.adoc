[[admin-guide-web-console-customization]]
...
xref:../admin_guide/master_node_configuration.adoc#admin-guide-master-node-configuration[master configuration file]. The scripts and
...
applications that use the.adoc5 history API:
...
*_/path/to/myExtension/index.adoc_* rather than a "404 Not Found" page.
. The element `<base href="/">` will be rewritten in
*_/path/to/myExtension/index.adoc_* to use the actual base depending on the
...
xref:../admin_guide/configuring_authentication.adoc#RequestHeaderIdentityProvider[Request
Header] and OAuth or
xref:../admin_guide/configuring_authentication.adoc#OpenID[OpenID] identity
