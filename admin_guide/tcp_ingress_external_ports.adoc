...
- Have an IP address assigned from the `ingressIPNetworkCIDR`
...
then you must set `ingressIPNetworkCIDR` in the *_/etc/origin/master/master-config.yaml_* file before you assign an
...
[NOTE]
====
If you are using xref:../admin_guide/high_availability.adoc#admin-guide-high-availability[high availability], then this range must be less than 255 IP addresses.
====
endif::[]
