...
the *ovs-multitenant* SDN plugin
...
or *ovs-networkpolicy* plug-in enabled] in order to limit pod access via egress
policy.
====
...
[IMPORTANT]
====
Currently, the egress router is not compatible with Amazon AWS and Azure Cloud
due to their incompatibility with macvlan traffic.
====
...
ifdef::openshift-origin[]
  ...
  - name: egress-dns-proxy
ifdef::openshift-enterprise[]
    image: registry.access.redhat.com/openshift3/ose-egress-dns-proxy
endif::openshift-enterprise[]
ifdef::openshift-origin[]
