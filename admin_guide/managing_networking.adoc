...
domain from local non-authoritative server, or 30 minutes if unable to fetch the TTL.
We assume that pod also resolves the domain from the same local non-authoritative
server if needed, otherwise we could have a situation where IP addresses for the
domain perceived by egress network policy controller and the pod will be different
and the egress network policy may not be enforced as expected.
In the above example, suppose `www.foo.com` resolved to `10.11.12.13` and has a
DNS TTL of one minute, but was later changed to `20.21.22.23`. {product-title}
will then take up to one minute to adapt to these changes.
