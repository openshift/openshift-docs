...
[CAUTION]
====
Exposing services by creating xref:../dev_guide/routes.adoc#creating-routes[routes]
will ignore `EgressNetworkPolicy`. We do egress network policy service endpoint filtering
at the node kubeproxy and when the router is involved, kubeproxy is bypassed and egress
network policy enforcement is not applied. Admins can prevent this bypass by
limiting access to create routes.
====
