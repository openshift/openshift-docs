...
* If you merge two projects together using `oc adm pod-network join-projects`,
...
project, and can cause all external network traffic to be dropped.
...
`1.2.3.0/24`, but denies access to all other external IP addresses. (Traffic to
other pods is not affected because the policy only applies to _external_
traffic.)
+
The rules in an `EgressNetworkPolicy` are checked in order, and the first one
that matches takes effect. If the two rules in the above example were reversed,
then traffic would not be allowed to `1.2.3.0/24` because the `0.0.0.0/0` rule
would be checked first, and it would match and deny all traffic.
+
...
. To enable multicast in the `default` project, you must also enable it
...
entirely by `NetworkPolicy` objects and the link:https://github.com/kubernetes/community/blob/master/contributors/design-proposals/network/network-policy.md[associated Namespace annotation]. In particular, by default, all projects are able to access pods
