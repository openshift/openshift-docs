...
all traffic.
+
Domain name updates are polled based on TTL(time to live) value of the domain
or 30 minutes if unable to fetch TTL. In the above example,
suppose `www.foo.com` resolved to `10.11.12.13` and has DNS TTL of 1 minute,
but later it was changed to `20.21.22.23`. Then, {product-title} will take up to
1 minute if it is able to fetch DNS TTL from authoritative server for `www.foo.com`.
If fetching of DNS TTL fails then it will take up to 30 minutes to adapt to these
DNS updates.
