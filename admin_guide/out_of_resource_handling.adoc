...
. Edit the node configuration file (the *_/etc/origin/node/node-config.yaml_* file) to add or modify the `system-reserved` parameter for `eviction-hard` or `eviction-soft`.
+
----
kubeletArguments:
  eviction-hard: <1>
