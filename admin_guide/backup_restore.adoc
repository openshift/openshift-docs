...
Snapshot provides commands to save and restore a snapshot of a running etcd server into
a fresh cluster. 

'snapshot save' writes a point-in-time snapshot of the etcd
backend database to a file. The backend snapshot is written to the given file path.

For example, to save a snapshot to *_snapshot.db_*:
...
[NOTE]
====
`etcdctl snapshot` can only back up v3 API data. For etcdctl to speak to etcd
using the v3 API, the API version must be set to version 3 via the `ETCDCTL_API`
environment variable. 
====

`snapshot restore` creates an etcd data directory for an etcd cluster member from
a backend database snapshot and a new cluster configuration. Restoring the
snapshot into each member for a new cluster configuration will initialize a new
etcd cluster preloaded by the snapshot data.

For example, to save a snapshot and restore the snapshot into a new, three-node cluster:
