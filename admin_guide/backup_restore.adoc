...
To restore your etcd backup and configuration:

. Stop etcd for all members and delete the *_/var/lib/etcd/member_* directory. Run the following command on all nodes:
+

[source, bash]
----
$ systemctl stop etcd; rm -rf /var/lib/etcd/member
----

. Restore the *_/var/lib/etcd/member_* directory from a backup in one of your nodes. For example, run the following command in one of your nodes (`node#1`):
+

[source, bash]
----
$ cp -pr /var/backup/etcd-20170905/member /var/lib/etcd
----

. Apply the SELinux context in the same node you restored the backup in previous step. For example, on `#node1` in this case.
+

[source, bash]
----
$ restorecon -R /var/lib/etcd/member
----

. Edit *_/etc/etcd/etcd.conf_* in other nodes and replace `new` with `existing`. Do this for all other nodes except the one where we restored the backup in previous steps. For example, edit the configuration file on `node#2` and `node#3` and not on `#node#1`.
+
...
. Run the following commands on the member where you restored the backup. For example, on `#node1` in this case.
.. Set the owner to `etcd` for the *$ETCD_DIR* (_/var/lib/etcd/member_):
+
[source, bash]
----
...
.. Edit the *_/usr/lib/systemd/system/etcd.service_* file, and add `--force-new-cluster` flag.
...
. Start etcd on all other nodes. For example, restart it on `node#2` and `node#3`:
+
...
$ etcdctl --ca-file=/etc/origin/master/master.etcd-ca.crt \
  ...
  --endpoints=https://node1:2379,node2:2379,node3:2379

cluster-health
member 1dcc3bfe289a75dc is healthy:
got healthy result from https://10.26.239.75:2379
member 2685124b79cf0ca2 is healthy:
got healthy result from https://10.26.239.74:2379
member a9570e9ec1752ecb is healthy:
got healthy result from https://10.26.239.73:2379

cluster is healthy
----

. Restore previous values of `ETCD_INITIAL_CLUSTER_STATE` in file *_/etc/etcd/etcd.conf_* for all nodes that were modified previously. For example, edit the configuration file on `node#2` and `node#3`.
+
...
. Restart etcd on nodes where *_/etc/etcd/etcd.conf_* is modified. For example, restart etcd on `node#2` and `node#3`.
+
