...
Backup and restore procedures are not fully supported in {product-title} 3.6 due
to dependencies on cluster state.
====
...
. Capture *etcd* v2 data by creating a backup:
+
----
# etcdctl backup \
...
. Capture *etcd* v3 data by copying the *_db_* file over to the backup you
created:
+
----
...
. Back up the *_/etc/etcd/ca/_* directory on the master host that contains
...
When using a separate etcd cluster, you must first restore the etcd backup
by creating a new, single node etcd cluster. If you run etcd as a stand-alone
service on your master nodes, you can create the single node etcd cluster on a
master node. If you use separate etcd with multiple members, you must then also
add any additional etcd members to the etcd cluster one by one.
...
Otherwise, if you only want a single node standalone etcd, continue to
...
. If you have additional etcd members to add to your cluster, continue to
xref:adding-addtl-etcd-members[Adding Additional etcd Members].
Otherwise, if you only want a single node separate etcd cluster, continue to
xref:bringing-openshift-services-back-online[Bringing {product-title}
Services Back Online].
...
+
.. Make a backup of the *_/etc/etcd/ca/_* directory. Ensure that the directory
contains the etcd certificates.
