...
:context: masking-sensitive-annotations-in-the-argocd-web-ui

toc::[]

Argo CD hides sensitive annotation values on Secret resources from the Web UI and CLI. Users configure this by masking specific annotation keys in the Argo CD Custom Resource (CR). This feature enhances security by preventing accidental exposure of sensitive information, such as tokens or API keys, stored in annotations on Secret resources.
...
.Prerequisites
* Ensure that you have created an Argo CD instance. For more information, see Installing a user-defined Argo CD instance.

// Configuring sensitive annotations in the Argo CD Web UI
include::modules/gitops_configuring_sensitive_annotations_in_the_argo_cd_web_ui.adoc[leveloffset=+1]
