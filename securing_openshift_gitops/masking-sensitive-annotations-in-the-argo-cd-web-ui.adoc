...
{OCP} automatically masks the `openshift.io/token-secret.value` annotation key by default. {OCP} adds this annotation to secrets of type `kubernetes.io/dockercfg`, and it contains a service account token that requires protection. You can extend the default list with additional annotation keys as needed.
