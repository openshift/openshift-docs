...
= Shared Resource Container Storage Interface Driver Operator
...
The Shared Resource CSI driver supports the following types of custom resources (CR):

* `SharedSecret` CR for sharing `Secret` objects
* `SharedConfigMap`CR for sharing `ConfigMap` objects
