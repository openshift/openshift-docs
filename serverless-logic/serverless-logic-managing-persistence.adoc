...
To ensure workflow recovery in such scenarios, you must configure workflow runtime persistence. Use the `SonataFlowPlatform` custom resource (CR) or the `SonataFlow` custom resource (CR) to provide this configuration. The scope of the configuration varies depending on which resource you use.
