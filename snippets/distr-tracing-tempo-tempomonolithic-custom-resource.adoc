...
spec:
  storage:
    ...
    tls:
      caName: <ca_certificate_configmap_name> # <5>
  jaegerui:
    enabled: true # <6>
    route:
      enabled: true # <7>
...
<5> Optional: Name of a `ConfigMap` object that contains a CA certificate.
<6> Enables the Jaeger UI.
<7> Enables creation of a route for the Jaeger UI.
