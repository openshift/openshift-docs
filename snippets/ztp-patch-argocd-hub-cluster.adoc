...
----
{
  ...
  "image": "<multicluster_operators_subscription_image>",
...
* In the `image` field, replace `<multicluster_operators_subscription_image>` with the `multicluster-operators-subscription` image that matches your {rh-rhacm} version, for example `registry.redhat.io/rhacm2/multicluster-operators-subscription-rhel9:v2.10`. In disconnected environments, replace the URL with the disconnected registry equivalent for your environment.
--
