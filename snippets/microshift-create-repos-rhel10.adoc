...
//
// * modules/microshift-4-22-known-issues
...
----
+
. Create the `fast-datapath.repo` repository for Fast Datapath packages by entering the following information:Expand commentComment on line R38Resolved
+
[source,terminal,subs="verbatim"]
----
$ CERT=$(ls /etc/pki/entitlement/[0-9]*.pem | grep -v "-key.pem" | head -n1)
