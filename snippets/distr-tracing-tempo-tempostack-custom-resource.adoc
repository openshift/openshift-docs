...
<3> Red{nbsp}Hat supports only the custom resource options that are available in the {dt} documentation.
...
<13> The unique identifier of the tenant. Must be unique throughout the lifecycle of the `TempoStack` deployment. The {dt} uses this ID to prefix objects in the object storage. You can reuse the value of the UUID or `tempoName` field.
