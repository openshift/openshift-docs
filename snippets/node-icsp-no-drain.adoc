...
The following actions do not trigger a node reboot:

* When you create an `ImageDigestMirrorSet` or `ImageContentSourcePolicy` object, it applies the update without draining or rebooting the node. 
+
[NOTE]
====
* Creating an `ImageTagMirrorSet` object does require a reboot and drain. 
* Deleting an `ImageTagMirrorSet`, `ImageDigestMirrorSet`, or `ImageContentSourcePolicy` object does require a drain and reboot.
====
