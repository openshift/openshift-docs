...
- `placementBindingDefaults.name` applies the policies to all clusters with this label.
- `policies.manifests.path` the `DefaultCatsrc.yaml` file contains the catalog source for the disconnected registry and related registry configuration details.
- `policies.manifests` files listed under `policies.manifests` create the Operator policies for installed clusters.
