...
Where:
+
* `lca.openshift.io/apply-label`: Replace the example resource name with your actual resources.
* `lca.openshift.io/apply-wave`: The `lca.openshift.io/apply-wave` value must be higher than the value in the platform `Restore` CRs and lower than the value in the application namespace-scoped `Restore` CR.