...
If you use a multi-architecture deployment, review the latency requirements in the support matrix. In bare metal host (BMH) deployments, where the Bare Metal Operator starts machines, baseboard management controllers (BMCs) must be reachable from the hosted control plane, which can create a security concern. To avoid that issue, use BYO ISO.
