...
[IMPORTANT]
====
For {hcp} on {ibm-z-title}, you must run the control plane on machine types based on 64-bit x86 architecture, and node pools on {ibm-power-title} or {ibm-z-title}. For information about {hcp} on {ibm-z-title} on other architectures, see the xref:../../hosted_control_planes/hcp-prepare/hcp-requirements.adoc#hcp-support-matrix_hcp-requirements[Support matrix for {hcp}].
====
