:_mod-docs-content-type: ASSEMBLY
include::_attributes/common-attributes.adoc[]
[id="hcp-observability"]
= Observability for {hcp}
:context: hcp-observability

toc::[]

[role="_abstract"]
You can gather metrics for {hcp} by configuring metrics sets. Monitoring dashboards are created in the management cluster for each hosted cluster that it manages.

// Metrics sets
include::modules/hosted-control-planes-metrics-sets.adoc[leveloffset=+1]

include::modules/hcp-metrics-sets-ref.adoc[leveloffset=+2]

// Custom IDs
include::modules/hcp-cluster-ids.adoc[leveloffset=+1]

include::modules/hcp-cluster-ids-example.adoc[leveloffset=+2]

[role="_additional-resources"]
.Additional resources

* xref:../hosted_control_planes/hcp-observability.adoc#hcp-customize-dashboards_hcp-observability[Dashboard customization]
* xref:../hosted_control_planes/hcp-observability.adoc#hosted-control-planes-metrics-sets_hcp-observability[Configuring metrics sets for hosted control planes]
* xref:../hosted_control_planes/hcp-observability.adoc#hosted-control-planes-monitoring-dashboard_hcp-observability[Enabling monitoring dashboards in a hosted cluster]

// Monitoring dashboards for hosted clusters
include::modules/hosted-control-planes-monitoring-dashboard.adoc[leveloffset=+1]

[role="_additional-resources"]
.Additional resources

* xref:../hosted_control_planes/hcp-observability.adoc#hcp-cluster-ids_hcp-observability[Customized hosted cluster identifiers]

include::modules/hcp-customize-dashboards.adoc[leveloffset=+2]

//Connectivity metrics
include::modules/hcp-connectivity-metrics.adoc[leveloffset=+1]

include::modules/hcp-connect-data-plane.adoc[leveloffset=+2]

[role="_additional-resources"]
.Additional resources
* xref:../hosted_control_planes/hcp-troubleshooting.adoc#hcp-ts-connectivity_hcp-troubleshooting[Troubleshooting connectivity for {hcp}]

include::modules/hcp-connect-control-plane.adoc[leveloffset=+2]

// Control plane metrics forwarding

include::modules/hcp-cp-metrics-overview.adoc[leveloffset=+1]

include::modules/hcp-cp-metrics-forwarding-configure.adoc[leveloffset=+2]

include::modules/hcp-cp-metrics-forwarding-migrate.adoc[leveloffset=+2]

include::modules/hcp-cp-query-metrics.adoc[leveloffset=+2]

[role="_additional-resources"]
.Additional resources

* xref:../operators/understanding/olm/olm-understanding-metrics.adoc#olm-metrics_olm-understanding-metrics[Exposed metrics]

include::modules/hcp-cp-query-metrics-console.adoc[leveloffset=+2]

[role="_additional-resources"]
.Additional resources

* xref:../operators/understanding/olm/olm-understanding-metrics.adoc#olm-metrics_olm-understanding-metrics[Exposed metrics]

include::modules/hcp-cp-metrics-dashboards.adoc[leveloffset=+2]