...
[id="hcp-4-18-autoscaling_{context}"]
==== Autoscaling to and from zero on {hcp} node pools

For {hcp} on {aws-short}, you can configure node pools to autoscale down to zero compute nodes when no schedulable workloads remain on the pool. For more information, see xref:../hosted_control_planes/hcp-machine-config.adoc#hcp-autoscaling-to-from-zero_hcp-machine-config[Autoscaling to and from zero on {hcp} node pools].
