...
# Install hotfix by replacing the systemd, systemd-pam, systemd-libs, 
# systemd-journal-remote and systemd-udev packages.
RUN rpm-ostree override replace https://example.com/myrepo/systemd-239-70.el8.x86_64.rpm \
    https://example.com/myrepo/systemd-pam-239-70.el8.x86_64.rpm \
    https://example.com/myrepo/systemd-libs-239-70.el8.x86_64.rpm \
    https://example.com/myrepo/systemd-journal-remote-239-70.el8.x86_64.rpm \
    https://example.com/myrepo/systemd-udev-239-70.el8.x86_64.rpm && \
...
[NOTE]
====
In OCP 4.12 some packages require additional configuration options. For example, if you are replacing the kernel, you need to enable rpm-ostree's cliwrap by including the following command in your Containerfile: 
`rpm-ostree cliwrap install-to-root /`.
====

.Example Containerfile to replace the kernel packages using the above note
[source,yaml]
----
# Using a 4.12.0 image
FROM quay.io/openshift-release-dev/ocp-release@sha256:6499bc69a0707fcad481c3cb73225b867d
RUN rpm-ostree cliwrap install-to-root /
# Replace the kernel, kernel-core, kernel-modules and kernel-modules-extra packages.
RUN rpm-ostree override replace https://example.com/myrepo/kernel-4.18.0.x86_64.rpm \
    https://example.com/myrepo/kernel-core-4.18.0.x86_64.rpm \
    https://example.com/myrepo/kernel-modules-4.18.0.x86_64.rpm \
    https://example.com/myrepo/kernel-modules-extra-4.18.0.x86_64.rpm && \
    rpm-ostree cleanup -m && \
    ostree container commit
----
