...
* Previously, installing operators could fail when the Multiarch Tuning Operator was installed on a single-architecture cluster. With this update, operators can be successfully installed on a single-architecture cluster when the Multiarch Tuning Operator is installed. link:https://issues.redhat.com/browse/MULTIARCH-5546[*MULTIARCH-5546*]
...
* Previously, the image inspector failed if the image reference contained both a tag and a digest. With this update, the image inspector is able to inspect images that contain both a tag and a digest. link:https://issues.redhat.com/browse/MULTIARCH-5584[*MULTIARCH-5584*]

* Previously, the Multiarch Tuning Operator did not use the `.spec.registrySources.containerRuntimeSearchRegistries` field in the `config.openshift.io/Image` custom resource when workload manifests did not specify a registry URL. With this update, the `containerRuntimeSearchRegistries` field has been added to the custom resource so that workload manifests that do not contain a registry URL can be accessed successfully. link:https://issues.redhat.com/browse/MULTIARCH-5611[*MULTIARCH-5611*]

* Previously, if the `ClusterPodPlacement` object was deleted less than 1 second after its creation, some finalizers were not deleted. With this update, all finalizers are deleted when the `ClusterPodPlacement` object is deleted. link:https://issues.redhat.com/browse/MULTIARCH-5372[*MULTIARCH-5372*]
