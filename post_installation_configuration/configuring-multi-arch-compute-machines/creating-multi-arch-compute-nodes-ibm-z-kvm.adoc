...
[NOTE]
====
To create a {ibm-z-name} or {ibm-linuxone-name} (`s390x`) cluster with multi-architecture compute machines on `x86_64`, follow the instructions for 
xref:../../installing/installing_ibm_z/preparing-to-install-on-ibm-z.html[Installing a cluster on {ibm-z-name} and {ibm-linuxone-name}], then add compute machines on `x86_64` as described in xref:./creating-multi-arch-compute-nodes-bare-metal.adoc[Creating a cluster with multi-architecture compute machine on bare metal].
====
