...
:context: creating-multi-arch-compute-nodes-ibm-z-reg
[id="creating-multi-arch-compute-nodes-ibm-z-reg"]
= Creating a cluster with multi-architecture compute machines on {ibm-z-reg} and {ibm-linux-one-reg} with z/VM
include::_attributes/common-attributes.adoc[]

toc::[]

To create a cluster with multi-architecture compute machines on {ibm-z-reg} and {ibm-linux-one-reg} (`s390x`) with z/VM, you must have an existing single-architecture `x86_64` cluster. You can then add `s390x` compute machines to your {product-title} cluster.
...
include::modules/machine-user-infra-machines-ibm-z-reg.adoc[leveloffset=+1]
