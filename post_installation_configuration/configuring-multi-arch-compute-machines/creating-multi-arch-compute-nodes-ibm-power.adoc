...
:context: creating-multi-arch-compute-nodes-ibm-power-reg
[id="creating-multi-arch-compute-nodes-ibm-power-reg"]
= Creating a cluster with multi-architecture compute machines on {iibm-cloud-bm-reg}
include::_attributes/common-attributes.adoc[]

toc::[]

To create a cluster with multi-architecture compute machines on {iibm-cloud-bm-reg} (`ppc64le`), you must have an existing single-architecture (`x86_64`) cluster. You can then add `ppc64le` compute machines to your {product-title} cluster.
