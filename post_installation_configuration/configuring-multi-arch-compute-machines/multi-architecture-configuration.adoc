...
|xref:../../post_installation_configuration/configuring-multi-arch-compute-machines/creating-multi-arch-compute-nodes-gcp.adoc#creating-multi-arch-compute-nodes-gcp[Creating a cluster with multi-architecture compute machines on {gcp-full}]
|{gcp-full}
|
|&#10003;
|`aarch64` or `x86_64`
...
[IMPORTANT]
====
Autoscaling from zero is currently not supported on {gcp-full}.
====
