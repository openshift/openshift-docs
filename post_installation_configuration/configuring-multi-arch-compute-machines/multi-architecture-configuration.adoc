...
* If you are planning to manually configure a multi-architecture cluster with a compute node of a second architecture, ensure that your cluster is configured with `platform=none`. You must set this value manually in your `install-config.yaml`, when installing your x86 control plane node.
+
