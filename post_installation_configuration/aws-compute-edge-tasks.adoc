...
=== Changing Cluster Network MTU in AWS Local Zones
...
=== Creating subnet in AWS Local Zones
...
[id="extend_existing_to_local_zones_day2_review"]
=== Review nodes

// Verifying nodes that were created with edge compute pool
include::modules/machine-edge-pool-review-nodes.adoc[leveloffset=+3]
