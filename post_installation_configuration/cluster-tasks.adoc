...
To allow the cluster to use the new credentials, you must update the secrets that the xref:../operators/reference/operator-reference.adoc#cloud-credential-operator_red-hat-operators[Cloud Credential Operator (CCO)] uses to manage cloud provider credentials.
