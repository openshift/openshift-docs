...
|Configuration details related to ../networking/ingress-operator.adoc#nw-installation-ingress-config-asset_configuring-ingress[routing] such as the default domain for routes.
...
|Configures ../networking/ingress-operator.adoc#configuring-ingress[Ingress Operator] behavior such as domain, number of replicas, certificates, and controller placement.
...
../networking/dns-operator.adoc#dns-operator[view the DNS Operator status].
