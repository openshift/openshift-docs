...
* You have installation media and {op-system-first} images that you used to create your cluster. If you do not have these files, you must obtain them by following the instructions in the installation procedure.
...
* xref:../installing/installing_bare_metal/upi/installing-bare-metal.adoc#installation-user-infra-machines-advanced_disk_installing-bare-metal[Disk partitioning for {product-title}]

include::snippets/machine-user-provisioned-limitations.adoc[]

include::modules/post-install-deploying-machine-health-checks.adoc[leveloffset=+1]
...
include::modules/post-install-taints-tolerations.adoc[leveloffset=+1]
...
[role="_additional-resources"]
.Additional resources
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-create_post-install-node-tasks[Creating a ResourceOverride in a project]
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-adopt_post-install-node-tasks[Adopting ResourceOverride on an existing ClusterResourceOverride cluster]
...
.Additional resources
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-create_post-install-node-tasks[Creating a ResourceOverride in a project]
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-rbac_post-install-node-tasks[ResourceOverride roles and permissions]
...
[role="_additional-resources"]
.Additional resources
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-deploy-cli_post-install-node-tasks[Installing the Cluster Resource Override Operator using the CLI]
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-deploy-console_post-install-node-tasks[Installing the Cluster Resource Override Operator using the web console]
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-api_post-install-node-tasks[About resource override API fields]
* xref:../post_installation_configuration/node-tasks.adoc#nodes-cluster-resource-override-troubleshoot_post-install-node-tasks[Troubleshooting ResourceOverride admission]
...
include::modules/post-install-machine-scaling-static-ip.adoc[leveloffset=+1]
