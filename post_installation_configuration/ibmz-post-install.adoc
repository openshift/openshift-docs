...
= Configuring additional devices in an {ibm-z-reg} or {ibm-linux-one-reg} environment
include::_attributes/common-attributes.adoc[]
:context: post-install-configure-additional-devices-ibmz

toc::[]

After installing {product-title}, you can configure additional devices for your cluster in an {ibm-z-reg} or {ibm-linux-one-reg} environment, which is installed with z/VM. The following devices can be configured:
...
[NOTE]
====
The procedures described here apply only to z/VM installations. If you have installed your cluster with {op-system-base} KVM on {ibm-z-reg} or {ibm-linux-one-reg} infrastructure, no additional configuration is needed inside the KVM guest after the devices were added to the KVM guests. However, both in z/VM and {op-system-base} KVM environments the next steps to configure the Local Storage Operator and Kubernetes NMState Operator need to be applied.
====
