...
* The current {ServerlessLogicOperatorName} version is 1.35.0.

.Procedure

. Patch the CSV for the 1.35.0 {ServerlessLogicOperatorName} to update the deployment labels by running the following command:
+
[source,terminal]
----
...
* After applying the upgrade, verify that the Operator is running and in the `Succeeded` phase, run the following command: 
+
[source,terminal]
----
