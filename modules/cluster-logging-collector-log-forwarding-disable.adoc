...
To disable the Log Forwarding API and to stop forwarding logs to the speified endpoints, remove the `metadata.annotations.clusterlogging.openshift.io/logforwardingtechpreview:enabled` parameter from the `ClusterLogging` CR and delete the Log Forwarding CR. The container and node logs will be forwarded to the internal {product-title} Elasticsearch instance.
