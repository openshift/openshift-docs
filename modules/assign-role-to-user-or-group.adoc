...
. For *Key*, select one of the values from `userid`, `name`, `email`, `groups`, or `roles`. For example, to assign the `Analyst` role to a user by email, first select `email`.
. For *Value*, enter the value of the user ID, name, email address, group, or role based on the key you selected. For example, to assign the `Analyst` role to a user by email, after selecting `email`, enter the email address of the user.
