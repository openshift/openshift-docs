...
* `common-subscriptions-policy` contains Operator subscriptions. If you have followed the structure and content of the reference `{policy-gen-cr}` CRs, all Operator subscriptions are grouped into the `common-subscriptions-policy` policy.
--
+
[NOTE]
====
