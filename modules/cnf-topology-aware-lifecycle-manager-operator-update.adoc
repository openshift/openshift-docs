...
.. This update generates one policy, `du-upgrade-operator-catsrc-policy`, to update the `community-operators-disconnected` catalog source with the new index images that contain the desired Operators images.
endif::[]
+
[NOTE]
====
...
ifdef::openshift-origin[]
If you want to use the image pre-caching for Operators and there are Operators from a different catalog source other than `community-operators-disconnected`,  you must perform the following tasks:
endif::[]
