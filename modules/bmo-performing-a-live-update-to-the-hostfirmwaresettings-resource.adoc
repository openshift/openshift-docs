...
[role="_abstract"]
You can perform a live update to the `HostFirmwareSettings` resource after it has begun running workloads by configuring the `HostUpdatePolicy` resource and validating changes against the `FirmwareSchema`. Live updates do not trigger deprovisioning and reprovisioning the host.
