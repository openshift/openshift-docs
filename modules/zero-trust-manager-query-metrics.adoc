...
. In the query field, enter the following command to query SPIRE Server metrics:
+
[source,terminal]
----
$ {job="spire-server"}
...
. In the query field, enter the following expression to query SPIRE Agent metrics.
+
[source,terminal]
----
$ {job="spire-agent"}
