...
|A list of `portTarget` objects, each of which contextualizes a port target within a single failure domain. Each object must include an `id` parameter that specifies a port target.
...
|A `network` object that contains either a name or UUID. For example, `8db6a48e-375b-4caa-b20b-5b9a7218bfe6`.

|`platform.openstack.failuredomains.portTargets.portTarget.fixedIPs`
|Subnets to allocate fixed IP addresses to. These subnets must within the same network as the port.
