  ...
  gcp:
    serviceEndpoints:
      ...
      - name: Storage
        url:
      - name: TagManager
|The {gcp-short} service endpoint name and URL. The parameter stores the configuration of a custom URL to override the existing defaults of {gcp-short} endpoints. If you do not specify the endpoint, the default endpoint for the {gcp-short} region is used. For each {gcp-short} service, you can specify 1 endpoint only. You can specify a maximum of 9 endpoint overrides.
[NOTE]
====
Use the `name` field to specify the {gcp-short} endpoint to override. You must provide a value.

Use the `url` field to specify a URL that overrides the default endpoint for a client using the {gcp-short} service that you specified in the `name` field. You must provide a value.
====
|String. Valid link:https://cloud.google.com/vpc/docs/private-service-connect[Private Service Connect] URL.
