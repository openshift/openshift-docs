...
Beginning with {product-title} 4.12, there is a unified definition of failure domains for {rh-openstack} deployments that covers all supported availability zone types. You can use failure domains to control related aspects of Nova, Neutron, and Cinder configurations from a single place. 
...
* The `portTarget` object with the ID `control-plane`.
...
|A list of `portTarget` objects, each of which contextualizes a port target within a single failure domain. Each object must include an `id` parameter that specifies a port target. Optionally, you can include other parameters.
|A list of `portTarget` objects.

|`platform.openstack.failuredomains.portTargets.portTarget.id`
|The ID of an individual port target. Set the value of this parameter to `control-plane` to select that port target as the first network for machines. If this parameter is not set, the cluster default network is used.
|`control-plane` or an arbitrary string.

|`platform.openstack.failuredomains.portTargets.portTarget.network`
|Required. The name or ID of a network to use in place of the primary cluster network.
|A network object that contains a name or UUID. For example, `8db6a48e-375b-4caa-b20b-5b9a7218bfe6`.

|`platform.openstack.failuredomains.portTargets.portTarget.fixedIPs`
|Subnets to to allocate fixed IP addresses to. These subnets must within the same network as the port.
|A list of `subnet` objects.
|====

NOTE: You cannot combine availability zones and failure domains. If you want to use failure domains, compute and storage availability zones must be left unset. 
endif::osp[]
