...
ifdef::gcp[If you are installing on {gcp-full} into a shared virtual private cloud (VPC), `credentialsMode` must be set to `Passthrough` or `Manual`.]
====
endif::aws,gcp[]
ifdef::aws,gcp,azure[]
...
== Additional {gcp-full} configuration parameters

Additional {gcp-full} configuration parameters are described in the following table:

.Additional {gcp-full} parameters
[cols=".^l,.^a",options="header"]
|====
|Parameter|Description
...
|controlPlane:
  platform:
    gcp:
      osImage:
        project:
|Optional. By default, the installation program downloads and installs the {op-system-first} image that is used to boot control plane machines. You can override the default behavior by specifying the location of a custom {op-system} image that the installation program is to use for control plane machines only. Control plane machines do not contribute to licensing costs when using the default image. But, if you apply a {gcp-full} Marketplace image for a control plane machine, usage costs do apply.

*Value:* String. The name of {gcp-full} project where the image is located.
...
|platform:
  gcp:
    network:
|The name of the existing Virtual Private Cloud (VPC) where you want to deploy your cluster. If you want to deploy your cluster into a shared VPC, you must set `platform.gcp.networkProjectID` with the name of the {gcp-full} project that contains the shared VPC.
...
|platform:
  gcp:
    networkProjectID:
|Optional. The name of the {gcp-full} project that contains the shared VPC where you want to deploy your cluster.
...
|platform:
  gcp:
    projectID:
|The name of the {gcp-full} project where the installation program installs the cluster.
...
|platform:
  gcp:
    region:
|The name of the {gcp-full} region that hosts your cluster.
...
*Value:* A list of valid link:https://cloud.google.com/compute/docs/regions-zones#available[{gcp-full} availability zones], such as `us-central1-a`, in a
link:https://yaml.org/spec/1.2/spec.html#sequence//[YAML sequence].
[IMPORTANT]
====
When running your cluster on {gcp-full} 64-bit ARM infrastructures, ensure that you use a zone where Ampere Altra Arm CPU's are available. You can find which zones are compatible with 64-bit ARM processors in the "{gcp-full} availability zones" link.
====
...
|platform:
  gcp:
    defaultMachinePlatform:
      osDisk:
        diskType:
|The link:https://cloud.google.com/compute/docs/disks#disk-types[{gcp-full} disk type].
...
|platform:
  gcp:
    defaultMachinePlatform:
      type:
|The link:https://cloud.google.com/compute/docs/machine-types[{gcp-full} machine type] for control plane and compute machines.

*Value:* The {gcp-full} machine type, for example `n1-standard-4`.
    ...
    defaultMachinePlatform:
      osDisk:
        encryptionKey:
          kmsKey:
            location:
|The link:https://cloud.google.com/kms/docs/locations[{gcp-full} location] in which the KMS key ring exists.

*Value:* The {gcp-full} location.
...
|The {gcp-full} service account used for the encryption request for control plane and compute machines. If absent, the Compute Engine default service account is used. For more information about {gcp-full} service accounts, see Google's documentation on link:https://cloud.google.com/compute/docs/access/service-accounts#compute_engine_service_account[service accounts].

*Value:* The {gcp-full} service account email, for example `<service_account_name>@<project_id>.iam.gserviceaccount.com`.
    ...
    gcp:
      osDisk:
        encryptionKey:
          kmsKey:
            location:
|For control plane machines, the {gcp-full} location in which the key ring exists. For more information about KMS locations, see Google's documentation on link:https://cloud.google.com/kms/docs/locations[Cloud KMS locations].

*Value:* The {gcp-full} location for the key ring.
...
|controlPlane:
  platform:
    gcp:
      osDisk:
        diskType:
|The link:https://cloud.google.com/compute/docs/disks#disk-types[{gcp-full} disk type] for control plane machines.
...
|controlPlane:
  platform:
    gcp:
      type:
|The link:https://cloud.google.com/compute/docs/machine-types[{gcp-full} machine type] for control plane machines. If set, this parameter overrides the `platform.gcp.defaultMachinePlatform.type` parameter.
    ...
    gcp:
      osDisk:
        encryptionKey:
          kmsKey:
            location:
|For compute machines, the {gcp-full} location in which the key ring exists. For more information about KMS locations, see Google's documentation on link:https://cloud.google.com/kms/docs/locations[Cloud KMS locations].
...
|compute:
  platform:
    gcp:
      osDisk:
        diskType:
|The link:https://cloud.google.com/compute/docs/disks#disk-types[{gcp-full} disk type] for compute machines.
...
|compute:
  platform:
    gcp:
      type:
|The link:https://cloud.google.com/compute/docs/machine-types[{gcp-full} machine type] for compute machines. If set, this parameter overrides the `platform.gcp.defaultMachinePlatform.type` parameter.
