...
|compute:
  platform:
    azure:
      bootDiagnostics:
        type:
|This parameter enables boot diagnostics collection for compute machines. `type` specifies the {azure-short} boot diagnostics type for the created compute machines. When you set `type` to `UserManaged`, you must provide the values for `resourceGroup` and `storageAccountName`. For `storageAccountName` and  {product-title} cluster nodes, ensure that you use the same region and subscription. When you set `type` to `Managed`, {azure-short} stores the data blobs for the boot diagnostics in a managed storage account. When you set `type` to `Disabled`, you turn off the parameter.
|String. For compute machines, the default value is `Disabled`.
...
|compute:
  platform:
    azure:
      bootDiagnostics:
        resourceGroup:
|Specifies the name of the {azure-short} resource group that contains the storage account for diagnostics for compute machines. `resourceGroup` is used only when you set `type` to `UserManaged`.
|String.
...
|compute:
  platform:
    azure:
      bootDiagnostics:
        storageAccountName:
|Specifies the {azure-short} storage account to store the logs of diagnostics for compute machines. `storageAccountName` is used only where `type` is set to `UserManaged`.
|String.
...
|controlPlane:
  platform:
    azure:
      bootDiagnostics:
        type:
|This parameter enables boot diagnostics collection for control plane machines. `type` specifies the {azure-short} boot diagnostics type for the created control plane machines. When you set `type` to `UserManaged`, you must provide the values for `resourceGroup` and `storageAccountName`. For `storageAccountName` and {product-title} cluster nodes, ensure that you use the same region and subscription. When you set `type` to `Managed`, {azure-short} stores the data blobs for the boot diagnostics in a managed storage account. When you set `type` to `Disabled`, you turn off the parameter.
...
|controlPlane:
  platform:
    azure:
      bootDiagnostics:
        resourceGroup:
|Specifies the name of the {azure-short} resource group that contains the storage account for diagnostics for control plane machines. `resourceGroup` is used only when you set `type` to `UserManaged`.
|String.
...
|controlPlane:
  platform:
    azure:
      bootDiagnostics:
        storageAccountName:
|Specifies the {azure-short} storage account to store the logs of diagnostics for control plane machines. `storageAccountName` is used only where `type` is set to `UserManaged`.
|String.
...
|platform:
  azure:
    defaultMachinePlatform:
       bootDiagnostics:
        type:
|This parameter enables boot diagnostics collection for all machines. `type` specifies the {azure-short} boot diagnostics type for all the created machines. When you set `type` to `UserManaged`, you must provide the values for `resourceGroup` and `storageAccountName`. For `storageAccountName` and {product-title} cluster nodes, ensure that you use the same region and subscription. When you set `type` to `Managed`, {azure-short} stores the data blobs for the boot diagnostics in a managed storage account. When you set `type` to `Disabled`, you turn off the parameter.
|String.
...
|platform:
  azure:
    defaultMachinePlatform:
       bootDiagnostics:
        resourceGroup:
|Specifies the name of the {azure-short} resource group that contains the storage account for diagnostics for all machines. `resourceGroup` is used only when you set `type` to `UserManaged`.
|String.
...
|platform:
  azure:
    defaultMachinePlatform:
       bootDiagnostics:
        storageAccountName:
|Specifies the {azure-short} storage account to store the logs of diagnostics for all machines. `storageAccountName` is used only where `type` is set to `UserManaged`.
|String.
