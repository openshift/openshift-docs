...
[id="installation-configuration-parameters-zone-region-vsphere_{context}"]
== VMware vSphere zone and region enablement configuration parameters

The following table describes VMware vSphere zone and region enablement configuration parameters:

.VMware vSphere zone and region enablement parameters
[cols=".^2,.^3a,.^3a",options="header"]
|====
|Parameter|Description|Values

|`vsphere`
a|Parameter covers two purposes in the context of a vSphere configuration: (1) Describes your account on the cloud platform that hosts your cluster, and you use the parameter to customize the platform; (2) The parameter is optional when providing additional configuration settings for the machine pool parameters associated with compute and control plane machines.

[NOTE]
====
You can only specify one vCenter server for your {product-title} cluster.
====
|String

|`vcenters`
| The fully-qualified hostname or IP address for each listed vCenter server.
| String

|`datacenters`
|Defines the multiple datacenters that operate in a single VMware vCenter.
|String
...
|`region`
|Specifies a vCenter datacenter. You define a region by using a tag from a created tag category, such as `openshift-region`.
|String

|`zone`
|Specifies a vCenter cluster. You define a region by using a tag from from a created tag category, such as `openshift-zone`.
|`String`

|`folder`
|Optional parameter that stores manifest configuration files for the cluster.
|Absolute directory path

|`resourcePool`
|Optional parameter that an existing resource pool for machine creation. If you do not specify a value, the installation program uses the root resource pool of the vSphere cluster.
|Absolute directory path
|====
