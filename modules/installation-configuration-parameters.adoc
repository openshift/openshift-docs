...
You can distribute the control plane across availability zones by using failure domains. 

To define failure domains, add items to the `platform.openstack.failuredomains` list that contain the following parameters:
...
|`platform.openstack.failuredomains.portTargets.id`
|The port target ID. To use failure domains, the value of this parameter must be `control-plane`. If not specified, the cluster default is used.
|`control-plane`

|`platform.openstack.failuredomains.portTargets.network`
|The network ID of a network to use in place of the primary cluster network. This ID must be different for each control plane node. If not specified, the cluster default is used.
|A UUID. For example, `8db6a48e-375b-4caa-b20b-5b9a7218bfe6`.
|====
