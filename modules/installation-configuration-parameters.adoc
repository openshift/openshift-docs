...
|compute:
  platform:
    azure:
      bootDiagnostics:
        type:
|This parameter enables boot diagnostics collection for compute machines. `type` specifies the {azure-short} boot diagnostics type for the created compute machines. When you set `type` to `UserManaged`, you must provide the values for `resourceGroup` and `storageAccountName`. When you set `type` to `Managed`, the installation program provides the values for these parameters. When you set `type` to `Disabled`, you turn off the parameter.
...
|String. For all machines, the default value is `Disabled`.
