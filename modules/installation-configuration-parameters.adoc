...
ifeval::["{context}" == "installation-config-parameters-generic"]
:generic:
endif::[]
...
|platform:
  gcp:
    projectID:
|The name of the {gcp-short} project where the installation program installs the cluster. For Google Cloud Dedicated sovereign cloud installations, use the domain-scoped project ID format, for example `eu0:my-project`. The installer detects the sovereign cloud environment from a domain-scoped project ID prefix.
...
|platform:
  gcp:
    region:
|The name of the {gcp-short} region that hosts your cluster. For Google Cloud Dedicated sovereign cloud installations, use a region provisioned in the Google Cloud Dedicated universe, such as `europe-west3`, `europe-west4`, or `u-france-east1`. The installer validates the region by querying the GCP Compute API for the sovereign project.
    ...
    defaultMachinePlatform:
      osDisk:
        encryptionKey:
          kmsKey:
            name:
|The name of the customer managed encryption key to be used for machine disk encryption. This key also encrypts the bootstrap ignition storage bucket and the internal image registry storage bucket.
...
|Optional. The {gcp-short} service account used for the encryption request for control plane and compute machines. If absent, the Compute Engine default service account is used. The installer automatically provisions the required IAM bindings for KMS encryption, so you typically do not need to specify this field. For more information about {gcp-short} service accounts, see Google's documentation on link:https://cloud.google.com/compute/docs/access/service-accounts#compute_engine_service_account[service accounts].
