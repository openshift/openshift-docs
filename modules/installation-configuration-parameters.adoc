...
When omitted (default behavior), the CNO automatically installs network observability on standard multi-node clusters and Hosted Control Plane clusters, but does not reinstall if removed. {sno} clusters skip installation by default due to resource constraints.
