...
* The `portTarget` object with the ID `control-plane`, as long as that object exists.
...
|The ID of an individual port target. Set the value of this parameter to `control-plane` to select that port target as the first network for machines. If this parameter has a different value, it is ignored.
