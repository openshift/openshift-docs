...
When set to `InstallAndEnable`, the Cluster Network Operator (CNO) installs the Network Observability Operator and creates a default `FlowCollector` during cluster creation on all cluster types, including {sno} clusters.

When set to `NoAction`, automatic installation does not occur.

When omitted (default behavior), the CNO automatically installs network observability on standard multi-node clusters and Hosted Control Plane clusters. {sno} clusters skip installation by default due to resource constraints.

After deployment, the CNO does not monitor or reinstall the Network Observability Operator if it is removed.

[NOTE]
====
Network observability automatic installation is a Technology Preview feature. You must enable the `NetworkObservabilityInstall` feature gate before you can use this field. For more information, see "Install network observability during cluster creation" in _Network observability_.
====

*Value:* `InstallAndEnable`, `NoAction`, or omit the field for default behavior.
