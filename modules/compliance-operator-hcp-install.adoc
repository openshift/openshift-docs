...
:navtitle: Install the Compliance Operator on Hypershift {hcp}
[id="installing-compliance-operator-hcp_{context}"]
= Installing the Compliance Operator on {hcp}

[role="_abstract"]
Install the Compliance Operator on {hcp} by creating a `Subscription` file in the software catalog so you can run compliance scans in a hosted control plane environment.
...
----
* In {product-title} {product-version}, the pod security label must be set to `privileged` at the namespace level.
