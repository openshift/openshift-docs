...
[role="_abstract"]
You can install the Compliance Operator in {hcp} by using the software catalog by creating a `Subscription` file.
...
----
+
The `metadata.labels.pod-security.kubernetes.io/enforce` field specifies the pod security label that must be set to `privileged` at the namespace level in {product-title} {product-version}.
