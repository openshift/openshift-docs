...
. Run the container as `privileged` and partition the disk. In the following example, the size of the partition is 250 GiB to allow pre-caching the DU profile for Day 2 Operators:
+
[source,terminal]
----
...
Where:
+
* `factory-precaching-cli partition` specifies the partitioning function of the {factory-prestaging-tool}.
...
----
+
The mount point is `/var/mnt` because the `/mnt` folder in {op-system} is a link to `/var/mnt`.
