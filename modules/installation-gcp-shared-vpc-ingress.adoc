...
.Prerequisites
* You completed the installation of {product-title} on {gcp-short} into a shared VPC.
...
----
. Add a record to your {gcp-short} public zone with the router's external IP address and the name `*.apps.<cluster_name>.<cluster_domain>`. You can use the `gcloud` command-line utility or the {gcp-short} web console.
