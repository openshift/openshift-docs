...
= Verify host connectivity and image access

[role="_abstract"]
Verify that your installation host can reach the registry and pull {product-title} release images using the agent console application, and troubleshoot network configuration issues if the pull check fails.
