...
After the cluster administrator installs the {oadp-short} Operator and provides the necessary permissions, you can use the Self-Service feature.
...
This also means that a namespace admin user cannot access backup and restore data of other users.
...
Additionally, the cluster administrator also has the ability to control the allowed options within the custom resources, restricting certain operations for added security. This happens by using `spec` enforcements in the `DataProtectionApplication` (DPA) custom resource (CR).
