...
The {oadp-short} cluster administrator creates a user with the namespace `admin` role and provides the necessary Role Based Access Controls (RBAC) to the user to perform {oadp-short} Self-Service actions. As this user has limited access compared to the `cluster-admin` role, this user is referred to as a non-admin user. 
...
Additionally, the cluster administrator also has the ability to control the allowed options within the custom resources, restricting certain operations for added security. This happens by using `spec` enforcements in the `DataProtectionApplication` (DPA).
