...
[NOTE]
====
If your VM uses non-standard paths for RPM, DNF, or yum repository configuration, adjust the `Volume` entries accordingly. For example, if yum repository files are in `/etc/yum/repos.d` instead of `/etc/yum.repos.d`, update the corresponding bind mount path.
====
