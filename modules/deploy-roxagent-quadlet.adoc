...
. Create the `tmpfiles.d` configuration for the `roxagent` lock directory by copying the following content to `/etc/tmpfiles.d/roxagent.conf`:
+
[source,text]
----
...
. Copy the following content to `/etc/containers/systemd/roxagent.container`:
+
[source,ini]
----
[Unit]
...
Image=registry.redhat.io/advanced-cluster-security/rhacs-main-rhel9:<rhacs_version>
Exec=--host-path /host
Network=host
...
----
Where:
<rhacs_version>:: The version of your {product-title-short} deployment, for example, {rhacs-version}.

. If you have SELinux enabled, restore the SELinux contexts by entering the following commands:
+
[source,terminal]
----
...
. Reload systemd and enable the timer by entering the following commands:
+
[source,terminal]
----
