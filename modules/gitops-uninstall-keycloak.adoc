...
[role="_abstract"]
You can delete the Keycloak resources and their relevant configurations by removing the `SSO` field from the Argo CD Custom Resource (CR) file.

.Procedure

. Remove the `SSO` field from the Argo CD CR file. After you remove the `SSO` field, the values in the file look similar to the following:
+
[source,yaml]
----
