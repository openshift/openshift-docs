...
By default, when you install an Operator, {product-title} installs the Operator pod on one of your worker nodes randomly. However, the following examples describe situations where you might want to schedule an Operator pod to a specific node or set of nodes:
