...
[source,terminal]
----
$ podman inspect --format='{{ index .Config.Labels "io.openshift.s2i.scripts-url" }}' wildfly/wildfly-centos9
----
+
.Example output
[source,terminal]
...
----
+
You inspected the `wildfly/wildfly-centos9` builder image and found out that the scripts are in the `/usr/libexec/s2i` directory.
+
