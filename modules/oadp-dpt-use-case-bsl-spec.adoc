...
[id="oadp-dpt-use-case-bsl-spec.adoc_{context}"]
= Running a data protection test by configuring a backup storage location specification

[role="_abstract"]
In this use case, you configure the `DataProtectionTest` (DPT) custom resource (CR) by specifying the backup storage location (BSL) specification instead of a BSL name. You then run the DPT CR to verify the Container Storage Initiative (CSI) snapshot readiness and the data upload performance to the storage bucket.
...
<5> Configure the `uploadSpeedTestConfig` object by setting the `fileSize` and `timeout` fields.
