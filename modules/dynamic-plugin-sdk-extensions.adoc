...
`scope`: The scope
in which actions should be provided for. A hook might be required if you want to remove the `ModifyCount` action from a deployment with a horizontal pod autoscaler (HPA).
|===
...
|`operatorRowLoader`
|`CodeRef<React.ComponentType<OperatorRowProps<T>>>` |yes |Loader for pop-up row component.
...
|`path`
|`string` |yes |An optional, fully-qualified path to a resource property to used as the details item value. You can render only primitive type values, linked in Additional resources, directly. Use the component property to handle other data types.
...
|`sortWeight`
|`number` |yes |An optional sort weight, relative to all other details items in the same column. You can represent it using any valid JavaScript Number, linked in Additional resources. Items in each column are sorted independently, lowest to highest. Items without sort weights are sorted after items with sort weights.
...
* link:https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#number_type[JavaScript Number]
