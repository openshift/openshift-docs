...
|`insertBefore` |`string` \| `string[]` |yes |Insert this item before the referenced item. For arrays, the first match in order is used.

|`insertAfter` |`string` \| `string[]` |yes |Insert this item after the referenced item. For arrays, use the first match found in order. The `insertBefore` value takes precedence.
|===
...
|`provider` |`CodeRef<ExtensionHook<Action[], any>>` |no |A React hook that returns actions for the given scope. If `contextId` = `resource`, then
...
|`priority` |`number` |yes |Priority for this provider. Defaults to `0`. Higher priority providers might override catalog items provided by other providers.
|===
...
|`popupComponent`
|`CodeRef<React.ComponentType<PrometheusHealthPopupProps>>` |yes |Loader for menu content. If defined, a health item is represented as a link, which opens a menu with the given content.
...
|`popupComponent` |`CodeRef<WatchK8sResults<T>>` |yes |Loader for menu content. If defined, a health item is represented as a link, which
opens a menu with the given content.
...
|`popupComponent`|`CodeRef<React.ComponentType<{ healthResult?: T; healthResultError?: any; k8sResult?: FirehoseResult<R>; }>>`|yes |Loader for menu content. If defined, a health item will be
represented as a link which opens a menu with given content.
...
|===
|Name |Value Type |Optional |Description
|`model` |`ExtensionK8sGroupModel` |no |The model to customize. Specify only a group, or optional version and kind.
...
|`perspective` |`string` |yes |The perspective ID to which this item belongs. If not specified, contributes to the default perspective.
...
|`insertBefore` |`string` \| `string[]` |yes |Insert this item before the referenced item. For arrays, the extension uses the first matching item in order.

|`insertAfter` |`string` \| `string[]` |yes |Insert this item after the referenced item. For arrays, the extension finds the first matching item in order. `insertBefore` takes precedence.
...
Use this extension to add a navigation item that points to a cluster resource details page. Use the resource's K8s model to define the navigation item.
...
|`insertBefore` |`string` \| `string[]` |yes |Insert this item before the item referenced here. For arrays, the extension uses the first matching item in order.

|`insertAfter` |`string` \| `string[]` |yes |Insert this item after the item referenced here. For arrays, the extension uses the first matching item in order. `insertBefore` takes precedence.
...
Use this extension to add a navigation item that points to a namespaced resource details page. Use the resource's K8s model to define the navigation item.
...
|`insertBefore` |`string` \| `string[]` |yes |Insert this item before the referenced item. For arrays, use the first matching item in order.

|`insertAfter` |`string` \| `string[]` |yes |Insert this item after the referenced item. For arrays, insert after the first matching item in order. `insertBefore` takes precedence.
...
|`insertAfter` |`string` \| `string[]` |yes |Insert this item after the item referenced here. For arrays, use the first matching item in order. `insertBefore` takes precedence.
...
|`insertAfter` |`string` \| `string[]` |yes |Insert this item after the referenced item. For arrays, use the first matching item in order. `insertBefore` takes precedence.
|===
...
Adds a new page to the web console router. 
...
Adds a new standalone page, rendered outside the common page layout, to the web console router.
...
|`path`
|`string` |yes |An optional, fully-qualified path to a resource property to used as the details item value. Only primitive type values can be rendered directly. Use the component property to handle other data types.
...
|`sortWeight`
|`number` |yes |An optional sort weight, relative to all other details items in the same column. Represented by any valid JavaScriptNumber. Items in each column are sorted independently, lowest to highest. Items without sort weights are sorted after items with sort weights.
...
This component can be used to register a listener function receiving telemetry events. These events include user identification, page navigation, and other application specific events. The listener might use this data for reporting and analytics purposes.
...
returns a component, or if null or undefined, renders in the topology sidebar. SDK component: `<Section title=\{}>...` padded area
...
|`create` |`CodeRef<RelationshipProviderCreate>` |no |Callback to run when connector is drop over target node to create a connection
...
* link:https://v5.reactrouter.com/[React Router]
* link:https://developer.mozilla.org/en-US/docs/Glossary/Primitive[Primitive type]
* link:https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#number_type[JavaScriptNumber]