...
* With this update, {tekton-results} users can retrieve logs from Splunk that were forwarded by OpenShift Logging. To enable this functionality, set the following environment variables:
...
* With this update, the Git resolver can now use personal access tokens to authenticate with GitHub or GitLab, avoiding rate limits associated with anonymous API usage. To enable this feature, add a `token:` field to your Git resolver specification. {pipelines-shortname} automatically injects the token as an HTTP header to reduce the risk of quota-related errors during remote resolution.
...
* With this update, the CLI and `Task` components use a consistent environment variable for provider credentials. Setting the environment variable to `CRED_STORE=/tekton/creds/creds.json` works uniformly across all supported backends, including OCI, GCS, and S3.
+
...
* Before this update, YAML syntax errors in `PipelineRun` were only reported in logs and Kubernetes events, making them difficult to detect and troubleshoot. With this release, {pac} comments directly on pull requests when `PipelineRun` YAML validation errors occur. This improves error visibility and simplifies troubleshooting on GitHub, GitLab, and Gitea providers.
...
*** Global level. This option applies to all namespaces except those prefixed with `kube-` and `openshift-`. 
*** Namespace level. This option applies to all resources in a specific namespace
...
* With this release, support for `ClusterTask` objects is removed starting with {pipelines-title} v1.0.0. As a result, the `tkn clustertask` and `tkn task create` commands are no longer available.
...
* Before this update, the console plugin used shared proxy endpoints that were associated with known security vulnerabilities (CVEs). This posed potential risks and reduced the reliability of the plugin. With this update, the console plugin uses dedicated API endpoints to mitigate the identified security issues and enhance reliability and security.
