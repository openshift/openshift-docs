...
* With this update, the event-based pruner can now be enabled or disabled through the `TektonConfig` CR by setting the `spec.tektonpruner.disabled` parameter to `false` or `true`. Fine-grained configuration is not yet available through the `TektonConfig` CR and should be managed using config maps.
+
[NOTE]
====
