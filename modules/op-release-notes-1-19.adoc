              ...
              containers:
                - name: api
                  env:
                    ...
                    - name: LOGS_TYPE
                      value: "Splunk"
                    - name: LOGGING_PLUGIN_NAMESPACE_KEY
                      value: "kubernetes.namespace_name"
                    - name: LOGGING_PLUGIN_CONTAINER_KEY
                      value: "kubernetes.container_name"
                    - name: SPLUNK_SEARCH_TOKEN
                      value: "Token that has permission to use the `Search` API in Splunk."
                    - name: LOGGING_PLUGIN_API_URL
                      value: "The URL of Splunk that includes the port."
                    - name: LOGGING_PLUGIN_QUERY_PARAMS
                      value: "Specify the index as `index=name`, where `name` is the index name."
----
+
[NOTE]
====
...
* With this update, the `pac_watcher_pipelines_as_code_git_provider_api_request_count` and `pac_controller_pipelines_as_code_git_provider_api_request_count` metrics track the number of API calls that {pac} makes to Git providers such as GitHub, GitLab, and Gitea. These metrics also help monitor API rate limit usage by Git provider, namespace, event type, and repository.
