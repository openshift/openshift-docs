...
zones:: Specifies the zones to deploy the control plane instances into, for example `us-central1-a`, `us-central1-b`, and `us-central1-c`.
control_subnet:: Specifies the `selfLink` URL to the control subnet.
...
service_account_email:: Specifies the email address for the master service account that you created.
ignition:: Specifies the contents of the `master.ign` file.
