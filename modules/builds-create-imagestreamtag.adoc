...
* To create an `ImageStreamTag` in the `openshift` namespace, so it is available to developers in all projects, enter the following command:
+
[source,terminal]
----
...
* To create an `ImageStreamTag` in a single project, enter the following command:
+
[source,terminal]
----
