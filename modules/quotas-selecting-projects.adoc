...
`quota`:: The `ResourceQuotaSpec` object that will be enforced over the selected projects.
`annotations`:: A simple key-value selector for annotations.
`labels`:: A label selector that can be used to select projects.
`namespaces`:: A per-namespace map that describes current quota usage in each selected project.
`total`:: The aggregate usage across all selected projects.
+
...
----
+
`for-name`:: Both `clusterresourcequota` and `clusterquota` are aliases of the same command. `for-name` is the name of the `ClusterResourceQuota` object.
