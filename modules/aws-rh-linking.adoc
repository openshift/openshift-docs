...
[role="_abstract"]
Link your AWS and Red{nbsp}Hat accounts to associate your {product-title} subscription with a billing account.

.Procedure
. Click the *Continue to Red{nbsp}Hat* button to proceed with account linking:
+
image::rosa-continue-rh-6.png[]

. Log in to your Red{nbsp}Hat account, if prompted.
+
...
If the AWS account is linked to a different Red{nbsp}Hat account, an error is displayed. See link:https://access.redhat.com/articles/7066995[Correcting Billing Account Information for HCP clusters] for troubleshooting.
====

. On the *Complete your account connection* page, verify that the Red{nbsp}Hat and AWS account numbers are correct.
+
image::rosa-rh-account-connection-8.png[]

. Click the *Connect accounts* button if you agree with the service terms.
+
If this is the first time you are using the {hybrid-console}, you must agree with the general managed services terms and conditions before creating the first cluster:
+
