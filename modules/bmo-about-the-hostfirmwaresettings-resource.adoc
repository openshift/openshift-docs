...
:navtitle: The `HostFirmwareSettings` resource
...
where:
--
`spec.settings.ProcTurboMode: Disabled`:: Specifies a name/value pair that sets the `ProcTurboMode` BIOS setting to `Disabled`.
--
