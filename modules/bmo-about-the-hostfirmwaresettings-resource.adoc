...
= The HostFirmwareSettings resource

[role="_abstract"]
The `HostFirmwareSettings` resource enables you to view and configure BIOS settings for a bare-metal host.
...
+
--
`ProcTurboMode: Disabled`:: In the foregoing example, the `spec.settings` section contains a name/value pair that will set the `ProcTurboMode` BIOS setting to `Disabled`.
--
