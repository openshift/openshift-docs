...
When the `LimitRange` parameter and the minimum values for container resource requests are specified in the project, {pipelines-title} looks at all the `LimitRange` values in the project. In this case, {pipelines-title} uses the minimum values instead of zero.
