...
[IMPORTANT]
====
Both `serving.knative.dev/visibility` and `networking.knative.dev/visibility` labels are available and same functionality but `serving.knative.dev/visibility` will not be supported in the future release.
====
