...
:navtitle: Enable Swift on {rh-openstack}
...
[IMPORTANT]
====
If link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/16.0/html-single/storage_guide/index#ch-manage-containers[the {rh-openstack-first} object storage service], commonly known as Swift, is available, {product-title} uses Swift as the image registry storage. If Swift is unavailable, the installation program relies on the {rh-openstack} block storage service, commonly known as Cinder.
...
Before installation, check if your {rh-openstack} deployment is affected by this problem. If your deployment is affected by this problem, reconfigure Ceph RGW.
====
