...
[IMPORTANT]
====
// QE: The Swift/Object Storage workflow changed in RHOSO 18. Verify whether this procedure and the prerequisites (swiftoperator role, Ceph RGW account-in-url option) still apply as documented. Update or remove steps that are no longer relevant.
If the {rh-openstack-first} object storage service, commonly known as Swift, is available, {product-title} uses it as the image registry storage. If it is unavailable, the installation program relies on the {rh-openstack} block storage service, commonly known as Cinder.
