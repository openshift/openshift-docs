...
[NOTE]
====
{rh-openstack} with the Ceph storage back end supports only the `.raw` image format. You must store the {op-system} image in the `.raw` format. Follow link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/16.2/html/creating_and_managing_images/ch-image-service#section-convert-image[the manual process] to transform the image from the `.qcow2` to `.raw` format.
====
