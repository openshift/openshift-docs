...
{lvms} provides two ways to use RAID-protected storage:

* **Native LVM RAID:** Set `raidConfig` on a device class in the `LVMCluster` CR to create RAID1, RAID4, RAID5, RAID6, or RAID10 volumes. Native RAID uses thick provisioning. Snapshots and clones are not supported with this configuration because `raidConfig` is mutually exclusive with `thinPoolConfig`.

* **External RAID with mdadm:** Create a RAID array by using the `mdadm` utility before the node joins the cluster, then add the resulting device path (for example, `/dev/md0`) to the `deviceSelector` field. Because {lvms} treats the mdadm device as a regular block device, you can use thin provisioning, snapshots, and clones with this configuration.

[NOTE]
====
If you need both RAID protection and snapshot or clone support, use the external mdadm path. For information on creating a RAID array with mdadm, see:
