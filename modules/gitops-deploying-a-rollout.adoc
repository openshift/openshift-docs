...
//
// * extend/getting-started-with-argo-rollouts.adoc
...
`spec.strategy.canary.steps.pause.duration`:: Instructs the Argo Rollouts controller to pause for a specified duration. You can set the duration value in seconds (`s`), minutes (`m`), or hours (`h`). For example, you can specify `1h` for an hour. If no value is specified, the duration value defaults to seconds.
