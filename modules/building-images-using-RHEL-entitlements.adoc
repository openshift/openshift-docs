...
= RHEL entitlement support for container image builds

[role="_abstract"]
The Insights Operator manages {op-system-base-full} entitlement keys by using Simple Content Access (SCA) and stores them in the `etc-pki-entitlement` secret in the `openshift-config-managed` namespace.
