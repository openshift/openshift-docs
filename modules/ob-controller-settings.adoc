...
| `GIT_ENABLE_REWRITE_RULE` | Enables Git wrapper to set up an  `insteadOf` Git config rewrite rule URL for the required source hostname. The default value is `false`.
