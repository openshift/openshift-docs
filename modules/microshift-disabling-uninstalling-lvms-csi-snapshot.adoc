...
[IMPORTANT]
====
Automated uninstallation is not supported as this can orphan the provisioned volumes. Without the LVMS CSI driver, the node does not detect the underlying storage interface and cannot perform provisioning and deprovisioning or mounting and unmounting operations.
====
