...
. Create a `Secret` object with your object storage credentials. For provider-specific parameters, see xref:../installing/configuring-storage-for-lokistack.adoc#configuring-storage-for-lokistack[Configuring storage for LokiStack].
+
...
`endpoint`:: The full endpoint URL. For {aws-short}: `\https://s3.<region>.amazonaws.com`. For S3 Compatible storage: `\https://s3.example.com:9000`.
