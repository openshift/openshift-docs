...
<4> If the PVC source has appropriate labels, the instance type can be inferred from the selected `DataSource` object.
<5> If the PVC source has appropriate labels, the preference can be inferred from the selected `DataSource` object.
