...
** `spec.instancetype.inferFromVolume` defines appropriate labels for the PVC source so that the instance type is inferred from the selected `DataSource` object.
** `spec.preference.inferFromVolume` defines appropriate labels for the PVC source so that the preference is inferred from the selected `DataSource` object.
