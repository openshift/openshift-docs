...
|`LATENCY_TEST_MEMORY`
|Specifies the memory request and limit for the pod that runs the latency tests. If you do not set the variable, the test sets the memory to the greater of `32Mi` multiplied by `LATENCY_TEST_CPUS`, or `1Gi`. To override the calculated value, set `LATENCY_TEST_MEMORY` to a valid Kubernetes quantity greater than `0`, for example `2Gi`.
