...
. Navigate to *Workloads* -> *Pods* in the metering namespace and verify that Pods are being created. This can take up to a minute after installing the metering stack.
+
You can run the same check using kubectl:
+
...
+
Using kubectl, the same check should eventually look like this:
+
...
. Next, use kubectl to verify that the ReportDataSources are beginning to import data, indicated by a valid timestamp in the `EARLIEST METRIC` column (this may take a few minutes). We filter out the "-raw" ReportDataSources which do not import data:
+
----
