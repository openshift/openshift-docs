...
[NOTE]
====
In {product-title} {versions}, egress IP is only assigned to the primary interface. Consequentially, setting `routingViaHost` to `true` wil not work for egress IP in {product-title}{version}.
====
