...
[NOTE]
====
Network observability automatic installation configuration is a Technology Preview feature. You must enable the `NetworkObservabilityInstall` feature gate before you can use the `networkObservability` field.
====
endif::operator[]
ifdef::operator[]
...
--
- `InstallAndEnable`: Install the Network Observability Operator and create a default `FlowCollector` configuration during cluster creation on all cluster types, including {sno} clusters.
- `NoAction`: Do not install the Network Observability Operator during cluster creation.
--

When the `networkObservability` field is omitted entirely (default behavior), the CNO automatically installs network observability on standard multi-node clusters and Hosted Control Plane clusters. {sno} clusters skip installation by default due to resource constraints.

After deployment, the CNO does not monitor or reinstall the Network Observability Operator if it is removed.
