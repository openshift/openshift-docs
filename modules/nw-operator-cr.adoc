...
[role="_abstract"]
Configure the Cluster Network Operator (CNO) `Network` custom resource (CR) named `cluster` so the cluster uses the correct IP ranges and network plugin settings.
...
[IMPORTANT]
====
For clusters that deploy objects across multiple networks, use the same `clusterNetwork.hostPrefix` value for each network type in the `install-config.yaml` file. Different values can prevent the OVN-Kubernetes plugin from routing object traffic among nodes.
====
