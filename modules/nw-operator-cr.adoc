...
[NOTE]
====
After cluster installation, you can modify the `clusterNetwork` IP address CIDR range and increase the `hostPrefix` value to allocate a larger per-node pod subnet. You cannot decrease the per-node subnet size. The `serviceNetwork` range cannot be modified post-installation, either directly or by using the `ServiceCIDR` API. For more information about increasing `hostPrefix`, see "Increasing the cluster network hostPrefix".
====
endif::[]
