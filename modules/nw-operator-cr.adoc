...
--
- `InstallAndEnable`: Install the Network Observability Operator and create a default FlowCollector configuration during cluster creation. When set, the CNO monitors the installation and reinstalls the operator if removed.
- `NoAction`: Do not install the Network Observability Operator during cluster creation.
--

When omitted (default), the CNO automatically installs network observability on standard multi-node clusters and Hosted Control Plane clusters, but does not reinstall if removed. {sno} clusters skip installation by default due to resource constraints.
