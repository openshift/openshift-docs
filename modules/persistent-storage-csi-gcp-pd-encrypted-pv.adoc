...
----
+
The `parameters.disk-encryption-kms-key` field must be the resource identifier for the key that will be used to encrypt new disks. Values are case-sensitive. For more information about providing key ID values, see "Retrieving a resource's ID" and "Getting a Cloud KMS resource ID".
+
[NOTE]
====
...
. Deploy the storage class on your {product-title} cluster by using the `oc` command:
+
[source,terminal]
----
