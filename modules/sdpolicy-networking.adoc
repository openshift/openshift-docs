...
Network verification checks run automatically when you deploy an {product-title} cluster into an existing VPC or create an additional machine pool with a subnet that is new to your cluster. The checks validate your network configuration and highlight errors, enabling you to resolve configuration issues before deployment.
