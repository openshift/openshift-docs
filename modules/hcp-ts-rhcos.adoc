...
:navtitle: Resolve {op-system} image mirroring failures
[id="hcp-ts-rhcos_{context}"]
= Resolving {op-system} image mirroring failures

[role="_abstract"]
For {hcp} on {VirtProductName} in a disconnected environment, if `oc-mirror` fails to automatically mirror the {op-system-first} image to the internal registry, you can manually mirror the {op-system} image to the internal registry.
...
. Mirror the {op-system} image to your internal registry by running the following command:
+
[source,terminal]
----
...
----
+
* Replace `<rhcos_image>` with your {op-system} image; for example, `quay.io/openshift-release-dev/ocp-v4.0-art-dev@sha256:d9643ead36b1c026be664c9c65c11433c6cdf71bfd93ba229141d134a4a6dd94`.
...
* `spec.repositoryDigestMirrors.source` specifies your {op-system} image without its digest.
