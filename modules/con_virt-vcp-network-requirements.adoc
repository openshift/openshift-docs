...
* L2 network connectivity between control plane VMs. The {product-title} installation program requires all control plane nodes to share the same L2 network segment. The `apiVIPs` must reside on this L2 segment.
* A separate L2 network segment for worker nodes. The `ingressVIPs` must reside on the same L2 segment.
