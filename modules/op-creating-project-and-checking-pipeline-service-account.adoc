...
+
{pipelines-title} Operator adds and configures a service account named `pipeline` that has enough permissions to build and push an image. This service account is used by the `PipelineRun` object.
+
----
