...
where:
+
`name`:: Specifies the namespace for the deployment. This must be the same namespace as the secret provider class.
`namespace`:: Specifies the path to mount secrets in the volume mount.
`replicas`:: Specifies the name of the secret provider class.
