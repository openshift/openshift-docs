...
:navtitle: Prepare the provisioner node on {ibm-cloud-bm} infrastructure
...
[role="_abstract"]
Before you install {product-title} on {ibm-cloud-bm} infrastructure, prepare the provisioner node by creating a non-root user, configuring network bridges, registering the node, installing required packages, and downloading the pull secret.

.Procedure

. Log in to the provisioner node via `ssh`.

. Create a non-root user (`kni`) and give that user `sudo` privileges:
+
[source,terminal]
----
# useradd kni
...
. Change the user to add the `libvirt` group to the newly created user:
+
[source,terminal]
----
...
----
+
Go to link:https://console.redhat.com/openshift/install/metal/user-provisioned[Install on Bare Metal with user-provisioned infrastructure]. In step 1, click **Download pull secret**. Paste the contents into the `pull-secret.txt` file and save the contents in the `kni` user's home directory.
