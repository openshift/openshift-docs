...
[role="_abstract"]
Create a `ClusterUserDefinedNetwork` CR using the CLI to enable cluster-wide network segmentation and isolation across multiple namespaces, supporting either Layer 2 or Layer 3 topologies. based on your network architecture requirements. Based upon your use case, create your request using either the `cluster-layer-two-udn.yaml` example for a `Layer2` topology type or the `cluster-layer-three-udn.yaml` example for a `Layer3` topology type.
