...
The following procedure creates a cluster-wide user-defined network custom resource definition (CRD). Based upon your use case, create your request using either the `cluster-layer-two-udn.yaml` example for a `Layer2` topology type or the `cluster-layer-three-udn.yaml` example for a `Layer3` topology type.

[IMPORTANT]
====
The `ClusterUserDefinedNetork` CRD is intended for use by cluster administrators and should not be used by non-administrators. If used incorrectly, it might result in security issues with your deployment, cause disruptions, or break the cluster network. 
====
  ...
  network: # <5>
    ...
    layer2: # <7>
      ...
      subnets:
        ...
        - "10.100.0.0/16" # <9>
----
<1> Name of your `ClusterUserDefinedNetwork` resource.
...
<3> Uses the `matchLabels` selector type, where terms are evaluated with an _*AND*_ relationship. 
...
<6> Because the `matchExpressions` type is use, provisions namespaces matching either `<example_namespace_one>` _or_ `<example_namespace_two>`.
...
[source, terminal]
----
$ oc get clusteruserdefinednetwork <cudn_name> --validate=true -o yaml
----
+
...
[id="cudn-status-conditions"]
== ClusterUserDefinedNetwork status condition types

The following tables explain the status conditions types returned for `ClusterUserDefinedNetwork` CRs when describing the resource. These can be used to troubleshoot your deployment.

.NetworkReady condition types
...
|`NetworkAttachmentDefinitionReady`
...
|`SyncError`
|`primary network already exist in <namespace_name>: <primary_network_name>`
...
|`NetworkAttachmentDefinition are being deleted: [<namespace>/<namespace>]`
|===
...
|`InternalError`
|`Network allocation failed for at least one node: %v, check UDN events for more info.`
