...
[role="_abstract"]
Create a `ClusterUserDefinedNetwork` CR using the CLI to enable cluster-wide network segmentation and isolation across multiple namespaces, supporting either layer 2 or layer 3 topologies. Based upon your use case, create your request by using either the `cluster-layer-two-udn.yaml` example for a `Layer2` topology type or the `cluster-layer-three-udn.yaml` example for a `Layer3` topology type.
