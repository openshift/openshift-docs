...
. To check that SinkBinding is set up correctly, create a Service to use as an event sink:
+
----
$ kn service create event-display --image quay.io/openshift-knative/knative-eventing-sources-event-display:v0.13.2
----
+
The command shown creates a Service named `event-display` that uses a sample container image.
// TODO: Add docs about creating Services using container images.
