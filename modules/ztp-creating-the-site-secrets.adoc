...
+
--
* `namespace` specifies the namespace that must match the namespace configured in the related `ClusterInstance` CR.
* `password` and `username` specify the Base64-encoded BMC credentials.
* `.dockerconfigjson` specifies the Base64-encoded pull secret.
--
