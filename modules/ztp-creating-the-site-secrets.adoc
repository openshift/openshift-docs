...
Add the required secrets for the site to the hub cluster. These resources must be in a
namespace with a name that matches the cluster name.

.Procedure

. Create a secret for authenticating to the site Baseboard Management Controller (BMC). Ensure
the secret name matches the name used in the `SiteConfig`.
