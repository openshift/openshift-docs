...
. Convert the file name into a valid configuration key by replacing special characters, such as `.`, with underscores and converting letters to lowercase. For example, change `subtraction.yaml` to `subtraction_yaml`.
...
Alternatively, you can use a service discovery URI so that the {ServerlessLogicOperatorName} resolves the endpoint when it deploys the workflow. For a deployment that the {ServerlessLogicOperatorName} manages, you must set a service discovery URI in the user-defined properties `ConfigMap` of the workflow, which is named `<workflow_name>-props`. The Operator does not scan the `application.properties` file of the workflow project for these URIs.
====
