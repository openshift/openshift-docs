...
+
.Example resource slice for a device that allows multiple allocations and contains a consumable bandwidth capacity
[source,yaml]
----
kind: ResourceSlice
apiVersion: resource.k8s.io/v1
...
--
+
This example allows the same devices to be consumed by multiple independent resource claims, with the {product-title} scheduler managing how much of the device's capacity is used up by each claim. 
...
--
+
The following example resource claim template can be used with a consumable bandwidth capacity resource slice. 
+
