...
+
You can define these slices by partitioning a device explicitly. A partitioned device is a single physical hardware device that is split into smaller, logical instances, such as Multi-Instance GPUs, based on workload demands. By partitioning devices, you can safely and efficiently share expensive accelerators across multiple pods.
+
...
+
Alternatively, you can allow the {product-title} scheduler to define the slices on demand by the scheduler. By using this method, you can allows the same devices to be consumed by multiple independent resource claims in a more dynamic process, with the {product-title} scheduler managing how much of the device's capacity can be consumed by each claim. To configure this method, set the `allowMultipleAllocations` field in the resource slice to `true`.  
+
[NOTE]
====
...
--
+
The following example resource claim template can be used with a consumable capacity resource slice. 
+
