...
+
You can allow the {product-title} scheduler to define the slices on-demand by the scheduler. By using this method, called _consumable capacity_ upstream, you can allow the same devices to be consumed by multiple independent resource claims in a more dynamic process than manual partitioning, with the {product-title} scheduler managing how much of the device's capacity can be consumed by each claim. To configure this method, set the `allowMultipleAllocations` field in the resource slice to `true`.  
+
[NOTE]
====
The NVIDIA DRA driver does not support consumable capacity.
====
+
The following example allows the {product-title} scheduler to define how much of a device's capacity can be consumed by resource claims. 
+
...
+
** `default` defines the default requested capacity, if no `validRange` is specified. The default amount must be greater than or equal to the `min` value and less than or equal to the `max` value.
