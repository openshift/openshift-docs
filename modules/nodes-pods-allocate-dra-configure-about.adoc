...
+
A partitioned device is a single physical hardware device that you can split into smaller, logical instances, such as Multi-Instance GPUs, based on workload demands. By partitioning devices, you can safely and efficiently share expensive accelerators across multiple pods.
+
...
`spec.devices.capacity.bandwidth.requestPolicy`:: Specifies how the device's capacity can be consumed. Configure the following parameters:
+
...
** `validRange.max` defines the largest capacity slice a single request can take from the device. This must be less than or equal to the capacity `value`.
** `validRange.step` defines the step size between valid capacity amounts within the range. Requests falling between `min` and `max` values must align with the step increments. If a request is not aligned, it rounds up to the next valid step. In this example, a request for 3 GiB would be rounded to 4 GiB. 
...
+
The following example resource claim template can be used when the scheduler is defining the slices. This example includes a strict list of one or more device requests that the scheduler must satisfy together for the claim to succeed.
+
