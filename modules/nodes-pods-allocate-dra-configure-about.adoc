...
+
A partitioned device is a single physical hardware device that is split into smaller, logical instances, such as Multi-Instance GPUs, based on workload demands. By partitioning devices, you can safely and efficiently share expensive accelerators across multiple pods.
+
...
+
You can allow the {product-title} scheduler to define the slices on-demand by the scheduler. By using the _consumable capacity_ method, you can allows the same devices to be consumed by multiple independent resource claims in a more dynamic process, with the {product-title} scheduler managing how much of the device's capacity can be consumed by each claim. To configure this method, set the `allowMultipleAllocations` field in the resource slice to `true`.  
+
[NOTE]
====
...
`spec.devices.allowMultipleAllocations`:: Specifies that the device can be allocated to multiple requests.
...
** `validRange.step` defines the step size between valid capacity amounts within the range. Requests falling between `min` and `max` values must align with the step increments. If a request is not aligned, it rounds up to the next valid step.
** `default` defines the default requested capacity. The default amount must be greater than or equal to the `min` value and less than or equal to the `max` value.
