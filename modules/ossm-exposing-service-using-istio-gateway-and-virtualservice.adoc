...
= Exposing a service using an Istio gateway and a VistualService
:context: ossm-exposing-service-using-istio-gateway-and-virtualservice

This procedure configures a gateway that was deployed using gateway injection to expose a service in the mesh.
...
. Deploy a service in mesh namespace. 
+
[source,terminal]
----
...
. Using the `curl` client, curl the `/headers` endpoint of the httpbin application via the ingress gateway `Service`. Set the `Host` header of the request to `httpbin.example.com` to match the host specified in the Istio `Gateway` and `VirtualService` resources.
+
[source,terminal]
----
...
[NOTE]
====
A gateway can also be exposed to traffic outside the cluster by using OpenShift Routes.
====
