...
<http_proxy_url>:: Specifies a proxy URL to use for creating HTTP connections outside the cluster. The URL scheme must be `http`. For example: `http://<username>:<pswd>@<ip>:<port>`.
<https_proxy_url>:: Specifies a proxy URL to use for creating HTTPS connections outside the cluster. For example: `https://<username>:<pswd>@<ip>:<port>`.
