...
where:
+
<username>:<pswd>@<ip>:<port>:: Specifies the proxy URL to use for creating HTTP or HTTPS connections outside the cluster. For HTTP connections, the URL scheme must be `http`. For HTTPS connections, use `https` scheme.
ifndef::aws[]
