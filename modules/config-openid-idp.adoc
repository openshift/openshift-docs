...
[role="_abstract"]
Configure an OpenID identity provider to integrate with an OpenID Connect provider using the Authorization Code Flow so users can authenticate with existing credentials.
...
Claims are read from the JWT `id_token` returned from the OpenID identity provider and, if specified, from the JSON returned by the Issuer URL. At least one claim must be configured to use as the user's identity.
