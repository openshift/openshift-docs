...
//
// * networking/hardware_networks/sriov-performance-dpdk-rdma.adoc
...
. Verify that the `SriovNetworkNodePolicy` object is applied and the SR-IOV operator pods are running:
+
[source,terminal]
----
...
. Verify that the DPDK pod is in a `Running` state:
+
[source,terminal]
----
$ oc get pod dpdk-app -n <target_namespace>
----
+
.Example output
[source,terminal]
...
dpdk-app   1/1     Running   0          2m
----

. Verify that the SR-IOV virtual function device is attached to the pod:
+
[source,terminal]
----
$ oc exec dpdk-app -n <target_namespace> -- ls -l /dev/vfio/
----
+
.Example output
[source,terminal]
----
total 0
crw------- 1 root root 236, 0 <date> 0
crw-rw-rw- 1 root root  10, 196 <date> vfio
----
