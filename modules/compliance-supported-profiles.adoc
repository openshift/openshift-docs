...
2. Node profiles must be used with the relevant Platform profile. For more information, see _Compliance Operator profile types_.
3. If the `rhcos4-high` or the `rhcos4-high-rev-4` profile includes the `rhcos4-service-sshd-disabled` rule, the `sshd` service is disabled and this blocks access to control plane nodes and compute nodes. If you want to keep access to these nodes and exclude a rule from the base profile enforcement, create a `TailoredProfile` object and set the `rhcos4-service-sshd-disabled` annotation in the configuration for the object. 
...
4. If the `rhcos4-stig` or the `rhcos4-stig-v1r1` profile includes the `rhcos4-service-sshd-disabled` rule, the `sshd` service is disabled and this blocks access to control plane nodes and compute nodes. If you want to keep access to these nodes and exclude a rule from the base profile enforcement, create a `TailoredProfile` object and set the `rhcos4-service-sshd-disabled` annotation in the configuration for the object. 
