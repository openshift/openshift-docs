...
|ocp4-cis-vm-extension ^[5]^
|CIS Red{nbsp}Hat {product-title} Benchmark — {VirtProductName} extension
|Platform
|link:https://www.cisecurity.org/cis-benchmarks/[CIS Benchmarks &#8482;] ^[4]^
|`x86_64`
 `ppc64le`
...
5. OpenShift Virtualization extension profiles extend the base `ocp4-cis` and `ocp4-cis-node` profiles. Run them together on clusters that use {VirtProductName}. For more information, see _OpenShift Virtualization CIS extension profiles_.
