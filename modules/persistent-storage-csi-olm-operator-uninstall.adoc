...
:navtitle: Uninstall the {FeatureName} CSI Driver Operator
[id="persistent-storage-csi-olm-operator-uninstall_{context}"]
= Uninstalling the {FeatureName} CSI Driver Operator

[role="_abstract"]
To remove the {FeatureName} Container Storage Interface (CSI) Driver Operator and free cluster resources, uninstall the operator after stopping applications and deleting persistent volumes (PVs).
