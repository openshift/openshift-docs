...
[IMPORTANT]
====
The following example Create API key `Secret` objects for plan tiers that you want to use. The following example uses API keys to authenticate the requests. Other options, such as OpenID Connect, might be more appropriate for production use. Use the "least-access" approach that is best for your use case.
====
