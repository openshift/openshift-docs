...
* `periodSeconds`: The delay, in seconds, between performing probes. The default is `10`. Must be greater than `timeoutSeconds`. 
* `timeoutSeconds`: The number of seconds of inactivity after which the probe times out and the container is assumed to have failed. The default is `1`. Must be lower than `periodSeconds`.
