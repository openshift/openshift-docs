...
. In the `OpenTelemetryCollector` CR, set the `enableMetrics` field to `true` to scrape internal metrics by using `PodMonitors` or `ServiceMonitors` depending on the deployment mode of the OpenTelemetry Collector:
:
+
[source,yaml]
----
...
[NOTE]
====
Alternatively, if you don't set the `enableMetrics` field to `true`, you can access the metrics endpoint at `+http://localhost:8888/metrics+`.
====

. In the *Observe* section of the OpenShift Console, enable *User Workload Monitoring* to visualize the scraped metrics. You can filter by metrics from an OpenTelemetry Collector instance or OpenTelemetry components such as processors, receivers, or exporters.
