...
[role="_abstract"]
You can check the compliance status of your entire infrastructure by performing a compliance scan, which creates a data snapshot including alerts, images, network policies, and deployments.
...
* After data collection is complete, Central checks the data to find the results. You can view the results in the compliance dashboard and create compliance reports based on the results.


[NOTE]
====
* The following terms relate to a compliance scan:
...
Check:: Is the single test that runs during a single control assessment.
* Some controls have many checks that relate to them. If one of the checks for a control fails, the entire control state shows as Fail.
====

.Procedure

. In the {product-title-short} portal, click *Compliance -> Dashboard*.
. Optional: By default, the compliance results display information on all standards.
+
To display information about specific standards only, perform the following steps:

.. Click *Manage standards*.
.. By default, the tool selects all standards. Clear the checkbox for any specific standard that you do not want to display.
.. Click *Save*.
+
Standards that you do not select do not appear in the dashboard display, including the widgets, in the compliance results tables accessible from the dashboard, and in the PDF files that the *Export* button creates. However, when exporting the results as a CSV file, all default standards appear. 
