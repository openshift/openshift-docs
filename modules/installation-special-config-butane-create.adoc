...
:navtitle: Create a MachineConfig object by using Butane
...
. Create a Butane config file. The following example creates a file named `99-worker-custom.bu` that configures kernel debug messages and specifies custom settings for the chrony time service:
+
