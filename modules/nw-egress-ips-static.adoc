...
----
+
You can set `egressIPs` to two or more IP addresses on different nodes to provide high availability. If multiple egress IP addresses are set then pods use both egress IPs roughly equally. If the node hosting one of the egress IP addresses fails then pods continue using the remaining functioning node and egress IP after a short delay.
