...
- Set up and configure the {product-title} control plane and data plane to use Amazon EC2 instances for cluster compute.
...
**Compute:** Provide the Amazon EC2 service, used for {product-title} relevant resources.

**Storage:** Provide Amazon EBS, used by {product-title} to provision local node storage and persistent volume storage for the cluster.

**Storage:** Provide Amazon S3, used for the {product-title}
built-in image registry.

**Networking:**
Provide the following AWS Cloud services, used by {product-title}
...
**Networking:**
Provide the following AWS services, which customers can optionally integrate with {product-title}:
...
creation.
- Optionally configure a customer-managed VPC for use with {product-title} clusters (required for PrivateLink and HCP clusters).
...
2. For more information on pruning images, see xref:../../registry/index.adoc#pruning-images_registry-overview[Automatically pruning images].
