...
The following example shows a `{policy-gen-cr}` CR (`{common-ranGen}`) extracted from the `ztp-site-generate` reference container. The `{common-ranGen}` file defines two {rh-rhacm-first} policies. The policies manage a collection of configuration CRs, one for each unique value of `policyName` in the CR. `{common-ranGen}` creates a single placement binding and a placement to bind the policies to clusters based on the labels listed in the `{binding-field}` section.

.Example {policy-gen-cr} CR - {common-ranGen}
