...
[id="telco-upgrade-monitoring-second-part-y-upgrade_{context}"]
= Monitoring the second part of a <y+1> cluster upgrade

Follow the same steps as described in "Monitoring the cluster upgrade" except this time track the update for the second part of the <y+1> upgrade.

.Procedure
* Monitor the progress of the second part of the <y+1> upgrade.
For example to monitor the upgrade from 4.15 to 4.16, run the following command:
+
[source,terminal]
----
...
----
+
As soon as the last control plane node is complete, the cluster version is upgraded to the new EUS release.
For example:
+
[source,terminal]
----
