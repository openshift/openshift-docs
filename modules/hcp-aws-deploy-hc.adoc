...
:navtitle: Create a hosted cluster on {aws-short} by using the CLI
...
* `--release-image` specifies the supported {product-title} version that you want to use, for example, `4.22.0-multi`.
...
+
--
`metadata.name`:: Specifies the name of the `NodePool` resource.
