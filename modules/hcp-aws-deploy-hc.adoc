...
. In the command to create the hosted cluster, if you included the `--render-into` flag and specified a YAML file, edit the `NodePool` specification in the YAML file to indicate whether the EC2 instance should run on shared or single-tenant hardware.
+
.Example YAML file
[source,yaml]
----
