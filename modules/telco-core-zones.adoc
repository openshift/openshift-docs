...
OpenShift and Kubernetes use the well known label `topology.kubernetes.io/zone` to create pools of nodes which are subject to a common failure domain.
...
OpenShift and Kubernetes applies a default TopologySpreadConstraint to all replica constructs (Service, ReplicaSet, StatefulSet or ReplicationController) which spreads the replicas based on the `topology.kubernetes.io/zone` label.
...
** With sufficient zones in the cluster design a zone can be disrupted/lost without violating the PDB. If there are insufficient zones, or other scheduling constraints restrict the set of available nodes, the pods may be scheduled such that the PDB will be violated when the zone is disrupted/lost. During upgrades with simultaneous node updates this may lead to partial serialization of updates.
