...
| String 
| None 
| _Required field._ Unique username for the local user.  
...
| String
| `0h`
| Duration that the token remains valid, for example `24h` or `168h`. Uses Go duration format. The default `0h` value creates a non-expiring token.
