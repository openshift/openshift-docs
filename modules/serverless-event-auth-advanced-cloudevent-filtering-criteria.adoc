...
[NOTE]
====
Filters are applied in addition with `.spec.from` method. This means that an event must satisfy both the sender authorization and the filter criteria before it is delivered.
====
