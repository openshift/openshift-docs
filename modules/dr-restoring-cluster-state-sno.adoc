...
:navtitle: Restore to an earlier cluster state for a single node
[id="dr-restoring-cluster-state-sno_{context}"]
= Restoring to an earlier cluster state for a single node

[role="_abstract"]
To restore your {product-title} cluster on a single node, use a saved etcd snapshot to roll back to an earlier state after quorum loss or critical data deletion.
