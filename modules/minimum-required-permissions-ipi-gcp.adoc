...
The following permissions are required for installer-provisioned infrastructure for creating and deleting the {product-title} cluster.
...
** This permission can be limited to act as the control plane and compute service accounts. Alternatively, you can grant the service account that the installation program uses the `iam.serviceAccountUser` role on the control plane and compute service accounts.
