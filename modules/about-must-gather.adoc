...
By default, the `oc adm must-gather` command collects cluster diagnostic data by using the default plugin image. The command writes output to a new directory that follows the `must-gather.local.<cluster_id_suffix>.<timestamp>.<random_number>` format, for example, `must-gather.local.fa96a0bf69fd.20260617T143025Z.4394764343467089039`.
...
When you run `oc adm must-gather`, a new pod with a random name is created in a new project on the cluster. The `oc adm must-gather` command collects the cluster data and saves it to the new directory.
