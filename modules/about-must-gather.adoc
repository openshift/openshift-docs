...
By default, the `oc adm must-gather` command collects cluster diagnostic data by using the default plugin image. The command saves collected cluster diagnostics to a directory formatted as `must-gather.local.<cluster_id_suffix>.<timestamp>.<random_number>`, for example, `must-gather.local.fa96a0bf69fd.20260617T143025Z.4394764343467089039`.
...
Running `oc adm must-gather` command creates a new pod in a new project on the cluster. The command collects the cluster data and saves it to the new directory.
