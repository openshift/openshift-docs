...
[WARNING]
====
You must grant the `ClusterRoleBindings` to the service account *before* adding the corresponding input type to the `ClusterLogForwarder` custom resource (CR). If you add an input type to the CR without the required RBAC binding, the entire log collector DaemonSet is deleted and all log collection fails.
====
