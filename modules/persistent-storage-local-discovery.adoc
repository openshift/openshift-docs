...
:navtitle: Automate discovery and provisioning for local storage devices
[id="local-storage-discovery_{context}"]
= Automating discovery and provisioning for local storage devices

[role="_abstract"]
Automate local storage discovery and provisioning using the Local Storage Operator (LSO) to simplify installation when dynamic provisioning is not available, such as with bare metal, {vmw-first}, or {aws-first} instances with attached devices.
