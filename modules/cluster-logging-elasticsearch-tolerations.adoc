...
You apply tolerations to Elasticsearch pods through the `ClusterLogging` custom resource (CR)
...
. Edit the `logstore` section of the `ClusterLogging` custom resource (CR) to configure a toleration for the Elasticsearch pods:
+
[source,yaml]
----
  logStore:
