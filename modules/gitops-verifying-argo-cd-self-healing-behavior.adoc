...
.. Fork the repository https://github.com/redhat-developer/openshift-gitops-getting-started.

.. Go to `deployment.yaml` -> change the `failureThreshold` value to 5.

.. Go to the deployment in the cluster and run the following command to verify the changed value of `failureThreshold`:
+
[source,terminal]
----
