...
:navtitle: Restart hosted control plane components
:chunk-to-content:
[id="hosted-restart-hcp-components_{context}"]
= Restarting hosted control plane components

[role="_abstract"]
If you are an administrator for {hcp}, you can use the `hypershift.openshift.io/restart-date` annotation to restart all control plane components for a particular `HostedCluster` resource.
