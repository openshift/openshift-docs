...
The offline migration method is a manual process that includes some downtime, during which your cluster is unreachable. You can use an Ansible playbook that automates the offline migration steps so that you can save time. These methods are primarily used for self-managed {product-title} deployments, and are an alternative to the limited live migration procedure. Consider only using these methods when you cannot perform a limited live migration to the OVN-Kubernetes network plugin.
...
Although a rollback procedure is provided, the offline migration nethods are intended as one-way processes.
