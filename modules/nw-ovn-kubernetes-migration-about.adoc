...
. Set the `migration` field of the `Network.operator.openshift.io` custom resource (CR) to `OVNKubernetes`. The Cluster Network Operator (CNO) updates the status of the `Network.config.openshift.io` CR accordingly. Make sure the `migration` field is `null` before setting it to a value.

. The Machine Config Operator (MCO) is trigger by the change of `Network.config.openshift.io` CR. The MCO rolls out an update to the systemd configuration necessary to OVN-Kubernetes. The MCO updates a single machine per pool at a time by default, so the total time the migration takes increases with the size of the cluster.

. Update the `networkType` field of the `Network.config.openshift.io` CR. The update causes the CNO to destroy the OpenShift SDN control plane pods and deploy the OVN-Kubernetes control plane pods. Additionally, it updates the Multus objects to reflect the new cluster network provider.
...
. Set the `migration` field of the `Network.operator.openshift.io` custom resource (CR) to `OpenShiftSDN`. The Cluster Network Operator (CNO) updates the status of the `Network.config.openshift.io` CR accordingly. Make sure the `migration` field is `null` before setting it to a value.

. Update the `networkType` field. The update causes the CNO to destroy the OpenShift SDN control plane pods and deploy the OVN-Kubernetes control plane pods. Additionally, it updates the Multus objects to reflect the new cluster network provider.
