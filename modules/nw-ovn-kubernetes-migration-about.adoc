...
* Before migrating to OVN-Kubernetes, ensure that the following IP address ranges are not in use: `100.64.0.0/16` and `100.88.0.0/16`. OVN-Kubernetes uses these ranges internally. If these ranges are in use, see xref:../../../networking/cluster-network-operator.adoc#cluster-network-operator[Understanding the Cluster Network Operator] for instructions on how to change these ranges before migration.
