...
The `openshift-pipelines` namespace includes versioned tasks and step actions alongside standard non-versioned tasks and step actions. For example, installing the {pipelines-title} Operator v1.16 creates `buildah-1-16-0` versioned task, `buildah` non-versioned task, `git-clone-1-16-0` versioned `StepAction` definition, and `git-clone` non-versioned `StepAction` definition.

Both, non-versioned and versioned tasks and step actions have the same metadata, behavior, and specifications, including `params`, `workspaces`, and `steps`. However, they behave differently when you disable them or upgrade the Operator.
...
a| If you deploy pipelines that use non-versioned tasks and step actions, they might break after an Operator upgrade if the automatically upgraded tasks and step actions are not backward-compatible. 
...
| Upgrading the Operator installs the latest version of versioned tasks and step actions and retains the earlier version. The latest version corresponds to the upgraded Operator. For example, installing Operator 1.17 installs `git-clone-1-17-0` step action and retains `git-clone-1-16-0` step action. 
