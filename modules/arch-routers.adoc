...
{product-title} cluster, which enable routes created by developers to be
used by external clients. The routing layer in {product-title} is pluggable, and
several router plug-ins are provided and supported by default.

A router uses the service selector to find the service and the endpoints backing
the service. When both router and service provide load balancing,
...
Routes can be sharded among the set of routers.
