...
* The VF index must be in range of `0` to `<numVfs>-1`. For example, if you have a policy with `numVfs` set to `8`, then the `<first_vf>` must not be smaller than 0, and the `<last_vf>` must not larger than 7.
* The VFs ranges in different policies must not overlap.
* The `<first_vf>` must not larger be than the `<last_vf>`.
