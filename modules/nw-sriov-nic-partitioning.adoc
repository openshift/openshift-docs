...
In such a deployment, you can use the `pfNames` selector in your `SriovNetworkNodePolicy` custom resource (CR) YAML file to specify a range of VFs for a pool by using the following format: `<pfname>#<first_vf>-<last_vf>`. For example, `pfNames: ["netpf0#2-7"]` shows the selector for an interface named `netpf0` with VF `2` through `7`. Consider that `2` is the first VF index (0-based) and `7` is the last VF index (0-based) that get included in the range.
