...
== Cluster failure
Single-AZ {product-title} clusters have three control plane nodes and two infrastructure nodes in the same availability zone (AZ) in the private subnet.

Multi-AZ {product-title} clusters have three control plane nodes and three infrastructure nodes that are preconfigured for high availability, one in each AZ. Control plane and infrastructure nodes have the same resiliency as worker nodes, with the added benefit of being managed completely by Red{nbsp}Hat.
...
== Zone failure
A zone failure from AWS affects all virtual components, such as worker nodes, block or shared storage, and load balancers that are specific to a single availability zone. To protect against a zone failure, {product-title} provides the option for machine pools that are deployed across three availability zones. Existing stateless workloads are then redistributed to unaffected zones in the event of an outage, as long as there is enough capacity.
