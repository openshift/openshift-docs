...
[role="_abstract"]
Create cluster ID labels for metrics to uniquely identify and track metrics across clusters and workloads by adding the `write_relabel` settings for remote write storage in the `{configmap-name}` config map in the `{namespace-name}` namespace. 

// tag::UWM[]
[NOTE]
====
In {ocp} and {product-rosa-hcp}, when Prometheus scrapes user workload targets that expose a `namespace` label, the system stores this label as `exported_namespace`.
