...
+
* Name: *oci-cluster*
* Bucket: Select the bucket where you uploaded the agent ISO image
...
. Go to link:https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/launchinginstance.htm#top[Creating an instance] (Oracle documentation) for instructions on how to create a compute instance from the supplied base image for your cluster topology. The following {product-title} cluster topologies are supported on {oci}:
* A single-node OpenShift cluster.
* A highly-availabile cluster that has a minimum of 3 control plane instances and two compute instances.
