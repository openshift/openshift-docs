...
[role="_abstract"]
{ols-official} 1.0.7 provides maintenance updates, security fixes, and stability improvements. This release ensures continued support and optimized performance for clusters running {ocp-product-title} 4.16 and later.

[id="ols-1-0-7-enhancements_{context}"]
== Enhancements

[role="_abstract"]
{ols-official} 1.0.7 introduces PatternFly 6 styling, a unified ecosystem software catalog, and automated CA certificate integration. This release also implements a breaking security update requiring secrets for MCP server headers and adds virtual machine context support for AI prompts.
