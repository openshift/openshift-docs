...
|Optional: Specifies whether to collect audit logs. The valid values are `true` and `false`. You must not set this field if you are using a custom image, or if you are using `spec.gatherSpec.command` with the default image..
|`boolean`
...
[NOTE]
====
Because `default` service account has minimal permissions, you can specify the service account that you created.
====
|`string`
