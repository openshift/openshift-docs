...
a|Specifies the name of the service account.
...
|Optional: Specifies the path within the PVC to store the bundle. Each run creates an isolated subdirectory under this path named `must-gather.local.<cluster-id-suffix>.<timestamp>.<random>`. The value must not contain relative parent directory indicators (`..`).
|`string`
...
|`status.status`
|Specifies the overall resource status. Valid values are `Completed`, `Failed`, or empty while running.

|`status.completed`
|Set to `true` when the job finishes.

|`status.reason`
|Specifies the description of the outcome or validation failure.

|`status.conditions`
|Specifies standard Kubernetes conditions. On validation failure, the object sets type: ReconcileError and reason: ValidationFailed.
