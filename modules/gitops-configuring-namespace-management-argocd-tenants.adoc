...
where:
+
`<argocd-ns>`:: Specifies the replace `<argocd-ns>` with the Argo CD instance namespace.
`<tenant-ns>`:: Specifies the replace `<tenant-ns>` with the namespace that you want Argo CD to manage. You can also use a glob pattern with wildcards such as `/\*` and `?` for pattern matching. For example, specifying `dev-/\*` matches any namespace with the `dev-` prefix.
