...
<2> Replace `<tenant-ns>` with the namespace you want Argo CD to manage.
...
<2> `.spec.managedBy` - Namespace where the Argo CD instance runs.  

.Verification

After creating the `NamespaceManagement` CR, the {gitops-title} Operator automatically:  
