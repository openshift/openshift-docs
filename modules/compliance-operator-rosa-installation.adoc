...
`metadata.labels.pod-security.kubernetes.io/enforce`:: Specifies the pod security label that must be set to `privileged` at the namespace level in {product-title} {product-version}.
...
----
+
The `spec.config.nodeSelector.node-role.kubernetes.io/worker` field specifies to update the Operator deployment to deploy on `worker` nodes.
