...
= Removing Cluster Logging Operator artifacts from deployed clusters with {policy-gen-cr} CRs

When you update to Cluster Logging Operator version 6.x from version 5.x, you must add a cleanup policy that removes the old Operator artifacts after creating the new API custom resources (CRs).

.Prerequisites

.Procedure

. Add the `acm-group-du-clo5-cleanup.yaml` in the git repository that contains your other `PolicyGenerator` CRs.
...
. Commit the cleanup `PolicyGenerator` CR and `kustomization.yaml` file and push to their git repository.

. Wait for ArgoCD to generate and sync the cleanup policy to the hub cluster.

.Verification

. Verify that the cleanup policy is bound to all clusters which upgraded from Cluster Logging Operator version 5.x to 6.x.
