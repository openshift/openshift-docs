...
:navtitle: Configure egress firewall and network policy auditing for a cluster
[id="nw-networkpolicy-audit-configure_{context}"]
= Configuring egress firewall and network policy auditing for a cluster

[role="_abstract"]
To customize egress firewall and network policy audit logging in {product-title}, you can configure the `policyAuditConfig` section in the cluster `Network` custom resource. Adjust log destination, file size, and rate limits to control how ACL events are recorded.
