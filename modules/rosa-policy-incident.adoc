...
- Monitor Amazon EBS volumes attached to cluster nodes and Amazon S3 buckets used for the {product-title} service’s built-in container image
...
- There is no Red{nbsp}Hat-provided backup method available for {product-title} clusters with STS.
