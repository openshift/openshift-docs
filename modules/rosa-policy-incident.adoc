...
- There is no Red{nbsp}Hat-provided backup method available for {product-title} clusters with Security Token Service (STS).
...
All {product-title} control planes are backed up. Notably, this does not include customer data stored on persistent volumes (PVs). All snapshots are encrypted and isolated within the {product-title} service account.
...
|6 hours
|24 hours
|This is a full snapshot of the control plane etcd database.

|Customer PVs
|Never
|N/A
|Customer PVs must be backed up by the customer.
|===
