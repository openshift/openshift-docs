...
Before you can use the oc-mirror plugin v2 to mirror image sets, you must create an image set configuration file.
...
.Procedure
////
these first steps are from the v1 doc, is there an equivalent for v2?
...
[source,terminal]
----
$ oc mirror init <--registry <storage_backend> > imageset-config.yaml --v2 <1>
----
...
* Create an `ImageSetConfiguration` file and modify it to include your required images.
+
