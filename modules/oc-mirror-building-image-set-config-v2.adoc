...
mirror:
  platform:
    channels:
    - name: stable-{product-version} <1>
      minVersion: {product-version}.2
      maxVersion: {product-version}.2
    graph: true <2>
  operators:
    - catalog: registry.redhat.io/redhat/redhat-operator-index:v{product-version} <3>
      packages: <4>
