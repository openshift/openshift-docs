...
. Create a secret named `mongo-url` with the `MONGO_SERVER_URL` file that contains the Mongo server URL value by entering the following command:
+
[source, terminal]
----
...
----
<1> The full path and name of the `MONGO_SERVER_URL` file that contains the Mongo server URL value.
