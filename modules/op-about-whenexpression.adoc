...
* `operator`: Specifies the relationship of an input to a set of `values`. Enter `in` or `notin` as the value for this field.
...
* The task meets all criteria: Tasks and the when expression symbol, which is displayed as a diamond shape, are displayed in a **success** state.
