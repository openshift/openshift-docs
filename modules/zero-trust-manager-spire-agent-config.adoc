...
`spec.workloadAttestors.workloadAttestorsVerification.hostCertBasePath`:: Specifies the directory containing the kubelet CA certificate. You must specify a value when `spec.workloadAttestors.workloadAttestorsVerification.type` is `hostCert`. Optional when type is `auto` (defaults to /etc/kubernetes if not specified).

`spec.workloadAttestors.workloadAttestorsVerification.hostCertFileName`:: Specifies the file name for the kubelet's CA certificate. When combined with `hostCertBasePath`, forms the full path. Required when type is `hostCert`. Optional when `spec.workloadAttestors.workloadAttestorsVerification.type` is `auto`. The default value is `/etc/kubernetes`.
