...
= Create certificates for a multi-cluster topology 
...
.. Create an OpenSSL configuration file named `intermediate.conf` in the `west/` directory for the intermediate certificate of the West cluster. Copy the following example file and save it locally:
+
You can see the following example configuration for reference:
+
[source,subs="attributes,verbatim"]
