...
ifndef::restricted[Alternately, you can specify any of the following NTP servers: `1.rhel.pool.ntp.org`, `2.rhel.pool.ntp.org`, or `3.rhel.pool.ntp.org`. When you use NTP with a DHCP, you must set the `sourcedir /run/chrony-dhcp` parameter in the `chrony.conf` file.]
