...
where:
+
`dynamicScalingEnabled`:: Set to `true` to enable dynamic scaling.
`minShards`:: Specifies the minimum number of shards. The value must be set to `1` or greater.
`maxShards`:: Specifies the maximum number of shards. The value must be greater than the value of `minShards`.
`clustersPerShard`:: Specifies the number of clusters per shard. The value must be set to `1` or greater.
