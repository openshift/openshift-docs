...
[NOTE]
====
`HTTPRoute` CRs do not support passthrough termination. To forward TLS traffic that the backend terminates, use a `TLSRoute` CR with a TLS passthrough listener. `TLSRoute` is the only Gateway API route kind that supports passthrough. The `openshift-default` GatewayClass does not support passthrough. For more information, see xref:../../../networking/ingress_load_balancing/configuring_gateway_api/routing-tls-requests-to-services.adoc#routing-tls-requests-to-services[Route TLS requests to services].
====