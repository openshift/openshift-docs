...
[NOTE]
====
`HTTPRoute` CRs do not support passthrough termination. To forward TLS traffic that the backend terminates, use a `TLSRoute` CR with a TLS passthrough listener. `TLSRoute` is the only Gateway API route kind that supports passthrough.
====
