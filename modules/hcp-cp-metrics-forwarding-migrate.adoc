...
[id="hcp-cp-metrics-forwarding-migrate_{context}"]
...
. For each cluster that requires migration, set `spec.monitoring.metricsForwarding` by entering the following command:
+
[source,terminal]
----
...
+
.. Confirm the `metrics-proxy` deployment in the hosted control plane namespace is healthy by entering the following command:
+
[source,terminal]
----
...
+
.. Confirm the `control-plane-metrics-forwarder` deployment in the hosted cluster is healthy by entering the following command:
+
[source,terminal]
----
...
. Optional: After confirming that forwarding is active through the spec field, remove the now-redundant annotation by entering the following command:
+
[source,terminal]
----
