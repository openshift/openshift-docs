...
|`kubeVirtContainer`
|When set to `true`, includes images from the hosted control planes KubeVirt CoreOS container.
|Boolean
...
|`mirror.additionalImages.platforms`
|The compute architectures to mirror for this additional image entry. When set, the oc-mirror plugin v2 applies sparse-manifest filtering, where the manifest list is pushed unchanged and child manifests for excluded architectures are omitted. When not set, all architectures present in the source are mirrored.
|Array of objects
...
|`mirror.additionalImages.platforms.os`
|The operating system of the platform to mirror for this additional image.
|String

Example: `linux`

|`mirror.additionalImages.platforms.architecture`
|The CPU architecture of the platform to mirror for this additional image.
|String
...
|`mirror.helm.repositories.charts.platforms`
|The compute architectures to mirror for the container images referenced in this remote Helm chart. When set, the oc-mirror plugin v2 applies sparse-manifest filtering, where manifest lists are pushed unchanged and child manifests for excluded architectures are omitted. When not set, all architectures present in the source are mirrored.

|Array of objects

Supported values include `amd64`, `arm64`, `ppc64le`, and `s390x`.
...
|`mirror.helm.repositories.charts.platforms.os`
|The operating system of the platform to mirror for this Helm chart.

|String

Example: `linux`

|`mirror.helm.repositories.charts.architecture`
|The CPU architecture of the platform to mirror for this Helm chart.
...
|`mirror.operators.platforms`
|The compute architectures to mirror for all packages in this Operator catalog entry. When set, the oc-mirror plugin v2 applies sparse-manifest filtering, where the manifest list is pushed unchanged and child manifests for excluded architectures are omitted. When not set, all architectures present in the source are mirrored.
|Array of objects
...
mirror:
  operators:
    - catalog: registry.redhat.io/redhat/
        redhat-operator-index:v4.17
      platforms:
        - os: linux
          architecture: amd64
        - os: linux
...
|`mirror.operators.platforms.os`
|The operating system of the platform to mirror for this catalog entry.
|String

Example: `linux`

|`mirror.operators.platforms.architecture`
|The CPU architecture of the platform to mirror for this catalog entry.
|String
...
|*Deprecated.* Use `mirror.platform.platforms` instead. The `architectures` field does not enable sparse-manifest behavior and does not preserve the manifest-list digest on the target registry. The special value `multi` mirrors all available architectures and is not available in the `mirror-platform.platforms` parameter.

The `architectures` and `platforms` fields are mutually exclusive. Do not use them together in the same `ImageSetConfiguration` file.
|Array of strings
...
|`mirror.platform.platforms`
|The compute architectures of the platform release payload to mirror. This replaces the deprecated `mirror.platform.architectures` field. When set, oc-mirror plugin v2 pushes manifest lists unchanged and omits child manifests for architectures that are not listed. When not set, all available architectures are mirrored without filtering
|Array of objects
...
|`mirror.platform.platforms.os`
|The operating system of the platform to mirror.
|String

Example: `linux`

|`mirror.platform.platforms.architecture`
|The CPU architecture of the platform to mirror.
|String
...
|Determines whether the OSUS graph is also deleted on the mirror registry.
|Boolean
The default value is `false`
endif::microshift[]
|===
