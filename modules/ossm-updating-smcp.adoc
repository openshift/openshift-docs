...
. Log in to the {product-title} CLI as a user with the `cluster-admin` role. If you are using {product-dedicated}, log in as a user with the `dedicated-admin` role. Enter the following command. Then, enter your username and password when prompted.
+
[source,terminal]
----
