...
* With this release, you can configure multiple Git providers, or you can add multiple configurations for the same Git provider, in the `TektonConfig` custom resource (CR). You can then specify a Git configuration to use in different task runs and pipeline runs by using the `configKey` parameter.
+
...
[id="cli-new-features-1-17_{context}"]
=== CLI
...
[source,terminal]
----
$ tkn task start skopeo-copy ARGS=”--all --preserve-digests”
----

[id="pac-new-features-1-17_{context}"]
=== {pac}

* With this update, the `pipelines_as_code_running_pipelineruns_count` metric is added to show the number of running `PipelineRun` resources in {pac}.

* With this update, the `pipelines_as_code_pipelinerun_duration_seconds_sum` metric is added to show the total duration in seconds taken by all `PipelineRun` resources in {pac}. The metric can be filtered by repository or by namespace.

[id="tekton-chains-new-features-1-17_{context}"]
=== {tekton-chains}

* With this update, you can configure the `TektonConfig` custom resource (CR) to generate the `x509` key pair of `ecdsa` type and use it with {tekton-chains} to sign artifacts. You can generate the key pair by setting the the `generateSigningSecret` field in the `TektonConfig` custom resource (CR) to `true`:
+
.Example of creating ecdsa key pair
[source,yaml]
----
...
* With this update, the default values for `artifacts.taskrun.storage` and `artifacts.pipelinerun.storage` {tekton-chains} parameters are changed from `tekton` to `oci` to enhance the initial setup and performance of {tekton-chains}.
...
* With this update, the {pac} controller no longer processes the GitLab push event if the push event contains no commit. Instead, it correctly displays an error message warning the user that no commit is attached.
