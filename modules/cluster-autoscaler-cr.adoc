...
* `Enabled`: The cluster autoscaler ensures that the node group never scales below the configured size set by the `minReplicas` parameter in a machine autoscaler, even if nodes are deemed unneeded.
