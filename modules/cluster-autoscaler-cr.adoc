...
a|Optional: Specifies whether the cluster autoscaler maintains the minimum size of a node group.

* `Enabled`: The cluster autoscaler ensures that the node group never scales below the configured size set by the `minReplicas` parameter in your `MachineAutoscaler` resources, even if nodes are deemed unneeded.
* `Disabled`: The cluster autoscaler can scale the node group below the configured minimum size if nodes are deemed unneeded. This is the default.
