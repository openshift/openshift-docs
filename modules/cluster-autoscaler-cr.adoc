...
a|`scaleDown.cordonNodeBeforeTerminating
|Optional: Specify whether the autoscaler should cordon a node before terminating that node by using one of the following values:

* `true`: The autoscaler cordons the node and drains any pods before deleting that node. This is the default.
* `false`: The autoscaler does not cordon the node before deleting that node. 
...
<12> Optional: Specify whether the autoscaler should cordon a node before terminating that node by using one of the following values:
+
--
* `true`: The autoscaler cordons the node and drains any pods before deleting that node. This is the default.
* `false`: The autoscaler does not cordon the node before deleting that node. 
