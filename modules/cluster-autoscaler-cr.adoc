...
[NOTE]
====
Making changes to the configuration of an existing cluster autoscaler causes it to restart.
====
...
This value must match the `spec.template.spec.metadata.labels[cluster-api/accelerator]` label in the machine set that manages the GPU-enabled nodes of that type.
For more information, see _Labeling GPU machine sets for the cluster autoscaler_.
