...
# After expanding ovsDpdk to include CPUs 4-5
spec:
  cpu:
    isolated: "6-7"
    ovsDpdk: "2-5"
...
* For an updated `ovsDpdk` set, complete the verification checks in _Reserve OVS-DPDK CPUs with a performance profile_ against the new CPU list. Confirm the following:
