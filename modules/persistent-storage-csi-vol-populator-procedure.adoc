...
The following procedure explains how to create a prepopulated persistent volume claim (PVC)  using the example `hellos.hello.example.com` Custom Resource Definition (CRD) created previously. In this example, rather than creating an actual data source, you are creating a file called "example.txt" that contains the string "Hello, world!" in the root directory of the volume.

For a more realistic implementation, you need to create a custom data source that points to a real source, for example a bucket, that the PVC would reference in the `dataSourceRef` field.
