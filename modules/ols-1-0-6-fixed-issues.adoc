...
{ols-official} 1.0.6 resolves issues with Azure OpenAI TLS certificate integration and Model Context Protocol (MCP) server deployments. These fixes ensure reliable communication with large language models (LLMs) and consistent service availability when transcript collection is disabled.
