...
Before you can add compute machines that use {op-system-base-full} as the operating system to an {product-title} {product-version} cluster, you must prepare a {op-system-base} 7 or 8 machine to run an Ansible playbook that adds the new node to the cluster. This machine is not part of the cluster but must be able to access it.
...
. Ensure that the `kubeconfig` file for the cluster and the installation program that you used to install the cluster are on the {op-system-base} machine. One way to accomplish this is to use the same machine that you used to install the cluster.
...
** On a {op-system-base} 8 system, run the following command:
+
[source,terminal]
----
...
** On a {op-system-base} 7 system, run the following command:
+
[source,terminal]
----
# subscription-manager repos \
...
[IMPORTANT]
====
As of {product-title} 4.10.23, running the Ansible playbooks on {op-system-base} 7 is deprecated, and suggested only for the purpose of updating existing installations. Starting with {product-title} 4.11, the Ansible playbooks are provided only for {op-system-base} 8.
====
+
