...
----
+
.. Check that the `matchLabels` `app` label in the ServiceMonitor matches the label output in the preceding step:
+
[source,terminal]
----
...
----
+
If there is a issue with the ServiceMonitor, the logs might include an error similar to this example:
+
[source,terminal]
----
...
[NOTE]
====
If an unrecognized Prometheus Operator `loglevel` value is included in the ConfigMap, the `prometheus-operator` pod might not restart successfully.
====
+
