...
[role="_abstract"]
Troubleshoot missing user-defined project metrics by diagnosing common `ServiceMonitor` configuration issues when you have created a `ServiceMonitor` resource but cannot see corresponding metrics in the Metrics UI.

[NOTE]
====
This section about investigating why user-defined project metrics are unavailable applies to the following distributions:
...
. Review the target status for your endpoint on the *Metrics targets* page in the web console UI.
.. Log in to the web console and go to *Observe* → *Targets*.
