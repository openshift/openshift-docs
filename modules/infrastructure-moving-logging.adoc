...
. Patch the Cluster Logging Custom Resource in the `openshift-logging` project:
+
----
$ oc patch ClusterLogging instance -n openshift-logging --type=merge -p '{"spec": {"curation": {"curator": {"nodeSelector": {"node-role.kubernetes.io/infra": ""}}}}}'
$ oc patch ClusterLogging instance -n openshift-logging --type=merge -p '{"spec": {"logStore": {"elasticsearch": {"nodeSelector": {"node-role.kubernetes.io/infra": ""}}}}}'
$ oc patch ClusterLogging instance -n openshift-logging --type=merge -p '{"spec": {"visualization": {"kibana": {"nodeSelector": {"node-role.kubernetes.io/infra": ""}}}}}'
----
