...
.Procedure
. Create a new Transport Layer Security (TLS) secret from a private key and a public certificate, where `fullchain.pem` is your full wildcard certificate chain, including any intermediaries, and `privkey.pem` is your wildcard certificate's private key.
+
.Example
[source,terminal]
----
...
. Create a new `IngressController` resource, for example, a `waf-ingress-controller.yaml`:
+
[source,yaml]
----
...
`domain: apps.example.com`:: Specifies the custom domain you want to use for the `IngressController`.
`routeSelector`:: Filters the set of routes serviced by the Ingress Controller. If you don't provide a value, no filtering occurs. In this tutorial, you use the `waf` route selector.
--
