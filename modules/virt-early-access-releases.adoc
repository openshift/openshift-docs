...
The *stable* channel, which matches the underlying {product-title} version and is fully tested, is recommended for production systems. It is possible to switch between the *stable* and *candidate* channels in Operator Hub. However, updating from the *candidate* channel to the *stable* channel is not tested by Red Hat.
