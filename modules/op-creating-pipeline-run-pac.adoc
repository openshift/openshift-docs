...
. Depending on the requirements of your pipeline run definition, complete any number of the following optional steps:
...
** Use _dynamic variables_ in your pipeline run specification. The {pac} resolver replaces these variables with values that represent the current context. For example, `{{ repo_url }}` is the current URL for the repository and `{{ revision }}` is the commit SHA on which the pipeline run was started.
+
...
<6> {pac} replaces the `{{ repo_url }}` dynamic variable with the URL for the Git repository.
<7> {pac} replaces the `{{ revision }}` dynamic variable with the revision of the branch for which the pipeline run was started.
