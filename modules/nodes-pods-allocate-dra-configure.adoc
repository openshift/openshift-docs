...
. Examine the pod status to see the resource claim was used by running the following command:
+
[source,terminal]
----
kind: Pod
...
# ...
  resourceClaimStatuses:
    - name: nic
      resourceClaimName: pod1-nic-l8bqt
# ...
----

. Examine the resource claim object by running the following command:
+
[source,terminal]
----
...
metadata:
  annotations:
    resource.kubernetes.io/pod-claim-name: nic
  name: pod1-nic-l8bqt
# ...
status:
  allocation:
    devices:
      ...
      - consumedCapacity:
        ...
        pool: ip-10-0-124-194.us-west-2.compute.internal
        request: nic
        shareID: 265cc3ad-60d6-4f49-a9ac-9d5af08cb2d9
# ...
----
+
In this example, the claim is requesting the `nic-0` device from the `net.example.com` DRA driver.
