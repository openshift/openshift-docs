...
For example, to view the documentation for the `outputs` and `options` fields of the `ClusterLogForwarder` custom resource, you can use:

[source,terminal]
----
$ oc explain clusterlogforwarders.logging.openshift.io.spec.outputs
$ oc explain clusterlogforwarders.logging.openshift.io.spec.outputs.options
----
...
[source,terminal]
----
$ oc explain lokistacks.loki.grafana.com.spec.replicas
----

This shows that if `replicas` is not set in the `LokiStack` definition, a default of 1 will be used.
