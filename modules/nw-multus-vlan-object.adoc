...
[IMPORTANT]
====
Consider the following limitations when you use a VLAN as an additional network for a cluster:

* You cannot specify the same `vlanId` for multiple `NetworkAttachmentDefinition` custom resource definitions (CRDs) that exist in the same project namespace.
* A VLAN that is configured in a `NetworkAttachmentDefinition` CRD can support only one pod per node in the same project namespace. 
** Based on this limitation, if you want additional pods to exist on the same node, the additional pods must exist in a separate project namespace and this project must use the same `NetworkAttachmentDefinition` CRD.
====
