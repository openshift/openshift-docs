...
You can enable monitoring and metrics collection for the {external-secrets-operator} by using a service monitor to perform the custom metrics scraping.
...
. Enable cluster monitoring by labeling the external secrets Operator namespace by running the following command:
