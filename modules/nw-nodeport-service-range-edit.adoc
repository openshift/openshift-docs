...
You can expand the node port range for your cluster. You can extend the range on either side but you must preserve the default port range of `30000-32767` within your new range.

[IMPORTANT]
====
You cannot contract the range on either side. If you need to re-expand the node port range, include the previous expanded range in the newly expanded port range so that control planes do not experience port allocation issues.
====
