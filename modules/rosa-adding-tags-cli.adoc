...
= Adding tags to a machine pool by using the {rosa-cli}

[role="_abstract"]
You cannot not edit the tags after after you create the machine pool.
...
* You installed and configured the latest AWS (`aws`), {rosa-cli-first}, and OpenShift (`oc`) CLIs on your workstation.
...
`<key> <value>,<key> <value>`:: Specifies a key and value for each tag, separated by commas.
--
+
.Example output
[source,terminal]
