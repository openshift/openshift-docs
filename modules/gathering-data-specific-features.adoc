...
+
You can use the `must-gather` tool with additional arguments to gather data that is specifically related to the {logging} and the
ifndef::openshift-dedicated[]
Red Hat OpenShift
endif::openshift-dedicated[]
ifdef::openshift-dedicated[]
...
{clo} in your cluster. For the {logging}, run the following command:
+
[source,terminal]
----
