...
`--image-stream=openshift/must-gather`:: The default {product-title} `must-gather` image
`--image=registry.redhat.io/container-native-virtualization/cnv-must-gather-rhel9:v{HCOVersion}`:: The must-gather image for {VirtProductName}
+
...
`--image-stream=openshift/must-gather`:: The default {product-title} `must-gather` image
`--image=quay.io/kubevirt/must-gather`:: The must-gather image for KubeVirt
