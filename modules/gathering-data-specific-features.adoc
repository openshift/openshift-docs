...
Use this image if your NHC Operator version is less than 0.9.0.  For more information, see the 'Gathering data' section about the specific Operator in the RHWA documentation.

|`registry.redhat.io/workload-availability/node-healthcheck-must-gather-rhel9:v<installed-version-NHC>`
|Data collection for the Red{nbsp}Hat Workload Availability Operators, including the Self Node Remediation (SNR) Operator, the Fence Agents Remediation (FAR) Operator, the Machine Deletion Remediation (MDR) Operator, the Node Health Check Operator (NHC) Operator, and the Node Maintenance Operator (NMO) Operator.

Use this image if your NHC Operator version is 0.9.0. or greater. For more information, see the 'Gathering data' section about the specific Operator in the RHWA documentation.
