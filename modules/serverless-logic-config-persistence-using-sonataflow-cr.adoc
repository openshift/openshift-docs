...
The `SonataFlow` CR enables workflow-specific persistence configuration. You can use this configuration independently, even if `SonataFlowPlatform` persistence is already set up in the current namespace.

To configure persistence, use the `persistence` field in the `SonataFlow` CR specification as shown in the following example:
...
<7> Key in the Secret containing the database username.
<8> Key in the Secret containing the database password.
