...
ifndef::cloud[]
Before you configure a secured cluster, you must apply the CRS by using it to create the required resources on the cluster. Applying the CRS allows the services on the secured cluster to communicate with Central.
endif::cloud[]

//Show for ACSCS
ifdef::cloud[]
Before you configure a secured cluster, you must apply the CRS by using it to create the required resources on the secured cluster. Applying the CRS allows the services on the secured cluster to communicate with {product-title-managed-short}.
endif::cloud[]
...
* Create resources using the {ocp} web console: In the {ocp} web console, make sure that you are in the `stackrox` namespace. In the top menu, click *+* to open the *Import YAML* page. You can drag the CRS file or copy and paste its contents into the editor, and then click *Create*. When the command is complete, the display shows that the `tls-cert-collector`, `tls-cert-sensor`, and tls-cert-admission-control` resources were created.

* Create resources using the {osp} CLI: Using the {osp} CLI, run the following command to create the resources:
+
[source,terminal]
----
$ oc create -f <crs_name>.yaml \// <1>
  -n <stackrox> <2>
----
...
$ kubectl create -f <crs_name>.yaml \// <2>
  -n <stackrox> <3>
----
