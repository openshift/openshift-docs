...
ifndef::cloud[]
Before you configure a secured cluster, you must apply the CRS by using it to create the required resources on the cluster. After you apply the CRS, the services on the secured cluster can communicate securely with Central.
endif::cloud[]

//Show for ACSCS
ifdef::cloud[]
Before you configure a secured cluster, you must apply the CRS by using it to create the required resources on the secured cluster. After you apply the CRS, the services on the secured cluster can communicate securely with {product-title-managed-short}.
endif::cloud[]
...
* Create resources using the {ocp} web console: In the {ocp} web console, make sure that you are in the `stackrox` namespace. In the top menu, click *+* to open the *Import YAML* page. You can drag the CRS file or copy and paste its contents into the editor, and then click *Create*. When the command is complete, the display shows that the secret named `cluster-registration-secret` was created.
