...
= Configure {TempoName} with Service Mesh ambient mode

[role="_abstract"]

Generate Layer 7 (L7) tracing spans in {SMProduct} ambient mode by using waypoint or gateway proxies to capture application-level telemetry that the Layer 4 (L4) `ztunnel` component does not offer.

.Prerequisites

* You have installed the {TempoOperator}. For more information, see "Installing the Tempo Operator".

* You have installed the {OTELOperator}. For more information, see "Installing the Red Hat build of OpenTelemetry".
