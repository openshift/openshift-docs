...
[role="_abstract"]
{Dit} is made up of several components that work together to collect, store, and display tracing data.

* *{dt}* - This component is based on the open source link:https://grafana.com/oss/tempo/[Grafana Tempo project].
...
[IMPORTANT]
====
The Jaeger UI is a deprecated feature. Deprecated functionality is still included in {dt} and continues to be supported; however, it will be removed in a future release of this product and is not recommended for new deployments.
====
...
** *OpenTelemetry Collector* - The OpenTelemetry Collector is a vendor-agnostic way to receive, process, and export telemetry data. The OpenTelemetry Collector supports open source observability data formats, for example, Jaeger and Prometheus, sending to one or more open source or commercial back-ends. Instrumentation libraries export their telemetry data to the Collector by default.
