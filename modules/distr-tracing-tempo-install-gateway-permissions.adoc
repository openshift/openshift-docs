...
[id="distr-tracing-tempo-install-gateway-permissions_{context}"]
= Configuring tenants and permissions

Authentication and authorization are provided by the Tempo Gateway service. OpenShift OAuth and the Kubernetes `TokenReview` API are used for authentication, and the Kubernetes `SubjectAccessReview` API is used for authorization.

To define tenants and manage their read and write access, the distributed tracing stack—built on the Red Hat distribution of OpenTelemetry and Tempo requires a well-configured authorization setup. This setup relies on Kubernetes Role-Based Access Control (RBAC) through ClusterRole and ClusterRoleBinding. By default, no users are granted read or write permissions, ensuring a secure baseline until explicit configurations are defined.

You can install a Configure those permissions from the *Administrator* view of the web console or using command line CLI.
...
<3> The service account name for the client to use when exporting trace data. The client sends the service account token, `/var/run/secrets/kubernetes.io/serviceaccount/token`, as the bearer token header.
// Curious about why the service account token is mentioned here in the callout about the service account name.
+
