...
This conflict occurs when an currently active {product-title} node from an older deployment uses your current {product-title} cluster name without actually being part of the new deployment.
...
. Attempt to reach the service from a control plane (master) node by running the following command:
+
[source,terminal]
----
