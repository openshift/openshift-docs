...
= Configure content filters to prune log records

[role="_abstract"]
Remove low-value fields from log records by using the prune filter to reduce log volume.
...
** Kubernetes labels that contain dots (`.`) or slashes (`/`) are normalized by the collector before the prune filter processes them. You can specify such labels in either their original or normalized form. Labels that contain hyphens (`-`) must be specified in their normalized form, with hyphens replaced by underscores. For example, to preserve `my-label`, specify `.kubernetes.labels.my_label`.
