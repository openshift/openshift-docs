...
[role="_abstract"]
To create a private cluster on {ibm-cloud-name}, you must provide an existing private VPC and subnets. The installation program must also resolve the DNS records that the cluster requires, and configures the Ingress Operator and API server for only internal traffic.
