...
[NOTE]
====
You can use this setting as a security precaution. With the default behaviour, {pac} uses the pipeline definition in the submitted pull request. With the `default-branch` setting, you must merge the pipeline definition into the default branch before {pac} runs it. This requirement ensures maximum possible verification of any changes during merge review.
====
