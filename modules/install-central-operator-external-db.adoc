...
Important things to know about installing Central:
...
.Prerequisites
* You must be using {ocp} {ocp-supported-version} or later. For more information about supported {ocp} versions, see the Red{nbsp}Hat Advanced Cluster Security for Kubernetes Support Matrix.
* You must use Postgres 15 or another supported version. For detailed requirements, see "Requirements for using an external database".
...
. Create a password secret in the deployed namespace by using the {ocp} web console or the terminal. Take one of the following actions:
...
+
--
`<declarative-configs>`:: Specifies the name of the config maps that you are using.
`<sensitive-declarative-configs>`:: Specifies the name of the secrets that you are using.
--
