...
----
for i in {0..9}; do
  kubectl exec --context="${CTX_CLUSTER1}" -n sample deploy/sleep -c sleep -- curl -sS helloworld.sample:5000/hello;
done
----
+
...
----
for i in {0..9}; do
  kubectl exec --context="${CTX_CLUSTER2}" -n sample deploy/sleep -c sleep -- curl -sS helloworld.sample:5000/hello;
done
----
+
Verify that you see responses from both clusters. This means you will see version 1 and version 2 of the service in the responses. 
