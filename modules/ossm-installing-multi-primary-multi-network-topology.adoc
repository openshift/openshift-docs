...
[id="ossm-installing-multi-primary-multi-network-topology_{context}"]
= Installing a multi-primary multi-network topology 
:context: ossm-installing-multi-primary-multi-network-topology

Install {istio} in a multi-primary multi-network topology on two {ocp-product-title} clusters. 
...
* You have completed _Creating certificates for a multi-cluster mesh_. 

* You have completed _Applying certificates to a multi-cluster topology_.
...
[source,terminal]
----
$ oc apply --context="${CTX_CLUSTER1}" -n sample -l service=helloworld \
...
[source,terminal]
----
$ oc apply --context="${CTX_CLUSTER1}" -n sample -l version=v1 \
     -f https://raw.githubusercontent.com/istio/istio/${ISTIO_VERSION}/samples/helloworld/helloworld.yaml
----
...
[source,terminal]
----
$ oc apply --context="${CTX_CLUSTER1}" -n sample \
    -f https://raw.githubusercontent.com/istio/istio/${ISTIO_VERSION}/samples/sleep/sleep.yaml
----
...
$ oc --context="${CTX_CLUSTER1}" wait --for condition=available -n sample \
     deploy/helloworld-v1 deploy/sleep
----
...
[source,terminal]
----
$ oc apply --context="${CTX_CLUSTER2}" -n sample -l service=helloworld \
     -f https://raw.githubusercontent.com/istio/istio/${ISTIO_VERSION}/samples/helloworld/helloworld.yaml
----
...
[source,terminal]
----
$ oc apply --context="${CTX_CLUSTER2}" -n sample -l version=v2 \
     -f https://raw.githubusercontent.com/istio/istio/${ISTIO_VERSION}/samples/helloworld/helloworld.yaml
----
...
[source,terminal]
----
$ oc apply --context="${CTX_CLUSTER2}" -n sample \
     -f https://raw.githubusercontent.com/istio/istio/${ISTIO_VERSION}/samples/sleep/sleep.yaml
----
...
$ oc --context="${CTX_CLUSTER2}" wait --for condition=available -n sample \
     deploy/helloworld-v2 deploy/sleep
----
...
----
$ for i in {0..9}; do
    oc exec --context="${CTX_CLUSTER1}" -n sample deploy/sleep -c sleep \
...
----
$ for i in {0..9}; do
    oc exec --context="${CTX_CLUSTER2}" -n sample deploy/sleep -c sleep \
       -- curl -sS helloworld.sample:5000/hello;
  done
----
+
...
[source,terminal]
----
$ oc --context="${CTX_CLUSTER1}" delete istio/default ns/istio-system ns/sample && \
  oc --context="${CTX_CLUSTER2}" delete istio/default ns/istio-system ns/sample
----
