...
where
+
`storage`: Specifies the storage details. Accepted values include: Not defining `optionalCsiComponents`; Specifying `optionalCsiComponents` field with an empty value (`[]`) or a single empty string element (`[""]`); Specifying `optionalCsiComponents` with one of the accepted values that are `snapshot-controller` or `none`. A value of `none` is mutually exclusive with all other values.
+
[NOTE]
====
