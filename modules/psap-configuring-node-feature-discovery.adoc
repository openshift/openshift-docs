...
[discrete]
[id="configuring-node-feature-discovery-operator-core-label-allow_{context}"]
=== core.labelallow

`core.labelallow` specifies a regular expression for filtering feature labels based on the label name. Non-matching labels are not published.
...
This value is overridden by the deprecated `--label-allow` command line flag, if specified.
...
This value is overridden by `sources.cpu.cpuid.attributeAllow`, if specified.
...
[discrete]
[id="configuring-node-feature-discovery-operator-sources-cpu-cpuid-attributeAllow_{context}"]
=== sources.cpu.cpuid.attributeAllow

Only publish the `cpuid` features listed in this option.

`sources.cpu.cpuid.attributeAllow` takes precedence over `sources.cpu.cpuid.attributeBlacklist`.
...
sources:
  cpu:
    cpuid:
      attributeAllow: [AVX512BW, AVX512CD, AVX512DQ, AVX512F, AVX512VL]
----
...
[discrete]
[id="configuring-node-feature-discovery-operator-sources-pci-deviceClassAllow_{context}"]
=== sources.pci.deviceClassAllow

`sources.pci.deviceClassAllow` is a list of link:https://pci-ids.ucw.cz/read/PD[PCI device class IDs] for which to publish a label. It can be specified as a main class only (for example, `03`) or full class-subclass combination (for example `0300`). The former implies that all
...
[discrete]
[id="configuring-node-feature-discovery-operator-sources-usb-deviceClassAllow_{context}"]
=== sources.usb.deviceClassAllow

`sources.usb.deviceClassAllow` is a list of USB link:https://www.usb.org/defined-class-codes[device class] IDs for
