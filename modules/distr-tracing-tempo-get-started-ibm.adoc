...
[id="get-started-distr-tracing-tempo-object-storage-setup-ibm-storage_{context}"]
= Set up {ibm-cloud-title} storage for the demo
...
. On {ocp-product-title}, set the storage section in the `TempoMonolithic` custom resource as follows:
+
[source,yaml]
----
