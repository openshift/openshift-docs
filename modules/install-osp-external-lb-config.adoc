...
<3> While user-managed load balancers are in Technical Preview, you must include the `TechPreviewNoUpgrade` value to deploy a cluster that uses a user-managed load balancer.