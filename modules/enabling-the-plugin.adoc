...
[role="_abstract"]
To view vulnerability information for a secured cluster in {ocp}, make sure that the {rh-rhacs-console-plugin} dynamic plugin is enabled. 

The {rh-rhacs-console-plugin} dynamic plugin is enabled by default when you install the {product-title-short} Operator on a cluster. You can choose to disable the plugin if you do not want to use it. For upgrades, depending on the version you are upgrading from, you might need to enable the plugin during or after the installation of the {product-title-short} Operator.
