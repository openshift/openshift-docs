...
[role="_abstract"]
The cluster autoscaler adjusts the size of a {product-title} cluster to meet its current deployment needs. It uses declarative, Kubernetes-style arguments for infrastructure management that are not specific to a cloud provider.

The cluster autoscaler has a cluster scope and is not associated with a particular namespace. In {product-title}, the cluster autoscaler is fully managed and hosted with the control plane.
