...
:navtitle: Verify the status of the hosted control plane feature
...
+
.. To avoid a timeout when the condition is `Degraded`, enter the following command:
+
[source,terminal]
----
...
----
+
.. To avoid a timeout when the condition is `Available`, enter the following command:
+
[source,terminal]
----
