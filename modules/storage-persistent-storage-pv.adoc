...
ifdef::openshift-rosa,openshift-rosa-hcp[]
{product-title} functions with Kubernetes Container Storage Interface (CSI) compatible volume provisioners from other storage vendors. For more information about CSI drivers in {product-title}, see "Configuring CSI volumes".
endif::openshift-rosa,openshift-rosa-hcp[]
