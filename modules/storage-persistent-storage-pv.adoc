...
{product-title} functions with Kubernetes Container Storage Interface (CSI) compatible volume provisioners from other storage vendors. For more information about CSI drivers in {product-title}, see xref:../storage/container_storage_interface/persistent-storage-csi.adoc[Configuring CSI volumes].
