...
* MACVLAN and IPVLAN cannot colocate on the same main interface due to their reliance on the same underlying kernel mechanism, specifically the `rx_handler`.
...
* EgressIP
** The Network CR `egressIPConfig` field accepts two equivalent serialization forms:
+
--
* Empty object: `egressIPConfig: {}`
* Explicit default: `egressIPConfig:` with `reachabilityTotalTimeoutSeconds: 1`
+
Both forms are valid and represent the same default configuration.
When the `egressIPConfig` field is serialized as an empty object (`egressIPConfig: {}`), this indicates that the configuration uses default values for all EgressIP settings.
--
...
The default value is `1` second when not explicitly set.
The recommended value for telco-core deployments is also `1` second.
** When running kube-compare validation (a telco reference configuration validation tool) against telco-core reference configurations, both serialization forms are accepted and will not generate configuration difference reports.
...
For more information, see xref:../../networking/ovn_kubernetes_network_provider/configuring-egress-ips-ovn.adoc#configuring-egress-ips-ovn[Configuring an egress IP address].
