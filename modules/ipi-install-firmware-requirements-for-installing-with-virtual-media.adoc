...
[role="_abstract"]
The installation program for installer-provisioned {product-title} clusters depends on the hardware and firmware compatibility with Redfish virtual media. The installation may not succeed if the node firmware is not compatible.

The following tables list the firmware versions tested and verified to work for installer-provisioned {product-title} clusters deployed by using Redfish virtual media.
...
[NOTE]
====
Always confirm that your server supports {op-system-first} on UCS Hardware and Software Compatibility. For more information, see "UCSHCL".
====
