...
[role="_abstract"]
Learn about the monitoring architecture and each component's role in the monitoring stack.

By default, the monitoring stack includes the following components:
...
|Prometheus
|The monitoring stack is based on the Prometheus monitoring system. Prometheus is a time-series database and a rule evaluation engine for metrics. Prometheus sends alerts to Alertmanager for processing.
...
|monitoring-plugin
|The monitoring-plugin dynamic plugin component deploys the monitoring pages in the *Observe* section of the web console.
...
|The openshift-state-metrics exporter (OSM in the preceding diagram) expands upon kube-state-metrics by adding metrics for cluster-specific resources.
...
|Thanos Querier
|Thanos Querier aggregates and optionally deduplicates core cluster metrics and metrics for user-defined projects under a single, multi-tenant interface.
...
The monitoring stack monitors all components within the stack. The components are automatically updated when your cluster is updated.
