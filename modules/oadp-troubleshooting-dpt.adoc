...
[role="_abstract"]
You can refer to the following table to troubleshoot common issues when running the `DataProtectionTest` (DPT) custom resource (CR).
...
| Bucket encryption/versioning not populated | Cloud provider limitations | Not all object storage providers expose these fields consistently.
