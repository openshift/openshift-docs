...
= Kafka compression for network flow export

[role="_abstract"]
To optimize network traffic, you can enable Kafka compression for your flow records. This configuration reduces network bandwidth consumption while increasing the CPU load on the eBPF agent.
...
Enabling compression reduces the amount of data transmitted over the network, which is beneficial when:
...
The trade-off is increased CPU usage on the eBPF agent pods, which run as a `DaemonSet` on every node. Different compression algorithms provide different balances between compression ratio and CPU cost:

* `lz4`: Very low CPU cost with 2-3x compression. Best for most deployments.
* `zstd`: Moderate CPU cost with 3-5x compression. Good for bandwidth-constrained environments.
* `snappy`: Similar to lz4 with slightly lower compression ratio.
* `gzip`: High CPU cost with 3-5x compression. Maximum compression at highest CPU expense.
