...
:navtitle: Create a custom SCC for the Compliance Operator
...
`priority`:: Specifies the priority of this SCC. This value must be higher than any other SCC that applies to the `system:authenticated` group.
`system:serviceaccount:openshift-compliance:api-resource-collector`:: Specifies the Service Account used by Compliance Operator Scanner pod.
