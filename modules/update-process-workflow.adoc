...
The following steps represent a detailed workflow of the {product-title} update process:
...
. The CVO detects that the `desiredUpdate` field in the `ClusterVersion` resource differs from the current cluster version.
