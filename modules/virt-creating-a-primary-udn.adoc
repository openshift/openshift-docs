...
= Creating a primary namespace-scoped UserDefinedNetwork (UDN) by using the CLI

You can create an isolated primary network in your project namespace by using the CLI. You must use the OVN-Kubernetes layer 2 topology and enable persistent IP address allocation in the UserDefinedNetwork (UDN) configuration to ensure VM live migration support.
