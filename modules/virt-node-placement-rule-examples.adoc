...
[role="_abstract"]
Examples of node placement rules for different objects that determine where {VirtProductName} components are deployed.
...
The following pods are affected by the `Subscription` object node placement rules. These pods are created when the {VirtProductName} Operator is first installed, even without an HyperConverged custom resource (CR):
