...
- `spec.matchLabels` configure the labels that match the clusters you want to reboot.
- `spec.managedPolicies` add all required configuration policies before the reboot policy. {cgu-operator} applies the configuration changes as specified in the policies, in the order they are listed.
- `spec.remediationStrategy.timeout` specify the timeout in seconds for the entire upgrade across all selected clusters. Set this field by considering the worst-case scenario.
