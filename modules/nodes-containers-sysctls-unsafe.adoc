...
. Add a label to the MachineConfigPool where the containers where containers 
...
. When the cluster is ready, check for the merged `KubeletConfig` in the new MachineConfig:
+
[source,terminal]
----
