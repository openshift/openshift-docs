...
[id="zstream-4-22-8-technical-changes_{context}"]
== Notable technical changes

Metrics forwarding annotation::
+
As of {product-title} 4.22.8, the `hypershift.openshift.io/enable-metrics-forwarding` annotation is deprecated. The `spec.monitoring.metricsForwarding` field on the `HostedCluster` API replaces this annotation and provides per-cluster configuration that the annotation cannot support.
+
