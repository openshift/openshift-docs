...
. Create a federated identity credential for the {product-title} service account for use by all components of the {TempoShortName} except the Query Frontend:
+
[source,terminal]
----
...
. Create a federated identity credential for the {product-title} service account for use by the Query Frontend component of the {TempoShortName}:
+
[source,terminal]
----
$ az identity federated-credential create \ # <1>
...
. Assign the Storage Blob Data Contributor role to the Azure service principal identity of the created Azure managed identity:
+
[source,terminal]
----
...
. Create an {product-title} secret for the Azure workload identity federation (WIF):
+
[source,terminal]
----
