...
. Create a federated identity credential for the Kubernetes service account for use by all components of the {TempoShortName} except the Query Frontend:
+
[source,terminal]
----
...
. Create a federated identity credential for the Kubernetes service account for use by the Query Frontend component of the {TempoShortName}:
+
[source,terminal]
----
$ az identity federated-credential create \ # <1>
