...
//
//{microshift-short} has the simplicity of single-node deployment with the functions and services you need for computing in resource-constrained locations. You can have many deployments on 
//different hosts, creating the specific system image needed for each of your applications.
{microshift-short} is a lightweight Kubernetes distribution designed for resource-constrained edge environments. You can use it to run the same Kubernetes workloads at the edge that you run in the cloud, overcoming challenges like minimal system resources, network constraints, and hard-to-access locations.
