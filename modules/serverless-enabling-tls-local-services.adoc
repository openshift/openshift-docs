...
[NOTE]
====
Subject Alternative Name (SAN) validation ensures that these certificates can serve requests to `<app_name>.<namespace>.svc.cluster.local`.
====
...
.. Deploy a secret in `knative-serving-ingress` namespace from the certificates created by the earlier steps:
+
[source,terminal]
----
...
.. Update the `KnativeServing` custom resource (CR) spec to use the secret that the Kourier gateway created:
+
