...
[role="_abstract"]
The `rosa create network` command is available in v.1.2.48 or later of the {rosa-cli-first}. The command uses AWS CloudFormation to create a virtual private cloud (VPC) and associated networking components necessary to install a {product-title} cluster. CloudFormation is a native AWS infrastructure-as-code tool and is compatible with the AWS CLI.
