...
You can revert on-cluster image layering from the nodes in specific machine config pools. The Machine Config Operator (MCO) reboots those nodes with the cluster base {op-system-first} image, overriding the custom layered image.

To remove a {op-system-first} custom layered image from your cluster, you need to remove the label from the layered machine config pool.

.Procedure

. Remove the remove the label from the layered machine config pool by using the following command:
+
[source,terminal]
----
