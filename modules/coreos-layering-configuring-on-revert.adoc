...
If you used a custom machine config pool (MCP) to apply an on-cluster layered image to a node, you can remove the custom layered image from the node and revert to the base image. 
...
[IMPORTANT]
====
When you remove the label, make sure the node is associated with another MCP.
====
