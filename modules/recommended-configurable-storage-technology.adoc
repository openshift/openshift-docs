...
[options="header"]
|===
|Storage type |ROX footnote:rox[ReadOnlyMany]|RWX footnote:rwx[ReadWriteMany] |Registry|Scaled registry|Metrics footnote:metrics-prometheus[Prometheus is the underlying technology used for metrics.]|Logging|Apps

| Block
| Yes footnote:disk[This does not apply to physical disk, VM physical disk, VMDK, loopback over NFS, AWS EBS, and Azure Disk.]
| No
| Configurable
| Not configurable
| Recommended
...
| Configurable
| Configurable
| Configurable footnote:metrics-warning[For metrics, using file storage with the ReadWriteMany (RWX) access mode is unreliable. If you use file storage, do not configure the RWX access mode on any PersistentVolumeClaims that are configured for use with metrics.]
| Configurable footnote:logging-warning[For logging, using any shared
...
| Not configurable
| Not configurable
| Not configurable footnote:object[Object storage is not consumed through {product-title}'s PVs/persistent volume claims (PVCs). Apps must integrate with the object storage REST API. ]
|===
