...
[options="header"]
|===
|Storage type |ROX footnoteref:[rox,ReadOnlyMany]|RWX footnoteref:[rwx,ReadWriteMany] |Registry|Scaled registry|Metrics footnoteref:[metrics-prometheus,Prometheus is the the underlying technology used for metrics.]|Logging|Apps
...
| Configurable
| Not configurable
| Recommended footnoteref:[metrics-warning,For metrics, it is an anti-pattern to
use shared storage and a single volume (RWX).]
| Recommended
| Recommended
...
| Configurable
| Configurable
| Configurable footnoteref:[metrics-warning]
...
* The preferred storage technology is block storage. Do not use file storage for
metrics.
