...
<6> For new installations, set this date to yesterdays date. This ensures the schema applies to all future logs. However, if you are migrating from an older version and the collectors are going to be started for the first time, set the date to several months ago.
