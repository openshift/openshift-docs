...
[NOTE]
====
To ensure that nodes are able to reach the control plane, all subnets added to a {product-title} cluster must have https access to the VPC Endpoint for that availability zone. Routing and firewalls (including NACLs and Security Groups) must permit this traffic at all times. To ensure availability, it is highly recommended to allow all https traffic between all subnets of the cluster.
====
endif::openshift-rosa-hcp[]
