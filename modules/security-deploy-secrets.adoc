...
:navtitle: Create secrets and config maps
[id="security-deploy-secrets_{context}"]
= Creating secrets and config maps

[role="_abstract"]
You can use the `Secret` object type to provide a mechanism to hold sensitive information such as passwords, {product-title} client configuration files, `dockercfg` files, and private source repository credentials. Secrets decouple sensitive content from pods.
