...
. Optional: Generate Day 2 configuration CRs from the reference `PolicyGenTemplate` CRs:
...
[source,terminal,subs="attributes+"]
----
$ podman run -it --rm -v `pwd`/out/argocd/example/policygentemplates:/resources:Z -v `pwd`/ref:/output:Z,U registry.redhat.io/openshift5/ztp-site-generate-rhel9:v{product-version} generator config -N . /output
----
+
