...
= List the contents of the {microshift-short} RPM release package
...
----
+
Replace `_<release_version>_` with the numerical value of the release you are deploying, using the entire version number, for example, `4.17.1`.
+
.Example output
[source,terminal,subs="+quotes"]
----
...
[source,terminal,subs="+quotes"]
----
$ rpm2cpio _<microshift_release_info>_ | cpio -idmv
----
+
Replace `_<microshift_release_info>_` with the name of the RPM package from the previous step. For example, `microshift-release-info-4.17.10-202408291007.p0.g6e4ee4d.assembly.4.17.10.el9.noarch.rpm`.
+
.Example output
[source,terminal]
----
