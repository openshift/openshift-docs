...
[NOTE]
====
When updating between {product-title} Extended Update Support (EUS) versions, you must also include images for an intermediate minor version between the current and target versions. For example, if the current version is `4.14` and the target version is `4.16`, you must include a version such as `4.15.8` in the `ImageSetConfiguration` file when using the oc-mirror plugin v2.

The oc-mirror plugin v2 might not always detect this requirement automatically, so check the link:https://access.redhat.com/labs/ocpupgradegraph/update_path[Red{nbsp}Hat {product-title} Update Graph page] to confirm any required intermediate versions and add them manually to your configuration.
====
