...
[NOTE]
====
Configuration outside the scope of the RAN DU RDS that do not meet these criteria require additional analysis to determine the impact on resource utilization and ability to meet KPI targets.
...
. Disable RHACM addons with the exception of the `policyController` and configure observability with the default configuration.
