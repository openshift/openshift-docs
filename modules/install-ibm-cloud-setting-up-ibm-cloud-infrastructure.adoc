...
To deploy an {product-title} cluster on {ibm-cloud-bm-reg} infrastructure, you must first provision the IBM Cloud nodes.
...
{ibm-cloud-bm-reg} does not run DHCP on the public or private VLANs. After provisioning IBM Cloud nodes, you must set up a DHCP server for the public VLAN, which corresponds to {product-title}'s `baremetal` network.

[NOTE]
====
The IP addresses allocated to each node do not need to match the IP addresses allocated by the {ibm-cloud-bm-reg} provisioning system.
====
