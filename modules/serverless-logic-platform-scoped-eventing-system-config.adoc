...
This configuration instructs the {ServerlessLogicOperatorName} to automatically link every workflow in the specified namespace, using the `preview` or `gitops` profile, to produce and consume events through the defined broker.

The supporting services deployed in the namespace without a custom eventing configuration will also be linked to this broker.
...
<2> Optional: Specifies the namespace of the Knative Eventing Broker. Defaults to the namespace of the SonataFlowPlatform. It is recommended to create the broker in the same namespace as the SonataFlowPlatform.