...
:navtitle: Create networking and load balancing components in {cp}
[id="installation-creating-azure-dns_{context}"]
= Creating networking and load balancing components in {cp}

[role="_abstract"]
To enable cluster communication on Microsoft {cp}, you must deploy networking and load balancing components by using the Azure Resource Manager (ARM) template.
