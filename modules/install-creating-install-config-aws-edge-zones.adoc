...
<zone_name>:: Specifies a {zone-type} name. The list of {zone-type} names that you use must exist in the same AWS Region specified in the `platform.aws.region` field.
endif::local-zone[]
ifdef::wavelength-zone[]
+
[source,yaml]
