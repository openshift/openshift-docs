...
[.small]
--
1. By default, the router reloads not more frequently than once every 5s (tunable: `ingresscontroller.spec.tuningOptions.reloadInterval`) based on how often backends are updated in the cluster. Each reload generates a new process to handle connection state and as a result, the `roundrobin` state is not preserved between reloads. This algorithm works best when pods have nearly identical computing capabilites and storage capacity. If your application or service has continuously changing endpoints, for example, due to the use of a CI/CD pipeline, uneven balancing can result. In this case, use a different algorithm.
