...
. To access the Argo Rollouts UI extension, complete the following steps:

.. Configure a sample application that includes Argo Rollouts resources.

.. In the Argo CD Web UI, open the application that includes a `Rollout` resource.

.. Click the `Rollout` resource.

.. Click the *ROLLOUT* tab to view the Argo Rollouts UI extension.
+
image::OpenShift_GitOps_Argo_CD_Rollouts_UI.png[Argo Rollouts UI extension in the Argo CD Web UI]
