...
volumeMounts:: Add a `volumeMounts` field to each container that needs the secret.
mountPath:: Specifies an unused directory name where you would like the secret to appear. Each key in the secret data map becomes the filename under `mountPath`.
readOnly:: Set to `true`.  If true, this instructs the driver to provide a read-only volume.
...
secretKeyRef:: Specifies the environment variable that consumes the secret key.
+
