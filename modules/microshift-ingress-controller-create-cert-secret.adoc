...
* Root access to {microshift-short}.
...
* A TLS private key in PEM format that is decrypted and not password-protected.
* A PEM-encoded TLS certificate and intermediate issuers.
* A concatenated single PEM file containing the leaf certificate and intermediates, if the server sends a certificate chain to clients.
* A valid certificate for the {microshift-short} apps wildcard where the `subjectAltName` extension includes DNS names covering `*.apps.<nodename>.<domain>` for your node.
