...
* This release introduces a check for the total size of all uploaded attachments, in addition to the existing check for individual attachment sizes. This helps maintain a manageable storage limit for attachments in the system, preventing potential issues due to excessive storage usage and ensuring optimal performance for a smoother user experience.

This release introduces a check for the total size of all uploaded attachments, in addition to the existing check for individual attachment sizes. This helps to prevent exceeding the context window of the large language model (LLM) so that the model does not receive more text than it can process in a single response.
