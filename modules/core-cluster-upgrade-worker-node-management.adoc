...
:navtitle: Pause and unpausing worker nodes by using {cgu-operator}
...
. Include the pause policy in your `ClusterGroupUpgrade` custom resource (CR) before the update policy:
+
[source,yaml]
----
...
+
The unpause policies for each machine config pool are included as managed policies in the final `ClusterGroupUpgrade` CR. When {cgu-operator} enforces these policies, the machine config pools unpause and all accumulated configuration changes are applied in a single reboot per node.

. Apply the unpause policy by using a `ClusterGroupUpgrade` CR:
+
[source,yaml]
----
apiVersion: ran.openshift.io/v1alpha1
...
. Apply and enable the `ClusterGroupUpgrade` CR to unpause workers by running the following commands:
+
[source,terminal]
----
