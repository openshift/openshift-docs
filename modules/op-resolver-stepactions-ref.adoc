...
After this step, you can run the build process. If the cache is successfully fetched, it includes the dependencies that the build process downloaded previously. If the cache was not fetched, the build process download dependencies normally.

The result of `cache-fetch` indicates whether a cached image was fetched. The subsequent `cache-upload` step can use this results and skip uploading a new cache image if the cache for the current hash was already available.

The following example task retrieves the source from a repository, fetches the cache (if available), then runs a Maven build, and then, if the cache was not fetched, uploads the new cached image of the build directory.
