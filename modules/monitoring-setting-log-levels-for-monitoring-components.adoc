...
. Verify that the log level is applied. The following example checks the log level for the `prometheus-operator` deployment:
+
[source,terminal,subs="attributes+"]
----
...
// tag::CPM[]
. Verify that the log verbosity is applied. The following example checks the log verbosity for the `metrics-server` deployment:
+
[source,terminal]
----
