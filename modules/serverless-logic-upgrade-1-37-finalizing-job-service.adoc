...
[role="_abstract"]
After the {ServerlessLogicOperatorName} is upgraded to version 1.37.0, the Jobs Service is automatically updated. You can optionally finalize the upgrade by identifying and removing ReplicaSets that belong to the earlier 1.36.0 version.

.Procedure
. Optional: Verify that a new ReplicaSet for the Jobs Service 1.37.0 version is created by running the following command:
+
[source,terminal]
----
...
. Optional: Delete your old `ReplicaSet` running the Jobs Service 1.36.0 version by running the following command:
+
[source,terminal]
----
