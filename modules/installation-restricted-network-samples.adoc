...
[role="_abstract"]
Most image streams in the `openshift` namespace managed by the Cluster Samples Operator point to images located in the Red Hat registry at link:https://registry.redhat.io[registry.redhat.io]. But, you can also use an alternate or mirror registry to host your images streams.
