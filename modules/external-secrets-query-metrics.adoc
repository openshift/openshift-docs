...
= Querying metrics for the {external-secrets-operator} operands

As a cluster administrator, or as a user with view access to all namespaces, you can query {external-secrets-operator} operands metrics by using the {product-title} web console or the command-line interface (CLI). For more information, see "Accessing metrics".
...
[source,promql]
----
{job="external-secrets-cainjector"}
----
