...
= Inject sidecars with an IstioRevisionTag resource
...
. Verify that a new `IstioRevisionTag` resource exists in your cluster by running the following command:
+
[source,terminal]
----
...
. Confirm that the pods are running without sidecars by running the following command. Any workloads that are already running in the required namespace should show `1/1` containers in the `READY` column.
+
[source,terminal]
----
...
. To ensure the control plane applies sidecar injection, redeploy the workloads in the `bookinfo` namespace by running the following command:
+
[source,terminal]
----
