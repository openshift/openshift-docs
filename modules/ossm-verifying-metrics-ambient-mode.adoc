...
. Find the status of `Metrics Targets` by searching for targets such as `istiod-monitor`, `ztunnel-monitor` and `waypoint-monitor`. `waypoint-monitor` is created only when waypoint proxy is created to use Layer 7 (L7) {SMProduct} features.
+
[NOTE]
====
