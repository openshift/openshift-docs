...
To delete images, you must use the `DeleteImageSetConfiguration` file that the mirroring process automatically pinned by digest in the `working-dir`.
  ...
  platform:
    channels:
      - name: stable-4.13
        ...
        maxVersion: 4.13.3
  operators:
    - catalog: registry.redhat.io/redhat/redhat-operator-index@sha256:12312343425
      packages:
