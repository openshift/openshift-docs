...
----
+
* The `metadata.name` parameter specifies the name for the object. The SR-IOV Network Operator creates a NetworkAttachmentDefinition object with same name.
* The `metadata.namespace` parameter specifies the namespace where the SR-IOV Network Operator is installed.
...
[role="_abstract"]
Verify the successful creation of the `NetworkAttachmentDefinition` CR, the additional SR-IOV network attachment, and the pod.
...
To verify that the tuning CNI is correctly configured and the additional SR-IOV network is attached, do the following:
