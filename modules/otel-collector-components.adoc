...
The Journald receiver is currently a link:https://access.redhat.com/support/offerings/techpreview[Technology Preview] feature only.

Journald receiver parses Journald events from systemd journal and sends them as logs.
...
EOF
----
<1> Filter output by message priorities or priority ranges. Defaults to `info`.
<2> A list of units to read entries from. Empty list will read all units.
<3> A very long logs and logs with unprintable characters will also be included. Defaults to `false`.
<4> The receiver will pause reading a file and attempt to resend the current batch of logs if it encounters an error from downstream components. Defaults to `false`.
<5>	Time to wait after the first failure before retrying. Defaults to `1s`. The units are `ms`, `s`, `m`, `h`.
<6> Upper bound on retry backoff interval. Once this value is reached the delay between consecutive retries will remain constant at the specified value. Defaults to `30s`. The units are `ms`, `s`, `m`, `h`.
<7> Maximum amount of time (including retries) spent trying to send a logs batch to a downstream consumer. Once this value is reached, the data is discarded. Retrying never stops if set to `0`. Defaults to `5m`. The units are `ms`, `s`, `m`, `h`.
