...
The HostMetrics receiver ingests metrics in the otlp format.

.OpenTelemetry Collector custom resource with an enabled HostMetrics receiver
[source,yaml]
----
  config: |
    receivers:
...
----
<1> The HostMetrics collection interval as time duration. If omitted, the default `+1m+` is used.
<2> The HostMetrics collection initial time delay as time duration. If omitted, the default `+1s+` is used.
<3> Configures the root_path so the hostmetrics receiver knows where the root filesystem is. If running multiple instances of the host metrics receiver, they must all have the same root_path.
<4> The list of enabled HostMetrics scrapers. Available scrapers are `cpu`, `disk`, `load`, `filesystem`, `memory`, `network`, `paging`, `processes` and `process`.
