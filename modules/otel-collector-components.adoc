...
The Cumulative to Delta processor is currently a link:https://access.redhat.com/support/offerings/techpreview[Technology Preview] feature only.

The Cumulative to Delta processor converts monotonic, cumulative sum and histogram metrics to monotonic, delta metrics.
Non-monotonic sums and exponential histograms are excluded.

.OpenTelemetry Collector custom resource with an enabled Cumulative to Delta processor
[source,yaml]
----
config: |
  processors:
    cumulativetodelta:
      ...
      exclude: <4>
        ...
        - "metric_.*"
----
<1> Configuration of included metrics. This field is optional, by default all metrics are converted to delta metrics.
<2> Defines if the metric must be an exact match with any metric listed in `metrics` or match any of the regular expressions listed in `metrics`. Supported values: `strict` and `regexp`.
<3> List of metric names or regular expressions to convert to delta metrics.
<4> Configuration of excluded metrics. If a metric matches both `include` and `exclude` filters, `exclude` takes precedence. This field is optional, by default no metrics are excluded from delta conversion.
