...
The Kubernetes cluster receiver is responsible for gathering cluster-level metrics and entity events from the Kubernetes API server. It utilizes the K8s API to stay informed about updates. Authentication for this receiver is exclusively supported through service accounts.
...
This receiver also requires to configure a Service Account with some RBAC rules

.Kubernetes cluster receiver service account
[source,yaml]
----
apiVersion: v1
kind: ServiceAccount
...
And the ClusterRoleBinding to bind the RBAC with the ServiceAccount
...
.OpenTelemetry Collector custom resource with the enabled Kubernetes cluster receiver with openshift distribution
[source,yaml]
----
  k8s_cluster:
...
Also if you enable OpenShift distribution you need to add the following rules to your ClusterRole.
[source,yaml]
----
- apigroups:
  - quota.openshift.io
