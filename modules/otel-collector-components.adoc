...
.OpenTelemetry Collector custom resource with the enabled Filelog receiver Tailing a plain text file
[source,yaml]
----
receivers:
  filelog:
...
<2> An array of operators. Each operator performs a simple responsibility, such as parsing a timestamp or JSON. Chain together operators to process logs into a desired format.
