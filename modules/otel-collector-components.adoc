    ...
    service:
      pipelines:
        ...
        metrics:
          receivers: [otlp]
----
<1> The OTLP gRPC endpoint. If omitted, the default value is `+0.0.0.0:4317+`.
    ...
    service:
      pipelines:
        traces:
          receivers: [jaeger]
----
<1> The Jaeger gRPC endpoint. If omitted, the default value is `+0.0.0.0:14250+`.
<2> The Jaeger Thrift HTTP endpoint. If omitted, the default value is `+0.0.0.0:14268+`.
<3> The Jaeger Thrift Compact endpoint. If omitted, the default value is `+0.0.0.0:6831+`.
<4> The Jaeger Thrift Binary endpoint. If omitted, the default value is `+0.0.0.0:6832+`.
    ...
    service:
      pipelines:
        traces:
          receivers: [zipkin]
----
<1> The Zipkin HTTP endpoint. If omitted, the default value is `+0.0.0.0:9411+`.
...
EOF
----
<1> Filters output by message priorities or priority ranges. Defaults to `info`.
<2> Lists the units to read entries from. If empty, entries are read from all units.
<3> Includes very long logs and logs with unprintable characters. Defaults to `false`.
<4> If set to `true`, the receiver pauses reading a file and attempts to resend the current batch of logs when encountering an error from downstream components. Defaults to `false`.
<5> The time interval to wait after the first failure before retrying. Defaults to `1s`. The units are `ms`, `s`, `m`, `h`.
<6> The upper bound for the retry backoff interval. When this value is reached, the time interval between consecutive retry attempts remains constant at this value. Defaults to `30s`. The supported units are `ms`, `s`, `m`, `h`.
<7> The maximum time interval, including retry attempts, for attempting to send a logs batch to a downstream consumer. When this value is reached, the data are discarded. If the set value is `0`, retrying never stops. Defaults to `5m`. The supported units are `ms`, `s`, `m`, `h`.
  ...
  processors:
    cumulativetodelta:
      ...
      exclude: # <2>
        ...
        - "metric_.*"
----
<1> Optional: Configures which metrics to include. When omitted, all metrics, except for those listed in the `exclude` field, are converted to delta metrics. The `match_type` field supports two values, `strict` and `regexp`, and defines if the metric must be an exact match with any metric or regular expression that is listed in `metrics` field. The `metrics` field defines specific metric names or regular expressions to convert to delta metrics.
<2> Optional: Configures which metrics to exclude. When omitted, no metrics are excluded from conversion to delta metrics. The `match_type` field supports two values, `strict` and `regexp`, and defines if the metric must be an exact match with any metric or regular expression that is listed in `metrics` field. The `metrics` field defines specific metric names or regular expressions to convert to delta metrics. If a metric matches both `include` and `exclude` filters, `exclude` takes precedence.
...
<5> Defines the maximum size of the compaction transaction. To ignore the transaction size, set to zero. If omitted, the default is `+65536+`.
