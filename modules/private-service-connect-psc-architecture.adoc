...
* A unique PSC service attachment is created for each OSD cluster in the customer {gcp-full} project. The PSC service attachment points to the cluster API server load balancer created in the customer {gcp-full} project.

* Similar to service attachments, a unique PSC endpoint is created in the Red Hat Management {gcp-full} project for each OSD cluster.

* A dedicated subnet for {gcp-full} Private Service Connect is created in the cluster's network within the customer {gcp-full} project. This is a special subnet type where the producer services are published via PSC service attachments. This subnet is used to Source NAT (SNAT) incoming requests to the cluster API server. Additionally, the PSC subnet must be within the Machine CIDR range and cannot be used in more than one service attachment.
