...
|`object`
|Specifies settings for ingress controller TLS connections, including the minimum TLS version and cipher suites. If you do not set a profile, the default value is based on the `apiservers.config.openshift.io/cluster` resource.

The ingress controller advertises default TLS curves. When the node does not run in FIPS mode, the default curves are `X25519MLKEM768`, `X25519`, `P-256`, `P-384`, and `P-521`. If the node runs in FIPS mode, the curves are `P-256`, `P-384`, and `P-521`. You cannot configure TLS curves in the `config.yaml` file.

|`ingress.tlsSecurityProfile.type`
|`Old`, `Intermediate`, `Custom`
|Specifies the profile type for the TLS security profile. The default value is `Intermediate`. The `Modern` value is not supported.

When using the `Old` and `Intermediate` profile types, the effective profile configuration is subject to change between releases. For example, given a specification to use the `Intermediate` profile deployed on release `X.Y.Z`, an upgrade to release `X.Y.Z+1` might cause a new profile configuration to be applied to the ingress controller, resulting in a rollout.
