...
|`tls`
|`list`
|Defines the transport later protocol (TLS) used and the cipher suites allowed. Provides security for exposed {microshift-short} endpoints such as the default router and the API server.
...
|Specifies the minimum version of TLS to serve from the API serve. Default is value is `VersionTLS12`.
