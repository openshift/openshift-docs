      ...
      groups:
        - usbs: <4>
            - vendor: "0x1a86" <5>
              product: "0x7523" <6>
----
<1> Default domain for the GDP.
<2> Exposes all the USB serial devices that are matched by this glob.
<3> Exposes all the ACM serial devices that are matched by this glob.
<4> Exposes a specific USB device by Vendor ID and Product ID.
<5> For example, the Vendor ID for a CH340 serial converter.
