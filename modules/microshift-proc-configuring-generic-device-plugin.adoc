...
[role="_abstract"]
The Generic Device Plugin (GDP) is disabled by default in {microshift-short}. To use the GDP, you must enable it and specify which host devices your Kubernetes applications can access. To enable the GDP, you must modify the {microshift-short} `config.yaml` file or create a configuration snippet file.
...
----
+
* The `serial` group exposes all the USB serial devices that match the `/dev/ttyUSB*` glob and all the ACM serial devices that match the `/dev/ttyACM*` glob.
* The `fuse` group exposes, for example, the file path for a fuse device at `/dev/fuse`.
* The `converter` group exposes a specific USB device by Vendor ID and Product ID. For example, `0x7523` is the Product ID and `0x1a86` is the Vendor ID for a CH340 serial converter.
* The `domain` field sets `device.microshift.io` as the default domain for the GDP.
+
[IMPORTANT]
====
...
+
--
`_<microshift_node_name>_`:: Specifies your node name.
--
+
