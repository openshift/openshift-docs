...
Failed DNS queries when using HAProxy pods::
Before this update, HAProxy pods maintained stale connection tracking UDP entries, causing DNS queries to fail. With this release, the issue has been resolved by actively updating the connection tracking UDP entry cleanup logic to use service ports instead of endpoint ports. link:https://issues.redhat.com/browse/OCPBUGS-66049[OCPBUGS-66049].
