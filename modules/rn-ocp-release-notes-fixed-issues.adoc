...
* Before this release, when you created a namespace-scoped `ProjectHelmChartRepository` object in the {product-title} web console, the Certificate Authority (CA) certificate and Transport Layer Security (TLS) client config drop-down menus listed resources from the `openshift-config` namespace instead of your selected project namespace. With this release, these drop-down menus list resources from the correct project namespace. Cluster-scoped `HelmChartRepository` objects continue to use resources from the `openshift-config` namespace. (link:https://issues.redhat.com/browse/OCPBUGS-76328[OCPBUGS-76328])
