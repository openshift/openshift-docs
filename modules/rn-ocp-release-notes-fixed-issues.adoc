...
* Before this update, cluster namespaces with "nodes" suffix in their name would cause the Performance Profile Creator (PPC) to fail when mistaking incorrect `namespace` directories with the must-gather `nodes` directories in the must-gather processed by the PPC. With this release, the PPC now correctly excludes the `namespaces` directory when processing the must-gather data to create a suggested `PerformanceProfile`. (link:https://issues.redhat.com/browse/OCPBUGS-60218[OCPBUGS-60218])
