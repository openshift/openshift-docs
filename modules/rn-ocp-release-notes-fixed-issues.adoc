...
Helm repository form uses the correct namespace for CA and TLS resources::
In previous releases, when you created a namespace-scoped `ProjectHelmChartRepository` in the OpenShift web console, the Certificate Authority (CA) certificate and Transport Layer Security (TLS) client config drop-down menus only listed resources from the `openshift-config` namespace. In {product-title} 4.22, the form lists CA and TLS resources from the selected project namespace for `ProjectHelmChartRepository` objects, while cluster-scoped `HelmChartRepository` objects continue to use `openshift-config`.
+
link:https://redhat.atlassian.net/browse/OCPBUGS-76328[OCPBUGS-76328]
