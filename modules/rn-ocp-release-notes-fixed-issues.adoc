...
* Before this update, an issue occurred with Global Navigation Satellite System (GNSS) sync recovery that was due to the RHEL-114088 firmware. As a consequence, the Direct Digital Phase-Locked Loop (DPLL) oscillated for 15 minutes during GNSS recovery, which impacted system stability. With this release, a firmware update (4.91) addresses the DPLL instability issue, which ensures that the DPLL problem no longer leads to prolonged switching between locked and holdover states. As a result, Global Positioning System (GPS) synchronization stability is improved. (link:https://issues.redhat.com/browse/OCPBUGS-66352[OCPBUGS-66352])

* Before this update, excessive `pmc` process spawning within the `linuxptp-daemon` container thread led to thread exhaustion. As a consequence, frequent restarts and service interruptions occurred. With this release, the creation of `pmc` processes is reduced, which prevents thread exhaustion. As a result, the `linuxptp-daemon` container no longer restarts due to thread exhaustion, which enhances system stability. (link:https://issues.redhat.com/browse/OCPBUGS-65889[OCPBUGS-65889])
