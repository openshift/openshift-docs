...
Do not set the `spec.kubeconfig` field. After the hosted cluster is created, the DPF HCP Provisioner Operator automatically creates the admin kubeconfig secret in the `dpf-operator-system` namespace and populates the `spec.kubeconfig` field of this resource to reference it.
