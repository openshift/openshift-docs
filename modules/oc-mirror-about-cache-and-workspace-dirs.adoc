...
[IMPORTANT]
====
In a fully disconnected workflow, the `--cache-dir` on the connected host and the `--cache-dir` on the disconnected host are independent. You do not need to copy the cache directory across the air gap with the image set archives.

* On the connected host (`mirror-to-disk`), `--cache-dir` stores image blobs and manifests for the local cache used before packing content into the archives.
* On the disconnected host (`disk-to-mirror`), `--cache-dir` stores image blobs and manifests for the local cache used when unpacking archives and pushing content to the target registry.
* Transfer only the archives produced by `mirror-to-disk` to the disconnected environment. Manage a separate cache directory on each host.
====
