...
[role="_abstract"]
Layer 2 topologies create a distributed virtual switch across cluster nodes, enabling virtual machine (VM) live migration within the same subnet, while Layer 3 topologies create unique segments per node with routing between them, effectively managing large broadcast domains.
