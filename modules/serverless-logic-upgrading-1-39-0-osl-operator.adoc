...
[id="serverless-logic-upgrading-1-39-0-osl-operator_{context}"]
= Upgrading the OpenShift Serverless Logic Operator to 1.39.0

[role="_abstract"]
You can upgrade the {ServerlessLogicOperatorName} from version 1.38.1 to 1.39.0 by performing the following steps.
...
* You have version 1.38.1 of {ServerlessLogicOperatorName} installed.
...
install-YYYY    logic-operator.v1.39.0   Manual     false
install-XXXX    logic-operator.v1.38.1   Manual     true
----
...
logic-operator.v1.39.0   OpenShift Serverless Logic Operator   1.39.0    logic-operator.v1.38.1   Succeeded
----
...
$ oc get jobs \
  -n <target_namespace> \
  -l app=<platform_name>,app.kubernetes.io/version=1.39.0
----
+
You get an output similar to the following:
+
