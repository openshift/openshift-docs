...
- `spec.profile.ptpSchedulingPolicy` scheduling policy for `ptp4l` and `phc2sys` processes. Use `SCHED_FIFO` on systems that support FIFO scheduling.
- `spec.profile.ptpSchedulingPriority` required. Sets the integer value 1-65 used to configure FIFO priority for `ptp4l` and `phc2sys` processes.
