...
You can configure cross-origin resource sharing on the {product-title} 4 source cluster to enable communication between the CAM tool on the target cluster and the source cluster's API server.
...
. Add `corsAllowedOrigins` and the CORS configuration value to the `unsupportedConfigOverrides` stanza:
+
[source,yaml]
----
spec:
