...
You must configure cross-origin resource sharing on the {product-title} 4 source cluster to enable communication between the CAM tool on the target cluster and the source cluster's API server.
