...
. Confirm that both `istioRevisions` are ready by running the following command:
+
[source,terminal]
----
...
. Verify that the column is set to `true` for the `istiorevisiontag` resource by running the following command:
+
[source,terminal]
----
