// Module is included in the following assemblies:
//
// * configure/configure-resource-requests-and-limits-for-gitops-plugin-components.adoc

:_mod-docs-content-type: PROCEDURE
[id="gitops-enabling-the-gitopsservice-custom-resource_{context}"]
= Enable the GitOpsService custom resource

[role="_abstract"]
To configure resources for {gitops-shortname} plugin components, specify `.spec.consolePlugin.backend.resources` for the backend and `.spec.consolePlugin.gitopsPlugin.resources` for the console plugin. The `resources` section defines `requests` and `limits` for each component.

.Prerequisites

* You have logged in to the {OCP} cluster as an administrator.
* You have installed the {gitops-title} Operator on your {OCP} cluster.

.Procedure

. Get the `GitOpsService` CR by running the following command.
+
[source,terminal]
----
$ oc get gitopsservice -A
----
+
Example output:
+
[source,terminal]
----
NAMESPACE          NAME      AGE
openshift-gitops   cluster   5d
----

. Edit the `GitOpsService` CR by running the following command.
+
[source,terminal]
----
$ oc edit gitopsservice cluster -n openshift-gitops
----

. In the `.spec.consolePlugin` section, add the `.backend.resources` and `.gitopsPlugin.resources` fields. These sections define requests and limits for CPU and memory.
+
--
*Example configuration:*
[source,yaml]
----
apiVersion: pipelines.openshift.io/v1alpha1
kind: GitOpsService
metadata:
  name: cluster
spec:
  consolePlugin:
    backend:
      resources:
        limits:
          cpu: 100m
          memory: 1Gi
        requests:
          cpu: 100m
          memory: 1Gi
    gitopsPlugin:
      resources:
        limits:
          cpu: 200m
          memory: 2Gi
        requests:
          cpu: 100m
          memory: 1Gi
----
--

. Save and exit the editor.
+
After you update the `GitOpsService` CR, the `GitOpsService` controller applies the specified resource requests and limits to the backend and the {gitops-shortname} plugin components.

.Verification

To verify that the resource requests and limits have been applied:

. Verify the resource configuration:
+
[source,terminal]
----
$ oc describe pod <pod-name> -n openshift-gitops
----
+
Look for the `Limits` and `Requests` sections in the output and ensure they match your configuration.
