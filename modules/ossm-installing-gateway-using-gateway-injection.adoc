...
. Optional: Create a YAML file named `gateway-pdb.yml` that defines a `PodDisruptionBudget` for the gateway. This example allows gateway pods to be evicted only when at least 1 healthy gateway pod will remain on the cluster after the eviction.
+
[source,yaml,subs="attributes,verbatim"]
----
