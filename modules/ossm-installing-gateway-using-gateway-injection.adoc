...
<3> Enables gateway injection by setting the `sidecar.istio.io/inject` label to `true`. If the name of the {istio} resource is not `default` you must use the `istio.io/rev: <istio_revision>` label, where the revision represents the active revision of the {istio} resource.
...
spec:
  ...
  ports:
    ...
    - name: https
      ...
      targetPort: 443
----
<1> When you  set `spec.type` to `ClusterIP` the gateway `Service` can only be accessed from within the cluster. If the gateway has to handle ingress traffic from outside the cluster, set `spec.type` to `LoadBalancer`. Alternatively, you can use OpenShift Routes. 
...
. Optional: Create a YAML file named `gateway-pdb.yml` that defines a `PodDisruptionBudget` for the gateway. This example allows gateway pods to be evicted only when at least 1 healthy gateway pod will remain on the cluster after the eviction. The following example uses `gateway-pdb.yml` as the name of the file.
+
[source,yaml,subs="attributes,verbatim"]
----
