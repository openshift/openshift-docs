...
[role="_abstract"]
To enforce `Allow` or `Deny` rules for selected namespaces in {product-title}, you can create a single cluster-scoped `BaselineAdminNetworkPolicy` custom resource named `default` with ingress and egress subject rules.
...
`spec.ingress.action`:: Specifies an ingress rule for the BANP. BANP have both ingress and egress rules. Accepts values of `Deny` and `Allow` for the `action` field.
