...
To ensure proper integration, you can configure the Broker in the `SonataFlowPlatform` CR referenced by the `SonataFlowClusterPlatform` CR.
...
.Example of cluster-scoped eventing system configuration 
[source,yaml]
----
...
[NOTE]
====
The `SonataFlowClusterPlatform` can refer to any `SonataFlowPlatform` that has already been deployed.
====