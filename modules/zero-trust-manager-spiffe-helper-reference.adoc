...
|`agent_address`
|Path to the SPIRE Agent Workload API socket. On Linux, SPIFFE Helper connects with `unix://<path>`. With the SPIFFE CSI driver, use `/spiffe-workload-api/spire-agent.sock`. You can also set the `SPIFFE_ENDPOINT_SOCKET` environment variable.
...
|`cmd_args`
|Space-separated arguments for `cmd`. Use quoted strings for arguments that contain spaces. Not parsed by a shell unless you invoke a shell explicitly, for example `/bin/sh -c "..."`.
...
Replace `<version>` with the tag that matches your {zero-trust-full} installation. For complete deployment manifests, see xref:zero-trust-manager-deploy-spiffe-helper_{context}[Deploying PostgreSQL with SPIFFE Helper].
