...
VFs can then be assigned to multiple pods to achieve higher throughput performance while isolating the Pods.
...
See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/hardware_networks/about-sriov#supported-devices_about-sriov[Supported devices] for more information.
...
This is because the implementation uses `iptables`, that cannot manage vfio interfaces.
...
* NICs that do not support firmware updates with UEFI secure boot or kernel lockdown must be preconfigured with sufficient virtual functions (VFs) enabled to support the number of VFs required by
the application workload.
For Mellanox NICs, you must disable the Mellanox vendor plugin in the SR-IOV Network Operator.
See link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/hardware_networks/configuring-sriov-device[Configuring an SR-IOV network
