...
When performing a `volumeSnapshotLocation` (VSL) backup on {gcp-full} Workload Identity, the Velero {gcp-full} plugin creates an invalid API request if the {gcp-full} project is also specified in the `snapshotLocations` configuration of `DataProtectionApplication` (DPA). As a consequence, the {gcp-full} API returns a `RESOURCE_PROJECT_INVALID` error, and the backup job finishes with a `PartiallyFailed` status. No known workaround exists.
