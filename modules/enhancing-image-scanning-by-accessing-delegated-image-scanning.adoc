...
The delegated image scanning feature extends scanning functionality by allowing Scanner-slim to index images from any registry and then send them to Central for vulnerability matching. To use this feature, ensure that you install Scanner-slim in your secured clusters. If Scanner-slim is not present, the system sends scan requests directly to Central.