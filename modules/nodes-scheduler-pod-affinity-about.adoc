...
podAffinity:: Stanza to configure pod affinity.
requiredDuringSchedulingIgnoredDuringExecution:: Defines a required rule.
key, values:: The key and value (label) that must be matched to apply the rule.
...
podAntiAffinity:: Stanza to configure pod anti-affinity.
