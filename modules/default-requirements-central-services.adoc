...
Beginning with release 4.4, {product-title-short} includes two image vulnerability scanners: the StackRox Scanner and Scanner v4. The StackRox Scanner is deprecated and will be removed in a future release. In release 4.4, the StackRox Scanner is required to perform node scanning. The {product-title-short} scanning services scan images that are not already scanned by scanners integrated into image registries.
