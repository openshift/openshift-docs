...
[role="_abstract"]
In {product-title} {product-version}, you can deploy a cluster into an existing VPC and subnets in {gcp-first} to avoid limit constraints in new accounts or to meet the operational constraints that your company's guidelines set.

You must also use existing routing rules. If you cannot obtain the infrastructure creation permissions that are required to create the VPC yourself, use this installation option.
...
* The VPC must be in the same {gcp-short} project that you deploy the {product-title} cluster to.
