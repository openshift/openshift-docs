...
.. Run the command provided in the output of the preceding step to log in, replacing `<api_url>` and `<cluster_admin_password>` with the API URL and cluster administrator password for your environment:
+
[source,terminal]
----
$ oc login <api_url> --username cluster-admin --password <cluster_admin_password>
----
.. Verify if you are logged in to the {product-title} cluster as the `cluster-admin` user:
+
[source,terminal]
----
$ oc whoami
