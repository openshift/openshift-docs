...
It only knows the sum of all free resources on a worker node.
...
** For example, consider a pod requesting 6 CPUs that is being scheduled to an empty node that has 4 CPUs per NUMA node.
