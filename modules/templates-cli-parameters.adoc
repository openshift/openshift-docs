...
[role="_abstract"]
You can list template parameters with the CLI on your {product-title} cluster to see which values you can override before processing a template. Use the `oc process --parameters` command with a template file or uploaded template name.
