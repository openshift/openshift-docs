...
[role="_abstract"]
Understand the Bare Metal Operator (BMO) architecture and its resources—`BareMetalHost`, `HostFirmwareSettings`, `FirmwareSchema`, and `HostFirmwareComponents`—to effectively configure and troubleshoot bare-metal host provisioning, firmware management, and lifecycle operations using Ironic and BMC interfaces. The following diagram illustrates the architecture of these resources:
