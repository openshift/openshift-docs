...
The following example also uses multiple containers. It uses {product-title} `ImageStream` image references to image streams that are running in the 'openshift' namespace:```
