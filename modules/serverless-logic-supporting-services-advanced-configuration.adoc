...
* `podTemplate.metadata.labels` - User-defined labels added to the pods created for this service. Use these labels to attach custom metadata for tracking, policy enforcement, or third-party tool integration.

[IMPORTANT]
====
The following labels are reserved and cannot be overridden: `app`, `app-namespace`, `app.kubernetes.io/component`, `app.kubernetes.io/instance`, `app.kubernetes.io/managed-by`, `app.kubernetes.io/name`, `app.kubernetes.io/part-of`, `app.kubernetes.io/version`, `sonataflow.org/service`.
====
