...
[role="_abstract"]
Use the `podTemplate` field in the `SonataFlowPlatform` custom resource (CR) to customize supporting service pod deployments. You can specify the number of replicas, environment variables, container images, resource requirements, and initialization options.
...
* `podTemplate.metadata.labels` - The platform adds these labels to all pods for this service. Use these labels for tracking, policy enforcement, or third-party tool integration.
...
[IMPORTANT]
====
You cannot set the following labels in the `podTemplate.metadata.labels` field because the platform reserves them for its operation:
