...
<path>:: Specifies the path to the folder to store the pull secret in.
<pull_secret_file_in_json>:: Specifies the name for the JSON file that you create.
+
.Example pull secret
[source,json]
----
...
<user_name>:: Specifies the user name that you configured for your registry.
<password>:: Specifies the password that you configured for your registry.
+
.Example output
[source,terminal]
----
...
<mirror_registry>:: Specifies the registry domain name, and optionally the port, that your mirror registry uses to serve content. For example, `registry.example.com` or `registry.example.com:8443`.
<credentials>:: Specifies the base64-encoded user name and password for the mirror registry.
