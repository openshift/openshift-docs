...
* You configured a container image registry, or _mirror registry_ to use in your restricted network.
...
====
ifdef::restricted[]
* You identified an image repository location on the container image registry to mirror images into.
...
you configured for your mirror registry. These credentials must have write access.
...
file and add a section that describes your mirror registry to it:
+
[source,json]
----
ifndef::openshift-origin[]
...
----
<1> For `<mirror_registry>`, specify the mirror registry domain name, and optionally the
