...
If you select *Manual* updates, the OLM creates an update request when a new version of the Operator becomes available. As a cluster administrator, you must then manually approve the update request to have the Operator updated to the new version. The *Manual* approval strategy requires a user with appropriate credentials to approve the Operator installation and subscription process.
====
