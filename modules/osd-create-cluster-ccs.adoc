...
ifdef::osd-on-gcp[]
 Red Hat recommends deploying your cluster with {GCP} Marketplace as it offers a lot more benefits such as: flexible, consumption based billing, and consuming additional capacity is frictionless and requires no Red Hat intervention.
endif::osd-on-gcp[]
