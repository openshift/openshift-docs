...
[id="nodes-containers-health-about_{context}"]
...
For example, a liveness probe with on a node with a `restartPolicy` of `Always` or `OnFailure`
...
[id="nodes-containers-health-about_types_{context}"]
