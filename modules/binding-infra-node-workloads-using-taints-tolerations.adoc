...
====
+
These examples place a taint on `node1` that has key `node-role.kubernetes.io/infra` and taint effect `NoSchedule`. Nodes with the `NoSchedule` effect schedule only pods that tolerate the taint, but allow existing pods to remain scheduled on the node. 
+
[NOTE]
====
...
====
+
These examples place a taint on `node1` that has the key `node-role.kubernetes.io/infra` and taint effect `NoExecute`. Nodes with the `NoExecute` effect schedule only pods that tolerate the taint. The effect will remove any existing pods from the node that do not have a matching toleration, removing pods that do not belong on an infra node. 
+

. Add tolerations to the pods that you want to schedule on the infra node, such as the router, registry, and monitoring workloads. Following the previous examples, add the following tolerations to the `Pod` object specification:
+
[source,yaml]
----
apiVersion: v1
...
# ...
  ...
  - key: node-role.kubernetes.io/infra <1>
    value: reserved <2>
    effect: NoSchedule <3>
  - key: node-role.kubernetes.io/infra <4>
    value: reserved <5>
    effect: NoExecute <6>
    operator: Equal <7>
----
...
<4> Specify the key that you added to the node.
<5> Specify the value of the key-value pair taint that you added to the node.
<6> Specify the effect that you added to the node.
<7> Specify the `Equal` Operator to require a taint with the key `node-role.kubernetes.io/infra` to be present on the node.
+
...
. Schedule the pod to the infra node using a scheduler. See the documentation for "Controlling pod placement using the scheduler" for details.
