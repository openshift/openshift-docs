...
----
+
`key`:: Specify the key that you added to the node.
`value`:: Specify the value of the key-value pair taint that you added to the node.
`effect`:: Specify the effect that you added to the node.
`operator`:: Specify the `Equal` Operator to require a taint with the key `node-role.kubernetes.io/infra` to be present on the node.
+
