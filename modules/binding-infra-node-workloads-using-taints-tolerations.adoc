...
+
If you added a `NoSchedule` taint to the infrastructure node, any existing pod running on that node that is controlled by a daemon set is marked as `misscheduled`. You must either delete or link:https://access.redhat.com/solutions/6592171[add toleration on `misscheduled` DNS pods]. You cannot add a toleration to a `DaemonSet` object.
+
[NOTE]
====
