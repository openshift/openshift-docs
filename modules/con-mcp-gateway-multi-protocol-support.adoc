...
* The client sends a `server/discover` request with a `MCP-Protocol-Version: 2026-07-28` header.
* The {mcpg} responds with `supportedVersions`, which is the union of all upstream MCP server protocol versions.
* The client checks `supportedVersions` against its connected upstream MCP servers and picks the latest supported version that they both share. If the client and the {mcpg} share a common version, the {mcpg} uses the most recent one.
