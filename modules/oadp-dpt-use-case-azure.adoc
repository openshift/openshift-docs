...
= Running a data protection test on an Azure cluster

[role="_abstract"]
If you are using {oadp-short} on an Azure cluster, you need to specify the Azure `STORAGE_ACCOUNT_ID` as part of the secret object. Use the following procedure to run a `DataProtectionTest` (DPT) custom resource (CR) on an Azure cluster.
