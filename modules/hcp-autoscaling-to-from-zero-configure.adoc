...
[role="_abstract"]
Configure a `NodePool` object to autoscale between zero and a maximum replica count on supported {aws-first} or {azure-short} hosted clusters. After configuration, verify the `AutoscalingEnabled` condition and validate scale-down and scale-up behavior.

.Prerequisites

* A hosted cluster on {aws-short} or {azure-short} running {product-title} 4.18 or later
...
* For {aws-short} and {azure-short}, scale-from-zero provider credentials

.Procedure

. Confirm that your hosted cluster platform supports `spec.autoScaling.min: 0`.
+
On {aws-short} and {azure-short}, the `NodePool` API accepts `min: 0`. On other platforms, `min` must be at least `1`.

. Confirm that the HyperShift Operator enables scale-from-zero.
+
On {aws-short} and {azure-short}, confirm that `--scale-from-zero-provider` and `--scale-from-zero-creds` are configured so instance-type metadata is available.
