...
+
--
`<crs_file_name.yaml>`:: Specifies the name of the file in which you stored the generated CRS.
...
`--set scanner.disable=false`:: Sets the value of the `scanner.disable` parameter to `false`, which enables Scanner-slim during the installation. In Kubernetes, the secured cluster services now include Scanner-slim.
--
** If you are using an init bundle, run the following command:
+
[source,terminal]
