...
//
// * gitops-docs/gitops/gitops-release-notes.adoc
...
where:
+
`data`:: Specifies the restore only the `data` section of the content in the `argocd-cm.yml` config map file manually.
`openshift-gitops`:: Specifies the replace the URL value in the config map entry with the new instance name `openshift-gitops`.
