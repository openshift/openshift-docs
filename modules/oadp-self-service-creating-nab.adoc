...
* You have created a `NonAdminBackupStorageLocation` (NABSL) CR to store the backup data.
...
spec:
  backupSpec:
    includedNamespaces:
    - "test-nac-ns" # <2>
    defaultVolumesToFsBackup: true # <3>
...
3. To use File System Backup (FSB), set `defaultVolumesToFsBackup` to `true`.
4. If you want to backup your data volumes by using the Data Mover, set the `snapshotMoveData` to `true`. This example uses the FSB for backup.
5. Set a NABSL CR as a storage location.
