...
Starting with {oadp-short} 1.3, if you have an immutability option configured on the target object storage, backups work as expected for specific object storage. These immutability options are referred to by different names as listed below. 
...
Backups that are stored in an {aws-short} S3 bucket support bucket versioning.
