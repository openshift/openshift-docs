...
. Create two namespaces, deploy a Broker in `namespace-1`, and configure one `PingSource` in each namespace as shown in the following example:
+
[source,yaml]
----
apiVersion: v1
...
. After you enable OIDC, create an `EventPolicy` that authorizes only the `PingSource` from `namespace-2`, as shown in the following example:
+
[source,yaml]
----
