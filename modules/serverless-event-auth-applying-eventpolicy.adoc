...
. Create an event-display service to show incoming events and add a Trigger to connect it to the Broker as per shown in the following example:
+
[source,yaml]
----
...
[source,terminal]
----
$ $ oc -n knative-eventing patch cm config-features \
  --type merge \
  -p '{"data":{"authentication-oidc":"enabled"}}'
----

. Once OIDC is enabled, create an EventPolicy that authorizes only the Pin`gSource from `namespace-2` as per shown in the following example:
+
[source,yaml]
----
...
[source,terminal]
----
$ oc -n namespace-1 logs -f -l serving.knative.dev/service=event-display
----
