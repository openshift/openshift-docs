...
[role="_abstract"]
You can verify the signatures for container images included in an {product-title} release image by pulling those signatures from the {product-title} release mirror site. Because the signatures on the mirror site are not in a format readily understood by Podman or CRI-O, you can use the `skopeo standalone-verify` command to verify that your release images are signed by Red Hat.
...
. Get the full SHA for your release by running the following command, substituting `<release_version>` with your release number, for example, `4.14.3`:
+
[source,terminal]
----
...
. Get the signature file for the specific release that you want to verify by running the following command. Replace `<sha_from_version>` with the SHA value from your release, which you obtained in the previous step:
+
[source,terminal]
----
...
. Get the manifest for the release image by running the following command. Replace `<quay_link_to_release>` with the output of the `oc adm release info` command. For example, `quay.io/openshift-release-dev/ocp-release@sha256:e73ab4b33a9c3ff00c9f800a38d69853ca0c4dfa5a88e3df331f66df8f18ec55`:
+
[source,terminal]
----
