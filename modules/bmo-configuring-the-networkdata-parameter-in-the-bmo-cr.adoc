...
[role="_abstract"]
To control network configuration when you create a bare-metal host, you can use the `networkData` field in the `BareMetalHost` custom resource (CR). This field references a Kubernetes secret that the `cloud-init` service uses to apply network settings and customize services on the host.
