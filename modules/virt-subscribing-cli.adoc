...
----
+
If the CSV was successful, the output contains an entry with `NAME=kubevirt-hyperconverged-operator-*` and `PHASE=Succeeded`.

. Check that the `HyperConverged` custom resource (CR) has the correct version. Run the following command:
+
[source,terminal]
----
