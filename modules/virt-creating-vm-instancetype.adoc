...
+
If bootable volumes for a certain operating system (Windows, RHEL, or Linux) are missing in the list, this is indicated by the operating system logo that appears under the list followed by a suggestion to add a volume for the missing operating system and a link to the *Add volume* dialog box. In addition, there is a link to the *Create a Windows boot source* quick start. The same link appears in a popover if you hover the pointer over the question mark icon next to the _Select volume to boot from_ line.
+
