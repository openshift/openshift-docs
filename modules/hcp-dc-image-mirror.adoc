...
<4> Specified images in the `additionalImages` field are examples of images only and are not strictly needed.
...
. If you have a partially disconnected environment, mirror the images from the specified image set configuration to a specified registry by entering the following command:
+
[source,terminal]
----
...
. If you have a fully disconnected environment, perform the following steps. For more information, see "Mirroring an image set in a fully disconnected environment".
