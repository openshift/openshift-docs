...
`fooServer`:: Specifies all options from `flexVolume.options`.

`kubernetes.io/fsType`:: Specifies the value of `flexVolume.fsType`.

`kubernetes.io/readwrite`:: Specifies the value `ro` or `rw` based on `flexVolume.readOnly`.

`kubernetes.io/secret/<key name>`:: Specifies all keys and their values from the secret referenced by `flexVolume.secretRef`.
