...
[role="_abstract"]
As a namespace admin user, use the steps in the following procedure to delete a `NonAdminBackup` (NAB) custom resource (CR).
...
. To delete a NAB CR, update the NAB CR YAML manifest file and add the `deleteBackup` flag as shown in the following example:
+
[source,yaml]
----
...
<nab_cr>:: Specifies the name of the NAB CR you deleted.
