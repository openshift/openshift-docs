...
. You should have created the `stackrox` namespace when you applied the CRS or the init bundle. Make sure that you are in this namespace by verifying that the menu shows *Project:stackrox*.
...
* Use the default value of `central.stackrox.svc:443` _only_ if you are installing secured cluster services in the same cluster where you installed Central.
* Do not use the default value when you are configuring many clusters. Instead, use the hostname when configuring the *Central Endpoint* value for each cluster.
...
* *Conditions: Deployed, Initialized*: Installation of the secured cluster services is complete and the secured cluster is communicating with Central.
