...
[role="_abstract"]
To finish your installation of the {mcpg} deployment Operator by using Operator Lifecycle Manager (OLM), create a `MCPGatewayExtension` custom resources (CR) manually and apply it to complete your initial setup. 

After the extension is ready, both an `HTTPRoute` object and an Envoy filter are automatically created.
...
* Set `spec.httpRouteManagement` to `Disabled` if you want to create your own `HTTPRoute` CR instead of using the automatically generated one.
...
+
* Replace `_<mcp_gateway_one>_` with name you used.
* Replace `_<mcp_system>_` with name of your {mcpg} deployment.
+
