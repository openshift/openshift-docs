...
.. In the window that opens, confirm that the **{rh-virtualization} Software Version** is **{rhv-version}** or higher.
...
** 16 GiB required for the bootstrap node
...
----
<1> For `<username>`, specify the username of an {rh-virtualization} administrator. For `<profile>`, specify the login profile, which you can get by going to the {rh-virtualization} Administration Portal login page and reviewing the *Profile* dropdown list. For `<password>`, specify the admin password.
