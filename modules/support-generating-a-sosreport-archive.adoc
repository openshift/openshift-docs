...
= Generating an sosreport archive for an {product-title} cluster node

The recommended way to generate an `sosreport` for an {product-title} {product-version} cluster node is through a debug pod.
