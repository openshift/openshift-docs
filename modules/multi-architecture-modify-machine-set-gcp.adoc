...
= Adding a multi-architecture compute machine set to your {gcp-full} cluster
...
* You used the installation program to create a 64-bit x86 or 64-bit ARM single-architecture {gcp-full} cluster with the multi-architecture installer binary.
...
+
--
.Example `MachineSet` object for a {gcp-full} 64-bit ARM or 64-bit x86 compute node
[source,yaml]
----
...
----
<4> Optional: Specify custom metadata in the form of a `key:value` pair. For example use cases, see the {gcp-full} documentation for link:https://cloud.google.com/compute/docs/metadata/setting-custom-metadata[setting custom metadata].
<5> Specify a machine type that aligns with the CPU architecture of the chosen OS image. For more information, see "Tested instance types for {gcp-full} on 64-bit ARM infrastructures".
<6> Specify the name of the {gcp-full} project that you use for your cluster.
