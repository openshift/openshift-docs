...
[NOTE]
====
You can use the `enable-cancel-in-progress-on-pull-requests` and `enable-cancel-in-progress-on-push` settings in the `platforms.openshift.pipelinesAsCode.settings` spec of the `TektonConfig` custom resource (CR) configure {pac} to enable `cancellation-in-progress` for all pipeline runs by default.
====
