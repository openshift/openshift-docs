...
By default, {pac} does not cancel pipeline runs automatically. Every pipeline run that {pac} creates and starts executes until it completes.

However, events that trigger pipeline runs can come in quick succession. For example, if a pull request triggers a pipeline run and then the user updates the pull request with a new commit, a new copy of the pipeline run starts every time. If several pushes happen, several copies can run, resulting in a drain on cluster resources.
...
You can use the following example to enable automatic cancellation for the `sample-pipeline` pipeline run:
