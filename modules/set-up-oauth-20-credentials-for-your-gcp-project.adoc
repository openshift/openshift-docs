...
.Procedure
. Create a new {gc} project. See the Google documentation topic "Creating and managing projects".
...
[NOTE]
====
The OAuth consent screen configuration is valid for the entire GCP project, and not only to the application you created in the earlier steps.
...
This name is displayed to users on the consent screen when they sign in.
...
Single sign-on requires only these scopes.
