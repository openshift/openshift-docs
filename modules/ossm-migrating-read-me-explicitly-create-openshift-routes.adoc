...
An OpenShift `Route` resource allows an application to be exposed with a public URL by using {ocp-product-title} Ingress Operator for managing HAProxy based Ingress controllers.
...
Thus, in {SMProduct} 3.2, when a `Route` is required to expose an {istio} gateway, you must create and manage it manually. You can also expose an {istio} gateway through a Kubernetes service of type `LoadBalancer` if a route is not required.