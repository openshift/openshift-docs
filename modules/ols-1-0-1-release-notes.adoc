...
{ols-official} 1.0.1 provides maintenance updates, security fixes, and stability improvements. This release ensures continued support and optimized performance for clusters running {ocp-product-title} 4.16 and later.
...
{ols-official} 1.0.1 is now generally available for {ocp-product-title} 4.15 and later. This release introduces the BYO Knowledge tool as a Technology Preview feature, allowing users to create retrieval-augmented generation (RAG) databases from custom content to enhance AI-driven troubleshooting.
