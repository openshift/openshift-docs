...
----
+
`metadata.name`:: The name for the `NetworkAttachmentDefinition` object.
`spec.config.name`:: The name for the configuration. It is recommended to match the configuration name to the `name` value of the network attachment definition.
`spec.config.type`:: The actual name of the Container Network Interface (CNI) plugin that provides the network for this network attachment definition. This example uses a Linux bridge CNI plugin. You can also use an OVN-Kubernetes localnet or an SR-IOV CNI plugin.
`spec.config.bridge`:: The name of the Linux bridge configured on the node.
`spec.config.macspoofchk`:: Optional: A flag to enable the MAC spoof check. When set to `true`, you cannot change the MAC address of the pod or guest interface. This attribute allows only a single MAC address to exit the pod, which provides security against a MAC spoofing attack.
`spec.config.vlan`:: Optional: The VLAN tag. No additional VLAN configuration is required on the node network configuration policy.
`spec.config.preserveDefaultVlan`:: Optional: Indicates whether the VM connects to the bridge through the default VLAN. The default value is `true`.
