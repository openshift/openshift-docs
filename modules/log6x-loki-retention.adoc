...
<2> Contains the link:https://grafana.com/docs/loki/latest/logql/query_examples/#query-examples[LogQL query] used to define the log stream. The `=~` operator is a regular expression.

** Enable stream-based retention for an individual namespace as shown in the following example. Stream rules under the tenant override the tenant default `days` value when `priority` is set. Use `=` for an exact namespace match:
+
.Example per-tenant retention for a single namespace
[source,yaml]
----
apiVersion: loki.grafana.com/v1
kind: LokiStack
...
spec:
      ...
      application:
        retention:
          ...
          streams:
            - days: 1
              priority: 1
              selector: '{kubernetes_namespace_name="logy-logger"}' <1>
  managementState: Managed
  replicationFactor: 1
  size: 1x.small
  storage:
  ...
  tenants:
    mode: openshift-logging
----
<1> Exact-match LogQL selector for one namespace. Application logs in `logy-logger` are retained for 1 day; other application logs use the tenant default of 14 days.
