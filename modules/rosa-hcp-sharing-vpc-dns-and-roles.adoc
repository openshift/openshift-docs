...
$ rosa create account-roles --oidc-config-id <oidc-config-ID> <1>
    ...
    --route-53-role-arn <Created_Route_53_Role_Arn> <3>
...
. After you create the Operator roles, share the full domain name, which is created with `<intended_cluster_name>.<reserved_dns_domain>`, your _Ingress Operator Cloud Credentials_ role's ARN, your _Installer_ role's ARN, and your _Control plane Operator Cloud Credentials_ role's ARN with the *VPC Owner* to continue configuration.
+
...
* Insert control plane operator info here