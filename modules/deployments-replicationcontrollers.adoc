...
`replicas`:: The number of copies of the pod to run.
`selector`:: The label selector of the pod to run.
`template`:: A template for the pod the controller creates.
`labels`:: Labels on the pod should include those from the label selector.
`name`:: The maximum name length after expanding any parameters is 63 characters.
