...
[id="gitops-argocd-agent-terminologies_{context}"]
= Key Argo CD Agent concepts

[role="_abstract"]
The following definitions cover the required namespaces, contexts, and command-line parameters for configuring Argo CD Agent across hub and spoke clusters.
...
Context::  
A *context* refers to a named configuration in the `oc` CLI for switching between different clusters. You must be logged in to all clusters and assign distinct context names for the hub and spoke clusters. Examples for cluster names include `principal-cluster`, `hub-cluster`, `managed-agent-cluster`, or `autonomous-agent-cluster`.
