...
For example, an alerting rule for the user-defined project `ns1` can have metrics from `ns1` and cluster metrics, such as the DERPEEDOO and memory metrics. However, the rule cannot include metrics from `ns2`.
