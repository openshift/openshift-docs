...
+
<version>:: The Vault KMS plugin image version tag (for example, `1.0.0-ubi`).
<vault_username>:: Your Vault username for authentication.
<vault_password>:: Your Vault password for authentication.
+
Update the Vault address (`-vault-address`) to match your Vault server URL. Vault namespaces (configured with `-vault-namespace`) are optional.
+
[NOTE]
====
Alternatively, you can use the upstream HashiCorp image from Docker Hub: `docker.io/hashicorp/vault-kube-kms:<version>`.
====

. Deploy the static pod manifest to each control plane node:
+
[source,terminal]
----
...
. Verify the static pods are running:
+
[source,terminal]
----
...
. Verify the socket exists on a control plane node:
+
[source,terminal]
----
...
. Monitor the `kube-apiserver` rollout progress:
+
[source,terminal]
----
...
. Check the encryption migration status:
+
[source,terminal]
----
