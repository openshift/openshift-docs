...
. Choose the appropriate example `{policy-gen-cr}` CR for your configuration category from the `{argocd-folder}` folder, for example, `{group-sno-ranGen}`, `{group-3node-ranGen}`, or `{group-standard-ranGen}`.

. Configure group or site-specific values in the `ConfigMap` resources in the `template-values/` directory. `PolicyGenTemplate` CRs use hub-side templating to substitute these values into the generated policies. For more information, see xref:../../edge_computing/ztp-using-hub-cluster-templates.adoc#ztp-specifying-nics-in-pgt-crs-with-hub-cluster-templates_hub-cluster-templates-pgt[Specifying group and site configurations in PolicyGenerator or PolicyGenTemplate CRs].

. Ensure the `{binding-field}` field in the `{policy-gen-cr}` CR matches the labels defined in the related `ClusterInstance` CR. For example, the {sno} group example binds on `group-du-sno: ""`.
+
[NOTE]
====
...
. Configure group or site-specific values in the `ConfigMap` resources in the `template-values/` directory. `PolicyGenerator` CRs use hub-side templating to substitute these values into the generated policies. For more information, see xref:../../edge_computing/ztp-using-hub-cluster-templates.adoc#ztp-specifying-nics-in-pgt-crs-with-hub-cluster-templates_hub-cluster-templates-pgt[Specifying group and site configurations in PolicyGenerator or PolicyGenTemplate CRs].

. Ensure the `{binding-field}` field in the `{policy-gen-cr}` CR matches the labels defined in the related `ClusterInstance` CR. For example, the {sno} group example binds on `group-du-sno: ""`.
+
[NOTE]
====
Ensure that the labels defined in your `PolicyGenerator` `{binding-field}` field correspond to the labels that are defined in the related managed clusters `ClusterInstance` CR.
====
endif::[]

. Optional: Create a `{policy-gen-cr}` CR for any common configuration CRs that apply to the entire fleet of clusters.

.. Select the appropriate example for your CR from the `{argocd-folder}` folder, for example, `{common-ranGen}`.
...
.. Select the appropriate example for your CR from the `{argocd-folder}` folder, for example, `{group-sno-ranGen}`.
