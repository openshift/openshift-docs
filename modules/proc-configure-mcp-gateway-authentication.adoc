...
= Configure {mcpg} authentication with an AuthPolicy

[role="_abstract"]
Configure authentication for your {mcpg} by using a {prodname} `AuthPolicy` custom resource (CR) and an identity provider such as {keycloak}.
...
* The `spec.targetRef.name:` parameter value is the name of your {mcpg}. Replace `_<mcp_gateway>_` as needed.
