...
:navtitle: Configure NBDE with static IP in an {ibm-z-title} or {ibm-linuxone-title} environment
...
[role="_abstract"]
Enabling NBDE disk encryption in an {ibm-z-name} or {ibm-linuxone-name} environment requires additional steps.
...
ifndef::ibm-z-kvm[]
`storage.luks.device`:: Specifies the device to encrypt. For installations on DASD-type disks, replace with `device: /dev/disk/by-label/root`.
endif::ibm-z-kvm[]
...
ifdef::ibm-z[]
`coreos.inst.install_dev`:: Specifies the block device type. For installations on DASD-type disks, specify `/dev/dasda`. For installations on FCP-type disks, specify `/dev/sda`.
endif::ibm-z[]
ifdef::ibm-z-lpar[]
`coreos.inst.install_dev`:: Specifies the block device type. For installations on DASD-type disks, specify `/dev/dasda`. For installations on FCP-type disks, specify `/dev/sda`. For installations on NVMe-type disks, specify `/dev/nvme0n1`.
endif::ibm-z-lpar[]
