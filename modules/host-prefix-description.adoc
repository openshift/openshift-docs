...
After cluster installation, you can increase the per-node pod subnet size by changing `hostPrefix` to a smaller prefix length. Decreasing the per-node subnet size is not supported. For more information, see "Increasing the cluster network hostPrefix".
