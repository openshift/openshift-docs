...
You can optionally assign a container runtime class to `controller` and `speaker` pods by configuring the MetalLB custom resource. For example, for Windows workloads on a Windows node, you can assign a Windows runtime class to the pod, which uses this runtime class for all containers in the pod. This ensures MetalLB components operate as required in clusters with mixed operating system environments. 
...
metadata:
  name: myclass
handler: myconfiguration <1>
----
<1> The `handler` parameter specifies the Container Runtime Interface (CRI) configuration. See the _Additional resources_ for more information about the specifications for the `RuntimeClass` resource.
