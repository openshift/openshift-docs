...
== Exposing the federation ingress on clusters running on {iibm-cloud-bm-reg} and {ibm-z-reg}
If the cluster runs on {iibm-cloud-bm-reg} or {ibm-z-reg} infrastructure and fully supports `LoadBalancer` services, the IP address found in the `.status.loadBalancer.ingress.ip` field of the ingress gateway `Service` object should be specified as one of the entries in the `.spec.remote.addresses` field of the `ServiceMeshPeer` object.
