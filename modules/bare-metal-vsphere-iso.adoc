...
To add bare-metal compute machines to your vSphere cluster, you must manually provision them using an {op-system} ISO image and the `coreos-installer` utility.
...
----
+
** Replace `/dev/sda` with the target install device for your hardware.
** Replace `<HTTP_server>` with the address of your web server.
