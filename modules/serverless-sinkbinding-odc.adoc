...
After Knative Eventing is installed on your cluster, you can create a sink binding by using the web console. Using the {ocp-product-title} web console provides a streamlined and intuitive user interface to create an event source.
...
. Browse the `event-display` service to see events produced by the heartbeats cron job.
+
image::servlerless-pingsource-showcase-gui.png[View the events in the web UI]
