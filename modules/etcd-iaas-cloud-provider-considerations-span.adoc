...
* The minimum effective MTU across the infrastructure should be the maximum MTU used for the deployment. Using a lower MTU is OK. See link:https://access.redhat.com/articles/7010220[Understanding and Validating MTU setting with {product-title} 4.x] for more information.
...
* For guidance on cluster deployments that span data centers using {rh-storage} as the storage provider, see link:https://docs.redhat.com/en/documentation/red_hat_openshift_data_foundation/4.19/html-single/configuring_openshift_data_foundation_disaster_recovery_for_openshift_workloads/index[Configuring {rh-storage} Disaster Recovery for OpenShift Workloads].