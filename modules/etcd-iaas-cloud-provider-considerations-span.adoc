...
[role="_abstract"]
Review infrastructure and cloud provider constraints for multisite {product-title} control planes so you can plan maximum transmission unit (MTU), latency, and storage requirements.

This guidance applies to any infrastructure provider for which {product-title} control plane nodes are supported by the user-provisioned infrastructure installer (platform=none) or the agent-based installer (platform=metal) using the ”User Managed Network” option.
...
* For guidance on cluster deployments that span data centers using {rh-storage} as the storage provider, see "Configuring {rh-storage} Disaster Recovery for OpenShift Workloads".
