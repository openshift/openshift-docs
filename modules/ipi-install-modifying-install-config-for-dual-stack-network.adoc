...
Use the following procedure to deploy dual-stack static IP addressing for the bootstrap VM. The bootstrap functions with a single IP version. Configuring dual-stack with a static IP set in install-config requires one VIP each for API and Ingress. Add secondary virtual IP addresses (VIPs) after deployment.
