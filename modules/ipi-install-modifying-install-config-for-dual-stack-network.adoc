...
When deploying static IP addressing with dual-stack networking for the bootstrap VM, the bootstrap VM functions with a single IP version. Configuring dual-stack with a static IP set in install-config requires one virtual IP address (VIP) each for API and Ingress. Add secondary VIPs after deployment.

[NOTE]
====
DHCP-based dual stack clusters can deploy with one IPv4 and one IPv6 VIP each from Day 1.

Deploying a cluster with static IP addresses involves configuring IPv4 addresses for the bootstrap VM, and the API and Ingress VIPs. The following examples are not for static IPs and are for DHCP, where both IP stacks can be configured from Day 1.
====
