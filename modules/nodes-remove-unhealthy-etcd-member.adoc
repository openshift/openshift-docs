...
----
+
Where `<etcd_pod>` is the name of an etcd pod associated with one of the healthy nodes.
+
.Example command
[source,terminal]
----
...
----
+
Where `<unhealthy_member_id>` is the ID of the etcd member on the unhealthy node.
+
.Example command
[source,terminal]
----
...
----
+
Where `<node_name>` is the name of the failed node whose etcd member you removed.
+
.Example command
[source,terminal]
----
