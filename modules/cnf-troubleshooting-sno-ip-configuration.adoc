...
|Investigate cluster health issues. As a last resort, skip health checks using the `lca.openshift.io/ipconfig-skip-pre-configuration-cluster-health-checks` and `lca.openshift.io/ipconfig-skip-post-configuration-cluster-health-checks` annotations.
...
|Pre-existing pods might not automatically receive an IP address from the new family due to CNI behavior.
|Delete and recreate affected pods to obtain addresses from the new IP family.
