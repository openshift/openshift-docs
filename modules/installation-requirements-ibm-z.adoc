...
[role="_abstract"]
To install {product-title} on {ibm-z-name} or {ibm-linuxone-name}, your environment must meet minimum hardware, operating system, network, and storage requirements.
...
When running {product-title} on {ibm-z-name} in an LPAR without a hypervisor, use the Dynamic Partition Manager (DPM) to manage your machine.
