...
[NOTE]
====
If the scheduler has a default node selector set, the `oc debug node/<node>` command might not succeed. To work around this, first create a temporary project using `oc adm new-project` that overwrites the default node selector. Then, run the `oc debug node/<node>` command in that temporary project.
====
