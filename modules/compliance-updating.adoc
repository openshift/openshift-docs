...
----
+
The command output lists the configurations flagged with an outdated status:
+
[source,terminal]
----
NAME                              STATE
...
. Verify that the remediation state has switched from `Outdated` to `Applied`:
+
[source,terminal]
----
...
----
+
The status display confirms the state change:
+
[source,terminal]
----
NAME                              STATE
...
. Monitor the process as the cluster nodes apply the newer remediation version and automatically reboot.
+
[IMPORTANT]
====
