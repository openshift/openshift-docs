...
Resolve the `KubePersistentVolumeFillingUp` alert being triggered for Prometheus. The critical alert fires when persistent volumes claimed by `prometheus-k8s-*` pods in the `openshift-monitoring` project have less than 3% total space remaining. This can cause Prometheus to function abnormally.
