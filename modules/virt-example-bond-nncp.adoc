...
Choose the bonding mode appropriate for your cluster.

image::nw-configuringRoutes-BondingModeDecisionTree.png["A decision tree for deciding the best bonding mode to use for your use case. Only bonding modes 1, 2, and 4 are fully supported. Other bonding modes may function in specific use cases. Bonding mode 1 is only for active-backup."]

[NOTE]
====
{VirtProductName} only supports the following bonding modes:

* mode=1 active-backup +
* mode=2 balance-xor +
* mode=4 802.3ad +

Other bonding modes are not supported.
====
