...
[role="_abstract"]
{ols-official} 0.3.4 is now generally available for {ocp-product-title} 4.15 and later.
...
[role="_abstract"]
{ols-official} 0.3.4 introduces the ability to use the `NetworkPolicy` resource to limit ingress traffic to {ols-long} components.
