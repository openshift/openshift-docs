...
. Create a `credentials-velero-snapshot` file for the snapshot location in the appropriate format for your cloud storage provider.
. Create a `Secret` for the snapshot location with the default name:
+
[source,terminal,subs="attributes+"]
----
$ oc create secret generic {cloud-credentials} -n openshift-adp --from-file cloud=credentials-velero-snapshot
----

. Create a `credentials-velero` file for the backup location in the appropriate format for your MCG object storage bucket.
