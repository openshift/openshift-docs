...
When the Kubernetes API is unavailable, check the control plane nodes to ensure that they are running the correct components. Also check hostname resolution.
...
. Check the cluster nodes to ensure they have a fully qualified domain name, and not just `localhost.localdomain` using the following command:
+
[source,terminal]
----
$ hostname
