...
:navtitle: Configure the PF Status Relay Operator for LACP state monitoring on SR-IOV networks
...
.. Simulate an LACP failure on your upstream physical switch. To simulate this scenario, you can filter LACP traffic on the switch port that you want to test the failure on. This ensures that the physical link remains up while the LACP polling fails. The command to do this is vendor-dependent.
