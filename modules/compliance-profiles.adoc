...
[role="_abstract"]
The Compliance Operator provides many compliance profiles for different security benchmarks. Understanding available profiles helps you select the appropriate compliance standards for your cluster security requirements.

View the available profiles by running the following command:
...
The following text shows the command output:
...
These profiles represent different compliance benchmarks. Each profile name uses a prefix that identifies the product it applies to. For example, `ocp4-e8` applies the Essential 8 benchmark to {product-title}, while `rhcos4-e8` applies the Essential 8 benchmark to {op-system-first}.
