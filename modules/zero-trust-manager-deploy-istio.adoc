...
----
+
<1> Replace `<version>` with the Istio version supported by your {SMProductName} Operator. You can find supported versions by running `oc get IstioCNI -o jsonpath='{.items[*].spec.version}'` after the operator is installed.
