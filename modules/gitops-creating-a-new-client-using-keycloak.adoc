...
* Optional: Customize the route name of the Keycloak instance by adding a host name attribute in the Argo CD CR. Use this feature to support advanced routing use cases such as balancing incoming traffic load among multiple link:https://docs.openshift.com/container-platform/latest/networking/ingress-sharding.html#nw-ingress-sharding_ingress-sharding[Ingress Controller shards].
+
** To add a `host name` attribute in the Argo CD CR, add this YAML snippet in your namespace.
+
[source,yaml]
----
...
----
+
** Verify that the `host name` attribute is added by running the following command in the {gitops-shortname} CLI:
+
[source,terminal]
----
...
----
+
.Example output of the `host name` attribute in the `keycloak` route
+
[source,yaml]
----
kind: Route
