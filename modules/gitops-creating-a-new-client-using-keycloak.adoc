...
where:
+
`<resource_name>`:: Specifies the replace `<resource_name>` with the name of the `ArgoCD` CR.
`<hostname>`:: Specifies the replace `<hostname>` with the name of the host key, for example, `sso.test.example.com`.
+
...
`labels`:: Specifies the name of the host key.
+
[NOTE]
====
