...
<subnet_id>:: Specifies a list of subnet IDs created in the zones: Availability and {zone-type}. For example: `publicSubnetId-1` or `publicSubnetId-LocalZone-1`.
endif::local-zone[]
ifdef::wavelength-zone[]
.Example installation configuration file with {zone-type} subnets
[source,yaml]
...
<subnet_id>:: Specifies a list of subnet IDs created in the zones: Availability and {zone-type}. For example: `publicSubnetId-1` or `publicOrPrivateSubnetID-Wavelength-1`.
endif::wavelength-zone[]
