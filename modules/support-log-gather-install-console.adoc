...
. From *Version* drop-down list, select the {support-log-gather} version , and click *Install*.

. On the *Install Operator* page, configure the installation settings:

.. Choose the *Installed Namespace* for the Operator.
+
The default Operator namespace is `must-gather-operator`.If the `must-gather-operator` namespace does not exist, it is created for you.

.. Select an *Update approval* strategy:

*** Select **Automatic** to have the Operator Lifecycle Manager (OLM) update the Operator automatically when a new version is available.
