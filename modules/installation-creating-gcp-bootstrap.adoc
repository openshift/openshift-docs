...
You must create the bootstrap machine in {gcp-first} to use during
...
zone:: Specifies the zone to deploy the bootstrap instance into, for example `us-central1-b`.
...
root_volume_size:: Specifies the boot disk size for the bootstrap machine.
bootstrap_ign:: Specifies the URL output when creating a signed URL.
