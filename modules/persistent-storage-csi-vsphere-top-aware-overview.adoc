...
:navtitle: VSphere CSI topology overview
[id="persistent-storage-csi-vsphere-top-aware-overview_{context}"]
= vSphere CSI topology overview

[role="_abstract"]
You can improve cluster resilience and avoid single points of failure by deploying {product-title} across multiple vSphere zones and regions using topology-aware storage provisioning.
...
[id="persistent-storage-csi-vsphere-top-aware-overview-top-requirements_{context}"]
