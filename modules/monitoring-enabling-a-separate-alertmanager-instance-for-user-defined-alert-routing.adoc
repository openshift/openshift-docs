...
* For {ocp} and {product-rosa-hcp}: verify that the `user-workload` Alertmanager instance has started:
+
[source,terminal]
----
...
* For {product-rosa} and {product-dedicated}: verify that the `alert-manager-user-workload` pods are running:
+
[source,terminal]
----
