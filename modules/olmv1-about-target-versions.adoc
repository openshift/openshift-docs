...
[IMPORTANT]
====
Because {product-title} environments operate under a restricted administrative delegation model, the service account specified in the `serviceAccount.name` field cannot possess or request privileges that exceed the scope of the `dedicated-admin` role. The extension installation will fail at the validation phase if the designated service account attempts to grant restricted cluster-wide permissions or interact with Red{nbsp}Hat SRE-managed system configurations.
====
endif::[]
