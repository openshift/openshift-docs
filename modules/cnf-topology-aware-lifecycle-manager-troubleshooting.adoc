...
By default, this `Placement` CR includes the necessary tolerations. 
This ensures proper {cgu-operator} behavior. 
+
The expected `spec.tolerations` settings in the `Placement` CR are:
+
[source,terminal]
----
  tolerations:
...
----
+
If you use a custom `Placement` CR instead of the one generated by the {rh-rhacm} `PolicyGenerator`, you should include these tolerations in that `Placement` CR.
+
