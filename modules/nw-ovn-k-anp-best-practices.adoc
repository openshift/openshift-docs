...
* You can create ANPs that have the same priority. When creating two ANPs at the same priority make sure they do not apply overlapping rules to the same traffic because there is no guarantee which rule will get applied. Because there is no guarantee which policy will take precedence when overlapping ANPs are created, it is preferable to create ANPs at different priorities so that precedence is well defined.
