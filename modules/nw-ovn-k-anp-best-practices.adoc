...
:navtitle: Design AdminNetworkPolicy
[id="anp-best-practices_{context}"]
= Designing AdminNetworkPolicy

[role="_abstract"]
You can use this reference when you design `AdminNetworkPolicy` and `BaselineAdminNetworkPolicy` resources in {product-title}. It describes priority ranges, actions, selector rules, BANP patterns, and how these policies differ from `NetworkPolicy`.
