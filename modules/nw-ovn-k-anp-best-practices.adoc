...
[role="_abstract"]
To design effective `AdminNetworkPolicy` (ANP) and `BaselineAdminNetworkPolicy` (BANP) resources in {product-title}, you can follow guidance on priority ranges, `Allow` and `Pass` actions, selector overlap, BANP defaults, and differences from `NetworkPolicy`.
