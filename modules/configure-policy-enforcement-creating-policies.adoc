...
[role="_abstract]
You can configure if {product-title-short} should only inform you with a notification when this policy is violated, or inform you and enforce the policy. Policy enforcement is determined by whether or not the admission controller is enabled on the cluster and the lifecycle stage of the policy. 
