...
[IMPORTANT]
====
On an {product-title} cluster that uses the `LoadBalancerService` endpoint publishing strategy, the host network pod on a node cannot communicate with a pod on a different node in the same namespace. This configuration in combination with setting the `allow-from-same-namespace` network policy for your cluster would still result in the same communication limitation.
====
+
