...
Compress all logs::
You can compress all the uncompressed diagnostic logs that are less than 10MB into a `.tar.gz` archive file by setting the shell variable `REDUCE_LOGS=compress_logs` before running the `gather` script.
+
