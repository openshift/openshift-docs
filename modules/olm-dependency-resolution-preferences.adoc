...
Specify the value of `legacy` or `restricted` for the `<security_mode>` variable. If the field is not set, the default value is `legacy`. In a future {product-title} release, it is planned that the default value will be `restricted`.
