...
[role="_abstract"]
After the *Cluster Creator* provides the DNS domain and Identity and Access Management (IAM) roles, create two hosted zones and update the trust policy on the shared virtual private cloud (VPC) IAM roles.
...
* You have the _Ingress Operator Cloud Credentials_ role's Amazon Resource Name (ARN) from the *Cluster Creator*.
