...
* AWS Account B: Contains a VPC (including subnets, route tables, and network connectivity). The EFS filesystem is created in this VPC.

To use AWS EFS across accounts:

. Set up the environment:

.. Configure environment variables by running the following commands in the AWS CLI:
+
[source,terminal]
----
...
+
* If your cluster is deployed as a Hosted Control Plane (HyperShift), set the `NODE_SELECTOR` environment variable to hold the worker node label by running the following command in the AWS CLI:
+
[source,terminal]
----
...
----
+
* For all other {product-title} types, set the `NODE_SELECTOR` environment variable to hold the master node label by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Ensure that your AWS CLI is configured with JSON output format as the default for both accounts by running the following commands in the AWS CLI:
+
[source,terminal]
----
...
.. Switch to your Account B profile by running the following command in the AWS CLI:
+
[source,terminal]
----
export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
...
.. Define the IAM role name for the EFS CSI Driver Operator by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Create the IAM trust policy file by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Create the IAM role for the EFS CSI Driver Operator by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Create the IAM policy document by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Create the IAM policy by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Attach the policy to the role by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Switch to your Account A profile by running the following command in the AWS CLI:
+
[source,terminal]
----
export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_A}
...
.. In AWS Account A, attach the AWS-managed policy "AmazonElasticFileSystemClientFullAccess" to the {product-title} cluster master role by running the following command in the AWS CLI:
+
[source,terminal]
----
...
. Attach the policy to the IAM user to allow role assumption:
+
...
.. Identify the IAM User used by the EFS CSI Driver Operator by running the following command in the AWS CLI:
+
[source,terminal]
----
...
. Attach the policy to the IAM user by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Identify the IAM role name currently used by the EFS CSI Driver Operator by running the following command in the AWS CLI::
+
[source,terminal]
----
...
.. Attach the policy to the IAM role used by the EFS CSI Driver Operator by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Initiate a peering request from Account A to Account B by running the following command in the AWS CLI:
+
[source,terminal]
----
export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_A}
...
.. Accept the peering request from Account B by running the following command in the AWS CLI:
+
[source,terminal]
----
export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
...
.. Retrieve the route table IDs for Account A and add routes to the Account B VPC by running the following command in the AWS CLI:
+
[source,terminal]
----
export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_A}
...
.. Retrieve the route table IDs for Account B and add routes to the Account A VPC by running the following command in the AWS CLI:
+
[source,terminal]
----
export AWS_DEFAULT_PROFILE=${AWS_ACCOUNT_B}
...
.. Configure the VPC security groups for EFS access by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Create a region-wide EFS file system by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Configure region-wide mount targets for EFS by running the following command in the AWS CLI:
+
[source,terminal]
----
...
.. Define custom names for the secret and storage class that you will create in subsequent steps by running the following command in the AWS CLI:
+
[source,terminal]
----
