...
. Perform a single manual rescan by annotating the compliance scan with the `compliance.openshift.io/rescan=` option:
+
[source,terminal]
----
...
. Verify the generated machine configs for the profile:
+
[source,terminal]
----
$ oc get mc
