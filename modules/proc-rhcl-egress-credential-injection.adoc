...
* `metadata.vault_secret`, priority 1, runs after `vault_login`, reads the external credential from Vault from the namespace and service account name of the workload, such as `secret/egress/egress-test/default`. Each workload identity gets its own credential.
...
* HashiCorp Vault is deployed and configured for egress credential injection, including the Kubernetes auth method, an `egress-read` policy, and an `egress-workload` role.
...
kind: AuthPolicy
metadata:
  name: _<httpbin_external_auth>_
...
* Replace the `metadata.namespace` parameter value with the namespace where your egress Gateway object is applied.
