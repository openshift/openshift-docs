...
* HashiCorp Vault is deployed and configured for egress credential injection, including the Kubernetes auth method, an `egress-read` policy, and an `egress-workload` role. For instructions, see xref:proc-rhcl-egress-vault-setup_{context}[Deploy and configure Vault for egress credential injection].
