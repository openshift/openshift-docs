...
spec:
  ...
  memory:
    guest: 128Mi
----
* `spec.cpu.guest` is a required field that defines the number of vCPUs to allocate to the guest.
* `spec.memory.guest` is a required field that defines an amount of memory to allocate to the guest.
