...
Both instance type objects use the same spec.
...
When you create a VM by using an instance type, a `ControllerRevision` custom resource (CR) retains an immutable, snapshot copy of the instance type spec. This snapshot locks in resource-related characteristics defined in the instance type spec, such as the required guest CPU and memory. A reference to the `ControllerRevision` CR is contained in the VM spec.

This snapshot is essential for versioning, ensuring that the VM instance created when starting a VM does not change if the underlying instance type CR is updated during the VM's lifetime.
