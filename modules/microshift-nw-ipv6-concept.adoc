...
* The multus plug-in can enable IPv6 for pods, but if you require cluster-wide IPv6 access, such as for the control plane and other services, use the following configuration examples.
* For dual-stack networking, each {microshift-service} cluster network and service network supports up to two values in the cluster and service network configuration parameters.
