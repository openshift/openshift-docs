...
:navtitle: Mirrore the {product-title} image repository for a disconnected registry
...
[role="_abstract"]
Mirror the {product-title} image repository to a local registry to provide your disconnected environment with the container images.
...
----
+
For `<release_version>`, specify the tag that corresponds to the version of {product-title} to install, such as `4.5.4`.
