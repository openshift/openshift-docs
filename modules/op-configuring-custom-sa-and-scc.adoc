...
[IMPORTANT]
====
At this time, enabling the `allowPrivilegeEscalation` setting is required for Buildah to run successfully in the container. With this setting, Buildah can use `SETUID` and `SETGID` capabilities when running as a non-root user.
====
...
<3> At this time, enabling the `allowPrivilegeEscalation` setting is required for Buildah to run successfully in the container. With this setting, Buildah can use `SETUID` and `SETGID` capabilities when running as a non-root user.
