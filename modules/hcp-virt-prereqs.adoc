...
= Prerequisites to deploy {hcp} on {VirtProductName}

[role="_abstract"]
To create an {product-title} cluster on {VirtProductName}, you must meet several prerequisites.

* You have administrator access to an {product-title} cluster, version 4.14 or later, specified in the `KUBECONFIG` environment variable. For {ibm-z-title} (`s390x`), the cluster version must be 4.22 or later.
...
* The {product-title} management cluster has {VirtProductName}, version 4.14 or later, installed on it. For {ibm-z-title} (`s390x`), {VirtProductName} version 4.22 or later is required. For more information, see "Installing OpenShift Virtualization using the web console".
...
* On the {product-title} cluster that hosts the {VirtProductName} virtual machines, you must use a `ReadWriteMany` (RWX) storage class so that live migration can be enabled.
