...
[role="_abstract"]
If you no longer need a {product-title} cluster, you can delete it to stop incurring costs. After deleting the cluster, you must also remove the cluster-specific Operator roles and OpenID Connect (OIDC) provider to avoid leaving unused IAM resources in your AWS account.
...
[IMPORTANT]
====
After the cluster is deleted, you need the cluster ID to delete the cluster-specific {sts-first} resources using the {rosa-cli}.
====
--
...
.Troubleshooting
* Verify that there are no add-ons for your cluster pending in the link:https://console.redhat.com/openshift[Hybrid Cloud Console].
