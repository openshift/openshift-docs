...
The Vertical Pod Autoscaler Operator (VPA) contains an optional `InPlaceOrRecreate` mode. This mode instructs VPA to attempt to perform an _in-place pod resize_ when updating CPU or memory resources, where the VPA first attempts to update the resources in a running pod without recreating the pod. If an in-place resize is not possible, the VPA falls back to `Recreate` mode and restarts the pod in order to update the resources.

The VPA will fallback to recreating pods in `InPlaceOrRecreate` mode in the following scenarios:
