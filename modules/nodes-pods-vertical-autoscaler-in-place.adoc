...
The Vertical Pod Autoscaler Operator (VPA) contains an optional `InPlaceOrRecreate` mode. This mode instructs VPA to attempt to perform an _in-place pod resize_ when updating CPU or memory resources, where the VPA first attempts to update the resources in a running pod without restarting the container. If an in-place resize is not possible, the VPA applies the changes, then recreate the entire pod. 

The VPA needs to recreate a pod in the following scenarios:
