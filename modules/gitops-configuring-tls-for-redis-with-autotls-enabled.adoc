...
[role="_abstract"]
You can configure TLS encryption for Redis by enabling the `autotls` setting on a new or already existing Argo CD instance. The configuration automatically provisions the `argocd-operator-redis-tls` secret and does not require further steps. Currently, {OCP} is the only supported secret provider.
...
`spec.redis.autotls`:: Enables automatic TLS certificate generation for Redis. Set to `openshift` to use the OpenShift service CA.
`spec.ha.enabled`:: Enables the HA feature. Omit this field or set it to `false` to disable HA.
--
+
[TIP]
====
