...
:navtitle: Protect control plane with admission plugins
[id="security-platform-admission_{context}"]
= Protecting control plane with admission plugins

[role="_abstract"]
Where RBAC controls access rules between users and groups and available projects, you can define access to the {product-title} master API by using _admission plugins_ .
