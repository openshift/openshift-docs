...
= Troubleshooting failure to fetch the console URL

The installation program retrieves the URL for the {product-title} console using `[route][route-object]` within the `openshift-console` namespace. If the installation program fails the retrieve the URL for the console, use the following procedure.

.Procedure

. Check if the console router is in the `Available` or `Failing` state by executing the following command:
+
[source,terminal]
----
