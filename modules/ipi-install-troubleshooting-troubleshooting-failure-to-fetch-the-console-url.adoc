...
:navtitle: Troubleshoot a failure to fetch the console URL
[id="troubleshooting-failure-to-fetch-the-console-url_{context}"]
= Troubleshooting a failure to fetch the console URL

[role="_abstract"]
The installation program retrieves the URL for the {product-title} console by using `[route][route-object]` within the `openshift-console` namespace. 
