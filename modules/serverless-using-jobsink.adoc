...
. Trigger a `JobSink`. `JobSink` can be triggered by any xref:../event-sources/knative-event-sources.adoc#knative-event-sources[event source] or xref:../triggers/serverless-triggers.adoc#serverless-triggers[trigger].
+
[source,terminal]
----
