...
* With this update, the {gitops-shortname} Operator has improved tenant isolation by using a separate service account per tenant for the sync operation, instead of using the same service account for both sync and control plane operations. This feature is in alpha state and disabled by default. For more information, see link:https://argo-cd.readthedocs.io/en/latest/proposals/decouple-application-sync-user-using-impersonation/#decouple-application-sync-using-impersonation[Decouple Application Sync using Impersonation]. link:https://issues.redhat.com/browse/GITOPS-3501[GITOPS-3501]
