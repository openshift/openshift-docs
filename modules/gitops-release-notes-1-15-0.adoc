...
where:
+
`volume`:: Specifies the name of the `volume` for the Argo CD `ApplicationSet` Controller.
`volume`:: Specifies the `volume` type. It can be any Kubernetes supported volume type, such as `emptyDir`, `hostPath`, `configMap`, and so on.
`volume`:: Specifies the name of the volume to be mounted, which must match the name of the `volume` defined in volumes.
`name`:: Specifies the path inside the container where the volume will be mounted.
...
----
+
Set this value to `true` to configure the `enableRolloutsUI` field.
