...
You can add 4 or 5 control plane nodes to your cluster and then verify the healthiness of the nodes. The example in the procedure uses `node-5` as the new control plane node. 

[IMPORTANT]
====
Red{nbsp}Hat supports only adding 4 or 5 control plane nodes to a cluster that runs on bare-metal infrastructure.
====
