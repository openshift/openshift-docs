...
[role="_abstract"]
Use {pac} with GitHub Webhook if you cannot create a GitHub App. However, GitHub Webhook does not provide access to the GitHub Check Runs API. {pac} reports task status as pull request comments instead of the *Checks* tab.
...
. Confirm that the webhook is displayed in the list with the {pac} controller URL.
...
* link:https://docs.github.com/en/developers/webhooks-and-events/webhooks/about-webhooks[About webhooks^]