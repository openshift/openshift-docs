...
As a cluster administator, you can configure the Cluster Network Operator to send network flows metadata about the pod network to a network flows collector.
...
. Configure the Cluster Network Operator with the network flows collectors:
+
[source,terminal]
----
...
Verification is not typically necessary. You can run the following command to confirm that Open vSwitch on each node is configured to send network flows records to one or more collectors.
...
. View the network flows configuration in Open vSwitch from each node:
+
[source,terminal]
----
