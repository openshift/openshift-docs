...
|Verifies the permission to list volumes in the default datastore. You must enable the permission to create volumes. The Operator verifies the permission by listing the `/` and `/kubevols` directories. When the Operator performs the check, the root directory must exist but the `/kubevols` directory must not exist. The creation of the `/kubevols` directory occurs when the datastore along with dynamic provisioning.
