...
If you use an external database, you cannot use the automatic backup option or start a back up from within {rh-rhacs-first}.

[IMPORTANT]
====
* When you upgrade your external database, you must scale down Central beforehand. Central attempts to connect to the database until it is successful, which can cause issues during the upgrade process.

* For a database you manage, you must use the backup procedures that your database vendor recommends.
====
