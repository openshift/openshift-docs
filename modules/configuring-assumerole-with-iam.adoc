...
[role="_abstract"]
Configure AssumeRole with container IAM to allow {product-title-short} to authenticate to AWS services like ECR without storing static credentials in the cluster.
...
+
--
`<role-name>`:: Specifies the role name, which should match with the new role you have created earlier.
--