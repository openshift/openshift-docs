...
----
* The `openshift-region` and `openshift-zone` tag categories are created on the vCenter server.
* Compute clusters are tagged with tags from the `openshift-region` tag category.
* Host groups are tagged with tags from the `openshift-zone` tag category.
* The `Host.Inventory.EditCluster` privilege is granted on the vSphere vCenter Cluster object.
...
+
.. Run the following command to copy your existing infrastructure settings to a file. 
+
[source,terminal]
----
oc get infrastructures.config.openshift.io cluster -o yaml > infra.yaml
----

.. Edit your infrastructure file to include a failure domain for each host group in your vSphere cluster. Refer to the example below: 
+
[source,yaml]
----
apiVersion: config.openshift.io/v1                                                                                                                             
...
----
+
. Update your `ControlPlaneMachineSet` with the new failure domains.
...
.. Edit `failureDomains:` as shown in the example below: 
+
[source,yaml]
----
spec:                           
...
[IMPORTANT]
====
After you have edited your `ControlPlaneMachineSet`, verify that your control plane nodes have finished updating before proceeding further. 
...
+
.. Run the following command to retrieve the configuration of an existing `MachineSet` for use as a template. 
+
[source,terminal]
----
...
.. Copy the template as needed to create .yaml `MachineSet` files for each failure domain you defined in your `infra.yaml` file. Refer to the example below: 
+
[source,yaml]
----
...
----
+
.. Run the following command for each `MachineSet` .yaml file: 
+
[source,terminal]
----
