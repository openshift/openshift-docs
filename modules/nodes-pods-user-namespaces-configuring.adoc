...
* Log in to an {product-title} cluster as a user configured with the `restricted-v3` or `nested-container` SCC, or as a user from a user group configured with either SCC. Alternatively, you can set the `restricted-v3` or  SCC directly in the workload object.
...
`spec.template.metadata.annotations.openshift.io/required-scc:: Specifies the required SCC if the user or user group is not configured with the required SCC, either `restricted-v3` or `nested-container`. This value is optional if the the user or user group is not configured with the required SCC.
...
[source,terminal]
----
uid=1000(1000) gid=1000(1000) groups=1000(1000),0(root) <1>
----
...
4026532630 user       3   1 1000 /usr/bin/coreutils --coreutils-prog-shebang=sleep /usr/bin/sleep 1000 <1>
----
...
----
        NS TYPE  NPROCS   PID USER       COMMAND
4026531837 user     250     1 root       /usr/lib/systemd/systemd --switched-root --system --deserialize 28
4026532378 user       1   965 root       /usr/sbin/irqbalance
4026532630 user       1 39580 3505521640 /usr/bin/coreutils --coreutils-prog-shebang=sleep /usr/bin/sleep 1000 <1>
----
