...
Also, you can optionally use the `procMount` parameter in a pod specification to configure the `/proc` file system in pods as `unmasked` within a Linux user namespace. Setting `/proc` to `unmasked`, which is generally considered as safe, bypasses the default masking behavior of the container runtime, and should be used only with an SCC that sets `hostUsers` to `false`. Running the `/proc` file system as `unmasked` within a user namespace has benefits that are beyond the scope of this documentation. 
...
<3> Specifies that processes inside the container run with a user that has any UID other than 0.
<4> Optional: Specifies the type of proc mount to use for the containers. The `unmasked` value ensures that a container's `/proc` file system is mounted as read/write by the container process. The default is `Default`.
<5> Specifies the user ID for processes that run inside of the container. This must fall in the range that you set in the namespace object. 
<6> Specifies the group ID for processes that run inside of the containers. This must fall in the range that you set in the namespace object.
