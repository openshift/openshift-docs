...
where:
+
`targetPort`:: Specifies the name of the port used by the application for running inside the container.
`tls`:: Specifies the TLS configuration used to secure the route.
`to.name`:: Specifies the name of the targeted stable service.
`to.weight`:: Specifies the traffic weight for the stable service. This field is automatically modified to stable weight by Route Rollout plugin.
`alternateBackends.name`:: Specifies the name of the targeted canary service.
`alternateBackends.weight`:: Specifies the traffic weight for the canary service. This field is automatically modified to canary weight by Route Rollout plugin.
+
...
`selector`:: Specifies the selector to match pods. Ensure that the contents of the `selector` field are the same as in stable service and `Rollout` custom resource (CR).
+
[IMPORTANT]
====
...
+
`ports`:: Specifies the name of the port used by the application for running inside the container.
`selector`:: Specifies the selector to match pods. Ensure that the contents of the `selector` field are the same as in canary service and `Rollout` CR.
+
[IMPORTANT]
====
...
`routes`:: Specifies the name of the route. This value must match the name of the created `Route` CR.
`steps`:: Specifies the steps for the rollout. This example gradually routes 30%, 60%, and 100% of traffic to the canary version.
