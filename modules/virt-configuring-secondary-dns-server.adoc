...
. Point to the DNS server by using one of the following methods, based on the type of your cluster:

* For a local cluster with private DNS, add the `kubeSecondaryDNSNameServerIP` value and the cluster FQDN to the enterprise DNS server records. For example:
+
[source,terminal]
----
...
* For a cluster that uses a public DNS, update the enterprise DNS server records as follows:
+
[source,terminal]
----
vm.<cluster_name>.<cluster_domain>  NS   ns.vm.<cluster_name>.<cluster_domain>
ns.vm.<cluster_name>.<cluster_domain> A  <kubeSecondaryDNSNameServerIP>
----