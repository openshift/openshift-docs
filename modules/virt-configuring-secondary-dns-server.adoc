...
. Add the external IP address that you previously retrieved to the `networking.kubeSecondaryDNSNameServerIP` field in the enterprise DNS server records. For example:
+
[source,yaml,subs="attributes+"]
----
apiVersion: hco.kubevirt.io/v1
