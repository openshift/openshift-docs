...
[role="_abstract"]
The AWS PrivateLink endpoint in the host VPC has a default security group that restricts access to the cluster's Machine Classless Inter-Domain Routing (CIDR) range. To grant API access from outside the VPC, you must create and attach an additional security group to the PrivateLink endpoint.
