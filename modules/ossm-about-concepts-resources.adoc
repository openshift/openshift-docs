...
To support canary updates of the control plane, {SMProduct} supports multiple {istio} versions. You can set the `version` field in the specification to the desired version, prefixed with `v`. To update, change this field to the new version. You can set the full version or use the `vX.Y-latest` alias to automatically select the latest patch version for a specific minor version. For example, setting `v1.23-latest` ensures the operator maintains the latest patch version of {istio} 1.23.
