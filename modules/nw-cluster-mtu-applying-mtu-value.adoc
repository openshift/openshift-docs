...
+
** If you are specifying the new MTU by applying a custom script and systemd service, apply the `MachineConfig` manifests for your control plane and worker nodes by entering the following command:
+
[source,terminal]
----
...
----
+
Applying these manifests deploys the `/usr/local/bin/update_mtu.sh` script and enables the `update-mtu.service` systemd unit on each node. The Machine Config Operator automatically performs a rolling reboot of the nodes in your cluster.
+
...
--
+
If the machine config is successfully deployed, the previous output contains the `/usr/local/bin/update_mtu.sh` file path and the ExecStart=/usr/local/bin/update_mtu.sh line from the systemd unit configuration.
