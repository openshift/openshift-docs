// Module included in the following assemblies:
//
// *install-guide/install-guide.adoc

:_mod-docs-content-type: PROCEDURE
[id="proc-install-on-ocp-cmd-cio_{context}"]
= Install {prodname} on {ocp} from the CLI

[role="_abstract"]
You can install {prodname} with {oc-first} using the {ocp} Cluster Ingress Operator as the default `Gateway` object controller. You must complete these steps on each {ocp} cluster that you want to use {prodname} on.

[WARNING]
====
{prodname} uses labels formatted as `kuadrant.io/*` to search and filter resources on the cluster. Removing any labels with the prefix might cause unexpected behavior and degradation of {prodname}.
====

.Prerequisites

* You are logged into {ocp} as a cluster administrator.
* You are using a supported configuration of {ocp} and required components.
* You installed the {oc-first}.

.Procedure

. Create the namespace where you want to install {prodname} by running the following command:
+
[source,terminal,subs="+quotes"]
----
$ oc create ns _<kuadrant_system>_
----
+
You can replace the default `_<kuadrant_system>_` with the namespace you want to use.

. Create the `Subscription` custom resource (CR) by using the following example:
+
[source,yaml,subs="+quotes"]
----
apiVersion: operators.coreos.com/v1alpha1
kind: Subscription
metadata:
  name: rhcl-operator
  namespace: _<kuadrant_system>_
spec:
  channel: stable
  installPlanApproval: Automatic
  name: rhcl-operator
  source: redhat-operators
  sourceNamespace: openshift-marketplace
----
+
Replace `_<kuadrant_system>_` with the namespace you created to install {prodname}.
+
[IMPORTANT]
====
For disconnected installations, you must replace the `spec.source` parameter value of the `Subscription` object with the name of the `CatalogSource` object created by `oc-mirror`. You can find the catalog source name by running the following command:

[source,terminal]
----
$ oc get catalogsource -n openshift-marketplace
----
====

. Apply the `Subscription` CR by running the following command:
+
[source,terminal]
----
$ oc apply -f subscription.yaml
----

. Create the `OperatorGroup` CR by using the following example:
+
[source,yaml,subs="+quotes"]
----
apiVersion: operators.coreos.com/v1
kind: OperatorGroup
metadata:
  name: kuadrant
  namespace: _<kuadrant_system>_
spec:
  upgradeStrategy: Default
----
+
Replace `_<kuadrant_system>_` with the namespace where you are installing {prodname}.

. Apply the `OperatorGroup` CR by running the following command:
+
[source,terminal]
----
$ oc apply -f operatorgroup.yaml
----

. Confirm that the {prodname} installation has finished by running one of the following commands:
+
[source,terminal]
----
$ oc wait --for=jsonpath={.status.installPlanRef.name} subscription rhcl-operator --timeout=10s
ip=$(oc get subscription rhcl-operator -o=jsonpath={.status.installPlanRef.name})
----
+
[source,terminal]
----
$ oc wait --for=condition=Installed installplan ${ip} --timeout=60s
----
+
Expect the status of `installplan.operators.coreos.com/install-<suffix>` when {prodname} is ready. The name of the install plan has a random suffix, for example, `4rql7`.

. Create your {prodname} CR by using the following example:
+
[source,yaml,subs="+quotes"]
----
apiVersion: kuadrant.io/v1beta1
kind: Kuadrant
metadata:
  name: kuadrant
  namespace: _<kuadrant_system>_
spec: {}
----
+
Replace `_<kuadrant_system>_` with the namespace where you installed {prodname}.

. Optional. If you have a {rhdh} subscription, enable the {rhdh} plugin by using the following example:
+
[source,yaml,subs="+quotes"]
----
apiVersion: kuadrant.io/v1beta1
kind: Kuadrant
metadata:
#...
spec:
  components:
    developerPortal:
      enabled: true
#...
----

. Apply the {prodname} CR by running the following command:
+
[source,terminal]
----
$ oc apply -f kuadrant.yaml
----

.Verification

. Check the status of the {prodname} CR generation by running the following command:
+
[source,terminal,subs="+quotes"]
----
$ oc wait kuadrant/kuadrant --for="condition=Ready=true" -n _<kuadrant_system>_ --timeout=300s
----
+
Replace `_<kuadrant_system>_` with the namespace where you installed {prodname}.
+
.Example output
[source,text]
----
kuadrant.kuadrant.io/kuadrant Ready
----

. Verify that all component Operator pods are running by entering the following command:
+
[source,terminal,subs="+quotes"]
----
$ oc get pods -n _<kuadrant_system>_
----
+
Replace `_<kuadrant_system>_` with the namespace where you installed {prodname}.
+
.Example output
[source,terminal]
----
NAME                                                READY   STATUS    RESTARTS   AGE
authorino-operator-controller-manager-<hash>        2/2     Running   0          5m
dns-operator-controller-manager-<hash>              2/2     Running   0          5m
kuadrant-operator-controller-manager-<hash>         2/2     Running   0          7m
limitador-operator-controller-manager-<hash>        2/2     Running   0          5m
----

. For a disconnected installation, verify that there are no image pull errors by running the following command:
+
[source,terminal,subs="+quotes"]
----
$ oc get events -n _<kuadrant_system>_ --field-selector reason=Failed
----
+
Replace `_<kuadrant_system>_` with the namespace where you installed {prodname}.
+
The output must not show image pull failures.
+
[NOTE]
====
Pod specifications still show original the image references, for example, `registry.redhat.io/redhat/...`, even when images are pulled from your mirror registry. The redirection happens transparently at the CRI-O level through `ImageDigestMirrorSet` or `ImageTagMirrorSet` configuration objects.
====
