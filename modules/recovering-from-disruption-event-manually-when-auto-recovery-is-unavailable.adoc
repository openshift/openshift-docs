<<<<<<< HEAD:modules/installation-manual-recovering-when-auto-recovery-is-unavail.adoc
...
>>>>>>> c2165f20e6 (Added a new module for Fencing Validator script):modules/recovering-from-disruption-event-manually-when-auto-recovery-is-unavailable.adoc
...
. Update fencing secrets:  Refresh the Baseboard Management Console (BMC) credentials if they are wrong or outdated.
...
----
+
The secret has the following data keys:
+
.Data keys
...
----
+
If the cluster recovers after updating the fencing secrets, there is no need to do any further action. If the issue persists, proceed to the next step.
...
+
Reset the resources on your cluster and instruct Pacemaker to start them fresh by running the following command:
+
[source,terminal]
----
...
----
+
If the recovery is successful, there is no need to do any further action. If the issue persists, proceed to the next step.

. Recover from a complete node failure:

.. Power on both control plane nodes.
+
Pacemaker starts automatically and begins the recovery operation when it detects both nodes are online. If the recovery does not start as expected, use the diagnostic commands described in the earlier step to investigate the issue.
...
----
+
If the recovery is successful, there is no need to do any further action. If the issue persists, perform manual recovery as described in the next step.
...
+
If manual recovery is still required and it fails, collect a must-gather and an SOS report, and file a bug.
