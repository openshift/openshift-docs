...
= The {mcpg} audit trail

[role="_abstract"]
You can use an audit trail for compliance and accountability that captures caller identity, tool names, and Model Context Protocol (MCP) session context through the {mcpg}.
...
{mcpg} sets the following routing headers on every request:
