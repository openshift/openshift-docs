// Module included in the following assemblies:
//
// *deployment/deployment.adoc

:_mod-docs-content-type: PROCEDURE
[id="proc-create-grpcroute_{context}"]
= Create a GRPCRoute custom resource

:FeatureName: `GRPCRoute` policy attachment support

include::snippets/technology-preview.adoc[]

[role="_abstract"]
You can implement policy attachments based on qualified service and method names by using `GRPCRoute` policy attachment support. First, you must create a `GRPCRoute` custom resource (CR).

.Prerequisites

* You installed {ocp}.
* You installed {oc-first}.
* You installed the {mcpg} Operator.
* You created a `Gateway` object.

.Procedure

. Create a `GRPCRoute` YAML by using the following example:
+
.Example GRPCRoute CR
[source,yaml,subs="+quotes"]
----
apiVersion: gateway.networking.k8s.io/v1
kind: GRPCRoute
metadata:
  name: _<grpcstore>_
spec:
  parentRefs:
    - name: _<kuadrant_ingressgateway>_
      namespace: _<gateway_system>_
  hostnames: [_<"grpc.example.com">_]
  rules:
    - matches:
        - method:
            service: _<"talker.TalkerService">_
            method: _<"Echo">_
      backendRefs:
        - name: grpcstore
          port: 9000
----
+
See the "GRPCRoute custom resource parameters" reference information for details about parameters.
+
[IMPORTANT]
====
If you include a `spec.rules.matches.method` block, you must specify at least one `service` or `method`. Omitting the method block entirely matches all gRPC traffic.
====

. Apply your `GRPCRoute` CR by running the following command:
+
[source,terminal,subs="+quotes"]
----
$ oc apply -f _<grpcstore_route.yaml>_
----
+
Replace `_<grpcstore_route.yaml>_` with the name of your gRPC YAML.
+
.Example output
[source,text]
----
grpcroute.gateway.networking.k8s.io/grpcstore-route created
----
+
The output indicates that the route to the object exists.

.Next steps

* Create your policies with `targetRef` values that reference your `GRPCRoute` CR.
* If you are migrating `HTTPRoute` CRs to `GRPCRoute` CRs, update existing policies to reference your `GRPCRoute` CR.
