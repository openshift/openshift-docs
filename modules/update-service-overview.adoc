...
For clusters with internet accessibility, Red Hat provides over-the-air updates through an OpenShift Container Platform update service as a hosted service located behind public APIs. However, for clusters in a restricted network, there’s no way to access these public APIs to obtain this information. In order to provide a similar user upgrade experience in a restricted network users can install the OpenShift Update Service locally, i.e. accessible within their disconnected network, as described in the following sections.

The OpenShift Update Service is comprised of an operator and one or more applcation instances.
