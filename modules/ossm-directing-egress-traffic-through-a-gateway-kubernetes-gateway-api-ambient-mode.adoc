...
= Route egress traffic from an ambient mesh with the Gateway API
...
. Create a YAML file named `egress-se.yaml` that defines the `ServiceEntry`, similar to the following example:
+
[source,yaml]
----
