...
ifdef::private[]
<12> You can customize your own outbound routing. Configuring user-defined routing uses any type of outbound connectivity applied to your pre-existing VNET or subnets such as routing or NAT. When enabled, it does not create the objects used by the default `outboundType`, like the public IP and load balancer. User-defined routing for egress requires deploying your cluster to an existing VNet.
endif::private[]
ifdef::vnet[]
