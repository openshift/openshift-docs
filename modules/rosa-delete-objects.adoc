...
.Example
Delete all AWS account roles with the prefix of `mycluster`.
...
|--cluster
|Required: The name or ID (string) of the cluster to delete to the identity provider (IDP).
|===
...
Delete autoscaler configuration for a given cluster. 
...
.Example
Delete the autoscaler on a cluster named "mycluster".
...
.Examples
Delete an ocm-role with `arn:aws:iam::123456789012:role/xxx-OCM-Role-1223456778` arn.
...
Deletes the OIDC config based on registered OIDC Config ID.
...
.Examples
Delete an OIDC config with an ID of `A1B2C3D4`.
...
Deletes the OIDC provider of deleted STS cluster.
...
.Examples
* Delete the OIDC provider using the OIDC config ID of `A1B2C3D4`.
+
[source,terminal]
----
...
* Delete the OIDC provider using the cluster name of `mycluster`.
+
[source,terminal]
----
...
Deletes Operator roles of deleted STS cluster.
...
Delete the operator-roles on the cluster named `mycluster`.
...
Delete a specified tuning configuration that is on a specified cluster.
...
Delete the tuning config named `tuned1` from a cluster named `mycluster`.
...
.Example
Delete the user role that has a prefix of `rh-user` and a user-role name of `Auditor`.
