...
. Create a YAML file named `ossm-3.yaml` that creates the {istio} resource for the 3.2 installation in the same namespace as the `ServiceMeshControlPlane` resource for the 2.6 installation.
+
[NOTE]
====
...
* `spec.namespace` The 3.2 and 2.6 control planes must run in the same namespace.
