...
[role="_abstract"]
To allocate dedicated processing resources to your workloads, configure the CPU Manager. Applying a KubeletConfig custom resource (CR) to your nodes ensures optimal performance for latency-sensitive applications.
...
----
+
** `cpuManagerPolicy`: Define after you create the `KubeletConfig` CR.
** `cpuManagerReconcilePeriod`: Define after  you create the `KubeletConfig` CR.
