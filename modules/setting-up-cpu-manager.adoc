...
metadata:
  ...
  generation: 3
  labels:
    custom-kubelet: cpumanager-enabled <1>
----
<1> Add the `custom-kubelet`, if there is no label set already.
...
[NOTE]
====
In `matchLabels`, the `custom-kubelet` label should not match the worker MCP `custom-kubelet`.
====
