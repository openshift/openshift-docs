...
[NOTE]
====
Provisioning new bare-metal hosts to the cluster using the BMO sets the `spec.externallyProvisioned` specification in the `BareMetalHost` custom resource to `false` by default. Do not set the `spec.externallyProvisioned` specification to `true`, as this can result in unexpected behavior.
====
...
----
+
<1> Replace `<num>` with a unique worker number for the bare-metal nodes in the `name` and `credentialsName` fields.
...
<num>:: Specifies the worker node number.
