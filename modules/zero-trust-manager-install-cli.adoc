...
[role="_abstract"]
Install the {zero-trust-full} by using the command-line interface (CLI) to create the required project, `OperatorGroup`, and `Subscription` objects. This allows you to deploy the Operator components necessary for managing workload identities on your {product-title} cluster.
...
+
The following is an exampe of an `operatorGroup.yaml` file.
+
[source, yaml]
----
