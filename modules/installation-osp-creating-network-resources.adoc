...
. To ensure that you network resources have unique names on the {rh-openstack} deployment, create an environment variable and JSON file that the Ansible playbooks will use:
...
.. Verify that the variable is set in your shell by running the following command:
+
[source,terminal]
----
$ echo $OS_NET_ID
