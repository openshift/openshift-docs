...
----
+
`matchExpressions`:: Add the worker role and your custom role.
`nodeSelector`:: Add the label you added to the node as a `nodeSelector`.
+
[NOTE]
====
...
----
+
`machineconfiguration.openshift.io/role`:: Add the label you added to the node as a `nodeSelector`.
