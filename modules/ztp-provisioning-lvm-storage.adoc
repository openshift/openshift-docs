...
. To configure {lvms} for new managed clusters, add the following YAML to `{rangen-yaml-path}` in the `{common-ranGen}` file:
+
...
In {product-title} {product-version}, you can use the Storage LVMS subscription instead of the LVMO subscription. The LVMS subscription does not require manual overrides in the `{common-ranGen}` file. Add the following YAML to `{rangen-yaml-path}` in the `{common-ranGen}` file to use the Storage LVMS subscription:
...
. Add the `LVMCluster` CR to `{rangen-yaml-path}` in your group configuration file. For example, in the `{group-sno-ranGen}` file, add the following:
+
--
ifeval::["{policy-gen-cr}" == "PolicyGenTemplate"]
