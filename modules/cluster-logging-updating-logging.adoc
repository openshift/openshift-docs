...
* If your internal Elasticsearch instance uses persistent volume claims (PVC), the PVCs must contain a `logging-cluster:elasticsearch` label. After {product-title} 4.4.30, the Elasticsearch operator automaticallt adds the label when it creates the PVC. If you are upgrading from an {product-title} version prior to version 4.4.30, you must manually add the label to the Elasticseach PVCs or the PVCs get removed by the garbage collection process and recreated by the operator during the upgrade. 
+
For example:
+
[source,yaml]
