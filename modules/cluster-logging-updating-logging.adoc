...
* If your internal Elasticsearch instance uses persistent volume claims (PVC), the PVCs must contain a `logging-cluster:elasticsearch` label. After {product-title} 4.4.30, the label is automatically added by the Elasticsearch operator. If you are upgrading from prior to {product-title} 4.4.30, you must manually add the label to the Elasticseach PVCs or the PVCs get garbage collected and recreated during the upgrade. 
+
For example:
+
[source,yaml]
