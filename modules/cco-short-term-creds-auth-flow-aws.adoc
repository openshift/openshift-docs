...
[role="_abstract"]
You should familiarize yourself with the process that the AWS Security Token Service (STS) and the link:https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html[`AssumeRole`] API action perform to allow pods to retrieve access keys that are defined by an IAM role policy.
