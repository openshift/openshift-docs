  ...
  config:
    ...
    systemd:
      units:
        - contents: |
            [Unit]
            Description=Run update-kernel-args script
            After=network-online.target
            Wants=network-online.target

            [Service]
            Type=oneshot
            ExecStart=/usr/local/bin/update-kernel-args.sh
            RemainAfterExit=yes

            [Install]
            WantedBy=multi-user.target
          enabled: true
          name: update-kernel-args.service
    storage:
      files:
        - contents:
            source: data:;base64,<base-64-encoded-content> 
          mode: 493
          path: /usr/local/bin/update-kernel-args.sh}
----
