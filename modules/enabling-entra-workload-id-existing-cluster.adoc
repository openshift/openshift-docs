...
:navtitle: Enable {entra-first} on an existing cluster
...
`--preserve-existing-roles`:: Optional: Specify this flag to ensure that any custom role assignments you define on managed identities are not removed during {product-title} updates.
