...
|`isolated`
|Specifies the CPUs for application container workloads. These CPUs have the lowest latency with no interruptions. The kubelet places `Guaranteed` latency-sensitive workloads that request exclusive CPUs on this set.
|===
