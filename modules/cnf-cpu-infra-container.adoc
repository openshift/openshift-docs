...
|`reserved`
|Specifies the CPUs for cluster and operating system housekeeping duties. System daemons and platform components run on these CPUs. Threads in the `reserved` group are often busy. Guaranteed QoS class pods are not placed on these CPUs. Workload partitioning can further protect them from `Burstable` and `Best-effort` pods as well. Do not run latency-sensitive applications in the `reserved` group.

|`isolated`
|Specifies the CPUs for application container workloads. These CPUs have the lowest latency with no interruptions and can, for example, reach much higher DPDK zero packet loss bandwidth. The kubelet places `Guaranteed` latency-sensitive workloads that request exclusive CPUs on this set.
|===
