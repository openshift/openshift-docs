...
[role="_abstract"]
Partition CPUs into reserved and isolated pools to prevent housekeeping tasks from interfering with latency-sensitive workloads by separating infrastructure and application container processes.
...
----
* `spec.cpu.reserved`: Specifies which CPUs are for infrastructure containers to perform cluster and operating system housekeeping duties.
* `spec.cpu.isolated`: Specifies which CPUs are for application containers to run workloads.
* `spec.nodeSelector`: Optional parameter to specify a node selector to apply the performance profile to specific nodes.
