...
+
The following is an example `OperatorGroup` object for `SingleNamespace` install mode:
+
[source,yaml]
----
...
====
+
The following is an example `Subscription` object:
+
[source,yaml]
----
...
----
+
`<namespace_per_install_mode>`:: For default `AllNamespaces` install mode usage, specify the `openshift-operators` namespace. Alternatively, you can specify a custom global namespace, if you have created one. For `SingleNamespace` install mode usage, specify the relevant single namespace.
`<channel_name>`:: Name of the channel to subscribe to.
`<operator_name>`:: Name of the Operator to subscribe to.
`<catalog_name>`:: Name of the catalog source that provides the Operator.
`<catalog_source_namespace>`:: Namespace of the catalog source. Use `openshift-marketplace` for the default software catalog sources.
`config.env`:: The `env` parameter defines a list of environment variables that must exist in all containers in the pod created by OLM.
`config.envFrom`:: The `envFrom` parameter defines a list of sources to populate environment variables in the container.
`config.volumes`:: The `volumes` parameter defines a list of volumes that must exist on the pod created by OLM.
`config.volumeMounts`:: The `volumeMounts` parameter defines a list of volume mounts that must exist in all containers in the pod created by OLM. If a `volumeMount` references a `volume` that does not exist, OLM fails to deploy the Operator.
`config.tolerations`:: The `tolerations` parameter defines a list of tolerations for the pod created by OLM.
`config.resources`:: The `resources` parameter defines resource constraints for all the containers in the pod created by OLM.
`config.nodeSelector`:: The `nodeSelector` parameter defines a `NodeSelector` for the pod created by OLM.
+
The following is an example `Subscription` object with a specific starting Operator version:
+
[source,yaml]
----
apiVersion: operators.coreos.com/v1alpha1
...
----
+
Set the approval strategy to `Manual` in case your specified version is superseded by a later version in the catalog. This plan prevents an automatic upgrade to a later version and requires manual approval before the starting CSV can complete the installation. Set a specific version of an Operator CSV with the `startingCSV` field.
+
...
+
The following is an example `Subscription` object with manual update approvals:
+
[source,yaml]
----
kind: Subscription
...
+
The following is an example `Subscription` object with {aws-short} {sts-short} variables:
+
[source,yaml]
----
kind: Subscription
...
----
+
Include the role ARN details in the `ROLEARN` environment variable.
+
If the cluster is in {entra-short} mode, include the following fields:
+
The following is an example `Subscription` object with {entra-short} variables:
+
[source,yaml]
----
kind: Subscription
...
----
+
Include the client ID in the `CLIENTID` environment variable, the tenant ID in the `TENANTID` environment variable, and the subscription ID in the `SUBSCRIPTIONID` environment variable.
+
If the cluster is in {gcp-wid-short} mode, include the following fields:
+
The following is an example `Subscription` object with {gcp-wid-short} variables:
+
[source,yaml]
----
kind: Subscription
