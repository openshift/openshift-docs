...
= Troubleshooting restic Backup CR issue that cannot be recreated after bucket is emptied
...
If you create a Restic `Backup` CR for a namespace, empty the object storage bucket, and then recreate the `Backup` CR for the same namespace, the recreated `Backup` CR fails. In this case, the `velero` pod log displays the following error message:
.Sample error
[source,text]
----
