...
= `GatewayClass` object status conditions reference

[role="_abstract"]
To verify that your `GatewayClass` object is valid and ready to provision gateways, review its `status` conditions. A healthy `GatewayClass` object reports a status of `True` for core conditions like `Accepted` and `SupportedVersion`.

.`GatewayClass` object status conditions
...
|`Accepted`
|`True`
|The `GatewayClass` object is valid and the controller has claimed it.
...
|`Accepted`
|`Unknown`
|The `GatewayClass` object is waiting for the controller to process it.
