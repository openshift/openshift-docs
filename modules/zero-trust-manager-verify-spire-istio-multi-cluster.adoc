...
:navtitle: Verify SPIRE integration with Istio on each cluster
[id="zero-trust-manager-verify-spire-istio-multi-cluster_{context}"]
= Verifying SPIRE integration with Istio on each cluster

[role="_abstract"]
Verify that {SMProductName} on Cluster A and Cluster B obtains workload certificates from {spire-full} (SPIRE). This confirms Istio sidecars use SPIRE-issued identities rather than the built-in Istio certificate authority (CA) before you proceed with cross-cluster mesh verification.
