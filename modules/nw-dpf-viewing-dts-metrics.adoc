...
[NOTE]
====
The DTS dashboard renders against the platform Thanos/UWM Prometheus - no Grafana dependency is required for basic metric viewing.
====

**Quick check (ad hoc query):** Go to *Console* -> *Observe* -> *Metrics*, enter a DTS query, and click *Run queries*.
...
**Dashboard access:** Go to *Console* -> *Observe* -> *Dashboards*, then in the *Dashboard* dropdown, select *DOCA DPU Telemetry (DTS)*.
...
Grafana provides richer dashboards with per-DPU dropdown filters and customizable panels.
