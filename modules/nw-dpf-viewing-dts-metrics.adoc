...
. Verify that the DTS `DPUService` is ready on the management cluster.
...
. Deploy the DTS console dashboard for the {product-title} web console:
+
...
[NOTE]
====
The {product-title} web console reads from the cluster Prometheus instance through Thanos and user workload monitoring.
Grafana is not required for basic metric viewing.
====
+
...
|===
+
To view the console dashboard, go to *Observe* -> *Dashboards*, then in the *Dashboard* dropdown menu, select *DOCA DPU Telemetry (DTS)*.
...
After you install the Grafana Operator and Grafana instance, retrieve the route URL:
+
[source,terminal]
----
...
----
+
Open the outputted URL in a browser. Anonymous access provides read-only viewer permissions.
