...
Anonymous access provides read-only viewer permissions.
...
. Go to *Dashboards* and open *DOCA DPU Telemetry (DTS)*.
Use the *DPU (source)* dropdown to focus on a specific DPU or select *All*.
Adjust the time range in the upper-right corner. The dashboard refreshes every 30 seconds.
