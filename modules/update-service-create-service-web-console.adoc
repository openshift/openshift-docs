...
. Enter the local pullspec in the *Graph Data Image* field to the graph-data container image created in xref:modules/update-service-graph-data.adoc#update-service-graph-data_{context}[Creating the OpenShift Update Service graph data container image], for example, `registry.example.com/openshift/graph-data:latest`.

. In the *Releases* field, enter the local registry and repository created to contain the release images in xref:modules/update-service-mirror-release.adoc#update-service-mirror-release_{context}[Mirroring the OpenShift Container Platform image repository], for example, `registry.example.com/ocp4/openshift4-release-images`.
...
+
** Ensure that the *Update Service* application is listed with *Status Condition: ReconcileCompleted*. If an additional CA certificate had to be configured to allow access to the registry the *Update Service* application is listed with *Status Condition: ReconcileCompleted, RegistryCACertFound*.
