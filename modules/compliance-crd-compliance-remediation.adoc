...
metadata:
  ...
  - apiVersion: compliance.openshift.io/v1alpha1
    ...
    uid: <UID>
spec:
  apply: false # Specifies whether to apply the remediation
  object:
    current: # Contains the active remediation payload
    ...
    outdated: {} # Stores older remediation payloads for review
----

* **apply**: `true` indicates the remediation was applied. `false` indicates the remediation was not applied.
* **current**: Includes the definition of the remediation.
* **outdated**: Indicates remediation that was previously parsed from an earlier version of the content. The Compliance Operator still retains the outdated objects to give the administrator a chance to review the new remediation before applying them.

To get all the remediation from a suite, run the following command:
[source,terminal]
----
