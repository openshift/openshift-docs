...
`spec.apply`:: A `true` value specifies the remediation was applied. A `false` value indicates the remediation was not applied.
`spec.object.current`:: Specifies the definition of the remediation.
`spec.object.outdated`:: Specifies remediation that was before parsed from an earlier version of the content. The Compliance Operator still retains the outdated objects to give the administrator a chance to review the new remediations before applying them.
+
