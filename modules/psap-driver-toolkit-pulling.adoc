...
The `driver-toolkit` image is available from the link:https://quay.io/openshift-community/[Container images section of the Red Hat Ecosystem Catalog] and in the {product-title} release payload. The image corresponding to the most recent minor release of {product-title} will be tagged with the version number in the catalog. The image URL for a specific release can be found using the `oc adm` CLI command.
endif::[]
...
ifdef::openshift-origin[]
== Pulling the Driver Toolkit container image from quay.io/openshift-community
endif::[]
...
ifdef::openshift-origin[]
Instructions for pulling the `driver-toolkit` image from `quay.io/openshift-community` with `podman` or in {product-title} can be found on the link:https://catalog.redhat.com/software/containers/openshift4/driver-toolkit-rhel8/604009d6122bd89307e00865?container-tabs=gti[Red Hat Ecosystem Catalog].
endif::[]
ifdef::openshift-enterprise,openshift-rosa,openshift-dedicated[]
...
ifdef::openshift-origin[]
The driver-toolkit image for the latest minor release are tagged with the minor release version on `quay.io/openshift-community`, for example: `quay.io/openshift-community/openshift4/driver-toolkit-rhel8:v{product-version}`.
endif::[]
