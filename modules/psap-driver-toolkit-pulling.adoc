...
[role="_abstract"]
You can pull the `driver-toolkit` image from the Red Hat Ecosystem Catalog or extract its URL from the {product-title} release payload by using the `oc adm` CLI.
...
The driver-toolkit image for the latest minor release is tagged with the minor release version on `registry.redhat.io`, for example: `registry.redhat.io/openshift4/driver-toolkit-rhel8:v{product-version}`.
