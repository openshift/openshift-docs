...
----
+
Where `<target_namespace>` is the namespace where the checkup is to be run. This must be an existing namespace where the `NetworkAttachmentDefinition` object resides.
...
----
+
`data.spec.param.networkAttachmentDefinitionName`:: The name of the `NetworkAttachmentDefinition` object.
`data.spec.param.maxDesiredLatencyMilliseconds`:: Optional: The maximum desired latency, in milliseconds, between the virtual machines. If the measured latency exceeds this value, the checkup fails.
`data.spec.param.sampleDurationSeconds`:: Optional: The duration of the latency check, in seconds.
...
----
+
Where `data.status.result.maxLatencyNanoSec` is the maximum measured latency in nanoseconds.
