...
[IMPORTANT]
====
When the `EgressIP` IP namespace selector matches the label on multiple namespaces, consider the following behaviors:
...
* Packets must jump from a pod within the node to the host node of the `EgressIP` object, and this approach adds a network hop.
====
