...
= Configure OpenTelemetry with Service Mesh
...
[NOTE]
====
You must create the {ocp-short-name} route for the Jaeger UI in the Tempo namespace. You can either manually create it for the `tempo-sample-query-frontend` service, or update the `Tempo` custom resource with `.spec.template.queryFrontend.jaegerQuery.ingress.type: route`.
====