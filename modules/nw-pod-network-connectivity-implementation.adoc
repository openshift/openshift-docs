...
<3> Optional: Specifies a selector for connectivity check source pods. You can use the `nodeSelector` and `tolerations` fields to further specify the `sourceNode` pods. While `nodeSelector`` and `tolerations` are optional for both source and target pods, you do not need to use both.
<4> Optional: Specifies a selector for connectivity check target pods.
You can use the `nodeSelector` and `tolerations` fields to further specify the `targetNode` pods. While `nodeSelector` and `tolerations` are optional for both source and target pods, you do not need to use both.