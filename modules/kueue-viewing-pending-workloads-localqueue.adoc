...
[role=”_abstract”]
To view the pending workloads submitted by a specific tenant within their namespace, users can query the LocalQueue visibility endpoint of Kueue's visibility API. This provides an ordered list of their jobs waiting in that queue. 
...
+
limit <integer>:: 1000 (default). Indicates the maximum number of pending workloads that should be fetched.
+
offset <integer>:: 0 (default). Indicates the position of the first pending workload that should be fetched, starting from 0.
