...
[NOTE]
====
For {product-title} version {product-version}, {op-system} is based on {op-system-base} version 9.8, which has the micro-architecture requirements. The following list contains the minimum instruction set architectures (ISA) that each architecture requires:
...
For more information, see the KCS article link:https://access.redhat.com/articles/7074533[Memory Ballooning and OpenShift].
====
endif::vsphere[]
