...
* The Bookinfo sample application cannot be installed on {iibm-cloud-bm-reg}, {ibm-z-reg}, and {ibm-linux-one-reg}.

* WebAssembly extensions are not supported on {iibm-cloud-bm-reg}, {ibm-z-reg}, and {ibm-linux-one-reg}.

* LuaJIT is not supported on {iibm-cloud-bm-reg}, {ibm-z-reg}, and {ibm-linux-one-reg}.

* Single stack IPv6 support is not available on {iibm-cloud-bm-reg}, {ibm-z-reg}, and {ibm-linux-one-reg}.
...
* link:https://issues.redhat.com/browse/MAISTRA-2648[MAISTRA-2648] Service mesh extensions are currently not compatible with meshes deployed on {ibm-z-reg}.
endif::openshift-rosa[]
