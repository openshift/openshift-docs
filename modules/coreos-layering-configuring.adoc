...
[IMPORTANT]
====
As soon as you apply an out-of-cluster custom image to your cluster, you effectively _take ownership_ of your custom layered images and those nodes. {product-title} no longer automatically updates any node that uses the custom layered image. You become responsible for maintaining and updating your nodes as appropriate. If you roll back the custom layer, {product-title} resumes automatically updating the node. See the "Updating with a {op-system} custom layered image" for important information about updating nodes that use a custom layered image.
====
...
+
--
`FROM`:: Specifies the {op-system} base image of your cluster.
`RUN`:: Replaces the kernel packages.
--
+
[NOTE]
====
...
`metadata.labels`:: Specifies the machine config pool to deploy the custom layered image.
`spec.osImageURL`:: Specifies the path to the custom layered image in the repository.
