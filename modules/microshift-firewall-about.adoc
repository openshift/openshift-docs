...
[role="_abstract"]
To ensure your pods and services maintain seamless communication, understand how network traffic flows through the firewall in {product-title}. The firewalld service dynamically manages connection requests and must be configured to explicitly allow essential OVN-Kubernetes, CoreDNS, and API server traffic.
