...
[role="_abstract"]
Configure persistent volume size and rotation for raw compliance scan results using the `rawResultStorage` attributes on `ScanSetting` and `ComplianceScan` resources.
