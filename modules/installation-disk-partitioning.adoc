...
<partition_start_offset>:: Specifies the partition start offset in mebibytes. When adding a data partition to the boot disk, a minimum value of 25000 mebibytes is recommended. The root file system is automatically resized to fill all available space up to the specified offset. If no value is specified, or if the specified value is smaller than the recommended minimum, the resulting root file system will be too small, and future reinstalls of {op-system} might overwrite the beginning of the data partition.
<partition_size>:: Specifies the size of the data partition in mebibytes.
<mount_options>:: Specifies mount options. The `prjquota` mount option must be enabled for filesystems used for container storage. For example: `[defaults, prjquota]`.
+
[NOTE]
====
