...
. Mount the Shared Resource Container Storage Interface (CSI) Driver into a pod or any other resource that accepts `csi` volumes. The following example shows a sample configuration.
+
[source,yaml]
----
apiVersion: v1
