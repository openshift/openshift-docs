...
To access a `SharedConfigMap` CR instance from a pod, you grant a given service account role-based access control (RBAC) permissions to use that `SharedConfigMap` CR instance.
...
[NOTE]
====
If you are unable to complete the last two prerequisites, the cluster administrator can grant you the RBAC to enable the service accounts to use the `SharedConfigMap` CR.
====
...
[IMPORTANT]
====
If the value of `sharedConfigMap` attribute is different from the name of `sharedConfigMap` instance, the pod fails to start. A validation check returns an error to the `kubelet` during pod startup.
====