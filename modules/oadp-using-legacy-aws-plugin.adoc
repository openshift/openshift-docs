...
If you are using an {aws-short} S3-compatible backup storage location, you might get a `SignatureDoesNotMatch` error while backing up your application. This is because, some backup storage locations still use the `AWS SDK for Go V1` instead of the newer `AWS SDK for Go V2`. The S3 APIs in older version of the GO SDK cause errors. To solve this issue, you can use the `legacy-aws` Velero plugin in the `DataProtectionApplication` custom resource (CR). The `legacy-aws` Velero plugin uses the older `AWS SDK for Go V1` to interact with {aws-short} S3. In the following use case, you configure the `DataProtectionApplication` CR with the `legacy-aws` Velero plugin and then back up an application.
...
. Verify that the `DataProtectionApplication` CR is created successfully by running the following command. In the example output, you can see the `status` object has `type` field set to `Reconciled`. This means, the `DataProtectionApplication` CR is successfully created.
+
[source,terminal]
----
$ oc get dpa -o yaml
