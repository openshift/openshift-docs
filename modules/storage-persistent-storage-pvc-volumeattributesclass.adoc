...
Volume Attributes Classes in {product-title} is available only with AWS Elastic Block Storage (EBS) and {gcp-full} persistent disk (PD) Container Storage Interface (CSI).
...
* With {gcp-full} PD, volume modification using Volume Attributes Classes is only possible for hyperdisk-balanced disk types.
