...
This validation layer also actively cross-references the operator's requested manifests against Red{nbsp}Hat SRE security policies. If the pre-flight check fails with an `Unauthorized` or `Forbidden` error despite you granting the rules to your service account, it indicates that the extension is attempting to request root `cluster-admin` privileges, modify host-level nodes, or intercept traffic in protected namespaces.
endif::[]
...
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
You can do this by using `ClusterRole` and `ClusterRoleBinding` resource types, provided they do not intersect with Red{nbsp}Hat SRE protected platform baselines.
endif::[]
...
Privilege escalation:: The service account does not have enough permission to create a role or cluster role that the extension needs. When this happens, the preflight check reports the verbs as missing to prevent privilege escalation.
ifndef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
To resolve this issue, grant enough permission to the service account so that it can create the roles.
endif::[]
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
If an extension requests cluster-scoped or infrastructure-level permissions that exceed your delegated `dedicated-admin` boundaries, the preflight check will block the installation. These security baselines are enforced by Red{nbsp}Hat SRE and cannot be bypassed.
endif::[]
