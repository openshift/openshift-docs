...
When you try to install an extension, the Operator Controller performs a dry run of the installation process. This dry run verifies that the specified service account can perform all the actions required to install the extension. This includes creating all the Kubernetes objects in the bundle and the role-based access control (RBAC) for the roles and bindings defined by the bundle.

:FeatureName: The preflight permissions check for cluster extensions
include::snippets/technology-preview.adoc[]

If service account is missing any required RBAC, the preflight check fails before the actual installation proceeds. If the preflight check fails, the Operator Controller reports the errors in the status conditions of the extension and in the logs of the Operator Controller.
...
* RBAC to perform `list` and `watch` actions for the `services` resource in the core API group for the entire cluster
* RBAC to perform `create` actions for `customresourcedefinitions` resources in the `apiextensions.k8s.io` API group for the entire cluster
...
`Namespace`:: Specifies the scope of the required RBAC at the namespace level. An empty namespace value, `""`, indicates that you must scope the permission to the cluster.
...
+
If a resource belongs to a named API group, the report lists the name in between the brackets. For example, the value of `APIGroups:[apiextensions.k8s.io]` indicates the extension requires RBAC to act on resources in the `apiextensions.k8s.io` API group.
...
`Verbs`:: Specifies the actions, or _verbs_, that the service account needs permission to perform. If the report lists several verbs, all of the listed verbs require RBAC.
