...
+
--
`spec.securityContext.runAsNonRoot`:: When `true`, containers cannot run as root to minimize compromise risks.
`spec.securityContext.seccompProfile.type`::  Set to `RuntimeDefault` to use the default seccomp profile, limiting to essential system calls, to reduce risks.
