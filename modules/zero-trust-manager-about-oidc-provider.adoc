...
While SPIRE primarily issues identities for workloads, additional workload-related claims can be embedded into JWT-SVIDs through the configuration of SPIRE, which allows these claims to be included in the token and verified by OIDC-compliant clients.