...
[id="talo-precache-create-cr_{context}"]
== Creating a CR to pre-cache container images
...
----
<1> The `image` field has a container image in digest format to facilitate the authentication of the image.
...
== Creating a ClusterGroupUpgrade CR for pre-caching
