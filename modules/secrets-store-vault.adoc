...
:navtitle: Mount secrets from HashiCorp Vault
[id="secrets-store-vault_{context}"]
= Mounting secrets from HashiCorp Vault

[role="_abstract"]
You can use the {secrets-store-operator} to mount secrets from HashiCorp Vault to a Container Storage Interface (CSI) volume in {product-title}. Using an external secret store protects information that you do not want developers to have and can be more secure than `secret` objects.
