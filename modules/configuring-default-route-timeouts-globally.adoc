...
[source,terminal]
----
$ oc patch subscription.operators.coreos.com serverless-operator -n openshift-serverless --type='merge' -p '{"spec": {"config": {"env": [{"name": "ROUTE_HAPROXY_TIMEOUT", "value": "<new_timeout_value>"}]}}}'
----
+
`ROUTE_HAPROXY_TIMEOUT` is an enviroment managed by the Serverless Operator and by default is set to `600`. This subscription update causes pods to be redeployed in the `openshift-serverless` namespace.
...
. Set the maximum revision timeout as follows in the `KnativeServing` CR:
+
.Example `KnativeServing` CR with `max-revision-timeout-seconds` set to 900 seconds
[source,yaml]
----
...
. Optional: Adjust the load balancer timeout if necessary for your cloud provider:
+
...
. Deploy a Knative service with the desired timeouts less or equal to the maximum set:
+
.Example of a service definition with timeouts set to 800 seconds
[source,yaml]
----
