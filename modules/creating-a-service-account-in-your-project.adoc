...
[role="_abstract"]
You can add a service account to your user-defined project by including an `eks.amazonaws.com/role-arn` annotation in the service account configuration that references the Amazon Resource Name (ARN) for the AWS Identity and Access Management (IAM) role that you want the service account to assume.

AI
[role="_abstract"]
To allow applications in a pod to assume an AWS Identity and Access Management (IAM) role, you can create a service account in your project. Adding a service account configured with your IAM role details enables your workloads to perform specific AWS operations securely.
