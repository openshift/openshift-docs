...
//
// * /serverless/eventing/triggers/triggers-event-delivery-order.adoc

:_mod-docs-content-type: PROCEDURE
[id="config-event-delivery-order-for-triggers_{context}"]
...
* You have installed the {ServerlessOperatorName}, Knative Eventing, and Knative Kafka on the cluster.
...
. Create or change a `Trigger` object and set the `kafka.eventing.knative.dev/delivery.order` annotation using the following example Trigger YAML file::
+
[source,yaml]
----
