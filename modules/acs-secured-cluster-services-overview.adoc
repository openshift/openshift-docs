...
ifeval::["{context}" == "acs-architecture"]
...
[role="_abstract"]
Install the secured cluster services on each cluster that you want to secure by using {title}.
...
* *Scanner V4*: Scanner V4 retrieves and scans images and indexes them. It is the default scanner for {product-title-short} and has the following components:
** *Scanner V4 Indexer*: The Scanner V4 Indexer performs image indexing, formerly known as image analysis. Given an image and registry credentials, the Indexer pulls the image from the registry. The Indexer finds the base operating system, if one exists, and looks for packages. It stores and outputs an index report, which has the findings for the given image.
...
[NOTE]
====
When you install secured cluster services on the same cluster as Central services and install them in the same namespace, secured cluster services do not deploy Scanner V4 components. Instead, we assume that Central services already include a deployment of Scanner V4.
====
