...
== Using CA certificates with velero command aliased for velero deployment

You might want to use the Velero CLI without installing it locally on your system.
...
. If you want to use the Velero CLI without installing it locally you can do so by using an aliased Velero command, by running the following command:
+
[source,terminal]
----
...
. If you want to use CA certificate with this command, you can add a certificate to the Velero deployment by running the following commands:
+
[source,terminal]
----
...
----
+
. If the Velero pod restarts, the `/tmp/your-cacert.txt` file will disappear, and you need to re-create the `/tmp/your-cacert.txt` file by re-running the commands from the previous step.
...
In future versions of OpenShift API for Data Protection (OADP) we might mount the certificate to the Velero pod for your convenience to remove this extra step.