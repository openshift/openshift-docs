...
You can generate a 32-character random string using the command below.
...
When the following options are set to `true`, authentication using a bearer token is enabled for the reporting REST API. Bearer tokens may come from serviceAccounts or users.
...
The `metering-operator` is capable of creating `RoleBindings` for you, granting these permissions by specifying a list of subjects in the `spec.permissions` section. For an example, see the `advanced-auth.yaml` example configuration below.
...
By default, the `reporting-operator` and `metering-operator` `serviceAccounts` both have these permissions, and their tokens can be used for authentication. In this document, most examples use this method.
