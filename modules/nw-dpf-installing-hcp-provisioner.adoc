...
----
<1> The container registry repository for BlueField OpenShift Container Platform layer images. The operator queries this repository for an image tag that matches the OpenShift Container Platform version.
<2> Disables MetalLB configuration even when a virtual IP is specified.
<3> Controls whether the operator creates and manages the `DPUServiceTemplate` resources for OVN-Kubernetes, DTS, and HBN in the `DPUCluster` namespace. This value must be `true`; otherwise DPU provisioning fails because the required `DPUServiceTemplate` resources are missing. This field is deprecated and will be removed in a future release, at which point `DPUServiceTemplate` management is always enabled.
