...
= Configuration fields mapping between Service Mesh 2.6 and 3.2

[role="_abstract"]

Many of the `spec` fields in the {SMProduct} 2.6 `ServiceMeshControlPlane` can be configured in the {SMProduct} 3.2 `Istio` resource.

The following tables offer guidance for configuring your `Istio` resource in {SMProduct} 3.2.
...
[cols="1,1"]
|===
| SMCP 2.6 configuration | Istio 3.2 configuration
...
The mode configurations in the {SMProduct} 2.6 `ServiceMeshControlPlane` resource were:

* Multitenant
* Cluster-wide
* Federation

In {SMProduct} 3.2, the mode is not configured by using a single field in the `Istio` resource.

By default, the {SMProduct} 3.2 control plane has access to all namespaces, which is equal to cluster-wide mode in {SMProduct} 2. For a similar configuration to `MultiTenant` mode in {SMProduct} 2.6 in {SMProduct} 3.2, you must use the `discoverySelectors` field. For more information, see "Deploying multiple service meshes on a single cluster".

[id="profile-configurations_{context}"]
== Profile configurations

The profile configuration options for {SMProduct} 3.2 are:
