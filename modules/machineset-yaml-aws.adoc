...
----
ifndef::infra,edge[]
<zone>:: Specify the infrastructure ID, role node label, and zone.
<role>:: Specify the role node label to add.
endif::infra,edge[]
ifdef::infra[]
<zone>:: Specify the infrastructure ID, `infra` role node label, and zone.
<infra>:: Specify the `infra` role node label.
endif::infra[]
ifdef::edge[]
<zone>:: Specify the infrastructure ID, `edge` role node label, and zone name.
edge:: Specify the `edge` role node label.
endif::edge[]
ami-046fe691f52a953f9:: Specify a valid {op-system-first} Amazon
...
----
ifndef::edge[]
<zone>:: Specify the zone name, for example, `us-east-1a`.
endif::edge[]
ifdef::edge[]
<zone>:: Specify the zone name, for example, `us-east-1-nyc-1a`.
endif::edge[]
<region>:: Specify the region, for example, `us-east-1`.
ifndef::edge[]
<infrastructure_id>-private-<zone>:: Specify the infrastructure ID and zone.
endif::edge[]
ifdef::edge[]
<value_of_PublicSubnetIds>:: The ID of the public subnet that you created in AWS {zone-type}. You created this public subnet ID when you finished the procedure for "Creating a subnet in an AWS zone".
endif::edge[]
<custom_tag_name>:: Optional: Specify custom tag data for your cluster. For example, you might add an admin contact email address by specifying a `name:value` pair of `Email:\admin-email@example.com`.
+
[NOTE]
====
