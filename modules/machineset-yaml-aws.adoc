...
ifdef::edge[`node-role.kubernetes.io/edge: ""`.]
...
ifdef::edge[]
          machine.openshift.io/parent-zone-name: <value_of_ParentZoneName> <10>
          machine.openshift.io/zone-group: <value_of_ZoneGroup> <11>
...
ifdef::edge[]
<2> Specify the infrastructure ID, `edge` role node label, and zone.
...
ifdef::edge[]
<6> Specify the zone, for example, `us-east-1-nyc-1a`.
endif::edge[]
