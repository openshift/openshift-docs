...
ifdef::edge[]
This sample YAML defines a compute machine set that runs in {aws-service-name-single} `{edge-zone-sample}` and creates nodes that are labeled with `node-role.kubernetes.io/edge: ""`.
endif::[]
...
ifdef::edge[]
<6> Specify the zone name, for example, `{edge-zone-sample}`.
endif::edge[]
...
ifdef::edge[]
<8> The ID of the subnet that you created in {edge-feature-full} on the procedure for "Creating a subnet in {edge-feature-full}.
endif::edge[]
ifdef::infra,edge[]
