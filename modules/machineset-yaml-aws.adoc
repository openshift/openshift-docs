...
ifdef::infra[]
`<infrastructure_id-role-zone>`:: Specifies the infrastructure ID, `infra` role node label, and zone.
`machine.openshift.io/cluster-api-machine-type`:: Requires the `infra` role node label.
endif::infra[]
ifdef::edge[]
`<infrastructure_id-role-zone>`:: Specifies the infrastructure ID, `edge` role node label, and zone.
...
ifndef::edge[]
`<infrastructure_id>-private-<zone>`:: Specifues the infrastructure ID and zone, with the string `-private-` between them.
endif::edge[]
ifdef::edge[]
