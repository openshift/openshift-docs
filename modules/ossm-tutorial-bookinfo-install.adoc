...
. Log in to the {product-title} web console as a user with cluster-admin rights. If you are using {product-dedicated}, you must have an account with the `dedicated-admin` role.
...
[NOTE]
====
You need `cluster-admin` privileges to edit the Istio Service Mesh Member Roll. If you are using {product-dedicated}, you must have an account with the `dedicated-admin` role.
====
+
