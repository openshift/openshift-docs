...
* *Unclear error message for IP address exhaustion*: When a user-defined network's subnet runs out of available IP addresses, new pods fail to start. The pod's error message, `Warning: Failed to create pod sandbox` does not clearly specify that IP depletion is the cause. To confirm the issue, check the *Events* in the pod's namespace, where an explicit message about subnet exhaustion is reported.