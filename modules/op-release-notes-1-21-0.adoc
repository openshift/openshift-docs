...
With this update, a `displayName` field is added to `Step` objects.
//+
//link:https://issues.redhat.com/browse/SRVKP-9135[SRVKP-9135]

Pipelines can execute embedded Pipelines::
With this update, pipelines can be executed using the `PipelineSpec` field under tasks.
//+
//link:https://issues.redhat.com/browse/SRVKP-9135[SRVKP-9135]
...
With this update, the {tekton-results} component automatically creates an OpenShift `Route` CRD for its API endpoint. Users can optionally configure a custom host and path for the `Route`. This helps  ensure the {tekton-results} API is accessible externally without requiring additional user configuration.
//+
...
The Pipeline Overview page persists user selections for the *Namespace*, *Time Range*, and *Refresh Interval* filters. These selections are stored in the application state and URL query parameters. As a result, users have a consistent experience when navigating away, returning to the page, or refreshing the browser. The filters reset only when the user switches namespaces.
//+
...
AI integration for automated pull request analysis::
...
With this update, the `PipelineRun` and `TaskRun` resources include a `spec.managedBy` configuration field. You can use this field to delegate responsibility for managing the lifecycle of the run to an external controller or operator.
+
...
Unclear error messages for the cel subcommand::
When using the `opc pac cel` subcommand, providing no values for the `--body` or `--header` flags results in unclear error messages. This behavior makes it difficult to diagnose configuration issues when testing Common Expression Language (CEL) expressions.
+
To work around this problem, ensure that both the `--body` and `--header` flags are correctly specified with valid file paths when running the subcommand.
+
link:https://issues.redhat.com/browse/SRVKP-9400[SRVKP-9400]


The cel subcommand panics with invalid GitLab data::
The `cel` subcommand panics when invalid GitLab headers and body values are provided. This prevents the command from exiting gracefully during webhook evaluation.
+
To work around this problem, validate the formatting of your GitLab webhook JSON payload and headers before passing them to the CLI tool.
+
...
//also included in 1.19.4 RNs
Git resolver correctly trusts OpenShift Proxy custom PKI::
Before this change, a regression caused the Git resolver to no longer use the OpenShift Proxy custom-configured public key infrastructures (PKI). This issue could prevent the Git resolver from resolving references to a self-hosted Git provider. With this update, the OpenShift Proxy full CA bundle is trusted by all components, including the Git resolver, which trusts any certificates configured in the cluster custom PKI.
+
