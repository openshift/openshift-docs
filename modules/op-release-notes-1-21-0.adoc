...
+
While existing pruning mechanisms, such as the default job-based pruner, {pac} `keep-max-run`, and {tekton-results} based `retention`, continue to function, users currently relying on previous pruning approaches are encouraged to adopt the event-driven pruner `tektonpruner` to help ensure smoother performance, more predictable cleanup, and reduced operational overhead.
+
