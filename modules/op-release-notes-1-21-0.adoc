...
+
* You can configure caching using your `TektonConfig` custom resource (CR), where you can set the cache size and adjust the time to live (TTL) value without restarting controllers:
+
[source,yaml]
----
...
+
* Caching mode can be set globally or on the task-level:
+
[source,yaml]
----
#...
...
:FeatureName: Manual Approval Gate
...
* *System-wide maximum limits added for the event-driven pruner configuration*: With this update, the event-driven pruner enforces system-wide maximum limits for configuration fields. Folowing validation helps ensure both global and namespace-specific pruner settings do not exceed defined maximum values, preventing potential resource overuse.
...
Namespace-level pruner configuration updates do not take effect immediately::
Updates to a namespace-level pruner config map do not immediately take effect if no `PipelineRun` or `TaskRun` completion events occur in the namespace. This is because the namespace-level pruner reconciler reacts only to resource completion events and not to the config map change events. As a consequence, pruning continues to use the previous configuration, and the number of retained resources might exceed the newly configured limits.
+
To work around this problem, trigger a `PipelineRun` or `TaskRun` in the affected namespace and allow it to complete. The completion event triggers the reconciler to use the updated pruning configuration.
+
