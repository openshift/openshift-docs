...
│   ├── example-cnf.yaml
│   ├── ran-common.yaml
│   ├── ran-group-du-sno-templated.yaml
│   ├── ran-group-du-sno-validator.yaml
│   └── ns.yaml
├── source-crs
...
. Add the CRs to your common `PolicyGenerator` CR:
+
[source,yaml]
----
...
.. Add the CRs to your site `PolicyGenerator` CR with overrides:
+
[source,yaml]
----
apiVersion: policy.open-cluster-management.io/v1
...
metadata:
    name: example-cnf
placementBindingDefaults:
    name: example-cnf-placement-binding
policyDefaults:
    namespace: ztp-site
    placement:
        labelSelector:
...
policies:
    - name: oadp-config-policy
      ...
      manifests:
        ...
        - path: source-crs/data-protection/OadpSecret.yaml
          patches:
            - data:
                cloud: <your_credentials>
        - path: source-crs/data-protection/OadpDataProtectionApplication.yaml
          patches:
            - spec:
                backupLocations:
                  - velero:
...
`bucket` and `prefix`:: The `bucket` defines the bucket name that is created in S3 backend, for example `ibu`. The `prefix` defines the name of the subdirectory that will be automatically created in the `bucket`. The combination of `bucket` and `prefix` must be unique for each target cluster to avoid interference between them. To ensure a unique storage directory for each target cluster, you can use the {rh-rhacm-title} hub template function, as shown in the example.