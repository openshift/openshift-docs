...
maxRetryOnTimeout: 3
----
* **timeout**: Defined as a duration string, such as `1h30m`. The default value is `30m`. To disable the timeout, set the value to `0s`.
* **maxRetryOnTimeout**: Defines how many times a retry is attempted. The default value is `3`.