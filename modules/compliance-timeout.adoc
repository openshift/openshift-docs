...
:navtitle: Configure ScanSetting timeout
...
`timeout`:: Specifies a duration string, such as `1h30m`. The default value is `30m`. To disable the timeout, set the value to `0s`.
`maxRetryOnTimeout`:: Specifies the `maxRetryOnTimeout` variable defines how many times a retry is attempted. The default value is `3`.
