...
You should only modify the {op-system} image to use an Azure Marketplace image for compute machines. Control plane machines and nodes do not generate subscription costs when using the public RHCOS default image. However, if you assign an Azure Marketplace image to a control plane machine, usage costs will apply. Similarly, infrastructure machines and nodes can use the public RHCOS default image without incurring usage costs, but applying an Azure Marketplace image to them will incur licensing costs.
====
