...
In most regions, the bootstrap and worker machines use `m4.large` machines, which each use 2 vCPUs,
and the master machines use `m4.xlarge` instances, which each use 4 vCPUs. In some regions, including
all regions that do not support these instance types, `m5.large` and `m5.xlarge`
instances are used instead.

Each default cluster requires 20 vCPU, which exceeds a new account's default limit. To deploy
more worker nodes, enable autoscaling, deploy large workloads, or use a
different instance type, you must further increase your account limits to ensure that your cluster
