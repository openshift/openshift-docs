...
`-f`:: Specifies the index to prune.
`-p`:: Specifies a comma-separated list of packages to keep.
`-i`:: Specifies the Operator Registry base image with the tag that matches the target {product-title} cluster major and minor version. Required only for {ibm-power-name} and {ibm-z-name} images. This parameter is optional.
`-t`:: Specifies the custom tag for the new index image being built.
