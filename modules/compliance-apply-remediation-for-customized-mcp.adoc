...
:navtitle: Apply remediation when using customized machine config pools
...
`metadata.labels.pools.operator.machineconfiguration.openshift.io/<machine_config_pool_name>`:: The `labels` field defines the label name to add for the machine config pool (MCP).
