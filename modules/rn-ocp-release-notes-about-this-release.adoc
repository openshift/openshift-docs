...
[id="rn-ocp-release-notes-about-this-release_{context}"]
= About this release

[role="_abstract"]
{product-title} {product-version} release uses link:https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.33.md[Kubernetes 1.33] with CRI-O runtime. New features, changes, and known issues that pertain to {product-title} {product-version} are included in this topic.
