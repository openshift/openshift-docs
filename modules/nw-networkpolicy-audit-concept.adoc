...
[role="_abstract"]
You can enable network policy audit logging in {product-title} by annotating namespaces with the `k8s.ovn.org/acl-logging` key and configuring log destinations for the OVN-Kubernetes plugin.
