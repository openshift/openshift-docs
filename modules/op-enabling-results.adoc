...
. Optional: If you want to use an external PostgreSQL database server to store {tekton-results} information, add the following lines to the file, instead of the values for `db_host` and `db_port` that are listed in the previous step:
+
[source,yaml]
----
