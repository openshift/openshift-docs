...
In addition, to configure SSH authentication in a non-root security context, refer to the `git-clone-and-check` step in the link:https://github.com/tektoncd/pipeline/blob/main/examples/v1/taskruns/authenticating-git-commands.yaml[example for authenticating git commands].
