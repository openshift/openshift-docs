...
//does oc mirror v2 still use a dry run? this is a different procedure from the one in "Creating custom catalogs using the oc-mirror plugin"
[NOTE]
====
...
mirror:
  ...
  - catalog: registry.redhat.io/redhat/redhat-operator-index:v4.17 <1>
    ...
    - name: amq-broker-rhel8
      ...
      - name: 7.11.x
----
<1> Use the value in the `mirror.catalog` catalog image reference for the following `jq` command to get the image digest. In this example, _<registry.redhat.io/redhat/redhat-operator-index:v4.17>_.
