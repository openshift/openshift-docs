...
Use the value in the `mirror.catalog` catalog image reference for the following `jq` command to get the image digest. In this example, _<registry.redhat.io/redhat/redhat-operator-index:v{product-version}>_.
endif::[]
ifdef::openshift-origin[]
Use the value in the `mirror.catalog` catalog image reference for the following `jq` command to get the image digest. In this example, _<quay.io/openshift-community/redhat/redhat-operator-index:v{product-version}>_.
endif::[]
...
----
ifdef::openshift-enterprise,openshift-rosa,openshift-dedicated[]
Use the value in the `mirror.catalog` catalog image reference for the `jq` command to get the image digest. In this example, _<registry.redhat.io/redhat/redhat-operator-index:v{product-version}>_.
endif::[]
ifdef::openshift-origin[]
Use the value in the `mirror.catalog` catalog image reference for the `jq` command to get the image digest. In this example, _<quay.io/openshift-community/redhat/redhat-operator-index:v{product-version}>_.
endif::[]
+
.Example output
[source,text]
...
ifdef::openshift-origin[]
Ensure that you include the non-optional {microshift-short} container images necessary for a disconnected deployment and references for the catalog index in your `containerfile`. For example, source = `<"quay.io/openshift-release-dev/ocp-v4.0-art-dev>` and source = `<"quay.io/openshift-community/redhat/redhat-operator-index>`."
endif::[]
