...
[NOTE]
====
Workload Identity Federation (WIF) is {gcp-full}'s recommended method of authentication for {product-title} installation. It greatly improves a cluster's resilience by using short-lived, least-privilege credentials and eliminates the need for static service account keys.
====
+
...
[NOTE]
====
In a Shared VPC deployment, you can choose one of the following permission models for the host project:

* *Reduced permissions (recommended):* Pre-create the required firewall rules and a custom role with minimal permissions on the host project before creating the cluster. This approach replaces the need for `roles/compute.networkAdmin` and `roles/compute.securityAdmin`. For more information, see xref:../osd_planning/gcp-ccs.adoc#osd-gcp-shared-vpc-firewall-rules_gcp-ccs[Configuring reduced permissions for a Shared VPC host project on {gcp-short}].

* *Broad permissions:* Grant `roles/compute.networkAdmin` and `roles/compute.securityAdmin` on the host project to allow the installer to create the firewall rules automatically.

Additionally, the `roles/dns.admin` role is required on the host project unless you use a managed DNS zone (by selecting a zone from the *DNS Zone* list).
====
+
