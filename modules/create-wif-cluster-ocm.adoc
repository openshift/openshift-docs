...
[IMPORTANT]
====
Private Service Connect (PSC) is supported only with *Install into an existing VPC*.
====
+
[NOTE]
====
Starting with {product-title} version 4.21, you must create the required firewall rules in your {gcp-short} project before creating the cluster. For more information, see _Creating firewall rules for reduced Workload Identity Federation permissions on {gcp-short}_ in the _Additional resources_ section.
====
+
