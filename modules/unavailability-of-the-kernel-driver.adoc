...
** `Attempting to download kernel object from https://sensor.stackrox.svc:443/kernel-objects/2.5.0/collector-ebpf-5.15.82-0-virt.o.gz` indicates the attempt to locate the module first, followed by any efforts to download the driver from Sensor.
** `HTTP Request failed with error code 404` indicates that the node's kernel does not have a kernel driver.
** `Failed to initialize collector kernel components` indicates that as a result of missing a driver, Collector enters the `CrashLoopBackOff` state.
