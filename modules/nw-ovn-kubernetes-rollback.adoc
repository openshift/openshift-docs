...
. To start the migration, set cluster network provider back to OpenShiftSDN by entering the following commands:
+
[source,terminal]
----
...
.. To remove the OVN-Kubernetes custom config, enter the following command:
+
[source,terminal]
----
$ oc patch Network.operator.openshift.io cluster --type='merge' \
