...
| The value for the Mongo server URL to use for `docdb` storage (`MONGO_SERVER_URL`). This URL can include authentication information, so, for production environments, providing it as plain-text configuration might be insecure. Use the alternative `storage.docdb.mongo-server-url-dir` configuration setting for production environments.
|
|
...
| Authentication token for the KMS server (the value of `VAULT_TOKEN`). Providing the token as plain-text configuration might be insecure. Use the alternative `signers.kms.auth.token-dir` configuration setting for production environments.
|
|


| `signers.kms.auth.token-dir`
| The directory where a file named `KMS_AUTH_TOKEN` is located. This file contains the authentication token for the KMS server (the value of `VAULT_TOKEN`).
