...
[role="_abstract"]
To run your single-node edge deployment on a device with no external network connectivity, prepare networking for fully disconnected hosts in {product-title}. This configuration assigns an IP address to the system loopback device, ensuring your deployment maintains essential intra-node communication.
