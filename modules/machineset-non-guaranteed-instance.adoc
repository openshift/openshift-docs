...
Compute Engine sends a best-effort preemption notice to the user indicating that an interruption will occur in up 30 seconds.
...
An ACPI G3 Mechanical Off signal is sent to the operating system after up 30 seconds if the instance is not stopped.
...
* The supply of Spot VMs decreases
* The instance reaches the end of the allotted 24-hour period for Spot VMs
