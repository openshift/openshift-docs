...
`defaultClusterScopedRoleDisabled`:: Specifies the flag value that disables the creation of the default cluster roles for the cluster-scoped instance. If you want the Operator to recreate the default cluster roles and cluster role bindings for the cluster-scoped instance, set the field value to `false`.
+
.Sample output
[source,terminal]
----
