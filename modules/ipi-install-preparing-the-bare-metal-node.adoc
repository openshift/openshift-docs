...
`openshift-worker-<num>-network-config-secret`:: Replace `<num>` for the worker number of the bare metal node in the `name` fields, the `credentialsName` field, and the `preprovisioningNetworkDataName` field.
`nmstate`:: Add the NMState YAML syntax to configure the host interfaces.
`interfaces:: Optionally, if you have configured the network interface with `nmstate`, and you want to disable an interface, set `state: up` with the IP addresses set to `enabled: false` as shown:
+
[source,yaml]
----
---
   ...
   - name: <nic_name>
     ...
     ipv6:
       enabled: false
----
`name`:: Replace `<nic1_name>`, `<ip_address>`, `<dns_ip_address>`, `<next_hop_ip_address>` and `<next_hop_nic1_name>` with appropriate values.
`base64`:: Replace `<base64_of_uid>` and  `<base64_of_pwd>` with the base64 string of the user name and password.
--
//stopped here.
