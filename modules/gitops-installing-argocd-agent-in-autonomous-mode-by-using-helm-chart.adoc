...
[role="_abstract"]
Use the Helm chart from the Argo CD Agent project to install the autonomous-mode Agent component on the spoke (workload) cluster. In *autonomous* mode, the Agent must communicate with the spoke Redis server.
...
`<argocd_name>`:: Specifies the name of the Argo CD instance installed alongside the Agent. This value constructs Redis-related resource names.
