...
[role="_abstract"]
With boot image management enabled, the Machine Config Operator (MCO) manages and updates the {op-system-first} version of the boot image on each node. This means that the MCO updates the boot image whenever you update your cluster. Without boot image management enabled, if your cluster was originally created with an older {product-title} version, the boot image that the MCO would use to create new nodes is an older {op-system-first} version, even if your cluster is at a later {product-title} version. 
...
.Example .coreos-aleph-version.json with an older boot image
[source,terminal]
----
{
# ...
...
////
Not sure I want this example, to avoid versioning issues
.Example .coreos-aleph-version.json with an updated boot image
