...
[role="_abstract"]
With boot image management enabled, the Machine Config Operator (MCO) manages and updates the boot image that is used to scale up new nodes. This means that the MCO updates the boot images whenever you update your cluster. Without boot image management enabled, if your cluster was originally created on an older {product-title} version, the boot image that the MCO would use to create nodes is from that same {product-title} version, even if your cluster is at a later {product-title} version. 
...
For example, with the feature disabled, if your cluster was originally created with {product-title} 4.16, the boot image that the MCO would use to create new nodes is the same 4.16 version, even if your cluster is at a later version.
...
To view the current boot image used in your cluster, you can refer to the {op-system-first} boot image version on a node by viewing the `/sysroot/.coreos-aleph-version.json` file on that node.
...
`<version>` specifies that boot image is the same as the originally-installed {product-title} version, regardless of the current version of the cluster.
...
`<version>` specifies that the boot image has been updated.
