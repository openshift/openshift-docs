...
PTP Telecom Grandmaster (T-GM) on Intel Granite Rapids-D (GNR-D) (Technology Preview)::
+
You can run a Precision Time Protocol (PTP) Telecom Grandmaster (T-GM) on Intel Granite Rapids-D servers so a single Global Navigation Satellite System (GNSS) feed synchronizes timing across onboard Network Acceleration Complex (NAC) ports and optional Carter Flat expansion NICs. 
