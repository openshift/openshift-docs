...
// OSDOCS-18198
OVN-Kubernetes exposes UDN reconciliation and programming metrics::
+
With this release, the OVN-Kubernetes plugin exposes Prometheus metrics that measure user-defined network (UDN) reconciliation and programming performance. You can use these metrics to identify bottlenecks when you create or scale UDNs and to find how long each phase of UDN readiness takes. The following metrics are available from the `ovnkube-cluster-manager` and `ovnkube-controller` metrics endpoints:
+
...
+
// For more information, see xref:../networking/multiple_networks/primary_networks/udn-metrics.adoc#udn-metrics[Metrics for user-defined networks].
