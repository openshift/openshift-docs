...
Configuration of throughput for {aws-full} gp3 root volumes::
+
With this update, you can configure the maximum throughput for gp3 `rootVolume` drives for clusters installed on {aws-first}.
You enable this by setting the `throughputMib` spec for your control plane and compute machine sets.
Valid values are integers, representing the maximum throughput in MiB/s.
The minimum value is 125 MiB/s and the maximum value is 2000 MiB/s.
