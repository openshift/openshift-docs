...
// TELCODOCS-2613
[id="ocp-release-notes-metallb-configuration-status_{context}"]
== MetalLB ConfigurationState resource reports controller and speaker configuration health

With this release, you can verify that the MetalLB controller and speakers have successfully consumed and applied your configuration by using the new `ConfigurationState` custom resource. MetalLB creates one `ConfigurationState` resource for the controller and one for each speaker node. Each resource reports whether the configuration is valid and surfaces error details when validation fails, such as incompatible `IPAddressPool`, `BGPPeer`, `BFDProfile`, `Community`, or `FRRConfiguration` objects.

This improves observability by giving you a single API surface to troubleshoot configuration problems that were previously only visible through logs or FRR errors.
