...
Bare-metal nodes on {vmw-full} clusters are generally available::
+
...
+
Bare-metal nodes on {vmw-short} clusters was introduced in {product-title} 4.21 with Technology Preview status. In {product-title} 4.22, it is now generally available.
+
