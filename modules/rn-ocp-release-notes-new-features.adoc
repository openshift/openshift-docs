...
[id="release-notes-storage-adding-bm-nodes-vsphere_{context}"]
=== Adding bare-metals nodes on vSphere is generally available
{product-title} 4.21 added the ability to add bare-metal nodes to an {product-title} cluster on vSphere as a Technology Preview feature. In {product-title} 4.22, this feature is generally available.
...
For more information, see xref:../storage/container_storage_interface/persistent-storage-csi-gcp-pd.adoc#c3-instance-type-for-bare-metal-and-n4-machine-series[Hyperdisk Balanced High Availability disks overview].
...
For more information, see xref:../storage/persistent_storage_local/persistent-storage-local.adoc#local-storage-operator-symlinks-management[Local Storage Operator symlinks management].
...
For more information about EUSC, see ???
