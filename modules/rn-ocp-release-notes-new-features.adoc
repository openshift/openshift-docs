...
// TELCODOCS-2914
OVS-DPDK CPU reservation (Technology Preview)::
+
With this release, you can reserve CPUs for Open vSwitch Data Plane Development Kit (OVS-DPDK) poll mode driver (PMD) threads by setting `spec.cpu.ovsDpdk` in a `PerformanceProfile` custom resource. The Node Tuning Operator applies multi-layer isolation so that those host-side threads are protected from interference from the operating system, interrupts, and Kubernetes scheduling. This capability is a Technology Preview feature.
+
For more information, see xref:../scalability_and_performance/cnf-reserving-cpus-for-ovs-dpdk.adoc#cnf-reserving-cpus-for-ovs-dpdk[Reserving CPUs for OVS-DPDK].
