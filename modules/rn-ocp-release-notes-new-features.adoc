...
KMS v2 encryption updates (Technology Preview)::
+
With this update, {KMS} encryption for etcd uses an OLM-managed architecture where KMS plugins run as sidecar containers alongside the API server. An OLM Operator manages the plugin image lifecycle and configuration through a `VaultKMSConfig` custom resource. This release also adds support for migrating between KMS providers without an intermediate migration to local encryption, enhanced monitoring through per-node KMS plugin health reporting, and pre-flight validation before applying KMS configuration changes. HashiCorp Vault Enterprise is the only supported KMS provider.
+
For more information, see xref:../etcd/KMS_v2/kms-configuring.adoc#kms-configuring[Configuring KMS encryption].
