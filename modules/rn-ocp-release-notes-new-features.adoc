...
Control plane node replacement with the ClusterInstance custom resource::
+
You can replace a failed control plane node on a bare-metal multi-node cluster that you deployed by using the SiteConfig Operator by editing the `ClusterInstance` custom resource (CR).
You scale in the failed node and then scale out a replacement node by using GitOps.
The cluster must keep an `etcd` quorum of at least two healthy control plane nodes.
You can replace only one control plane node at a time.
+
For more information, see xref:../edge_computing/ztp-advanced-install-ztp.adoc#ztp-advanced-install-ztp[Advanced managed cluster configuration with ClusterInstance resources].
