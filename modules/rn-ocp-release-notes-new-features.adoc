...
+
{product-title} {product-version} supports Gateway API version 1.5. This version of the API includes the CORS feature. CORS lets a browser permit a web application from one origin to access resources from a different origin. Use a CORS filter when a browser-based application needs to request resources from a different origin.
+
For more information about CORS, see xref:../networking/ingress_load_balancing/configuring_gateway_api/routing-http-requests-to-services.html#supported-httproute-filters_routing-http-requests-to-services[Supported HTTPRoute filters].
