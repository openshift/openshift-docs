...
Updated boot images is now supported for clusters the use {aws-short} Marketplace boot images. This feature allows you configure your cluster to update the node boot image whenever you update your cluster. By default, the boot image in your cluster is not updated along with your cluster. 
+
...
+
As a result, workloads can use only the device capacity they need, which improves utilization of accelerators compared to exclusive allocation or pre-defined partitions.
+
For more information, see xref:../nodes/pods/nodes-pods-allocate-dra.adoc#nodes-pods-allocate-dra[Allocating GPUs to pods by using DRA].
...
* Device health status in pod status. With this update, reporting the health of allocated DRA devices in the pod status is available. When you DRA driver reports that a device such as a GPU is unhealthy, the kubelet records that information on each container that uses the device. You can inspect the pod status to see whether an allocated device is `Healthy`, `Unhealthy`, or `Unknown`. If the driver provides extra detail, the status can also include a message that explains the health result.
+
