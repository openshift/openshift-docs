...
O-Cloud Manager for infrastructure and cluster lifecycle management (Technology Preview)::
+
Using the O-Cloud Manager, you can provision and manage bare-metal {product-title} clusters at scale by separating reusable cluster blueprints from site-specific deployment parameters. {product-title} 5.0 introduces the O-Cloud Manager as a Technology Preview feature.
+
Managing large fleets of edge sites requires consistent configuration and coordinated lifecycle operations. With the O-Cloud Manager, you can automate provisioning, hardware selection, scaling, configuration, and upgrades from a hub cluster by using a GitOps workflow or O-RAN Alliance O2 IMS-compatible REST APIs.
+
For more information, see xref:../../infrastructure_lifecycle_management/ocloud-understanding-the-ocloud-manager.adoc#ocloud-understanding-the-ocloud-manager[Infrastructure and cluster lifecycle management with the O-Cloud Manager].
