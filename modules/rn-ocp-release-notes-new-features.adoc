...
+
You can configure Dedicated Host placement in two ways:
+
--
* At installation, you can specify Dedicated Host IDs in the `install-config.yaml` file to place compute nodes on specific Dedicated Hosts.
* After installation, you can create compute machine sets that use dynamic host allocation to place machines on available Dedicated Hosts from a tagged pool.
--
+
For more information, see xref:../installing/installing_aws/installation-config-parameters-aws.adoc#installation-config-parameters-aws[Installation configuration parameters for {aws-short}] and xref:../machine_management/creating_machinesets/creating-machineset-aws.adoc#machineset-creating-dedicated-hosts_creating-machineset-aws[Place machines on Dedicated Hosts by using machine sets].
