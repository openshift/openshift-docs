...
:navtitle: Resolve agent service failures for {hcp} on {ibm-z-title}
[id="agent-service-failure_{context}"]
= Resolving agent service failures for {hcp} on {ibm-z-title}
...
. Replace the base64-encoded content in the `ignitionConfigOverride` with the required `/etc/containers/policy.json` configuration according to your image registries. See the following example:
+
.Example
[source,json]
----
