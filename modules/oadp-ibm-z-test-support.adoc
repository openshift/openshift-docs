...
* {ibm-z-name} running with {product-title} 4.19, and {oadp-version-1-5} was tested successfully against an AWS S3 backup location target. Although the test involved only an AWS S3 target, Red Hat supports running {ibm-z-name} with {product-title} 4.19, and {oadp-version-1-5} against all S3 backup location targets, which are not AWS, as well.