...
`<aws_zone_b>`::
Specifies the subnet name for an additional failure domain, using the infrastructure ID and the {aws-short} availability zone.
`<cluster_id>-private-<aws_zone_b>`::
Specifies the cluster's infrastructure ID and the {aws-short} availability zone for the additional failure domain.
