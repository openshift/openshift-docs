...
:navtitle: Respond to symlinks alerts for the Local Storage Operator
...
.. Go to the `localVolumeDeviceLink.status` field and view its nested fields that are shown in the following table for a list of valid symlink targets, current link (`by-id`), and the generated preferred symlink.
+
...
.. Got to the `localVolumeDeviceLink.spec.policy` field and set its value to one of the following options:
+
