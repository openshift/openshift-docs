...
[role="_abstract"]
The following `ClusterInstance` custom resource (CR) fields control how the SiteConfig Operator adds or removes a node on a {product-title} cluster.
Use these fields when you scale in or scale out a node.
...
| string
| BMC URL that the installation program uses to power and provision the host, for example a Redfish virtual media address.
