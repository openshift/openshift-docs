...
where:
+
`sharding.enabled`:: Specifies the set the `sharding.enabled` parameter to `true` to enable sharding.
`3`:: Specifies the set the number of replicas to the wanted value, for example, `3`.
`round-robin`:: Specifies the set the sharding algorithm to `round-robin`.
`debug`:: Specifies the set the log level to `debug` so that you can verify to which shard each cluster is attached.
