...
. Reapply the `storage-checkup-config` config map by running the following command, using the namespace where the checkup runs as the target namespace:
+
[source,terminal]
----
$ oc apply -f storage_checkup.yaml -n <target_namespace>
----
