...
[role="_abstract"]
Accept a private offer for {product-title} by reviewing available offers and selecting a new offer or upgrading an existing one.

.Procedure
. Open the private offer URL that was provided by the seller. The URL is accessible only by the specific AWS account ID that was specified by the seller.
+
[NOTE]
====
Verify that you are logged in using the AWS account that was specified as the buyer. Attempting to access the offer using another AWS account produces a "Page not found" error message.
====

. On the *Offer selection* page, review *Available offers*. Options can include:
.. A regular private offer that is pre-selected. This type of offer can be accepted only if {product-title} was not activated before using the public offer or another private offer.
+
[caption="Regular private offer"]
image::rosa-regular-private-offer.png[]
+
.. A private offer that was created for an AWS account that previously activated {product-title} using the public offer. The selected private offer is labeled as *Upgrade* and accepting this offer replaces the currently running contract for {product-title}.
+
[caption="Private offer selection screen"]
image::rosa-private-offer-selection-selection-screen.png[]

.. If multiple offers are available, the previously activated public offer is shown together with the newly provided agreement-based offer that is labeled as "Upgrade".
+
[caption="Private offer selection dropdown"]
image::rosa-private-offer-selection-dropdown.png[]

. Verify that your offer configuration is selected and review the offer details.
+
[NOTE]
====
