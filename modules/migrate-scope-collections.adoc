...
[role="_abstract"]
When migrating from RocksDB to PostgreSQL, {product-title-short} automatically converts existing access scopes used in vulnerability reporting to deployment collections.

Database changes in {product-title-short} from `rocksdb` to PostgreSQL became available as a Technology Preview beginning with release 3.74 and became generally available in release 4.0. When you migrate the database from `rocksdb` to PostgreSQL, the system migrates existing access scopes used in vulnerability reporting to collections. You can verify that the migration resulted in the correct configuration for your existing reports by navigating to *Vulnerability Management* -> *Reporting* and viewing the report information.

The migration process creates collection objects for access scopes that you used in report configurations. {product-title-short} generates two or more collections for a single access scope, depending on the complexity of the access scope. The generated collections for a given access scope include the following types:
...
====
* Root collection for the access scope: The system adds this collection to the report configurations. The collection name is in the format of `System-generated root collection for the scope`. This collection does not define any rules, but attaches one or more embedded collections. The combination of these embedded collections results in the same selection of clusters and namespaces as the original access scope.

For access scopes that define cluster or namespace label selectors, {product-title-short} can only migrate those scopes that have the 'IN' operator between the key and values. Access scopes with label selectors that you created by using the {product-title-short} portal used the 'IN' operator by default. Migration of scopes that used the 'NOT_IN', 'EXISTS' and 'NOT_EXISTS' operators is not supported. If the system cannot create a collection for an access scope, it creates log messages during the migration. Log messages have the following format:
...
You can also click the report in *Vulnerability Management* -> *Reporting* to view the report information page. This page has a message if a report needs a collection attached to it.
