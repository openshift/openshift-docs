...
[id="support-collecting-network-trace-10_{context}"]
= Collecting a network trace from an {product-title} node or container in {op-system} 10.x

[role="_abstract"]
You can use a debug pod to capture a network trace in {product-title}. Red Hat Support might request a network packet trace from a specific {product-title} cluster node or from a specific container to help investigate potential network-related {product-title} issues.
...
. Start the `tcpdump` capture for the current node or a specific container on the node.
...
.. Start a Toolbx container by running the following command:
+
[source,terminal]
----
# toolbox create
...
----
+
.. Obtain the node's interface names by running the following command:
+
[source,terminal]
----
# ip ad
...
`/host/var/tmp/my-cluster-node_$(date +%d_%m_%Y-%H_%M_%S-%Z).pcap`:: The `tcpdump` capture file path is outside of the `chroot` environment.

* Optionally, start a `tcpdump` capture for a specific container on the node by running the following commands:

.. Determine the target container ID by running the following commandin:
+
[source,terminal]
----
...
`/host/var/tmp/my-cluster-node-my-container_$(date +%d_%m_%Y-%H_%M_%S-%Z).pcap`:: The `tcpdump` capture file path is outside of the `chroot` environment.
