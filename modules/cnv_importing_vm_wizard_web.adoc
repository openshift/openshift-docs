...
. Click `Download SDKs` to download the VDDK archive file, `VMware-vix-disklib-_version_.x86_64.tar.gz`.
. Save the VDDK archive file on an HTTP-accessible server.
. In the namespace where you will import the virtual machine, update the `url` with the path of the VDDK package and enter the following command:
+
...
spec:
  source:
    http:
      url: "http://_server.example.com_/VMware-vix-disklib-_version_.x86_64.tar.gz"
  contentType: "archive"
  pvc:
    accessModes:
    - ReadWriteMany
