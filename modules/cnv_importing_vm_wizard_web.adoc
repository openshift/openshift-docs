...
[IMPORTANT]
====
Currently, when using local storage, only one virtual machine can be imported into a namespace. To import additional virtual machines into the same namespace, delete the permanent volume claim before importing:

[options="nowrap" subs="+quotes,verbatim"]
----
# oc delete pvc/vddk-pvc
----
