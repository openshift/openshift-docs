...
[IMPORTANT]
====
Currently, when using local storage, only one virtual machine can be imported into a namespace. If you import additional virtual machines into the same namespace, delete the conversion pod first:

[options="nowrap" subs="+quotes,verbatim"]
----
# oc delete pod *_kubevirt-v2v-conversion-<id>_*
----
