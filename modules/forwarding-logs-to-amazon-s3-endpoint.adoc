...
To configure log forwarding to an Amazon S3 endpoint, you must create a `ClusterLogForwarder` custom resource (CR) with an output for an S3 endpoint, and a pipeline that uses the output.
...
<5> Bucket specifies the S3 bucket name where logs will be stored.
...
<7> Specify the custom S3-compatible endpoint URL. If not specified, the default AWS S3 endpoint will be used.
