...
ifndef::sts[]
[role="_abstract"]
You can delete a {product-title} cluster using the {rosa-cli}.
endif::sts[]

ifdef::sts[]
[role="_abstract"]
You can delete a {product-title} with AWS Security Token Service (STS) cluster by using the {rosa-cli} or {cluster-manager-first}.
