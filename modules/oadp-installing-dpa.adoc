...
* You must configure the MCG object storage as a backup location.
...
<8> If you do not specify this value, the default name, `{cloud-credentials}`, is used. If you specify a custom name, the custom name is used for the backup location.
...
<3> Set to `false`, if you want to disable the Restic installation. Restic deploys a daemon set, which means that each worker node has `Restic` pods running. You can configure Restic for backups by adding `spec.defaultVolumesToRestic: true` to the `Backup` CR.
<4> Specify on which nodes Restic is available. By default, Restic runs on all nodes.
<5> Specify the URL of the MCG S3 endpoint.
...
<7> Specify a MCG bucket as the backup storage location. If the bucket is not a dedicated bucket for Velero backups, you must specify a prefix.
