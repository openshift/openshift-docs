...
The Windows containerd container service does not stream its logs to stdout, but instead, logs to the event log for Windows. You can view the containerd event logs to investigate issues you think might be caused by the Windows containerd container service.
