...
<4> Optional: Specifies a config map that contains PEM-encoded trusted certificate authorities (CAs). The files from the config map is loaded by the KEDA operator during start-up and to used by scalers to authenticate TLS-enabled metrics data sources. The default is empty.
