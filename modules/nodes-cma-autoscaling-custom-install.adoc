...
spec:
  ...
  operator:
    ...
    caDirs: /ca-files/ca <4>
    caDirs: /other-ca-files/ca
  metricsServer:
    logLevel: '0' <5>
    auditConfig: <6>
      logFormat: "json"
...
<4> Optional: Specifies one or more directories with CA certificates, which the Custom Metrics Autoscaler Operator uses to encrypt the traffic between the custom metrics autoscaler components. The Operator registers the certificates inside these directories as trusted CAs. You can specify multiple directories by using the parameter multiple times, as shown in the example. The default is `/custom/ca`.
