...
//
// * networking/hardware_networks/sriov-attach-ethernet.adoc
...
* Confirm that the `{rs}` object was created by running the following command:
+
[source,terminal]
----
$ oc get {rs} -n openshift-sriov-network-operator
----
+
.Example output
...
* Confirm that the `NetworkAttachmentDefinition` object that is associated with the `{rs}` object exists by running the following command. Replace `<namespace>` with the `networkNamespace` value you specified in the `{rs}` object.
+
[source,terminal]
----
