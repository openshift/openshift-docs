...
. Create an {product-title} cluster on {gcp-short} with attached disks provisioned with hyperdisk-balanced disks. This can be achieved by provisioning the cluster with compute node types that support hyperdisk-balanced disks, such as the C3 and N4 machine series from {gcp-short}.
...
----
+
* `metadata.name`: Specifies the name for your storage class. In this example, the name is `hyperdisk-sc`.
* `provisioner`: Specifies the GCP CSI provisioner as `pd.csi.storage.gke.io`.
* `parameters.storage-pools`: If using storage pools, specifies a list of specific storage pools that you want to use in the following format: `projects/PROJECT_ID/zones/ZONE/storagePools/STORAGE_POOL_NAME`.
* `parameters.type`: Specifies the disk type as `hyperdisk-balanced`.
+
[NOTE]
====
...
ifndef::openshift-dedicated[]
.. Follow the procedure in "Installing a cluster on GCP with customizations".
+
...
ifndef::openshift-dedicated[]
.. After step 3 in "Incorporating the Cloud Credential Operator utility manifests", copy the following manifests into the manifests directory created by the installation program:
+
...
----
+
`spec.storageClassState` specifies disabling creation of the default {product-title} storage classes.
+
