...
<2> Specify the {gcp-short} CSI provisioner as `pd.csi.storage.gke.io`.
...
[NOTE]
====
If you use storage pools, you must first create a Hyperdisk Storage Pool of the type "Hyperdisk Balanced" in the {gcp-full} console prior to referencing it in the OpenShift Storage Class. The Hyperdisk Storage Pool must be created in the same zone as the compute node supporting Hyperdisk is installed in the cluster configuration. For more information about creating a Hyperdisk Storage Pool, see link:https://cloud.google.com/compute/docs/disks/create-storage-pools#create-pool[Create a Hyperdisk Storage Pool] in the {gcp-full} documentation.
====

endif::openshift-dedicated[]

ifndef::openshift-dedicated[]
. Create a {gcp-short} cluster with attached disks provisioned with hyperdisk-balanced disks.
endif::openshift-dedicated[]
...
ifndef::openshift-dedicated[]
.. Follow the procedure in the _Installing a cluster on {gcp-short} with customizations_ section.
+
