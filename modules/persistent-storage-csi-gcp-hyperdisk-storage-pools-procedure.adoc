...
<2> Specify the {gcp-full} CSI provisioner as `pd.csi.storage.gke.io`.
...
. Create a {gcp-full} cluster with attached disks provisioned with hyperdisk-balanced disks.
endif::openshift-dedicated[]
...
ifndef::openshift-dedicated[]
.. Follow the procedure in the _Installing a cluster on {gcp-full} with customizations_ section.
+
...
<2> `pd.csi.storage.gke.io` specifies {gcp-full} CSI provisioner.
