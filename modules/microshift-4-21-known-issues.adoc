...
The workaround is to create {op-system-base} 9 Content Delivery Network (CDN) repository files under the `/etc/yum.repos.d/` directory by using entitlement certificates, and re-install {microshift-short}. For more information, see xref:../microshift_install_rpm/microshift-install-rpm.adoc#microshift-install-rpm[Installing {microshift-short} from an RPM package].