...
:navtitle: Troubleshoot bootstrap VM issues
...
. About 10 to 15 minutes after triggering the installation program, ensure the bootstrap VM is operational by running the `virsh` command:
+
[source,terminal]
----
$ sudo virsh list
...
. If the bootstrap VM is not running after 10-15 minutes, verify `libvirtd` is running on the system by using the following command:
+
[source,terminal]
----
...
. After you obtain the IP address, log in to the bootstrap VM by running the `ssh` command:
+
[NOTE]
====
