...
. To enable {oadp-short} Self-Service, edit the DPA and set the `nonAdmin.enable` field to `true` as shown in the following example.
+
...
. To verify that the non-admin-controller (NAC) pod is running in the {oadp-short} namespace, run the following command:
+
[source,terminal]
----
