...
<6> Specify the number of GPU devices you want to attach to each virtual machine (VM) in node pools, for example, if you attach 2 GPU devices to 3 node pool replicas, all 3 VMs in the node pool are attached 2 GPU devices. The default count is `1`.
