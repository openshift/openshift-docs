...
[role="_abstract"]
A deployed {product-title} cluster provisions the following {gcp-first} components. For a more detailed listing of all provisioned {gcp-short} components, see the link:https://access.redhat.com/documentation/en-us/openshift_container_platform/[{OCP} documentation].
...
== VPC requirements for installing a new cluster into an existing VPC

You must have at least one Virtual Private Cloud (VPC) network within the {GCP} project where the {product-title} cluster is being installed. The VPC network must include the following subnets within the same region as the cluster:
...
For a list of services that must be enabled on a {gcp-short} Customer Cloud Subscription (CCS) cluster, see the _Required API services_ table.