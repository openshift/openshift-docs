...
Through the Customer Cloud Subscription (CCS) model, you can deploy clusters in an existing AWS or {gcp-full} cloud account that you own.
...
As an alternative to the CCS model, you can deploy {product-title} clusters in AWS or {gcp-full} cloud accounts that are owned by Red Hat. With this model, Red Hat is responsible for the cloud account and the cloud infrastructure costs are paid directly by Red Hat. The customer only pays the Red Hat subscription costs.
