...
.. Run the following command, specifying your {product-title} version:
+
[source,terminal]
----
...
.. Inspect the output to identify the image digest. The following is example output:
+
[source,terminal]
----
{
...
. Create a `NodeFeatureDiscovery` CR, as in the following example:
+
...
+
--
* `<operand_image>` specifies the `operand.image` field, which is mandatory.
--
