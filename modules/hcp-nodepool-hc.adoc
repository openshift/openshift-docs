...
:navtitle: Create a NodePool object for the hosted cluster
...
* `spec.release.image` specifies which {product-title} version the nodes in the `NodePool` object are based on. Replace the `<dns.base.domain.name>` value with your DNS base domain name and the `4.x.y` value with the supported {product-title} version you want to use.
