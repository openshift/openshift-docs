...
[role="_abstract"]
The `BareMetalHost` resource is a custom object that represents a physical server and its properties within a cluster. It contains a specification section to define the desired state of the host and a status section that reports the current hardware details and operational health of the physical machine.
