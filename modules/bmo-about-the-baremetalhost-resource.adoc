...
:navtitle: The `BareMetalHost` resource
...
| `bootMACAddress`
| The MAC address of the network interface controller (NIC) used for provisioning the host.
...
** `name`: The name of the volume as a string. It should be unique within the server. If not specified, the volume name will be autogenerated.
