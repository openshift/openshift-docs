...
[role="_abstract"]
Functions run as Knative services on an {ocp-product-title} cluster. When you create an event source in the {ocp-product-title} web console, specify a deployed function to receive events from that source.

.Prerequisites

* You have installed the {ServerlessOperatorName}, Knative Serving, and Knative Eventing are  on your {ocp-product-title} cluster.
