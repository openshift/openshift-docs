...
[role="_abstract"]
The following table lists the default security policies in {product-title} that are of low severity. The policies organize by lifecycle stage.
...
|Build or Deploy |ADD Command used instead of COPY |Alerts when an image build by using an `ADD` command. |Disabled
...
|Build or Deploy |Docker CIS 4.1: Ensure That a User for the Container Exists |Ensures that containers are running as non-root users. |Enabled
...
|Build or Deploy |Required Image Label |Alerts when a deployment includes images that lack the specified label. |Disabled
