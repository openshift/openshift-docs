...
When this persistence configuration is in place, the {ServerlessLogicOperatorName} configures every workflow deployed in this namespace, with the `preview` or `gitops` profile, to connect with the PostgreSQL database by injecting relevant JDBC connection parameters as environment variables. 
