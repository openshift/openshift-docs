...
The reference DU profile includes the Performance Addon Operator in the `{policy-gen-cr}` CR `{common-ranGen}`. To remove the subscription from deployed managed clusters, you must update `{common-ranGen}`.
...
. Change the `complianceType` to `mustnothave` for the Performance Addon Operator namespace, Operator group, and subscription in the `{common-ranGen}` file.
+
[source,yaml]
----
...
. Delete the Performance Addon Operator namespace, Operator group and subscription CRs from `{rangen-yaml-path}` in the `{common-ranGen}` file.
