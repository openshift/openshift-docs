...
* With this release, the builds controllers use Tekton's V1 API to create and access the `TaskRun` that backs a `BuildRun`.

* With this release, you can now define a build without any source. This enhancement is useful for running builds that use only a local source.

* The output image section includes an optional timestamp field to change the image creation timestamp. Use the `SourceTimestamp` string to match the source timestamp.

* With this release, the `.source.type` field is now required in both `build` and `buildRun` resources.
