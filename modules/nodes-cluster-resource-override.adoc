...
{product-title} administrators can control the level of overcommit and manage container density on nodes by using the Cluster Resource Override Operator. The Operator modifies the ratio between requests and limits set on developer containers. In conjunction with a per-project limit range that specifies limits and defaults, you can achieve the desired level of overcommit.

You must install the Cluster Resource Override Operator using the {product-title} console or CLI as shown in the following sections. After you deploy the Cluster Resource Override Operator, the Operator modifies all new pods in the specified namespace. The Operator does not edit pods that existed before you deploy the operator. 
...
For example, the original request of a pod has the following resources limits:
...
The Cluster Resource Override Operator intercepts the original pod request and overrides the resources according to the configuration set in the `ClusterResourceOverride` object.
...
# ...
----
<1> The CPU limit has been overridden to `1` because the `limitCPUToMemoryPercent` parameter is set to `200` in the `ClusterResourceOverride`. As such, 200% of the memory limit, 512Mi in CPU terms, is 1 CPU core. 
<2> The CPU request is now `250m` because the `cpuRequestToLimit` is set to `25` in the `ClusterResourceOverride`. As such, 25% of the CPU limit=1 core is 250m.
