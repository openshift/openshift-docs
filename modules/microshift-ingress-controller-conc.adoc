...
Default TLS curves::
* By default, the ingress controller advertises TLS key-exchange groups that include the `X25519MLKEM768` hybrid post-quantum group. You cannot set TLS curves in the {microshift-short} configuration file. If the node runs in FIPS mode, {microshift-short} automatically restricts the advertised groups to FIPS-approved NIST curves.
