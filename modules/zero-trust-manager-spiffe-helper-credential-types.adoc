...
[role="_abstract"]
SPIFFE Helper supports the X.509 SVID, JSON Web Token (JWT) bundle, and JWT SVID outputs configured in the SPIFFE Helper configuration file and written under `cert_dir` for workloads that cannot use the Workload API directly.
...
The `cert_dir` directory must exist before SPIFFE Helper starts. Mount an `emptyDir` or persistent volume at the `cert_dir` path so the directory exists when SPIFFE Helper starts.
