...
ifdef::virt-importing-vmware-vm[]
. In the *Connect to Provider* screen, select *VMware* from the *Provider* list.
...
ifdef::virt-importing-rhv-vm[]
. In the *Connect to Provider* screen, select *Red Hat Virtualization (RHV)* from the *Provider* list.
. Select *Connect to New Instance* or a saved RHV instance from the *RHV Instance* list.
...
ifdef::virt-importing-rhv-vm[]
* *General* -> *Name*: The VM name is limited to 23 characters. link:https://bugzilla.redhat.com/show_bug.cgi?id=1857165[(*BZ#1857165*)]
...
** *Size*
** *Interface*
** *Storage Class*: If you do not select a storage class, {VirtProductName} uses the default storage class to create the virtual machine.
