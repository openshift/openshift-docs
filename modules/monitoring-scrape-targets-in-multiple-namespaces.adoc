...
. Verify that the Prometheus instance adds new targets and that the alerts are firing. Use a port-forward command to expose the Prometheus or the Alertmanager user interface that has been deployed by the `Monitoringstack` instance.
+
Prometheus::
+
[source,terminal]
