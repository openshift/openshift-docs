...
:navtitle: Create subnets in Local Zones
[id="installation-creating-aws-vpc-subnets-lz_{context}"]
= Creating subnets in Local Zones

[role="_abstract"]
Before you configure a machine set for edge compute nodes in your {product-title} cluster, you must create the subnets in {zone-type}.
...
* You added your {aws-short} keys and region to your local {aws-short} profile by running `aws configure`.
...
`${ROUTE_TABLE_PUB}`:: Specifies the `PublicRouteTableId` extracted from the output of the VPC's CloudFormation stack.
