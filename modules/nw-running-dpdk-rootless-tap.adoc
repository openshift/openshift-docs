...
+
--
* `test-namespace` specifies the same `target_namespace` where the `SriovNetwork` object is created.
--
...
+
--
* `test-namespace` specifies the same `target_namespace` in which the `SriovNetwork` object is created. If you want to create the pod in a different namespace, change `target_namespace` in both the `Pod` spec and the `SriovNetwork` object.
* `fsGroup` sets the group ownership of volume-mounted directories and files created in those volumes.
* `runAsGroup` specifies the primary group ID used for running the container.
* `<DPDK_image>` specifies the DPDK image that contains your application and the DPDK library used by application.
...
* The `add` section specifies additional capabilities required by the application inside the container for hugepage allocation, system resource allocation, and network interface access. These capabilities must also be set in the binary file by using the `setcap` command.
* Mellanox network interface controller (NIC) requires the `NET_RAW` capability.
* `runAsUser` specifies the user ID used for running the container.
* Setting `privileged` to `false` indicates that the container or containers within the pod should not be granted privileged access to the host system.
* Setting `allowPrivilegeEscalation` to `true` allows a container to escalate its privileges beyond the initial non-root privileges it might have been assigned.
* Setting `runAsNonRoot` to `true` ensures that the container runs with a non-root user. This helps enforce the principle of least privilege, limiting the potential impact of compromising the container and reducing the attack surface.
