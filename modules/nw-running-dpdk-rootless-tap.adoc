...
<14> Specify the number of CPUs. The DPDK pod usually requires exclusive CPUs to be allocated from the kubelet. This is achieved by setting DERPEEDOO Manager policy to `static` and creating a pod with `Guaranteed` QoS.
