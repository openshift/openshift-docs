...
[source,terminal]
----
$ oc get pod dpdk-app -n test-namespace
----
+
.Example output
[source,terminal]
...
. Verify that the SR-IOV and TAP network interfaces are attached to the pod:
+
[source,terminal]
----
$ oc exec dpdk-app -n test-namespace -- ip link show
----
+
.Example output
[source,terminal]
...
2: eth0@if22: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1400 qdisc noqueue state UP mode DEFAULT
3: net1: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc mq state UP mode DEFAULT
4: ext0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UNKNOWN mode DEFAULT
----

. Verify that the `vhost-net` device is mounted in the container:
+
[source,terminal]
----
$ oc exec dpdk-app -n test-namespace -- ls -l /dev/vhost-net
----
+
.Example output
[source,terminal]
----
crw-rw-rw- 1 root root 10, 238 <date> /dev/vhost-net
----