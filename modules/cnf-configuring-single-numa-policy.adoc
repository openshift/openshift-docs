...
worker-cnf   rendered-worker-f11f2ea0550dbc10fd91cdde7c4c93c7   True      False      False      3              3                   3                     0                      101m
----
...
spec:
  machineConfigPoolSelector:
    matchLabels:
      pools.operator.machineconfiguration.openshift.io/worker-cnf: "" <1>
  kubeletConfig:
...
----
<1> This a label to match the created MCP configuration for example `worker-cnf`
<2> Adjust based on the CPU on your nodes
<3> For `cpuManagerPolicy`, `static` must use a lowercase `s`.
