...
...
  ols:
    storage: {} <1>
----
<1> By default, the persistent volume allocated for the PostgreSQL database is 1 GB in size and of the default storage class. Use the empty braces for the `storage` parameter to use the default values.
+
