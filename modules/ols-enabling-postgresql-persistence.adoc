...
* You are logged in to the {ocp-product-title} web console as a user account that has permission to create a cluster-scoped CR file, such as a user with the `cluster-admin` role.
...
...
  ols:
    storage: {} <1>
----
<1> By default, the persistent volume allocated for the PostgreSQL database is 1 GB in size and of the default storage class.
+
If you do not know what values to use, enter empty braces for the `storage` parameter to use the default values.
+
Alternatively, if specific values are known you can enter them for the `size` and `class` parameters. 
+
[source,yaml,subs="attributes,verbatim"]
----
ols:
