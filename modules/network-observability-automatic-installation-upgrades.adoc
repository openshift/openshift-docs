...
[role="_abstract"]
Understand how upgrading to {product-title} 4.22 to 5.0 affects automatic network observability installation when the `NetworkObservabilityInstall` feature gate is enabled.

When you upgrade an existing cluster to {product-title} 4.22 to 5.0 with the `NetworkObservabilityInstall` feature gate enabled, the CNO evaluates the `installationPolicy` field in the Network custom resource (CR) to determine whether to install network observability.
...
When the feature gate is enabled and you upgrade to {product-title} 4.22 to 5.0, the following behaviors apply by default:
