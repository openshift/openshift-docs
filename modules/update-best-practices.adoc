...
[id="mtc-upgrade-restriction_{context}"]
=== {mtc-short} upgrade restriction

{product-title} 4.20 is the final version that supports {mtc-first}. Starting with {mtc-short} version 1.8.13, the {mtc-short} Operator uses the `olm.maxOpenShiftVersion` property to enforce this constraint. This upgrade restriction is intentional and prevents the cluster from running an unsupported version of {mtc-short}.

[IMPORTANT]
====
It is not possible to update a cluster to {product-title} 4.21 or later if the {mtc-short} Operator is installed. You must uninstall the {mtc-short} Operator before you can update the cluster. For information about uninstalling {mtc-short}, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/migration_toolkit_for_containers/installing-mtc#migration-uninstalling-mtc-clean-up_installing-mtc[Uninstalling {mtc-short} and deleting resources].
====
