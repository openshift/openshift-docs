...
----
+
. To monitor the advanced audit log file, it is specified in the `auditLogPath` and written to the file system on the node where the pod is running. To monitor or inspect the audit logs, you must access the node directly and check the file at the specified path such as `/tmp/logs/audit1.log`.
+
