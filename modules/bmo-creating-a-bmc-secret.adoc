...
[role="_abstract"]
Create a `Secret` resource with Base64-encoded BMC credentials to securely store baseboard management controller authentication, enabling automated host provisioning and power management without exposing passwords in `BareMetalHost` resources.
