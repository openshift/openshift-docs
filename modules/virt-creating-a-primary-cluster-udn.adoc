...
= Creating a primary cluster-scoped UserDefinedNetwork (UDN) by using the CLI

You can connect multiple namespaces to the same primary UserDefinedNetwork (UDN) to achieve native tenant isolation by using the CLI.
