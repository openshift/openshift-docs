...
[IMPORTANT]
====
Ensure that *Auto Create Namespace* is selected so that the destination namespace `spring-petclinic` is automatically created when creating the Argo CD application.
====
endif::app[]

. Click *CREATE* to create your application.
+
The `spring-petclinic` namespace is automatically created when you enable *Auto Create Namespace*.
