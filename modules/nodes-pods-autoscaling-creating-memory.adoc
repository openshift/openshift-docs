...
** To scale for a specific memory value, create a `HorizontalPodAutoscaler` object similar to the following for an existing deployment config or replication controller:
+
.Example output
[source,yaml,options="nowrap"]
----
