...
= Install {builds-shortname} by using the web console

[role="_abstract"]
Use the {ocp-product-title} web console to install the {builds-title} components. The web console provides a guided workflow for installing and configuring the Operator.
...
* You have installed the {pipelines-operator}. For more information, see link:https://docs.redhat.com/en/documentation/red_hat_openshift_pipelines/1.15/html-single/installing_and_configuring/index[Installing {pipelines-operator}].

.Procedure

. In the *Administrator* perspective of the web console, navigate to the *Operators* -> *OperatorHub* page.

. Use the *Filter by keyword* field to search for {builds-operator} in the catalog.
...
.. Check that the *Installation Mode* is set to *All namespaces on the cluster (default)*. This mode installs the Operator in the `openshift-builds` namespace to watch all namespaces in the cluster. The `openshift-builds` namespace is created automatically during installation.

.. Check that the *Installed Namespace* is set to `openshift-builds` by default.

.. Select *Automatic* for the *Approval Strategy*. This ensures that the future upgrades to the Operator are handled automatically by the Operator Lifecycle Manager (OLM). If you select the *Manual* approval strategy, OLM creates an update request that a cluster administrator must manually approve.
...
. In the *Administrator* perspective of the web console, select the `openshift-builds` project.

. Go to the *Operators* -> *Installed Operators* page. 

. On the *Installed Operators* page, set the namespace to `All Projects`.

. Verify that the status of {builds-operator} and {pipelines-title} Operator is displayed as *Succeeded Up to date*.
