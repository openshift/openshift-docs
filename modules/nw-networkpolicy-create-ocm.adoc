...
= Create a network policy using {cluster-manager}
...
+
*Allow pods from the same namespace*:: This limits traffic to pods within the same namespace. You can specify the pods in a namespace, but leaving this option blank allows all of the traffic from pods in the namespace.

*Allow pods from inside the cluster*:: This limits traffic to pods within the same cluster as the policy. You can specify namespaces and pods from which you want to allow inbound traffic. Leaving this option blank allows inbound traffic from all namespaces and pods within this cluster.

*Allow peers by IP block*:: This limits traffic from a specified Classless Inter-Domain Routing (CIDR) IP block. You can block certain IPs with the exceptions option. Leaving the CIDR field blank allows all inbound traffic from all external sources.
...
.. Select *Add egress rule* to configure a new rule. This action creates a new *Egress rule* row with an *Add allowed destination* drop-down menu where you specify how to limit outbound traffic. The list offers three options to limit your egress traffic:
+
*Allow pods from the same namespace*:: This limits outbound traffic to pods within the same namespace. You can specify the pods in a namespace, but leaving this option blank allows all of the traffic from pods in the namespace.

*Allow pods from inside the cluster*:: This limits traffic to pods within the same cluster as the policy. You can specify namespaces and pods from which you want to allow outbound traffic. Leaving this option blank allows outbound traffic from all namespaces and pods within this cluster.

*Allow peers by IP block*:: This limits traffic from a specified CIDR IP block. You can block certain IPs with the exceptions option. Leaving the CIDR field blank allows all outbound traffic from all external sources.
