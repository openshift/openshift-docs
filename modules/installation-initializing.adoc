...
ifeval::["{context}" == "installing-rhv-installer-default"]
:rhv:
endif::[]
ifeval::["{context}" == "installing-rhv-installer-customizations"]
:rhv:
endif::[]
...
. To create the `install-config.yaml` file, run the following command:
+
----
