...
= Create an Ingress Controller with global access on {gcp-full}
You can create an Ingress Controller that has global access to a {gcp-full} cluster. Global access is only available to Ingress Controllers using internal load balancers.
...
Create an Ingress Controller with global access on a new {gcp-full} cluster.
