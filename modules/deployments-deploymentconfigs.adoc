...
`type`:: A configuration change trigger results in a new replication controller whenever changes are detected in the pod template of the deployment configuration.
`type`:: An image change trigger causes a new deployment to be created each time a new version of the backing image is available in the named image stream.
`type`:: The default `Rolling` strategy makes a downtime-free transition between deployments.
