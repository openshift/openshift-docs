...
[role="_abstract"]
A `DeploymentConfig` object in {product-title} builds on replication controllers to manage the application deployment lifecycle. You can use it to roll out image updates, run lifecycle hooks, trigger automated deployments, and scale or roll back your application.
...
* `strategy.type.Rolling` is the default strategy that makes a downtime-free transition between deployments.
