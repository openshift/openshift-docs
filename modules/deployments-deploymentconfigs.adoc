...
[role="_abstract"]
You can use `DeploymentConfig` objects in {product-title} to roll out image updates, run lifecycle hooks, trigger automated deployments, and scale or roll back applications. A `DeploymentConfig` object builds on replication controllers to manage the application deployment lifecycle.
