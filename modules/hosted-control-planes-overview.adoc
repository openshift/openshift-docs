...
[role="_abstract"]
{hcp-capital} is available by using a supported version of {mce} on several platforms.

You can deploy {hcp} on the following platforms:
...
[NOTE]
====
The {mce-short} is an integral part of {rh-rhacm-first} and is enabled by default with {rh-rhacm}. However, you do not need {rh-rhacm} to use {hcp}.
====
