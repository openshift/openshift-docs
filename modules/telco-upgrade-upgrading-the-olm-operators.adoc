...
[id="telco-upgrade-upgrading-the-olm-operators_{context}"]
= Upgrading the OLM Operators

In telco clusters all of the software needs to vetted before it is loaded onto a production cluster.
The production clusters are also configured in a disconnected network, which means that they are not always directly connected to the internet.
Due to this requirement, the OpenShift Operators are configured as "manual" upgrade when they are installed, so that new versions can be managed on a cluster by cluster basis.
This extra security does mean that the following procedure will need to be followed to move the Operators to the newer versions.
...
----
+
When the update is complete, the updated namespace resources look like this:
+
[source,terminal]
----
NAME                                                      READY   STATUS    RESTARTS   AGE
...
[NOTE]
====
Sometimes you need to approve an update twice because some Operators have interim z-stream release versions that need to be installed before the final version.
====

[NOTE]
====
If you are performing a y-stream or z-stream upgrade than you can skip to "Upgrading the worker nodes".
====
