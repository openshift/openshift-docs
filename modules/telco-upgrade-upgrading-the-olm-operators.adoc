...
In telco, software needs to vetted before it is loaded onto a production cluster.
...
Because the clusters are in a disconnected network, the OpenShift Operators are configured for manual upgrade during installation so that new versions can be managed on a cluster by cluster basis.
