...
[id="breaking-changes-1-15-1_{context}"]
=== Breaking changes
...
* Before this update, the pipeline controller could stop working when a matrix task had no `TaskRun` custom resource (CR) to execute. With this update, the pipeline controller performs validation of a matrix task, and when the `TaskRun` CR is not present, the resulting `ResolvedTask` task is nil and the controller performs no further validation.
...
[IMPORTANT]
====
If your pipeline runs fetch remote tasks or pipelines from other private repositories in the same installation, add those repositories to `secret-github-app-scope-extra-repos` in your `pipelines-as-code` config map (comma-separated `owner/repo` format).
====

* Before this update, cached remote `Pipeline` and `Task` objects could be mutated during inlining. As a consequence, subsequent pipeline runs referencing the same remote resource could receive corrupted definitions. With this update, cached objects are deep-copied before use.
