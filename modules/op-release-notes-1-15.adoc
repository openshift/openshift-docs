...
** `Continued`: There was an error during execution of the step, for example a command returned a nonzero exit code, but the step execution continued because the `onError` value was set to `continue`. See the log output for the details of the error.
** `Error`: There was an error during execution of the step, for example a command returned a nonzero exit code, and this error caused the step to fail. See the log output for the details of the error.
...
* The new `skopeo-copy` task, available from the `openshift-pipelines` namespace through the cluster resolver, does not work when the `VERBOSE` parameter is set to `false`, which is the default setting. As a workaround, when you use this task, set the `VERBOSE` parameter to `true`. The issue does not apply to the `skopeo-copy` `ClusterTask`.

* The new `skopeo-copy` task, available from the `openshift-pipelines` namespace through the cluster resolver, fails when you use it to push or pull an image to or from an {OCP} internal image repository, such as `image-registry.openshift-image-registry.svc:5000`. As a workaround, set the `DEST_TLS_VERIFY` or `SRC_TLS_VERIFY` parameter to `false`. Alternatively, use an external image repository that has a valid SSL/TLS certificate. The issue does not apply to the `skopeo-copy` `ClusterTask`.
...
* The new `git-clone` task, available from the `openshift-pipelines` namespace through the cluster resolver, does not set the `COMMIT` result value. The issue does not apply to the `git-clone` `ClusterTask`.
