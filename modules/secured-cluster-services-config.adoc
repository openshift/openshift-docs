...
| `centralEndpoint`
| Address of the Central endpoint. If you are using a non-gRPC capable load balancer, use the WebSocket protocol by prefixing the endpoint address with `wss://`. When configuring many clusters, use the hostname for the address. For example, `central.example.com`.
...
| If you specify `false`, tolerations apply to Collector, and the collector pods can schedule onto all nodes with taints.
If you specify it as `true`, no tolerations apply, and the collector pods do not schedule onto nodes with taints.
...
| This parameter determines if you configured the admission controller to enforce policies that have enforcement enabled. For a new secured cluster deployed with {product-title-short} 4.9, the default value is `true`. For secured clusters updating from {product-title-short} versions before 4.9, earlier values for the admission controller configuration parameters decide the value of this parameter. Before the update, if you set either of the `admissionControl.enforceOnCreates` or `admissionControl.enforceOnUpdates` parameters to `true`, the value of this parameter defaults to `true` after upgrade. If you set both of these parameters to `false`, the default value becomes `false` on update.

|`admissionControl.failurePolicy`
| Controls whether to allow the API server request (fail open) or block it (fail closed) if an error or timeout happens in the {product-title-short} validating webhook's evaluation. Valid values are `Ignore` and `Fail`. The default value is `Ignore` to fail open.
...
| The ability to configure this parameter is deprecated. {product-title-short} uses a preset value for the timeout period and you cannot change it. {product-title-short} ignores this parameter.
...
| `scanner.logLevel`
| Set this parameter to change the scanner log level. Use this option only for troubleshooting purposes.
...
a| For security at the network level, {product-title-short} creates default `NetworkPolicy` resources in the namespace where you install secured cluster resources. These network policies allow ingress to specific components on specific ports. If you do not want {product-title-short} to create these policies, set this parameter to `False`. This is a Boolean value. The default value is `True`, which means the default policies are automatically created.
