...
[IMPORTANT]
====
To prevent the contents of input secrets and config maps from appearing in build output container images, use build volumes in your xref:../Builds-with-BuildConfig/build-strategies.html#builds-using-build-volumes_build-strategies-docker[Docker build] and xref:../Builds-with-BuildConfig/build-strategies.html#builds-using-build-volumes_build-strategies-s2i[source-to-image build] strategies.
====
