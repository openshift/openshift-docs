...
* If the configuration has invalid YAML data, the Cluster Monitoring Operator stops reconciling the resources and reports `Degraded=True` in the status conditions of the Operator.
====
...
[IMPORTANT]
====
This setting is deprecated and is planned to be removed in a future {ocp} version.
