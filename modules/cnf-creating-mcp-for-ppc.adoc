...
[role="_abstract"]
Create a machine config pool to identify target nodes for performance tuning when you deploy performance profiles in multi-node clusters.
...
.. Create a YAML file that defines the `MachineConfigPool` resource. The following example shows the `mcp-worker-cnf.yaml` file:
+
[source,yaml]
----
...
* `spec.nodeSelector.matchLabels` specifies the nodes with the target label (`node-role.kubernetes.io/worker-cnf`) that you defined.
