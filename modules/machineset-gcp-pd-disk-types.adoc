...
where:
ifndef::cpmso[]
`.spec.providerSpec.value.disks.type`:: Specifies the persistent disk type. Valid values are `pd-ssd`, `pd-standard`, and `pd-balanced`. The default value is `pd-standard`.
endif::cpmso[]
ifdef::cpmso[]
`.spec.providerSpec.value.disks.type`:: Control plane nodes must use the `pd-ssd` disk type.
endif::cpmso[]
