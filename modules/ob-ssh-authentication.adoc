...
= Authorize access by using SSH keys
...
`ssh-privatekey`:: Specifies the Base64-encoded SSH private key used to authenticate to the Git repository. You can generate this value by running `base64 ~/.ssh/id_ed25519` or `base64 ~/.ssh/id_rsa`, where `~/.ssh/id_ed25519` or `~/.ssh/id_rsa` is the location of your SSH private key.
