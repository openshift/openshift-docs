...
[role="_abstract"]
Use Model Context Protocol (MCP) server prompts to query a Large Language Model (LLM) to identify and diagnose issues across your {product-title} cluster.
...
[id="ai-app-mcp-server-overview-prompting-instructions-prompts_{context}"]
== MCP prompts
...
[id="ai-app-mcp-server-overview-prompting-instructions-custom_{context}"]
...
[id="ai-app-mcp-server-overview-prompting-instructions-built-in_{context}"]
== Built-in prompts
...
[id="ai-app-mcp-server-overview-prompting-instructions-config-file_{context}"]
...
[id="ai-app-mcp-server-overview-prompting-instructions-toolset_{context}"]
== Toolset prompts

Some toolsets include predefined prompts that guide AI assistants through multi-step troubleshooting workflows. When you enable a toolset in `config.toml`, any prompts bundled with that toolset become available to MCP clients.

The `ovn-kubernetes` and `cni-diagnostics` toolsets focus on diagnostic tools rather than bundled prompts. Use natural-language prompts with your MCP client to invoke toolset tools. For example, ask the client to list `ovnkube-node` pods, trace pod connectivity with `ovn_trace`, or capture packets with `tcpdump`.

When a toolset does provide prompts, enabling the toolset makes those prompts available alongside any custom prompts you define in `config.toml`.

[id="ai-app-mcp-server-overview-prompting-instructions-merging_{context}"]
== Prompt merging
