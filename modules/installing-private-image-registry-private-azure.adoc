...
where:
+
`<vnet_resource_group>`:: Optional. If you have an existing VNet and subnet setup, specify the resource group name that contains the existing virtual network (VNet).
`<subnet_name>`:: Optional. If you have an existing VNet and subnet setup, specify the name of the existing compute subnet within the specified resource group.
`<vnet_name>`:: Optional. If you have an existing VNet and subnet setup, specify the name of the existing virtual network (VNet) in the specified resource group.
+
[NOTE]
====
