...
[role="_abstract"]
You can use the OpenTelemetry Collector's internal metrics for monitoring the Collector's performance, resource consumption, and other internal behaviors. You can also use a Prometheus instance or another monitoring tool to scrape these metrics from a `<instance_name>-collector-monitoring` service.
...
The {OTelOperator} automatically creates a service named `<instance_name>-collector-monitoring` that exposes the OpenTelemetry Collector's internal metrics, including these listed metrics:
