...
* With this update, all communication between the Argo CD components and the Redis cache is secured using modern TLS encryption. link:https://issues.redhat.com/browse/GITOPS-720[GITOPS-720]
...
* Before this update, while reconciling cluster permissions, it was deleted if a secret belonged to a cluster config instance. This update fixes this issue. Now, the `namespaces` field from the secret is deleted instead of the secret. link:https://issues.redhat.com/browse/GITOPS-1777[GITOPS-1777]
