...
[role="_abstract"]
When you delete an `LVMCluster`, the Operator enforces deletion gates to prevent data loss. The gates that apply depend on the reclaim policy that is configured for the storage class.
...
. Delete all Persistent Volume Claims (PVCs) that reference LVM `StorageClasses`.
+
