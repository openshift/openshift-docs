...
[role="_abstract"]
To create virtual Model Context Protocol (MCP) servers for different use cases, you must define the virtual server with an `MCPVirtualServer` custom resource (CR) that specifies required fields.

When a client includes the virtual MCP server header, the {mcpg} filters responses to only include the specified tools.
