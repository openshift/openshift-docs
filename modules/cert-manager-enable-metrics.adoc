...
:navtitle: Configure metrics collection for {cert-manager-operator} operands by using a ServiceMonitor
...
[role="_abstract"]
You can configure metrics collection for the {cert-manager-operator} operands by creating a `ServiceMonitor` custom resource (CR).

The {cert-manager-operator} operands expose metrics by default on port `9402` at the `/metrics` service endpoint. The `ServiceMonitor` CR enables Prometheus Operator to collect custom metrics. 
