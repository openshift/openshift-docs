...
The following examples shows how you can use JSONata in `EventTransform` resources to reshape and enrich events for common business scenarios.

[id="serverless-event-user-registration-event-transformer-example_{context}"]
== User registration event transformer

The following example processes user registration events. It normalizes email addresses, derives a display name if one is missing, sets a registration timestamp, and applies default values for region and subscription tier if they are not provided:
