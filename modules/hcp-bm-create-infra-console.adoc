...
:navtitle: Create an InfraEnv resource by using the console
[id="hcp-bm-create-infra-console_{context}"]
= Creating an InfraEnv resource by using the console

[role="_abstract"]
If you prefer to use the {product-title} web console, you can use it to create an `InfraEnv` resource for a hosted cluster on bare metal.
