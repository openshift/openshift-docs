...
* You can now configure feature flags and components by using the `TektonConfig` custom resource, and the custom resources for individual components, such as `TektonPipeline` and `TektonTriggers`. This level of granularity is useful for customization and testing of cutting-edge features such as the Tekton OCI bundle for individual components, without affecting other custom resources.
