...
Manually add {ibm-z-name} agents with {op-system-base} KVM to your cluster.
[NOTE]
====
...
+
--
`--location <path_to_kernel_initrd_image>,kernel=kernel.img,initrd=initrd.img \//`:: For the `--location` parameter, specify the location of the `kernel` and `initrd` files. The location can be a local server path or a URL using HTTP or HTTPS.
--
...
+
--
`--cdrom \<path_to_image>/<agent_iso_image>`:: For the `--cdrom` parameter, specify the location of the ISO image on the local server, for example, `<path_to_image>/home/<image>.iso`. + [NOTE] ==== For KVM-based installations using DASD devices on {ibm-z-title}, a partition (for example, `/dev/dasdb1`) must be created using the `fdasd` partitioning tool. ==== + endif::pxe-boot[]
--
...
+
--
`--location <path_to_kernel_initrd_image>,kernel=kernel.img,initrd=initrd.img`:: Specifies the location of the kernel/initrd on the HTTP or HTTPS server.
`fips=1`:: Enables FIPS mode. This entry is required in addition to setting the `fips` parameter to `true` in the `install-config.yaml` file. Note: Currently, only PXE boot is supported to enable FIPS mode on {ibm-z-name}.
--
