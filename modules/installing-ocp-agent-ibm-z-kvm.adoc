...
----
<1> For the `--location` parameter, specify the location of the kernel/initrd. The location can be a local server path or a URL using HTTP or HTTPS.
...
$ virt-install
    ...
    --wait=-1
----
<1> For the `--cdrom` parameter, specify the location of the ISO image on the local server.
endif::pxe-boot[]
