...
----
where:
`spec.providerSpec.value.disks.encryptionKey.kmsKey.name`:: Specifies the name of the customer-managed encryption key that is used for the disk encryption.
`spec.providerSpec.value.disks.encryptionKey.kmsKey.keyRing`:: Specifies the name of the KMS key ring that the KMS key belongs to.
`spec.providerSpec.value.disks.encryptionKey.kmsKey.location`:: Specifies the {gcp-short} location in which the KMS key ring exists.
`spec.providerSpec.value.disks.encryptionKey.kmsKey.projectID`:: Optional: Specifies the ID of the project in which the KMS key ring exists. If a project ID is not set, the machine set `projectID` in which the machine set was created is used.
`spec.providerSpec.value.disks.encryptionKey.kmsKey.kmsKeyServiceAccount`:: Optional: Specifies the service account that is used for the encryption request for the given KMS key. If a service account is not set, the Compute Engine default service account is used.
+
