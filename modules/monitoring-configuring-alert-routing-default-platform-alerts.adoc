...
<2> You can create routes within other routes for more complex alert routing. This example routes alerts of `critical` severity that are fired by the `example-app` service to the `team-frontend-page` receiver. Typically, these types of alerts are paged to an individual or a critical response team.
+
[TIP]
====
If you are struggling to add the receiver configuration to the Alertmanager secret, try configuring your receiver through the web console first to see how the configuration appears in the secret. For more information, see "Configuring alert routing with the {product-title} web console".
====
...
. Verify that your routing is configured correctly by visualizing the routing tree:
+
[source,terminal]
----
