...
To secure BGP sessions with MD5 authentication, you can configure BGP peers to reference a Secret containing the password. When you enable the `secretPassthrough` feature in the MetalLB CR, the password is passed as a Secret reference to FRR-K8s instead of being exposed in cleartext.
