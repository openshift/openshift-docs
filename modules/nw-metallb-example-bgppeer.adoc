...
Example of specifying BGP authentication with MD5 passwords::

To secure BGP sessions with MD5 authentication, you can configure BGP peers to reference a Secret containing the password. When you enable the `secretPassthrough` feature in the MetalLB CR, the password is passed as a Secret reference to FRR-K8s instead of exposing the password value.

First, enable secret passthrough in the MetalLB CR:
...
Then create a Secret in the `openshift-frr-k8s` namespace:
...
Finally, configure the BGPPeer to reference the Secret:
...
[IMPORTANT]
====
When using the `secretPassthrough` feature, the Secret must reside in the `openshift-frr-k8s` namespace, not the `metallb-system` namespace. Cross-namespace secret references are not supported by FRR-K8s.
====