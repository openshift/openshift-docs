...
spec:
  ...
  ttl: 720h0m0s
----
<1> Use this as the `backupName` value while restoring the Redis database.
...
NAME				                AGE
mysql-backup			          7m16s
operator-install-backup 	  122m
operator-resources-secrets 	105m
operator-resources-apim	    109m
redis-backup 			          2m
----