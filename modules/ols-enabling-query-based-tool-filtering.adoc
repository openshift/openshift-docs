...
Enable query-based tool filtering to automatically select relevant tools and specify the maximum number of tool call iterations for {ols-long}.
...
* `spec.olsConfig.maxIterations` defines the number of rounds {ols-short} executes when invoking the LLM with tools.
