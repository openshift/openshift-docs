...
[role="_abstract"]
Configure kernel page sizes in the performance profile to optimize memory-intensive workloads, with support for 4k on x86_64 architectures and 4k or 64k on AArch64 architectures.
...
. Create a performance profile to target nodes where you want to configure the kernel page size by creating a YAML file that defines the `PerformanceProfile` resource. The following example shows the `pp-kernel-pages.yaml` file:
+
[source,yaml]
----
