...
Service discovery is not mandatory. You can set the endpoint of the called workflow in either of the following ways:

Kubernetes service discovery:: You set the URL to a service discovery URI, and the {ServerlessLogicOperatorName} resolves the URI to a network endpoint when it deploys the calling workflow.
+
[IMPORTANT]
====
...
Kubernetes DNS name of the service:: You set the URL to the DNS name of the Kubernetes `Service` that the {ServerlessLogicOperatorName} creates for the called workflow. This name uses the `\http://<called_workflow_id>.<namespace>.svc.cluster.local` format. Use this option when you prefer a fixed URL, or when the startup time of the workflow is critical.
...
. You set the URL of the generated REST client to a service discovery URI in the user-defined properties `ConfigMap` of the calling workflow.
. You build and deploy the calling workflow.
. The {ServerlessLogicOperatorName} resolves the URI and writes the resolved endpoint to the operator-managed properties `ConfigMap` of the calling workflow.
