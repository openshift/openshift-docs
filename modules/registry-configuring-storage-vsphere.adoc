...
[IMPORTANT]
====
Testing shows issues with using the NFS server on {op-system-base} as storage backend for
...
Therefore, using {op-system-base} NFS to back PVs used by core services is not recommended.
