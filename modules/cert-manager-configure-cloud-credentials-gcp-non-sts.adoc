...
. Verify that the cert-manager controller pod is updated with {gcp-full} credential volumes that are mounted under the path specified in `mountPath` by running the following command:
+
[source,terminal]
----
