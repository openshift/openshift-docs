...
= Set up network policies to be in place during migration

This procedure applies only if you want network policies in place during your migration.
...
----
+
.Example expose route network policy in a bookinfo namespace
[source,yaml]
--
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
...
--
+
.Example mesh network policy in a bookinfo namespace
[source,yaml]
--
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
...
--
+
.Example expose route network policy iin a bookinfo2 namespace
[source,yaml]
--
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
...
--
+
.Example mesh network policy in a bookinfo2 namespace
[source,yaml]
--
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
...
. Copy the ACTIVE REVISION name to use as your `istio.io/rev` label when creating your second Istiod network policy for {SMProduct} 3.
