...
[IMPORTANT]
====
* During the recreation of network policies from {SMProduct} 2.6 to {SMProduct} 3.2, both control planes must have access to all workloads and all workloads must have access to control planes.
...
* In {SMProduct} 2.6, you have set `spec.security.manageNetworkPolicy=true` in your  `ServiceMeshControlPlane` resource.
...
. Disable network policies in {SMProduct} 2.6 by setting the `spec.security.manageNetworkPolicy` field to `false` in your `ServiceMeshControlPlane` resource.
+
[NOTE]
====
...
. Create a second Istiod network policy for {SMProduct} 3.2 by using the following `NetworkPolicy` example configuration:
+
[source,yaml]
----
apiVersion: networking.k8s.io/v1
...
* In {SMProduct} 2.6, set the `spec.security.manageNetworkPolicy` field to `false` in your `ServiceMeshControlPlane` resource, and continue with the migration checklists.