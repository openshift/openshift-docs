...
<node1>     NotReady,SchedulingDisabled   worker    1d        v1.17.1
----
...
* The following command ignores pods managed by a daemon set by using the `--ignore-daemonsets` flag set to `true`:
+
----
