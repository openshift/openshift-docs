...
[role="_abstract"]
IP multicast sends one data stream to many hosts at once in {product-title}. With OVN-Kubernetes, multicast is off by default and is not affected by network policies when you enable it in a project.
