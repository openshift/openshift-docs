...
In OVN-Kubernetes, `NetworkPolicies` apply strict isolation rules. If a service is exposed using an ExternalIP, NetworkPolicies can prevent access from other namespaces unless explicitly allowed.

To allow access to ExternalIPs across namespaces, create a `NetworkPolicy` that permits ingress from the required namespaces:
+
.Example output
[source,yaml]
----
...
kind: NetworkPolicy
metadata:
  name: allow-externalip-access
spec:
  ingress:
  - from:
    - namespaceSelector:
        matchLabels:
          kubernetes.io/metadata.name: <namespace-name> <1>
  podSelector: {}
  policyTypes:
  - Ingress
----
<1> Specify the namespace that requires access to the service.

For more details, see "NetworkPolicy configuration".