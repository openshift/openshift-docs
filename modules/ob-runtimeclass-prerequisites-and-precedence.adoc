...
[IMPORTANT]
====
OpenShift Sandboxed Containers is only supported on bare-metal clusters. Cloud-managed clusters such as ROSA, ARO, or hosted control plane deployments cannot use RuntimeClass isolation with Kata Containers.
====
...
[NOTE]
====
Creating the `KataConfig` CR triggers an automatic reboot of worker nodes. This reboot can take from 10 to more than 60 minutes.
====
