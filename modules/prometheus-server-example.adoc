...
* You have configured the `prometheus-server` service account and given it the necessary permissions.
//do we need to say what those permissions are?

.Procedure

. Install the Prometheus Operator in the `stackrox` namespace and verify that the Prometheus custom resource (CR) contains the following information:
+
[source,yaml]
----
