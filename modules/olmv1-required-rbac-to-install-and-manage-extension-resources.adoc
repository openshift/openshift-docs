...
These cluster-scoped bindings cannot grant permissions to access or modify Red{nbsp}Hat SRE-managed resources or namespaces.
endif::[]
...
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
Any CRDs that attempt to replicate or intercept core {product-title} platform APIs will be blocked by SRE gatekeeping mechanisms.
endif::[]
...
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
You must include RBAC for the explicit cluster permissions requested by the extension's deployment manifests, ensuring they remain within your delegated `dedicated-admin` capabilities.
endif::[]
...
Roles and role bindings:: You must define RBAC for any roles or role bindings defined in the
ifndef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
CSV.
endif::[]
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
...
The installation service account needs permission to create and manage those roles and role bindings.
