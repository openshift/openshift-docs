...
. Verify that the {clo} is installed by switching to the *Operators* → *Installed Operators* page.
...
<7> Specify the CPU and memory requests for Elasticsearch as needed. If you leave these values blank, the {es-op} sets default values that should be sufficient for most deployments. The default values are `16Gi` for the memory request and `1` for the CPU request.
...
.Troubleshooting
* If Alertmanager logs alerts such as `Prometheus could not scrape fluentd for more than 10m`, make sure that `openshift.io/cluster-monitoring` is set to `"true"` for the {es-op} and the {clo}. See the Red Hat KnowledgeBase for more information: link:https://access.redhat.com/solutions/5692801[Prometheus could not scrape fluentd for more than 10m alert in Alertmanager]
