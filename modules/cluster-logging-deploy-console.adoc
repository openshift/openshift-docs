...
You can use the {product-title} web console to install the Loki Operator and the Red{nbsp}Hat OpenShift Logging Operator.
endif::[]
ifdef::openshift-rosa,openshift-dedicated[]
You can install the Loki Operator and the Red{nbsp}Hat OpenShift Logging Operator by using the {product-title} {cluster-manager-url}.
endif::[]
...
.. Type *Loki* into the filter and choose the *Loki Operator* from the list of available Operators.
.. Click *Install*.
.. In the *Install Operator* page:
+
* Select *stable* as the *Update channel*.
* Select the newest version in the *Version* drop-down list.
* Select *All namespaces on the cluster* as the *Installation Mode*.
* Select the *Operator recommended Namespace* (`openshift-operators-redhat`) as the *Installed Namespace*.
...
.. Verify that the Loki Operator is installed by navigating to the *Operators* → *Installed Operators* page.

.. Verify that *OpenShift Elasticsearch Operator* is listed in all projects with a *Status* of *Succeeded*.

// SPLIT INTO SECOND PROCEDURE
