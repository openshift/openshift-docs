...
+
You must specify the `openshift-operators-redhat` Namespace. To prevent
...
.. Verify that the Cluster Logging Operator installed by switch to the *Operators* → *Installed Operators* page.
...
<4> Specify the time that Elasticsearch should retain each log source. Logs older than the `maxAge` are deleted.
<5> Specify the number of Elasticsearch nodes. See the note that follows this list.
<6> Specify that each Elasticsearch node in the cluster is bound to a Persistent Volume Claim.
...
<8> Settings for configuring Curator. Using the CR, you can set the Curator schedule. For more information, see *Configuring Curator*.
