...
:navtitle: Update mirror registry for Red Hat OpenShift from a remote host
[id="mirror-registry-remote-host-update_{context}"]
= Updating mirror registry for Red Hat OpenShift from a remote host

[role="_abstract"]
You can update the _mirror registry for Red{nbsp}Hat OpenShift_ from a remote host by using the `upgrade` command. Updating to the latest version ensures bug fixes and security vulnerability fixes.
