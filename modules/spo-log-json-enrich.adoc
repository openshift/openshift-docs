...
. Create a file such as `patch-volume-source.json` that contains the following content:
+
[source,json]
----
{
...
. Verify the file contents by running the following command:
+
[source,terminal]
----
...
. Update the `security-profiles-operator-profile` `configmap` by using the following command:
+
[source,terminal]
----
...
. Set the audit log file path by configuring the JSON log enricher with the full path to your audit log file, including the file name, by running the following command:
+
[source,terminal]
----
