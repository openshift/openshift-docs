...
[role="_abstract"]
You can retrieve Prometheus-format HAProxy ingress router metrics from port `1936` to monitor ingress load and troubleshoot routing behavior.
Analyzing these metrics helps you identify capacity bottlenecks and determine when to scale your router deployment.
...
. Read the stats user and password from the router pod (under `/var/lib/haproxy/conf/metrics-auth/`):
+
[source,terminal]
----
...
. Get pod details (including the IP):
+
[source,terminal]
----
...
. Fetch Prometheus text metrics (default port 1936):
+
[source,terminal]
----
...
----
+
If the stats endpoint serves TLS-protected Prometheus text, retrieve metrics over HTTPS instead:
+
[source,terminal]
----
