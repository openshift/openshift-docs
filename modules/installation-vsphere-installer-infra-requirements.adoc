...
These resources use 856 GB of storage in total.

Bootstrap node resources are not configurable. The bootstrap node uses the following resources:

* vCPU: 4
* Virtual RAM: 16 GB
* Storage: 120 GB

The bootstrap node is destroyed before compute machines are created, so the maximum amount of resources used by cluster is the greater of the following:

* {blank}
+
----
resources for control plane nodes + resources for bootstrap node
----

* {blank}
+
----
resources for control plane nodes + resources for compute machines
----

A minimum of 800 GB of storage is required to use a standard cluster. If you deploy more compute machines, the {product-title} cluster will use more storage.
...
You must create DNS records for the two reserved static IP addresses in the appropriate DNS server for the vCenter instance that hosts your {product-title} cluster.
...
[NOTE]
====
When you install an {product-title} cluster on your vCenter that uses infrastructure that the installer provisions, the installation program uses its own internal load balancer based on HAProxy and keepalived. The installation program creates the HAProxy and keepalived pods in the `openshift-vsphere-infra` namespace.
====
