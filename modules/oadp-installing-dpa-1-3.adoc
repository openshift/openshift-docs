...
----
<1> The default namespace for OADP is `openshift-adp`. The namespace is a variable and is configurable.
<2> The `aws` plugin is mandatory for all S3 storage.
  ...
  configuration:
    velero:
      defaultPlugins:
        ...
        - csi # <5>
        - openshift # <6>
      resourceTimeout: 10m # <7>
    nodeAgent: # <8>
      enable: true # <9>
      uploaderType: kopia # <10>
      podConfig:
        nodeSelector: <node_selector> # <11>
  backupLocations:
    - velero:
        ...
        credential:
          key: cloud
          name: <default_secret> # <13>
        objectStorage:
          bucket: <bucket_name> # <14>
          prefix: <prefix> # <15>
----
<1> The default namespace for OADP is `openshift-adp`. The namespace is a variable and is configurable.
<2> The `aws` plugin is mandatory for all S3 storage.
<3> Optional: The `kubevirt` plugin is used with {VirtProductName}.
<4> Specify the default plugin for the backup provider, for example, `gcp`, if appropriate.
<5> Specify the `csi` default plugin if you use CSI snapshots to back up PVs. The `csi` plugin uses the link:https://{velero-domain}/docs/main/csi/[Velero CSI beta snapshot APIs]. You do not need to configure a snapshot location.
<6> The `openshift` plugin is mandatory.
<7> Specify how many minutes to wait for several Velero resources before timeout occurs, such as Velero CRD availability, volumeSnapshot deletion, and backup repository availability. The default is 10m.
<8> The administrative agent that routes the administrative requests to servers.
<9> Set this value to `true` if you want to enable `nodeAgent` and perform File System Backup.
<10> Enter `kopia` or `restic` as your uploader. You cannot change the selection after the installation. For the Built-in DataMover you must use Kopia. The `nodeAgent` deploys a daemon set, which means that the `nodeAgent` pods run on each working node. You can configure File System Backup by adding `spec.defaultVolumesToFsBackup: true` to the `Backup` CR.
<11> Specify the nodes on which Kopia or Restic are available. By default, Kopia or Restic run on all nodes.
<12> Specify the backup provider.
<13> Specify the correct default name for the `Secret`, for example, `cloud-credentials-gcp`, if you use a default plugin for the backup provider. If specifying a custom name, then the custom name is used for the backup location. If you do not specify a `Secret` name, the default name is used.
<14> Specify a bucket as the backup storage location. If the bucket is not a dedicated bucket for Velero backups, you must specify a prefix.
<15> Specify a prefix for Velero backups, for example, `velero`, if the bucket is used for multiple purposes.
endif::[]
