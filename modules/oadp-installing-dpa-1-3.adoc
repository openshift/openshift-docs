...
<4> Specify the S3 URL of the COS bucket. For example, `http://s3.eu-gb.cloud-object-storage.appdomain.cloud`. Here eu-gb is the region name, replace the region name as per your bucket location. 
<5> `cloud-credentials` is the name of the file you created when you `access-key` and `secret-access-key` from the `HMAC` credentials.
endif::[]
