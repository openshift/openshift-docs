...
You can configure `linuxptp` services (`ptp4l`, `phc2sys`) as ordinary clock with dual-port NIC redundancy by creating a `PtpConfig` custom resource (CR) object.
This configuration ensures that if one port fails, the standby port takes over, maintaining synchronization.
...
<2> Specify the interface configuration for the `ptp4l` service. In this example, setting `masterOnly 0` for both interfaces enables both ports on the `ens3` interface to run as leader or follower clocks. When the CR is applied, one port operates as the active ordinary clock, the other port operates as a standby ordinary clock in the `Listening` port state. 
