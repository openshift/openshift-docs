...
To transition from OSL version 1.34.0 to 1.35.0, you must upgrade the {ServerlessLogicOperatorName} using the OpenShift web console. This upgrade ensures compatibility with newer features and proper functioning of your workflows.
...
. In the *Update* channel, select, `stable-1.35` and click *Save* to apply the changes.
. To monitor the upgrade process, execute the following command: 
+
[source,terminal]
----
$ oc get subscription serverless-logic-operator -n openshift-serverless-logic -o jsonpath='{.status.currentCSV}'
----
+
.Expected output
[source,terminal]
----
$ serverless-logic-operator.v1.35.0
----

.Verification

. To verify the new Operator version is installed, execute the following command: 
+
[source,terminal]
----
