...
After the Operator upgrade, a new ReplicaSet is automatically created for Data Index 1.36.0. You must delete the old one manually. If your deployment is configured to use a Knative Eventing Kafka Broker, you must delete the old `data-index-process-definition` trigger that was created by the {ServerlessLogicProductName} 1.35.0 version.
...
. Identify the old Data Index ReplicaSet (with version 1.35.0) and delete it by running the following command:
+
[source,terminal]
----
...
----
+
.Example command as per the generated example output
[source,terminal]
----
