...
After the Operator upgrade, If your deployment is configured to use a Knative Eventing Kafka Broker, you must delete the old `data-index-process-definition` trigger that was created by the {ServerlessLogicProductName} 1.35.0 version. Optionally you can delete the old `ReplicaSet` as well.
