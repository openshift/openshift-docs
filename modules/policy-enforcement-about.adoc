...
{product-title-short} offers two types of policy enforcement:

* Sensor enforcement: Also known as _soft_ enforcement, if you examine a workload and it violates a policy, Sensor scales down pods by using the Kubernetes API.
* Admission controller enforcement: Also known as _hard_ enforcement, an admission controller works with validating webhooks and the Kubernetes API server to block workload admission or update attempts that violate an enforced policy. When you make a request to deploy or update, the validating webhooks communicate with the admission controller and the admission controller responds to either allow or block the action based on whether you enforce a policy. The API server accepts or rejects the request based on whether it violates the policy and if you enforce the policy.
