...
Sometimes, the installer times out while the RHOCP cluster is spinning up in the background. The installer has three stages where it waits for cluster events. As the events happen, the installer displays status messages:
