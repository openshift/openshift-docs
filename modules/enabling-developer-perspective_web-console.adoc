...
* To modify console settings, you must be logged in as a cluster-admin.

.Procedure

. Navigate to the *Cluster Settings* page by clicking  *Administration* -> *Cluster Settings* in the navigation.
...
.Optional: You can enable the *Developer* perspective using the CLI by running the following command:
+
[source,terminal]
----
...
[NOTE]
====
It will take some time for the change to reflect in the web console as the console pod gets restarted.
====