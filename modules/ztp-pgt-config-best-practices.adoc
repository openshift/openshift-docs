...
* The default setting for `policyDefaults.consolidateManifests` is `true`. This is the recommended setting for DU profile. Setting it to `false` could impact large scale deployments.
...
If you use the `{policy-gen-cr}` CR to change the order of the policies, this can create conflicts and result in failure to apply the configuration.
