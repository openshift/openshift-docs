...
----
+
A ready revision should have the `reason: ServiceReady`, `status: "True"`, and `type: Ready` conditions in its status. If these conditions are present, you might want to check pod status or Istio routing. Otherwise, the resource status has the error message.
