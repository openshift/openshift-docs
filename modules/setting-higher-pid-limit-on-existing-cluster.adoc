...
= Setting the process IDs (PIDs) limit using the ROSA CLI

You can set the process IDs (PIDs) limit on an existing {product-title} cluster by using the ROSA CLI (`rosa`) to create or edit a `KubeletConfig` object that changes the `--pod-pids-limit` parameter.
...
. Create or edit the `KubeletConfig` object to change the PIDs limit.
+
--
** If this is the first time you are changing the default PIDs limit, create the `KubeletConfig` object and set the `--pod-pids-limit` value by running the following command: 
+
[source,terminal]
----
