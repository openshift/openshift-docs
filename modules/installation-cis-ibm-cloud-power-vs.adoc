...
IBM Cloud Internet Services (CIS) is used by the installation program to configure cluster DNS resolution and provide name lookup for the cluster to external resources. Only public DNS is supported for Power VS IPI.

[NOTE]
====
Power VS does not support IPv6, so dual stack or IPv6 environments are not possible.
====
