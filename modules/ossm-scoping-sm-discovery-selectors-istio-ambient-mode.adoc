...
To limit the scope of the {SMProduct} in {istio} ambient mode, you can configure `discoverySelectors` in the `meshConfig` section of the `{istio}` resource. The configuration controls which namespaces the control plane discovers based on label selectors.
...
. Add a label to the namespace containing the `{istio}` control plane resource, for example, the `istio-system` namespace.
+
[source,terminal]
----
...
. Add a label to the namespace containing the `IstioCNI` resource, for example, the `istio-cni` namespace.
+
[source,terminal]
----
...
. Add a label to the namespace containing the `Ztunnel` resource, for example, the `ztunnel` namespace.
+
[source,terminal]
----
