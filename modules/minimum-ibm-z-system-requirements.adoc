...
[IMPORTANT]
====
Installing {product-title} {product-version} on {ibm-name} z17 or {ibm-linuxone-name} 5 requires a minimum kernel version 5.14.0-427.81.1 or later. This kernel version is included in {product-title} 4.16.64, which is therefore the minimum supported version for {ibm-name} z17 systems.
====
