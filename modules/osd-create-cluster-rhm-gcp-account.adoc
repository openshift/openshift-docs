...
= Creating a cluster on {gcp-full} with Red Hat Marketplace
...
. If you selected *Service account* as the Authentication type, provide your {gcp-full} service account private key in JSON format. You can either click *Browse* to locate and attach a JSON file or add the details in the *Service account JSON* field.
...
[NOTE]
====
If you are installing a cluster into a {gcp-full} Shared VPC, the VPC name and subnets are shared from the host project.
====
+
