...
When creating an {product-title} (OSD) cluster on {gcp-full} through the OpenShift Cluster Manager Hybrid Cloud Console, customers can select Red Hat Marketplace as their preferred billing model.
...
.. Select *{gcp-full}* as your cloud provider.
