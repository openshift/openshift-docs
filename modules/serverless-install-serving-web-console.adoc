...
After you install the {ServerlessOperatorName}, install Knative Serving by using the {ocp-product-title} web console. You can install Knative Serving by using the default settings or configure more advanced settings in the `KnativeServing` custom resource (CR).
