...
<luks_key_argument>:: Specifies the location of the key that is required to decrypt the device. For example: `rd.luks.key=/etc/luks/cex.key`. You can not change this value.
endif::ibm-z-kvm[]
ifndef::ibm-z-kvm[]
