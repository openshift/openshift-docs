...
+
ifdef::ibm-z-kvm[]
Create a file named `main-storage.bu` with the following example Butane configuration for a control plane node with disk encryption:
+
...
<2> Specifies the location of the key that is required to decrypt the device. This is a fixed value that can not be changed.
endif::ibm-z-kvm[]
ifndef::ibm-z-kvm[]
For installations on DASD-type disks, create a file named `main-storage.bu` with the following example Butane configuration for a control plane node with disk encryption:
+
[source,yaml,subs="attributes+"]
----
variant: openshift
...
<2> Specifies the location of the key that is required to decrypt the device. This is a fixed value that can not be changed.
+
For installations on FCP-type disks, create a file named `main-storage.bu` with the following example Butane configuration for a control plane node with disk encryption:
+
[source,yaml,subs="attributes+"]
----
variant: openshift
