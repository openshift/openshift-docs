...
.OADP 1.5.0 introduces a new feature names Self-Service
...
Previously, when a restore operation was triggered with the `includedNamespace` field in a DPA specification, restore operation was completed successfully but no `volumeSnapshotContents` custom resources (CR) were created and the PVCs were in a `Pending` status.
