...
:navtitle: Gather data for a hosted cluster by using the CLI
[id="hcp-must-gather-cli_{context}"]
= Gathering data for a hosted cluster by using the CLI

[role="_abstract"]
You can gather {product-title} debugging information for a hosted cluster by using the command-line interface (CLI).
