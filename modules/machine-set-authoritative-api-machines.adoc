...
[role="_abstract"]
When migrating between Machine API and Cluster API, the authoritative API of a compute machine is determined by the `.spec.authoritativeAPI` and `.spec.template.spec.authoritativeAPI` fields in the compute machine set that creates it.
