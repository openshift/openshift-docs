...
<5> Specifies the policy types that define the root of trust, either a public key or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certifice], or a BYOPKI. Here, a public key with Rekor verification.
...
<6> For a PKI, the following parameters are required:
* `caRootsData`. Specifies a base64-encoded CA root certificates in the PEM format. The maximum length is 8192 characters.
* `caIntermediatesData`  Specifies a base64-encoded intermediate CA root certificates in the PEM format. The maximum length is 8192 characters.
* `email`. Specifies the certificate subject email address.
* `hostname`. Specifies the certificate subject hostname. 
<7> Specify `MatchRepository` to verify the identity in the signature and the actual image identity. The the default signed identity is `matchRepoDigestOrExact`, which requires digest specification. The signature in this case was not created for digested image.
--
// <8> is from the test case: https://polarion.engineering.redhat.com/polarion/#/project/OSE/workitem?id=OCP-83752
