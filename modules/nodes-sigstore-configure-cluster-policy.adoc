...
* You have a sigstore-supported public key infrastructure (PKI), a bring your own public key infrastructure (BYOPKI), or a link:https://docs.sigstore.dev/cosign/[Cosign public and private key pair] for signing operations.
...
* If you are using a BYOPKI validation as the root of trust, you enabled the required Technology Preview features for your cluster by editing the `FeatureGate` CR named `cluster`:
+
[source,terminal]
----
...
<5> Specifies the policy types that define the root of trust, either a public key, a BYOPKI, or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate]. Here, a public key with Rekor verification.
...
<3> Specifies the parameters that define how the images are verified.
<4> Defines a root of trust for the policy.
<5> Specifies the policy types that define the root of trust, either a public key, a BYOPKI, or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate]. Here, a Fulcio certificate with required Rekor verification.
...
+
--
.Example cluster image policy object for BYOPKI signature verification
[source,yaml]
----
...
<3> Specifies the parameters that define how the images are verified.
<4> Defines a root of trust for the policy.
<5> Specifies the policy types that define the root of trust, either a public key, a BYOPKI, or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate]. Here, a BYOPKI.
<6> For a BYOPKI, the following parameters are required:
* `caRootsData`. Specifies a base64-encoded CA root certificate in the PEM format. The maximum length is 8192 characters.
* `caIntermediatesData`. Specifies a base64-encoded intermediate CA root certificate in the PEM format. The maximum length is 8192 characters.
...
<8> For a BYOPKI, specify `MatchRepository` to verify the identity in the signature and the actual image identity. The default signed identity is `matchRepoDigestOrExact`, which requires digest specification. The signature in this case was not created for digested image.
--
// <7> is from the test case: https://polarion.engineering.redhat.com/polarion/#/project/OSE/workitem?id=OCP-83752
...
----
+
The Machine Config Operator (MCO) updates the machine config pools (MCP) in your cluster and scheduling on each node is disabled as the change is being applied.
...
----
+
.Example output for the cluster image policy object for BYOPKI signature verification showing the new cluster image policy
[source,json]
----
# ...
  "transports": {
...
spec:
  ...
  policy:
      ...
      policyType: PKI
      pki:
        caRootsData: LS0tLS1CRbWRPSlpycb1dJWkYFV
        caIntermediatesData: LS0tLS1CRbWRPSlpycb1dJWkYFV
