...
<2> Defines a list of image IDs assigned to this policy. In a cluster image policy, make sure that the policy allows deployment of the {product-title} images in the `quay.io/openshift-release-dev/ocp-release` and `quay.io/openshift-release-dev/ocp-v4.0-art-dev` repositories. Images in these repositories are required for cluster operation..
<3> Specifies the parameters that define how the image IDs are verified.
...
<8> Specifies one of the following processes to verify the identity in the signature and the actual image ID:
...
* `exactRepository`
* `remapIdentity`
<9> For the `remapIdentity` match policy, specifies the repository that contains the image ID and signature.
<10> For the `remapIdentity` match policy, specifies the prefix of the image ID to be matched in the signature. The format is the same as "prefix".
--
