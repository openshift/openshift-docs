...
. After the nodes in your cluster are updated, verify that the cluster image policy has been configured:
...
----
+
The following example output shows the cluster image policy with a public key:
+
[source,json]
----
# ...
...
----
+
The following example output shows the cluster image policy with a BYOPKI certificate:
+
[source,json]
----
# ...
...
----
+
The following example output shows the cluster image policy with a Fulcio certificate:
+
[source,json]
----
# ...
...
----
+
The following example output shows that the scoped registry was added:
+
[source,yaml]
----
docker:
...
. Verify that unsigned or incorrectly signed images are blocked from running:

.. Create a test pod that uses an unsigned image from the scoped registry:
+
[source,terminal]
----
$ oc run test-unsigned --image=<scoped_registry>/<unsigned_image>:<tag> --restart=Never
----
+
where:
+
* `<scoped_registry>` -- Specifies the registry defined in your `ClusterImagePolicy` scope.
* `<unsigned_image>` -- Specifies an image that does not have a valid sigstore signature.
...
[source,terminal]
----
$ oc get pod test-unsigned -o jsonpath='{.status.containerStatuses[0].state}'
----
+
The following example output shows that the image was blocked:
+
[source,json]
----
{"waiting":{"message":"rpc error: code = Unknown desc = Source image rejected: A]...]","reason":"ErrImagePull"}}
----
+
The `ErrImagePull` status with a "Source image rejected" message confirms that the signature verification policy is blocking unsigned images.

.. Check the pod events for detailed error information:
+
[source,terminal]
----
$ oc describe pod test-unsigned
----
+
The following example output shows signature verification failure in the pod events:
+
[source,terminal]
----
Events:
  ...
  ----     ------     ----  ----               -------
  Normal   Scheduled  10s   default-scheduler  Successfully assigned default/test-unsigned to worker-0
  Normal   Pulling    10s   kubelet            Pulling image "<scoped_registry>/<unsigned_image>:<tag>"
  Warning  Failed     9s    kubelet            Failed to pull image "<scoped_registry>/<unsigned_image>:<tag>": rpc error: code = Unknown desc = Source image rejected: Signature for identity <image_identity> is not accepted
...
[source,terminal]
----
$ oc delete pod test-unsigned --ignore-not-found
----
