...
// Abstract from https://github.com/openshift/openshift-docs/pull/81985/files#diff-1f70afdcabd5c3ddafa70557887dc5dc5d5b00705e9f67b20ca4d2e99b17c8efR48

[role="_abstract"]
Cluster administrators can use a `ClusterImagePolicy` custom resource (CR) to declare an image policy for the cluster, requiring signature validation and other policy checks in addition to image integrity.
...
[NOTE]
====
The default `openshift` cluster image policy provides sigstore support for the required {product-title} images, which are stored in the `quay.io/openshift-release-dev/ocp-release` repository. You must not remove or modify this cluster image policy object. Cluster image policy names beginning with `openshift` are reserved for future system use.
====
