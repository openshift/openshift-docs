...
:navtitle: Create the {op-system-first} image
...
[WARNING]
====
If the installation program finds multiple images with the same name, the program chooses one of them at random. To avoid this behavior, create unique names for resources in {rh-openstack}.

After you upload the image to {rh-openstack}, the image is usable in the installation process.
====
