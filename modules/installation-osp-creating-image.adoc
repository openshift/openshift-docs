...
+
[IMPORTANT]
Depending on your {rh-openstack} environment, you might be able to upload the image in either the `.raw` or `.qcow2` format. If you use Ceph, you must use the `.raw` format. Follow link:https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/16.2/html/creating_and_managing_images/ch-image-service#section-convert-image[the manual process] to transform the image from `.qcow2` to `.raw`.
+
[WARNING]
