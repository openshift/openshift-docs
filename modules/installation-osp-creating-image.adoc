...
[NOTE]
====
The {op-system} ISO image supports booting in both UEFI and legacy BIOS modes. Instances that boot from the image use legacy BIOS firmware by default.
====

. Optional: If you use UEFI boot in your {rh-openstack} environment, you can set the `hw_firmware_type` property on the image by running the following command:
+
[source,terminal]
----
$ openstack image set --property hw_firmware_type=uefi rhcos
----
+
Replace `rhcos` with your image name if it differs from the example.
