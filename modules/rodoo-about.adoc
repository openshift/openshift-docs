...
:navtitle: The {run-once-operator}
:chunk-to-content:
[id="rodoo-about_{context}"]
= About the {run-once-operator}

[role="_abstract"]
The {run-once-operator} enforces time limits on run-once pods to prevent tasks such as builds and deployments from running indefinitely.
...
Cluster administrators can use the {run-once-operator} to force a limit on the time that those run-once pods can be active. After the time limit expires, the cluster tries to actively terminate those pods.
