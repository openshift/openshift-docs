...
|`spec.passwordSecret`
|`string`
|Optional: Specifies name of the authentication secret for the BGP peer. The secret must be of type `kubernetes.io/basic-auth` with the password stored under the `password` key.

When using the `frrk8sConfig.secretPassthrough` feature in the MetalLB CR, the secret must reside in the `openshift-frr-k8s` namespace. Otherwise, the secret must reside in the `metallb-system` namespace.
...
[IMPORTANT]
====
Starting in {product-title} 4.19, when you enable the `frrk8sConfig.secretPassthrough` field in the MetalLB CR, the MetalLB Operator passes BGP passwords as Secret references to the FRR-K8s daemon rather than extracting the password values from the Secrets. This prevents password values from being exposed in the generated `FRRConfiguration` custom resources.

When `secretPassthrough` is enabled, ensure that:

* The Secret referenced in `spec.passwordSecret.name` exists in the `openshift-frr-k8s` namespace.
* The Secret is of type `kubernetes.io/basic-auth`.
* The password is stored under the `password` key in the Secret.

Cross-namespace secret references are not supported by FRR-K8s.
====
