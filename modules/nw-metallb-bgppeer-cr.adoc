...
[IMPORTANT]
====
Starting in {product-title} 4.19, when you enable the `frrk8sConfig.secretPassthrough` field in the MetalLB CR, the MetalLB Operator passes BGP passwords as Secret references to the FRR-K8s daemon rather than resolving them to cleartext. This prevents passwords from being exposed in cleartext in the generated `FRRConfiguration` custom resources.
