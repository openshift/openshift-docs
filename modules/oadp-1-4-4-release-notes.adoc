...
[NOTE]
====
When you restore a stateful application that uses `azurefile-csi` storage class, the restore operation remains in `Finalizing` phase. This is because Azure File cloning does not support the Network File System protocol but only `azurefile-csi` storage class, which uses Server Message Block protocol.
====