...
Collected data is written into a new directory that starts with `must-gather.local` in the current working directory by default.
...
----
$ oc adm must-gather \
  --image=registry.redhat.io/kueue-tech-preview/kueue-must-gather-rhel9
----
