...
The custom resource definition (CRD) defines the configuration used when you deploy an instance of Jaeger. The default CRD for Jaeger is `jaeger-all-in-one` and it is configured to ensure that you can install it on a default {product-title} installation. You can use this default CRD to create a Jaeger instance, or you can create your own custom resource file.

[NOTE]
====
In-memory storage is not persistent, which means that if the Jaeger pod shuts down, restarts, or is replaced, that your trace data will be lost.  For persistent storage, you must use the `production` (Elasticsearch) deployment strategy.
====
...
////
DEFAULT strategy creates the following:
* 1 Deployment
* 4 Services
* 1 ReplicaSet
* 1 Pod
