...
This policy blocks _all_ cross-pod networking other than network traffic allowed by the configuration of other deployed network policies and traffic between host-networked pods. This procedure enforces a strong deny policy by applying a `deny-by-default` policy in the `my-project` namespace.
...
----
ifdef::multi[]
<1> `namespace: my-project` deploys this policy to the `my-project` namespace.
<2> `network_name`: specifies the name of a network attachment definition.
<3> `podSelector:` is empty, this means it matches all the pods. Therefore, the policy applies to all pods in the `my-project` namespace.
...
ifndef::multi[]
<1> `namespace: my-project` deploys this policy to the `my-project` namespace.
<2> `podSelector:` is empty, this means it matches all the pods. Consequently, the policy applies to all pods in the `my-project` namespace.
