...
= Build images by using a Dockerfile in restricted networks
...
`--source-url`:: Defines the URL of the Git repository containing the source code.
...
buildah-golang-build  True         Succeeded   ClusterBuildStrategy   buildah             68s
----
+
[source,terminal]
----
...
+
`--follow`:: (Optional) Streams the build logs until the build completes.
...
. Validate whether the image has been pushed to the registry that is specified in the `build.spec.output.image` field:
...
[NOTE]
====
To deploy the built image, create a Deployment that references the image from the internal registry. For more information, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/building_applications/deployments[Deployments] in the {ocp-product-title} documentation.
====
