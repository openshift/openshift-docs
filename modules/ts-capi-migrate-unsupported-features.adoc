...
* {aws-short} machine templates that use Amazon EC2 Instance Metadata Service configuration options cannot migrate to the Machine API unless the `httpPutResponseHopLimit` and `httpTokens` fields use their default values.
//OCPCLOUD-2710
...
* When migrating a Machine API resource to the Cluster API, the ignition version is hardcoded and might not match the user data secret that is passed through.
//OCPCLOUD-2719
