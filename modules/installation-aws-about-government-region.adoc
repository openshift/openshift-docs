// Module included in the following assemblies:
//
// * installing/installing_aws/installing-aws-government-region.adoc
// * installing/installing_aws/installing-aws-secret-region.adoc

ifeval::["{context}" == "installing-aws-government-region"]
:aws-gov:
endif::[]
ifeval::["{context}" == "installing-aws-secret-region"]
:aws-secret:
endif::[]

:_mod-docs-content-type: CONCEPT
:chunk-to-content:
[id="installation-aws-about-gov-secret-region_{context}"]
ifdef::aws-gov[]
= AWS government regions

endif::aws-gov[]
ifdef::aws-secret[]
= AWS secret regions

endif::aws-secret[]

[role="_abstract"]
ifdef::aws-gov[]
{product-title} supports deploying a cluster to an AWS GovCloud (US) region.

The following AWS GovCloud partitions are supported:

* `us-gov-east-1`
* `us-gov-west-1`

See "AWS GovCloud (US)" for more information.
endif::aws-gov[]

ifdef::aws-secret[]
{product-title} supports deploying a cluster to an AWS Secret Cloud region.

The following AWS secret partitions are supported:

* `us-isob-east-1` (SC2S)
* `us-iso-east-1` (C2S)

[NOTE]
====
The maximum supported MTU in an AWS SC2S and C2S Regions is not the same as
AWS commercial. For more information about configuring MTU during installation,
see the _Cluster Network Operator configuration object_ section in _Installing
a cluster on AWS with network customizations_
====
endif::aws-secret[]

ifdef::aws-gov[]
[role="_additional-resources"]
.Additional resources

* link:https://aws.amazon.com/govcloud-us[AWS GovCloud (US) ({aws-first} documentation)]
endif::aws-gov[]

ifeval::["{context}" == "installing-aws-government-region"]
:!aws-gov:
endif::[]
ifeval::["{context}" == "installing-aws-secret-region"]
:!aws-secret:
endif::[]
