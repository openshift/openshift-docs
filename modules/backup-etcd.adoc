...
====
* If the scheduler has a default node selector set, create a temporary project that overwrites the default node selector:
+
[source,terminal]
----
$ oc adm new-project <temporary_project> --node-selector=""
----
+
This temporary project is where you will run the `oc debug` command in this procedure.
...
----
+
If the scheduler has a default node selector set, run this command in the temporary project that you created prior to starting this procedure:
+
[source,terminal]
----
$ oc debug node/<node_name> -n <temporary_project>
----
