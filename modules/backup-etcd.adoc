...
. If the cluster-wide proxy is enabled, to export the `NO_PROXY`, `HTTP_PROXY`, and `HTTPS_PROXY` environment variables, run the following commands:
+
[source,terminal]
----
...
* `static_kuberesources_<datetimestamp>.tar.gz`: This file has the resources for the static pods. If etcd encryption is enabled, it also contains the encryption keys for the etcd snapshot.
+
[NOTE]
====
