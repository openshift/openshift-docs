...
You can also attach an IPoIB interface to a network bond. For this configuration, you must set `active-backup.` mode for the node network so that failover redundancy applies to network connections.
...
. Optional: If you want to attach an IPoIB interface to a network bond, add the following configuration to the (NNCP) manifest file:
+
[source,yaml]
----
apiVersion: nmstate.io/v1
...
`<name>`:: Create a Linux network bond, such as `bond0`.

`<mode>`:: Specify `active-backup` mode.

`<port>`:: List the physical network interfaces that are members of the bond network.
--
