...
[id='router-environment-variables-{context}']
...
|`ROUTER_HAPROXY_CONFIG_MANAGER` | | When set to `true` or `TRUE`, enables a dynamic configuration manager with HAproxy, which can manage certain types of routes and reduce the amount of HAproxy router reloads.
