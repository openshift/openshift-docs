...
. Configure the VPN connection. Log in to the {product-title} AWS Account Dashboard, and navigate to the VPC Dashboard.
...
. Establish the VPN connection. From the VPC dashboard, under Virtual private network (VPN) click on *Site-to-Site VPN connections*.
...
====
. Enable VPN route propagation. After you have set up the VPN connection, ensure that route propagation is enabled so that the necessary routes are added to the VPC’s route table. From the VPC Dashboard, under Virtual private cloud, click on *Route tables*.
