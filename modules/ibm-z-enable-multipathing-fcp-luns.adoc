...
:navtitle: Enable multipathing for FCP LUNs
[id="enabling-multipathing-fcp-luns_{context}"]
= Enabling multipathing for FCP LUNs

[role="_abstract"]
After installation, you can enable multipathing for Fibre Channel Protocol (FCP) logical unit numbers (LUNs) on {ibm-z-name} or {ibm-linuxone-name} nodes. This configuration persists across node restarts, but you must redo the steps if you replace the node.
