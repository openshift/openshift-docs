// Module included in the following assemblies:
//
// * ai_applications/mcp_server/mcp-server-overview.adoc

:_mod-docs-content-type: PROCEDURE
[id="ai-app-mcp-server-configure-rbac_{context}"]
= Configure RBAC enforcement

[role="_abstract"]
To control which cluster resources the Model Context Protocol (MCP) server can access, configure role-based access control (RBAC) so that AI agents are limited to only the resources they need.

By default, RBAC is enabled, and you can extend the ClusterRoles, ClusterRoleBindings, Roles, and Rolebindings.

.Prerequisites
* Access to OpenShift console with admin rights.
* MCP gateway is installed.
* MCP Lifecycle Operator is enabled.
* The MCP server is installed.
* MCP gateway is configured and verified.
* MCP-compatible client connected.

.Procedure
. Configure RBAC enforcement by extending ClusterRoles, ClusterRoleBindings, Roles, and Rolebindings.
+
Use the following example RBAC YAML file to extend ClusterRoles, ClusterRoleBindings, Roles, and Rolebindings:
+
.Example RBAC settings YAML file
[source,yaml]
----
  apiVersion: rbac.authorization.k8s.io/v1
  kind: ClusterRole
  metadata:
    name: acm-managed-clusters
  rules:
    ...
  ---
  apiVersion: rbac.authorization.k8s.io/v1
  kind: ClusterRoleBinding
  metadata:
    name: acm-managed-clusters
    ...
  ---
  apiVersion: rbac.authorization.k8s.io/v1
  kind: Role
  metadata:
    name: acm-cluster-proxy-discovery
    namespace: multicluster-engine
    ...
  ---
  apiVersion: rbac.authorization.k8s.io/v1
  kind: RoleBinding
  metadata:
    name: acm-cluster-proxy-discovery
    namespace: multicluster-engine
    ...
----

. Apply the RBAC configuration by saving the preceding example to a file (for example, `rbac-values.yaml`), and then run the following command:
+
[source,terminal]
----
$ oc apply -f rbac-values.yaml
----
+
[NOTE]
====
If you enable the `ovn-kubernetes` toolset, grant the MCP server service account `get` and `list` on pods and `create` on `pods/exec` so that tools can run OVN and OVS commands inside `ovnkube-node` pods.

If you enable the `cni-diagnostics` toolset, grant permissions to list nodes, create and delete pods, and use `pods/exec` and `pods/log` so that the server can run privileged debug pods and pod-level packet capture.

For complete ClusterRole examples for both toolsets, see "Troubleshooting OVN-Kubernetes with the MCP server" in the Networking documentation.
====

