...
[role="_abstract"]
File-based catalogs use a format that is based on the CUE language specification and that can be extended with arbitrary schemas.

The following `_Meta` CUE schema defines the format that all file-based catalog blobs must adhere to:
