...
. Create a GCS bucket on the {gcp-full}.
...
SERVICE_ACCOUNT_EMAIL=$(gcloud iam service-accounts create <iam_service_account_name> \ # <1>
    ...
    --quiet)
----
<1> The name of the service account on the {gcp-full}.
<2> The project ID of the service account on the {gcp-full}.

. Bind the required {gcp-full} roles to the created service account at the project level. You can do this by running the following command:
+
[source,terminal]
----
