...
spec:
  extends: rhcos4-moderate # Base profile to modify (optional)
...
status:
  id: xccdf_compliance.openshift.io_profile_rhcos4-with-usb # Generated XCCDF identifier
  outputRef:
    name: rhcos4-with-usb-tp # Name of the generated tailoring ConfigMap
    namespace: openshift-compliance
  state: READY # Object validation state (e.g., READY, PENDING, ERROR)
----

* **extends**: This is optional. Name of the `Profile` object upon which the `TailoredProfile` is built. If no value is set, a new profile is created from the `enableRules` list.
* **id**: Specifies the XCCDF name of the tailored profile.
* **outputRef**: Specifies the `ConfigMap` name, which can be used as the value of the `tailoringConfigMap.name` attribute of a `ComplianceScan`.
* **state**: Shows the state of the object such as `READY`, `PENDING`, and `FAILURE`. If the state of the object is `ERROR`, then the attribute `status.errorMessage` provides the reason for the failure.

With the `TailoredProfile` object, it is possible to create a new `Profile` object by using the `TailoredProfile` construct. To create a new `Profile`, set the following configuration parameters :
