...
zero trust::
A security architecture that requires verification for every interaction between network endpoints. Requests for access to specific services and other communication needs to be dynamically validated using identity management and other access controls before an endpoint can be trusted.
