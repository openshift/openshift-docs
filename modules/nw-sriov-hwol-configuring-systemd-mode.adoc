...
* Verify that the SR-IOV Network Operator is configured in `systemd` mode:
+
[source,terminal]
----
$ oc get sriovoperatorconfig default -n openshift-sriov-network-operator -o jsonpath='{.spec.configurationMode}'
----
+
.Example output
[source,terminal]
...
* Verify that all SR-IOV Operator pods are running after the configuration change:
+
[source,terminal]
----
...
network-resources-injector-6b7d6d5b5-xkq5h   1/1     Running   0          5m
operator-webhook-5c4f7d4f6b-2r9kl             1/1     Running   0          5m
sriov-network-config-daemon-7fpbh              1/1     Running   0          5m
sriov-network-operator-7b8c8d6b5-9j2kl        1/1     Running   0          5m
----