...
** cgroups v1: {product-title} workloads can use any combination of memory and swap, up to the pod's memory limit, if set.
** cgroups v2: {product-title} workloads cannot use swap memory.
...
* You have enabled the `TechPreviewNoUpgrade` feature set on the cluster (see _Nodes -> Enabling features using feature gates_).
+
[NOTE]
====
