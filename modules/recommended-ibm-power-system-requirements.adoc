...
[role="_abstract"]
{product-title} on {ibm-power-name} requires a minimum of six LPARs across multiple PowerVM servers, with specific hardware, storage, and network configuration for each node type.
...
* One instance of an {ibm-power-name}9, {ibm-power-name}10 or {ibm-power-name}11 processor-based system for {op-system-base}9 based {op-system} nodes
* One instance of an {ibm-power-name}10 or {ibm-power-name}11 processor-based system for {op-system-base}10 based {op-system} nodes
