...
= Deploying a cluster quickly

By default, the installation program acts as an installation wizard, prompting you for required values that it cannot determine, such as the platform and base domain of the cluster, and then providing default values for the remaining cluster parameters.

Using the default parameters is the quickest way to deploy a cluster.

[NOTE]
====
In all other installation use cases, you must first create the installation configuration file (`install-config.yaml`), and modify it before deploying the cluster. For more information, see "Deploying a cluster with customizations".
====

.Prerequistes

ifndef::osp,rhv,vsphere,nutanix[]
* Configure an account with the cloud platform that hosts your cluster.
endif::osp,rhv,vsphere,nutanix[]

ifdef::rhv[]
* Open the `ovirt-imageio` port to the {rh-virtualization-engine-name} from the machine running the installer. By default, the port is `54322`.
endif::rhv[]
...
ifeval::["{context}" == "installing-vsphere-ipi"]
:!vsphere:
endif::[]
