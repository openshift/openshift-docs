...
[NOTE]
====
The `.ins` file and `initrd.img.addrsize` are not automatically generated as part of boot-artifacts from the installer. You must manually generate these files.
====

.. Run the following script to get the size of the `kernel` and `initrd`: 
+
[source,terminal]
----
...
----
.. Round the `kernel` size up to the next megabytes (MB) boundary. This value is the starting address of `initrd.img`: 
+
[source,terminal]
----
offset=$(( (kernel_size + 1048575) / 1048576 * 1048576 ))
----
.. Create the kernel binary patch file that contains the `initrd` address and size by running the following script:
+
[source,terminal]
----
