...
. Set the `frontendipconfig_id` variable by running this command:
+
[source,terminal]
----
...
. Set the `frontendipconfig_name` variable by running this command:
+
[source,terminal]
----
...
. Set the `frontendpublicip_id` variable by running this command:
+
[source,terminal]
----
$ frontendpublicip_id=$(az network lb frontend-ip show -n ${frontendipconfig_name} --lb-name ${infra_id} -g ${cluster_resource_group_name} --query "publicIPAddress.id" -otsv)
----
