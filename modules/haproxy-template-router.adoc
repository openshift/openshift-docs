...
[id='haproxy-template-router-{context}']
= About the HAProxy router in {product-title}

A router can be assigned to a node to control traffic in an {product-title}
cluster. {product-title} uses HAProxy as the default router
