...
Error: `Account with ID <username> denied access to perform create on NotificationContact`::
This error occurs when you are logged in to the cluster using an account that does not have the cluster owner or cluster editor roles. Log in to a different account that has these roles and try again.

Error: `Could not find any account identified by <email-address>`::
