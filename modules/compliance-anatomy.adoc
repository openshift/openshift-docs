...
= Anatomy of a scan
:context: compliance-anatomy-sections
...
Sometimes, the `ComplianceSuite` CR cannot create a scan if the scan exceeds the default memory limit of the Compliance Operator. Exceeding this limit can result in an Out of Memory (OOM) error message. You can increase the memory for the Compliance Operator by setting custom resource limits.

To increase the default memory and CPU limits of scanner pods, see xref:../compliance-scansetting-cr.adoc[`ScanSetting` Custom resource].

The following example creates a patch file named `co-memlimit-patch.yaml` that sets a custom resource memory limit of 500 Mi for the Compliance Operator:
...
The following example applies the patch file to the Compliance Operator:
