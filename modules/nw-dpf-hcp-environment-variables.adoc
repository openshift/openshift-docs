...
Set these values before creating secrets, the `DPUCluster` resource, or the `DPFHCPProvisioner` resource.
...
|`BLUEFIELD_OCP_IMAGE`
|The BlueField {product-title} layer container image URL. When set, this value is used as `machineOSURL` in the `DPFHCPProvisioner` resource and skips automatic image lookup.
