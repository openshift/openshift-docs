...
:navtitle: Evict KubeVirt virtual machines
...
[source,terminal]
----
$ oc explain hyperconverged.spec.virtualization.evictionStrategy
----
...
$ oc patch -n {CNVNamespace} {HCOCliKind} kubevirt-hyperconverged \
  --type=merge \
  -p '{"spec": {"virtualization": {"evictionStrategy": "External"}}}'
----
...
[source,terminal,subs="attributes+"]
----
$ oc get -n {CNVNamespace} {HCOCliKind} kubevirt-hyperconverged -ojsonpath='{.spec.virtualization.evictionStrategy}'
----
+
.Example output
[source,terminal]
