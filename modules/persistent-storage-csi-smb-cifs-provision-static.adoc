...
spec:
  ...
  csi:
    ...
    volumeHandle: smb-server.default.svc.cluster.local/share#subdir#app-data <2>
    volumeAttributes:
...
<2> `volumeHandle` format: {smb-server-address}#{sub-dir-name}#{share-name}. Ensure that this value is unique for every share in the cluster. The `sub-dir-name` is optional. If you are using a sub directory, the sub directory must be exist or the mount will fail.
