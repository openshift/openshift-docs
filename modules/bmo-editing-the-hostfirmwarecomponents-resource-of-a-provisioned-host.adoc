...
[role="_abstract"]
Edit the `HostFirmwareComponents` resource to update BIOS, BMC, or NIC firmware on provisioned hosts, ensuring you maintain current firmware versions for security patches and performance improvements.
...
+
--
`$ oc edit <hostname> hostfirmwarecomponents -n openshift-machine-api`:: Where `<hostname>` is the name of the host. The `HostFirmwareComponents` resource will open in the default editor for your terminal.
--
...
+
--
`- name: bios`:: To set a BIOS version, set the `name` attribute to `bios`.
`url: https://myurl.with.firmware.for.bios`:: To set a BIOS version, set the `url` attribute to the URL for the firmware version of the BIOS.
`- name: bmc`:: To set a BMC version, set the `name` attribute to `bmc`.
`url: https://myurl.with.firmware.for.bmc`:: To set a BMC version, set the `url` attribute to the URL for the firmware version of the BMC.
--
...
+
--
`$ oc get bmh <host_name> -n openshift-machine name`:: Where `<host_name>` is the name of the host. The terminal displays the machine name under the `CONSUMER` field.
--
...
+
--
`$ oc annotate machine <machine_name> machine.openshift.io/delete-machine=true -n openshift-machine-api`:: Where `<machine_name>` is the name of the machine to delete.
--
...
+
--
`$ oc scale machineset <machineset_name> -n openshift-machine-api --replicas=<n-1>`:: Where `<machineset_name>` is the name of the machine set and `<n-1>` is the decremented number of worker nodes.
--
...
+
--
`$ oc scale machineset <machineset_name> -n openshift-machine-api --replicas=<n>`:: Where `<machineset_name>` is the name of the machine set and `<n>` is the number of worker nodes.
--
