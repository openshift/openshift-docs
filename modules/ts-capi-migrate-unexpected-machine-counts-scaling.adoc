...
The two-way synchronization controller that handles resource conversion between authoritative API types does not synchronize values in the `.status` stanza.
+
The value in the `DESIRED` column in the output of the `oc get` command for a compute machine set reflects the `.spec.replicas` value of a compute machine set.
...
. Scale the migrated machine set up by setting a larger value in the `.spec.replicas` field.
. The machine set creates machines with the current authoritative API to satisfy the number of requested replicas.
. Scale the machine set down such that one of the following conditions causes the deletion of machines that do not use the current authoritative API:
** Total number of replicas requested is fewer than the number of machines that do not use the current authoritative API.
...
To verify that the scale-down operation successfully deleted the compute machines, run the `oc get` command that lists the compute machines.

Result::

If the scale-down operation succeeded, the count in the output of the `oc get` command for the compute machines reflects the `.spec.replicas` value of the machine set.
