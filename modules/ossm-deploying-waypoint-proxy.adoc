...
= Deploy waypoint proxies for Bookinfo
...
----
+
After enrolling the namespace, requests from any pods by using the ambient data plane to services in `bookinfo` will route through the waypoint for L7 processing and policy enforcement.

.Verification

. Confirm that the waypoint proxy manages all the services in the `bookinfo` namespace by running the following command:
+
[source,terminal]
----
