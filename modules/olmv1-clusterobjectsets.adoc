...
The `ClusterObjectSet` tracking mechanism applies strictly to standard in-cluster Kubernetes objects, such as Deployments, Services, and CRDs. If an operator provisions external infrastructure resources directly inside your cloud account, such as AWS Elastic Load Balancers or IAM objects, those external entities exist outside the tracking bounds of the `ClusterObjectSet` state record.
endif::[]
