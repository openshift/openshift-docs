...
[id="otel-forwarding-logs-to-lokistack_{context}"]
...
+
The following is an example `ServiceAccount` object:
+
[source,yaml]
----
apiVersion: v1
...
+
The following is an example `ClusterRole` object:
+
[source,yaml]
----
...
+
The following is an example `ClusterRoleBinding` object:
+
[source,yaml]
----
apiVersion: rbac.authorization.k8s.io/v1
...
+
The following is an example `OpenTelemetryCollector` CR object:
+
[source,yaml]
----
...
`attributes`:: Provides the following resource attributes to be used by the web console: `kubernetes.namespace_name`, `kubernetes.pod_name`, `kubernetes.container_name`, and `log_type`.
`extensions`:: Enables the BearerTokenAuth Extension that is required by the OTLP HTTP Exporter.
`exporters`:: Enables the OTLP HTTP Exporter to export logs from the Collector.
