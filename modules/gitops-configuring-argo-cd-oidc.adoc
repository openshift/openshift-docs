...
where:
+
`issuer`:: Specifies `issuer` must end with the correct realm name (in this example `myrealm`).
`clientID`:: Specifies `clientID` is the Client ID you configured in your Keycloak account.
`clientSecret`:: Specifies `clientSecret` points to the right key you created in the argocd-secret secret.
`requestedScopes`:: Specifies `requestedScopes` contains the groups claim if you did not add it to the Default scope.
