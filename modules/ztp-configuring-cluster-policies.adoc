...
|Sites
|Site-specific values that differ per cluster are supplied through `ConfigMap` resources and hub-side templating in common or group `{policy-gen-cr}` CRs, rather than through separate per-site policy CR files.
|====
