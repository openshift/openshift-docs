...
* When a configuration is unique to an individual site, use hub-side templating with `ConfigMap` resources to inject group or site-specific data into the common or group policy.
====
...
|Sites
|Site-specific values that differ for each cluster are supplied through `ConfigMap` resources and hub-side templating in common or group `{policy-gen-cr}` CRs, rather than through separate site-specific `Policy` CR files.
|====
