...
Use the {k8s} Gateway API to create `Gateway` and `HTTPRoute` resources and deploy a gateway. The resources configure the gateway to expose a service in the mesh to traffic outside the mesh. Then, set the the Service for the gateway to `LoadBalancer` to expose the gateway to traffic outside the cluster.
