...
spec:
  ...
  - backendRefs:
    - name: httpbin
      port: 8000
----
* `spec.parentRefs` binds the `HTTPROUTE` resource to the {k8s} `Gateway` resource you created in the previous step.
...
[source,terminal]
----
$ INGRESS_PORT=$(oc get gtw <gateway_name> -n <gateway_namespace> -o jsonpath='{.spec.listeners[?(@.name=="default")].port}')
----
