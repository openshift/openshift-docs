...
:navtitle: Delete an LVMCluster CR by using the web console
[id="lvms-deleting-lvmcluster-using-web-console_{context}"]
= Deleting an LVMCluster CR by using the web console

[role="_abstract"]
You can delete an `LVMCluster` custom resource (CR) when decommissioning {lvms} or reconfiguring storage by using the {product-title} web console.
