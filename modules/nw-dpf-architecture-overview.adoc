...
[role="_abstract"]
The NVIDIA DOCA Platform Framework (DPF) v26.4 deployment on {product-title} {product-version} creates a dual-cluster topology that offloads OVN-Kubernetes data plane operations to NVIDIA BlueField-3 DPUs.

In Host Trusted mode, DPF uses BlueField DPUs as host accelerators. Administrators can orchestrate both workloads and DPU-accelerated infrastructure by using standard {product-title} APIs and custom resource definitions (CRDs).
