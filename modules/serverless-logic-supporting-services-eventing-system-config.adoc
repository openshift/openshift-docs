...
This section describes the supporting services eventing system configurations for a SonataFlow workflow.
...
A service-scoped eventing system configuration allows for fine-grained control over the eventing system, specifically for the Data Index or the Jobs Service.
...
To configure a service-scoped eventing system for the Jobs Service, you must use the `spec.services.jobService.source.ref` and `spec.services.jobService.sink.ref` fields in the `SonataFlowPlatform` CR. These fields instruct the {ServerlessLogicOperatorName} to automatically link the Jobs Service to consume and produce SonataFlow system events, based on the provided configuration.
...
The following example displays the Jobs Service eventing system configuration:

.Example of Jobs Service eventing system configuration 
[source,yaml]
----
apiVersion: sonataflow.org/v1alpha08
kind: SonataFlowPlatform
...
<1> Specifies the Knative Eventing Broker from which the Jobs Service consumes events.
...
<3> Specifies the Knative Eventing Broker on which the Jobs Service produces events.
...
1. If the supporting service has its own eventing system configuration, using either the Data Index eventing system or Jobs Service eventing system configuration, that configuration takes precedence.
...
<1> Used by the Data Index, Jobs Service, and workflows, unless overridden.
<2> Data Index ephemeral deployment, configures the Data Index service.
<3> Jobs Service ephemeral deployment, configures the Jobs Service.
...
The following command displays the list of triggers set up for the Data Index and Jobs Service events, showing which services are subscribed to the events:
...
To see the SinkBinding for the Jobs Service, use the following command:
