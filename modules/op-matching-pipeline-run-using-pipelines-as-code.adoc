...
[id="matching-pipeline-run-use-pipelines-as-code_{context}"]
= Match pipeline runs to events
...
[NOTE]
====
If you use the `header` or `body` field for event matching, you might be unable to trigger the pipeline run by using Git commands such as `retest`. If you use a Git command, the payload body is the comment that has this command, and not the original payload.
