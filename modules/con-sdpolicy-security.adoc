...
As an administrator of {product-title}, you can access the `cluster-admin` role within your organization’s cluster. While logged into an account with the `cluster-admin` role, users have increased permissions to run privileged security contexts.
