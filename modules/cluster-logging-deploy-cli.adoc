...
<3> Specifying `Automatic` allows Operator Lifecycle Manager (OLM) to automatically update the Operator when a new version is available. Specifying `Manual` requires a user with appropriate credentials to approve the Operator update.
