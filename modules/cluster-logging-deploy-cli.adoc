...
spec:
  ...
  - openshift-logging <2>
----
<1> For logging versions 5.7 and older, `ClusterLogForwarder` and `openshift-logging` custom resource can only be installed in the `openshift-logging` namespace. So, you must specify the `openshift-logging` namespace.
<2> For logging versions 5.8 and later, `ClusterLogForwarder` and `openshift-logging` custom resource can be installed in any namespace. You can leave `targetNamespaces` empty to make it available in all namespaces or specify a list of namespaces.
