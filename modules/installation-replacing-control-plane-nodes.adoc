...
[NOTE]
====
Take an etcd backup before you begin to ensure that you can restore the cluster if any issues occur.
====

[IMPORTANT]
====
If both control plane nodes lost power and only one node can be restarted, you must restore corosync quorum on the surviving node before proceeding with this replacement procedure. See "Restore corosync quorum after dual node power loss".
====
...
[NOTE]
====
If the output shows `Quorate: No`, both nodes lost power and corosync quorum was not restored. Return to the step to restore corosync quorum before proceeding.
====

.. If quorum is lost and one control plane node is still running, restore quorum manually on the survivor node.
+
[source,terminal]
----
...
.. If only one node failed, verify that etcd is running on the survivor node.
+
[source,terminal]
----
...
.. If etcd is not running, restart etcd.
+
[source,terminal]
----
...
[IMPORTANT]
====
Before running these commands, ensure that the replacement node is inaccessible. Otherwise, you risk etcd corruption.
====
+
[source,terminal]
----
...
. Delete etcd secrets for the failed node.
+
[source,terminal]
----
...
[NOTE]
====
To replace the failed node, you must delete its etcd secrets first. When etcd is running, the API server might take some time to respond to these commands.
====

. Delete resources for the failed node:

.. If you have the `BareMetalHost` (BMH) objects, list them to identify the host you are replacing.
+
[source,terminal]
----
...
.. Delete the BMH object for the failed node.
+
[source,terminal]
----
...
.. List the `Machine` objects to identify the object that maps to the node that you are replacing.
+
[source,terminal]
----
...
.. Get the label with the machine hash value from the `Machine` object.
+
[source,terminal]
----
...
.. Delete the `Machine` object for the failed node.
+
[source,terminal]
----
...
[NOTE]
====
Deleting the `Machine` object automatically deletes the node object.
====
...
[IMPORTANT]
====
Perform this step only if you are using installer-provisioned infrastructure or the Machine API to create the original node.  
...
.. Create the new BMH object and the secret to store the BMC credentials.
+
[source,terminal]
----
cat <<EOF | oc apply -f -
...
----
+
* Secret `metadata.name`: Specify the name of the secret.
* BareMetalHost `metadata.name`: Replace `{bmh_name}` with the name of the BMH object that you deleted.
...
. Verify that the new node has reached the `Provisioned` state.
+
[source,terminal]
----
$ oc get bmh -o wide
...
. Verify that both control plane nodes are in the `Ready` state.
+
[source,terminal]
----
$ oc get nodes
...
. Apply the `detached` annotation to the BMH object to prevent the Machine API from managing it.
+
[source,terminal]
----
...
. Rejoin the replacement node to the Pacemaker cluster.
+
[NOTE]
====
...
. Delete stale jobs for the failed node.
+
[source,terminal]
----
$ oc project openshift-etcd
