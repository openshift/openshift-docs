...
= Replacing control plane nodes in a two-node {product-title} cluster with fencing

[role="_abstract"]
You can replace a failed control plane node in a two-node {product-title} cluster. The replacement node must use the same host name and IP address as the failed node.
...
. Update the fencing secret so that STONITH can fence the new node. You must update the secret if the replacement node has a different BMC system UUID than the failed node.
+
[NOTE]
====
...
* For clusters using MAC-based fencing credentials, run `update-fencing-credentials.sh` from a TNF control-plane node. The script locates the existing secret by hostname, MAC hash, or the existing STONITH address, then updates the secret and STONITH configuration.
====
+
...
.. Update the fencing secret for the replacement node by running the following command:
+
[source,terminal]
----
...
* Replace `<node_name>` with the hostname of the replacement node as it appears in `oc get nodes` command.
...
+
* Replace `<Disabled_or_Enabled>` with `Disabled` to skip certificate validation and match `disableCertificateVerification: true` on the BMH object or `Enabled` if your BMC has a valid TLS certificate.
...
. Delete stale jobs for the failed node by running the following commands:
+
[source,terminal]
----
$ oc project openshift-etcd
