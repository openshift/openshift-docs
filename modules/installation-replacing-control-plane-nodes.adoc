...
You can replace a failed control plane node in a two-node OpenShift cluster by using the same host name and IP address.

.Prerequisites

* You have a functioning survivor control plane node.  
* You have verified that the machine is not running or that the node is not in the `Ready` state.
* You have access to the cluster as a user with the `cluster-admin` role.
* You know the host name and IP address of the failed node.

[NOTE]
====
Back up etcd before proceeding to ensure that you can restore the cluster if issues occur.
====
...
[IMPORTANT]
====
Ensure that the node you are replacing is inaccessible before you proceed. Operating with an accessible failed node can cause etcd corruption.
====
+
[source,terminal]
----
...
[NOTE]
====
To replace the failed node, you must delete its etcd secrets first. The API server can take some time to respond when etcd is running.
====
...
[NOTE]
====
Deleting the `Machine` object also deletes the node object.
====
...
[IMPORTANT]
====
Perform this step only for installer-provisioned infrastructure or if you used the Machine API to create the node. 
...
* `spec.providerSpec.value.image.checksum`: Replace `<image_checksum>` with the checksum of the image used to provision the node.
* `spec.providerSpec.value.image.url`: Replace `<image_url>` with the URL of the image used to provision the node.
...
[NOTE]
====
Run both commands on the survivor control plane node, not on the node being replaced.
====
+
[source,terminal]
----
