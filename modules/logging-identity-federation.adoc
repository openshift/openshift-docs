...
= Authenticate with workload identity federation to access cloud-based log stores

[role="_abstract"]
Use workload identity federation with short-lived tokens to eliminate long-lived credentials.
...
* Your cluster was installed with workload identity federation enabled. Workload identity federation is available on clusters installed with the Cloud Credential Operator in manual mode with short-lived credentials, such as AWS STS, Azure Workload Identity, or GCP Workload Identity Federation. It is not available on clusters that use standard long-lived cloud credentials or on on-site installations.
