...
NOTE: This example uses the Let's Encrypt TLS certificate issuer for simplicity, but you can use any certificate issuer supported by `cert-manager`. In multicluster environments, you must add your TLS issuer in each {ocp} cluster.
