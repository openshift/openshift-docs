...
[role="_abstract"]
You can use the Machine Config Operator to manage system updates, including the kernel, Network Manager, and CRI-O, and write host configuration files through the `MachineConfig` custom resource definition.

Understanding what MCO does and how it interacts with other components is critical to making advanced, system-level changes to an {product-title} cluster. Here are some things you should know about MCO, machine configs, and how they are used:
