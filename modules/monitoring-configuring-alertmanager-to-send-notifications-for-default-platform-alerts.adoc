...
[id="configuring-alertmanager-to-send-notifications-for-default-platform-alerts_{context}"]
= Configuring the Alertmanager to send notifications for default platform alerts

You can customize the default platform instance of Alertmanager to send notifications by overwriting the default Alertmanager configuration. You can achieve this by editing the `alertmanager-main` secret in the `openshift-monitoring` namespace.
...
<4> Specify the service that fires the alerts.
<5> Specify the target alerts.
<6> Specify the receiver to use for the alert.
...
----
+
With this configuration, alerts of `critical` severity that are fired by the `example-app` service are sent by using the `team-frontend-page` receiver. Typically, these types of alerts would be paged to an individual or a critical response team.
