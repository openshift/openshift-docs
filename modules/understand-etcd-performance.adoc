...
[role="_abstract"]
Disk latency, network latency and jitter, consensus delay, database size, and Kubernetes API transaction rate all influence etcd reliability on {product-title}.
...
[id="etcd-perf-disk-latency_{context}"]
...
[id="etcd-perf-network-latency-jitter_{context}"]
...
[id="etcd-perf-consensus-latency_{context}"]
...
[id="etcd-perf-kube-api_{context}"]
