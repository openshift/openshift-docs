...
. Verify that the collector is listening on the service that has a name in the format `<ClusterLogForwarder_CR_name>-<input_name>`. In this example, the service name is `collector-http-receiver`:
+
[source,terminal]
----
$ oc get svc
...
. Extract the certificate authority (CA) certificate file:
+
[source,terminal]
----
...
. Send logs by using the `curl` command:
+
[source,terminal]
----
$ curl --cacert <openshift-service-ca.crt> https://collector-http-receiver.<namespace>.svc:8443 -XPOST -d '{"<prefix>":"<msessage>"}'
----
+
Replace <openshift-service-ca.crt> with the extracted CA certificate file.
+
[NOTE]
====
