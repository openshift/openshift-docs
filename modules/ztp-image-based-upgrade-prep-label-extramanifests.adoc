...
. Label your required extra manifests with the `lca.openshift.io/target-ocp-version: <target_version>` label in your existing site `PolicyGenerator` CR:
+
        ...
        - path: source-crs/sriov-operator/SriovNetwork.yaml
          patches:
            - metadata:
                name: "sriov-nw-du-fh"
                labels:
                    lca.openshift.io/target-ocp-version: "{product-version}"
              spec:
                resourceName: du_fh
                vlan: 140
        - path: source-crs/sriov-operator/SriovNetworkNodePolicy.yaml
          patches:
            - metadata:
