...
{gcp-first} Compute Engine enables users to add GPUs to VM instances. Workloads that benefit from access to GPU resources can perform better on compute machines with this feature enabled. {product-title} on {gcp-first} supports NVIDIA GPU models in the A2 and N1 machine series.
