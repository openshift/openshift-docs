...
= Configure the hub cluster for backup and restore
...
[IMPORTANT]
====
When you restore `BareMetalHost` resources as part of restoring the cluster activation data, you must restore the `BareMetalHost` status.
The following {rh-rhacm} `Restore` resource example restores activation resources, including `BareMetalHost`, and also restores the status for the `BareMetalHost` resources:
[source,yaml]
----
apiVersion: cluster.open-cluster-management.io/v1beta1
kind: Restore
