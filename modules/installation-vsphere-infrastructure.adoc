...
A limitation of using VPC is that the Storage Distributed Resource Scheduler (SDRS) is not supported. See link:https://vmware.github.io/vsphere-storage-for-kubernetes/documentation/faqs.html#is-sdrs-supported-on-vms-hosting-kubernetes-cluster[vSphere Storage for Kubernetes FAQs ] in the VMware documentation.
====
