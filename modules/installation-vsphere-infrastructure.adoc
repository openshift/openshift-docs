...
[IMPORTANT]
====
Support for a cluster on VMware vSphere version 7.0.1 or earlier is now removed. If your vSphere nodes are below hardware version 15 or your VMware vSphere version is earlier than 7.0.2, upgrading from {product-title} 4.11 to {product-title} 4.12 is not available.
====

[IMPORTANT]
====
{product-title} 4.13 will require vSphere CSI driver version shipped by Red Hat to be installed. Please remove any other vSphere CSI driver, including its Deployment, DaemonSet and CSIDriver objects prior upgrading to 4.13. Red Hat's version of the driver will be installed automatically.
====
...
|Hypervisor
|vSphere 7.0.2 and later with HW version 15
