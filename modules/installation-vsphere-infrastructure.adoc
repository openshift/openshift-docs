...
[NOTE]
====
Red Hat follows Broadcom's End of Support dates for VMware products that {product-title} runs on. After a VMware product version reaches End of Support, that version is no longer supported for use with {product-title}.
====
...
[IMPORTANT]
====
Version 4.12 of {product-title} requires VMware virtual hardware version 15 or later. To update the hardware version for your vSphere virtual machines, see the "Updating hardware on nodes running in vSphere" article in the _Updating clusters_ section.
====
