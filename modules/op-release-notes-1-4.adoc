...
{pipelines-title} General Availability (GA) 1.4 is now available on {OCP} 4.7.

[NOTE]
====
In addition to the stable and preview Operator channels, the {pipelines-title} Operator 1.4.0 includes the ocp-4.6, ocp-4.5, and ocp-4.4 deprecated channels. These deprecated channels and support for them will be removed in the following release of {pipelines-title}.
====
...
[NOTE]
====
Support for `when` expressions and `finally` tasks are unavailable in the {OCP} 4.7 web console.
====
...
* The `pipelines-scc` Security Context Constraint (SCC) is used with the default `pipeline` service account for pipelines. This new service account is similar to `anyuid`, but with a minor difference as defined in the YAML for SCC of {OCP} 4.7:
+
...
* In the *Developer* perspective, the pipeline metrics and triggers features are available only on {OCP} 4.7.6 or later versions.

* On {ibmpowerProductName} Systems, {ibmzProductName}, and LinuxONE, the `tkn hub` command is not supported.

* When you run Maven and Jib Maven cluster tasks on an {ibmpowerProductName} Systems (ppc64le), {ibmzProductName}, and LinuxONE (s390x) clusters, set the `MAVEN_IMAGE` parameter value to `maven:3.6.3-adoptopenjdk-11`.
...
* In {pipelines-shortname} 1.4.1, the `ConsoleQuickStart` custom resource has been updated to align with {OCP} 4.7 capabilities and behavior.