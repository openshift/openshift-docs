...
[NOTE]
====
Upgrades from {pipelines-shortname} 1.3.x and earlier versions to {pipelines-shortname} 1.4.0 breaks event listeners because of the unavailability of `template.name`. For such cases, use {pipelines-shortname} 1.4.1 to avail `template.name` which has been restored.
====
...
* On X86 architectures, IBM Power Systems, IBM Z, and LinuxONE, a Jib Maven cluster task fails when the docker pull rate limit is exceeded.
...
* In {pipelines-shortname} 1.4.1, the pipeline quickstart custom resource has been updated to align with {product-title} {product-version} capabilities and behavior.
