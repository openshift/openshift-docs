...
|The earlier automatic upgrade failed.
...
|The upgrade is in progress. Before performing automatic upgrade, the upgrade installation program runs a pre-flight check. During the pre-flight check, the installation program verifies if certain conditions satisfy and then only starts the upgrade process.
