...
When multiple EventPolicies apply to the same resource, the system evaluates them in parallel. An event is accepted if it matches at least one applicable EventPolicy. This approach allows flexibility,valid events are not rejected even under strict authorization, as long as they satisfy the requirements of at least one policy.
