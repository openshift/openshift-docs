...
Updater::
The VPA updater checks if the pods in the associated workload object have the correct resources. If the resources are correct, the updater takes no action. If the resources are not correct, the updater ensure that the pods have the updated requests. This action depends upon the VPA update mode, as described in this section.
...
You specify the mode that the VPA operates in:

* The `InPlaceOrRecreate` mode automatically applies the VPA CPU and memory recommendations throughout the pod lifetime. When any pod in the project is out of alignment with the VPA recommendations, the VPA attempts to apply updates _in-place_, without restarting any of the containers on the pod. If the VPA is not able to update the containers in-place, the VPA deletes the pod. When redeployed by the workload object, the VPA updates the new pod and containers with its recommendations. For specific information, see "About the in place or recreate mode". 
