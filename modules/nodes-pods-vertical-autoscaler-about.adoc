...
Updater::
The VPA updater checks if the pods in the associated workload object have the correct resources. If the resources are correct, the updater takes no action. If the resources are not correct, the updater ensure that the pods have the updated requests. This action depends upon the VPA update mode, as described in "About using the Vertical Pod Autoscaler Operator".
