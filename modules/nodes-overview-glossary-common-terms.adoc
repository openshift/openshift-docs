...
[id="nodes-overview-glossary-common-terms_{context}"]
...
[id="commonterms-node-container_{context}"]
Container::
...
[id="commonterms-node-hpa_{context}"]
...
[id="commonterms-node-ingress_{context}"]
Ingress::
Incoming traffic to a pod.


[id="commonterms-node-job_{context}"]
Job::
...
[id="commonterms-node-label_{context}"]
Labels::
You can use labels, which are key-value pairs, to organize and select subsets of objects, such as a pod.


[id="commonterms-node-nodenew_{context}"]
Node::
A worker machine in the {product-title} cluster. A node can be either a virtual machine (VM) or a physical machine.


[id="commonterms-node-tuningop_{context}"]
...
[id="commonterms-node-toleration_{context}"]
Toleration::
...
[id="commonterms-node-taint_{context}"]
Taint::
