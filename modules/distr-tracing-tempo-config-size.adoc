...
= Size deployment for your workload

[role="_abstract"]
Before deploying {dt} on {ocp-product-title}, select an appropriate deployment size profile and understand how to customize resources for individual components so that your tracing infrastructure meets your ingestion and performance requirements.

To use a preset deployment sizing profile, you can configure the `spec.size` field in the `TempoStack` custom resource (CR) as follows:
