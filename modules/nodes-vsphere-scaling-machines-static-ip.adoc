...
+
The following example machine resource YAML file defines static IP address information in the `network` parameter:
+
[source,yaml]
----
...
where:
+
`gateway`:: The IP address for the default gateway for the network interface.
`ipAddrs`:: Lists IPv4, IPv6, or both IP addresses that installation program passes to the network interface. Both IP families must use the same network interface for the default network.
`nameservers`:: Lists a DNS nameserver. You can define up to 3 DNS nameservers. Consider defining more than one DNS nameserver to take advantage of DNS resolution if that one DNS nameserver becomes unreachable.
