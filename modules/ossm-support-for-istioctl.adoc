...
|`proxy-config`, `pc` | Retrieve information about the proxy configuration from Envoy ({k8s} only) 
|`proxy-status`, `ps` | Retrieve the synchronization status of each Envoy in the mesh
|`remote-clusters` | List the remote clusters each `istiod` instance connects to 
...
[NOTE]
====
Any other commands display the _WARNING: Not supported in OpenShift Service Mesh context_ message. Do not use these commands in production environments.
====