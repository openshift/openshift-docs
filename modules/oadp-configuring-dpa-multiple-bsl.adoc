...
. Configure the `DataProtectionApplication` CR with more than one `BackupStorageLocation` CR. See the following DPA example:
+
[source,yaml]
----
...
. Specify the BSL to be used in the backup CR. See the following backup CR example.
+
[source,yaml]
----
...
----
* `<namespace> `: Specify the namespace to back up.
* `<backup_storage_location>`: Specify the storage location.
