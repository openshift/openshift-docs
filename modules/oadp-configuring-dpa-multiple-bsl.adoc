...
You can configure the `DataProtectionApplication` (DPA) custom resource(CR) with more than one `BackupStorageLocation` (BSL) and specify the credentials provided by the cloud provider.

[NOTE]
====
Consider the following scenario, where you have configured the following two BSLs:
...
In this scenario, as you have already set the BSL created through the DPA as the default, you cannot set the independently created BSL again as default.
...
. Configure the `DataProtectionApplication` with more than one `BackupStorageLocation`. See the following example:
+
.Example DPA
[source,yaml]
----
