...
[WARNING]
====
Importing a virtual machine whose disk size is larger than the available storage space will fail without an error message. There is no way to remove the virtual machine and its PVC. You will not be able to import another virtual machine.

You must ensure that you have sufficient storage space before you import the virtual machine.
====

.Prerequisites

* You must create a VDDK image, push it to a private container repository, and add it to the `v2v-vmware` ConfigMap.
* You must have sufficient storage space for the imported virtual disk.
