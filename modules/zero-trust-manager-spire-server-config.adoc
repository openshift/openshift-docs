...
`spec.defaultJWTValidity`:: Specifies the default validity period (TTL) for JWT SVIDs issued to workloads. This value is used if a specific TTL is not configured for a registration entry.

`spec.jwtKeyType`:: Optional parameter. Specifies the key type used for JWT signing. The valid options are `rsa-2048`, `rsa-4096`, `ec-p256`, and `ec-p384`.
...
`spec.datastore.tlsSecretName`:: Optional parameter. Specifies the name of a Kubernetes Secret containing TLS certificates for database connections. The Secret will be mounted at `/run/spire/db/certs`.
