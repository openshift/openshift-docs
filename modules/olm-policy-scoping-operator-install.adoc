...
<1> Specify a catalog source that already exists in the designated namespace or one that is in the global catalog namespace, for example `community-operators`.
endif::[]
ifdef::openshift-enterprise,openshift-rosa,openshift-dedicated[]
...
ifdef::openshift-origin[]
<2> Specify a namespace where the catalog source was created, for example `openshift-marketplace` for the `community-operators` catalog.
endif::[]
====
+
