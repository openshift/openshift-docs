...
[role="_abstract"]
You can deploy a pod in a user-defined project from a container image that includes an AWS SDK by specifying the service account that includes the `eks.amazonaws.com/role-arn` annotation in your pod configuration.

AI:
[role="_abstract"]
To enable your application to automatically assume an AWS IAM role within a pod, you can deploy the pod using a service account configured with your IAM role details. This setup allows your containerized AWS SDK operations to interact securely with AWS services.
