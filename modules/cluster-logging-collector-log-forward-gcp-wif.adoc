...
spec.providerSpec.predefinedRoles::
  The GCP IAM role granting permissions to write log entries to Google Cloud Logging.
...
spec.providerSpec.serviceAccountEmail::
  The email address of the GCP service account that `ccoctl` will create or use.
...
spec.outputs.googleCloudLogging.authentication.token.from::
  Specifies that the bearer token comes from the service account. The collector uses the service account token as the subject token for WIF token exchange with GCP.
