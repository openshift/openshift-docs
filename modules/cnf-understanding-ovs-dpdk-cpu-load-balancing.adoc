...
Disabling load balancing removes the CPUs from that shared scheduler pool (`cpuset.cpus.partition=isolated`). The advantage is stronger exclusive ownership for PMD threads: the kernel is less likely to migrate or run unrelated tasks on the same CPUs, which helps keep poll-loop timing stable.
