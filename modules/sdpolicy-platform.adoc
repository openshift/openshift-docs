...
Node root volumes are not backed up because {op-system-first} is fully managed by the {OCP} cluster and no stateful data should be stored on the root volume of a node.
...
== Operating system
{product-title} runs on OpenShift 4 and uses {op-system-first} as the operating system for all control plane and worker nodes.
