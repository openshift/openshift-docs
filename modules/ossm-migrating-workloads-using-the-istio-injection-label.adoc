...
+
Because you cannot create the default `IstioRevisionTag` yet, it is necessary to temporarily remove the `istio-injection=enabled`. Leaving the label applied prevents the proxy injection by 3.0 control plane as the `istio-injection` label takes precedence over `istio.io/rev` label.
