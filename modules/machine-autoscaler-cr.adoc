...
If your machines are running in AWS, GCP, Azure, {rh-openstack}, or vSphere, you can set the `spec.minReplicas` value to `0`. You can save on costs by setting this value to zero for use cases such as running expensive or limited-usage hardware used for specialized workloads, or scaling a machine set with extra large machines. The machine autoscaler scales the machine set down to zero when the machines are not in use.
