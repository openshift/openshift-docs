...
|`object`
|Optional: Determines which `FRRConfiguration` CR the OVN-Kubernetes driven `FRRConfiguration` CR is based on. When omitted, all `FRRConfiguration` CRs are considered.

|`networkSelector`
|`object`
|Optional: Specifies which networks to advertise. When omitted, advertises all networks. To select the default network, match the predefined `k8s.ovn.org/default-network` label.
