...
|`object`
|Optional: Determines which `FRRConfiguration` CR the OVN-Kubernetes driven `FRRConfiguration` CR is be based on. When omitted, all `FRRConfiguration` CRs are be considered.

|`networkSelector`
|`object`
|Optional: Specifies which networks to advertise. When omitted, advertises all networks. To select the default network, match on the predefined `k8s.ovn.org/default-network` label.
