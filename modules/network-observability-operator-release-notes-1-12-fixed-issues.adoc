...
[role="_abstract"]
The Network Observability Operator 1.12 release contains several fixed issues that improve performance, system status reporting, and the user experience in the web console and CLI.
...
Fixed dependency check for help and version commands in netobserv-cli::
Before this update, executing `help` or `version` commands in the Network Observability CLI (`netobserv-cli`) without the `yq` tool installed caused a dependency check error. As a consequence, you could not access basic utility information without installing additional tools.
+
With this release, the dependency check is omitted for basic informational commands. As a result, `netobserv-cli` now correctly displays help and version information without requiring `yq`.
+
...
+
With this release, the warning message only appears during the initial data load and does not overlay other content. As a result, interface clarity is improved when navigating the dashboard.
+
...
Before this update, the default setting for Prometheus metrics was unassigned during `FlowCollector` resource creation. As a consequence, you had to manually ensure that metrics collection was active to query accurate flow data.
+
...
Fixed user interface error on Netflow Traffic page::
Before this update, navigating to the **Netflow traffic** page could trigger a minified user interface error under specific cluster conditions. As a consequence, network flow charts failed to render correctly.
+
With this release, the underlying interface rendering logic is corrected. As a result, the **Netflow traffic** page operates without errors.
+
link:https://issues.redhat.com/browse/NETOBSERV-2643[NETOBSERV-2643]
...
Before this update, flow durations and network latencies greater than one second were improperly formatted as milliseconds (`kms`). As a consequence, donut graphs and latency metrics displayed confusing or inaccurate time designations.
+
With this release, the duration formatting function handles values above one millisecond accurately using decimal seconds. As a result, you can view precise network latency values in console charts.
+
...
+
With this release, the operator checks for a zero-pod deployment count. As a result, the `FlowCollector` resource now correctly identifies when zero eBPF pods are active, improving cluster error diagnostics.
+
link:https://issues.redhat.com/browse/NETOBSERV-2674[NETOBSERV-2674]

Optimized field exports for OpenTelemetry exporters::
Before this update, the {OTELName} exporter processed missing or null keys as non-null data. As a consequence, unpopulated metadata fields were exported to log streams, which increased storage usage and cluttered telemetry files.
+
With this release, the {OTELName} exporter filters out null or unrelated fields, exporting only keys that belong to explicitly enabled features. As a result, exported log sizes are reduced and data efficiency is improved.
+
link:https://issues.redhat.com/browse/NETOBSERV-2705[NETOBSERV-2705]

Added sampling probability fields to IPFIX exports::
Before this update, IPFIX record exports omitted per-flow sampling information. As a consequence, data exports failed to comply with the standard IPFIX specifications for `samplingProbability` usage.
+
With this release, the exporter includes sampling probability details within the IPFIX packet metadata. As a result, exported network telemetry data matches industry compliance standards.
+
...
Kafka compression set to optional to lower eBPF agent memory usage::
Before this update, enforcing Kafka compression rules by default within certain data paths caused a significant memory overhead inside the eBPF agent layer. As a consequence, memory consumption grew unexpectedly during high-throughput runs.
+
With this release, Kafka compression is configured as an optional parameter that is disabled by default. As a result, the core memory footprint of the eBPF agent is reduced, which improves overall cluster agent stability.
+
link:https://issues.redhat.com/browse/NETOBSERV-2733[NETOBSERV-2733]
