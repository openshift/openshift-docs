...
In {istio}, gateways are used to manage traffic entering (ingress) and exiting (egress) the mesh. {SMProductName} 2.6 deployed and managed an ingress gateway and an egress gateway with the Service Mesh control plane. Both an ingress gateway and an egress gateway were configured using the `ServiceMeshControlPlane` resource.

The {SMProduct} 3.2 Operator does not create or manage gateways.

Instead, gateways in {SMProduct} 3.2 are created and managed independent of the Operator and control plane by using gateway injection or the Kubernetes Gateway API. This provides greater flexibility and ensures that gateways can be fully customized and managed as part of a {gitops-title} pipeline. This allows the gateways to be deployed and managed alongside their applications with the same lifecycle.
...
Gateways might continue to be deployed onto nodes or namespaces independent of applications. For example, a centralized gateway node. Istio gateways also remain eligible to be deployed on {ocp-product-title} infrastructure nodes.
