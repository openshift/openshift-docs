...
mirror:
  platform: <1>
    ...
    - name: stable-{ocp-version}
      minVersion: {ocp-version}.1
      maxVersion: {ocp-version}.1
    graph: true
  operators:
  ...
  helm: <3>
----
<1> The `platform` field and related fields are not supported by {microshift-short}. You must delete the field.
...
<3> Helm is not supported by {microshift-short}. You must delete the field.
+
[NOTE]
====
