...
spec:
  ...
  external_services:
    ...
    grafana:
      enabled: false
----
* `spec.version` specifies the version of Kiali to use. You can use the `default` value of the version parameter if you install {SMProduct} 3.2 before updating Kiali. The default version is compatible with both 2.6 and 3.2 control planes.
...
* `spec.external_services.grafana` disables the Grafana configuration. Grafana is not supported with {SMProduct} 3.2.