...
[NOTE]
====
If a PipelineRun has failed on a specific TaskRun, on the side panel of the application node in the *Topology* page, the *Pipeline Runs* section under the *Resources* tab displays a log snippet of the failed PipelineRun for the application. The log snippet provides a general error message and a snippet of the log, along with a link to the *Logs* section to quickly access the details about the failed run.
====
