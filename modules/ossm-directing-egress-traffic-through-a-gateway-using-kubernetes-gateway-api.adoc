...
= Route egress traffic from a sidecar mesh with the Gateway API
...
. Create a YAML file named `egress-gateway-cr.yaml` that defines the egress gateway, similar to the following example:
+
[source,yaml,subs="attributes,verbatim"]
----
