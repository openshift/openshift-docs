...
.. Create a YAML file named `gateway-cr` that enables the Kubernetes Gateway API custom resource definitions (CRDs). 
+
.Example {k8s} Gateway CR file
...
. Create a YAML file named `egress-gateway-cr` that defines the egress gateway.
+
...
[source,terminal]
----
oc apply -f egress-gateway-cr.yml
----
