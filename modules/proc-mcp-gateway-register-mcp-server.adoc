...
[role="_abstract"]
Direct your {mcpg} to discover and route to your Model Context Protocol (MCP) servers by registering them. To connect each MCP server to the {mcpg}, you must first create an `HTTPRoute` custom resource (CR) that routes to the server. Then, you must create an `MCPServerRegistration` custom resource (CR) that references the `HTTPRoute` CR.
