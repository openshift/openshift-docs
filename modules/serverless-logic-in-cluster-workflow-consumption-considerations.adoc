...
Service discovery scans the user-defined properties `ConfigMap`:: For a deployment that the {ServerlessLogicOperatorName} manages, service discovery scans the user-defined properties `ConfigMap` of the workflow, which is named `<workflow_name>-props`. Set your service discovery URIs in that `ConfigMap`. The {ServerlessLogicOperatorName} does not scan the `application.properties` file of the workflow project.
