...
{SMProductName} 2.6 supported using pod annotations and labels to configure sidecar injection and there was no need to indicate which control plane a workload belonged to.

With {SMProduct} 3.2, even though the {istio} control plane discovers a namespace, the workloads present in that namespace still require sidecar proxies to be included as workloads in the service mesh, and to be able to use Istio's many features.

In {SMProduct} 3.2, sidecar injection works the same way as it does for Istio, with pod or namespace labels used to trigger sidecar injection. However, it might be necessary to include a label that indicates which control plane the workload belongs to.
