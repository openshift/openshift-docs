...
The `DNSPolicy` can then interact with the CoreDNS provider within the {ocp} Container Platform cluster. This CoreDNS instance becomes authoritative for the delegated subdomain and manages the necessary DNS records (A, CNAME, and so on) for Gateways within that subdomain.
...
* {prodnamefull} is installed on two separate {ocp} clusters (primary and secondary).
