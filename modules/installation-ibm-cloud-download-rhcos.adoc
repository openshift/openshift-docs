...
:navtitle: Download the RHCOS cluster image
[id="installation-ibm-cloud-download-rhcos_{context}"]
= Downloading the RHCOS cluster image

[role="_abstract"]
The installation program requires the {op-system-first} image to install the cluster. While optional, downloading the {op-system-first} image before deploying removes the need for internet access when creating the cluster.
