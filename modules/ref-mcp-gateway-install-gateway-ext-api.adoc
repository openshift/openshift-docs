...
//
// * reference/mcp-gateway-install-reference.adoc
...
[role="_abstract"]
You can use the following API reference information for each {mcpg} `MCPGatewayExtension` custom resource (CR).
...
|String
|No
|Controls whether the operator manages the gateway `HTTPRoute` CR. The default value is `Enabled`: creates and manages the `HTTPRoute` CR. `Disabled`: does not create an `HTTPRoute` CR. Disabling does not delete a route that was created before.
