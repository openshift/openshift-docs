...
[role="_abstract"]
Manually restore your cluster from an etcd backup by starting a three-member etcd cluster on existing control plane nodes, without recreating nodes as required by the standard restore procedure.
