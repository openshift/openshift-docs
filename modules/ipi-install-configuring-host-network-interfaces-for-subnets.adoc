...
`networkConfig.interfaces.name`:: Specifies the interface name. Replace `<interface_name>` with the interface name.
`networkConfig.interfaces.ipv4.address.ip.<node_ip>`:: Specifies the IP address of the node. Replace `<node_ip>` with the IP address of the node.
`networkConfig.interfaces.ipv4.gateway`:: Specifies the IP address of the gateway. Replace `<gateway_ip>` with the IP address of the gateway.
`networkConfig.dns-resolver.cofig.server.<dns_ip>`:: Specifies the IP address of the DNS server. Replace `<dns_ip>` with the IP address of the DNS server.
