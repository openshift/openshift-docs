...
Spot Instances use spare {aws-short} EC2 capacity and are less expensive than On-Demand Instances. 
...
[IMPORTANT]
====
{aws-short} EC2 can reclaim the capacity for a Spot Instance at any time. 
====
...
`spec.template.spec.spotMarketOptions`:: Specifies the use of Spot Instances.
`spec.template.spec.spotMarketOptions.maxPrice`:: Optional parameter. Specifies an hourly cost limit in US dollars for the Spot Instance. 
...
{aws-short} gives a two-minute warning to the user when an interruption occurs. 
{product-title} begins to remove the workloads from the affected instances when {aws-short} issues the termination warning.

When {aws-short} terminates an instance, a termination handler running on the Spot Instance node deletes the machine resource. 
