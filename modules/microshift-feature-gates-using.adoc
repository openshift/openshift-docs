...
.. If you have a support exception for a customized cluster, make sure that the custom feature you want to use appears in the `specialHandlingSupportExceptionRequired` field and is enabled. The custom feature must also be enabled in the `customNoUpgrade` field.
+
NOTE: If a feature is enabled in the `specialHandlingSupportExceptionRequired` field, your customized cluster can upgrade in the same manner as a supported cluster.
+
.Example of a custom feature enabled in the `specialHandlingSupportExceptionRequired` field
[source,yaml]
----
# ...
apiServer:
