...
OpenJDK defaults to using a maximum of 25% of available memory (recognising any container memory limits in place) for "heap" memory. This default value is conservative, and in a properly-configured container environment, this value
would result in 75% of the memory assigned to a container being mostly unused. A much higher limit, such as 80%, is more suitable.

Most of the Red Hat containers replaces the OpenJDK feature for a start-up script with OpenJDK default. 
