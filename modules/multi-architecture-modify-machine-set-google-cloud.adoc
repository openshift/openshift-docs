...
:navtitle: Add a multi-architecture compute machine set to your Google Cloud cluster
:chunk-to-content:
[id="multi-architecture-modify-machine-set-google-cloud_{context}"]
...
----
+
`gcpMetadata`:: Optional parameter. Specifies custom metadata in the form of a `key:value` pair. For example use cases, see "Setting custom metadata".
+
`machineType`:: Specifies a machine type that aligns with the CPU architecture of the chosen OS image. For more information, see "Tested instance types for {gcp-short} on 64-bit ARM infrastructures".
+
`projectID`:: Specifies the name of the {gcp-short} project that you use for your cluster.
+
`region`:: Specifies the region. For example, `us-central1`. Ensure that the zone you select has machines with the required architecture.
