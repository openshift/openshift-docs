...
[NOTE]
====
These etcd metrics are collected by the {product-title} metrics system in Prometheus. You can access them from the CLI by using a Prometheus query. For more information, see the Red{nbsp}Hat Knowledgebase solution, "How to query from the command line Prometheus statistics".
====
...
`etcd_disk_wal_fsync_duration_seconds_bucket`:: Reports the etcd write-ahead log (WAL) fsync duration.
