...
[role="_abstract"]
To evaluate your environment against the telco core reference configuration, use the `cluster-compare` plugin. By comparing this known baseline with settings from a live cluster or must-gather data, you can validate your telecommunications deployment and identify unauthorized deviations.

The procedure example compares a configuration from a live cluster with the telco core reference configuration. The telco core reference configuration is derived from the telco core reference design specifications (RDS). The telco core RDS is designed for clusters to support large scale telco applications including control plane and some centralized data plane functions.
...
`metadata.yaml`:: Specifies the configuration file for the reference configuration.

`optional`:: Specifies the directory for optional templates.

`required`:: Specifies the dDirectory for required templates.
--
...
`annotations`:: Specifies the output in Linux diff format that shows the difference between the template and the cluster CR.
