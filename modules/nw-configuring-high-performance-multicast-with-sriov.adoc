...
. Verify that the multicast pod is in a `Running` state:
+
[source,terminal]
----
$ oc get pod testpmd -n default
----
+
.Example output
[source,terminal]
...
testpmd   1/1     Running   0          2m
----

. Verify that the SR-IOV network attachment is present on the pod and has an IP address from the configured range:
+
[source,terminal]
----
$ oc exec testpmd -n default -- ip addr show
----
+
.Example output
[source,terminal]
...
3: net1: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc mq state UP
