...
* The `openshift-adp` namespace for the OADP `ConfigMap` object must exist on all managed clusters and the hub cluster for the OADP `ConfigMap` to be generated and copied to the clusters.
...
<2> The namespace must exist on all managed clusters and the hub cluster for the OADP `ConfigMap` to be generated and copied to the clusters.
