...
Extensions watch all namespaces by default.
endif::[]
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
Extensions watch all worker-level namespaces by default. Extensions cannot watch control place nodes or any other nodes managed by Red{nbsp}Hat SRE.
endif::[]
Some Operators support only namespace-scoped watching based on {olmv0} install modes. Configure the `.spec.config.inline.watchNamespace` field to install these Operators.
