...
This resource configures SR-IOV device plugin pods on worker nodes to allocate virtual functions for DPU management and workload traffic.
...
`bf3_vfs`:: Allocates VF indices 2-45 on PF0 and VF indices 0-45 on PF1 for workload traffic with RDMA enabled. The `pfIndex` values refer to the first (`0`) and second (`1`) physical functions of the BlueField-3 DPU.
