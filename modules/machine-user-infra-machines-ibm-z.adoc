...
[id="machine-user-infra-machines-ibm-z-reg_{context}"]
= Creating {op-system} machines on {ibm-z-reg} with z/VM

You can create more {op-system-first} compute machines running on {ibm-z-reg} with z/VM and attach them to your existing cluster.
...
+
Currently, UDP aggregation is not supported on {ibm-z-reg} and is not automatically deactivated on multi-architecture compute clusters with an `x86_64` control plane and additional `s390x` compute machines. To ensure that the addtional compute nodes are added to the cluster correctly, you must manually disable UDP aggregation.
