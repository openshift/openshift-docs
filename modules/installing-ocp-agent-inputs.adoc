...
= Create configuration inputs for Agent-based installation

[role="_abstract"]
Create the `install-config.yaml` and `agent-config.yaml` files to define your cluster topology, network settings, and node configurations for the Agent-based Installer.
...
[NOTE]
====
For bare-metal platforms, host settings made in the platform section of the `install-config.yaml` file are used by default. These settings can be overridden by configurations made in the `agent-config.yaml` file.
====
...
You must provide the rendezvous IP address when you do not specify at least one host's IP address in the `networkConfig` parameter. If you do not provide this address, one IP address is selected from the provided hosts' `networkConfig`.
