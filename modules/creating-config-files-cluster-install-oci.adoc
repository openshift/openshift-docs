...
// I think that maybe these two steps below need to be changed for the PCA use case
...
// Depending on when in the PCA procedure users are instructed to create a web server for disconnected environments, the step below may need to be altered or removed for PCA docs. For example, maybe I can add a step here to go to the Oracle instructions for "Start a web server to upload rootfs".
