...
// I think that maybe these two steps below need to be changed for the c3 use case
...
// Depending on when in the C3 procedure users are instructed to create a web server for disconnected environments, the step below may need to be altered or removed for C3 docs. For example, maybe I can add a step here to go to the Oracle instructions for "Start a web server to upload rootfs".
