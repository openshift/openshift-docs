...
. For a disconnected environment, mirror the {product-mirror-registry} to your local container image registry. See xref:../../installing/disconnected_install/installing-mirroring-installation-images.html#installation-mirror-repository_installing-mirroring-installation-images[Mirroring the {product-title} image repository].
+
[IMPORTANT]
====
Check that your `openshift-install` binary version relates to your local image container registry and not a shared registry such as {quay} as demonstrated in the following example:
...
----
+
The earlier command also completes the following actions:
+
