...
ifdef::windows[]
The Windows Machine Config Operator (WMCO) minimizes node reboots whenever possible. However, certain operations and updates do require a reboot to ensure that changes are applied correctly and securely. To safely reboot your Windows nodes, you can use the graceful reboot process as needed. For inforamation on gracefully rebooting a standard {product-title} node, see "Rebooting a node gracefully" in the Nodes documentation. 
endif::windows[]
...
ifdef::windows[]
. SSH into the Windows node and enter PowerShell by using the following command:
+
[source,terminal]
----
C:\> powershell
...
`<gateway_ip>`:: Specifies the corresponding IP address of the gateway in Windows instance, which you can find by running the following command:
+
[source,terminal]
---- 
