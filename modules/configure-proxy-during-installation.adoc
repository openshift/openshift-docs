...
====
. Edit the fields you want to change in the configuration file:
+
[source,yaml]
----
apiVersion: v1
...
The values in this list apply to all outgoing connections, regardless of protocol.

`|-`::  Specifies the start of the configuration data when inserted into the `stringData` section.
+
[NOTE]
====
* When you first open the file, the `\#` sign at the beginning of each line comments out all values.
