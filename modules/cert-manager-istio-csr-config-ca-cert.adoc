...
where:
+
`istioCSRConfig.certManager.istioCACertificate.key`:: Specifies the key name in the `ConfigMap` that contains the CA bundle.
`istioCSRConfig.certManager.istioCACertificate.name`:: Specifies the name of the `ConfigMap`. Ensure that the referenced `ConfigMap` and key exist before you update this field.
`istioCSRConfig.certManager.istioCACertificate.namespace`:: Optional. Specifies the namespace where the `ConfigMap` exists. If you do not set this field, the {cert-manager-operator} searches for the `ConfigMap` in the namespace where you have installed the `IstioCSR` CR.
+
[NOTE]
====
