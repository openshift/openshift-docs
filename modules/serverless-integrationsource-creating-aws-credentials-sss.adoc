...
[id="serverless-integrationsource-creating-aws-credentials-sss_{context}"]
= Creating AWS credentials for S3

The `IntegrationSource` uses a Kubernetes Secret to store the AWS credentials required to connect to S3. The Secret must reside in the same namespace as the `IntegrationSource` resource and include both the AWS access key and secret key.
