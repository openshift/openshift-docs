...
To trace HTTP requests and responses, OpenShift Serverless Logic uses the Apache HTTP Client. You can enable tracing by setting the following property:
...
. After restarting, check your logs for HTTP request traces. Here is an example of a traced HTTP request:
+
[source,terminal]
----
...
. Check the logs for HTTP response traces following the request logs. Here is an example:
+
[source,terminal]
----
