...
+
* `Terminate`: Decrypts traffic at the gateway. Use this mode on `HTTPS` listeners with `HTTPRoute` CRs. You must set `certificateRefs` to a `Secret` that exists before you create the `Gateway` CR. The `openshift-default` GatewayClass supports this mode.
...
[IMPORTANT]
====
The `openshift-default` GatewayClass does not support `tls.mode: Passthrough` or mixed-mode listeners that share a port between passthrough and terminated TLS. For TLS passthrough, use a `TLSRoute` CR with a GatewayClass whose implementation supports it, such as {SMProductName}.
====
