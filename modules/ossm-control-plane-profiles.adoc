...
* An account with the `cluster-admin` role. If you are using {product-dedicated}, you must have an account with the `dedicated-admin` role.
* Location of the Operator deployment.
* Access to the {product-title} Command-line Interface (CLI) also known as `oc`.

.Procedure

. Log in to the {product-title} CLI as a `cluster-admin`. If you are using {product-dedicated}, you must have an account with the `dedicated-admin` role.
