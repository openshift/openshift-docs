...
[role="_abstract"]
Integrate a Security Assertion Markup Language (SAML) 2.0 identity provider with {product-title} ({product-title-short}).
...
. In the *Name* field, enter a name to identify this authentication provider; for example, *Okta* or *Google*. The login page shows the integration name to help users select the correct sign-in option.
...
** *Option 1: Dynamic Configuration*: If you select this option, enter the *IdP Metadata URL*, or the URL of *Identity Provider metadata* available from your identity provider console. The system acquires the configuration values from the URL.
...
** If your login try failed, {product-title-short} shows a message describing why the system could not process the identity provider's response.
