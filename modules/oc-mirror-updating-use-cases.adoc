...
`mirror.platform.channels.name`:: Replacing by `stable-4.13` prunes all the images of `stable-4.12`.
...
`mirror.operators.packages.channels.name`:: Using the same channel without specifying a version prunes the existing images and updates with the latest version of images.
...
`mirror.operators.packages.name`:: Replacing `rhacs-operator` with `new_operator_name` prunes the Red Hat Advanced Cluster Security for Kubernetes Operator.
