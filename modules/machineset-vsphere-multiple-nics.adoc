...
`<vm_template_name>`:: Specifies the {vmw-short} VM template to use, such as `user-5ddjd-rhcos`.
`<vcenter_data_center_name>`:: Specifies the vCenter data center to deploy the machine set on.
`<vcenter_datastore_name>`:: Specifies the vCenter datastore to deploy the machine set on.
...
`<vcenter_server_ip>`:: Specifies the vCenter server IP or fully qualified domain name (FQDN).
end::compute[]
tag::controlplane[]
