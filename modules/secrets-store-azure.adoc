...
:navtitle: Mount secrets from {azure-short} Key Vault
[id="secrets-store-azure_{context}"]
= Mounting secrets from {azure-short} Key Vault

[role="_abstract"]
You can use the {secrets-store-operator} to mount secrets from {azure-first} Key Vault to a Container Storage Interface (CSI) volume in {product-title}. Using an external secret store protects information that you do not want developers to have and can be more secure than `secret` objects.
