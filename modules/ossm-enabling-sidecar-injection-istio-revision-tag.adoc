...
. Create the `IstioRevisionTag` by running the following command:
+
[source,terminal]
----
$ oc apply -f - <<EOF
...
. Verify that the `IstioRevisionTag` has been created successfully by running the following command:
+
[source,terminal]
----
...
. To apply the injection label to the `bookinfo` namespace, run the following command:
+
[source,terminal]
----
