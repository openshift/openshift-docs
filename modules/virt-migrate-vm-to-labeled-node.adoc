...
= Migrating a virtual machine to a node with additional labels
...
----
+
In this example, the `vmiName` specifies the running VM (`vmi-fedora`), and the `addedNodeSelector` specifies additional constraints for selecting the target node.

. Apply the manifest:
+
[source,bash,subs="specialchars"]
----
