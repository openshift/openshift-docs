...
= Support considerations for {product-title} clusters with STS
...
The files and `aws` commands are generated for review purposes only and must not be modified in any way. Red{nbsp}Hat cannot provide support for {product-title} clusters that have been deployed by using modified versions of the policy files or `aws` commands.
====
