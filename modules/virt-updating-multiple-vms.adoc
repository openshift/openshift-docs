...
* LABEL_SELECTOR env can be used for partially selecting vms to apply the job on, leave it empty if you want the job to go over all vms in the cluster
* NAMESPACE env is for controlling the namespace to look for vms, leave it empty and the job will go over all namespaces in the cluster
