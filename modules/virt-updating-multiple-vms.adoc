...
<3> If the `RESTART_REQUIRED` value is set to `true`, VMs are restarted after an upgrade. If you do not want them to be restarted, set the value to `false`.
<4> The `namespace` environment value indicates the namespace to look for VMs. Leave it empty for the job to go over all namespaces in the cluster.
<5> You can use the `LABEL_SELECTOR` environment value for selecting VMs to apply the job on. If you want the job to go over all VMs in the cluster, leave the value empty.
