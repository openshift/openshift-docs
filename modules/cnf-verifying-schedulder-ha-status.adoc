...
----
+
Using the default HA mode, the number of pods equals the number of control-plane nodes. A standard HA {product-title} cluster typically has three control-plane nodes, and therefore displays three pods similar to the following:
+
[source,terminal]
----
...
----
+
If you customized the replicas, the number of pods matches the value you set. If you disabled the scheduler, there are no running pods with this label.
+
[NOTE]
====
...
----
+
The `Replicas` line in the output shows a deployment configured for 3 replicas, with all 3 updated and available:
+
[source,yaml]
----
