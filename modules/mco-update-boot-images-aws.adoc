...
|===
| Variant | Product ID
| {product-rosa} (classic architecture)| `34850061-abaf-402d-92df-94325c9e947f`
|===
+
[NOTE]
====
The {product-rosa} (classic architecture) product ID applies to {product-rosa} (classic architecture) only. The default architecture of Red Hat OpenShift Service on AWS uses `NodePool` objects rather than `MachineSet` objects. Boot images are fully managed by Red Hat, with no manual update required.
====
endif::openshift-rosa[]
