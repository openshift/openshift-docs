...
To use Progressive Sync in {gitops-shortname}, enable this feature in the `ApplicationSet` controller before creating and managing `ApplicationSet` resources with the `RollingSync` strategy.
...
[source,terminal]
----
$ oc apply -f guestbook-rollingsync
----
+
[NOTE]
====
...
. Verify that {gitops-title} Operator has enabled Progressive Sync in the web UI by completing the following steps:
.. In the {gitops-title} web console, navigate to the *Argo CD Application details* page for your deployed application.
