...
. Run a service connectivity test from a worker pod to another worker pod.
+
...
[source,terminal]
----
Connection to 172.30.187.147 5000 port [tcp/*] succeeded!
----
+
The test verifies end-to-end connectivity through the DPU-accelerated service chain between worker pods.

. (Optional) Run an external service connectivity test.
+
In the following example, replace `<worker_pod_name>` with the name of a `traffic-test-worker` pod and replace `<node_ip>` with the IP address of a cluster node, and replace `<nodeport>` with the NodePort for one of the services:
+
[source,terminal]
----
...
[source,terminal]
----
Connection to 10.0.110.10 31368 port [tcp/*] succeeded!
----
+
This test validates that NodePort services are reachable through the DPU networking stack.
