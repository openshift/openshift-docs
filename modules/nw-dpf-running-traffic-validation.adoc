...
+
In the following example, replace `_<worker_pod_name>_` with the name of a `traffic-test-worker` pod and replace `_<target_pod_ip>_` with the IP address of a `traffic-test-worker` pod on a different worker node:
+
...
+
In the following example, replace `_<worker_pod_name>_` with the name of a `traffic-test-worker` pod and replace `_<service_cluster_ip>_` with the cluster IP address of the `traffic-test-master` service:
+
[source,terminal]
----
