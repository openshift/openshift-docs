...
[role="_abstract"]
To configure data backups on Google Cloud Storage (GCS), create an integration in {product-title}. Once configured, {product-title} automatically backs up all data according to the specified schedule.

.Prerequisites
* An existing *bucket*.
To create a new bucket, see "Creating storage buckets" in the official Google Cloud Storage documentation.
