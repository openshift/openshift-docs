...
|`hypershift` |Backs up and restores HyperShift cluster resources. ^[5]^ |Object store
|===
[.small]
--
1. Mandatory.
...
4. OADP 1.2 only.
5. Do not add the `hypershift` plugin in the `DataProtectionApplication` custom resource, if the cluster is not a HyperShift cluster. 
--
