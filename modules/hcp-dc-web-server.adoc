...
:navtitle: Configure the web server for {hcp} in a disconnected environment
[id="hcp-dc-web-server_{context}"]
= Configuring the web server for {hcp} in a disconnected environment

[role="_abstract"]
You must configure an additional web server to host the {op-system-first} images that are associated with the {product-title} release that you are deploying as a hosted cluster.
