...
= Using Git SSH authentication in tasks

You can invoke directly Git commands in the steps of a task and use SSH authentication, but an additional step is required.
...
As an example of using SSH authentication in `git` type tasks, refer to link:https://github.com/tektoncd/pipeline/blob/main/examples/v1/taskruns/authenticating-git-commands.yaml[authenticating-git-commands.yaml].
