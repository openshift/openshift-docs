...
When updating your cluster, it is useful to find out which versions are recommended for your update. While the `oc adm upgrade` command returns a list of the next available versions, this command decouples that information and narrows down your suggestions to recommend a new target release before you launch your update.

The `oc adm upgrade recommend` command is read-only and will never alter any state in your cluster. 
...
[IMPORTANT]
====
While your cluster does not need to be a Technology Preview-enabled cluster, you must enable the `OC_ENABLE_CMD_UPGRADE_RECOMMEND` Technology Preview environment variable, otherwise the {oc-first} will not recognize the command and you will not be able to use the feature. 
====
...
.. If you want a list of all of the recommended versions for your update, run the `oc adm upgrade recommend` command:
+
[source,terminal]
----
...
. If you want to determine if a specific version is recommended for your update, run the `oc adm upgrade recommend --version` command:
+
[source,terminal]
----
