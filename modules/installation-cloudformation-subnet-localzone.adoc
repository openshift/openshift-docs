...
[role="_abstract"]
ifndef::outposts[Use the CloudFormation template to deploy the private and public subnets in a zone on {zone-type} infrastructure.]
ifdef::outposts[Use the CloudFormation template to deploy the Outpost subnet.]
