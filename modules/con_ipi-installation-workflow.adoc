...
The installation program supports interactive mode, but Red Hat recommends preparing an `install-config.yaml` file in advance, containing the provisioning details for all of the bare-metal hosts.

Add the relevant cluster details to the `install-config.yaml` file. The installation program ships the `install-config.yaml` file. The administrator generates the manifests and verifies all prerequisites.
...
* Metal3-baremetal-operator: The operator that provides a higher-level API for Ironic and replaces `terraform-provider-ironic` (available in {openshift} 4.16 and later)
