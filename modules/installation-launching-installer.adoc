...
.. Paste the pull secret that you obtained from the
link:https://cloud.redhat.com/openshift/install/pull-secret[Pull Secret] page on the {cloud-redhat-com} site. If you do not have a pull secret from the Red Hat OpenShift Cluster Manager site, you can paste the pull secret for your registry. If you do not need the cluster to pull images from private repositories, paste `{"auths":{"fake":{"auth":"aWQ6cGFzcwo="}}}`. 
endif::openshift-origin[]
endif::rhv[]
ifdef::rhv[]
