...
.. You can store the credentials of the service principal in `~/.azure/osServicePrincipal.json`. In case that you use more 
then one service principal can you define the environement variable `AZURE_AUTH_LOCATION` with the full path of the service principal json.
+
[source,json]
----
{
