...
.Procedure
ifdef::gcp[]
. Remove any existing {gcp-full} credentials that do not use the service account key
for the {gcp-full} account that you configured for your cluster and that are stored in the
following locations:
...
.. If you have not configured the service account key for your {gcp-full} account on
your host, you must obtain it from {gcp-full} and paste the contents of the file
