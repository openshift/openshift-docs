...
* Discover which component generated an error, such as Envoy, an {mcpg} component, or a backend MCP server.
...
* Replace the value of `metadata.namespace:` with the namespace of your {mcpg} deployment.
...
* If you do not see any traces, check if the `NetworkPolicy` CR in your namespace allows traffic from the {mcpg} to the {OTELName} collector service.
