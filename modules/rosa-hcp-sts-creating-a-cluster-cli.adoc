...
. The following command creates your {hcp-title} cluster.
+
[NOTE]
====
...
. If you set the environmental variables, create a cluster with a single, initial machine pool, using either a publicly or privately available API, and publicly or privately available Ingress by running the following commands:

.. For a private cluster, run:
+
[source,terminal]
----
