...
[role="_abstract"]
You can configure Keycloak as an external authentication provider for your {product-title} cluster by using the {rosa-cli-first}. Before you configure this provider, you must have a Keycloak server already set up.

For more information about setting up Keycloak, see the link:https://www.keycloak.org/server/configuration[Keycloak documentation].
