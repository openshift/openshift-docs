...
To enable the availability of PCI devices in the cluster, add details about the PCI devices to the HyperConverged Operator in the YAML file. The PCI devices are exposed and permitted to be used in the cluster.

.Procedure
. Use the following command to add the PCI device information to the YAML file.
+
[source,terminal]
