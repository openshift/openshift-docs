...
[role="_abstract"]
You can disable etcd encryption when it is no longer required by setting the API server encryption type to `identity`. This change decrypts previously encrypted resources so that etcd stores them without encryption.
