...
* Before this update, the {gitops-title} Operator used an older version of the HA proxy router container image. This container image had multiple security vulnerabilities. With this update, the {gitops-title} Operator uses the latest version of the HA proxy container image, which includes a fix for the security vulnerabilities. link:https://issues.redhat.com/browse/GITOPS-4761[GITOPS-4761]

* Before this update, the {gitops-title} Operator used an older version of Redis container image. This container image had multiple security vulnerabilities. With this update, the {gitops-title} Operator uses the correct version of the Redis container image, Redis 6, which includes a fix for the security vulnerabilities. link:https://issues.redhat.com/browse/GITOPS-3514[GITOPS-3514]
