...
This procedure explains how to install the _mirror registry for Red Hat OpenShift_ on a local host using the `mirror-registry` installer tool. By doing so, users can create a local host registry running on port 8443 for the purpose of storing a mirror of {product-title} images.
...
$ sudo ./mirror-registry install \
  --quayHostname <host_example_com> \ <1>
  --quayRoot <example_directory_name>
----
<1> If port 8443 is already in use, you can use another port by specifying `quayHostname <host_example_com>:<port>` instead. Be sure to use the port number that you specify in the rest of the commands instead of 8443.
