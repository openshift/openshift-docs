...
Rules:: Sets of permitted actions on specific objects. For example, determining whether a user or service account can `create` pods. Each rule specifies an API resource, the resource within that API, and the allowed action.

Roles:: Collections of rules that define what actions users or groups can perform. You can associate or bind rules to multiple users or groups. A role file may contain one or more rules specifying the actions and resources allowed for that role. 
+
