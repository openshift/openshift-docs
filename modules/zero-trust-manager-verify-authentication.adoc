...
[role="_abstract"]
Deploy real Mutual Transport Layer Security (mTLS) workloads to verify cross-cluster authentication by using SPIFFE identities. This process helps you confirm that your federated trust bundles and identity policies are working correctly across both clusters.
...
.. Start the mTLS connection by running the following command:
+
[source,terminal]
----
$ oc exec -n mtls-client mtls-client -c client -- \
