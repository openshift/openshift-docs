...
== Updating the Log Forwarding Custom Resource

If you make changes to the log forwarding outputs or pipelines, you need to disable log forwarding, edit the CR, then enable log forwarding again.

.Procedure

To modify the log forwarding outputs or pipelines:
...
. Set the `clusterlogging.openshift.io/logforwardingtechpreview` annotation to `disabled`:
+
[source,yaml]
----
apiVersion: "logging.openshift.io/v1"
...
metadata:
  annotations:
    clusterlogging.openshift.io/logforwardingtechpreview: disabled <1>
  name: "instance"
  namespace: "openshift-logging"
...
----
<1> Disable the Log Forwarding feature.

. Edit the Log Forwarding Custom Resource:
+
----
$ oc edit  LogForwarding instance -n openshift-logging
----
...
. Set the `clusterlogging.openshift.io/logforwardingtechpreview` annotation to `enabled`:
+
[source,yaml]
----
apiVersion: "logging.openshift.io/v1"
...
----
<1> Enable the Log Forwarding feature.
