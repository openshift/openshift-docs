...
metadata:
  ...
  namespace: openshift-logging
spec:
  disableDefaultForwarding: true <1>
  outputs: <3>
  ...
  pipelines: <9>
   ...
   - name: audit-logs
     ...
     - secureforward-offcluster
----
<1> Set the annotation to `enabled` to enable log forwarding.
<2> Add one or more endpoints:
...
* Specify the name of one or outputs configured in the CR.
...
The following Log Forwarding custom resource sends all logs to the internal {product-title} Elaticsearch instance, the default log forwarding method.
