...
<3> Set to `true` to disable the default log forwarding behavior of sending all logs to the internal Elasticsearch instance. 
...
* Specify the data source: `logs.app`, `logs.infra`, or `logs.audit`.
