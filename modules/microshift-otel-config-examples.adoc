...
The `opentelemetry-collector.yaml` file includes a minimal set of data for monitoring the system, node, and workload resource use for the CPU, memory, disk drive, and network. All warnings for cluster events are included in the collected data. This setting transmits the following data:
...
receivers:
  kubeletstats:
    ...
    endpoint: "${env:OTEL_BACKEND}:10250"
    node: ${env:OTEL_BACKEND}
    k8s_api_config:
