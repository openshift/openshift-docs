...
. Login to the Red Hat Ecosystem Catalog by running the following command and entering your credentials:
+
[source,bash]
----
...
. Move the extracted binary to a directory that is on the system's PATH, such as `/usr/local/bin/`, by running the following command:
+
[source,bash]
----
