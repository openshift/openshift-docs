...
:navtitle: Troubleshoot worker nodes that cannot join the cluster
...
. Add a DNS A/AAAA or CNAME record to internally identify the API load balancer. For example, when using dnsmasq, modify the `dnsmasq.conf` configuration file by running the following command:
+
...
. Add a DNS PTR record to internally identify the API load balancer. For example, when using dnsmasq, modify the `dnsmasq.conf` configuration file by running the following command:
+
[source,terminal,options="nowrap",role="white-space-pre"]
----
$ sudo nano /etc/dnsmasq.conf
