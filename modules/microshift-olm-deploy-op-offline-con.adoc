...
For {microshift-short} clusters that are installed on disconnected or offline clusters, Operator Lifecycle Manager (OLM) by default cannot access sources hosted on remote registries because those remote sources require full internet connectivity. Therefore, mirroring a highly available container registry is essential.
...
//Q: do we have to use oc mirror for this, even with home-grown operators?
...
* Ensure that the network settings are configured to run in offline mode.
