...
[IMPORTANT]
====
To limit the clients that can mount the persistent volume (PV), set an IP or a subnet mask in the `nfs-shareClient` storage class parameter.
====
...
[IMPORTANT]
====
To limit the clients that can mount the PV, set an IP or a subnet mask in the `nfs-shareClient` storage class parameter.
====
+
