...
See the following example of internal webserver hosting {op-system} images:
...
----
.. Inspect the `bootkube` logs to check bootstrap progress by running the following command:
+
[source,terminal]
----
...
. Verify all the pods, including `dnsmasq`, `mariadb`, `httpd`, and `ironic`, are running by using the following command:
+
[source,terminal]
----
