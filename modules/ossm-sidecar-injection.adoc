...
{ProductName} creates a proxy sidecar within the application's pod to provide {ProductShortName} capabilities to the application. 

Automatic injection using annotation  need to label projects. This ensures that your application contains the appropriate configuration for the {ProductShortName} upon deployment. This method requires fewer privileges and does not conflict with other OpenShift capabilities such as builder pods.
