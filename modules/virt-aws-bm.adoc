...
====
* We do not recommend Amazon Elastic File System (EFS) and Amazon Elastic Block Store (EBS) for use with {VirtProductName} due to performance and functionality limitations. Use CSI storage that supports ReadWriteMany (RWX), cloning, and snapshots to enable live migration, fast VM creation, and VM snapshots capabilities.
--
