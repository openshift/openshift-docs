...
ifdef::openshift-rosa,openshift-dedicated,openshift-rosa-hcp[]
--
* If your application requires a flat layer 2 network that does not require egress traffic, consider using OVN-Kubernetes secondary overlay networks with a `Layer2` topology.
--
endif::openshift-rosa,openshift-dedicated,openshift-rosa-hcp[]
