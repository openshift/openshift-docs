...
// FIXME: Link targets RHOSP 16.0 docs. Update when RHOSO equivalent is available.
...
link:https://docs.redhat.com/en/documentation/red_hat_openstack_services_on_openshift/18.0/html/command_line_interface_reference/server#ref_server-group-create[creating a server group]. For most deployments, `anti-affinity` or `soft-anti-affinity` policies are recommended.
...
`<optional_UUID_of_server_group>`:: Sets a server group policy for the `MachineSet` YAML, by entering the value that is returned from
link:https://docs.redhat.com/en/documentation/red_hat_openstack_services_on_openshift/18.0/html/command_line_interface_reference/server_group_create[creating a server group]. For most deployments, `anti-affinity` or `soft-anti-affinity` policies are recommended.
