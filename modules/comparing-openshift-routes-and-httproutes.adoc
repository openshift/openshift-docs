...
|Rewrite target
|Supports with the `haproxy.router.openshift.io/rewrite-target` annotation.
|Supports with the `URLRewrite` filter.

|Sharding
|Supports with metadata labels.
|Supports with parent references via `parentRefs`.
|===
