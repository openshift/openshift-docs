...
[NOTE]
====
{product-title} routes can serve passthrough and edge or re-encrypt traffic on port `443` by using Server Name Indication (SNI). Gateway API uses a `TLSRoute` CR for passthrough and an `HTTPRoute` CR with a `BackendTLSPolicy` CR for re-encryption. `HTTPRoute` CRs do not support passthrough.
====
...
|Rewrite target
|Supports by using the `haproxy.router.openshift.io/rewrite-target` annotation.
|Supports by using the `URLRewrite` filter.

|Sharding
|Supports by using metadata labels.
|Supports by using parent references via `parentRefs`.
|===
