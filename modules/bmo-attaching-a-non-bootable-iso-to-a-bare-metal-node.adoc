...
[role="_abstract"]
Attach a non-bootable ISO image to a bare-metal node using the `DataImage` resource with a Redfish-compatible `BareMetalHost` for diagnostic purposes, firmware updates, or running specialized tools without reprovisioning the host.
...
+
--
`name: <node_name>`:: Specify the name of the node as defined in its `BareMetalHost` resource.
`url: "http://dataimage.example.com/non-bootable.iso"`:: Specify the URL and path to the ISO image.
--
...
+
--
`$ oc apply -f <node_name>-dataimage.yaml -n <node_namespace>`:: Replace `<node_namespace>` so that the namespace matches the namespace for the `BareMetalHost` resource. For example, `openshift-machine-api`.
--
