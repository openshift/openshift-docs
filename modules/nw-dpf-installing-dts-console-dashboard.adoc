...
. Create a file named `dts-console-dashboard.yaml` with the following content to define a console dashboard `ConfigMap` in the `openshift-config-managed` namespace:
+
[source,yaml]
----
apiVersion: v1
...
[NOTE]
====
The DTS console dashboard renders against the platform Thanos or user workload monitoring Prometheus instance.
No Grafana dependency is required for basic DPU telemetry viewing.
====
