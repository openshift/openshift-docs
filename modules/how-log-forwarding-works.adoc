...
Outputs::
Outputs specify the final destination for the telemetry. You can configure multiple outputs to satisfy different consumers simultaneously. Supported destinations include internal stores such as LokiStack, external providers such as Splunk, Datadog, or Amazon CloudWatch, and standard protocols such as Kafka, Syslog, or S3-compatible object storage.
