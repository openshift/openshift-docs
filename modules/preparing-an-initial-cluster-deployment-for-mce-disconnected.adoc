...
= Prepare an Agent-based cluster deployment for the {mce} while disconnected
...
[IMPORTANT]
====
The `oc mirror` command  creates `ImageDigestMirrorSet` (IDMS) and `ImageTagMirrorSet` (ITMS) files under `working-dir/cluster-resources` folder that identifies all the mirrors you need for {product-title} and your selected Operators.
====
