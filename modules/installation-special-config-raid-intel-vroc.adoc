...
[role="_abstract"]
Intel(R) VROC is a type of hybrid RAID, where some of the maintenance is offloaded to the hardware, but shows as software RAID to the operating system.
...
----
* `imsm`: The RAID device names. In this example, there are two devices listed. If you provide more than two device names, you must adjust the `-n` flag. For example, listing three devices would use the flag `-n3`.
...
$ coreos-installer install /dev/md/coreos \
  ...
  ...
----
* `<md_UUID>`: Specifies the UUID of the IMSM container.
+
