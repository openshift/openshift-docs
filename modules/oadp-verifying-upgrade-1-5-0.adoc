...
. Verify that the installation by viewing the {oadp-first} resources by running the following command:
+
[source,terminal]
----
...
In previous {oadp-first} versions, the `node-agent` pods are labeled as `restic`.
====
