...
. Verify that the installation by viewing the {oadp-short} resources by running the following command:
+
[source,terminal]
----
...
In previous {oadp-short} versions, the `node-agent` pods are labeled as `restic`.
====
