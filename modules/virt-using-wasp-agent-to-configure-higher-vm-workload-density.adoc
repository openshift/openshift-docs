...
* CLI
+
. Create a patch file that enables higher memory density and configures the overcommit rate:
+
[source,terminal]
----
$ cat patch.yaml
...
----
. Apply the patch file to your {VirtProductName} configuration:
+
[source,terminal]
----
$  oc -n openshift-cnv patch \
  HyperConverged/kubevirt-hyperconverged --type merge --patch-file patch.yaml
----
+
.Successful output
+
