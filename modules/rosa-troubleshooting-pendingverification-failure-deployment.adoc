...
= Failing to create a cluster with a `PendingVerification` error
...
Normally, this validation gets completed within 15 minutes, but in some cases it can take up to 4 hours for AWS to validate. In order to attempt successful provisioning, Red{nbsp} have configured our installer to reattempt installation if this issue occurs but the installation can still fail if the validation takes more time or if the validation itself fails.
