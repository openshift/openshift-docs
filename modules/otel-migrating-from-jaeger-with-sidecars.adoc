...
`endpoint`:: This endpoint points to the Gateway of a TempoStack instance deployed by using the `<example>` {TempoOperator}.
...
`<rule_a>`:: The `resourcedetectionprocessor` processor requires permissions for infrastructures and infrastructures/status.
