...
[role="_abstract"]
Use the Kubernetes API audit log to find the `SPO_EXEC_REQUEST_UID` correlation key so you can search the Advanced Audit Logging output for the same session. The log is YAML and includes that field.
