...
By default, running the `toolbox` command starts a container with the `registry.redhat.io/rhel10/support-tools:latest` image.
...
[NOTE]
====
If an existing `toolbox` pod is already running, the `toolbox` command outputs `'toolbox-' already exists. Trying to start...`. To avoid issues, remove the running toolbox container with `podman rm toolbox-` and then spawn a new toolbox container. 
====
