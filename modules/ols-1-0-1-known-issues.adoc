...
{ols-official} 1.0.1 contains a known networking limitation where the service incorrectly routes large language model (LLM) provider traffic through the cluster-wide proxy. This issue occurs even when the `no_proxy` environment variable is set, potentially affecting connectivity in restricted network environments.
