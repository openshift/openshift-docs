...
{gitops-title} 1.19.0 is available on {OCP} 4.14, 4.16, 4.17, 4.18, and 4.19.

[id="errata-updates-1-19-0_{context}"]
== Errata updates

RHEA-2025:XXXX - {gitops-title} 1.19.0 security update advisory::
Issued: 2025-XX-NN
+
The list of enhancements that are included in this release are documented in the following advisory:
+
* link:https://access.redhat.com/errata/RHEA-2025:XXXX[RHEA-2025:XXXX:NNNN]
...
----
+
For more details, see the Overview of the Argo CD Image Updater in the upstream documentation.
+
--
...
Consistent application deletion behavior across all UI views::
Before this update, deleting an application from the Resource Tree view treated the Application as a generic Kubernetes resource. This behavior could result in unexpected outcomes when performing non-cascading deletions. With this update, Argo CD applies consistent deletion behavior across all user interface views. When you delete an application from either the `Applications list` or the `Resource Tree view`, Argo CD uses the same deletion mechanism and presents the same options. In Argo CD 3.2, Argo CD correctly identifies Application resources in all views and uses the standard Application deletion API to ensure predictable and consistent results.
+
link:https://issues.redhat.com/browse/GITOPS-8075[GITOPS-8075]
