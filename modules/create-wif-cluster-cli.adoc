...
[NOTE]
====
The command displayed below is an example of a `non-interactive` command. Your parameters and parameter's values may differ.
====
+
[source,terminal]
----
...
--provider=gcp \ <2>
--ccs=true \ <3>
--subscription-type=marketplace-gcp \ <4>
--marketplace-gcp-terms=true \ <5>
--authentication-type <service-account|wif> \ <6>
--wif-config <wif_name> \ <7>
--region <gcp_region> \ <8>
--version <version> \ <9>
--multi-az=true  \ <10>
--enable-autoscaling=true \ <11>
--min-replicas=3 \ <12>
--max-replica=6 \ <13>
--secure-boot-fr-shielded-vms=true <14>
...
<9> test
<10> test
<11> Optional: Enable or disable autoscaling of compute nodes
<12> Optional: Specifies the minimum number of compute nodes when enabling autoscaling. Default: 2
<13> Optional: Specifies the maximum number of compute nodes when enabling autoscaling
<14> Optional:
