...
You can verify that proper configuration of resources associated with a wif-config are correct by running by executing `ocm gcp verify wif-config`. If a validation error within the configuration is found, you will be notified of the error and and given an actionable resolution.

You will need the ID or name of the the WIF config whose resources you would like to verify.
To obtain that information, run the following command:
...
To check if the associated resources for a wif-config are correct, run the following command:
[source,terminal]
----
$ ocm gcp verify wif-config --name <wif_name> <1>
----
<1> Replace `<wif_name>` with the returned name of the WIF configuration you want whose resources you want to verify.
--
.Example output
[source,terminal]
----
Error: verification failed with error: wif-config verification failed: failed to access the deployer service account
