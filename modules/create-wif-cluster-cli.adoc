...
You can verify the resources the proper configuration of resources associated with a wif-config by running the verify `ocm gcp verify wif-config`. If an error within the configuration is found, you can run the `update` command to resolve.
To verify all of your {product-title} clusters that have been deployed using the WIF authentication type, run the following command:
