...
:navtitle: Configure the {hcp} management cluster
[id="co-hcp-mgmt-config_{context}"]
= Configuring the {hcp} management cluster

[role="_abstract"]
If you are hosting your own {hcp-capital} or Hypershift environment and want to scan a Hosted Cluster from the management cluster, you will need to set the name and prefix namespace for the target Hosted Cluster. You can achieve this by creating a `TailoredProfile`.
...
`spec.extends`:: Specifies the name of the `Profile` object upon which the `TailoredProfile` is built. Only `ocp4-cis` and `ocp4-pci-dss` profiles are supported in {hcp} management clusters.
`spec.setValues.value`:: Specifies the output in the previous step.
`spec.setValues.value`:: Specifies the `NAMESPACE` from the output in the previous step.
