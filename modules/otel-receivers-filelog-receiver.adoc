...
[NOTE]
====
If your `include` glob pattern matches multiple actively-written files and you add `ordering_criteria.sort_by` to control the read order, always set `ordering_criteria.top_n` explicitly. As of {otel} Collector version 0.159, `top_n: 0` means "read all matched files" instead of "read only the first matched file," which can significantly increase the volume of logs read.
====
