...
. On the *Customize template parameters* page, expand *Storage* and select the *Boot from CD* checkbox.
. Select *Upload (Upload a new file to a PVC)* from the *CD source* list.
. Browse to the image file on your local machine and set the disk size.
. Click *Customize VirtualMachine*.
