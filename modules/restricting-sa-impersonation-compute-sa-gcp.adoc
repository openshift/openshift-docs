...
In the YAML file, this is the value of the `compute.platform.gcp.serviceAccount` parameter.
...
[source,terminal]
----
$ gcloud projects get-iam-policy demo-gcp-project \
...
----
+
where `<machine_api_role>` is the role ID of the custom role for the Machine API controller.
+
.Example output
[source,text]
----
...
. Grant the Machine API controller service account the Service Account User role for the service account of the compute nodes by running the following command:
+
[source,terminal]
----
...
This value is the `compute.platform.gcp.serviceAccount` parameter in your `install-config.yaml` file.
