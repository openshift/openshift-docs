...
democluster-us-east-1a   democluster   1               1               False         False        4.17.0    False             False            
----
...
$ podman run --entrypoint performance-profile-creator \
    ...
    --offlined-cpu-count=1 > my-hosted-cp-performance-profile.yaml
----
+
...
* `--topology-manager-policy=single-numa-node` specifies the NUMA topology policy. This must be set to 1 if installing the installing the NUMA Resources Operator. 
