...
= View the status of the {clo}

[role="_abstract"]
View the status of the {clo} to verify cluster logging health and deployment state.
...
status::
  The cluster status fields appear in the `status` stanza.
...
The following examples show some condition messages from the `Status.Nodes` section of the `ClusterLogging` instance.
