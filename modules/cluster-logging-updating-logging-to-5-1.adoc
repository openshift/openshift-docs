...
To update OpenShift Logging from 5.x to the current version, you change the subscriptions for the {es-op} and {clo}.

[IMPORTANT]
====
You must update the {es-op} _before_ you update the {clo}. You must also update _both_ Operators to the same version.
====
...
+
Verify that the {es-op} version is 5.x.x.
+
