...
The Envoy proxy component in Kourier handles inbound and outbound HTTP traffic for the Knative services. By default, Kourier contains an Envoy bootstrap configuration in the `kourier-bootstrap` configuration map in the `knative-serving-ingress` namespace. You can access the current configuration.
...
[source,terminal]
----
$ oc get cm kourier-bootstrap -n knative-serving-ingress
----
+
For example, with default configuration, the command produces the output that contains the following excerpts:
+
.Example output
[source,terminal]
----
