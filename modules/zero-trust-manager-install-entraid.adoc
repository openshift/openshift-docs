...
You need to configure Entra ID so that SPIRE can automatically provide your software workloads with short-lived, verifiable identities right within you infrastructure. The steps to do this include:
...
. Apply the Operator manifest. Copy the entire command block provided and paste it directory into your terminal. Press *Enter* to run.
+
[source,yaml]
----
oc apply -f - <<EOF
...
The SPIRE server, agent, CSI Driver, and OIDC Discovery Provider Operands need to be deployed so that {zero-trust-full} can use SPIFFE IDs.
...
. Define the JWT issuer endpoint for the OIDC provider, which are used for issuing JWT-SVIDs:
+
[source,terminal]
----
...
... To see if the certificate is provisioned and that its status is `Ready`, run the following command:
+
[source,terminal]
----
$ oc wait --for=condition=Ready certificate/$TLS_SECRET_NAME -n zero-trust-workload-identity-manager --timeout=5m
...
. Run the following command and ensure that the `$JWT_ISSUER_ENDPOINT` environment variable contains the hostname you configured in the earlier steps.
+
[source,terminal]
----
