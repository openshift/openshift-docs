...
[role="_abstract"]
You can enable Accelerated Networking on {azure-full} by adding `acceleratedNetworking` to your machine set YAML file.

.Prerequisites

* Have an existing {azure-full} cluster where the Machine API is operational.
...
`providerSpec.value.vmSize`:: Specifies a {azure-full} VM size that includes at least four vCPUs. For information about VM sizes, see link:https://docs.microsoft.com/en-us/azure/virtual-machines/sizes[{azure-full} documentation].
--
