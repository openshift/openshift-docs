...
:navtitle: Enable Accelerated Networking on an existing {azure-full} cluster
...
[role="_abstract"]
You can enable Accelerated Networking on {azure-full} by adding `acceleratedNetworking` to your machine set YAML file. Accelerated Networking uses SR-IOV to help improve network performance for new nodes.
...
`providerSpec.value.vmSize`:: Specifies an {azure-short} VM size that includes at least four vCPUs. For information about VM sizes, see the {azure-full} documentation link:https://docs.microsoft.com/en-us/azure/virtual-machines/sizes[Sizes for virtual machines in {azure-short}].
