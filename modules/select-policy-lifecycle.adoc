...
Audit logs::: Runtime policies that evaluate Kubernetes resource operations look for sensitive operations by using the Kubernetes audit log. You cannot configure enforcement for policies that use this source, because the operations have already occurred. Audit log policies support cluster and namespace scoping, but not deployment-level scoping, because audit events operate at the Kubernetes API level rather than at the deployment level.
