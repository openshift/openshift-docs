...
// * installing/installing_ibm_powervs/preparing-to-install-on-ibm-power-reg-vs.doc
...
ifeval::["{context}" == "preparing-to-install-on-ibm-power-reg-vs"]
:ibm-power-reg-vs:
endif::[]
...
The Cloud Credential Operator (CCO) manages cloud provider credentials as Kubernetes custom resource definitions (CRDs). To install a cluster on {iibm-cloud-bm-reg} Virtual Server, you must set the CCO to `manual` mode as part of the installation process.
