...
If your cluster uses the OVN-Kubernetes network plugin, assign `br-ex` as the value for `OPENSHIFT_HA_NETWORK_INTERFACE` so that no packet loss occurs. For a cluster that uses the OVN-Kubernetes network plugin, all listening interfaces do not serve VRRP but instead expect inbound traffic over a `br-ex` bridge.
