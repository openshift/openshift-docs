...
:navtitle: Prepare the registry node to host the mirrored registry
[id="ipi-install-preparing-a-disconnected-registry_{context}"]
= Preparing the registry node to host the mirrored registry

[role="_abstract"]
Prepare a registry node to host your local mirror registry for a {product-title} installation.
