...
The following diagram shows multiple VRF BGP instances that use VRF lite. This architecture supports only local gateway mode. VRF lite provides network virtualization by using UDNs to isolate pod traffic without incurring the heavy encapsulation typical of Multi-Protocol Label Switching (MPLS) or Ethernet Virtual Private Network (EVPN) protocols. Separate L3 links get mapped to specific VRFs, so independent BGP peering sessions route traffic to the next-hope router. Further, you can deploy this L3 mechanism to multi-cloud deployments to allow certain namespaces over the network.

.VRF lite with L3 links to separate VRFs
image::513-2-OpenShift-bgp-vrf-lite_0426.png[Image of VRF lite with L3 links to separate VRFs]
