...
:navtitle: Upload a custom {op-system} AMI in AWS
...
. Export your {aws-short} profile as an environment variable by running the following command:
+
[source,terminal]
----
...
----
+
Replace `<aws_profile>` with the {aws-short} profile name that holds your {aws-short} credentials, such as `govcloud` or `beijingadmin`.
...
----
+
Replace `<aws_region>` with the {aws-short} region, such as `us-gov-east-1` or `cn-north-1`.
...
----
+
Replace `<version>` with the {op-system} VMDK version, such as `{product-version}.0`.
...
. Create the `containers.json` file and define your {op-system} VMDK file by running the following command:
+
[source,terminal]
----
...
. Import the {op-system} disk as an Amazon EBS snapshot by running the following command:
+
[source,terminal]
----
...
`--disk-container`:: Specifies the file path to the JSON file describing your {op-system} disk. The JSON file should contain your Amazon S3 bucket name and key.
...
. Create a custom {op-system} AMI from the {op-system} snapshot by running the following command:
+
[source,terminal]
----
