...
[role="_abstract"]
The {gitops-title} Operator enhances multi-instance support by improving annotation-based resource tracking in Argo CD. Assign a unique installationID to each Argo CD instance to distinguish resources with identical application names, prevent conflicts and infinite sync loops, and enable multiple instances to operate safely in parallel.
