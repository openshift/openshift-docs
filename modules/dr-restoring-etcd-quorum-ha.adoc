...
[role="_abstract"]
You can restore etcd quorum on high availability (HA) clusters by running the `quorum-restore.sh` script on a recovery host. Restored quorum returns the {product-title} API to read and write mode when quorum loss takes the cluster offline.
...
.. Choose a pod by running the following command:
+
[source,terminal]
----
...
. Using SSH, connect to the chosen recovery node to restore the etcd quorum by running the following command:
+
[source,terminal]
----
...
.. Obtain the machine for one of the offline nodes by running the following command:
+
[source,terminal]
----
