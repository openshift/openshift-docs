...
* You have prior knowledge of the IP addresses or hostnames for your control plane nodes.

.Procedure

. Select an operational control plane host to use as the recovery host by using its known IP address or hostname.
+
[NOTE]
====
During total etcd quorum loss, the `kube-apiserver` does not respond to requests, making `oc` CLI commands unusable. You must use previously documented node IP addresses or out-of-band management tools to select a running control plane host.
====

. Using SSH, connect to the chosen recovery node.

. Verify that the local etcd container is running on the host by using the container runtime CLI:
+
[source,terminal]
----
...
.. In a terminal that has access to the cluster as a `cluster-admin` user, obtain the machine for one of the offline nodes by running the following command:
+
[source,terminal]
----
...
. For each node that is offline, repeat the previous steps to delete and re-create the node.
