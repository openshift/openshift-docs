...
:navtitle: Create the persistent volume claim
[id="creating-volume-claim_{context}"]
= Creating the persistent volume claim

[role="_abstract"]
You can create a persistent volume claim to dynamically provision and bind storage from a pre-configured storage class, so that your applications can consume persistent storage in {product-title}.
