...
* `seLinuxContext` is set to `MustRunAs` and `seLinuxOptions.type: container_engine_t`.
* `runAsUser` is set to `MustRunAsRange`.
* `requiredDropCapabilities` is set to `null`.
* `userNamespaceLevel` is set to `RequirePodLevel`.

This SCC allow a user to run a container engine inside of an {product-title} pod.
