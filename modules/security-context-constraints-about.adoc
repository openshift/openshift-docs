...
* Whether a pod is constrained by `no_new_privs` (allowPrivilegeEscalation)
...
[NOTE]
====
The `restricted` SCC still configures `allowPrivilegeEscalation: true`, which disables the `no_new_privs` protection.  If you wish to prevent this in your cluster, you can create a custom SCC with the flag disabled and configure appropriate users to only have access to this new SCC in place of `restricted`. For more details on flag, see link:https://www.kernel.org/doc/html/latest/userspace-api/no_new_privs.html[No New Privileges Flag].
====
