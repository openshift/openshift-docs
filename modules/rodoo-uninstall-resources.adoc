...
:navtitle: Uninstall {run-once-operator} resources
[id="rodoo-uninstall-resources_{context}"]
= Uninstalling {run-once-operator} resources

[role="_abstract"]
Optionally, after uninstalling the {run-once-operator}, you can clean up remaining resources by deleting custom resource definitions, the operator namespace, and namespace labels.
