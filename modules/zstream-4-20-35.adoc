...
* Before this update, when more than 70 primary user-defined networks (UDNs) were created on a cluster, broadcast and Address Resolution Protocol (ARP) packets exceeded the Open vSwitch (OVS) resubmit action limit. As a consequence, broadcast and ARP packet flooding caused resolution failures and pod reachability issues for some UDNs. With this release, the OVS resubmit action limit is increased to accommodate more UDNs. As a result, the ARP resolution failures are resolved, which ensures proper pod reachability. (link:https://redhat.atlassian.net/browse/OCPBUGS-105557[OCPBUGS-105557])
