...
[NOTE]
====
You should only modify the {op-system} image to use GCP Marketplace images for compute machines. Control plane machines do not contribute to licensing costs when using the default image, but if you apply a GCP Marketplace image for a control plane machine, usage costs will apply. In addition, infrastructure machines can run the default image without incurring licensing costs, but if you apply a GCP Marketplace image to infrastructure machines they will incur licensing costs.
====
