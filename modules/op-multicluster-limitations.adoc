...
[NOTE]
====
Both the Stop and Cancel actions are designed to run finally tasks as part of cleanup. Canceling from the spoke cluster allows finally tasks to run properly.
====
...
You cannot use `pipelineRef` or `taskRef` to reference existing pipelines or tasks stored in the cluster by using the cluster resolver. Instead, you must use one of the following approaches:
...
** `tkn taskrun list` - Returns no results because task runs run on spoke clusters
