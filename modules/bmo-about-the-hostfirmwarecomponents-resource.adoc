...
You can update the host firmware components of NICs on supported servers. The supported servers are the Intel Ethernet 800 Series (`ice` driver) and the NVIDIA Mellanox ConnectX-6 (CX6) and ConnectX-7 (CX7) (`mlx_5` driver). The following lists important considerations before you update the NICs for these servers: The following list outlines important considerations before you update the NICs:
...
The `spec` section of the `HostFirmwareComponents` resource defines the desired state of the host's BIOS and BMC versions, and, if the information is available by using Redfish, the host's firmware NIC components.
...
The `status` section of the `HostFirmwareComponents` resource returns the current status of the host's BIOS and BMC versions, and, if the information is available by using Redfish, the host's firmware NIC components.
