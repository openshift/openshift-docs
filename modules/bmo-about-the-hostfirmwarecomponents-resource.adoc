...
Metal^3^ provides the `HostFirmwareComponents` resource, which describes BIOS and baseboard management controller (BMC) firmware versions. To deploy a more versatile bare-metal environment and support the adoption of multi-architecture capabilities, you can update the host firmware components of network interface controllers (NICs) on supported Dell and Hewlett Packard Enterprise (HPE) servers.

The supported servers are the Intel Ethernet 800 Series (`ice` driver) and the NVIDIA Mellanox ConnectX-6 (CX6) and ConnectX-7 (CX7) (`mlx_5` driver). The following lists important considerations before you update the NICs for these servers:
...
* If a network adapter is listed on the HPE Fabric and Connectivity, this does not necessarily mean you can update the adapter. A machine might use Redfish to display their NIC and firmware information, but prevent you from updating the adapter through Redfish.
...
The `spec` section of the `HostFirmwareComponents` resource defines the desired state of the host's BIOS and BMC versions, and the host's firmware NIC components for the Intel Ethernet 800 Series and the NVIDIA Mellanox ConnectX-6 (CX6) and ConnectX-7 (CX7).
...
* `component`: The name of the component. The valid settings are `bios`, `bmc`, `nic:<ID1>`, and `nic:<ID2>`.
...
The `status` section of the `HostFirmwareComponents` resource returns the current status of the host's BIOS and BMC versions and the host's firmware NIC components for the Intel Ethernet 800 Series and the NVIDIA Mellanox ConnectX-6 (CX6) and ConnectX-7 (CX7).
