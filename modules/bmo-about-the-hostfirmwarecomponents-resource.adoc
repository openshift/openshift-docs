...
[role="_abstract"]
The `HostFirmwareComponents` resource tracks and manages the firmware versions for a host's BIOS, baseboard management controller, and network interface cards. You can use this resource to define desired firmware states and monitor update status, ensuring that compatible hardware components remain secure and up to date.
