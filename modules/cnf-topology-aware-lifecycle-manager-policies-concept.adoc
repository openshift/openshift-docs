...
[role="_abstract"]
The {cgu-operator-first} remediates a set of `inform` policies for the clusters specified in the `ClusterGroupUpgrade` custom resource (CR). {cgu-operator} remediates `inform` policies by controlling the `remediationAction` specification in a `Policy` CR through the `bindingOverrides.remediationAction` and `subFilter` specifications in the `PlacementBinding` CR. Each policy has its own corresponding {rh-rhacm-first} placement and {rh-rhacm} placement binding.

[NOTE]
====
{cgu-operator} uses the {rh-rhacm} `Placement` CR for cluster selection during policy remediation. The `Placement` CR requires a `ManagedClusterSetBinding` CR in each policy namespace. If you are using a deprecated `PolicyGenTemplate` CR, you must create a `ManagedClusterSetBinding` in each policy namespace. For more information, see the Knowledgebase solution link:https://access.redhat.com/articles/7147283[{cgu-operator} Placement API Migration - Mitigation Plan].
====

One by one, {cgu-operator} adds each cluster from the current batch to the placement that corresponds with the applicable managed policy. If a cluster is already compliant with a policy, {cgu-operator} skips applying that policy on the compliant cluster. {cgu-operator} then moves on to applying the next policy to the non-compliant cluster. After {cgu-operator} completes the updates in a batch, all clusters are removed from the placements associated with the policies. Then, the update of the next batch starts.
