...
* For cluster installations with custom DNS configurations, the installation program connects to the IP address of the cloud-provider load balancer provisioned by the installation program, because the program did not provision any DNS records.
As a result, the installation program fails to connect to the cluster if access is routed through a DNS-based proxy, such as dnsmasq. The cluster installation might still complete successfully, but the installation program is not able to confirm a successful installation.
+
As a workaround, confirm that the cluster is in a healthy state by running the following command:
+
[source,terminal]
----
...
----
+
When the cluster Operators report as `available`, you can then configure your external DNS and proceed to use the cluster as normal.
(link:https://issues.redhat.com/browse/OCPBUGS-54901[OCPBUGS-54901])
