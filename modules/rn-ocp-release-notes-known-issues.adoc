...
* For cluster installations with custom DNS configurations, the installation program fails to connect to the cluster if access is routed through a DNS-based proxy, such as dnsmasq. The cluster installation might still complete successfully, but the installation program is not able to confirm a successful installation.
+
...
+
As an alternative workaround, while installing a private cluster, you can run the installation program on a bastion host that is within the same Virtual Private Cloud (VPC) as the cluster to be deployed.
