...
----
ifdef::openshift-rosa-hcp[]
[id="rosa-create-idms_{context}"]
== create image mirror configurations
...
{product-title} clusters must be in the **Ready** state in order to create image mirrors.
====
Creates image mirror configurations for a {product-title} cluster.
...
--
.Examples
Add an image mirror configuration to a cluster named `mycluster`.
...
Add an image mirror configuration with a specific type to a cluster.
...
Add multiple mirror image configurations to a cluster.
