...
[IMPORTANT]
====
You can only use external authentication providers on {product-title} clusters.
====
...
Create a network that creates any necessary AWS resources through AWS CloudFormation templates. This helper command is intended to help create and configure a VPC for use with {product-title}. This command also supports {zero-egress} clusters.
