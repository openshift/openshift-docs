...
where:
+
* The `machineConfigPoolSelector` label must match the setting in the `NUMAResourcesOperator` CR that you configure later in "Creating the NUMAResourcesOperator custom resource".
* For `cpuManagerPolicy`, `static` must use a lowercase `s`.
* Adjust `reservedSystemCPUs` based on the CPU on your nodes.
