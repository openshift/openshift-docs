...
[role="_abstract"]
You can add a custom certificate to an existing installation by using the Operator, Helm, or roxctl CLI.

.Procedure

* If you have installed {product-title} using the Operator:
+
. Create a `central-default-tls-cert` secret that has the appropriate TLS certificates in the namespace where you installed the Central service by entering the following command:
+
...
. Use the `helm upgrade` command and give the updated configuration file:
+
[source,terminal]
----
...
+
--
`--reuse-values`:: Specifies that the upgrade will reuse existing values. You must use this parameter because the `values-private.yaml` file does not contain all of the required configuration values.
--
...
----
After you run this command, Central automatically applies the new key and certificate without restarting the pod. It might take up to a minute to propagate the changes.