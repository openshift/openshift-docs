...
:navtitle: Uninstall the Multiarch Tuning Operator by using the CLI
...
. Delete the CSV for the Multiarch Tuning Operator in the target namespace by using the `currentCSV` value by running the following command:
+
[source,terminal]
----
