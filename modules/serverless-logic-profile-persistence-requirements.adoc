...
The persistence configurations provided for both `SonataFlowPlatform` custom resource (CR) and `SonataFlow` custom resource (CR) apply equally to the `preview` and `gitops` profiles. However, these configurations should not be used with the `dev` profile, as this profile ignores them entirely.
