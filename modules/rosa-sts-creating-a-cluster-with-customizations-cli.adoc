...
[NOTE]
====
In this step, the {rosa-cli} also automatically creates the account-wide Operator IAM policies that are used by the cluster-specific Operator policies to permit the {product-title} cluster Operators to run core OpenShift functionality. For more information, see _Account-wide IAM role and policy reference_.
====
--
...
----
{
    ...
    "Statement": [
        ...
        {
            "Sid": "Allow {product-title} use of the key",
            "Effect": "Allow",
            "Principal": {
                "AWS": [
...
* In the `Sid: "Allow {product-title} use of the key"` and `Sid: "Allow attachment of persistent resources"` statements, add the ARN for the account-wide role that will be used when you create the {product-title} cluster (for example, `arn:aws:iam::<aws_account_id>:role/ManagedOpenShift-Installer-Role`).

* In the `Sid: "Allow {product-title} use of the key"` and `Sid: "Allow attachment of persistent resources"` statements, add the ARN for the operator role that will be used when you create the {product-title} cluster (for example, `arn:aws:iam::<aws_account_id>:role/<cluster_name>-xxxx-openshift-cluster-csi-drivers-ebs-cloud-credent`).
--
