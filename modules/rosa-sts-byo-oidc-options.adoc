...
[role="_abstract"]
The `rosa create oidc-config` command accepts several optional parameters. Running the `rosa create oidc-config` command without parameters creates an unmanaged OpenID Connect (OIDC) configuration.

[NOTE]
====
You are required to register the unmanaged OIDC configuration by posting a request to `/oidc_configs` through {cluster-manager}. You receive an ID in the response. Use this ID to create a cluster.
====
...
You use these files to set up the endpoint. This endpoint responds to `/.well-known/openid-configuration` with the discovery document and on `keys.json` with the JSON Web Key Set. The private key is stored in Amazon Web Services (AWS) Secrets Manager Service (SMS) as plain text.
...
Allows you to specify the mode to create your OIDC configuration. With the `manual` option, you receive AWS commands that set up the OIDC configuration in an S3 bucket. This option stores the private key in the Secrets Manager. With the `manual` option, the OIDC Endpoint URL is the URL for the S3 bucket. You must retrieve the Secrets Manager Amazon Resource Name (ARN) to register the OIDC configuration with {cluster-manager}.

You receive the same OIDC configuration and AWS resources as the `manual` mode when using the `auto` option. A significant difference between the two options is that when using the `auto` option, {product-title} calls AWS, so you do not need to take any further actions. The OIDC Endpoint URL is the URL for the S3 bucket. The {rosa-cli} retrieves the Secrets Manager ARN, registers the OIDC configuration with {cluster-manager}, and reports the second `rosa` command that the user can run to continue with the creation of the STS cluster.
