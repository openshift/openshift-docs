...
= Glossary of common terms for the monitoring stack
...
The Cluster Version Operator (CVO) manages the lifecycle of cluster Operators, many of which are installed in your cluster by default.
...
etcd::
etcd is the key-value store, which stores the state of all resource objects.
...
node::
A compute machine in your cluster. A node is either a virtual machine (VM) or a physical machine.

Operator::
The preferred method of packaging, deploying, and managing a Kubernetes application in your cluster. An Operator takes human operational knowledge and encodes it into software that is packaged and shared with customers.
...
Prometheus::
Prometheus is the monitoring system on which the {product-title-lowercase} is based. Prometheus is a time-series database and a rule evaluation engine for metrics. Prometheus sends alerts to Alertmanager for processing.
...
* {product-dedicated} supports many types of storage on AWS and GCP.
* {product-rosa-hcp} and {product-rosa} support many types of storage on AWS.
--
+
You can manage container storage for persistent and non-persistent data in your cluster.
...
Vector::
Vector is a log collector that deploys to each node. It collects log data from each node, transforms the data, and forwards it to configured outputs.

web console::
A user interface (UI) to manage your cluster.
