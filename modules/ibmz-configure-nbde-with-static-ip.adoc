...
= Configuring NBDE with static IP in an {ibm-z-reg} or {ibm-linux-one-reg} environment

Enabling NBDE disk encryption in an {ibm-z-reg} or {ibm-linux-one-reg} environment requires additional steps, which are described in detail in this section.
