...
= Gather logs from failed Agent-based installations

[role="_abstract"]
Collect debug logs from a failed Agent-based Installer deployment to diagnose issues and provide diagnostic data to Red Hat Support.
...
. If the output from the previous command indicates a failure, or if the bootstrap is not progressing, connect to the rendezvous host and collect diagnostic data. Run the following command:
+
[source,terminal]
----
...
[NOTE]
====
Red{nbsp}Hat Support can diagnose most issues using the data gathered from the rendezvous host. If some hosts are not able to register, gathering this data from every host might be helpful.
====
...
.. Create a compressed file from the `must-gather` directory that was created in your working directory by running the following command:
+
[source,terminal]
----
