...
[role="_abstract"]
Logging 6 introduces a new API under the observability.openshift.io group for log collection and forwarding.

Log collection and forwarding configurations are now specified under a new API, part of the `observability.openshift.io` API group. The following sections highlight the differences from the old API resources.
...
[id="syslog-output-changes_{context}"]
== Syslog output changes

The `addLogSource` parameter has been removed from the syslog output configuration. Use the `enrichment` parameter instead to add Kubernetes metadata fields to log records.

.5.x syslog output with addLogSource
[source,yaml]
----
apiVersion: "logging.openshift.io/v1"
kind: ClusterLogForwarder
...
.6.x syslog output with enrichment
[source,yaml]
----
apiVersion: "observability.openshift.io/v1"
kind: ClusterLogForwarder
...
The `enrichment: KubernetesMinimal` parameter adds minimal Kubernetes metadata fields such as namespace, pod name, and container name to each log record, providing the same functionality as `addLogSource: true` in previous releases.
...
The following example shows a Logging 6.x output configuration by using a service account token and config map:
[source,yaml]
----
...
spec:
...
The following example shows a Logging 6.x output authentication and TLS configuration by using secrets:
[source,yaml]
----
...
spec:
...
* link:https://github.com/openshift/cluster-logging-operator/blob/master/docs/reference/operator/api_observability_v1.adoc[API reference for observability.openshift.io]
