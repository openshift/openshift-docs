...
----
+
The output shows the CPUs that get allocated to a `NUMA` node, such as `NUMA node1`, but the SR-IOV network resource can use the NIC form another `NUMA` node, such as `NUMA node0`.
+
[source,terminal]
----
...
----
+
The output shows the `NUMA` node that associates with a NIC. 
.Example output
[source,terminal]
+
[NOTE]
