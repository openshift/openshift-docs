...
[source,terminal]
----
$ helm upgrade -i grafana-operator oci://ghcr.io/grafana/operator \
    --version 5.24.0 \
...
. Deploy the DTS console dashboard for OpenShift Console integration:
+
The DTS console dashboard integrates DPU telemetry metrics directly into the OpenShift web console, providing administrators with in-console visibility of DPU performance without requiring separate tools:
+
[source,yaml]
----
apiVersion: v1
