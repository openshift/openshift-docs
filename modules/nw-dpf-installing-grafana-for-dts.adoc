...
+
The community Grafana Operator requires additional RBAC permissions to manage {product-title} routes. Create a file named `grafana-operator-route-rbac.yaml` with the following content:
+
[source,yaml]
----
