...
[NOTE]
====
Neither the DPF Operator nor the DTS DPUService installs Grafana on {product-title}. Red Hat does not offer a certified Grafana Operator. The community Grafana Operator is the standard way to run Grafana on {product-title}.
====
...
+
The DTS Grafana dashboard adds a dedicated view for DPU hardware telemetry with per-DPU dropdown filters. It consists of a `ConfigMap` containing the dashboard JSON and a `GrafanaDashboard` CR that loads it into the Grafana instance:
+
[source,yaml]
----
apiVersion: v1
...
----
+
Open the returned URL in a web browser. Anonymous access is read-only. To edit dashboards, click **Sign in** and use `admin` / `admin`.
