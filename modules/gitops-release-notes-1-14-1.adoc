...
where:
+
`'false'`:: Specifies the specify this value to enable or disable the cluster-scoped installation. If the value is set to `'false'`, it means that the you have enabled cluster-scoped installation. If it is set to `'true'`, it means that you have enabled namespace-scoped installation. If the value is empty, it is set to `false`.
`test-123-cluster-scoped,test-456-cluster-scoped`:: Specifies a comma-separated list of namespaces that can host a cluster-scoped Argo CD instance, for example `test-123-cluster-scoped,test-456-cluster-scoped`.