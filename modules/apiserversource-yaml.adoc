...
. To verify the API server source configuration, create a Knative service as a YAML file that dumps incoming messages to its log:
+
[source,yaml]
----
...
. Create a `Trigger` object as a YAML file that filters events from the `default` broker to the service created in the earlier step:
+
[source,yaml]
----
apiVersion: eventing.knative.dev/v1
...
. Check that the controller maps correctly by entering the following command and inspecting the output:
+
[source,terminal]
----
