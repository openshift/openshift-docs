...
"VpcCidr":: Specifies the CIDR block for the VPC.
"10.0.0.0/16":: Specifis a CIDR block in the format `x.x.x.x/16-24`.
"AvailabilityZoneCount":: Specifies the number of availability zones to deploy the VPC in.
"1":: Specifies an integer between `1` and `3`.
"SubnetBits":: Specifies the size of each subnet in each availability zone.
"12":: Specifies an integer between  `5` and `13`, where `5` is `/27` and `13` is `/19`.
