...
| `DockerImage` | References or retrieves an image from an external registry. Uses the standard `docker pull` specification. | `openshift/ruby-20-centos7:2.0`, `quay.io/openshift-community/rhel7:latest`, `centos/ruby-22-centos7@sha256:3a335d7d...` 
endif::[]
|===
