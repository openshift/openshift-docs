...
ifdef::openshift-dedicated[]
. Optional: By default, {product-title} on {GCP} instances in the machine pools inherit the Shielded VM settings at the cluster level. You can override the cluster level Shielded VM settings at the machine pool level by selecting or deselecting *Enable Secure Boot support for Shielded VMs*.
+
[IMPORTANT]
====
