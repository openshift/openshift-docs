...
= Creating a machine pool using {cluster-manager}
...
[IMPORTANT]
====
The compute, also known as worker, node instance types, autoscaling options, and node counts that are available depend on your {product-title} subscriptions, resource quotas and deployment scenario. For more information, contact your sales representative or Red{nbsp}Hat support.
====
endif::openshift-rosa,openshift-rosa-hcp[]
...
[IMPORTANT]
====
Once a machine pool is created, the *Enable Secure Boot support for Shielded VMs* setting cannot be changed. This setting is not supported for {product-title} on {GCP} clusters created using bare-metal instance types.
====
