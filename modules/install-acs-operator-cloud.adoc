...
+
If you select manual updates, when a newer version of the Operator is available, OLM creates an update request. As a cluster administrator, you must manually approve the update request to update the Operator to the latest version. For {product-title-managed-short} secured clusters, use the latest version of the software that is available. (The software version for Central is automatically updated.)
. Click *Install*.
