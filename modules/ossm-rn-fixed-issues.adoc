...
* https://issues.redhat.com/browse/OSSM-2221[OSSM-2221] Gateway injection does not work in control plane namespace. If you use the Gateway injection feature to create a gateway in the same location as the control plane, the injection now succeeds on v2.4 SMCP. The gateway is created in the same location as the control plane.
