...
[role="_abstract"]
To provide the identity that the installation program requires on {azure-short}, you can create a service principal.
...
`<appId>`:: Specifies the `appId` parameter value for your service principal.
`<subscription_id>`:: Specifies the subscription ID.
