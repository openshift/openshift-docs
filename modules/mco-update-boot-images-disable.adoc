...
Disabling this feature does not rollback the nodes or machine sets to the originally-installed boot image. The machine sets retain the boot image version that was present when the feature was disabled and is not updated if the cluster is upgraded to a new {product-title} version in the future. Because a boot image is used only when a node is scaled up, this feature has no effect on existing nodes.
...
* Optional: Disable the b	ehavior for all machine sets:
+
[source,yaml]
----
