...
<2> Specifies an API group. This must be `machine.openshift.io`.
<3> Specifies the resource within the specified API group to apply the change to. This must be `machinesets`.
<4> Specifies that the default behavior is disabled on the nodes in specific machine sets.
<5> Specifies that any machine set with this label is to be updated. A positive match of the label selector disables the default behavior on the nodes in that machine set.
