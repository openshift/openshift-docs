...
ifdef::post[]
. Check that the `sys/fs/cgroup/cgroup` file is present on your nodes. This file is created by cgroup v1:
+
[source,terminal]
----
...
ifdef::nodes[]
. Check that the `sys/fs/cgroup/cgroup2fs` or `sys/fs/cgroup/cgroup` file is present on your nodes:
+
.Example command for cgroup v2
[source,terminal]
----
$ stat -c %T -f /sys/fs/cgroup2fs
----
+
.Example output for cgroup v2
[source,terminal]
