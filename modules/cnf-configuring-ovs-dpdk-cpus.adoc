...
* Workload partitioning is enabled at install time by setting `cpuPartitioning: AllNodes`, or the `kubelet` CPU Manager `strict-cpu-reservation` policy option is configured on the target nodes. The performance profile populates the workload partitioning CPU sets.
...
* `performance.openshift.io/cpu-load-balancing-ovs-dpdk` is optional. Set to `disable` to exclude `ovsDpdk` CPUs from kernel scheduler load balancing. Omit the annotation or set to `enable` to keep the default behavior.
* `spec.cpu.ovsDpdk` lists CPUs reserved for OVS-DPDK PMD threads. The PPC tool does not generate this field. You must add it manually. Always quote CPU set values because YAML interprets unquoted single numbers as integers, and the API requires strings.
