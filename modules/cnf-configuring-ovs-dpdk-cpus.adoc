...
----
<1> For {sno}, use `--mcp-name=master`.
+
For more information about PPC arguments, see xref:../scalability_and_performance/cnf-tuning-low-latency-nodes-with-perf-profile.adoc#running-the-performance-profile-profile-cluster-using-podman_{context}[Running the Performance Profile Creator using Podman].

. Edit the generated profile to add `spec.cpu.ovsDpdk` and optionally add the load-balancing annotation. Move the CPUs you want to assign to OVS-DPDK from the `isolated` set to the new `ovsDpdk` field so that the sets remain disjoint.
+
...
----
<1> Optional. Set to `disable` only when you need stronger scheduler isolation for PMD threads. Omit the annotation for the base reservation. For why and when to use this annotation, see _Disable load balancing on OVS-DPDK CPUs_.
<2> The PPC tool does not generate the `ovsDpdk` field. You must add this field manually.
+
where:
