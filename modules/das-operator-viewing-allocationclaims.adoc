...
* You have installed the Dynamic Accelerator Slicer (DAS) Operator.
* You have deployed workloads that request GPU resources.

.Procedure

. View all `AllocationClaim` resources in the `das-operator` namespace by running the following command:
+
[source,terminal]
----
...
. View `AllocationClaim` resources with specific state by running the following command:
+
[source,terminal]
----
...
. Monitor `AllocationClaim` events by running the following command:
+
[source,terminal]
----
