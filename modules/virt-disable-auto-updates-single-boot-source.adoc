...
. Disable automatic updates for an individual boot source by editing the `spec.workloadSources.dataImportCronTemplates` field.
+
Custom boot source::
...
.. Add the boot source to `spec.workloadSources.dataImportCronTemplates`.
+
[NOTE]
====
