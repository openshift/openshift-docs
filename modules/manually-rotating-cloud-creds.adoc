...
[role="_abstract"]
When cloud provider credentials change outside the cluster, update the secret that the Cloud Credential Operator (CCO) uses so that cluster components can authenticate with the new credentials.
