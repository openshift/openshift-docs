...
You can onfigure the write permissions for tenants from the *Administrator* view of the web console or command line.
...
. Add the tenants to a `ClusterRole` with the write (`create`) permissions to write traces.
+
...
. Grant the OpenTelemetry Collector the write permissions by defining a ClusterRoleBinding to attach the OpenTelemetry Collector ServiceAccount.
+
...
subjects:
  - kind: ServiceAccount
    ...
    namespace: otel
----
<1> The ServiceAccount that you created in a previous step. The client uses it when exporting trace data.
...
<3> The ServiceAccount token. The client sends the token to the tracing pipeline service as the bearer token header.
