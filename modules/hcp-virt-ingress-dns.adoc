...
Every {product-title} cluster includes a default application Ingress Controller, which must have an wildcard DNS record associated with it. By default, hosted clusters that are created by using {VirtProductName} automatically become a subdomain of the {product-title} cluster that the virtual machines run on.
