...
.. Create a YAML file, such as `cluster-udn-localnet.yaml`, to define your request for a `Layer2` topology as in the following example:
+
[source, yaml]
----
...
----
+
Where `<example_cluster_udn>.yaml` is the name of your `Localnet` configuration file.
