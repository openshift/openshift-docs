...
You can customize template or VM parameters, such as data sources, Cloud-init, or SSH keys, before you start the VM.
...
.. Click the *Scripts* tab to edit the parameters that must be set before the VM boots, such as `Cloud-init`, `SSH key`, or `Sysprep`.
.. Optional: Click the *Start this virtualmachine after creation* checkbox.
