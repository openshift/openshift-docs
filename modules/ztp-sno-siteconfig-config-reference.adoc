...
|Specifies the device for deployment. Identifiers that are stable across reboots are recommended. For example, `wwn: <disk_wwn>` or `deviceName: /dev/disk/by-path/<device_path>`. Values using a `by-path` scheme are preferred. For a detailed list of stable identifiers, see the "About root device hints" section.
