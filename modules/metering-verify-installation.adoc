...
. Navigate to *Workloads* -> *Pods* in the `openshift-metering` namespace and verify that Pods are being created. This can take up to several minutes after installing the metering stack.
+
...
. Use the oc CLI to verify that the `ReportDataSources` are beginning to import data, indicated by a valid timestamp in the `EARLIEST METRIC` column (this may take a few minutes). We filter out the "-raw" `ReportDataSources` which do not import data:
+
----
