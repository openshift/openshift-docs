...
. Enable `devEnableEvictionsInBackground` to improve VM eviction stability during live migrations. This option allows evictions to occur in the background, mitigating issues with oscillatory behavior during live migration:
+
[source,yaml]
----
apiVersion: operator.openshift.io/v1
...
This setting is useful when using the `LowNodeUtilization` profile to ensure more stable evictions.
