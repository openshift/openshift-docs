...
. Create a VPC with at least one private subnet. Ensure that your machine's classless inter-domain routing (CIDR) matches your virtual private cloud's CIDR. For more information, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html-single/installing_on_aws/index#installation-custom-aws-vpc-requirements_installing-aws-vpc[Requirements for using your VPC] and link:https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html-single/installing_on_aws/index#installation-custom-aws-vpc-validation_installing-aws-vpc[VPC validation].
+
[IMPORTANT]
====
...
. Create the account-wide Identity and Access Management (IAM) roles by running the following command:
+
[source,terminal]
----
...
. Create the OpenID Connect (OIDC) configuration by running the following command:
+
[source,terminal]
----
