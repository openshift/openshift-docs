...
This explains how to create heterogeneous nodepools on an agent platform. Please refer to set up the env using Agent Platform from section 4.6.1, this section only covers the things needed to configure to have heterogeneous nodepools.
...
[id="hcp-configure-dns"]
== Configure DNS

*.apps.<cluster_name> record can be pointed to either one of the worker nodes where ingress application is hosted, or if you are able to set up a load balancer on top of the worker nodes, point this record to this load balancer. When you are creating a heterogeneous nodepool, please make sure the workers are reachable from each other or keep them in the same network.

.Procedure

. Create a Hosted Cluster
+
Use a multi-architecture release image when creating the cluster to enable support for heterogeneous nodepools. Find the latest multi arch images from here.
+
[source,terminal]
----
...
[id="hcp-create-heterogeneous-nodepools"]
...
Once your agents are approved, you can scale the nodepools. agentLabelSelector configured in nodepool ensures that only matching agents added to the cluster. This also aids in descaling the nodepool. To remove specific architecture nodes from the cluster, you can descale the corresponding nodepool.
