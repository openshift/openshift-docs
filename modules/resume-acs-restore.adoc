...
[role="_abstract"]
If your connection drops during a restore operation or you need to go offline, you can resume the restore operation.
...
* If the connection drops, the `roxctl` CLI automatically attempts to resume a task as soon as the connection is available again.
...
* If a restore operation stalls, you have canceled it, or the time has expired, you can resume the earlier restore by running the original command again.
...
* If a restore operation stops, you must restart it within 24 hours and before restarting Central, otherwise {product-title-short} cancels the restore operation.
====