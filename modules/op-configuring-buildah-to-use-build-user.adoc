...
$ oc get task buildah -n openshift-pipelines -o yaml | \
  ...
  yq '.kind="Task"' | \
  yq '.metadata.name="buildah-as-user"' | \
  oc create -f -
----
...
----
+
`steps.securityContext.runAsUser`:: Run the container explicitly as the user id `1000`, which corresponds to the `build` user in the Buildah image.
`steps.script`:: Display the user id to confirm that the process is running as user id `1000`.
`steps.volumeMounts.mountPath`:: You can change the path for the volume mount as necessary.
