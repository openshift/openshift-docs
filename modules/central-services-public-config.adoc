...
== StackRox Scanner
The following table lists the configurable parameters for the StackRox Scanner. Before release 4.8, the StackRox Scanner was the default scanner used for node, platform, and image scanning. After release 4.8, Scanner V4 is the default scanner. See the next table for Scanner V4 parameters.
...
| `scannerV4.disable`
| When this value is `false`, Scanner V4 is enabled. When you set this parameter to `true` with the `helm upgrade` command, Helm removes the existing Scanner V4 deployment.
//is this correct?
