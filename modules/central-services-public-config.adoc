...
Policy as code provides a way to configure {product-title-short} to work with a continuous delivery tool such as Argo CD to track, manage, and apply policies that you have authored locally. You can also use the API to configure connections to your own GitOps repository such as GitHub.

|===
| Parameter | Description

| `configAsCode.enabled`
| Set to `true` to enable the policy as code feature.
