...
| The name of the PVC to manage persistent data for Scanner V4. By default, for Central, the system creates a PVC and uses the default value of `scanner-v4-db` for the name. To prevent data loss, the PVC is not removed automatically when Central is deleted.
