...
+
Ensure the *Issuer URL* and *Subject* fields are correctly set to match your {ocp} service account that ESO uses.
...
+
--
`spec.robotAccount`:: Specifies the actual name of your Quay robot account.
--
.Verification
...
`<namespace_where_secret_exists>`:: Specifies the namespace where you created the secret.
--