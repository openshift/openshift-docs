...
:navtitle: Configure Cinder volume security
...
`spec.template.metadata.labels`:: Specifies the labels on the pod. They must include labels from the label selector.
`spec.template.metadata.labels.name`:: Specifies the maximum name length after expanding any parameters is 63 characters.
