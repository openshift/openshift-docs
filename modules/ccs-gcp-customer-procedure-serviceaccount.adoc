...
[role="_abstract"]
In addition to the steps in _Prepare your {gcp-short} project for CCS_, you must complete additional prerequisites before creating an {product-title} cluster on {GCP} using a service account as the authentication type.

.Procedure

. To ensure that Red Hat can perform necessary actions, create an `osd-ccs-admin` IAM link:https://cloud.google.com/iam/docs/creating-managing-service-accounts#creating_a_service_account[service account] user within the {gcp-short} project.
