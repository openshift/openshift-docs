...
= Configuring storage or LokiStack forwarding for logging information
...
If you do not configure storage or forwarding for logging information, {tekton-results} does not store this information or provide it via the command-line interface or API.
...
<2> Provide the name of a bucket in Google Cloud Storage. {tekton-results} uses this bucket to store logging information for pipeline runs and task runs.
