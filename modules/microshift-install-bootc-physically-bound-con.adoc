...
Because embedded images might change with each system update, you cannot pull the images directory to the default container storage. Additional image stores do not work in this case because these image stores are read-only and intended for static image directories.

Therefore, the mechanism you must use for moving these container images between the different types of container storage is the `dir` transport type. The `dir` transport mechanism dumps the content of a container image into a local directory. Using this local directory as an alternative container storage is a workaround.
