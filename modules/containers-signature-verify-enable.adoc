...
.Procedure
. Create the files which link the registry URLs to the sigstore and specifies the key to verify the image.
...
. Create a machine config, which writes the exported files to disk on the worker nodes:
+
[source,terminal]
----
