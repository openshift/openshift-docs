...
= Migrating workloads using the Istio injection label with cert-manager
...
.. Removes the `istio-injection` label: This label prevents the 3.0 control plane from injecting the proxy. The `istio-injection` label takes precedence over `istio.io/rev` label. Because you cannot create the default `IstioRevisionTag` yet, you must temporarily remove the `istio-injection=enabled`. Leaving the `istio-injection=enabled` label applied would prevent the 3.0 control plane from performing proxy injection. 
