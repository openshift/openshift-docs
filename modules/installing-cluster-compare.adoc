...
[source,terminal]
----
$ podman cp cca:/usr/share/openshift/<arch>/kube-compare.<rhel_version> <directory/on/path>/kubectl-cluster_compare
----
+
...
* `<directory/on/path>` is the path to a directory included in your `PATH` environment variable.
