...
[source,terminal]
----
$ podman cp cca:/usr/share/openshift/<arch>/kube-compare.<rhel_version> <folder/on/path>/kubectl-cluster_compare
----
+
...
* `<folder/on/path>` is the path to a folder included in your `PATH` environment variable.
