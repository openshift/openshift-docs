...
Specify `internal` for a neighbor with the same ASN, or `external` for a neighbor with a different ASN. If you use this field, you cannot specify a value in the `spec.bgp.routers.neighbors.asn` field.
