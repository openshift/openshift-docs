...
|`string`
|Specifies the name of the authentication secret for the neighbor. The secret must be of type `kubernetes.io/basic-auth`, and must reside in the same namespace as the FRR-K8s daemon (`openshift-frr-k8s`). The password must be stored under the `password` key in the secret. `Password` and `PasswordSecret` are mutually exclusive.
+
When you enable the `frrk8sConfig.secretPassthrough` feature in the MetalLB CR, the MetalLB Operator automatically passes Secret references from BGPPeer CRs to this field instead of resolving passwords to cleartext.
