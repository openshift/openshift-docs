...
You can use the  `interface` field to dynamically discover IPv6 neighbors and create a direct connection without the need for IP addresses. 
For example:
...
spec:
  bgp:
    ...
    - asn: 64512
      ...
      - 5.5.5.5/32
----
<1> Activates a direct connection without the need for IP addresses.

[NOTE]
====
To use the `interface` field, there must be a direct, point-to-point connection between the two BGP peers, and you must enable IPv6 RAs (Router Advertisements).
...
Use this field to dynamically discover IPv6 neighbors and create a direct connection without the need for IP addresses. 
There must be a direct, point-to-point connection between the two BGP peers and you must enable IPv6 RAs (Router Advertisements).
If you use this field, you cannot specify a value in the `spec.bgp.routers.neighbors.address` field.
