...
= Configure an htpasswd identity provider with the CLI

[role="_abstract"]
You can create an htpasswd identity provider (IDP) with the {cluster-manager} CLI (`ocm`) tool.

.Prerequisites

* You have installed and configured the latest version of the {cluster-manager} CLI (`ocm`).
