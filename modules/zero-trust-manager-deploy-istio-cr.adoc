...
:navtitle: Deploy the Istio custom resource with the federation configuration
[id="zero-trust-manager-deploy-istio-cr_{context}"]
= Deploying the Istio custom resource with the federation configuration

[role="_abstract"]
Deploy the Istio custom resource on Cluster A and Cluster B with {spire-full} (SPIRE) federation and multi-cluster {SMProductName} settings. This configures Istiod to obtain workload certificates from SPIRE and to trust SPIFFE bundles from both clusters for cross-cluster mTLS.
