...
|Backup using File System Backup (FSB) of 4 TiBs logical unit number (LUN) of which, 3 TiBs is used as data on CephRBD storage class.footnote:[Application backed up with Kopia as File System Backup (FSB)]
...
|Backup using FSB of a single namespace with 1000 pods, including 499 MiB of data per pod.
...
|Backup using FSB of a single namespace with 2000 pods, including 499 MiB of data per pod.
...
|Backup using FSB using a single namespace with one file sized 1 TiB using duplicate data (dd).
