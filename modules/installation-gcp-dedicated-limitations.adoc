...
[role="_abstract"]
Before you install {product-title} on Google Cloud Dedicated (GCD), review the limitations around machine types, storage, networking, and identity that differ from standard {gcp-short} installations. The following constraints are based on the Berlin region and might differ for future GCD regions.
...
* You must change the default storage class to use `hyperdisk-balanced` after installation. The default storage class provisioned by the installation program uses `pd-ssd`, which is not available in GCD regions. For more information, see "Changing the default storage class" in the _Additional resources_ section.
...
Specify the operating system image by using the `platform.gcp.defaultMachinePlatform.osImage` field or the per-machine-pool fields for `osImage`. You must provide both the `name` and `project` fields. The installation fails if these fields are not set.
...
Additional GCD behavior affects cluster installation and operation.
