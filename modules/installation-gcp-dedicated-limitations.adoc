...
* You must change the default storage class to use `hyperdisk-balanced` after installation. The default storage class provisioned by the installation program uses `pd-ssd`, which is not available in GCD regions.
