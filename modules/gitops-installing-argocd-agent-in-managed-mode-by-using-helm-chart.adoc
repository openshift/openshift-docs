...
`destinationBasedMapping`:: Specifies the mapping mode used by the managed agent. By default, managed agents use namespace-based mapping. In this mode, the principal manages applications created in dedicated namespaces, and the managed agent deploys incoming applications to the `argocd` namespace. With destination-based mapping, applications can reside in any namespace on the control plane. The agent deploys incoming applications in the same namespace where they were created. Destination-based mapping only applies to managed agents.
