...
[role="_abstract"]
Use the Helm chart from the Argo CD Agent project to install the managed-mode Agent component on the workload cluster. In _managed mode_, the Agent must communicate with the spoke Redis server. However, the default network policy created by the {gitops-title} Operator does not allow this communication.
...
--set destinationBasedMapping="false" \
...
`destinationBasedMapping`:: Specifies the mapping mode used by the managed Agent. By default, managed Agents rely on namespace-based mapping where the Principal manages applications created in dedicated namespaces and the managed Agent deploys incoming applications to the `argocd` namespace. With destination-based mapping, the applications can reside in any namespace on the control plane, and the Agent will deploy incoming applications in the same namespace as created on the control plane. Destination-based mapping is not applicable for autonomous Agents.
`createNamespace`:: When enabled, the Agent automatically creates the application namespace on the workload cluster if it does not exist. Use this parameter with destination-based mapping.
