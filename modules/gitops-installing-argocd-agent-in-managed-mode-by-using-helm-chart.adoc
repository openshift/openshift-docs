...
[role="_abstract"]
Use the Helm chart from the Argo CD Agent project to install the managed-mode Agent component on the workload cluster. In _managed mode_, the Agent must communicate with the spoke Redis server.
