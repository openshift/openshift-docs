...
= Install the logging UI plugin

[role="_abstract"]
Create the logging UI plugin in the {ocp-product-title} web console to enable log querying and analysis within the {coo-full} dashboard.
...
`schema`:: One of `otel`, `viaq`, or `select`. The default is `viaq` if no value is specified. When you choose `select`, you can select the mode in the UI when you run a query.
+
[NOTE]
====
These are the known issues for the logging UI plugin:
...
* Non-administrator users cannot query logs using the `otel` attribute with {logging} {for} versions 5.8 to 6.2.

* In {logging} {for} version 5.9, the `severity_text` Otel attribute is not set.
====

// Removed below links from procedure
// for more information, see link:https://issues.redhat.com/browse/OU-587[OU-587]
// (link:https://issues.redhat.com/browse/LOG-6589[LOG-6589]) 