...
[role="_abstract"]
Choose a metrics collection profile for core monitoring components to balance monitoring coverage with resource consumption by editing the `cluster-monitoring-config` config map in the `openshift-monitoring` project.
