...
[role="_abstract"]
You must add several URLs to an allowlist to download required packages and tools for your cluster.
...
[role="_abstract"]
If you are using a firewall to control egress traffic from {product-title}, you must configure your firewall to grant access to certain domain and port combinations listed in the reference tables. {product-title} requires this access to provide a fully managed {OCP-short} service.
...
|443
|Hosts all the container images that are stored on the Red{nbsp}Hat Ecosystem Catalog. Additionally, the registry provides access to the `odo` CLI tool that helps developers build on OpenShift and Kubernetes.
