...
+
--
* `status: "True"` for the `ChangeDetected` condition indicates that a change in the host firmware settings has been detected.
* `status: "False"` for the `Valid` condition indicates that the host has an invalid firmware setting.
* `settings` specifies the complete list of configured host firmware settings returned under the `status.settings` field.
--
