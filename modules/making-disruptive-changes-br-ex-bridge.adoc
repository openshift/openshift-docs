...
[role="_abstract"]
Apply disruptive changes to a customized `br-ex` bridge using NMState configuration and `MachineConfig` resources during planned maintenance windows, minimizing impact to cluster workloads by coordinating node reboots.
...
+
--
`$ base64 -w0 <nmstate_configuration>.yml`:: Replace `<nmstate_configuration>` with the name of your NMState resource YAML file.
--
