...
[role="_abstract"]
To apply disruptive changes to a `br-ex` bridge with minimal workload impact, you must create a `MachineConfig` object to force a cluster reboot. Because the `br-ex` bridge manages external traffic, this process ensures that all nodes receive the updated configuration while maintaining environment stability.
