...
* You installed the `nmstatectl` package. For more information about this package, see link:https://nmstate.io/cli_guide.html#the-nmstatectl-user-guide[The nmstatectl User Guide] in the `nmstate` documentation.
...
. Use the `cat` command to base64 re-encode the contents of the NMState configuration by entering the following command:
+
[source,terminal]
----
...
[source,terminal]
----
$ oc apply -f <bare_machine_config.yaml>
----
