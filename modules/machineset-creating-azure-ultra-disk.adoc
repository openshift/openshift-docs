...
:navtitle: Create machines with ultra disks by using machine sets
...
ifdef::mapi,cpmso[]
. Create a custom secret in the `openshift-machine-api` namespace by using the `{machine-role}` data secret by running the following command:
+
[source,terminal]
----
