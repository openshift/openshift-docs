...
machineNetwork, clusterNetwork, serviceNetwork:: Specifies IP address ranges. You must specify an IP address range for both the IPv4 and IPv6 address families.
<ingress_vips>:: Specifies the virtual IP (VIP) address endpoints for the Ingress VIP services to provide an interface to the cluster. For example: `['192.168.25.79', 'fd2e:6f44:5dd8:c956:f816:3eff:fef1:1bad']`.
<api_vips>:: Specifies the virtual IP (VIP) address endpoints for the API VIP services to provide an interface to the cluster. For example: `['192.168.25.199', 'fd2e:6f44:5dd8:c956:f816:3eff:fe78:cf36']`.
controlPlanePort:: Specifies the dual-stack network details that are used by all of the nodes across the cluster.
fixedIPs:: Specifies fixed IP configurations. The CIDR of any subnet specified in this field must match the CIDRs listed on `networks.machineNetwork`.
<subnet_v4_name>, <subnet_v4_id>, <subnet_v6_name>, <subnet_v6_id>:: Specifies subnet names and IDs. You can specify a value for either `name` or `id`, or both.
<network_name>, <network_id>:: Specifies network name and ID. Specifying the `network` under the `ControlPlanePort` field is optional.
+
