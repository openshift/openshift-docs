...
* You used OperatorHub to install to the MetalLB Operator. A `Subscription` CR is created as part of the Operator installation and the CR has the `installPlanApproval` parameter set to `Automatic` by default.
...
. Edit the install plan that exists in the namespace by entering the following command. Set `spec.approval` to `Manual` and set `spec.approved` to `true`. Ensure that you replace `<name_of_installplan>` with the name of the install plan, such as `install-tsz2g`.
+
[source,terminal]
----
...
[NOTE]
====
After you edit the install plan, the upgrade operation starts. If you enter the `oc -n metallb-system get csv` command during the upgrade operation, the output might show `Replacing` or `Pending` status under the `PHASE` section heading. These statuses indicate the phases of the upgrade operation.
====

.Verification

. Verify the upgrade was succesful by entering the following command:
+
[source,terminal]
----
$ oc -n metallb-system get csv
