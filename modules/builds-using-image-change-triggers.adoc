...
You can use image change triggers to automatically invoke your build when a new version of an upstream image is available. For example, if a build is based on a {op-system-base} image, you can trigger that build to run any time the {op-system-base} image changes. As a result, the application image is always running on the latest {op-system-base} base image.
