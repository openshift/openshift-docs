...
apiVersion: "v1"
metadata:
  name: "ruby-20-centos9"
----
+
This defines the image stream that is tied to a container image repository located at `_<system-registry>_/_<namespace>_/ruby-20-centos9`. The `<system-registry>` is defined as a service with the name `docker-registry` running in {product-title}.
...
strategy:
  sourceStrategy:
    from:
      kind: "ImageStreamTag"
      name: "ruby-20-centos9:latest"
----
+
In this case, the `sourceStrategy` definition is consuming the `latest` tag of the image stream named `ruby-20-centos9` located within this namespace.
...
strategy:
  sourceStrategy:
    from:
      kind: "DockerImage"
      name: "172.30.17.3:5001/mynamespace/ruby-20-centos9:<immutableid>"
----
