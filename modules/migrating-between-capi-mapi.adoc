...
[source,terminal,subs="attributes+"]
----
$ oc edit <resource_name> -n openshift-machine-api
----
+
ifdef::machine-to-cluster[where `<resource_name>` is the fully qualified name of the Machine API resource that you want to migrate to a Cluster API resource.]
ifdef::cluster-to-machine[where `<resource_name>` is the fully qualified name of the Machine API resource that corresponds to the Cluster API resource that you want to migrate the Machine API.]
...
<3> The name of the resource that you want to migrate.
<4> Specify the authoritative API that you want this resource to use.
For example, to start migrating a {from-api-name} resource to the {to-api-name}, specify `{to-api-value}`.
<5> The value for the current authoritative API.
...
----
+
ifdef::machine-to-cluster[where `<resource_kind>/<resource_name>` is the fully qualified name of the Machine API resource that you want to migrate to a Cluster API resource.]
ifdef::cluster-to-machine[where `<resource_kind>/<resource_name>` is the fully qualified name of the Machine API resource that corresponds to the Cluster API resource that you want to migrate the Machine API.]
+
--
