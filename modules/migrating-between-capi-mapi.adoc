...
Migrating a control plane machine in not supported.
====
...
----
+
where `<resource_name>` is the fully qualified name of the {from-api-name} resource that you want to migrate to a {to-api-name} resource.
...
[source,terminal]
----
$ oc -n {origin-namespace} get <resource_kind>/<resource_name> -o json | jq .status.authoritativeAPI
---- 
+
where `<resource_kind>/<resource_name>` is the fully qualified name of the {from-api-name} resource that you want to migrate to a {to-api-name} resource.
+
--
