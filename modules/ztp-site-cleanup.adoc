...
. Optional: If you want to permanently remove a site, you should also remove the `ClusterInstance` and any group or site-specific `ConfigMap` resources from the `template-values/` directory in the Git repository.

. Optional: If you want to remove a site temporarily, for example when redeploying a site, you can leave the `ClusterInstance` and group or site-specific `ConfigMap` resources in the Git repository.