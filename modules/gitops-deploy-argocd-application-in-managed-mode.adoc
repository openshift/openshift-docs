...
metadata:
name: my-demo-app
namespace: agent-managed # Namespace created for the agent in the hub
spec:
project: default
source:
repoURL: https://github.com/sample-developer/gitops-example-getting-started

targetRevision: HEAD
path: app
destination:
name: agent-managed # Name assigned to the agent in the hub
namespace: my-app
syncPolicy:
automated:
prune: true
