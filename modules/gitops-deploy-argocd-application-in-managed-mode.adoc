...
[NOTE]
====
The application can be created in any namespace if the destination-based mapping is enabled for both the Principal and the Agent.
====
...
Because the hub is the source of truth, any direct changes made in the spoke cluster are automatically reverted by the Agent component.