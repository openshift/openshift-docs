...
. Select a supported disk type for the instance type you select from the "Supported disk types" section of the machine series documentation.
...
----
+
.. Replace the `disks.type` parameter with the disk type you selected.
.. Replace the `disks.image` parameter with the `image` parameter you formed previously.
.. Replace the `machineType` parameter with the instance type you selected.

. To migrate an infra machine set to another architecture, run the following command using the ID of an infra machine set:
+
[source,terminal]
----
$ oc edit machineset <infra-machine-set-id> -n openshift-machine-api
----
+
.. Replace the `disks.type` parameter with the disk type you selected.
.. Replace the `disks.image` parameter with the `image` parameter you formed previously.
