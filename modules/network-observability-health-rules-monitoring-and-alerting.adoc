...
*OVN*:: Read-only context for OpenShift OVN-Kubernetes platform alerts. These alerts provide visibility into control plane network issues but do not affect the NetObserv health score.
*Third-party contexts*:: Additional read-only contexts for third-party monitoring tools, such as Kiali for service mesh health. These contexts are displayed when third-party alerts are labeled for Network Health integration.
