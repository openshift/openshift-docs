...
|`claimMappings`
|Configures the rules to be used by the Kubernetes API server for translating claims in a JSON web token (JWT) token, issued by the identity provider, to a cluster identity.
...
|`ExtraMapping`
a| An optional field that allows specifying a key and CEL expression to evaluate the keys' value. It is used to create additional mappings and attributes added to a cluster identity from a provided authentication token. The following parameters exist for this field:
+
* Key is a required field that specifies the string to use as the extra attribute key.
* Key must be a domain-prefix path. For example, `example.org/foo`.
* Key must not exceed 510 characters in length.
* Key must contain the '/' character, separating the domain and path characters.
* Key must not be empty.
* The domain portion of the key, the string of characters prior to the '/', must be a valid RFC1123 subdomain.
* Key must not exceed 253 characters in length.
* Key must start and end with an alphanumeric character.
* Key must only contain lower case alphanumeric characters and '-' or '.'.
* Key must not use the reserved domains, or be subdomains of, "kubernetes.io", "k8s.io", and "openshift.io".
* The path portion of the key, the string of characters after the '/', must not be empty and must consist of at least one alphanumeric character, percent-encoded octets, '-', '.', '_', '~', '!', '$', '&', ''', '(', ')', '*', '+', ',', ';', '=', and ':'.
* Key must not exceed 256 characters in length.
...
|`ValueExpression`
a| A required field to specify the CEL expression to extract the extra attribute value from a JWT token's claims. CEL expressions have access to the token claims
through a CEL variable, 'claims', which is a map of claim names to claim values. For example, the 'sub' claim value can be accessed as 'claims.sub' and nested claims can be accessed using dot notation like 'claims.foo.bar'. The following limitations exist for this field:
