...
// Note: This table duplicates the generated API reference. Now that these fields are generally available, consider whether to continue maintaining this table or rely on the API docs.
...
a|Optional parameter. CEL expression that produces a string or string array from JWT token claims.
...
a|Optional parameter. CEL expression that produces a string value from JWT token claims. Must result in a single string.
...
a|Optional parameter. Required when `type` is `CEL`. Contains `expression` (CEL expression to evaluate) and `message` (error text).

|`claimValidationRules.cel.expression`
a|CEL expression that validates token claims. Must evaluate to `true` for authentication to succeed.
...
a|Error message displayed when validation fails. Constraints: 1-256 characters.
...
* `CEL` - Validates the JWT against a CEL expression
...
|`issuer.discoveryURL`
a|Optional parameter. Custom OIDC discovery endpoint URL. Must be a valid HTTPS URL and differ from `issuer.issuerURL`.
...
|`userValidationRules`
a|Optional parameter. Validation rules for user objects created from authenticated tokens. All rules must pass (AND operation).
