...
<6> Specifies the policy types that define the root of trust, either a public key, a BYOPKI, or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate]. Here, a public key with Rekor verification.
...
<4> Specifies the parameters that define how the images are verified.
<5> Defines a root of trust for the policy.
<6> Specifies the policy types that define the root of trust, either a public key, a BYOPKI, or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate]. Here, a Fulcio certificate with required Rekor verification.
...
+
--
.Example image policy object for BYOPKI signature verification
[source,yaml]
----
...
<4> Specifies the parameters that define how the images are verified.
<5> Defines a root of trust for the policy.
<6> Specifies the policy types that define the root of trust, either a public key, a BYOPKI, or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate]. Here, a BYOPKI.
<7> For a BYOPKI, the following parameters are required:
...
<9> For a BYOPKI, specify `MatchRepository` to verify the identity in the signature and the actual image identity. The default signed identity is `matchRepoDigestOrExact`, which requires digest specification. The signature in this case was not created for digested image.
--
...
----
+
.Example output for the image policy object for BYOPKI signature verification showing the new image policy
[source,json]
----
# ...
 "transports": {
