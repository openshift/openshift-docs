...
= Interactive OpenShift Cluster Manager and user role creation mode options
...
The following tables describe the interactive OpenShift Cluster Manager role creation mode options:

.`--interactive` OpenShift Cluster Manager role creation mode options
...
|`Role prefix`
|Specify the prefix to include in the OpenShift Cluster Manager role name. The default is `ManagedOpenShift`. You can create only one OpenShift Cluster Manager role per AWS account for your Red Hat organization.

|`Enable admin capabilities for the OCM role (optional)`
|Enable the admin OpenShift Cluster Manager role, which is equivalent to specifying the `--admin` argument. The admin role is required if you want to use `auto` mode to automatically provision the cluster-specific Operator roles and the OIDC provider by using {cluster-manager}.

|`Permissions boundary ARN (optional)`
|Specify a permissions boundary Amazon Resource Name (ARN) for the OpenShift Cluster Manager role. For more information, see link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_boundaries.html[Permissions boundaries for IAM entities] in the AWS documentation.

|`Role Path (optional)`
|Specify a custom ARN path for your OpenShift Cluster Manager role. The path must contain alphanumeric characters only and start and end with `/`, for example `/test/path/dev/`. For more information, see _ARN path customization for IAM roles and policies_.

|`Role creation mode`
|Select the role creation mode. You can use `auto` mode to automatically create the OpenShift Cluster Manager role and link it to your Red Hat organization account. In `manual` mode, the `rosa` CLI generates the `aws` commands needed to create and link the role. In `manual` mode, the corresponding policy JSON files are also saved to the current directory. `manual` mode enables you to review the details before running the `aws` commands manually.

|`Create the '<ocm_role_name>' role?`
|Confirm if you want to create the OpenShift Cluster Manager role.

|`Link the '<ocm_role_arn>' role with organization '<red_hat_organization_id>'?`
|Confirm if you want to link the OpenShift Cluster Manager role with your Red Hat organization.
