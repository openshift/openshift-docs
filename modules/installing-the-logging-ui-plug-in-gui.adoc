...
= Install the logging UI plugin by using the web console
...
.Procedure
. Install the {coo-full}. For more information, see link:https://docs.redhat.com/en/documentation/red_hat_openshift_cluster_observability_operator/{coo-version}/html/installing_red_hat_openshift_cluster_observability_operator/installing-the-cluster-observability-operator[Install the Cluster Observability Operator].
...
* In {logging} {for} version 5.9, the `severity_text` OpenTelemetry attribute is not set.
====
