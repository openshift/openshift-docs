...
oc exec -it <etcd_pod_name> -n <hosted_cluster_namespace> -- curl -X PUT -T "/var/lib/data/snapshot.db" \
  -H "Host: <bucket_name>.s3.amazonaws.com" \
  -H "Date: <date>" \
  -H "Content-Type: <content_type>" \
  -H "Authorization: AWS <access_key>:<signature_hash>" \
  https://<bucket_name>.s3.amazonaws.com/<hosted_cluster_name>-snapshot.db
----
