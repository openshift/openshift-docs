...
:navtitle: Tak a snapshot of etcd for a hosted cluster
...
. To take an etcd snapshot, use the `exec` command in the etcd container by entering the following command:
+
[source,terminal]
----
...
. To check the snapshot status, use the `exec` command in the etcd container by running the following command:
+
[source,terminal]
----
$ oc exec -it <etcd_pod_name> -n <hosted_cluster_namespace> -- \
