...
| `SetOwnerReferenceFailed` | Setting owner references between a `Build` and a `BuildRun` resources failed. This status is triggered when you use the `build.shipwright.io/build-run-deletion` annotation in a build.
...
| `RemoteRepositoryUnreachable` | The defined `spec.source.url` specification was not found. This validation only takes place for HTTP and HTTPS protocols.
