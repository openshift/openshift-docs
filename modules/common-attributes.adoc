...
// Common prereqs only.
:prerequisite-cluster-admin: pass:q[You have access to the cluster as a user with the `cluster-admin` role.]
:prerequisite-install-oc: pass:q[You have installed the OpenShift CLI (`oc`).]
