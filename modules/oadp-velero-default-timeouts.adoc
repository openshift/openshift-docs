...
[id="oadp-velero-default-item-operation-timeout_{context}"]
= Velero default item operation timeout

The `defaultItemOperationTimeout` parameter defines the duration to wait on asynchronous `BackupItemActions` and `RestoreItemActions` to complete before timing out. The default value of this parameter is `1h`.
