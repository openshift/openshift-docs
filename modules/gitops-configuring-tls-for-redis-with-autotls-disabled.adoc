...
where:
+
`name`:: Specifies the name of the Argo CD instance.
`namespace`:: Specifies the namespace where you want to run the Argo CD instance.
`enabled`:: Specifies the flag value that enables the HA feature. If you do not want to enable HA, do not include this line or set the flag value as `false`.
