...
----
+
. Modify is example YAML to add your projects as members.  You can add any number of projects, but a project can only belong to *one* `ServiceMeshMemberRoll` resource.
+
...
----
+
. Run the following command to verify the `ServiceMeshMemberRoll` was created successfully. The installation has finished successfully when the `STATUS` column is `Configured`.
