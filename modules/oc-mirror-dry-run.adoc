...
$ oc mirror --config=./imageset-config.yaml \
  ...
  --dry-run                                  
----
`imageset-config.yaml:: Specifies the image set configuration file that was created. This procedure assumes that it is named `imageset-config.yaml`.
`docker://registry.example:5000`:: Specifies the mirror registry. Nothing is mirrored to this registry as long as you use the `--dry-run` flag.
