...
= Collect diagnostic data from the command line

[role="_abstract"]
When submitting a support case, it is helpful to include diagnostic information about your cluster to Red Hat Support.
You can use the `oc adm must-gather` tool to gather diagnostic data for resources of various types, such as `TempoStack` or `TempoMonolithic`, and the created resources such as `Deployment`, `Pod`, or `ConfigMap`.
