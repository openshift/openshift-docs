...
[role="_abstract"]
You must deploy a `Gateway` object in your {ocp} cluster to begin setting up the infrastructure used by application developers. The `Gateway` is the instantiation of your entry point. It tells the controller to provision load balancer with specific ports and security credentials.

[IMPORTANT]
====
In a multicluster environment, for {prodname} to balance traffic by using DNS across clusters, you must define your `Gateway` object with a shared hostname. You can define this by using an HTTPS listener with a wildcard hostname based on the root domain. You must apply these resources to each cluster that you want to use them.
====
