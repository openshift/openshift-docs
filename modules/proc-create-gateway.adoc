...
[role="_abstract"]
You must deploy a `Gateway` object in your {ocp} cluster to begin setting up the infrastructure used by application developers. The `Gateway` object is the instantiation of your entry point. 

The `Gateway` object causes the controller to provision a load balancer with specific ports and security credentials.
