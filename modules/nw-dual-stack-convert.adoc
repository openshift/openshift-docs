...
:navtitle: Convert to a dual-stack cluster network
[id="nw-dual-stack-convert_{context}"]
= Converting to a dual-stack cluster network

[role="_abstract"]
To convert your cluster network from IPv4 single-stack to dual-stack in {product-title}, you can patch the cluster network and, on installer-provisioned infrastructure, the infrastructure custom resources. You must re-create existing pods after conversion to receive IPv6 addresses.
