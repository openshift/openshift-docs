...
You can configure {rhelai} or {rhoai} as large language model (LLM) provider for the {ols-long} Service. {rhelai} or {rhoai} can use a server or inference service that processes inference queries. Configure the LLM provider prior to installing the {ols-long} Operator. 
