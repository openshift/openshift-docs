...
= Install a primary-remote multi-network mesh
...
.. Annotate the `istio-system` namespace in the West cluster so that the East cluster's control plane manages it by running the following command:
+
[source,terminal]
----
