...
[role="_abstract"]
The following table describes the {rh-rhacm} custom resources (CRs) for the telco hub reference configuration.
...
{rh-rhacm},`acmMCH.yaml`,"Configures a `MultiClusterHub` CR with high availability, enabling various components and specifying installation settings for Open Cluster Management.",No
{rh-rhacm},`acmMirrorRegistryCM.yaml`,Defines the SSL certificates and mirror registry configuration for various Red Hat and {product-title} registries used by the multicluster-engine Operator in the `multicluster-engine` namespace.,No
{rh-rhacm},`acmNS.yaml`,Defines the `open-cluster-management` namespace with a label to enable cluster monitoring.,No
{rh-rhacm},`acmOperGroup.yaml`,"Defines an `OperatorGroup` CR for the `open-cluster-management` namespace, targeting the same namespace.",No
...
{rh-rhacm},`acmSubscription.yaml`,Subscribes to the Red Hat Advanced Cluster Management for Kubernetes Operator using automatic install plan approval.,No
...
{rh-rhacm},`observabilityRoutePolicy.yaml`,Policy to propagate Red Hat Advanced Cluster Management for Kubernetes observability route to the managed cluster.,No
