...
{attribute-based-full} enables pods to request graphics processing units (GPU) based on specific device attributes. This ensures that each pod is scheduled on a node with the exact GPU specifications it requires.
