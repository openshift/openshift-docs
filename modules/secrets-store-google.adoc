...
:navtitle: Mount secrets from Google Secret Manager
[id="secrets-store-google_{context}"]
= Mounting secrets from Google Secret Manager

[role="_abstract"]
You can use the {secrets-store-operator} to mount secrets from Google Secret Manager to a Container Storage Interface (CSI) volume in {product-title}. Using an external secret store protects information that you do not want developers to have and can be more secure than `secret` objects.
