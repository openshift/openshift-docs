...
* You have deployed a cluster on {ocp-product-title} 4.15 or, which includes the supported {k8s} Gateway API custom resource definitions (CRDs) required for {istio} ambient mode.
...
. Label the `bookinfo` namespace with `istio.io/dataplane-mode=ambient` to add the application to the mesh in ambient mode by running the following command:
+
[source,terminal]
----
...
----
+
Alternatively, rather than including all workloads in a namespace, you can add the `istio.io/dataplane-mode=ambient` label to individual pods to include specific workloads. See the "Additional resources" section for more details on the labels used to add or exclude workloads in a mesh.
+
[NOTE]
====
