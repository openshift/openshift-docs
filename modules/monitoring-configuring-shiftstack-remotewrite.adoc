...
* You have installed the Cluster Observability Operator on your {rhoso} cluster.
- You have configured the monitoring stack for your {rhoso} cluster to collect the metrics that you are interested in.
- You have enabled Telemetry in the {rhoso} environment.
+
[NOTE]
====
...
. On your {rhoso} management cluster, create a secret named `mtls-bundle` in the `openstack` namespace that contains HTTPS client certificates for authentication to Prometheus by entering the following command:
+
[source,shell]
----
...
. Replace the `.spec.telemetry.template.metricStorage` section so that {rhoso} sends metrics to Prometheus. As an example:
+
[source,yaml]
----
      metricStorage:
...
. Create a cluster monitoring config map as a YAML file on the tenant cluster where your workloads run. The map must include a remote write configuration and cluster identifiers. As an example:
+
[source,yaml]
----
apiVersion: v1
...
. Create a secret named `mtls-bundle` in the `openshift-monitoring` namespace that has HTTPS client certificates for authentication to Prometheus by entering the following command:
+
[source,terminal]
----
