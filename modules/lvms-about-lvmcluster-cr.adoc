...
This field can be modified after the LVM Cluster is created. To update the parameter, use either of the following methods:

Edit the LVM Cluster using:

`oc edit lvmcluster <lvmcluster-name>`

Apply a patch using:

`oc patch lvmcluster <lvmcluster-name>`
