...
The value for this field must be configured in the range of 2 MiB to 16 GiB due to the underlying limitations of `lvm2`.
...
If this field is set to `Host`, the metadata size is calculated based on the `lvm2` default settings. 
|====
