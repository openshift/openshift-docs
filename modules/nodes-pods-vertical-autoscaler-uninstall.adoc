...
You can remove the Vertical Pod Autoscaler Operator (VPA) from your {product-title} cluster. After uninstalling, the resource requests for the pods already modified by an existing VPA CR do not change. Any new pods are allocated the resources that are defined in the workload object, not the previous recommendations made by the VPA.
