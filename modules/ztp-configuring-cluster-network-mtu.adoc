...
Setting the cluster network MTU with an extra buffer during deployment prevents the need for a Day 2 MTU update that requires at least two rolling reboots of all cluster nodes.
...
If you plan to enable IPsec as a Day 2 operation, allow an additional 46 bytes for IPsec overhead.
For example, with a machine network MTU of `9100` bytes, set the cluster network MTU to `8900` bytes, which accounts for the following overhead:

* OVN-Kubernetes overlay: 100 bytes
* IPsec buffer: 46 bytes
...
* You have reviewed the procedure for customizing extra installation manifests as described in "Customizing extra installation manifests in the {ztp} pipeline".
