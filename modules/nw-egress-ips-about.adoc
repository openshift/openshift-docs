...
ifndef::openshift-rosa[]
not supported on a cluster provisioned on {aws-first}. For more information, see "BZ#2039656".
endif::openshift-rosa[]
====
...
On {aws-short}, constraints on IP address assignments depend on the instance type configured. For more information, see "IP addresses per network interface per instance type".
