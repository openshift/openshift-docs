...
== Considerations for assigning non-OVN managed networks to egress IP addresses

Egress IP addresses can be set within _non-OVN managed networks_, or networks that are not managed by Open Virtual Network (OVN). These networks provide users with the option to route workload traffic over specific network interface cards (NICs) for purposes such as traffic segmentation or meeting specialized requirements.

For users who want to assign non-OVN managed networks to their egress IP addresses, the following conditions must be met:

* The network link should be operational. 
* Ip addresses must have a universal, or global, scope. 
* Links and their addresses must not be removed during runtime after an egress IP address is assigned to it. If you want to remove the link, you must first remove the egress IP address and then remove the address or network link. 
* IP forwarding must be enabled for the network link.  

[NOTE]
====
Assigning non-OVN managed networks to egress IP addresses relies on the absence of subnet overlap. This allows the system to pinpoint a suitable network interface for accommodating an egress IP assignment. The process includes verifying whether the egress IP can be allocated within the primary OVN managed network. If it cannot, then the system uses the longest prefix match mechanism to identify the appropriate interface for hosting the egress IP.
====
