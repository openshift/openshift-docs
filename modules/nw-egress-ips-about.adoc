...
While the Egress IPs capability manages the IP address capacity per node, it is important to plan for this constraint in your deployments. For example, if a public cloud provider limits IP address capacity to 10 IP addresses per node and there are 8 nodes in the cluster, the total number of assignable IP addresses is only 80. To increase this to 150 IP addresses per node, you would need to allocate 7 additional nodes.
