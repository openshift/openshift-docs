...
In {product-title}, egress IPs provide administrators a way to control network traffic. Egress IPs can be used with a `br-ex` Open vSwitch (OVS) bridge, a primary network interface, or additional network interfaces.
...
* If you have configured a secondary network interface card (NIC) that acts as the Egress IP interface for nodes in your cluster, you used the Node Tuning Operator to enable IP forwarding for the secondary NIC.
endif::ovn[]
endif::openshift-rosa[]
