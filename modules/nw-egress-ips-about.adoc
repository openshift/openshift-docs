...
When a namespace has multiple egress IP addresses and those addresses are hosted on multiple nodes, pods use all egress addresses roughly equally. If the node hosting one of the egress IP addresses fails then pods continue using the remaining functioning node and egress IP address after a short delay.
