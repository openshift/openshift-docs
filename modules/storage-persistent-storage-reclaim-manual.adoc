...
[role="_abstract"]
To reuse storage resources released by a deleted claim, manually reclaim the `PersistentVolume` (PV).
...
. As a cluster administrator, delete the PV by running the following command:
+
[source,terminal]
----
