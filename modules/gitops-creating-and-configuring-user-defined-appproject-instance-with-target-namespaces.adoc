...
where:
+
`AppProject`:: Specifies the name of the user-defined `AppProject` instance.
`AppProject`:: Specifies the control plane namespace where you want to run the user-defined `AppProject` instance.
`Application`:: Specifies the list of non-control plane namespaces for creating and managing `Application` resources.
`Application`:: Specifies the name of the target namespace for the Argo CD server to create and manage `Application` resources.
`pass:[*]`:: Specifies the with wildcards (`pass:[*]`), specifies the name of the target namespaces matching the pattern `app-team-pass:[*]`, such as `app-team-1` and `app-team-2`, for the Argo CD server to create and manage `Application` resources.
`AppProject`:: Specifies the references to the clusters and namespaces into which applications within the user-defined `AppProject` instance can deploy.
`AppProject`:: Specifies the references to the repositories from which applications within the user-defined `AppProject` instance can pull manifests.
.. Click *Create*.
+
