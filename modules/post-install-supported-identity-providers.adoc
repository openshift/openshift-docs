...
//
// Duplicate of modules/supported-identity-providers.adoc for this assembly.
// That module retains CQA-inherited xrefs and must only be included from one
// location; this copy keeps the same xrefs per doc_guidelines.adoc's exception
// for modules split from an assembly.

:_mod-docs-content-type: REFERENCE
[id="post-install-supported-identity-providers_{context}"]
