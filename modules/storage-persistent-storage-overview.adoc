...
[role="_abstract"]
To maintain data integrity for stateful applications, {microshift-short} uses a pre-provisioned framework called persistent volumes (PVs). By using this architecture, node administrators provision persistent storage that survives container restarts, ensuring data availability for your workloads.
...
ifdef::microshift[]
PVCs are specific to a namespace, and are created and used by developers as a means to use a PV. PV resources on their own are not scoped to any single namespace; they can be shared across the entire {product-title} node and claimed from any namespace. After a PV is bound to a PVC, that PV cannot then be bound to additional PVCs. This has the effect of scoping a bound PV to a single namespace.
endif::microshift[]
