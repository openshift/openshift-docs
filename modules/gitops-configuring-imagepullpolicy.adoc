...
You can configure how container images are pulled for Argo CD and it's related components that are managed by the {gitops-title} Operator. The {gitops-title} Operator provides multiple configuration levels for setting the `imagePullPolicy` value, enabling administrators to define policies globally or at an individual instance level.

The {gitops-shortname} Operator supports configurable `imagePullPolicy` settings at multiple levels, providing administrators with flexible control over how container images are pulled for Argo CD components. Similar configuration is available for the `GitOpsService` and `RolloutsManager` custom resources (CR).
...
* `Global-level policy` — Defined through the `IMAGE_PULL_POLICY` environment variable in the {gitops-shortname} Operators `Subscription` custom resource.
  ...
  config:
    env:
      - name: IMAGE_PULL_POLICY
        value: "Always"
----
An instance-level configuration overrides the Operator-level policy and applies the configuration only to the specific custom resource.
