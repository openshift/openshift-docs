...
[role="_abstract"]
To return connection details when users bind to your template service, add `template.openshift.io/expose-` or `template.openshift.io/base64-expose-` annotations to `ConfigMap`, `Secret`, `Service`, or `Route` objects. Binding clients receive the credentials and endpoints they need to connect to your application without reading cluster objects directly.
...
[NOTE]
====
`Bind` response key-value pairs can be used in other parts of the system as environment variables. Therefore, it is recommended that every annotation key with the prefix removed should be a valid environment variable name -- beginning with a character `A-Z`, `a-z`, or `pass:[_]`, and being followed by zero or more characters `A-Z`, `a-z`, `0-9`, or `pass:[_]`.
====
