...
[role="_abstract"]
To return connection details when users bind to your template service, add `template.openshift.io/expose-` or `template.openshift.io/base64-expose-` annotations to `ConfigMap`, `Secret`, `Service`, or `Route` objects. The {tsb-name} resolves JSONPath expressions in these annotations at bind time.
