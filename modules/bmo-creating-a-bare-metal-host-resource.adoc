...
[role="_abstract"]
To integrate and manage a physical server within your cluster, you can create a `BareMetalHost` custom resource (CR). This enables the Bare Metal Operator to register, inspect, and prepare the host for deployment by defining its network and management details.

.Procedure

. Create a `BareMetalHost` custom resource file by running the following command:
+
[source,terminal]
----
