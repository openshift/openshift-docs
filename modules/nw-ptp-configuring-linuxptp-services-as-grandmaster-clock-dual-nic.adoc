...
You can configure the `linuxptp` services (`ptp4l`, `phc2sys`, `ts2phc`) as a grandmaster clock (T-GM) for 2 card E810 NICs by creating a `PtpConfig` custom resource (CR) that configures the NICs.
...
For distributed RAN (D-RAN) use cases, you can configure PTP for 2 card NICs as follows:
