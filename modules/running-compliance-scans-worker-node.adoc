...
:navtitle: Schedule the result server pod on a worker node
...
`rawResultStorage.nodeSelector.node-role.kubernetes.io/worker`:: Specifies the Compliance Operator uses this node to store scan results in ARF format.
`rawResultStorage.tolerations.operator`:: Specifies the result server pod tolerates all taints.
