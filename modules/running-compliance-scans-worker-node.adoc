...
`node-role.kubernetes.io/worker`:: Specifies the Compliance Operator uses this node to store scan results in ARF format.
`operator`:: Specifies the result server pod tolerates all taints.
