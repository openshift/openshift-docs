...
rawResultStorage:
  nodeSelector:
  node-role.kubernetes.io/worker: "" # The Compliance Operator uses this node to store scan results in ARF format
  pvAccessModes:
  - ReadWriteOnce
  rotation: 3
  size: 1Gi
  tolerations:
  - operator: Exists # The result server pod tolerates all taints
roles:
- worker
- master
scanTolerations:
