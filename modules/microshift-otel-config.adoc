...
You can configure {microshift-short} Observability after it is installed to identify a valid endpoint.

[IMPORTANT]
====
You must specify an OpenTelemetry Protocol (OTLP)-compatible endpoint for each configuration, such as Prometheus, before starting {microshift-short}. If an endpoint is not specified, {microshift-short} Observability will not start.
====
