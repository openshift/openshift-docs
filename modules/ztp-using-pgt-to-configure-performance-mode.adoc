...
[role="_abstract"]
Follow this example to set performance mode by updating the `workloadHints` fields in the generated `PerformanceProfile` CR for the reference configuration, based on the `{policy-gen-cr}` CR in the `{group-sno-ranGen}`.
...
. Update the `{policy-gen-cr}` entry for `PerformanceProfile` in the `{group-sno-ranGen}` reference file in `{argocd-folder}/` as follows to set performance mode.
+
[source,yaml]
----
