...
[NOTE]
====
Although the HyperShift Operator supports the versions in the following table, {mce-short} supports only as far back as 2 versions earlier than the current version. For example, if the current version is 4.18, {mce-short} supports as far back as version 4.16. If you want to use a version that is earlier than one of the versions that {mce-short} supports, you can detach your hosted clusters from {mce-short} to be unmanaged.
====
