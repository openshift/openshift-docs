...
[role="_abstract"]
Define non-control plane namespaces in which users can create, update, and reconcile `Application` resources. Configure the target namespaces in the `ArgoCD` custom resource (CR) of your user-defined cluster-scoped Argo CD instance.
