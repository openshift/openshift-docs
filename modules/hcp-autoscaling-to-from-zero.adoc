...
[NOTE]
====
Scale-from-zero autoscaling is available on {product-title} 4.18 and later for eligible {hcp} deployments on {aws-first}.
...
+
Configure scale-to-zero only on {aws-short} hosted clusters running {product-title} 4.18 or later with a HyperShift Operator release that includes the backport.
