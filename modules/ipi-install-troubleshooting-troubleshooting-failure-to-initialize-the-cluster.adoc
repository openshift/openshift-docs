...
= Troubleshooting failure to initialize the cluster
...
----
+
The command returns the status of the cluster's operators.
+
.Example output
[source,terminal]
----
...
. Inspect individual operators by running the following command:
+
[source,terminal]
----
...
----
+
Replace `<operator>` with the name of one of the operators above. This command is useful for identifying why an operator hasn't acheieved the `Available` state or is in the `Failed` state.
+
.Example output
[source,yaml]
----
...
. To get the operator's status condition, execute the following command:
+
[source,terminal]
----
