...
* You must add the `topology.kubernetes.io/zone` label to your bare-metal hosts on your management cluster ensuring that you use a unique value for `topology.kubernetes.io/zone` on each host. Otherwise, all of the hosted control plane pods are scheduled on a single node, causing single point of failure.
