...
If you want to manually configure the CA content, you must create a 'cert-manager-operator-trusted-ca-bundle` config map in the 'cert-manager-operator` namespace.
...
* You have installed the `trust-manager` operand.
...
+
.Example YAML file that defines the Bundle object
...
----
You must manually create a `cert-manager-operator-trusted-ca-bundle` config map to use the `sources.userDefaultCAs` parameter.
+
[NOTE]
====
