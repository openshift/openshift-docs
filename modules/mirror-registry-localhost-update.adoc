...
:navtitle: Update mirror registry for Red Hat OpenShift from a local host
[id="mirror-registry-localhost-update_{context}"]
= Updating mirror registry for Red Hat OpenShift from a local host

[role="_abstract"]
You can update the _mirror registry for Red{nbsp}Hat OpenShift_ from a local host by using the `upgrade` command. Updating to the latest version ensures new features, bug fixes, and security vulnerability fixes.
