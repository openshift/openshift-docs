...
[role="_abstract"]
Mirroring the {product-title} content using the oc-mirror OpenShift CLI (`oc`) plugin creates resources, such as `catalogSource-certified-operator-index.yaml` and `imageContentSourcePolicy.yaml`, that you must install into the cluster after installation.
