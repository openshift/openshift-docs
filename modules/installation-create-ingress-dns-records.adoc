...
. Retrieve the Ingress Operator load balancer status:
+
----
$ oc -n openshift-ingress get service router-default
----
+
Note the value of the external IP address that the load balancer uses, which is shown in the `EXTERNAL-IP` column:
+
----
...
. Locate the hosted zone ID for the load balancer with the following command:
+
----
...
+
The output of this command is the load balancer hosted zone ID, for example:
+
----
Z3AADJGX6KTTL2
----
