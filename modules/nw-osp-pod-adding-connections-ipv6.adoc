...
:navtitle: Add IPv6 connectivity to pods on {rh-openstack}
...
where
+
`spec.additionalNetworks.namespace`:: Be sure to create pods in the same namespace.
`spec.additionalNetworks.rawCNIConfig`:: The interface in the network attachment `"master"` field can differ from `"ens4"` when more networks are configured or when a different kernel driver is used.
+
[NOTE]
====
