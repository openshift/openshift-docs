...
[NOTE]
====
Only images that you can successfully inspect with the `podman manifest inspect <IMAGE_URL>` can be used with a pinned image set. Image inspections could fail due to unsupported manifest formats, registry authorization issues, invalid schemas, network connectivity issues, or other issues.
====
