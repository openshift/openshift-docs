...
[role="_abstract"]
To expand your cluster using Virtual Media when the provisioning network is enabled, configure the provisioning custom resource to use the `baremetal` network. This configuration ensures that the cluster can deploy and manage new nodes through an external network interface.
...
+
--
`spec.virtualMediaViaExternalNetwork`:: Add `virtualMediaViaExternalNetwork: true` to the `provisioning` CR.
--
...
----
+
`metadata.spec.template.spec.providerSpec.value.image.checksum`:: Edit the `checksum` URL to use the API VIP address.
`metadata.spec.template.spec.providerSpec.value.image.url`:: Edit the `url` URL to use the API VIP address.
