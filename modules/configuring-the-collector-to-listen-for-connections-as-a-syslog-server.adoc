...
= Configure the collector to listen for connections as a syslog server
...
spec.inputs.receiver.type::
  Specify the input receiver type as `syslog`.

spec.inputs.receiver.port::
...
. As an example, send a test syslog message by running the following command:
+
[source,terminal]
----
$ logger --tcp --server collector-syslog-receiver.<namespace>.svc:10514 “test message”
----
+
Replace `<namespace>` with the namespace where the collector is deployed.
