...
[role="_abstract"]
You can custom labels and annotations from Knative services to the underlying {ocp-product-title} routes. You can apply metadata for routing policies, monitoring, or integration with other cluster services.
...
. Create a Knative service that has the label or annotation that you want to propagate to the {ocp-product-title} route:
...
. Verify that the {ocp-product-title} route includes the annotation or label that you added by inspecting the output of the following command:
+
...
<2> Specify the namespace where you created the service.
