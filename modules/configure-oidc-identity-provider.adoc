...
** *Name*: A name to identify your authentication provider; for example, *Google Workspace*. The integration name is displayed on the login page to help users select the correct sign-in option.
...
[NOTE]
====
`Fragment` mode works around the limitations of Single Page Applications (SPAs). Red{nbsp}Hat only supports the `Fragment` mode for early integrations and does not recommended using it for later integrations.
====
...
For example, you can optimize the Google Workspace login screen to a specific hosted domain by using the `hd` parameter, or preselect an authentication method in `PingFederate` by using the `pfidpadapterid` parameter.
====
...
** If your login failed, {product-title-short} shows a message describing why it could not process the identity provider's response.
