...
By default, {VirtProductName} automatically manages CPU and memory limits for virtual machines (VMs) to help meet namespace quota requirements. The memory limit is automatically set to twice the requested memory, and the CPU limit is set to one per vCPU.
...
[WARNING]
====
Manual resource quota management is not recommended. To avoid misconfigurations or scheduling issues, rely on the automatic managing of {VirtProductName} unless you have a specific requirement to set limits manually.
====
