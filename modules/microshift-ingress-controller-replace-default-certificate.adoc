...
[id="microshift-ingress-controller-replace-default-certificate_{context}"]
= Replacing the default ingress certificate

You can replace the default ingress certificate for all applications under the `.apps` subdomain. After you replace the certificate, all applications have encryption provided by the specified certificate.
...
. In the certificate file, list the wildcard certificate as the first certificate, followed by other intermediate certificates, and then end the list with the root CA certificate. The certificate file can contain one or more certificates in a chain.
...
:_mod-docs-content-type: PROCEDURE
[id="microshift-ingress-control-create-secret_{context}"]
= Replacing the default ingress certificate
...
$ oc patch ingresscontroller.operator default \
     --type=merge -p \
     '{"spec":{"defaultCertificate": {"name": "<secret>"}}}' \//<4>
     -n openshift-ingress-operator
----
<4> Replace `<secret>` with the name used for the secret in the previous step.
+