...
The `spec.exporterImage` field in the `NUMAResourcesOperator` custom resource controls which container image runs with these RTE privileges on worker nodes. Granting non-admin users write access to the `NUMAResourcesOperator` resource creates a security risk. An attacker with write access could substitute a malicious container image and potentially access sensitive node information or cluster topology data.
...
[NOTE]
====
Creating the `NUMAResourcesOperator` custom resource automatically deploys the Resource Topology Exporter (RTE) as a daemon set on the configured worker nodes. The RTE collects NUMA topology information and makes it available to the NUMA-aware scheduler. The `spec.exporterImage` field is optional; if not specified, the operator uses the default RTE image for your {product-title} release.
====
