...
[NOTE]
====
Creating the `NUMAResourcesOperator` custom resource automatically deploys the Resource Topology Exporter (RTE) as a daemon set on the configured worker nodes. The RTE collects NUMA topology information and makes it available to the NUMA-aware scheduler. The `spec.imageSpec` field is optional; if not specified, the operator uses the default RTE image for your {product-title} release.
====
