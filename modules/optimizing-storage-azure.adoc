...
:navtitle: storage performance for Microsoft Azure configuration
[id="optimizing-storage-azure_{context}"]
= Optimizing storage performance for Microsoft Azure

[role="_abstract"]
To ensure optimal cluster performance on {azure-full}, configure faster storage for {product-title} and Kubernetes. Red{nbsp}Hat recommends faster storage, particularly for etcd on the control plane nodes.
