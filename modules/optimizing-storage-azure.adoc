...
[role="_abstract"]
To ensure optimal cluster performance on {azure-full}, configure faster storage for {product-title} and Kubernetes. Prioritize high-performance disks for etcd on the control plane nodes, as these components are sensitive to disk latency.
