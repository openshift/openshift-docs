...
. Edit the `IngressController` CR in the `openshift-ingress-config` project to configure the TLS security profile:
+
[source,terminal]
----
$ oc edit IngressController default -n openshift-ingress-config
----
...
[source,terminal]
----
$ oc describe IngressController default -n openshift-ingress-config
----
+
.Example output
[source,terminal]
----
Name:         default
Namespace:    openshift-ingress-config
Labels:       <none>
Annotations:  <none>
