...
[NOTE]
====
The `tlsSecurityProfile` parameter configures the minimum TLS version and the cipher suites. {microshift-short} selects the TLS curves for the ingress router based on whether FIPS mode is enabled on the node. You cannot configure TLS curves in the `config.yaml` file.
====
