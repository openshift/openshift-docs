...
* The Principal DNS value in the Agent Helm chart (`server` parameter in the `values.yaml` file or the `--set server` flag) is incorrect.
...
* Confirm that the `server` parameter in the Agent Helm chart matches the Principal's DNS name.
