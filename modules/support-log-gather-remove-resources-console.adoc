...
. Remove the deployments of the support-log-gather components, such as `support-log-gather`, `cainjector`, and `webhook`, present in the `support-log-gather` namespace.
...
.. Click the Options menu {kebab} next to the *support-log-gather* and select *Delete Namespace*.
