...
This procedure performs a canary upgrade with the gradual migration of data plane namespaces for a cluster-wide deployment using the `istio-injection=enabled` label and `default` revision tag. 


In contrast to the simple migration method for a 2.6 installation using the `istio-injection=enabled` label, during this procedure, it is safe to restart any workloads at any time during the {SMProduct} {SMv2Version} to {SMProduct} 3 migration process. . To achieve this, this procedure contains more steps and you must re-label the data plane namespaces.
...
[NOTE]
====
Namespaces with `istio-injection=enabled` label applied and are still managed by {SMProduct} 2.6 control plane. To prevent the {SMProduct} 3.0 control pane from injecting proxies to workloads, do not use use the `default` name for the {istio} resource, and do not create the `default` revision tag at this time. You create the `default` revision tag later in this procedure.
====
