...
[NOTE]
====
To prevent the {SMProduct} 3.0 control pane from injecting proxies to workloads in namespaces with `istio-injection=enabled` label applied and are still managed by {SMProduct} 2.6 control plane, do not use use the `default` name for Istio resource and do not create the `default` revision tag as part of this step. The `default` revision tag  is created later in this procedure.
====
