...
The `bookinfo` application is used as an example for the `{istio}` resource. For more information about configuration differences between the {SMProduct} 2.6 `ServiceMeshControlPlane` resource and the {SMProduct} 3.2 `{istio}` resource, see "ServiceMeshControlPlane resource to Istio resource fields mapping".

.Prerequisites

* You have deployed {ocp-product-title} 4.18 or later.
...
* You have installed the {SMProduct} 3.2 Operator from the `stable-3.2` update channel.
...
ifdef::ossm-cert-manager-istio-injection[]
. Confirm that your {SMProduct} 2.6 `ServiceMeshControlPlane` resource is configured with the cert-manager tool.
+
...
. Update the `istio-csr` deployment to include your {SMProduct} 3.2 control plane by running the following command:
+
[source,terminal]
----
  helm upgrade cert-manager-istio-csr jetstack/cert-manager-istio-csr \
      ...
      --namespace istio-system \
      --wait \
      --set "app.istio.revisions={basic,ossm-3-v1-24-6}"
----
+
where:
+
`app.istio.revisions`:: This field must include your {SMProduct} 3.2 control plane revision _before_ you create your `{istio}` resource so that proxies can properly communicate with the {SMProduct} 3.2 control plane.
endif::[]
...
istio-system   install-istio-system   6/6     ComponentsReady   ["default"]   2.6     115m
----

. Create a YAML file named `ossm-3.yaml`. This procedure creates the {istio} resource for the 3.2 installation in the same namespace as the `ServiceMeshControlPlane` resource for the 2.6 installation.
+
[NOTE]
====
...
* `spec.namespace` specifies the namespace where the 3.2 and 2.6 control planes must run.
* `spec.version` specifies the {istio} version. For migration, you must set it to `v1.24-latest`, which ensures that the control plane uses the most recent `1.24` patch release. 
+
include::snippets/ossm-migrate-istio-version.adoc[]
* `spec.values` specifies the configuration values for the control plane. If you are migrating metrics and tracing, update the `extensionProviders` fields according to your tracing and metrics configurations.
+

[NOTE]
====
To prevent the {SMProduct} 3.2 control plane from injecting proxies in the namespaces that have the `istio-injection=enabled` label applied and are still managed by {SMProduct} 2.6 control plane, do not use the `default` name for the {istio} resource, and do not create the `default` revision tag in the following steps. You create the `default` revision tag later in this procedure.
====
--
...
[source,terminal]
----
$ oc logs deployments/istiod-ossm-3-v1-24-6 -n istio-system | grep 'Load signing key and cert from existing secret'
----
+
*Example output:*
+
