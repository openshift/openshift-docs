...
= Configuring backup PVC for Data Mover backup

Use the `nodeAgent.backupPVC` section of the `DataProtectionApplication` (DPA) to configure the backup PVC for a storage class.
...
3. As the `backupPVC` for `storage-class-1` is `readOnly`, the `spcNoRelabeling` field is set to `true`.
...
* Verify that the backup PVCs are created as read-only by running the following command:
+
.Example command
[source,terminal]
----
