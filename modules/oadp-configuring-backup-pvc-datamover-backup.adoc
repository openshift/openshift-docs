...
= Configuring `backupPVC` for Data Mover backup

Use the `nodeAgent.backupPVC` section of the `DataProtectionApplication` (DPA) to configure the backup persistent volume claim (PVC) for a storage class.
