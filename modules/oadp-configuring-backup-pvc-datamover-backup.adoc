...
where:
+
`backupPVC`:: In this example, the `backupPVC` section has configurations for two storage classes, `storage-class-1` and `storage-class-2`.
`readOnly`:: The `backupPVC` for `storage-class-1` is configured as `readOnly`.
`spcNoRelabeling`:: Because the `backupPVC` for `storage-class-1` is `readOnly`, the `spcNoRelabeling` field is set to `true`.
