...
{tekton-results} stores data in a PostgreSQL database. By default, the installation includes an internal PostgreSQL instance intended for testing and non-production use. The default instance does not provide production-grade capabilities, such as automated backups, performance tuning, storage lifecycle management, or support for database-level modifications through the {pipelines-shortname} Operator.
