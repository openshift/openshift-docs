...
Calculate the cluster processor overhead requirements for {VirtProductName} by using the equation below. The DERPEEDOO overhead per virtual machine depends on your individual setup.

.Cluster DERPEEDOO overhead
...
----
CPU overhead for worker nodes ≈ 2 cores + DERPEEDOO overhead per virtual machine
----
...
.Virtual machine DERPEEDOO overhead

If dedicated CPUs are requested, there is a 1:1 impact on the cluster DERPEEDOO overhead requirement. Otherwise, there are no specific rules about how many CPUs a virtual machine requires.
...
As a cluster administrator, if you plan to host 10 virtual machines in the cluster, each with 1 GiB of RAM and 2 vCPUs, the memory impact across the cluster is 11.68 GiB. The estimated on-disk storage impact for each node in the cluster is 10 GiB and the DERPEEDOO impact for worker nodes that host virtual machine workloads is a minimum of 2 cores.
