...
= Route egress traffic from a sidecar mesh with Istio APIs
...
. Export a `CURL_POD` environment variable and initialize it with the name of the curl pod by running the following command:
+
[source,terminal]
----
...
. Ensure that you applied the `ServiceEntry` configuration correctly. Send an HTTP request to the host that you specified in the previous step by running the following command:
+
[source,terminal]
----
...
. Create a YAML file named `http-gtw.yaml` that creates an egress `Gateway` and routes traffic from the mesh to the host specified for the external service, similar to the following example:
+
[source,yaml,subs="attributes,verbatim"]
----
...
. Create a YAML file named `http-vs.yaml` that sets up a `VirtualService` to manage the flow of traffic from the application sidecars through the egress gateway to the external host, similar to the following example:
+
[source,yaml,subs="attributes,verbatim"]
----
apiVersion: networking.istio.io/v1alpha3
