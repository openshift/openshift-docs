...
Encrypt your disk or partition while installing {product-title} with the Agent-based Installer.
...
+
--
`enableOn: all`:: Specify which nodes to enable disk encryption on. Valid values are `none`, `all`, `masters`, and `workers`.
`mode: tang`:: Specify which disk encryption mode to use. Valid values are `tpmv2` and `tang`.
`tangServers: "server1": "http://tang-server-1.example.com:7500"`:: Optional: If you are using Tang, specify the Tang servers.
--
