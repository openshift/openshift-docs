...
where:
+
`name`:: Specifies the this ensures that the operator pod is only scheduled on an infrastructure node.
`namespace`:: Specifies the this ensures that the pod is accepted by the infrastructure node.
+
.Example output
[source,terminal]
----
