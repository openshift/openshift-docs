...
<2> Indicates multiple mirror repositories for the each target repository. If a one mirror is down, the target repository can use another mirror.
<3> Indicates the registry and repository containing the content that is mirrored.
<4> Indicates the name of the mirror.
