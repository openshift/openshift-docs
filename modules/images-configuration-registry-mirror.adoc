...
ifdef::winc[]
* The mirror information you enter is added to the `hosts.toml` containerd configuration file on every Windows node in the {product-title} cluster.
endif::winc[]
...
ifdef::winc[]
The Windows Machine Config Operator (WMCO) watches for changes to the IDMS and ITMS resources and generates a set of `hosts.toml` containerd configuration files, one file for each registry mirror, with those changes. The WMCO then updates any existing Windows nodes to use the new registry configuration.
