...
[id="managing-apps-in-non-control-plane-namespaces_{context}"]
= Applications in non-control plane namespaces
...
. Configuring the `ArgoCD` CR of your user-defined cluster-scoped Argo CD instance with the target namespaces.
. Creating and configuring a user-defined `AppProject` instance in the `openshift-gitops` control plane namespace and specifying the target namespaces in the `.spec.sourceNamespaces` field of the user-defined `AppProject` instance.
. Configuring the `metadata.namespace` and `.spec.project` fields of the `Application` CR to reference the target namespaces and user-defined `AppProject` instance.
...
* xref:../deploy/manage-applications-across-namespaces.adoc#gitops-configuring-argo-cd-cr-of-your-user-defined-cluster-scoped-instance-with-target-namespaces_manage-applications-across-namespaces[Configuring the ArgoCD CR of your user-defined cluster-scoped Argo CD instance with the target namespaces]
* xref:../deploy/manage-applications-across-namespaces.adoc#gitops-creating-and-configuring-user-defined-appproject-instance-with-target-namespaces_manage-applications-across-namespaces[Creating and configuring a user-defined AppProject instance with target namespaces]
* xref:../deploy/manage-applications-across-namespaces.adoc#gitops-creating-and-configuring-the-app-cr-to-reference-the-target-namespace-and-user-defined-appproject-instance_manage-applications-across-namespaces[Configuring the Application CR to reference the target namespace and user-defined AppProject instance]
