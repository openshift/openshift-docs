...
|- Manage optional user access to AWS components through {cluster-manager}.
- Create AWS IAM roles and attached policies necessary to enable {product-title} service access.
...
used for {product-title} control plane and worker nodes.
endif::openshift-rosa-hcp[]
ifndef::openshift-rosa-hcp[]
used for {product-title} control plane, infrastructure, and worker nodes.
endif::openshift-rosa-hcp[]

**Storage:** Provide Amazon EBS, used to allow {product-title} to provision local node storage and persistent volume storage for the cluster.

**Storage:** Provide Amazon S3, used for the service's built-in image registry.

**Networking:** Provide AWS Identity and Access Management (IAM), used by customers to control access to {product-title} resources running on customer accounts.
...
- To enable {product-title} across your AWS organization, the customer is
responsible for managing AWS Organizations administrators.

- To enable {product-title} across your AWS organization, the customer is
responsible for distributing the {product-title} entitlement grant using AWS License Manager.
