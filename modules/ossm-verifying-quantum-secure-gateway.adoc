...
= Verify PQC on a gateway with a sample application

[role="_abstract"]

Verify the post-quantum gateway configuration by testing connections with both {pqc-short}-enabled and standard clients to confirm that the gateway accepts only quantum-safe handshakes.
