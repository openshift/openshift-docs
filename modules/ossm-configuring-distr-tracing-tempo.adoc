...
<3> Pipeline order is `memory_limiter` then `batch`. Without these processors, the OTLP exporter sending queue can fill under load and the collector drops spans (`sending queue is full`).
+
[NOTE]
====
