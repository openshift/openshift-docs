...
[WARNING]
====
Starting with {product-title} 4.14, the Custom Domain Operator is deprecated. To manage Ingress in {product-title} 4.14 or later, use the Ingress Operator.
====
...
ifdef::openshift-rosa-hcp[]
{product-title} with hosted control planes (HCP).
endif::openshift-rosa-hcp[]
ifndef::openshift-rosa-hcp[]
{product-title}.
