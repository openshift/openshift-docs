...
where:
+
`metadata.namespace`:: Specifies the namespace where the module will be deployed.
`spec.moduleLoader.serviceAccountName`:: Specifies the service account that will be used to run the module. The default service account does not have the required permissions to run a module that is privileged. For information on creating a service account, see "Creating service accounts".
