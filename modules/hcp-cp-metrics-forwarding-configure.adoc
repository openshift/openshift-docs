...
[id="hcp-cp-metrics-forwarding-configure_{context}"]
...
* You are running {product-title} 4.22.8 or later.
...
. Optional: Specify the metrics set for this hosted cluster.
+
...
. Confirm the `metrics-proxy` deployment is running in the hosted control plane namespace on the management cluster by entering the following command:
+
[source,terminal]
----
...
. Verify that platform Prometheus in the hosted cluster is scraping control plane metrics by querying a forwarded metric, such as `apiserver_request_total`, from the hosted cluster Prometheus endpoint.
+
[NOTE]
====
