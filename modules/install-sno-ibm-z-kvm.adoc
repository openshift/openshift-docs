...
:navtitle: Install {sno} with {op-system-base} KVM on {ibm-z-title} and {ibm-linuxone-title}
[id="installing-sno-on-ibm-z-kvm_{context}"]
= Installing {sno} with {op-system-base} KVM on {ibm-z-title} and {ibm-linuxone-title}

[role="_abstract"]
You can install {sno} with with {op-system-base} KVM on {ibm-z-title} and {ibm-linuxone-title}.
...
** For the `ip=` parameter, assign the IP address manually as described in "Installing a cluster with {op-system-base} KVM on {ibm-z-name} and {ibm-linuxone-name}".
