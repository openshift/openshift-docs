...
. Add `apps-1` and `apps-2` applications to the mesh in ambient mode:

.. Add the `istio.io/dataplane-mode=ambient` label to the namespace as follows:
+
[source, terminal]
----
$ label namespace apps-1 `istio.io/dataplane-mode=ambient`
$ label namespace apps-1 `istio.io/dataplane-mode=ambient`
----
 You can also add this label to individual pods to include only individual workloads.
