...
[role="_abstract"]
Before upgrading the Operator, delete workflows that run with the `gitops` profile. After the upgrade completes, rebuild the corresponding workflow image by using the {ServerlessLogicProductName} 1.37.0 Serverless Workflow builder image. Redeploy the workflow by using the rebuilt image.
