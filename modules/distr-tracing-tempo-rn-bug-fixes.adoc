...
Jaeger-gRPC port reopened in Tempo, resolving timeout issues in Kiali::
+
In this update, the issue of the Jaeger-gRPC port being closed in Tempo 0.20, causing gRPC client errors and timeouts, resulting in trace fetching failure in Kiali, has been resolved. The Tempo Operator PR has opened the Jaeger-gRPC port in Tempo 0.20, eliminating timeouts in Kiali and improving distributed tracing. This action enhances the performance of gRPC clients by addressing the port-related issue.
+
link:https://redhat.atlassian.net/browse/TRACING-6061[TRACING-6061]

Network policy update that restores tracing visuals in Kiali::
+
In this update, the Tempo Operator network policy has been actively modified to include ingress to port 16685. Previously, this lack of ingress caused Kiali to fail in displaying tracing visuals during the upgrade. Now, with this release, the Tempo Operator enables Kiali to access tracing visuals due to the added ingress port 16685 in the network policy.
+
link:https://redhat.atlassian.net/browse/TRACING-6070[TRACING-6070]

Network policy update that resolves an Operator issue::
In this update, the network policy for the Tempo TempoStack Gateway includes port 8081, making the Tempo Operator accessible and resolving the Target unreachable alert.
+
