...
. Create the `icsp.yaml` file with the `image-content-sources` information. The `image-content-sources` information is available in the `ImageContentSourcePolicy` YAML file that is generated after you mirror the images by using `oc-mirror`.
+
...
. When you create a hosted cluster, provide the `additional-trust-bundle` certificate to update the compute nodes with the certificates:
+
[source,terminal]
----
...
<7> Replace with the supported {ocp-short} version that you want to use, for example, `4.17.0-multi`.
