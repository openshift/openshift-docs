...
<installation_directory>:: Specifies the directory that stored any files created by the installation program.
<log_level>:: Specifies the log level. To view different log details, specify `error`, `info`, or `warn` instead of `debug`.
