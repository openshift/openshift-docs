...
:navtitle: Install {op-system} using {ibm-title} Secure Execution
[id="installing-rhcos-using-ibm-secure-execution_{context}"]
= Installing {op-system} using {ibm-title} Secure Execution

[role="_abstract"]
You can install {op-system} using {ibm-name} Secure Execution to run nodes as protected guests, isolating workloads from the host system. Before you begin, you must prepare the underlying infrastructure and verify hardware and software prerequisites.
...
.. Navigate to `/boot/loader/entries/` to modify your boot loader configuration file `*.conf`.
...
[NOTE]
====
You can add as many host keys as needed if you want your node to be able to run on multiple {ibm-z-name} machines.
====
