...
The resource created in this example scopes the access log to the `_<mcp_gateway>_` `Gateway` object pods only. Other workloads in the mesh are not affected.
...
* You created `AuthPolicy` audit custom resources (CRs) for both {mcpg} listeners.
