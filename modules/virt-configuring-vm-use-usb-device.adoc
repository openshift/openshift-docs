...
* You have attached the required USB device as a resource at the cluster level. For instructions, see xref:./virt-enabling-usb-host-passthrough.adoc#virt-enabling-usb-host-passthrough_virt-configuring-usb-host-passthrough[Enabling USB host passthrough].
...
. Open the VM instance CR by running the following command. Replace `vmi-usb` with the name of the instance:
+
[source,terminal]
----
$ oc edit vmi vmi-usb
