...
matchLabels:: The label selector that determines which pods belong to the daemon set.
labels (template metadata):: The pod template's label selector. Must match the label selector above.
nodeSelector:: The node selector that determines on which nodes pod replicas should be deployed. A matching label must be present on the node.
