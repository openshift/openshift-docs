...
{product-title} provides disaster recovery for failures that occur at the pod, node, and availability zone levels.
...
**Storage:** Provide the ability for the {product-title} service
...
|- Configure {product-title} clusters with multiple machine pools across multiple availability zones to improve fault tolerance and cluster availability.
...
across
Availability Zones. This functionality enables {product-title} to orchestrate automatic failover between zones without interruption.
