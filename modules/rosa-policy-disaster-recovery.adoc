...
One cluster with a single machine pool will not provide disaster avoidance or recovery in case of an Availability Zone or region outage. Multiple clusters with single machine pools with customer-maintained failover can account for outages at the zone or at the regional level.
...
across AZs. This functionality enables {product-title} to orchestrate automatic failover between zones without interruption.
