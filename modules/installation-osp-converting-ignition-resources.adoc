...
:navtitle: Prepare the bootstrap Ignition files
...
where:
+
`ignition.config.merge.source`:: Replace the value of `ignition.config.merge.source` with the bootstrap Ignition file storage URL.
`ignition.config.merge.source.httpHeaders.name`:: Specifies `name` in `httpHeaders` to `"X-Auth-Token"`.
`ignition.config.merge.source.httpHeaders.value`:: Specifies `value` in `httpHeaders` to your token's ID.
`security.tls.certificateAuthorities.source`:: If the bootstrap Ignition file server uses a self-signed certificate, include the base64-encoded certificate.
