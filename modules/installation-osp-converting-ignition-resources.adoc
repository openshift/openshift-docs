...
<storage_url>:: Specifies the bootstrap Ignition file storage URL. Replace the value of `ignition.config.merge.source` with this URL.
X-Auth-Token:: Set `name` in `httpHeaders` to `"X-Auth-Token"`.
<token_ID>:: Specifies your token's ID. Set `value` in `httpHeaders` to your token's ID.
<base64_encoded_certificate>:: If the bootstrap Ignition file server uses a self-signed certificate, include the base64-encoded certificate.
