...
. To ensure the first control plane discovers the namespace, add the `istio-discovery=mesh-1` label by running the following command:
+
[source,terminal]
----
...
. To enable sidecar injection into all the pods by default, while mapping the pods in this namespace to the first control plane, add the `istio.io/rev=mesh-1` label to the namespace by running the following command:
+
[source,terminal]
----
...
. After deploying the applications, use the `istioctl ps` command to verify that the correct control plane manages each workload:

.. Verify that the `istio-system-1` control plane manages the workloads by running the following command:
+
[source,terminal]
----
...
.. Verify that the `istio-system-2` control plane manages the workloads by running the following command:
+
[source,terminal]
----
...
.. Confirm that the communication works by sending a request from the `sleep` pod to the `httpbin` service that are present in the `app-ns-2` namespace, which `mesh-2` manages by running the following command:
+
[source,terminal]
----
