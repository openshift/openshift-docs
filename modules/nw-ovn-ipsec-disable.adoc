...
----
+
.. Where the `ipsecConfig.mode` parameter is set `Full` and the `ipsecConfig.full` configuration is added to `networks.operator.openshift.io`, enter the following command:
+
----
