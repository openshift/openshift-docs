...
.. For networks that have DHCP enabled, you do not need to set a static IP. Instead, do the following: 

... Run the following `coreos-installer` command from the target host console to install the system:
+
[source,terminal]
----
...
... If node installation hangs instead of proceeding automatically, manually reboot the worker node.
