...
where:
+
`--dry-run`:: The `--dry-run` option outputs files, including the new `keys.json` file, to the disk without making API calls.
`--public-key-file`:: Specify the path to the public key that you downloaded in the previous step.
`--name`:: Because the `--dry-run` option does not make any API calls, some parameters do not require real values.
`--region`:: Specify any valid {aws-short} region, such as `us-east-1`.
...
where:
+
`ccoctl aws`:: The `ccoctl azure` command does not include a `--dry-run` option.
