...
[role="_abstract"]
Configure the Bare Metal Operator to manage `BareMetalHost` resources across all namespaces by enabling the `watchAllNamespaces` setting in the `Provisioning` resource, enabling centralized bare-metal infrastructure management without namespace-specific BMO deployments.
