...
The {ocp-short-name} router provides a standard approach for managing ingress traffic, and enables you to manage certificates for all cluster ingress using the same methods. However, the  {ocp-short-name} router introduces an additional hop between the inbound traffic and the mesh applications. Typically, you route the traffic by decrypting it at the router and then re-encrypting it at the service mesh ingress gateway, which introduces latency.

To use Kubernetes load balancing, set the `spec.type` parameter for the gateway `Service` to `LoadBalancer`. 
...
To use the {ocp-short-name} router, see the "Exposing a gateway to traffic outside the cluster using {ocp-short-name} routes".
