...
In large environments (thousands of namespaces), the application of user scopes that cover large numbers of namespaces can lead to some pages being slow to render or even timing out (for example, timeouts when opening the main *Dashboard* page).

The performances were tested in single-cluster context on an environment with 20000 deployents across 5000 namespaces and 80000 alerts in the one test cluster. In that case, the load time of the main *Dashboard* page for scopes including up to 2000 namespaces on that one cluster was found to be still under a few seconds.

The performances were tested in multi-cluster context on an environment with 200000 deployments across 50000 namespaces and 800000 alerts across 10 test clusters. In that case, the load time of the main *Dashboard* page was found to be still under a few seconds for scopes including up to 500 namespaces across the ten clusters as well as for scopes including up to 2000 namespaces on a single cluster.

The load time of UI pages with user scopes larger than the tested ones cannot be guaranteed to stay under the timeout limit.
====
