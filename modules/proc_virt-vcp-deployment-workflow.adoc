...
. Create the control plane VMs on the hosting cluster and configure the VMs with appropriate resources and network settings.
...
. After installation completes, a new {product-title} cluster is deployed with its control plane running on VMs hosted by the {VirtProductName} cluster.
