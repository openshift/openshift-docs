...
= Identify the revision name for sidecar injection

[role="_abstract"]

Manage sidecar injection by applying revision-specific labels to workloads, which allows the {SMProductName} Operator to automate control plane association through `IstioRevision` resources.
...
== Enable sidecar injection with default revision
...
== Enable sidecar injection with other revisions
