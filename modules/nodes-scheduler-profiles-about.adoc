...
[NOTE]
====
Switching to the `HighNodeUtilization` scheduler profile will result in all pods of a ` ReplicaSet to be started on the same node. This will add an increased risk for pod failure if the node fails.
====
