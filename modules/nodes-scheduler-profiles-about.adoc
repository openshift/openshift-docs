...
[NOTE]
====
Switching to the `HighNodeUtilization` scheduler profile will result in all pods of a ` `ReplicaSet` being scheduled on the same node. This will add an increased risk for pod failure if the node fails.
====
