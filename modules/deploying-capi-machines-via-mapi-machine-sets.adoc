...
= Deploying Cluster API machines using a Machine API machine set
...
Where:
+
`spec.authoritativeAPI: MachineAPI`:: The unconverted value for the Machine API compute machine set.
Do not change the value in this part of the specification.
`spec.template.spec.authoritativeAPI: ClusterAPI`:: Specify `ClusterAPI` to configure the compute machine set to deploy Cluster API compute machines.
`status.authoritativeAPI: MachineAPI`:: The current value for the Machine API compute machine set.
Do not change the value in this part of the specification.
...
. Verify that a new machine has the correct configuration by examining the `status.authoritativeAPI` field in the machine CR:
+
[source,terminal]
----
...
* xref:../../machine_management/cluster_api_machine_management/cluster-api-getting-started.adoc#migrating-between-capi-mapi_cluster-api-getting-started[Migrating a Machine API resource to use the Cluster API]
