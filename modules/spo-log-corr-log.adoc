...
= Correlate with API Server Audit Log

[role="_abstract"]
Configure mutating webhook options for the Audit JSON log enricher so you can correlate API server audit logs with `exec` session activity, and optionally limit which namespaces the webhooks apply to.

By default, when you use the `kubectl exec` command to access a pod or container, Kubernetes does not pass authentication details for the user into the session environment. This means the Audit JSON log enricher cannot provide audit information for `exec` commands. The `UID` or `GID` shown, maps to the system user. In most cases this would be the root user.
