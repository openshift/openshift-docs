...
[NOTE]
====
Currently, ISO boot support on {ibm-z-name} (s390x) is available only for {op-system-base} KVM, which provides the flexibility to choose either PXE or ISO-based installation. For installations with z/VM and LPAR, only PXE boot is supported.
====
