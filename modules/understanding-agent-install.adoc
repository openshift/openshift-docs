...
*For more information on adding worker nodes to single-node OpenShift clusters, see xref:https://access.redhat.com/documentation/en-us/openshift_container_platform/4.13/html/nodes/worker-nodes-for-single-node-openshift-clusters[Adding worker nodes to sigle-node OpenShift clusters].
