...
[IMPORTANT]
====
Loki performs best with a limited number of label combinations (cardinality). Each unique set of labels creates a new stream, which increases storage and query overhead. The default label extraction provides good query performance while keeping cardinality manageable.
