...
* If you use the OVN-Kubernetes network plugin and configured `gatewayConfig.routingViaHost` to `false` for its configuration, the configuration impacts returned ingress replies because of egress firewall rules. If the egress firewall rules drop the ingress reply destination IP, the traffic gets dropped.
endif::ovn[]
