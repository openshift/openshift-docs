...
To allow the API servers, you may either add allow rules for each IP address or use the `nodeSelector` type allow rule in your xref:../networking/ovn_kubernetes_network_provider/configuring-egress-firewall-ovn.adoc#nw-egressnetworkpolicy-about_configuring-egress-firewall-ovn[egress policy rules].
