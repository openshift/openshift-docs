...
If a `PodDisruptionBudget` uses the `minAvailable: 1` setting, the eviction process while nodes are drained to apply pending machine configs can block the upgrade process. If several nodes are rebooted, all the pods can be running in only one node, and the pod disruption budget can prevent the node from draining. For more information, see link:https://access.redhat.com/solutions/4857671[PodDisruptionBudget (PDB) could cause Machine-Config-Operator (MCO) to be degraded during OCP4 upgrade].
