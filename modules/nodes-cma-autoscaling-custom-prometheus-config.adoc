...
You must use trigger authentication or cluster trigger authentication in order to include the authentication information required for your scaled objects to be able to read the {product-title} Prometheus metrics. The following procedure differs depending on which method you use. For more information on trigger authentications, see "Understanding custom metrics autoscaler trigger authentications". 
...
+
*  If you are using trigger authentication, specify the project with the object you want to scale.
*  If you are using cluster trigger authentication, specify the `openshift-keda` project.
...
spec:
  ...
  - parameter: ca
    ...
    key: ca.crt
----
<1> Specifies the one of the following trigger authentication type:
+
*  If you are using trigger authentication, specify `TriggerAuthentication`.
*  If you are using cluster trigger authentication, specify `ClusterTriggerAuthentication`.
+
...
----
<1> Specifies the one of the following object types:
+
*  If you are using trigger authentication, specify `RoleBinding`.
*  If you are using cluster trigger authentication, specify `ClusterRoleBinding`.
+
<2> Specifies the name of the role you created.
<3> Specifies the namespace of the object you want to scale.
+
*  If you are using trigger authentication, specify the project with the object you want to scale.
*  If you are using cluster trigger authentication, specify the `openshift-keda` project.
+
