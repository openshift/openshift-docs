...
. Apply the remediation by setting the attribute to `true`:
+
[source,terminal]
----
...
. Optionally, the Compliance Operator can apply remediations automatically. Set `autoApplyRemediations: true` in the `ScanSetting` top-level object.
+
[WARNING]
====
