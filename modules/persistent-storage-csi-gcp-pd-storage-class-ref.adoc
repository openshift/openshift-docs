...
For `hyperdisk-balanced`, be sure to check the limitations under "C3 and N4 instance type limitations".
...
[NOTE]
====
On Google Cloud Dedicated sovereign cloud infrastructure, the `type` parameter is limited to `hyperdisk-balanced`.
Other disk types (`pd-standard`, `pd-ssd`, `pd-balanced`) are not available on Google Cloud Dedicated.
The default `StorageClass` on Google Cloud Dedicated is automatically configured to use `hyperdisk-balanced` instead of `pd-standard`.
====
