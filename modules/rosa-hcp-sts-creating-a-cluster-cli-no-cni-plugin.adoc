...
[role="_abstract"]
You can create a {product-title} cluster without a CNI plugin by using the `--no-cni` flag with the {rosa-cli-first}.
...
* You have created an OpenID Connect (OIDC) configuration.
