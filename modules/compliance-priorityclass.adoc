...
[NOTE]
====
If the `PriorityClass` referenced in the `ScanSetting` cannot be found, the Operator leaves the `PriorityClass` empty, issues a warning, and continues scheduling scans without a designated `PriorityClass`.
====