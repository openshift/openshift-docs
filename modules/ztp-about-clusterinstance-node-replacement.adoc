...
[role="_abstract"]
You can replace a failed control plane node on a bare metal multi-node {product-title} cluster by removing the node from the `ClusterInstance` custom resource (CR) and then adding a replacement node.
The SiteConfig Operator applies the Git change and the Assisted Installer provisions the host again.
...
The Assisted Installer starts deprovisioning the host.
You must manually remove the failed `etcd` member.
Scale out::
...
Complete the scale-in operation before you start the scale-out operation.

[WARNING]
====
Adding a replacement node before you remove the failed node is not supported.
