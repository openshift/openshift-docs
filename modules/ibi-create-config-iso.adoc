...
<7> Optional. Specify the `ConfigMap` object that contains the network configuration files. When setting static IP, ensure that you set the `dhcp: false` field. Otherwise, the installation fails while waiting for DHCP.
