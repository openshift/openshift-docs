...
[id="cluster-compare-generate-patch_{context}"]
...
. Optional: Review the patch file before applying to the reference configuration. The following example shows the structure of a patch configuration file:
+
[source,yaml]
----
...
+
The following example shows the output after applying a patch:
+
[source,terminal]
----
...
