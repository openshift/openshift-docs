...
This sample YAML defines a compute machine set that runs in {gcp-full} and creates nodes that are labeled with
...
.Sample {gcp-full} `MachineSet` values
[source,yaml]
----
...
<5> For `<project_name>`, specify the name of the {gcp-full} project that you use for your cluster.
