...
This sample YAML defines a MachineSet that runs in Google Cloud Platform (GCP_
...
spec:
  ...
  selector:
    matchLabels:
      machine.openshift.io/cluster-api-cluster: <infrastructureID> <1>
      machine.openshift.io/cluster-api-machineset: <infrastructureID>-w-a <2>
  template:
    metadata:
    ...
    spec:
      ...
      providerSpec:
        value:
          ...
          region: us-central1
          serviceAccounts:
          - email: <infrastructureID>-w@openshift-gce-devel.iam.gserviceaccount.com <1>
            scopes:
