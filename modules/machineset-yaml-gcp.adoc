...
`<infrastructure_id>`:: Specify the infrastructure ID that is based on the cluster ID that you set when you provisioned the cluster.
ifndef::infra[]
...
`<path_to_image>`:: Specify the path to the image that is used in current compute machine sets.
+
...
--
`<gcpMetadata>`:: Optional: Specify custom metadata in the form of a `key:value` pair. For example use cases, see the {gcp-short} documentation for link:https://cloud.google.com/compute/docs/metadata/setting-custom-metadata[setting custom metadata].
`<project_name>`:: For `<project_name>`, specify the name of the {gcp-short} project that you use for your cluster.
