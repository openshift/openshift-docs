...
platform:
  ibmcloud:
    region: eu-gb <1>
    vpcName: eu-gb-example-network-1 <5>
    controlPlaneSubnets: <6>
...
<6> Specify the name of the existing subnet to deploy the control plane machines to. The subnet must belong to the VPC that you specified. Specify a subnet for each availability zone in the region.
<7> Specify the name of the existing subnet to deploy the compute machines to. The subnet must belong to the VPC that you specified. Specify a subnet for each availability zone in the region.
ifndef::openshift-origin[]
