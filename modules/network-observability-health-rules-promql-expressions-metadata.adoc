...
== Customize alert logic with PromQL: Incoming traffic surge
...
Threshold application:: A final threshold filters increases of less than a specific percentage. For example, `> 100` removes increases lower than 100%.
...
| `contextTab`
| String
| The context tab in which the alert is displayed. Valid values include `netobserv`, `ovn`, `kiali`, or other third-party context identifiers. If not specified, the alert is displayed in the default NetObserv tab.

| `displayName`
| String
| A custom display name for the alert shown in the *Network Health* UI. If not specified, the alert uses the default naming convention.

| `namespaceLabels`
| List of strings
| One or more labels containing namespaces. Alerts are displayed under the *Namespaces* tab.

| `nodeLabels`
| List of strings
| One or more labels containing node names. Alerts are displayed under the *Nodes* tab.

| `workloadLabels`
| List of strings
| One or more labels containing owner or workload names. Alerts are displayed under the *Owners* tab when `kindLabels` is also provided.
...
[NOTE]
====
The `namespaceLabels` and `nodeLabels` fields are mutually exclusive. If neither is provided, the alert is displayed under the *Global* tab.
====