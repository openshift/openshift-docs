...
|Your Vault server URL and secret path. Replace the `auth.identity.sub` variable according to how your claims uniquely identify an {mcpg} user if needed. The Vault path in the expression must match the KV Secrets Engine version.
