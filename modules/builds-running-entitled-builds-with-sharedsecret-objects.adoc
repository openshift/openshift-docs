...
[IMPORTANT]
====
* You must include the command to remove the `/etc/rhsm-host` directory and all its contents in the Dockerfile before executing any `yum` or `dnf` commands. The following example explains the command you can use to remove the `/etc/rhsm-host` directory:
+
[source,Dockerfile]
----
FROM registry.redhat.io:latest
