...
=======
=======
>>>>>>> f081ea5f78 (merge conflicts 20-dec-22)
...
<<<<<<< HEAD
>>>>>>> 5bf9bcecf3 (squashing commits to resolve rebase issues)
=======
=======
|`platform.ovirt.api_vips`
|Required. An IP address on the machine network that will be assigned to the API virtual IP (VIP). You can access the OpenShift API at this endpoint. For dual-stack networks, assign up to two IP addresses. The primary IP address must be from the IPv4 network.
...
Additional Alibaba Cloud configuration parameters are described in the following table. 

The `alibabacloud` parameters are the configuration used when installing on Alibaba Cloud. The `defaultMachinePlatform` parameters are the default configuration used when installing on Alibaba Cloud for machine pools that do not define their own platform configuration. 
...
|Optional. The ID of an existing private zone into which to add DNS records for the cluster internal API. To use an existing private zone you must use an existing VPC. The provate zone must be associated with the VPC containing the subjets. The installer will create the private zone if this value is not set.
|String.
...
|The ID list of already existing VSwitches where cluster resources will be created. The existing VSwitches can only be used when also using an existing VPC. If empty, the installer will create new VSwitches for the cluster.
|String list.
...
|For both compute machines and control plane machines, the ECS instance type used to create the ECS instance. Example: `ecs.g6.large`
|String. 
