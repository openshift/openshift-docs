...
To finish adding the new control plane node, add the new etcd member to the cluster.
...
.. Connect to one of the running etcd pods by running the following command. The etcd pod on the new node should be in `CrashLoopBackOff` state.
+
[source,terminal]
----
