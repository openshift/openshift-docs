...
{ols-long} supports automatic updates of BYO Knowledge images that use floating tags, such as `latest`. If over time a BYO Knowledge image tag points to different underlying images, {ols-long} detects the change and updates the corresponding BYO Knowledge database accordingly. This feature is built using OpenShift ImageStream objects. {ocp-product-title} clusters check for updates to ImageStream object every 15 minutes.