...
This procedure configures a T-GM (Telecom Grandmaster) clock on Single Node OpenShift (SNO) that uses an Intel E810 Westport Channel NIC as the PTP grandmaster clock with GNSS to NTP failover capabilities.
...
----
+
The output is similar to the following showing the NodePtpDevice resource for the SNO node with PTP-capable network interfaces:
+
[source,yaml]
----
apiVersion: v1
...
+
NOTE: On SNO clusters, you will see only one NodePtpDevice resource for the single master node.

. The PTP profile uses node labels for matching. Check your Machine Config Pool (MCP) to verify the master MCP. Run the following command:
+
[source,terminal]
----
$ oc get mcp
...
----
+
IMPORTANT: This configuration uses wildcard patterns for automatic interface detection. The `(?<iface_timeTx>[[:alnum:]]+)` pattern will automatically match your E810 NIC interface names. The `nodeLabel` is set to `node-role.kubernetes.io/master` to target the SNO master node which serves all roles.
+
