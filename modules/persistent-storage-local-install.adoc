...
. Optional: Allow local-storage operator installation on single node clusters.
+
To allow installation of local-storage operator in single node clusters, we need to add following annotation to the namespace in which operator is being installed.
+
[source,terminal]
----
$ oc annotate project openshift-local-storage workload.openshift.io/allowed='management'
----
+
Additionally the annotation can also be added by editing `openshift-local-storage` namespace:
+
[source,yaml]
----
apiVersion: v1
