...
:navtitle: View and interacting with certificates
[id="checking-mco-status-certs_{context}"]
= Viewing and interacting with certificates

[role="_abstract"]
Machine Config Operator certificates are used to secure connections between the {op-system-first} nodes and the Machine Config Server. 
