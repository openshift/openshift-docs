...
[NOTE]
====
If a machine config pool (MCP) is paused, the Kubernetes API Server Operator cannot rotate the `kubelet-ca.crt` bundle on nodes associated with the MCP. If the pool remains paused, the nodes in the specified pool stop working when their existing certificates expire. For more information and mitigation steps, see the link:https://github.com/openshift/runbooks/blob/master/alerts/machine-config-operator/MachineConfigControllerPausedPoolKubeletCA.md[MachineConfigControllerPausedPoolKubeletCA] runbook.
====
