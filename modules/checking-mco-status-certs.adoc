...
[NOTE]
====
If a machine config pool (MCP) is paused, the Kubernetes API Server Operator cannot rotate the `kubelet-ca.crt` bundle to that pool's nodes. If the pool remains paused, the nodes in the specified pool stop working when the certificates in the pool's existing kubelet-ca.crt bundle expire. For more information and mitigation steps, see the link:https://github.com/openshift/runbooks/blob/master/alerts/machine-config-operator/MachineConfigControllerPausedPoolKubeletCA.md[MachineConfigControllerPausedPoolKubeletCA] runbook.
====
...
[source,terminal]
----
$ oc get controllerconfig/machine-config-controller -o yaml | yq  '.status.controllerCertificates'
----
+
.Example output
+
...
[source,terminal]
----
$ oc get mcp master -o yaml | yq '.status.certExpirys'
----
+
.Example output
+
