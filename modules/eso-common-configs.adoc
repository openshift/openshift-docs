...
| `affinity` is used for setting scheduling affinity rules. See link:https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/[Assigning Pods to Nodes].
|
| 
...
| `proxy`
| _proxyConfig_
| `proxy` sets the proxy configurations which are made available in operand containers managed by the Operator as environment variables.
|
| 
