...
[role="_abstract"]
When you deploy a {product-title} cluster, you must create the Operator Identity and Access Management (IAM) roles. The cluster Operators use the Operator roles and policies to obtain temporary permissions to perform cluster operations, such as managing storage and external access.
...
`--oidc-config-id=`:: This value is the OpenID Connect (OIDC) configuration ID that you created for your {product-title} cluster.
`--installer-role-arn`:: This value is the installer role Amazon Resource Name (ARN) that you created when you created the {product-title} account roles.
--
+
