...
[role="_abstract"]
To understand OVN-Kubernetes logical networking entities, you must examine the southbound database running as a container inside the node pod. This inspection allows you to view active configurations and verify chassis and port binding properties.
...
. Open a remote shell into the `nbdb` or `sbdb` containers on the relevant node by entering the following command:
+
[source,terminal]
----
...
Any pods communicate out to the wider network by using source network address translation (SNAT).
