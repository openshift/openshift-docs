...
* To run `ovn-nbctl` or `ovn-sbctl` commands in a cluster, open a remote shell into the `nbdb` or `sbdb` containers on the relevant node.
...
. Navigate into a pod to examine the southbound database:
+
[source,terminal]
----
