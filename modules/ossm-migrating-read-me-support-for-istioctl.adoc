...
{SMProductName} 1 and 2 did not include support for Istioctl, the command line utility for the Istio project that includes many diagnostic and debugging utilities. {SMProduct} 3.2 introduces support for Istioctl for select commands.

The following commands are supported in {SMProduct} 3.2:
...
Installation and management of Istio is only supported by the {SMProduct} 3.2 Operator.