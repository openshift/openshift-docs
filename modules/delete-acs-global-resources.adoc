...
[NOTE]
====
You might receive the `error: the server doesn't have a resource type "psp"` error message in {product-title-short} 4.4 and later versions because the pod security policies (PSPs) are deprecated. Kubernetes removed the PSPs in version 1.25, except for clusters with older Kubernetes versions.
====
