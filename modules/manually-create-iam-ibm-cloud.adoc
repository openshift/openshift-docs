...
[role="_abstract"]
To install {product-title}, the Cloud Credential Operator (CCO) must operate in manual mode. While the installation program configures the CCO for manual mode, you must specify the identity and access management secrets for your cloud provider.
...
`credentialsMode`:: Specifies the CCO credentials mode. Set the value to `Manual`.
...
`--included`:: Specifies that only the manifests that your specific cluster configuration requires are included.
...
`<path_to_credential_requests_directory>`:: Specifies the directory that has the files for the `CredentialsRequest` objects.
`<cluster_name>`:: Specifies the name of the {product-title} cluster.
`<installation_directory>`:: Specifies the directory in which you want the `ccoctl` utility to create objects. By default, the utility creates objects in the directory in which you run the commands. This parameter is optional.
`<resource_group_name>`:: Specifies the name of the resource group used for scoping the access policies. This parameter is optional.
+
--
[NOTE]
====
