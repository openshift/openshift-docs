...
[NOTE]
====
When creating secrets to image pull information using `imagePullSecrets` object, you cannot use the `{serviceaccount-name}-dockercfg` pattern. When this pattern is used, the `openshift-controller-manager` does not create a token or pull secret for that service account.
====
