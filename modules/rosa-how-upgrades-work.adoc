...
The Cluster Version Operator (CVO) is the primary component that orchestrates and facilitates the OpenShift Container Platform update process. The scheduling, monitoring, and notifications of ROSA (classic architecture) cluster upgrades are orchestrated through the Managed Upgrade Operator (MUO). The MUO manages the orchestration of automated in-place cluster upgrades by satisfying the operating conditions that a managed cluster must hold before and after initiating the cluster upgrade.
...
You can observe the status of the cluster upgrade by running the `oc get clusterversion` command in the ROSA CLI (`rosa`).

[id="rosa-cluster-upgrade-overview_{context}"]
== ROSA (classic architecture) upgrade overview

The following high-level steps that occur during the ROSA (classic architecture)cluster update:
...
** Discovering any nodes that are not in a `Ready` status.
