...
= How cluster upgrades work

[role="_abstract"]
Knowing how cluster upgrades work helps you anticipate what happens during an update, including pre-health checks and temporary capacity provisioning, so you can plan maintenance windows and protect critical workloads.
...
The Managed Upgrade Operator (MUO) handles the scheduling, monitoring, and notifications of {product-title} cluster upgrades. The MUO orchestrates the automated in-place cluster upgrades by ensuring the operating conditions are met before and after the upgrade of a managed cluster.
...
You can observe the status of the cluster upgrade by running the `rosa describe upgrade --cluster=<cluster name_or_id>` command in the {rosa-cli-first}.

[id="rosa-cluster-upgrade-overview_{context}"]
== Cluster upgrade overview

The following are the high-level steps that occur during the {product-title} cluster update:
...
. Finally, the Machine Config Operator (MCO) updates the system configuration and operating system of every node. During this step, each node is rebooted after successfully draining the workloads running on the node.
