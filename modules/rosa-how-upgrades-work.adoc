...
== {product-title} upgrade overview
...
. Finally, th Machine Config Operator (MCO) updates the system configuration and operating system of every node. During this step, each node is rebooted after successfully draining the workloads running on the node.
