...
Red Hat recommends object-level backup solutions for {product-title} clusters. OpenShift API for Data Protection (OADP) is included in OpenShift but not enabled by default. Customers can configure OADP on their clusters to achieve object-level backup and restore capabilities.
...
ifdef::openshift-rosa-hcp[]
Alternatively, it is supported to run Windows based virtual machines on OpenShift Virtualization running on a {product-title} cluster.
endif::openshift-rosa-hcp[]
