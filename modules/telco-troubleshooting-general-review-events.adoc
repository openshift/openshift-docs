...
Review events in a given namespace by using the CLI.

.Procedure

. Check for events in your namespace:
+
[source,terminal]
----
$ oc get events -n <namespace> --sort-by=".metadata.creationTimestamp" <1>
----
<1> Adding the `--sort-by=".metadata.creationTimestamp"` flag places the most recent events at the bottom of the output.
...
----
<1> The `--sort-by=".metadata.creationTimestamp"` flag places the most recent events at the bottom of the output.
+
...
With `grep`, you can search for keywords, such as `error` or `failed`.
