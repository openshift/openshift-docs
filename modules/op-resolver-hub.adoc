...
[role="_abstract"]
Specify a remote pipeline, task, or step action from {artifact-hub} by using the hub resolver.

When you create a pipeline run, you can specify a remote pipeline from {artifact-hub}. When you create a pipeline or a task run, you can specify a remote task from {artifact-hub}. When you create a step within a task, you can reference a remote `StepAction` definition from {artifact-hub}.
...
| `type`
| The type of the catalog for pulling the resource.
| Default:  `artifact`
