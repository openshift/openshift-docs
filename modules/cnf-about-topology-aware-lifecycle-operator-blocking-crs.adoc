...
. Create the `ClusterGroupUpgrade` CRs, and then save the YAML files in the `cgu-a.yaml`, `cgu-b.yaml`, and `cgu-c.yaml` files. When you set the `enable` field to `true` in all the CRs, their status should be as depicted below:
+
[source,yaml]
----
...
status:
  ...
  status: {}
----
<1> The `cgu-c` update does not have any blocking CRs. TALO starts the `cgu-c` update when `enable` is set to `true`.
...
----
+
.Example blocking ClusterGroupUpgrade CRs after the `enable` field is set to `true`
+
[source,yaml]
----
apiVersion: ran.openshift.io/v1alpha1
