...
* Configure the `loadAffinity` object in the DPA spec by using one of the following methods:
** Global configuration: Configure the load affinity for all repositories as shown in the following example.
+
[source,yaml]
----
...
...
** Per-repository configuration: Configure the load affinity per repository as shown in the following example:
+
[source,yaml]
----
...
