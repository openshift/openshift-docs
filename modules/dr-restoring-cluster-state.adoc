...
.. If the `/etc/kubernetes/manifests/keepalived.yaml` file exists, complete the following steps. These steps are necessary to ensure that the API IP is listening on the recovery host.

... Move the `/etc/kubernetes/manifests/keepalived.yaml` file out of the kubelet manifest directory by entering the following command:
+
[source,terminal]
----
...
. Check the nodes to ensure they are in the `Ready` state. To check the nodes, you can use the bastion host or the recovery host.
...
** From the recovery host, run the following commands:
+
[source,terminal]
----
...
. If the `keepalived` daemon is in use, restore the configuration on the control plane nodes other than the recovery host by running the following command. Otherwise, the network operator will be stuck in the "Progressing" state.
+
[source,terminal]
----
