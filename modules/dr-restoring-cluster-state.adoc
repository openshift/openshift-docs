...
----
+
The output of this command should be empty. If it is not empty, wait for a few minutes and check again.
...
. If the `keepalived` daemon is in use, verify that the recovery control plane node owns the VIP by using the following:
+
[source,terminal]
----
$ ip -o address | grep <api_vip>
