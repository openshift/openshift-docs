...
* Access to the cluster as a user with the `cluster-admin` role through a certificate-based `kubeconfig` file, such as the one that was used during installation.
...
+
`kube-apiserver` becomes inaccessible after the restore process starts, so you cannot access the control plane nodes. For this reason, establish SSH connectivity to each control plane host in a separate terminal.
+
[IMPORTANT]
====
