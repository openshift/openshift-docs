...
[IMPORTANT]
====
It's highly recommended to have the `$INSTALL_DIR/auth/kubeconfig`. The `system:admin` is a hardcoded `cluster-admin` user that uses TLS client cert. It even works when there's no oauth pod (login service) working.
If you have lost the installation kubeconfig, see the following Red Hat KCS describing the procedure to create one:
link:++https://access.redhat.com/solutions/5286371++[https://access.redhat.com/solutions/5286371]
====
...
.. From a terminal that has access to the cluster as a `cluster-admin` user, verify that the etcd Pod is running.
+
[source,terminal]
----
