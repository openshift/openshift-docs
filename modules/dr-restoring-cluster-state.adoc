...
.. Verify that the `etcd` pods are stopped by running the following command:
+
[source,terminal]
----
...
.. Move the existing `kube-apiserver` file out of the kubelet manifest directory by running the following command:
+
[source,terminal]
----
...
.. Verify that the `kube-apiserver` containers are stopped by running the following command:
+
[source,terminal]
----
...
.. Move the existing `kube-controller-manager` file out of the kubelet manifest directory by running the following command:
+
[source,terminal]
----
...
.. Verify that the `kube-controller-manager` containers are stopped by running the following command:
+
[source,terminal]
----
...
.. Move the existing `kube-scheduler` file out of the kubelet manifest directory by running the following command:
+
[source,terminal]
----
...
.. Verify that the `kube-scheduler` containers are stopped by running the following command:
+
[source,terminal]
----
...
.. If the `/etc/kubernetes/manifests/keepalived.yaml` file exists, complete the following steps. These steps are necessary to ensure that the API IP address is listening on the recovery host.

... Move the `/etc/kubernetes/manifests/keepalived.yaml` file out of the kubelet manifest directory by running the following command:
+
[source,terminal]
----
...
. If the `keepalived` daemon is in use, verify that the recovery control plane node owns the VIP. Otherwise, repeat step 4.xi.
+
[source,terminal]
----
...
. Check the nodes to ensure they are in the `Ready` state. To check the nodes, you can use either the bastion host or the recovery host.
+
...
.. For user-provisioned installations, approve each valid kubelet service CSR by running the following command:
+
[source,terminal]
----
$ oc adm certificate approve <csr_name>
...
.. From the recovery host, verify that the `etcd` container is running by entering the following command:
+
[source,terminal]
----
$ sudo crictl ps | grep etcd | egrep -v "operator|etcd-guard"
...
.. From the recovery host, verify that the `etcd` pod is running by entering the following command:
+
[source,terminal]
----
...
. If not yet defined, in a separate terminal window within the recovery host, export the recovery `kubeconfig` file by running the following command:
+
[source,terminal]
----
$ export KUBECONFIG=/etc/kubernetes/static-pod-resources/kube-apiserver-certs/secrets/node-kubeconfigs/localhost-recovery.kubeconfig
...
.. Force a new rollout for the `kube-scheduler` by running the following command:
+
[source,terminal]
----
...
. Restart all the worker nodes in a rolling fashion.
