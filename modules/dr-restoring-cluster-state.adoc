...
+
The Kubernetes API server becomes inaccessible after the restore process starts, so you cannot access the control plane nodes. For this reason, establish SSH connectivity to each control plane host in a separate terminal.
+
[IMPORTANT]
====
