...
[role="_abstract"]
You can configure {aws-first} machine sets with existing placement groups. Controlling the physical strategy of node deployment helps ensure high availability and optimized network performance for critical control plane workloads.

You can configure a machine set to deploy machines on link:https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/efa.html[Elastic Fabric Adapter] (EFA) instances within an existing {aws-short} placement group.
