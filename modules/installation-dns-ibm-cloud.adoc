...
[role="_abstract"]
The installation program uses {ibm-cloud-name} DNS Services to configure cluster DNS resolution and provide name lookup for a private cluster. {ibm-cloud-name} does not support IPv6, so dual stack or IPv6 environments are not possible.
...
[NOTE]
====
Specify the fully qualified zone name. You can use either the root domain or subdomain value as the zone name, depending on which you plan to configure. A root domain uses the form `openshiftcorp.com`. A subdomain uses the form `clusters.openshiftcorp.com`.
====
