...
[role="_abstract"]
You can run a custom {insights-operator} gather operation on-demand from the {product-title} web console. 
...
. On the *CustomResourceDefinitions* page, in the *Search by name* field, find and then click the *DataGather* resource definition.
...
. On the console, select *Workloads* > *Pods*.
