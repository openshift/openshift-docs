...
You can create custom alert rules for network observability for your specific needs. The following example creates an alert that triggers if the current ingress traffic from `openshift-ingress` to a namespace has more than doubled (a 100% increase) compared to the previous day. It uses a similar `PromQL` query as provided in the example. For more information, see "Example of a base query for an alert in a surge in incoming traffic".

.Prerequisites

* You are familiar with `PromQL`.
* You have instsalled {product-title} 4.14 or later.
