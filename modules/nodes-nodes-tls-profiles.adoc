...
When defining a TLS security profile for the kubelet HTTP server, you use the `tlsSecurityProfile` parameter in a `KubeletConfig` custom resource (CR) to specify a predefined profile or define a custom profile for specific nodes.

The default TLS security profile is the predefined policy named `Intermediate`, described in this section.
...
There are three TLS security profile types that you can specify:
...
--
+
.Sample `KubeletConfig` CR that uses the `Old` TLS security profile on worker nodes
[source,yaml]
----
...
* `Intermediate`. This is a predefined profile, which is the default TLS secirty profile for the kubelet. The profile is based on link:https://wiki.mozilla.org/Security/Server_Side_TLS#Recommended_configurations[recommended configurations].
+
...
--
+
.Sample `KubeletConfig` CR that uses the `Intermediate` TLS security profile on master nodes
[source,yaml]
----
apiVersion: machineconfiguration.openshift.io/v1
kind: KubeletConfig
...
* `Custom`. This profile allows you to define the TLS ciphers and versions to use.
+
[WARNING]
====
...
====
+
.Sample `KubeletConfig` CR that uses a `Custom` TLS security profile on worker nodes
[source,yaml]
----
apiVersion: machineconfiguration.openshift.io/v1
kind: KubeletConfig
