...
** `OVNObservability` allows you to verify expected network behavior. Supports the following network API: `NetworkPolicy`, `AdminNetworkPolicy`, `BaselineNetworkPolicy`, `UserDefinedNetwork` isolation, and multicast ACLs. When enabled you can view network events in the console. For more information, see xref:../
** `gcpLabelsTags`
** `vSphereStaticIPs`
