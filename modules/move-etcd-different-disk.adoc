...
NOTE: This procedure cannot move parts of the root filesystem, such as /var/, to another disk or partition on an installed node.
...
+
*  An XFS filesystem is created on the specified disk.
...
. Apply the modified version that removes the logic for creating and syncing of the device by entering the following command:
+
[source,terminal]
----
...
----
+
The earlier step prevents the nodes from rebooting.
