...
By default, when it removes a node, the cluster autoscaler cordons the node and drains the pods on the node, moving the pods to a new node. You can configure the cluster autoscaler to remove the nodes without the cordon by setting the `spec.scaleDown.cordonNodeBeforeTerminating` parameter to `disabled` in the `ClusterAutoscaler` CR. Disabling this parameter can result in faster node removal and might be appropriate in clusters that run only stateless workloads. It is not recommended to disable this parameter in production clusters of the risk of data loss, application errors, pods getting stuck in the terminating state, or other issues. 
