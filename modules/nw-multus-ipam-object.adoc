...
----
<1> An array that describes the IP addresses to assign to the network interface. The plug-in supports IPv4 and IPv6 IP addresses.

<2> An IP address and network prefix that you specify. For example, if you specify `10.10.21.10/24`, then the plug-in assigns the additional network an IP address of `10.10.21.10` and the netmask is `255.255.255.0`.

<3> The default gateway to route egress network traffic to.

<4> An array that describes the routes to configure inside the pod.
...
<8> An of array of one or more IP addresses to send DNS queries to.

<9> The default domain to append to a host name. For example, if you specify
`example.com` as the domain, a DNS lookup query for `example-host` is
