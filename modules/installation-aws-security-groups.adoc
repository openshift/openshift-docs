...
You must increase the quota of security groups per network interface to a number equal to `3 + the number of additional security groups`, because the installation program creates 3 security groups for the control plane nodes. The maximum allowed quota is 16 and the maximum number of user-specified security groups is 10.
====
