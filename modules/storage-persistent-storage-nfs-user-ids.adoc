...
`spec.containers`:: Pods contain a `securityContext` definition specific to each container and a pod's `securityContext` which applies to all containers defined in the pod.
`spec.securityContext.runAsUser`:: Specifies the user ID to run the container as. In this example, `65534` is the `nfsnobody` user.
