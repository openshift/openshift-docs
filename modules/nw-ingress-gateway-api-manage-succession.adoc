...
Starting in {product-title} 4.19, the Ingress Operator manages the lifecycle of any Gateway API custom resource definitions (CRDs). This means you cannot create, update, or delete any CRDs within the API groups grouped under Gateway API.

[IMPORTANT]
====
When upgrading your {product-title} 4.18 cluster to {product-title} 4.19, you might see a `Gateway API CRDs have been detected` message. A cluster administrator and you understand the implications, apply the `ack-4.18-gateway-api-management-in-4.19`acknowledgement. {product-title} 4.19 can then manage the Gateway API CRDs during the upgrade operation.
====

Updating from a version before {product-title} 4.19 where the API CRD management was not present requires replacing or removing any Gateway API CRDs. These CRDs must conform to the specific {product-title} specification that is required by the Ingress Operator. {product-title} version 4.19 requires Gateway API Standard version 1.2.1 CRDs.
