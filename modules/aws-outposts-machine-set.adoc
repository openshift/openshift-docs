...
<providerSpec>:: Specifies the provider specification section that includes values that must be configured for your Outpost.
--
...
<infrastructure_id>:: Specifies the cluster infrastructure ID.
<machine_set_name>:: Specifies the name of the compute machine set. The name is composed of the cluster infrastructure ID, the `outposts` role name, and the Outpost availability zone.
...
<instance_type>:: Specifies the AWS instance type. You must use an instance type that is configured in your Outpost.
<region>:: Specifies the AWS region in which the Outpost availability zone exists.
<availability_zone>:: Specifies the Outpost availability zone.
