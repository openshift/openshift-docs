...
[role="_abstract"]
By default, only the Identity and Access Management (IAM) role that created the cluster can access the cluster's API. If other IAM roles in your AWS account need access to the cluster API, you can grant them access by specifying additional allowed principals during cluster creation.
