...
* You used the `dnf install -y firewalld microshift container-tool` command to install the `container-tools` meta-package on the host. The meta-package contains all container tools, such as Podman, Buildah, and Skopeo for additional support and troubleshooting. These tools are required for obtaining assistance from Red&nb.Hat Support when you are installing the image.
