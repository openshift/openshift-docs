...
= Creating a primary cluster-scoped UserDefinedNetwork (UDN) by using the web console

You can connect multiple namespaces to the same primary UserDefinedNetwork (UDN) by creating a `ClusterUserDefinedNetwork` custom resource in the {product-title} web console.
