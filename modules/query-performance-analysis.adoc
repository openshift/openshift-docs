...
[role="_abstract"]
Learn how to analyze and optimize Loki query performance metrics.
...
|Check the average chunk size by dividing `total_bytes` value by `cache_chunk_req` value. The average represents the average uncompressed bytes per chunk. The value for best performance should be in the order of magnitude of megabytes. If the chunks are only a few hundred bytes or KB in size, revisit labels to ensure that you are not splitting your data into very small chunks.
