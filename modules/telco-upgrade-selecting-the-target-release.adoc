...
[id="telco-upgrade-selecting-the-target-release_{context}"]
...
link:https://github.com/openshift/cincinnati-graph-data/tree/master/channels[cincinnati-graph-data repository] to determine what release to upgrade to.

[id="telco-upgrade-determining-available-z-streams_{context}"]
== Determining what z-stream upgrades are available

Before you can upgrade to a new z-stream release, you need to know what versions are available.

[NOTE]
====
You do not need to change the channel when performing a z-stream upgrade.
====
...
[id="telco-upgrade-changing-channel-eus-to-eus_{context}"]
== Changing the channel for an EUS to EUS upgrade

You must change the channel to the required version for an EUS to EUS upgrade.
...
.Procedure
. Determine the currently configured upgrade channel:
+
[source,terminal]
----
...
. Change the channel to point to the new channel you want to upgrade to:
+
[source,terminal]
----
...
[id="telco-upgrade-changing-channel-early-eus-to-eus_{context}"]
=== Changing the channel for an early EUS to EUS upgrade

The upgrade path to a brand new release of {product-title} is not available in either the EUS channel or the stable channel until 45 to 90 days after the initial GA of a minor release.

To begin testing an upgrade to a new release, you can use the fast channel.
...
. Check the upgrade path from the new channel.
Run the following command:
+
[source,terminal]
----
...
. Follow the upgrade procedure to get to version 4.16 (<y+1> from version 4.15)
...
. When you get to the required `<y+1>` release, change your channel again, this time to `fast-<y+2>`.

. Follow the EUS upgrade procedure that will take you to the required `<y+2>` release.

[id="telco-upgrade-upgrading-y-stream_{context}"]
== Changing the channel for a y-stream upgrade

In a y-stream upgrade you change the channel to the next release channel.

[NOTE]
====
For production clusters it is recommended that you use the stable or EUS release channels.
====

.Procedure
. Change the upgrade channel:
+
[source,terminal]
----
