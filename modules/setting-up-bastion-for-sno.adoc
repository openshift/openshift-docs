...
:navtitle: Set up bastion for {sno} with {ibm-power-title}
[id="setting-up-bastion-for-sno_{context}"]
= Setting up bastion for {sno} with {ibm-power-title}

[role="_abstract"]
Before installing {sno} on {ibm-power-name}, you must set up bastion.

Setting up a bastion server for {sno} on {ibm-power-name} requires the configuration of the following services:
...
`bootstrapInPlace.installationDisk`:: Specifies the path to the installation disk drive, for example, `/dev/disk/by-id/wwn-0x64cd98f04fde100024684cf3034da5c2`.
