...
Where:
+
** `mirror.platform.channels.minVersion`, `mirror.platform.channels.maxVersion`: The platform versions match the versions passed to the tool.
** `mirror.operators.packages.name: advanced-cluster-management`, `mirror.operators.packages.name: multicluster-engine`: The versions of {rh-rhacm} and the {mce-short} match the versions passed to the tool.
** `mirror.operators.packages.name: local-storage-operator`, `mirror.operators.packages.name: ptp-operator`, `mirror.operators.packages.name: sriov-network-operator`, `mirror.operators.packages.name: cluster-logging`, `mirror.operators.packages.name: lvms-operator`, `mirror.operators.packages.name: amq7-interconnect-operator`, `mirror.operators.packages.name: bare-metal-event-relay`, `mirror.operators.packages.name: sriov-fec`: The CR contains all the specified DU Operators.
...
** `--img quay.io/custom/repository` is optional and defines the repository where you store your additional images. These images are downloaded and pre-cached on the disk.
** `--du-profile -s` specifies pre-caching the Operators included in the DU configuration.
** `--skip-imageset` allows you to download the images that you specified in your customized `ImageSetConfiguration` CR.
