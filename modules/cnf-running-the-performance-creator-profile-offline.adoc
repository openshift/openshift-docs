...
. Use Podman to authenticate to `quay.io/openshift-community` by running the following command:
endif::[]
+
[source,terminal]
----
...
ifdef::openshift-origin[]
You can optionally set a path for the Node Tuning Operator image using the `-p` option. If you do not set a path, the wrapper script uses the default image: `quay.io/openshift-community/openshift4/ose-cluster-node-tuning-rhel9-operator:v{product-version}`.
endif::[]
====
