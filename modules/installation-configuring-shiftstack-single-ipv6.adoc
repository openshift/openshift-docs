...
[NOTE]
====
You can also deploy a dual-stack {product-title} cluster on a single-stack IPv6 {rh-openstack} infrastructure. The {product-title} cluster offers IPv4 connectivity internally, even when the underlying {rh-openstack} network only has IPv6 subnets.
====
