...
After the {ServerlessOperatorName} is upgraded to version 1.36.0, a new ReplicaSet for the Job Service component is created automatically. To clean up older resources, you can delete the Job Service ReplicaSets created by the previous 1.35.0 version.

.Procedure

. List all ReplicaSets for the Job Service by running the following command:
+
[source,terminal]
----
...
. Identify and delete the old ReplicaSet associated with version 1.35.0 by running the following command:
+
[source,terminal]
----
