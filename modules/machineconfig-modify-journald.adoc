...
If you need to configure setting for the `journald` service on {product-title}
...
. Create the MachineConfig, including the encoded contents of `journald.comf` (`jrnl_cnf` variable):
+
[source,terminal]
----
