...
<12> This secret must contain the Azure Storage Account name and key. The secret should be created in the namespace where the volume will be consumed, in other words, the namespace for the PVC.
...
pvc-name   Bound     pv-azurefile   5Gi        ReadWriteMany                 7m2s
----
