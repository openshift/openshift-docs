...
= Prepare an Agent-based cluster deployment for the {mce} in a connected environment

[role="_abstract"]
Create the required manifests to deploy the {mce-short} and the Local Storage Operator (LSO), enabling your agent-based {product-title} cluster to function as a hub cluster for managing spoke clusters.
...
* link:https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/latest/html/clusters/cluster_mce_overview[Cluster lifecycle with multicluster engine operator overview]
