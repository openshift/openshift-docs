...
|===
|Command |Description
|`virtctl addvolume <vm_name> --volume-name=<datavolume_or_PVC> [--bus=<bus_type>] [--persist] [--serial=<label>]`
|Hot plug a data volume or persistent volume claim (PVC).

Optional:

* `--bus=<bus_type>` specifies the bus type of the added disk. Supported values are `virtio` and `scsi`. The default bus type is `virtio`.
