...
You can configure the chunking and flushing behavior using the following parameters in the Cluster Logging Custom Resource (CR). The parameters are then automatically added to the Fluentd ConfigMap for use by Fluentd.
...
. Verify that the Flunentd Pods are redeployed:
+
[source,terminal]
----
...
. Check that the new values are in the `fluentd` ConfigMap:
+
[source,terminal]
----
