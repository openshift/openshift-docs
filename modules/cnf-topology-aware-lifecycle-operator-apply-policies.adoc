...
[source,terminal]
----
$ oc get cg oc get cgu -n default cgu -ojson | jq '.status'
----
+
.Example output
+
...
. If one of the managed policies is of `ClusterVersion` type, check the status of platform updates in the current batch by running the following command:
+
[source,terminal]
----
