...
`containerName`:: The `helloworld` name refers to `spec.template.spec.containers[0].name`.
`command`:: This `command` overrides any `ENTRYPOINT` defined by the `openshift/origin-ruby-sample` image.
`env`:: `env` is an optional set of environment variables for the hook container.
`data`:: `volumes` is an optional set of volume references for the hook container.
