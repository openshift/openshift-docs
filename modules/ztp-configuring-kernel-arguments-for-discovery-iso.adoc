...
+
--
* `operation: append` specifies the append operation to add a kernel argument.
* `value` specifies the kernel argument to configure. This example configures the `audit` kernel argument and the `trace` kernel argument.
--
...
* `custom-infraenv-cm` specifies the name of the `ConfigMap` that contains the custom `InfraEnv` CR.
...
+
--
* `custom-infraenv-cm` specifies the reference to the `ConfigMap` CR that contains the custom `InfraEnv` CR template.
--
