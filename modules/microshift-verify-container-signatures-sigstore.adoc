...
. Configure Red Hat remote registries to use sigstore attachments when pulling images to the local storage, by editing the `/etc/containers/registries.d/quay.io/openshift-community.yaml` file to contain the following configuration:
endif::[]
+
[source,terminal]
----
