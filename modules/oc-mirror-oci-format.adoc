...
The following example image set configuration mirrors an OCI catalog on disk along with an {product-title} release and a UBI image from `quay.io/openshift-community`.
endif::[]
+
...
ifdef::openshift-origin[]
<2> Optionally, include an {product-title} release to mirror from `quay.io/openshift-community`.
endif::[]
