...
<replicas>:: Specifies the number of compute machine replicas. Set to `0`.
+
...
<cloud_name>:: Specifies the Azure Stack Hub environment, which is used to configure the Azure SDK with the appropriate Azure API endpoints. For example: `AzureStackCloud`.
<azurestack_region>:: Specifies the name of your Azure Stack Hub region.
endif::ash[]
