...
[id="roxctl-deployment-check-options_{context}"]
= Options
...
|Define the policy categories to run. By default, the command runs all policy categories.
...
|`-n`, `--namespace string`
|Specify a namespace to enhance deployments with context information such as network policies, role-based access controls (RBACs), and services for deployments that do not have a namespace in their specification. The namespace defined in the specification is not changed. The default value is `default`.
