...
. Optional: If your cluster uses static routes or routing policies in the host network, set `routingViaHost` spec to `true` and `ipForwarding` to `Global` in the `gatewayConfig` annotation during migration.
+
[source,terminal]
----
...
.. Verify that `ipForwarding` has been set to `Global` and `routingViaHost` to `true` by running the following command:
+
[source,terminal]
----
