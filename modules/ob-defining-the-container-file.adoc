...
[role="_abstract"]
Use the `spec.paramValues` field to specify the image that contains the tools to build the output image in your `Build` CR. The following example specifies a `Dockerfile` image in a `Build` CR:
