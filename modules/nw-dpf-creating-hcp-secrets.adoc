...
. Ensure you have set the hosted cluster provisioning environment variables as described in "Hosted cluster provisioning environment variables".
...
[NOTE]
====
The `id_rsa.pub` secret data key is a fixed name that the provisioner expects; it does not require an RSA key. The `SSH_KEY` variable can point to any supported public key file, such as an Ed25519 key.
====
+
.Example output
[source,terminal]
