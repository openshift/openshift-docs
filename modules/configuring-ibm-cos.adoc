...
You create an {ibm-cloud-object-storage} instance to store the {oadp-short} backup data. After you create the COS instance, you configure the COS instance with `HMAC` service credentials.
...
. Install the {ibm-cloud-object-storage} plugin by running the following command. For more information, refer link:https://cloud.ibm.com/docs/cli?topic=cli-plug-ins[{ibm-cloud-title} plugin].
+
[source,terminal]
----
...
. Verify that the target resource group is correctly set, by running the following command. In the example output, the resource group is set to `Default`.
+
[source,terminal]
----
$ ibmcloud target
...
. Create an {ibm-cloud-title} resource `service-instance` by running the following command. For more details, refer link:https://cloud.ibm.com/docs/cloud-object-storage?topic=cloud-object-storage-provision[Choosing a plan and creating an instance].
+
[source,terminal]
----
...
----
<1> `cloud-object-storage` is the service name.
