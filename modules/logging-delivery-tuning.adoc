...
= Tune log payloads and delivery
      ...
      tuning:
        container:
          ...
          rateLimitPerContainer:
            maxRecordsPerSecond: 1000
----
`maxMessageSize`:: Specifies the maximum message length in bytes that a single log event can be when all partial log lines merge. Messages exceeding this limit are dropped. You can specify this value as an integer or as a string with units such as `Ki` or `Mi`.
