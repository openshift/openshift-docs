...
Be careful when setting this value, setting it improperly can cause an upgrade to fail.
...
[NOTE]
====
This does not mean that your deployment runs on only 2 pods for a period of time, for example, during an upgrade.
The cluster scheduler creates 2 new pods to replace the 2 older pods.
However, there will be a short period of time between the new pods coming online and the old pods being deleted.
====
