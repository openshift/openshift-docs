...
====
+
If your `noproxy` field includes subdomains or CIDR blocks, you must also explicitly specify the IP addresses you want to exclude in order to correctly route the request. This is because the cluster will not wait for DNS to return the IP address before assigning the route connection.
