...
[id="compliance-operator-1-0-0-new-features-and-enhancements_{context}"]
...
[id="compliance-operator-1-0-0-bug-fixes_{context}"]
== Bug fixes
...
* Before this update, the `ocp4-api-server-audit-log-maxsize` rule would result in a `FAIL` state. With this update, the error message has been removed from the metric, decreasing the cardinality of the metric consistent with best practices. (link:https://issues.redhat.com/browse/OCPBUGS-7520[*OCPBUGS-7520*])
