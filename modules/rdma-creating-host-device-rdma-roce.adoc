...
. Create the `NicClusterPolicy` CR on the cluster by running the following command: 
+
[source,terminal]
----
...
. Verify that the host device `NicClusterPolicy` CR by running the following command in the DOCA/MOFED container:
+
[source,terminal]
----
