...
This procedure describes how to create the workload pods for a host device RDMA for the NVIDIA Network Operator and how to test the pod configuration.
...
. Create the `NicClusterPolicy` CR on the cluster, using the following command: 
+
[source,terminal]
----
...
. Validate the host device `NicClusterPolicy` by running the following command in the DOCA/MOFED container:
+
[source,terminal]
----
...
. Confirm that the resources show up in the cluster `oc describe node` section using the following command:
+
[source,terminal]
----
...
. Create a `HostDeviceNetwork` custom resource file, as shown below:
+
[source,yaml]
----
...
. Create the `HostDeviceNetwork` resource on the cluster, using the following command:
+
[source,terminal]
----
