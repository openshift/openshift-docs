...
You can install the Principal component on the hub cluster by using the Argo CD custom resource (CR) to manage communication with connected Agent components. Deploy the Principal component on the hub cluster to create the required secrets and then deploy the Principal component on the hub cluster.
...
. The {gitops-title} Operator supports the deployment of the Principal component after the required secrets are created. After the required secrets are created, verify that the Principal component is enabled by running the following command.
+
[source,terminal]
----
$ oc get pod -n "<PRINCIPAL_NAMESPACE>" --context "<PRINCIPAL_CONTEXT>"
