...
However, when you enable destination-based mapping, managed applications can reside in any namespace on the control plane. A dedicated namespace for managed agents is optional when destination-based mapping is enabled. The principal uses the application's `spec.destination.name` field to determine the target agent. You must still create dedicated namespaces for autonomous agents because destination-based mapping only applies to managed agents.
====
