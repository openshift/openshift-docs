...
[NOTE]
====
The worker deployments set `replicas: 1` for a single DPU worker node.
Set the replica count of the `traffic-test-worker` and `traffic-test-worker-hostnetwork` deployments to the number of DPU-enabled worker nodes so that the `topologySpreadConstraints` place one pod on each node.
====
