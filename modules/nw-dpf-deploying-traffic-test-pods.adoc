...
* The DPF Operator is installed and the DPU hosted cluster is provisioned.
...
* A `RoleBinding` that grants the `default` service account in the `workload` namespace access to the `privileged` security context constraint.
