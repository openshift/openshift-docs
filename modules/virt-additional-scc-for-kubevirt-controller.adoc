...
[NOTE]
====
By default, `virt-launcher` pods run with the `default` service account in the namespace. If your compliance controls require a unique service account, assign one to the `VirtualMachine`. The setting applies to the `VirtualMachineInstance` and the `virt-launcher` pod.
====
