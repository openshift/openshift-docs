...
** `PKI` Indicates that the policy is based on a Bring Your Own Private Key Infrastructure. You must specify a base64-encoded PEM format public key. The `ClusterImagePolicy` object injects the BYOPKI configuration into the `policy.json` file, which can be used for image verification with BYOPKI.
+
