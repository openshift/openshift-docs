...
* `rootOfTrust`: Specifies the root of trust for the policy. Configure either a public key, a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate], or a Bring Your Own Public Key Infrastructure (BYOPKI).
...
** `PKI` Indicates that the policy is based on a bring your own private key infrastructure (BYOPKI). You must specify a base64-encoded PEM format public key. The `ClusterImagePolicy` object injects the BYOPKI configuration into the `policy.json` file, which can be used for image verification with BYOPKI.
+
--
:FeatureName: sigtore BYOPKI support
