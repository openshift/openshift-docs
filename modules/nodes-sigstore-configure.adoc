...
* `rootOfTrust`: Specifies the root of trust for the policy. Configure either a public key, a bring your own public key infrastructure (BYOPKI), or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certificate].
...
** `PKI` Indicates that the policy is based on a Bring-your-own PKI (BYOPKI). You must specify a base64-encoded PEM format public key. BYOPKI enables you to validate container images using an existing X.509 certificate while aligning with Cosign's bring-your-own PKI signing workflow.
+
--
