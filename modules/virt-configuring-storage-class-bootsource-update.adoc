...
<3>  Required: For the custom image to be detected as an available boot source, the value of the `managedDataSource` field must match the value of the `spec.dataVolumeTemplates.spec.sourceRef.name` field in the VM template YAML file.
