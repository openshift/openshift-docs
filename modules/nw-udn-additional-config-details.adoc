...
|N/A
|object
|When the `mode` field is set to `access` VLAN `id` (VID) is set for all traffic associated with this topology. The `id` specifies the VID for access ports. Values must be an integer between 1 and 4094.
|====

where:

`<topology>`:: Is one of `layer2` or `layer3` for the `UserDefinedNetwork` CR. For the `ClusterUserDefinedNetwork` the topology can also be `localnet`.
