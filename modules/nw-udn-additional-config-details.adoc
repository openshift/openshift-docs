...
|`spec.execludeSubnets`
|object
|The `excludeSubnets` field is optional and defines IP ranges that you want to exclude from the network. Setting these prevents OVN-Kubernetes from automatically assigning these. You can be useful if you have IP ranges in the broader subnets field that already in use or you want to reserve for a different purpose.
...
|`spec.vlan`
|object
|The `spec.vlan` field is optional and configures the virtual local area network (VLAN) tagging and allows you to segment the physical network into multiple independent broadcast domains.

|`spec.vlan.mode`
|object
|When the `mode` field is set to `access` a vlan `id` is set for all traffic associated with this topology.
