...
Hook that retrieves the k8s resources along with their status for loaded and error. It returns a map where keys are as provided in initResouces and value has three properties data, loaded and error.
...
|`options` |Monaco editor options. For more details, see link:https://microsoft.github.io/monaco-editor/docs.html#interfaces/editor.IStandaloneEditorConstructionOptions.html[Interface IStandAloneEditorConstructionOptions].
