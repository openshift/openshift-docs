...
[discrete]
== `useModal`
//move this section to depricated and add entry for useModal.
...
Deprecated: A component for making virtualized tables.
...
Deprecated: Component that generates filter for list page.
...
Deprecated: A hook that manages filter state for the ListPageFilter component. It returns a tuple containing the data filtered by all static filters, the data filtered by all static and row filters, and a callback that updates rowFilters.
