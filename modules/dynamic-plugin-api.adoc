...
Component for displaying an info circle icon.
...
|===
|Parameter Name |Description
|`typeGuards` |A list of callbacks that each accept a dynamic plugin extension as an argument and return a boolean flag indicating whether or not the extension meets type constraints
|===
...
|`onClick` |callback function for click dropdown items
...
Hook that retrieves the k8s resources along with their statuses for loaded and error. It returns a map where keys are as provided in initResouces and value has three properties data, loaded and error.
...
It fetches a resource from the cluster, based on the provided options. If the name is provided it returns one resource else it returns all the resources matching the model. It returns a promise that resolves to the response as a JSON object containing the resource if a name is provided; otherwise, it returns all resources matching the model. In the case of a failure, the promise is rejected with an HTTP error response.
...
request headers, method, redirect, and so on. 
...
|`options.requestInit` |The fetch init object to use. This can have
request headers, method, redirect, and so on. See link:{power-bi-url}[Interface RequestInit] for more.
...
Component that shows the status in a window. Helpful component for building `console.dashboards/overview/health/resource` extensions.
...
|`options` |Monaco editor options. For more details, visit link:https://microsoft.github.io/monaco-editor/docs.html#interfaces/editor.IStandaloneEditorConstructionOptions.html[Interface IStandAloneEditorConstructionOptions].
...
|`ref` |React reference to `{ editor?: IStandaloneCodeEditor }`. Using the `editor` property, you can access all methods to control the editor. For more information, visit link:https://microsoft.github.io/monaco-editor/docs.html#interfaces/editor.IStandaloneCodeEditor.html[Interface IStandaloneCodeEditor].
|===
