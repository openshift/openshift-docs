...
This release of {SMProductName} addresses Common Vulnerabilities and Exposures (CVEs), contains bug fixes, and is supported on {product-title} 4.10 or later.
...
This release introduces a generally available version of cluster-wide deployment. A cluster-wide deployment contains a Service Mesh Control Plane that monitors resources for an entire cluster. The control plane uses a single query across all namespaces to monitor each Istio or Kubernetes resource that affects the mesh configuration. Reducing the number of queries the control plane performs in a cluster-wide deployment improves performance.

=== Discovery selectors Support
This release introduces a generally available version of the `meshConfig.discoverySelectors` field, which can be used in cluster-wide deployment to limit which services the Service Mesh Control Plane can discover.
...
=== Integrating cert-manager with {SMProductShortName}
Beginning with this release, {SMProductName} integrates with `cert-manager`. `cert-manager` adds certificates and certificate issuers as resource types in Kubernetes clusters, and simplifies the process of obtaining, renewing and using those certificates.

=== IPv6 Single Stack Support
:FeatureName: IPv6 Single Stack Support
include::snippets/developer-preview.adoc[]
This release introduces a Developer Preview version of IPv6 Single Stack Support, providing access to a broader range of IP addresses.

=== {product-title} Gateway API Support
:FeatureName: {product-title} Gateway API
include::snippets/technology-preview.adoc[]
This release introduces a Technology Preview version of {product-title} Gateway API Support. By default, the functionality is disabled.

==== Enabling {product-title} Gateway API
To enable the {product-title} Gateway API set the `enabled` field to `true` in the `techPreview.gatewayAPI` specification of the SMCP resource.
...
{SMProductShortName} control plane deployment is now supported and documented on OpenShift infrastructure nodes.

=== Integration with external authorization systems 
* This release introduces a generally available method of integrating with external authorization systems using the Authorization policy’s `custom` field. You can use `envoyExtAuthzHttp` to implement an authorization cache for the Envoy proxy.

* This release introduces a generally available version of the Prometheus extension provider. You can use OpenShift monitoring to obtain visual metrics of mesh traffic by setting the `extensionProviders` field to `prometheus` in the `spec.meshConfig` specification.
+
[source,yaml]
----
spec:
...
* HBONE for sidecars is an experimental feature that is not supported.
...
[NOTE]
====
This documentation only applies to control planes deployed using SMCP v2.3.
====
