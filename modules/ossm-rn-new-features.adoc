...
* Native sidecars is supported only on {ocp-product-title} 4.16.
+
...
//Gateway API Update for 2.6 https://issues.redhat.com/browse/OSSM-5854
=== Gateway API default setting change
//Jacek
This release introduces a generally available Kubernetes Gateway API that is enabled by default only for cluster-wide deployments.

Gateway API is now enabled by default if cluster-wide mode is enabled (`spec.mode: ClusterWide`)

To disable this feature, it can be done with environment variable:
...
When updating existing instances of the `ServiceMeshControlPlane` resource to {SMProductName} version 2.6, {JaegerShortNamee} remains enabled by default.
...
//Includes 2.6, 2.5.3, 2.4.z. 2.6 ends support for v2.3
