...
* Native sidecars, which you can enable by:
...
=== Gateway API enabled by default GA cluster-wide only

This release introduces a generally available Kubernetes Gateway API for cluster-wide only.

Gateway API is now enabled by default if cluster-wide mode is enabled (`spec.mode: ClusterWide`)

If you want to disable this feature, it can be done with environment variable:
