...
//Includes 2.6, 2.5.3, 2.4.z and 2.3.13

//OSSM 2.6 Release Day tentatively scheduled for Wednesday, July 10, 2024
//This release will include new features such as OTEL+Tempo, Gateway API, and perhaps some others. Previous Tempo content can be updated to reflect OTEL+Tempo integration. Waiting on Dev to provide updated content on what has changed for Gateway API since it was release as TP in 2.5.
...
This release ends maintenance support for OpenShift {SMProductShortName} version 2.3. If you are using OpenShift {SMProductShortName} version 2.3, you should update to a supported version.

//include::snippets/ossm-current-version-support-snippet.adoc[] --> uncomment once snippet merges/syncs which might be on 06/26/2024 since it was just merged late afternoon of 06/25/2024

//Check if this ends support for 2.3 --> maintenance support ends for 2.3 as listed here https://access.redhat.com/support/policy/updates/openshift_operators
...
//Component Versions will need to be updated, most likely the day before release day.
...
Service Mesh 2.6 is based on Istio 1.20, which brings in new features and product enhancements, including:

* Envoy sidecars now configures a startup probe by default, meaning that the pod's readiness probes will not start until the Envoy sidecar has started. It's described in more detail in the 1.20 upgrade notes.
* VirtualServices now support traffic mirroring to multiple destination hosts. This is described in the 1.20 blog, and change notes.

While {SMProductName} supports many Istio 1.20 features, the following exceptions should be noted:

//List what is NOT SUPPORTED
//Ambient mesh is still NOT SUPPORTED for OSSM 2.6
* Ambient mesh is not supported
* QuickAssist Technology (QAT) PrivateKeyProvider in Istio is not supported

//Update this with OTEL + Tempo info
=== {JaegerName} and {es-op} disabled by default
...
//Update for 2.6 https://issues.redhat.com/browse/OSSM-5854 waiting for "Release Note Text" field to contain a summary of what users need to know for 2.6 vs 2.5
...
// Release is scheduled for July 10, 2024.
//Includes 2.6, 2.5.3, 2.4.z and 2.3.z
...
// Release is scheduled for July 10, 2024.
//Includes 2.6, 2.5.3, 2.4.9 and 2.3.13
...
[id="new-features-ossm-2-3-13"]
== New Features {SMProductName} version 2.3.13

This release of {SMProductName} addresses Common Vulnerabilities and Exposures (CVEs), contains bug fixes, and is supported on {product-title} 4.12 and later.

=== Component versions for {SMProductName} version 2.3.13
