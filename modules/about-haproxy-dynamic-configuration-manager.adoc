...
With fork-and-reload, scaling an application to any number of replicas triggers an HAProxy configuration reload.
DCM handles the following scaling operations dynamically without triggering an HAProxy reload:

* Any scale-out operation
* Any scale-in operation

With Dynamic Configuration Manager (DCM), HAProxy still requires a reload for unsupported dynamic operations, including:
...
When DCM encounters an operation that it cannot handle dynamically, the router falls back to the standard fork-and-reload mechanism.
This mechanism writes a new `haproxy.config` file and reloads HAProxy, which forks a new process to handle incoming connections.
The Ingress Operator includes the `spec.tuningOptions.configurationManagement` API field for selecting between DCM and fork-and-reload to manage HAProxy configuration updates.
