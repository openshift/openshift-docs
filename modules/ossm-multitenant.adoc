...
{SMProductName} configures each member project to ensure network access between itself, the control plane, and other member projects. The exact configuration differs depending on how {product-title} OVN-Kubernetes network plugin is configured. See "About the OVN-Kuberentes network plugin".
