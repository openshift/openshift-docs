...
* Optional: Adjust what happens if the target VM is not ready. To do so, set the `targetReadinessPolicy` parameter in the `vmrestore` YAML configuration to one of the following values:
** `FailImmediate`` - The restoration process fails immediately if the VM is not ready. This is the default setting.
** `StopTarget` - If not ready, the VM is stopped.
** `WaitGracePeriod _<10>_` - The restoration process waits for a set amount of time, in minutes, for the VM to be ready. The default value is 5 minutes.
** `WaitEventually` - The restoration process waits indefinitely for the VM to be ready.
