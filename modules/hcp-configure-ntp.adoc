...
. Use Butane to generate a `MachineConfig` object file, `99-worker-chrony.yaml`, that contains a configuration to be sent to the nodes. Run the following command:
+
[source,terminal]
----
      ...
      config:
        ...
        storage:
          ...
          - contents:
            ...
            path: /example/path
----
<1> Replace `<namespace>` with the name of your namespace where you created the node pool, for example, `clusters`.
...
. Add the list of your NTP servers in the `infra-env.yaml` file, that defines the `InfraEnv` custom resource (CR). See the following example configuration:
+
...
* Verify that your host inventory is created by running the following command. Check the following fields in the command output:
+
