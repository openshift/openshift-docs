...
. On {rh-openstack-first}, complete the following steps to make the failover VIP addresses reachable on {rh-openstack} ports: 
+
.. Create a config map for allowed addresses:
+
[source,terminal]
----
$ oc create configmap <configmap_name>
----
.Example Config Map that specifies API and Ingress VIP addresses in the `allowed_address_pairs` parameter.
+
[source.json]
----
...
----
+
.. Verify that the failover VIP addresses are reachable on {rh-openstack} ports:
+
[source,terminal]
----
