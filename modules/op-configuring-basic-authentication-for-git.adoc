...
+
. Create the YAML manifest for the task run or pipeline run in the `run.yaml` file, associate the service account with the task run or pipeline run. Use one of the following examples:
+
...
+
. Apply the YAML manifests that you created.
+
[source,terminal]
----
