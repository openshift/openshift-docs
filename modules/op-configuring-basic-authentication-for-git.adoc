...
[id="op-configure-basic-authentication-for-git_{context}"]
= Configure basic authentication for Git
...
[NOTE]
====
GitHub deprecated authentication by using a plain password. Instead, use a personal access token.
====

.Procedure

. Create the YAML manifest for the secret in the `secret.yaml` file. In this manifest, specify the username and password or GitHub personal access token to access the target Git repository.
+
