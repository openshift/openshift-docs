...
+
. Create a YAML manifest for a task run or pipeline run as the `run.yaml` file. In this file, associate the service account with a task run or a pipeline run.
+
