...
= Share RHEL entitlements secrets across namespaces
...
`metadata.name`:: Specifies the name of the `SharedSecret` object.
...
`metadata.name`:: Specifies the name of the `ClusterRole` CR.
...
`metadata.name`:: Specifies the name of the `Role` CR.
--
+
...
`metadata.name`:: Specifies the name of the `RoleBinding` CR.
`subjects.namespace`:: Specifies the name of the namespace where `openshift-builds` is installed.
...
`metadata.name`:: Specifies the name of the `RoleBinding` CR for the `builder` and `pipeline` service accounts.
...
`volumes.csi.readOnly`:: Specifies the `readOnly` value to mount the shared resource in the build.
`volumes.csi.volumeAttributes.sharedSecret`:: Specifies the name of the `SharedSecret` object to include it in the build.
`volumes.output.image`:: Specifies the location where you want to push the built image.