...
You can use a `SharedSecret` object to securely share and synchronize the RHEL subscription keys of a cluster in {builds-shortname} across namespaces.
...
. Create a `SharedSecret` object instance with the subscription secret of a cluster by running the following command:
+
[source,terminal]
----
$ oc apply -f -<<EOF
