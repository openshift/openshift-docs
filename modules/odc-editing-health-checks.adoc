...
= Edit health checks using the Developer perspective
...
.. Modify the parameters as required, and click the checkmark to save your changes.
+
...
====
.. Click the checkmark at the bottom of the form. The *Liveness Probe Added* message is displayed.
...
. In the *Container Details* page, verify that the Liveness probe - `HTTP Get <pod_ip>:8080/` has been added to the container, in addition to the earlier existing probes.
