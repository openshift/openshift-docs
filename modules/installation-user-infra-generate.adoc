...
:navtitle: Create the installation files for {cp}
:chunk-to-content:
[id="installation-user-infra-generate_{context}"]
= Creating the installation files for {cp}

//[role="_abstract"]
//To install {product-title} on user-provisioned infrastructure, you generate and customize installation files so the cluster creates only the machines it uses.
