...
[role="_abstract"]
The Bare Metal Operator (BMO) manages and inspects bare-metal hosts by using the Ironic API service to interface with the Baseboard Management Controller. You can use the BMO to provision hosts, configure BIOS settings, and manage firmware updates to ensure your cluster hardware accurately matches your resource definitions.
