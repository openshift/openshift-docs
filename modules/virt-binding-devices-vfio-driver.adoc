...
[NOTE]
====
NVIDIA GPU is supported on `x86` and `aarch64` architectures and {ibm-name} Spyre is supported on `s390x` architecture.
====
+
[source,terminal]
----
...
----
Valid values for `<gpu_accelerator>` are `nvdia` and `spyre`.
+
Example output:
+
[source,terminal]
