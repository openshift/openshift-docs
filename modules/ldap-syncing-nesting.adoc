...
allowed groups can be synced when using this matching rule.
...
- The `oc adm groups sync` command must explicitly allow groups.
...
[NOTE]
====
You must explicitly allow the `cn=admins,ou=groups,dc=example,dc=com` group.
====
+
