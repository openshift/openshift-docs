...
----
    ...
    checkType: Platform # The target execution layer (Node or Platform)
    description: <description of the rule>
    id: xccdf_org.ssgproject.content_rule_configure_network_policies_namespaces # Unique upstream XCCDF identification name
    ...
    severity: high # The evaluation failure risk impact rating
    title: <summary of the rule>
----
* **checkType**: Specify the type of check this rule executes. `Node` profiles scan the cluster nodes and `Platform` profiles scan the Kubernetes platform. An empty value indicates there is no automated check.
* **id**: Specify the XCCDF name of the rule, which is parsed directly from the datastream.
* **severity**: Specify the severity of the rule when it fails.
