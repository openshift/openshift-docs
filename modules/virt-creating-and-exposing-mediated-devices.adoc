...
Both Telsa and Ampere series cards are supported.
...
+
For example, the name file for the `nvidia-745` type contains the selector string `NVIDIA A2-AQ`. Using `NVIDIA A2-AQ` as the `mdevNameSelector` value allows nodes to use the `nvidia-745` type.
+
The `mdevNameSelector` value for Tesla series cards is `GRID T4-1Q`.
...
# ...
  permittedHostDevices:
    mediatedDevices:
    - mdevNameSelector: NVIDIA A2-AQ <1>
      resourceName: nvidia.com/A2-AQ <2>
# ...
----
