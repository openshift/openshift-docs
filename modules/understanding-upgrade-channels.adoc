...
You can also configure an empty upgrade channel, which prevents the cluster's Cluster Version Operator from fetching available upgrades from the upgrade recommendation service.
For example, you might configure an empty upgrade channel because your cluster has restricted network access and cannot reach the upgrade recommendation services.
