...
CNFs are designed to run in cloud-native environments. They can provide network services such as routing,
firewalling, and load balancing. You can configure the node pool to use high-performance computing and networking devices to run CNFs.
...
----
+
IMPORTANT: If you do not already have environment variables set for the HyperShift Operator namespace, isolated and
reserved
CPUs, and huge pages count, create them prior to applying the performance profile.
...
--
// TODO: Since this is a module, I need to link in add'ls. Yeeaahh.
. Install the SR-IOV Network Operator in the hosted cluster by following the instructions in the OpenShift documentation:
+
