...
:navtitle: Remove a third-party vSphere CSI Driver Operator
[id="persistent-storage-csi-vsphere-install-issues_{context}"]
= Removing a third-party vSphere CSI Driver Operator

[role="_abstract"]
To enable cluster upgrades to {product-title} 4.13 or later, remove any third-party vSphere CSI drivers and allow the Red Hat-supported built-in driver to manage storage.
...
. Delete the third-party vSphere CSI driver `CSIDriver` object by running the following command:
+
[source,terminal]
----
