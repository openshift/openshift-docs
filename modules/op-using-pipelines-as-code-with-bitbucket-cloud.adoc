...
[id="use-pipelines-as-code-with-bitbucket-cloud_{context}"]
...
[NOTE]
====
* If you want to configure the webhook by using the `tkn pac` CLI, add the `Webhooks`: `Read` and `Write` permission to the token.
...
** To configure a webhook and create a `Repository` CR _automatically_ by using the `tkn pac` CLI tool, use the following command:
+
[source,terminal]
----
$ tkn pac create repo
