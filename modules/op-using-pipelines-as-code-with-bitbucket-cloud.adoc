...
.. Confirm that the webhook is displayed with the {pac} controller URL.
.. Check the webhook history for successful deliveries.

. Trigger a pipeline run by pushing a commit to your repository or creating a pull request.

. Verify that the pipeline status is reported back to Bitbucket Cloud in the pull request.
