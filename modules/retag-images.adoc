...
----
$ docker tag registry.redhat.io/advanced-cluster-security/rhacs-main-rhel9:{rhacs-version} <your_registry>/rhacs-main-rhel9:{rhacs-version}
----
and do not retag like the following example:
[source,terminal,subs=attributes+]
----
$ docker tag registry.redhat.io/advanced-cluster-security/rhacs-main-rhel9:{rhacs-version} <your_registry>/other-name:latest
----
