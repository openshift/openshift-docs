...
= Creating {gcp-full} resources with the Cloud Credential Operator utility

You can use the `ccoctl gcp create-all` command to automate the creation of {gcp-full} resources.
...
<2> Specify the {gcp-full} region in which cloud resources will be created.
<3> Specify the {gcp-full} project ID in which cloud resources will be created.
<4> Specify the directory containing the files of `CredentialsRequest` manifests to create {gcp-full} service accounts.
+
[NOTE]
====
If your cluster uses Technology Preview features that are enabled by the `TechPreviewNoUpgrade` feature set, you must include the `--enable-tech-preview` parameter.
...
----
+
You can verify that the IAM service accounts are created by querying GCP. For more information, refer to {gcp-full} documentation on listing IAM service accounts.
endif::google-cloud-platform[]
ifdef::azure-workload-id[]
+
.Example output
