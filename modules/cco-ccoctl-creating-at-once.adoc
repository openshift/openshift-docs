...
// Hosted control planes assemblies
...
ifdef::aws-sts,google-cloud-platform,azure-workload-id,hcp-cco[]
. Use the `ccoctl` tool to process all `CredentialsRequest` objects by running the following command:
+
endif::aws-sts,google-cloud-platform,azure-workload-id,hcp-cco[]
ifdef::aws-sts,hcp-cco[]
[source,terminal]
----
