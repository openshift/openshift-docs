...
* You configured the new management cluster to use {oadp-short}. The new management cluster must have the same Data Protection Application (DPA) as the management cluster that you backed up from so that the `Restore` CR can access the backup storage.
...
... In the management cluster that you backed up from, in the `ClusterDeployment` settings, set the `spec.preserveOnDelete` parameter to `true` by running the following command:
+
[source,terminal]
----
