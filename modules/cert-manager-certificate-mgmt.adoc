...
where:
+
`metadata.name`:: Specifies a name for the certificate.
`metadata.namespace`:: Specifies the namespace of the issuer.
`spec.commonName`:: Specifies the common name (CN).
`spec.secretName`:: Specifies the name of the secret to create that contains the certificate.
`spec.dnsNames`:: Specifies the domain name.
`spec.issuerRef.name`:: Specifies the name of the issuer.
