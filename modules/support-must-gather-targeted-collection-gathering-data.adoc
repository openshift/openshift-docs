...
* To compress all the uncompressed diagnostic logs that are less than 10MB, set the `REDUCE_LOGS` environment variable by running the following command:
+
[source,terminal]
----
...
* To compress service logs, set the `REDUCE_LOGS` environment variable by running the following command:
+
[source,terminal]
----
