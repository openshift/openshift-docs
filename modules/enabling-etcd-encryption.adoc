...
[role="_abstract"]
You can enable etcd encryption with AES-GCM or AES-CBC to protect sensitive API resources at rest. Verify that encryption completes before creating an etcd backup.

To protect sensitive API resources at rest in etcd, enable encryption on the API server and choose an AES-GCM or AES-CBC encryption type.
