...
spec.secrets:: Specifies any required secrets.
...
+
The following output shows the service account for an installed etcd Operator:
+
[source,terminal]
----
NAME            SECRETS   AGE
...
----
+
* `etcd-operator` is the service account for an installed etcd Operator.
