...
The following is a high-level overview of how the {zero-trust-full} works.
...
. After the agent gathers the information, the information is sent to the SPIRE server and the server checks its configured registration entries, which contain SPIFFE IDs.

. The agent receives the workload SVID and Trust Bundle and pass it on to the workload. The workload can now present their SVIDs to other SPIFFE-aware devices to communicate with them.
