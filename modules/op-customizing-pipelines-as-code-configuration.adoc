...
| `enable-cancel-in-progress-on-pull-requests` | Automatically cancels in-progress pipeline runs for a pull request when a new commit is pushed, ensuring the old pipeline run is cancelled only after the new one has successfully started. The `cancel-in-progress` annotation on individual runs still applies and overrides the global setting. | `false`

| `enable-cancel-in-progress-on-push` | Automatically cancels in-progress pipeline runs for a pull request when a new commit is pushed, ensuring the old pipeline run is cancelled only after the new one has successfully started. The `cancel-in-progress` annotation still applies and takes precedence. | `false`
