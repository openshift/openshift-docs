...
| `enable-cancel-in-progress-on-pull-requests` | Automatically cancels in-progress `PipelineRuns` for a pull request when a new commit is pushed. Ensures only the latest commit is processed. If disabled, the `cancel-in-progress` annotation on individual runs still applies and overrides the global setting. | `false`

| `enable-cancel-in-progress-on-push` | Automatically cancels in-progress `PipelineRuns` triggered by a push when a new push is made to the same branch. Ensures only the latest commit is processed. If disabled, the `cancel-in-progress` annotation still applies and takes precedence. | `false`
