...
:navtitle: Install the {FeatureName} CSI Driver Operator
[id="persistent-storage-csi-olm-operator-install_{context}"]
= Installing the {FeatureName} CSI Driver Operator

[role="_abstract"]
Install and configure the {FeatureName} Container Storage Interface (CSI) Driver Operator to enable {FeatureName} storage in your cluster. This Red{nbsp}Hat Operator is not installed by default and requires manual installation.
