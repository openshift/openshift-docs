...
* If you are using {FeatureName} with AWS Secure Token Service (STS), in the *role ARN* field, enter the ARN role.
+
[NOTE]
====
To find the role ARN, run the following command:

[source,terminal]
----
$ rosa describe cluster --cluster=<cluster_name> | grep 'Role (STS) ARN'
----
====
endif::[]
endif::restricted[]
