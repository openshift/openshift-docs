...
The following AWS regions are currently available for Red{nbsp}Hat OpenShift 4 and are supported for {product-title}.
...
[NOTE]
====
For GovCloud (US) regions, you must submit an link:https://console.redhat.com/openshift/create/rosa/govcloud[Access request for Red{nbsp}Hat OpenShift Service on AWS FedRAMP].
...
|Region
|Location
|Minimum {product-title} version required
|AWS opt-in required
...
Clusters can only be deployed in regions with at least three availability zones. For more information, see the link:https://aws.amazon.com/about-aws/global-infrastructure/regions_az/[Regions and Availability Zones] section in the AWS documentation.

Each new {product-title} cluster is installed within
ifdef::openshift-rosa-hcp[]
a
endif::openshift-rosa-hcp[]
ifndef::openshift-rosa-hcp[]
