...
[role="_abstract"]
You must configure an {product-title} provider for each cluster included in a migration, and each provider requires a certificate authority (CA) for the cluster. Configuring the root CA for the entire cluster is important to avoid CA expiration, which causes the provider to fail.
