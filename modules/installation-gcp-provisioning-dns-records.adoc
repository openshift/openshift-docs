...
= Optional: Provisioning your own DNS records

Before you use this feature, you must add the parameter `userProvisionedDNS` to the `install-config.yaml` file and enable the parameter. For more information, see "Enabling a user-managed DNS".

Use the IP address of the API server to provision your own DNS record with the hostname `api.<cluster_name>.<base_domain>.`, using your cluster name and base cluster domain. Use the IP address of the ingress service to provision your own DNS record with the hostname `*.apps.<cluster_name>.<base_domain>.`, using your cluster name and base cluster domain.
