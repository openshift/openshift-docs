...
. List the available presets by running the following command:
+
[source,terminal]
----
...
. Apply the preset that matches your scenario by running the following command:
+
[source,terminal]
----
...
. For the `tolerant-detection` preset, verify the Corosync token value by running the following command:
+
[source,terminal]
----
...
. For the `slow-bmc` preset, verify the fence agent timeouts by running the following command:
+
[source,terminal]
----
