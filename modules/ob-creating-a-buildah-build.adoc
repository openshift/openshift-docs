...
[role="_abstract"]
Use a `Buildah` build strategy when you want to build and push a container image using a Dockerfile.
...
`<project>`:: Defines the project name used when creating the `Build` resource.
`<image>`:: Defines the image name used when creating the `Build` resource.