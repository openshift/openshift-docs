...
[role="_abstract"]
Use a `Buildah` build when you want to build and push a container image using a Dockerfile.
...
`name`:: The `spec.build.name` field denotes the respective build to run, which is expected to be available in the same namespace.
