...
= Build an image by using a Dockerfile

[role="_abstract"]
Use a `buildah` build strategy to build and push a container image by using a Dockerfile.
...
spec:
  ...
  - name: dockerfile
    value: Dockerfile
  output:
    image: image-registry.openshift-image-registry.svc:5000/builds-sample/sample-go-app
EOF
----
+
--
...
--source-context-dir="buildah" \
--strategy-name="buildah" \
--dockerfile="Dockerfile" \
--output-image="image-registry.openshift-image-registry.svc:5000/builds-sample/go-app"
----
+
where:
...
`output-image`:: Defines the location where the built image is pushed. In this procedural example, the built image is pushed to the {ocp-product-title} cluster internal registry. `builds-sample` is the name of the current project. Ensure that the specified project exists to allow the image push.
...
[source,terminal]
----
$ podman manifest inspect image-registry.openshift-image-registry.svc:5000/builds-sample/sample-go-app
----
+
In the previous example, the project name is `builds-sample`, and the image name is `sample-go-app`.
+
Example output:
+
[source,terminal]
