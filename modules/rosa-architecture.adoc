...
[role="_abstract"]
In {product-title}, both the control plane and the worker nodes are deployed in your Virtual Private Cloud (VPC) subnets. This gives you full control over network access and traffic flow.
...
You can customize access patterns for your API server endpoint and Red{nbsp}Hat Site Reliability Engineering (SRE) management in the following ways:
...
* Private with AWS PrivateLink - API server endpoint and application routes are private. Public subnets or network address translation (NAT) gateways are not required in your VPC for egress. {product-title} SRE management uses AWS PrivateLink.
...
{product-title} clusters include infrastructure nodes where {OCP-short} components such as the ingress controller, image registry, and monitoring are deployed. The infrastructure nodes and the {OCP-short} components deployed on them are managed by {product-title} SREs.
