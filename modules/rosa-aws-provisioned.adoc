...
* *Subnets*: One subnet for a cluster with a single availability zone, or three subnets for a cluster with multiple availability zones.
* *Router tables*: One router table per private subnet, and one additional table per cluster.
* *Internet gateways*: Not required, although public egress is still required.
* *NAT gateways*: Not required.
