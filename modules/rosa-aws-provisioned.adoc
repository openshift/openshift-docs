...
This is an overview of the provisioned Amazon Web Services (AWS) components on a deployed {product-title} (ROSA) cluster.
//TODO OSDOCS-11789: Where in the OCP docs?? This link isn't useful
//For a more detailed listing of all provisioned AWS components, see the link:https://access.redhat.com/documentation/en-us/openshift_container_platform/[OpenShift Container Platform documentation].
...
At a minimum, the following EC2 instances will be deployed:
...
The instance type shown for worker nodes is the default value, but the instance type for worker nodes can be customized according to the needs of your workload.
...
//TODO OSDOCS-11789: The default is 300GB but the HCP minimum is 75GB. Need to confirm if this is also true for Classic.
* Worker Volume
** Size: 300GB
** Type: gp3
** Input/Output Operations Per Second: 900
...
ifdef::openshift-rosa-hcp[]
By default, one Network Load Balancer is created for use by the default ingress controller. You can create additional load balancers (Classic Load Balancers, Network Load Balancers, or Application Load Balancers) as required by your workload.
endif::openshift-rosa-hcp[]
...
Your VPC must be configured according to the following requirements:
