...
ifdef::ibm-z[To reach the minimum required DASD size for RHCOS installations, you need extended address volumes (EAV).]
If available, use HyperPAV to ensure optimal performance.
...
* Hipersockets, which are attached to a node either directly as a device or by bridging with one zVM vSwitch to be transparent to the z/VM guest. To directly connect Hipersockets to a node you must set up a gateway to the external network via a RHEL 8 guest to bridge to the Hipersockets network. See the "Additional resources" section.
