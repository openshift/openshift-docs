...
* FICON attached disk storage (DASDs). These can be z/VM minidisks, fullpack minidisks, or dedicated DASDs. ifdef::ibm-power[To reach the minimum required DASD size for RHCOS installations, you need extended address volumes (EAV). ]If available, use HyperPAV to ensure optimal performance.
...
* 16 GB for {product-title} control plane machines
ifndef::ibm-z[* 8 GB for {product-title} compute machines]
ifdef::ibm-z[* 16 GB for {product-title} compute machines]
* 16 GB for the temporary {product-title} bootstrap machine
endif::ibm-z[]
