...
[role="_abstract"]
To complete cluster initialization on Microsoft {cp}, you can wait for the bootstrap process to finish and then delete bootstrap resources.
...
`--log-level info`:: Specifies the installation details. Specify `warn`, `debug`, or `error` instead of `info` to view different installation details.
+
