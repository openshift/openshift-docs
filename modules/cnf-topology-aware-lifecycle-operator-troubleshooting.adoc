...
= Troubleshooting the Topology Aware Lifecycle Operator

The Topology Aware Lifecycle Operator is an {product-title} Operator that remediates {rh-rhacm} policies. When issues occur, use the `oc adm must-gather` command to gather details, logs and take steps in debugging the issues.
...
* link:https://docs.openshift.com/container-platform/latest/support/troubleshooting/troubleshooting-operator-issues.html[OpenShift Container Platform Troubleshooting Operator Issues]

[id="talo-general-troubleshooting_{context}"]
== General troubleshooting

You can determine the cause of the problem by going through the following questions:

* Is the configuration that you are applying supported? 
** Are the {rh-rhacm} and the {product-title} versions compatible? 
** Are the TALO and {rh-rhacm} versions compatible?
...
. Create the `ClusterGroupUpgrade` CR with `spec.enable` set to `false`. 

. Wait for the status to be updated and go through the troubleshooting questions. 

. If everything looks as expected, set `spec.enable` to `true` in the `ClusterGroupUpgrade` CR.

[WARNING]
====
After you set the `spec.enable` to `true` in the `ClusterUpgradeGroup` CR , the update procedure starts and you cannot edit the CR.
====
...
Resolution:: You must restart the procedure by doing the following:
+
. Remove the old `ClusterGroupUpgrade` CR by running the following command: 
+
[source,terminal]
----
$ oc delete cgu -n <ClusterGroupUpgradeCR-namespace> <ClusterGroupUpgradeCR-name>
----
+
. Check and fix the existing issues with the managed clusters and policies.
.. Ensure that all the clusters are managed cluster and available.
.. Ensure that all the policies exist and have the `spec.remediationAction` set to `inform`.
+
. Create a new `ClusterGroupUpgrade` CR with the correct configurations.
+
[source,terminal]
----
$ oc apply -f <ClusterGroupUpgradeCR-YAML>
----
...
Resolution:: You can run the following command:
+
[source,terminal]
----
...
Issue:: You want to check if the `remediationAction` field is set to `inform` in `spec` of managed policies.
...
. Alternatively, check the TALO manager logs:

.. Get the name of the TALO manager by running the following command:
+
[source,terminal]
----
...
.. Check the TALO manager logs by running the following command:
+
[source,terminal]
----
...
Issue:: You want to check if the `clusterSelector` is specified in the `ClusterGroupUpgrade` CR in at least one of the managed clusters.
...
Resolution:: You can run the following commands:
+
[source,terminal]
----
...
[NOTE]
====
A cluster can be present in both `spec.clusters` and to be matched by the `spec.clusterSelecter` label.
====

[id="talo-troubleshooting-remediation-strategy_{context}"]
== Remediation Startegy

[discrete]
=== Checking if remediationStartegy is present in the ClusterGroupUpgrade CR

Issue:: You want to check if the `remediationStartegy` is present in the `ClusterGroupUpgrade` CR.
...
[source,json]
----
map[canaries:[spoke3] maxConcurrency:2 timeout:240]
----
...
Issue:: You want to check the value of `status.conditions` in the `ClusterGroupUpgrade` CR.
...
[source,json]
----
[map[lastTransitionTime:2022-02-17T22:25:28Z message:The ClusterGroupUpgrade CR has managed policies that are missing: [policyThatDoesntExist] reason:UpgradeCannotStart status:False type:Ready]]
----
...
[discrete]
=== Checking if status.remediationPlan computed
...
[discrete]
=== Errors in TALO manager container

Issue:: You want to the logs of the manager container of TALO.
