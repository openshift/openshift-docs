...
. To confirm that route advertisements are enabled, check the `RouteAdvertisements` feature gate:
+
[source,terminal]
----
...
. Enable the Cluster Network Operator (CNO) and wait for all nodes to come up:
+
[source,terminal]
----
...
. To get the IP addresses of the nodes, enter the following command:
+
[source,terminal]
----
$ oc get node -owide
...
. To get the default pod network of each node, enter the following command:
+
[source,terminal]
----
...
. On the bare metal hypervisor, get the IP to be used by the external FRR container:
+
[source,terminal]
----
