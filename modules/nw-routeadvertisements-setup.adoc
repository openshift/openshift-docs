...
As a cluster administrator, you can enable Border Gateway Protocol (BGP) routing support for your cluster. This configuration is intended as a sample that demonstrates how to configure route advertisements.
...
* A bare metal system with access to the cluster on which to run the FRR daemon container.

.Procedure

. Confirm that the `RouteAdvertisements` feature gate is enabled:
+
[source,terminal]
----
...
. Enable the Cluster Network Operator (CNO):
+
[source,terminal]
----
...
----
+
It may take a few minutes for the CNO to restart all nodes.
...
. On the bare metal hypervisor, get the IP address for the external FRR container to use:
+
[source,terminal]
----
...
. Create a `frr.conf` file for FRR with each node's IP address:
+
...
. Create an external FRR container:
+
[source,terminal]
----
...
.. Create a `receive_all.yaml` file with the following content:
+
...
.. Create a `ra.yaml` file with the following content:
+
...
. Apply the `receive_all.yaml` and `ra.yaml` files:
+
[source,terminal]
----
$ oc apply -f receive_all.yaml
$ oc apply -f ra.yaml
----

. Verify that the configurations were applied:
.. Verify that the `FRRConfiguration` configurations were created:
+
[source,terminal]
----
...
.. Verify that the `RouteAdvertisements` configurations were created:
+
[source,terminal]
----
$ oc get ra -A
...
. Get the external FRR container ID:
+
[source,terminal]
----
...
. Check BGP neighbor and routes in the external FRR container's vtysh session with the container ID obtained in the previous step:
+
[source,terminal]
----
...
. Find the frr-k8s pod for each cluster node:
+
[source,terminal]
----
...
. From the {product-title} cluster, check BGP routes on the cluster node's frr-k8s pod in the FRR container:
+
[source,terminal]
----
$ oc -n openshift-frr-k8s -c frr rsh node1.example.com
----

. Check the IP routes from the cluster node:
+
[source,terminal]
----
sh-5.1# vtysh
...
. From the {product-title} cluster, debug the node:
+
[source,terminal]
----
...
. Confirm that the BGP routes are being advertised
+
[source,terminal]
----
