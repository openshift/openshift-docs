...
+
--
* `leftcert` specifies the certificate to identify the IPsec endpoint. The value must match the name of the certificate used on the remote system.
* `<external_host>` specifies the external host IP address or DNS hostname.
* `<external_address>` specifies the IP subnet of the external host on the other side of the IPsec tunnel.
* `ikev2: insist` specifies to use the IKEv2 VPN encryption protocol only. Do not use IKEv1, which is deprecated.
--
...
+
--
* `99-ipsec-*` files are the Butane and endpoint configuration CRs generated by the `ipsec/import-certs.sh` script.
* `ca.pem` and `left_server.p12` are certificate files that you provide, relevant to your network.
--
...
+
--
* `namespace` specifies the namespace, which must match the `ClusterInstance` namespace.
...
+
--
* `ipsec-manifests-cm` specifies the reference to the `ConfigMap` containing the IPsec certificate import manifests.
