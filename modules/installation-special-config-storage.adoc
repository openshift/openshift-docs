...
layout:: Specifies the instruction set architecture of the cluster nodes. Some examples include, `x86_64`, `aarch64`, or `ppc64le`.
tpm2:: Include this field if you want to use a Trusted Platform Module (TPM) to encrypt the root file system.
tang:: Include this section if you want to use one or more Tang servers.
advertisement:: Optional: Include this field for offline provisioning. Ignition will provision the Tang server binding rather than fetching the advertisement from the server at runtime. This lets the server be unavailable at provisioning time.
threshold:: Specifies the minimum number of TPM v2 and Tang encryption conditions required for decryption to occur.
...
http://tang1.example.com:7500:: In this example, `tangd.socket` is listening on port `7500` on the Tang server.
+
[NOTE]
====
...
<installation_directory>:: Specifies the path to the directory that you want to store the installation files in.
...
worker:: For control plane configurations, replace `worker` with `master` in both of these locations.
layout:: Specifies the instruction set architecture of the cluster nodes. Some examples include, `x86_64`, `aarch64`, or `ppc64le`.
luks:: Include this section if you want to encrypt the root file system. For more details, see "About disk encryption".
tpm2:: Include this field if you want to use a Trusted Platform Module (TPM) to encrypt the root file system.
tang:: Include this section if you want to use one or more Tang servers.
url:: Specifies the URL of a Tang server. In this example, `tangd.socket` is listening on port `7500` on the Tang server.
thumbprint:: Specifies the exchange key thumbprint, which was generated in a preceding step.
advertisement:: Optional: Specifies the advertisement for your offline Tang server in valid JSON format.
threshold:: Specifies the minimum number of TPM v2 and Tang encryption conditions that must be met for decryption to occur. The default value is `1`. For more information about this topic, see "Configuring an encryption threshold".
mirror:: Include this section if you want to mirror the boot disk. For more details, see "About disk mirroring".
devices:: Lists all disk devices that should be included in the boot disk mirror, including the disk that {op-system} will be installed onto.
fips:: Include this directive to enable FIPS mode on your cluster.
+
[IMPORTANT]
====
...
type:: Specifies the encryption format. When the TPM v2 or Tang encryption modes are enabled, the {op-system} boot disks are encrypted using the LUKS2 format.
...
/dev/sda4:: Specifies the device that is listed in the `device` field in the output of the preceding step.
+
.Example output
[source,terminal]
----
...
sss:: In the example output, the Tang plugin is used by the Shamir's Secret Sharing (SSS) Clevis plugin for the `/dev/sda4` device.
...
md126:: The `/dev/md126` software RAID mirror device uses the `/dev/sda3` and `/dev/sdb3` disk devices on the cluster node.
md127:: The `/dev/md127` software RAID mirror device uses the `/dev/sda4` and `/dev/sdb4` disk devices on the cluster node.
+
...
Raid Level:: Specifies the RAID level of the device. `raid1` indicates RAID 1 disk mirroring.
State:: Specifies the state of the RAID device.
Active Devices, Working Devices:: States the number of underlying disk devices that are active and working.
Failed Devices:: States the number of underlying disk devices that are in a failed state.
Name:: Specifies the name of the software RAID device.
/dev/sda3, /dev/sdb3:: Provides information about the underlying disk devices used by the software RAID device.
+
