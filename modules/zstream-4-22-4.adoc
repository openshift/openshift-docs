...
* Before this update, the Hosted Cluster Config Operator would crash loop on a uncustomized Kubernetes cluster because the Operator was unconditionally attempting to watch the {product-title} Route resource. With this release, the Operator attempts to watch the resource only when it exists on the management cluster. As a result, the Operator is no longer crash looping on a uncustomized Kubernetes cluster. (link:https://issues.redhat.com/browse/OCPBUGS-87364[OCPBUGS-87364])
