...
You can create an `IntegrationSource` to receive events from Amazon DynamoDB Streams by applying a YAML manifest. A manifest is a Kubernetes resource definition file in YAML format that acts as a blueprint, describing the resource and its configuration.
...
* A Knative broker or another valid event sink already exists to receive the events.
