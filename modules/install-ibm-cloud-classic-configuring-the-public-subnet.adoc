...
dhcp-range=<dhcp_range_start>,<dhcp_range_end>,<pub_cidr>
...
+
--
* `<dhcp_range_start>` and `<dhcp_range_end>` specify the DHCP range. Replace both with one unused IP address from the public subnet so that the `dhcp-range` for the `baremetal` network begins and ends with the same IP address. `<pub_cidr>` specifies the CIDR of the public subnet.
* `<pub_gateway>` specifies the IP address of the gateway for the `baremetal` network. `<prvn_priv_ip>` specifies the IP address of the provisioner node's private IP address on the `provisioning` network. `<prvn_pub_ip>` specifies the IP address of the provisioner node's public IP address on the `baremetal` network.
--
+
