...
:navtitle: Create {op-system} machines by PXE or iPXE booting
[id="machine-user-infra-machines-pxe_{context}"]
= Creating {op-system} machines by PXE or iPXE booting

[role="_abstract"]
To scale your {product-title} bare metal cluster, you can create more {op-system-first} compute machines by using PXE or iPXE booting.
...
* You have obtained the URLs of the {op-system} ISO image, compressed metal BIOS, `kernel`, and `initramfs` files that you uploaded to your HTTP server during cluster installation.
