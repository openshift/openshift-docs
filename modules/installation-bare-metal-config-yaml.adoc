...
====
<4> This parameter controls the number of workers that the cluster deploys for
you, so you must set the number of workers to `0` in this file. Remember that you
...
<10> The pull secret that you obtained from the
...
<11> The public portion of the default SSH key for the `core` user in
{op-system-first}.
+
[NOTE]
====
