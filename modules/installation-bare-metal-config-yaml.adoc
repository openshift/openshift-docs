...
* When using the oc-mirror plugin v1, use the `repositoryDigestMirrors` section of the `ImageContentSourcePolicy` file that results from running the command.
* When using the oc-mirror plugin v2, copy every `imageDigestMirrors` entry from the generated `working-dir/cluster-resources/idms-oc-mirror.yaml` file. The {product-title} release payload references images in both `quay.io/openshift-release-dev/ocp-v4.0-art-dev` and `quay.io/openshift-release-dev/ocp-v5.0-art-dev`. You must include both mappings.
