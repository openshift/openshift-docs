...
|string (JSON)
|Required if workload partitioning is not enabled. Configure kubelet settings as a JSON string. To satisfy the OVS-DPDK prerequisite when workload partitioning is disabled, set `cpuManagerPolicyOptions` with `strict-cpu-reservation` to `true`. Example: `\{"cpuManagerPolicyOptions":\{"strict-cpu-reservation":"true"}}`.
|===