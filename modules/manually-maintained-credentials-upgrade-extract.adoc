...
:navtitle: Extract and preparing credentials request resources
...
`--included`:: Includes only the manifests that your specific cluster configuration requires for the target release.
...
`openshift-cloud-credential-operator`:: Indicates the namespace which must exist to hold the generated secret.
+
