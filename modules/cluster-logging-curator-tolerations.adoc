...
You apply tolerations to the Curator Pod through the `ClusterLogging` custom resource (CR)
...
. Edit the `curation` section of the `ClusterLogging` custom resource (CR) to configure a toleration for the Curator Pod:
+
[source,yaml]
----
  curation:
