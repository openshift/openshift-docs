...
= Troubleshoot instrumentation injection into your workload
...
`<workload_project>`:: Namespace where the instrumentation was created.
...
`<workload_project>`:: Namespace where the instrumentation is injected for workloads.
+
Example output:
+
[source,terminal]
...
----
+
The following command gets pod annotations for an instrumented Python application:
+
[source,terminal]
----
