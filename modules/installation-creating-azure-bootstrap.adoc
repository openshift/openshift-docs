...
:navtitle: Create the bootstrap machine in {cp}
[id="installation-creating-azure-bootstrap_{context}"]
= Creating the bootstrap machine in {cp}

[role="_abstract"]
To initialize your {product-title} cluster on Microsoft {cp}, you must deploy the bootstrap machine by using the `04_bootstrap.json` ARM template.
