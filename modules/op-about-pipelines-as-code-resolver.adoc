...
// * [UNUSED] pac/using-pac-resolver.adoc
...
If {pac} observes a pipeline run with a reference to a task or a pipeline in any YAML file in the `.tekton/` directory, {pac} automatically resolves the referenced task. The result is a single pipeline run with an embedded spec in a `PipelineRun` object.

If {pac} cannot resolve the referenced tasks in the `Pipeline` or `PipelineSpec` definition, the run fails before applying any changes to the cluster. You can see the issue on your Git provider platform and inside the events of the target namespace that contains the `Repository` CR.
