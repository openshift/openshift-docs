...
Track installation progress and verify successful installation.
...
+
--
`$ ./openshift-install --dir <install_directory> agent wait-for bootstrap-complete \//`:: For `<install_directory>`, specify the path to the directory where the agent ISO was generated.
`--log-level=info`:: To view different installation details, specify `warn`, `debug`, or `error` instead of `info`.
--
...
+
--
`$ openshift-install --dir <install_directory> agent wait-for install-complete`:: For `<install_directory>` directory, specify the path to the directory where the agent ISO was generated.
--
