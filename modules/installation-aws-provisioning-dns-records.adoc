...
[role="_abstract"]
Use your cluster name and base cluster domain to configure a DNS A record for the API service `api.<cluster_name>.<base_domain>` with the API load balancer DNS name. Similarly, use the load balancer DNS name of the Ingress service to provision a DNS A record for the `*.apps.<cluster_name>.<base_domain>` hostname by using your cluster name and base cluster domain
...
[source,terminal]
----
$ aws --region ${REGION} elbv2 describe-load-balancers --names ${INFRA_ID}-ext --query 'LoadBalancers[*].DNSName' --output text
----
