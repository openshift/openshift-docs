...
`All resources or a subset of resources`:: The policy access scope should be set based on how granular you want to assign access. The scope can be set to *All resources* or *Resources based on selected attributes*.
`Identity and Access Management`:: This access policy is optional. It is only required if you want the installation program to create a resource group. For more information about resource groups, see the "{ibm-name} resource groups documentation".
