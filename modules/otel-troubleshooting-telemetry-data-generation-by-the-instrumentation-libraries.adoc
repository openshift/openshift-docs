...
= Troubleshoot the instrumentation libraries

[role="_abstract"]
You can troubleshoot telemetry generation by the instrumentation libraries by checking the endpoint, looking for errors in your application logs, and verifying that the Collector is receiving the telemetry.

.Procedure

. Verify that the instrumentation is transmitting telemetry to the correct endpoint:
+
[source,terminal]
----
