...
Kiali is based on the open source link:https://kiali.io/[Kiali project]. Kiali provided by Red Hat is composed of three parts:
...
Kiali Server (back end):: This component runs in the container application platform and communicates with the service mesh components, retrieves and processes data, and exposes this data to the console. The Kiali Server does not need storage. When deploying the Server to a cluster, configurations are set in ConfigMaps and secrets.

Kiali console (front end):: The Kiali console is a web application. 	The Kiali Server serves the Kiali console, which then queries back to the Kiali Server for data to present it to the user.
