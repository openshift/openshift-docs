...
[NOTE]
====
You can install only one instance of the `NonAdminController` (NAC) in the cluster. You get the following error, if you install the NAC again: `message: only a single instance of Non-Admin Controller can be installed across the entire cluster. Non-Admin controller is already configured and installed in openshift-adp namespace`.
====
