...
[role="_abstract"]
Edit the `HostFirmwareSettings` resource to modify BIOS configuration on provisioned hosts, enabling you to tune performance, enable hardware features, or adjust power management settings.
...
+
--
`name: value`:: Use the `FirmwareSchema` resource to identify the available settings for the host. You cannot set values that are read-only.
--
