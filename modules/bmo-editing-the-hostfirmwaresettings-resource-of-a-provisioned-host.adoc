...
[role="_abstract"]
To modify the firmware configuration of a provisioned host, you can edit the `HostFirmwareSettings` resource. This procedure involves updating the resource spec and cycling the host through the machine set to ensure the new BIOS or firmware settings take effect across your cluster.
...
----
+
In the `spec.settings.name` field use the `FirmwareSchema` resource to identify the available settings for the host. You cannot set values that are read-only.
...
----
+
Replace `<hostname>` with the name of the host. The terminal displays the machine name under the `CONSUMER` field.
