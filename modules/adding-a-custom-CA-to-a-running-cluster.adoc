...
For a running cluster, you can add a custom CA by creating a configMap that contains your certificate and then referencing that ConfigMap in the cluster Proxy object.
...
The procedure involves two stages: creating a ConfigMap with your certificate and then updating the cluster to trust it.

. Create a ConfigMap with your CA certificate

.. Create a YAML file named custom-ca.yaml to define the ConfigMap.
...
----
+
<1> The name of the `ConfigMap` that you will reference from the `Proxy` object.
<2> The `ConfigMap` must be created in the `openshift-config` namespace.
<3> The data key for the certificate bundle must be `ca-bundle.crt`.

.. Apply the manifuest to create the ConfigMap in the cluster:
+
[source,terminal]
----
...
. Reference the ConfigMap in the cluster Proxy object

.. Run the following oc patch command to update the cluster Proxy object to reference the ConfigMap you just created.
+
[source,terminal]
----
