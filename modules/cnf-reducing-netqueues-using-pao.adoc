...
[id="reducing-nic-queues-using-performance-addon-operator_{context}"]
...
In real-time or low latency systems all the unnecessary interrupt request lines (IRQs) that are pinned to the isolated CPUs must be moved to reserved or housekeeping CPUs. For Data Plane Development Kit (DPDK) based workloads for example to achieve low latency the number of NIC queues should be reduced to just the number of reserved or housekeeping CPUs.

In deployments with applications that require system, {product-title} networking or in mixed deployments with DPDK workloads, multiple queues are needed to achieve good throughput and the number of NIC queues should either be adapted or remain unchanged.
Enabling this feature in the performance profile ensures all IRQs are moved from isolated to housekeeping CPUs preventing the housekeeping CPUs from running out of vectors/capacity.


[id="adjusting-nic-queues-with-performance-profile_{context}"]
