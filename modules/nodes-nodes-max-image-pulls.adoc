...
By default, requests for image pulls are immediately sent to the image service and multiple images can be pulled concurrently.
...
To enable parallel image pulls, set the serializeImagePulls field to false in the kubelet configuration. When serializeImagePulls is disabled
Kubernetes v1.27
.Prerequisites
...
[source,terminal]
----
$ oc label machineconfigpool worker parallel-pulls=set
----
...
#...
----
<1> Set to `false` to enable parallel image pulls. Set to `true` to disable swap memory use. The default is `false`.
<2> Specify the maximum number of images that can be pulled simultaneously.
