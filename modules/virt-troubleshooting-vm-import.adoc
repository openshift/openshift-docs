...
* The following error message is displayed in the VM Import Controller Pod log if the RHV VM's system settings do not emulate the Intel Q35 chipset:
+
----
...
* The following error message is displayed in the VM Import Controller Pod log if the target VM name exceeds 23 characters link:https://bugzilla.redhat.com/show_bug.cgi?id=1857165[(*BZ#1857165*)]:
+
----
...
ifdef::virt-importing-rhv-vm[]
* In {VirtProductName} 2.4, the import process fails if the target VM name is longer than 23 characters. link:https://bugzilla.redhat.com/show_bug.cgi?id=1857165[(*BZ#1857165*)]

* In {VirtProductName} 2.4, you must create a Windows {VirtProductName} template and a custom ConfigMap in order to import a RHV VM with a Windows operating system.
