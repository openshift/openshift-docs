...
.Procedure
. Create a `NodeNetworkConfigurationPolicy` object to map the OVN-Kubernetes secondary network to an Open vSwitch (OVS) bridge. For example:
+
[source,yaml]
----
...
. Create a `ClusterUserDefinedNetwork` object to create a localnet secondary network. For example:
+
[source,yaml]
----
