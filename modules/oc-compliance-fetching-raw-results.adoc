...
** `<object-name>` is the name of the binding, suite, or scan object to gather the ARF file for.
** `<output-path>` is the local directory to place the results.
+
For example:
+
[source,terminal]
...
. View the list of files in the output directory:
+
[source,terminal]
----
...
. Decompress and extract the specific results file using `bunzip2`:
+
[source,terminal]
----
...
. Verify the extracted XML results file exists in the directory:
+
[source,terminal]
----
