...
* With this update, you can use the optional `.spec.nodeSelector` field with `shp` CLI to specify the nodes on which build run pods should run.
...
This method provides the fastest way to clone a Git repository and is suitable for all cases where your build logic doesn't rely on Git history.
+
...
* With this update, the following channel is deprecated:
** builds-1.3
+
