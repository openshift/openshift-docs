...
= Use CoreDNS with a single cluster
...
. Extract the CoreDNS manifests from the `rhcl-operator` bundle by running the following commands:

.. Create a writable container layer from your RHCL Operator bundle image by running the following command:
+
[source,terminal,subs="+quotes"]
----
$ podman create --name _<bundle>_ registry.redhat.io/rhcl-1/rhcl-operator-bundle:rhcl-_<1.3.0>_
----
+
...
----
* Replace `_<name>_` with the name of your `DNSRecord` CR.
* Replace `_<namespace>_` with the namespace where you applied your `DNSRecord` CR.
+
[source,terminal,subs="+quotes"]
----
