...
If you want to remove the options from your network interface, delete the options from the file, add `state: absent` under the `dns-resolver.config` section of the NNCP, and then run `oc apply -f <nncp_file_name>`. If you skip adding the `state: absent` step, the options persist on your network interface.
====
