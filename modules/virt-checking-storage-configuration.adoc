...
. Create a `ServiceAccount`, `Role`, and `RoleBinding` manifest file for the storage checkup. For example:
+
[%collapsible]
[source,yaml]
----
...
. Create a `ConfigMap` and `Job` manifest file. The config map contains the input parameters for the checkup job. For example:
+
