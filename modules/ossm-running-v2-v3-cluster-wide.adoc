...
If you are moving from {SMProductName} 2.6 in a cluster-wide deployment, you can run {SMProduct} 2.6 side-by-side with {SMProduct} 3.0, in one cluster, without them interfering with each other.

Since the control plane in the cluster-wide deployment model watches all namespaces, you must be do the following:

. Be running {SMProduct} 2.6
. Configure {SMProduct} 2.6 `ServiceMeshControlPlane` resource to restrict namespaces to only those belonging to {SMProduct} 2.6. This prevents conflicts with {SMProduct} 3.0.
...
* You are running {ocp-product-title} 4.15 or later
* You are running {SMProduct} 2.6

If you are not on {SMProduct} 2.6, you must upgrade to {SMProduct} 2.6 before following this procedure.
...
. Configure `discoverySelectors`` in your {SMProduct} 2.6 `ServiceMeshControlPlane` custom resource (CR):
+
...
. Install {SMProduct} Operator 3.0.
...
<3> Use `discoverySelectors` to ignore {SMProduct} 2.6 namespaces. All other namespaces will be part of the {SmProduct} 3.0 mesh.
...
[NOTE]
====
Do not use the `istio-injection=enabled` label for your {SMProduct} 3.0 workload namespaces unless you have changed `spec.memberSelectors` in `ServiceMeshMemberRoll` in the `ServiceMeshControlPlane` resource in {SMProduct} 2.6."
====
...
[id=additional-resources-cluster-wide]
...
* To better understand the steps described here, it's recommended to read the OSSM 2 [deployment models](https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/service_mesh/service-mesh-2-x#ossm-deployment-models)
//change to link:

* upstream Istio [Multiple Control Planes](https://istio.io/latest/docs/setup/install/multiple-controlplanes/) documentation.
//change to link:

* See upstream Istio documentation for [discoverySelectors](https://istio.io/v1.19/docs/setup/install/multiple-controlplanes/) usage.
//change to link: