...
* {SMProduct} Operator 3.0 is installed.

If you are not on {SMProduct} 2.6, you must upgrade before following this procedure.
...
----
    ...
    spec:
      ...
      version: v1.23.0
----
<1> Namespace must be different than a namespace running in {SMProduct} 2.6.
...
[NOTE]
====
If you have changed `spec.memberSelectors` in the `ServiceMeshMemberRoll` in your {SMProduct} 2.6 `ServiceMeshControlPlane` resource, then use `istio-injection=enabled` for your {SMProduct} 3.0 workload namespaces instead of `istio.io/rev=ossm3` label.
====
