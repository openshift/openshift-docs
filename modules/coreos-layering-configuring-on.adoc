...
Making changes to a Containerfile in a `MachineOSConfig` object triggers an automatic rebuild of the associated custom layered image. In 

In the event of a build failure, due for example to network issues or an invalid secret, the MCO attempts to retry the build three times before the job fails. The MCO creates a different build pod for each build attempt. You can use the build pod log to troubleshoot any build failures. 
...
spec:
  ...
  renderedImagePushSpec: image-registry.openshift-image-registry.svc:5000/openshift/os-image:latest  # <6>
...
<3> Specifies the architecture this containerfile is to be built for: `ARM64`, `AMD64`, `PPC64LE`, `S390X`, or `NoArch`. The default is `NoArch`, which defines a Containerfile that can be applied to any architecture. 
<4> Specifies the name of the image builder to use. This must be `Job`, which is a reference to the `job` object that is managing the image build.
<5> Optional: Specifies the name of the pull secret that the MCO needs to pull the base operating system image from the registry. By default, the global pull secret is used.
