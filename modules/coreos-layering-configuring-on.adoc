...
:navtitle: {image-mode-os-on-lower}
:chunk-to-content:
[id="coreos-layering-configuring-on_{context}"]
= About {image-mode-os-on-lower}

[role="_abstract"]
You can use the {image-mode-os-lower} on-cluster build process to apply a custom layered image to your nodes by creating a `MachineOSConfig` custom resource (CR).
