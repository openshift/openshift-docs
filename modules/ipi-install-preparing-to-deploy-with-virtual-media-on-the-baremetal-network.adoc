...
+
--
`spec.virtualMediaViaExternalNetwork: true`:: Add `virtualMediaViaExternalNetwork: true` to the `provisioning` CR.
--
...
+
--
`spec.template.spec.providerSpec.value.image.checksum: http:/172.22.0.3:6181/images/rhcos-<version>.<architecture>.qcow2.<md5sum>`:: Edit the `checksum` URL to use the API VIP address.
`spec.template.spec.providerSpec.value.image.url: http://172.22.0.3:6181/images/rhcos-<version>.<architecture>.qcow2`:: Edit the `url` URL to use the API VIP address.
--
