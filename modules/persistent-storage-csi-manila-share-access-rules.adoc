...
----
+
* `metadata.name`: Descriptive name for your custom storage class.
* `parameters.type`: The Manila share type. This type must match an existing share type in your {rh-openstack} environment.
...
----
+
* `spec.storageClassName`: The name of your custom storage class that has restricted access. In this example, the name is `csi-manila-gold-restricted`.
