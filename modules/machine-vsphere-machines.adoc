...
:navtitle: Add more compute machines to a cluster in vSphere
[id="machine-vsphere-machines_{context}"]
= Adding more compute machines to a cluster in vSphere

[role="_abstract"]
To scale a user-provisioned {product-title} cluster on VMware vSphere, you can add more compute machines by cloning the vSphere template into a virtual machine (VM).
