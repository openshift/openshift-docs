...
|minimalISO:
|Defines whether the Agent-based Installer generates a full ISO or a minimal ISO image. When set to `True`, the Agent-based Installer generates an ISO without a rootfs image file, and instead contains details about where to pull the rootfs file from.

When generating a minimal ISO, if a rootfs URL is not specified by the `bootArtifactsBaseURL` parameter, then the Agent-based Installer embeds a default URL that is accessible in environments with an internet connection.
