...
----
+
Destination-based mapping can be enabled for managed Agents by setting the following fields in the Argo CD CR. Optionally, enable `createNamespace` to allow the Agent to create the application namespaces automatically.
+
[source,yaml]
----
apiVersion: argoproj.io/v1beta1
