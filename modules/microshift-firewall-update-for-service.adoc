...
[role="_abstract"]
To ensure external clients can securely reach your applications, understand how firewall ports interact with exposed services in {product-title}. While OVN-Kubernetes automatically configures the internal routing for `NodePort` and `LoadBalancer` services, you must manually open the corresponding ports in the host firewall to allow external traffic to reach them.
