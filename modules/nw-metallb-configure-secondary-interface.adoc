...
= Enabling per-interface IP forwarding by using a MachineConfig
...
[IMPORTANT]
====
Per-interface IP forwarding is supported for IPv4 only. IPv6 per-interface forwarding is not supported because enabling it also requires enabling global IPv6 forwarding, which changes router advertisement behavior and can introduce security risks.
====

To enable IP forwarding for the secondary interface, you have two options:

* Enable IP forwarding for a specific interface by using a `MachineConfig` resource.
* Enable IP forwarding for all interfaces globally.

[NOTE]
====
Enabling IP forwarding for a specific interface provides more granular control, while enabling it for all interfaces applies a global setting. You can also use the Kubernetes NMState Operator to enable per-interface forwarding declaratively. For more information, see "Enabling per-interface IP forwarding by using the nmstate API".
====
