...
[role="_abstract"]
During a cluster deployment, the Federal Information Processing Standards (FIPS) change is applied when the {op-system-first} machines are deployed in your cluster. For {op-system-base-full} ({op-system-base}) machines, you must enable FIPS mode when you install the operating system on the machines that you plan to use as worker machines.
