...
You upgrade the worker nodes after you have upgraded the control plane by un-pausing the relevant `MCP` groups you created.
Un-pausing the `MCP` group starts the upgrade process for the worker nodes in that group.
Each of the worker nodes in the cluster reboot to upgrade to the new EUS, y-stream or z-stream version as required.

In the case of EUS to EUS upgrades note that when a worker node is upgraded it will only require one reboot and will jump <y+2>-release versions. This is a feature that was added to decrease the amount of time that it takes to upgrade large baermetal clusters.
