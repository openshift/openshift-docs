...
You can also configure your application to send requests to an internal cluster-local Gateway hostname that rewrites headers and originates TLS outbound so that you can centralize external security policies, simplify container configuration, and prevent breaking standard DNS resolution across the cluster.
