...
[role="_abstract"]
You can configure the node drain grace period for machine pools in your cluster. This setting controls how long the cluster respects Pod Disruption Budget protected workloads during upgrades or replacements. After this period, remaining workloads are forcibly evicted.

Valid values range from `0` to `1 week`. A value of `0`, or empty, drains without a time limit.
...
----
+
If the `Node drain grace period` value is empty, the machine pool drains without any time limitation until complete.
