...
The log collector sends logs using the syslog protocol over TCP or UDP. You can configure the RFC format, severity mapping, and facility in the `ClusterLogForwarder` output configuration.
...
UDP:: Connectionless transport with lower overhead but no delivery guarantees. Use for high-volume, non-critical logs.

TLS over TCP:: Encrypted transport with certificate-based authentication. Required for secure log forwarding over untrusted networks.
