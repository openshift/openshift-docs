...
. If the CSRs for new compute nodes that are trying to join the cluster are not approved, check the Machine Approver logs by entering the following command:
+
[source,terminal]
----
