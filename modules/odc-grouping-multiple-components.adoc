...
. To add the MongoDB service to the existing application group, press kbd:[Shift] and click the *mongodb* Pod to drag it to the application. The MongoDB service is added to the existing application group.
. The kbd:[Shift] and drag method of adding a component to an application group automatically adds the required labels to the component. Click on the MongoDB service node to see the label `app.kubernetes.io/part-of=myapp` added to the *Labels* section in the *Overview* Panel.
+
...
You can remove a component from an application group by pressing kbd:[Shift] and clicking the component to drag it out of the application group.
