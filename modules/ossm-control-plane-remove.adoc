...
You can remove the {ProductShortName} control plane by using the {product-title} web console or the CLI. When you remove the `ServiceMeshControlPlane`,  the {ProductShortName} Operator uninstalls the components the `ServiceMeshControlPlane` installed.
