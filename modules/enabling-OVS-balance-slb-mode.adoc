...
. In the `install-config.yaml` file for your cluster, define a `networkConfig` section similar to the following example:
+
[source,yaml]
----
# ...
...
<3> The second bonded interface that has both IPv4 and IPv6 addresses disabled. 
...
----
+
<1> For each node in your cluster, specify the hostname path to your node and the base-64 encoded Ignition configuration file data for the machine type.
<2> The name of the policy.
<3> Writes the encoded base64 information to the specified path.

. Use the command line to apply the `MachineConfig` file, which defines your network interfaces, to the installation program:
+
[source,terminal]
----
...
. Use the `cat` command to base64-encode the contents of the installation configruration file:
+
[source,terminal]
----
