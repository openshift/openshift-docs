...
where:
+
`name.enp1s0`:: The interface for the provisioned network interface controller (NIC). 
`name.enp2s0`:: The first bonded interface that pulls in the Ignition config file for the bond interface.
`mtu: 1500`:: Manually set the `br-ex` maximum transmission unit (MTU) on the bond ports.
`name: enp3s0`:: The second bonded interface is part of a minimal configuration that pulls ignition during cluster installation.
...
where:
+
`mtu: 1500`:: Manually set the `br-ex` MTU on the bond ports.
...
where
+
`mtu: 1500`:: Where the `-w0` option prevents line wrapping during the base64 encoding operation.
