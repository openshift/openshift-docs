...
where:
+
`name`:: The interface for the provisioned network interface controller (NIC), such as `enp1s0`. 
`name`:: The first bonded interface that pulls in the Ignition config file for the bond interface, such as `enp2s0`.
`mtu`:: Manually set the `br-ex` maximum transmission unit (MTU) on the bond ports.
`name`:: The second bonded interface is part of a minimal configuration that pulls ignition during cluster installation, such as `enp3s0`.
