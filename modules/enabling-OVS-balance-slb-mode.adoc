...
<3> Manually set the `br-ex` MTU on the bond ports.
...
+
The Machine Config Operator (MCO) then takes the content from each manifest file and consistently applies the content to all selected nodes during a rolling update.
