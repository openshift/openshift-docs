...
<provisioning_interface>:: Specifies the interface for the provisioned network interface controller (NIC).
<bond_interface_1>:: Specifies the first bonded interface that pulls in the Ignition config file for the bond interface.
<mtu_value>:: Specifies the MTU value. Manually set the `br-ex` maximum transmission unit (MTU) on the bond ports.
<bond_interface_2>:: Specifies the second bonded interface that is part of a minimal configuration that pulls ignition during cluster installation.
