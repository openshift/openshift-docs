...
[role="_abstract"]
Resolve the silent failure issue where the {oadp-short} Operator reports a `Running` status but the {aws-short} S3 buckets remain empty due to incorrect cloud credentials permissions. This helps you identify and fix credential permission problems in your backup storage locations.
...
.. Retrieve a list of BSLs by using the OADP CLI:
+
[source,terminal]
----
