...
[role="_abstract"]
Deploy {logging-uc} with Elasticsearch storage by using {oc-first} commands and YAML manifests.
...
metadata.name::
  You must specify the `openshift-operators-redhat` namespace. The `openshift-operators` namespace might contain Community Operators, which are untrusted and could publish a metric with the same name as an {ocp-product-title} metric, which would cause conflicts.
