...
[role="_abstract"]
Set how long Elasticsearch retains logs for infrastructure, application, and audit sources.
...
+
For example, the Red Hat OpenShift Logging Operator updated the following `Elasticsearch` CR to configure a retention policy that includes settings to roll over active indices for the infrastructure logs every eight hours and the rolled-over indices are deleted seven days after rollover. {ocp-product-title} checks every 15 minutes to determine if the indices need to be rolled over.
+
[source,yaml]
----
apiVersion: "logging.openshift.io/v1"
...
spec.indexManagement.policies::
  For each log source, the retention policy indicates when to delete and roll over logs for that source.

spec.indexManagement.policies.phases.delete.minAge::
  When {ocp-product-title} deletes the rolled-over indices. This setting is the `maxAge` you set in the `ClusterLogging` CR.

spec.indexManagement.policies.phases.hot.actions.rollover.maxAge::
  The index age for {ocp-product-title} to consider when rolling over the indices. This value is determined from the `maxAge` you set in the `ClusterLogging` CR.

spec.indexManagement.policies.pollInterval::
  When {ocp-product-title} checks if the indices should be rolled over. This setting is the default and cannot be changed.
+
[NOTE]
====
