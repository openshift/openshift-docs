...
[role="_abstract"]
Forward logs to an external Elasticsearch instance for centralized log management and long-term storage.

You can forward logs to an external Elasticsearch instance in addition to, or instead of, the internal log store. You are responsible for configuring the external log aggregator to receive log data from {ocp-product-title}.
...
spec.outputs.elasticsearch.version::
