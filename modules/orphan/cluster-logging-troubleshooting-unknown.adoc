...
[role="_abstract"]
Resolve Kibana-Elasticsearch connection failures caused by F-5 load balancers with X-Forwarded-For headers.
...
oc edit configmap/elasticsearch::
  This approach requires that Elasticsearch configurations are within a config map.
+
