...
[role="_abstract"]
Install OpenShift Logging with Elasticsearch using the web console.

You can use the {ocp-product-title} web console to install the OpenShift Elasticsearch and Red Hat OpenShift Logging Operators. Elasticsearch is a memory-intensive application. By default, {ocp-product-title} installs three Elasticsearch nodes with memory requests and limits of 16 GB. This initial set of three {ocp-product-title} nodes might not have enough memory to run Elasticsearch within your cluster. If you experience memory issues that are related to Elasticsearch, add more Elasticsearch nodes to your cluster rather than increasing the memory on existing nodes.
...
+
If you have the pull secret, add the `redhat-operators` catalog to the OperatorHub custom resource (CR) as shown in _Configuring {ocp-product-title} to use Red Hat Operators_.
endif::[]

.Procedure

To install the OpenShift Elasticsearch Operator and Red Hat OpenShift Logging Operator using the {ocp-product-title} web console:
...
a metric with the same name as an {ocp-product-title} metric, which would cause
conflicts.
...
spec.logStore.elasticsearch.nodeCount::
...
+
====
The maximum number of master nodes is three. If you specify a `nodeCount` greater than `3`, {ocp-product-title} creates three Elasticsearch nodes that are Master-eligible nodes, with the master, client, and data roles. The additional Elasticsearch nodes are created as Data-only nodes, using client and data roles. Master nodes perform cluster-wide actions such as creating or deleting an index, shard allocation, and tracking nodes. Data nodes hold the shards and perform data-related operations such as CRUD, search, and aggregations. Data-related operations are I/O-, memory-, and CPU-intensive. It is important to monitor these resources and to add more Data nodes if the current nodes are overloaded.
