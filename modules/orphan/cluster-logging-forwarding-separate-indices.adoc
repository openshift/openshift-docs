...
[role="_abstract"]
Send logs from different containers in the same pod to separate Elasticsearch indices using multi-container support and pod annotations.
...
spec.outputDefaults.elasticsearch.structuredTypeKey::
  Uses the value of the key-value pair that is formed by the Kubernetes `logFormat` label.

spec.outputDefaults.elasticsearch.enableStructuredContainerLogs::
