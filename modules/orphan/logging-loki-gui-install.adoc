...
[role="_abstract"]
Install the {loki-op} and Red{nbsp}Hat OpenShift Logging Operator using the web console to enable log storage and collection on your cluster.

To install and configure logging on your {ocp-product-title} cluster, an Operator such as {loki-op} for log storage must be installed first. This can be done from the OperatorHub within the web console.

.Prerequisites

* You have access to a supported object store (AWS S3, Google Cloud Storage, Azure, Swift, S3 Compatible storage, {rh-storage}).
