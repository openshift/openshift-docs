...
[role="_abstract"]
Install and configure logging with the Loki Operator and cluster logging on your OpenShift cluster using the CLI.
...
* You have access to a supported object store. For example: AWS S3, Google Cloud Storage, Azure, Swift, S3 Compatible storage, or {rh-storage}.
...
metadata.name::
  The Red{nbsp}Hat OpenShift Logging Operator is only deployable to the `openshift-logging` namespace.
