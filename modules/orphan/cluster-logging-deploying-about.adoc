...
[role="_abstract"]
Deploy and configure {logging} using the ClusterLogging custom resource with default or customized settings.

The {logging} is designed to be used with the default configuration, which is tuned for small to medium sized {ocp-product-title} clusters.
...
////
Log collectors::
You can select which log collector is deployed as a daemon set to each node in the {ocp-product-title} cluster, either:
