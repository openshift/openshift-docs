...
= Launching an {ibm-name} Secure Execution VM on {ibm-z-name} and {ibm-linuxone-name}

Before launching an {ibm-name} Secure Execution VMs on {ibm-z-name} and {ibm-linuxone-name} you must add the `launchSecurity` parameter to the `secure-execution-vm.yaml` file . Otherwise, the VM does not boot correctly because it does not have access to the devices.
