...
= Create a container image by using Buildpacks
...
* You have created the project where your final application image is stored by using the command `oc new-project builds-sample`.
...
. Optional: Run the following commands to use the `shp` CLI with `buildpacks` and grant the `pipeline` service account permission to access the image registry in the `builds-sample` project.
+
[source,terminal]
----
$ oc policy add-role-to-user system:image-puller system:serviceaccount:default:pipeline --namespace=builds-sample
----
+
[source,terminal]
----
$ oc policy add-role-to-user system:image-pusher system:serviceaccount:default:pipeline --namespace=builds-sample
----
...
spec:
  ...
  output:
    image: image-registry.openshift-image-registry.svc:5000/builds-sample/taxi-app
EOF
----
+
or
...
--output-image="image-registry.openshift-image-registry.svc:5000/builds-sample/taxi-app" \
--param-value="source-subpath=buildpack/nodejs" \
...
`--source-git-url`:: Specifies the URL of the Git repository containing your application source code.

`--strategy-name`:: Specifies the build strategy to build the container.

`--output-image`:: Specifies the location where the built image is pushed.

`--param-value`:: Specifies the parameters set for the buildpacks strategy.
...
buildpack-nodejs-build   True         Succeeded   ClusterBuildStrategy   buildpacks          68s
----
+
[source,terminal]
----
...
[source,terminal]
----
$ shp build run buildpack-nodejs-build --follow
----
+
where:
+
...
[source,terminal]
----
$ shp buildrun logs buildpack-nodejs-<buildrun_resource_name> --follow
----
====
...
buildpack-nodejs-buildrun-ttwkl-d8x97-pod   2/8     NotReady   0          63s
buildpack-nodejs-buildrun-ttwkl-d8x97-pod   0/8     Completed   0          72s
buildpack-nodejs-buildrun-ttwkl-d8x97-pod   0/8     Completed   0          73s
----
...
buildpack-nodejs-buildrun-ttwkl-d8x97   True        Succeeded   112s        38s
----
...
----
NAME                            SUCCEEDED   REASON      STARTTIME   COMPLETIONTIME
buildpack-nodejs-buildrun-ttwkl   True        Succeeded   107s        33s
----
+
[NOTE]
====
...
. Validate whether the image has been pushed to the registry you specified in the `build.spec.output.image` field:
...
[source,terminal]
----
$ podman manifest inspect image-registry.openshift-image-registry.svc:5000/builds-sample/taxi-app
----
+
In the previous example, the project name is `builds-sample`, and the image name is `taxi-app`.
+
Example output:
+
[source,terminal]
