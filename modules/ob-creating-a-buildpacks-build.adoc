...
* `buildpacks` strategy: The `buildpacks` cluster build strategy uses Cloud Native Buildpacks (CNB) to efficiently build and push container images based on the source code, without using a traditional Dockerfile. The configuration details are provided through parameters in the `Build` resource.
* buildpacks-extender strategy: The `buildpacks-extender` strategy uses CNB Stack Extensions and extends the standard `buildpacks` strategy. It allows you to modify the base operating system (OS) images using a custom Dockerfile. Based on the parameters configured in the `build` resource, it can patch either the builder image or the run image for each build execution.
...
`<buildrun_resource_name>`:: Specicies the buildrun resource name
