...
[IMPORTANT]
====
{builds-title-uppercase} supports the execution process for Cloud Native Buildpacks (CNB) within the build strategies. Red Hat does not provide support for the content of user-provided builder and runtime images.
====
...
----
In the above example, the project name is `buildpacks-example`, and the image name is `taxi-app`.
+
.Example output
[source,terminal]
----
