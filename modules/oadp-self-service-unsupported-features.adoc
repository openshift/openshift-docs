...
* A namespace admin user cannot create a `VolumeSnapshotLocation` (VSL) CR. The cluster administrator creates and configures the VSL in the `DataProtectionApplication` (DPA) for a namespace admin user.

* `ResourceModifiers` and volume policies are not supported for a namespace admin user.
