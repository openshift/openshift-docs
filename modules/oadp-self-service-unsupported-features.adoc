...
* A non-admin user can request backup or restore logs by using the `NonAdminDownloadRequest` CR, only if the backup or restore is created by using the `NonAdminBackupStorageLocation` CR created by the user and not the cluster-wide default backup storage location.
