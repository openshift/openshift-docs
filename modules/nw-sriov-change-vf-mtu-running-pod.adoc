...
----
+
The `command` and `args` fields set the VF MTU to the specified value before running the application command. The `NET_ADMIN` Linux capability is required for the container to change network interface settings.
+
where:
+
...
----
+
If the `mtu` field is not set in the `SriovNetworkNodePolicy` CR, the SR-IOV Network Operator does not revert the VF MTU value.
