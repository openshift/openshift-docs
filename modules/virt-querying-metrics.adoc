...
[NOTE]
====
The following examples use `topk` queries that specify a time period. If virtual machines (VM) are deleted during that time period, they can still appear in the query output.
====
...
This delay appears to the virtual machine as steal time and can affect performance, often indicating CPU overcommitment or contention on the host.  
Type: counter
