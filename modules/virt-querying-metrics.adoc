...
The following queries can identify virtual machines that are waiting for Input/Output (I/O):
...
`kubevirt_vmi_storage_read_traffic_bytes_total`:: Returns the total amount (in bytes) of the VM’s storage-related traffic. Type: Counter.
`kubevirt_vmi_storage_write_traffic_bytes_total`:: Returns the total amount of storage writes (in bytes) of the VM’s storage-related traffic. Type: Counter.

Example storage-related traffic query
[source,promql]
----
topk(3,
  sum by (name, namespace) (rate(kubevirt_vmi_storage_read_traffic_bytes_total[6m])) 
  + sum by (name, namespace) (rate(kubevirt_vmi_storage_write_traffic_bytes_total[6m]))
) > 0 <1>
----
<1> Returns the top 3 VMs performing the most storage traffic at every moment over six minutes.

Storage snapshot data:::
`kubevirt_vmsnapshot_disks_restored_from_source`:: Returns the total number of VM disks restored from the source VM. Type: Gauge.

Example kubevirt vmsnapshot disks restored from source
[source,promql]
----
kubevirt_vmsnapshot_disks_restored_from_source{vm_name="simple-vm", vm_namespace="default"} <1>
----
<1> Returns the total number of VM disks restored from the source VM.

`kubevirt_vmsnapshot_disks_restored_from_source_bytes`:: Returns the amount of space in bytes restored from the source VM. Type: Gauge.

Example kubevirt vmsnapshot disks restored from source bytes query
[source,promql]
----
kubevirt_vmsnapshot_disks_restored_from_source_bytes{vm_name="simple-vm", vm_namespace="default"} <1>
----
<1> Returns the amount of space in bytes restored from the source VM.
...
Returns the current quota usage and resource limits enforced by the AAQ Operator. Type: Gauge.
...
Example Operator metrics query
[source,promql]
----
kube_application_aware_resourcequota{resource= <resource_name>, type=<type of value, hard or used>} <1>
----
<1> This query returns the current quota usage and resource limits enforced by the AAQ Operator.
