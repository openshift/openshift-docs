...
[NOTE]
====
When the {gitops-title} Operator is deployed on an {OCP} cluster configured for FIPS mode, Single signon (SSO) configuration for Argo CD using Keycloak is not supported.
====
...
* With this update, support is provided to configure the `respectRBAC` option, which controls how Argo CD watches resources on a cluster in a cluster-scoped installation. You can now update `respectRBAC` configurations in the `ConfigMap` through an Argo CD resource, allowing for more flexible and granular control over resource viewing behavior. link:https://issues.redhat.com/browse/GITOPS-5212[GITOPS-5212]

* With this update, you can now view the cause of failure directly in the status of Argo CD resources. The error message is clearly provided in the resource status, reducing the need to analyze logs to identify the root cause of failures. link:https://issues.redhat.com/browse/GITOPS-5871[GITOPS-5871]
...
* With this update, specifying the container image when configuring a sidecar container for a config management plugin is now optional. If omitted, the image used by the repo server is automatically applied to the plugin. https://issues.redhat.com/browse/GITOPS-3372[GITOPS-3372]
...
With this update, the {gitops-shortname} Operator correctly updates the Redis HA config maps and `StatefulSet` during an upgrade process. As a result, Redis HA pods are now prevented from entering an `AUTH` error state post-upgrade. link:https://issues.redhat.com/browse/GITOPS-5975[GITOPS-5975]
