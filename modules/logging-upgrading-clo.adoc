...
+
If your previous forwarder is deployed in the namespace `openshift-logging` and named `instance`, then you have likely been using the service account `logcollector` created by earlier versions of the operator. In such a case you do not need to create a service account. However you must still assign RBAC permissions to the old service account as shown in the next step.
+
[source,terminal]
----
