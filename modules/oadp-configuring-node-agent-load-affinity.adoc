...
See the following example in which you can schedule the node agent pods on nodes with the label `label.io/role: cpu-1` _and_ `other-label.io/other-role: cpu-2`.
...
+
In the example, you ensure that the node agent pods are scheduled only on nodes with the label `label.io/role: cpu-1` _and_ the label `label.io/hostname` matching with either `node1` _or_ `node2`.
+
[source,yaml]
----
...
