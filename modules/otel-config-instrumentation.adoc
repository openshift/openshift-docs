...
// 
// * instrument-application/otel-instrument-application.adoc
...
The following `Instrumentation` CR example shows instrumentation options:
...
The following protocols are the defaults for auto-instrumentation:
...
You can use the `instrumentation.opentelemetry.io/inject-sdk` annotation in the `OpenTelemetryCollector` custom resource. This annotation instructs the {OTelOperator} to inject some of the following OpenTelemetry SDK environment variables into your pod, depending on the `Instrumentation` CR:
...
The following exporter TLS CA configuration example uses a config map:
...
The following exporter mTLS configuration example uses a Secret:
...
The following example shows the configuration for CA bundle injection by using a config map and `Instrumentation` CR:
...
The following `PodSpec` annotation enables injection:
