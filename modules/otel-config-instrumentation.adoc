...
The {OTELName} injects and configures the OpenTelemetry auto-instrumentation libraries into your workloads. This product supports injecting instrumentation libraries for Go, Java, Node.js, Python, .NET, and the Apache HTTP Server.

Auto-instrumentation in OpenTelemetry refers to the capability where the framework automatically instruments an application without manual code changes. This allows developers and administrators to observe their applications with minimal effort and changes to the existing codebase.
...
You can learn more about sidecar injection in the xref:otel-sending-traces-logs-and-metrics-to-otel-collector.adoc[OpenTelemetry Collector sidecar injection].
...
|`env`
|Defines common environment variables for all instrumentation types.
|
...
.Permissions required for Go auto-instrumentation in an OpenShift cluster.
[source,yaml]
----
...
[TIP]
====
The CLI command to apply permissions for Go auto-instrumentation is as follows:
[source,terminal]
----
...
The instrumentation is injected on the first container that is available by default according to the pod specification. You can also specify target container names for injection.
