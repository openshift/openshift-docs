...
# ...
----
<1> Specifies the OpenTelemetry Protocol (OTLP) endpoint using the HTTPS scheme and TLS.
...
[NOTE]
====
You can provide the CA certificate in a config map or Secret. If you provide it in both, the config map has higher precedence than the Secret.
====
...
[id="otel-configuration-auto-instrumentation-exporter_{context}"]
