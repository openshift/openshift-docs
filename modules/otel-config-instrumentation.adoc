...
[NOTE]
====
Note that this annotation is used when you want to inject a single instrumentation in multiple containers. For instrumenting multiple containers with different instrumentations, check the instructions below.
====

[id="otel-multi-container-pods-multi-instrumentations_{context}"]
=== Multi-container pods with multiple instrumentations

In those situations where the Pod has multiple containers and we want to inject different instrumentations for the different pods, we need to use the following annotations:

.Multi instrumentation for Java applications
[source,yaml]
----
instrumentation.opentelemetry.io/java-container-names: "<container_1>,<container_2>"
----

.Multi instrumentation for NodeJS applications
[source,yaml]
----
instrumentation.opentelemetry.io/nodejs-container-names: "<container_1>,<container_2>"
----

.Multi instrumentation for Python applications
[source,yaml]
----
instrumentation.opentelemetry.io/python-container-names: "<container_1>,<container_2>"
----

.Multi instrumentation for DotNet applications
[source,yaml]
----
instrumentation.opentelemetry.io/go-container-names: "<container_1>"
----

.Multi instrumentation for ApacheHTTPD applications
[source,yaml]
----
instrumentation.opentelemetry.io/apache-httpd-container-names: "<container_1>,<container_2>"
----

.Multi instrumentation for NGINX applications
[source,yaml]
----
instrumentation.opentelemetry.io/inject-nginx-container-names: "<container_1>,<container_2>"
----

.Multi instrumentation SDK
[source,yaml]
----
instrumentation.opentelemetry.io/sdk-container-names: "<container_1>,<container_2>"
----

[NOTE]
====
You can only inject one instrumentation to one container: multiple instrumentations for a single container application are not supported.
====
