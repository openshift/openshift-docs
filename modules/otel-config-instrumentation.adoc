...
The following OpenTelemetry SDK environment variables in your pod are injected by the {OTELName} Operator when `instrumentation.opentelemetry.io/inject-sdk` is used.
...
The inject annotation can accept following values:
...
[IMPORTANT]
====
This feature by default injects not supported upstream instrumentation libraries.
====
