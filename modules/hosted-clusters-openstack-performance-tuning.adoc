...
:navtitle: Tune performance for hosted cluster nodes
[id="hosted-clusters-openstack-performance-tuning_{context}"]
= Tuning performance for hosted cluster nodes

[role="_abstract"]
To run high-performance workloads on {hcp} on {rh-openstack-first}, you can create a performance profile and deploy a tuned `NodePool` resource.
