...
$ oc create secret generic my-registry-credentials \ # <1>
  --from-file=config.json=/path/to/config.json # <2>
----
<1> The name of the secret, in this case, `my-registry-credentials`
<2> The path name of the `config.json` file
...
. In the steps of the task, access the directory using the path in the `$(workspaces.<workspace_name>.path)` environment variable, for example, `$(workspaces.dockerconfig.path)`

. When running the task, specify the secret for the named workspace, as in the following example command:
+
[source, terminal]
----
