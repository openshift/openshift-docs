...
[NOTE]
====
If the previous command returns `false` or an empty result, hosted control planes is not enabled and DPU provisioning fails. Enable the `hypershift` component on the `MultiClusterEngine` resource. In current {mce-short} versions the component is named `hypershift`; earlier versions use `hypershift-preview`.
