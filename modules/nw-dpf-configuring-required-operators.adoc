...
[NOTE]
====
The manual installation procedure creates `mce` with `hypershift` enabled. If the previous command returns `true`, do not patch the resource. If it returns `false` or an empty result, hosted control plane is not enabled and DPU provisioning fails.
