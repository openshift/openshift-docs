...
[role="_abstract"]
Network observability is enabled by default on new {product-title} clusters, providing immediate visibility into network traffic patterns through the {product-title} web console without additional post-installation configuration.
...
The network observability installation behavior depends on the `installationPolicy` field in the Network custom resource definition (CRD):
