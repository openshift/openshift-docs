...
[id="generate-diagnostic-bundle-roxctl-admin-password_{context}"]
= Generating a diagnostic bundle by using the administrator password

[role="_abstract"]
You can generate a diagnostic bundle by using the {product-title-short} administrator password with the `roxctl` CLI.

.Prerequisites

* To generate a diagnostic bundle, you need `read` permission for the `Administration` resource.
* You must have configured the {product-title-short} administrator password and central address.
