...
:navtitle: Enable 64k pages on the {op-system-first} kernel
:chunk-to-content:
[id="multi-architecture-enabling-64k-pages_{context}"]
= Enabling 64k pages on the {op-system-first} kernel

[role="_abstract"]
You can enable the 64k memory page in the {op-system-first} kernel on the 64-bit ARM compute machines in your cluster. The 64k page size kernel specification can be used for large GPU or high memory workloads. 
...
`spec.kernelType`:: Specifies your desired kernel type. Valid values are `64k-pages` and `default`.
+
[NOTE]
====
