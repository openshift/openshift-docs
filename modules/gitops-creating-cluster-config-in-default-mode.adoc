...
[id="gitops-creating-cluster-config-in-default-mode_{context}"]
= Create cluster configuration in default mode by using GitOps CLI

[role="_abstract"]
You can create cluster configuration applications in the default mode by using the {gitops-shortname} `argocd` CLI.
...
. Log in to the Argo CD server by using the `admin` account password:
+
[IMPORTANT]
====
