...
[id="add-triggers_{context}"]
= Add triggers to a pipeline
...
+
* To create an `EventListener` resource by using a secure HTTPS connection:
+
...
+
* To create an `EventListener` resource by using an insecure HTTP connection:
+
.. Create the `EventListener` resource.
