...
= Specifying configurations for multiple Git providers

You can specify configurations for multiple Git providers, or multiple configurations for the same Git provider, to use in different task runs and pipeline runs.

You need to first add details in the `TektonConfig` custom resource (CR) with your unique identifier key prefix. Then you pass the unique `configKey` parameter when creating `TaskRun` and `PipelineRun` resources.

If no `configKey` parametr is passed, the default configuration is used. You can also specify default configuration by setting the `configKey` value to `default`.
...
. When creating task runs and pipeline runs, specify which configuration to use:
+
.Example of pipeline run that uses the configuration with `test2` value
[source,yaml]
----
