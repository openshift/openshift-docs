...
# ...
----
`# configuration 1`:: The default configuration to use if no `configKey` key is provided or the key is provided with the `default` value.
`# configuration 2`:: The configuration used if the `configKey` key is passed with the `test1` value.
`# configuration 3`:: The configuration used if the `configKey` key is passed with the `test2` value.
+
[WARNING]
====
