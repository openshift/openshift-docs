...
If the `Include versions with known issues` feature is disabled and only Recommended versions are displayed, that means no upgrade `versions with known issues` are available.
====
