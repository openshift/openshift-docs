...
The `NonAdminBackupStorageLocation` (NABSL) CR administrator approval workflow is an opt-in feature. As a cluster administrator, you must explicitly enable the feature in the DPA by setting the `nonAdmin.requireApprovalForBSL` field to `true`.
...
* To enable the NABSL administrator approval workflow, edit the DPA and perform the following steps:
