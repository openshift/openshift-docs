...
unhealthy for longer than 5 minutes, the controller creates a `MachineRemedation` 
Custom Resource (CR) for the node, provided it is permitted by the 
`MachineDisruptionBudget` controller.

`MachineRemedation` controller monitors the `MachineRemedation` CRs and applies 
