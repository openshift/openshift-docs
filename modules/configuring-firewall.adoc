...
ifdef::oci-agent[]
When using a firewall, make additional configurations to the firewall so that {product-title} can access the sites that it requires to function. You must configure the firewall before you attempt to install {product-title} on {oci-first}.
...
. Set your firewall's allowlist to any site that provides resources for a language or framework that your builds require.
...
Alternatively, if you select to not use a wildcard for AWS APIs, you must allowlist the following URLs:
|443
