...
* With this update, {loki-op} supports configuring the workload identity federation on the {gcp-first} by using the Cluster Credential Operator in {product-title} 4.17 or later. (link:https://issues.redhat.com/browse/LOG-6421[LOG-6421])

[id="logging-release-notes-6-0-3-bug-fixes_{context}"]
== Bug fixes

* Before this update, the collector used the default settings to collect audit logs, which did not account for back pressure from output receivers. With this update, audit log collection is optimized for file handling and log reading. (link:https://issues.redhat.com/browse/LOG-6034[LOG-6034])

* Before this update, any namespace beginning with `openshift` or `kube` was treated as an infrastructure namespace. With this update, only the following namespaces are treated as infrastructure namespaces: `default`, `kube`, `openshift`, and namespaces that begin with `openshift-` or `kube-`. (link:https://issues.redhat.com/browse/LOG-6204[LOG-6204])
...
* Before this update, pipeline validation could enter an infinite loop if a name was a substring of another name. With this update, stricter name equality checks prevent the infinite loop. (link:https://issues.redhat.com/browse/LOG-6352[LOG-6352])
...
* Before this update, setting up the custom audit inputs in the `ClusterLogForwarder` custom resource with a `LokiStack` output caused errors due to the nil pointer dereference. With this update, the Operator performs the nil checks, which prevents such errors. (link:https://issues.redhat.com/browse/LOG-6441[LOG-6441])

* Before this update, the *ValidLokistackOTLPOutputs* condition was listed in `status.conditions` of the `ClusterLogForwarder` custom resource even for non-`LokiStack` outputs. This update removes the condition and adds the correct validation messaging to the existing output conditions. (link:https://issues.redhat.com/browse/LOG-6476[LOG-6476])
