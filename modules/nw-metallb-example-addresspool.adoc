...
You can set the `autoAssign` field to `false` to prevent MetalLB from automatically reserving IP addresses from the address pool. You can then reserve a single IP address or multiple IP addresses from an IP address pool. To reserve an IP address, append the `/32` CIDR notation to the target IP address in the `spec.addresses` parameter. This setting ensures that only the specific IP address is avilable for assignment, leaving non-reserved IP addresses for application use.

.Example `IPAddressPool` CR that reserves multiple IP addresses
[source,yaml]
----
apiVersion: metallb.io/v1beta1
kind: IPAddressPool
