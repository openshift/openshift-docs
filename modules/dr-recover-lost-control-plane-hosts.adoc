...
+
----
[core@ip-10-0-156-255 ~]$ sudo -E /usr/local/bin/etcd-member-recover.sh 10.0.143.125 <1>
Downloading etcdctl binary..
etcdctl version: 3.3.10
API version: 3.3
...
Starting etcd..
----
<1> The IP address of the original master, where the signer server is running.
