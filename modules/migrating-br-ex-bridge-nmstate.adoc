...
[role="_abstract"]
You can migrate the `br-ex` bridge to NMState as a postinstallation task if you used the `configure-ovs.sh` shell script to set a `br-ex` bridge during cluster installation. NMState provides a declarative and idempotent way to handle configuring the `br-ex` bridge.
