...
[role="_abstract"]
To install the Model Context Protocol (MCP) server for Red Hat {product-title} feature, install the MCP server for Red Hat {product-title} by using Helm chart.

Alternatively, you can install MCP server by using Lifecycle Operator. For information about installing using Lifecycle operator, see _???RHOAI link??? in _Additional resources_.
