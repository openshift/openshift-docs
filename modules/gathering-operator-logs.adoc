...
//OSDOCS-19342 Need a sanity check on this from Dustin Row
ifdef::openshift-rosa[]
+
[NOTE]
====
{product-title} {product-version} cluster nodes running {op-system-first} are immutable and rely on Operators to apply cluster changes. Accessing cluster nodes by using SSH is not recommended. Before attempting to collect diagnostic data over SSH, ask Red Hat SRE or Red Hat Support to collect diagnostic data by running `oc adm must gather`. If the {product-title} API is not available, or the kubelet is not properly functioning on the target node, `oc` operations will be impacted. In such situations, it is possible to access nodes using `ssh core@<node>.<cluster_name>.<base_domain>`.
====
endif::openshift-rosa[]