...
[role="_abstract"]
When you enable schedulable control plane nodes, you allow user-defined pods to run on them, effectively turning them into a hybrid of control plane and worker nodes. This is especially beneficial in resource-constrained environments like compact clusters. It also enables the NUMA Resources Operator to manage and schedule NUMA-aware pods on these nodes, just as it does with worker nodes.
