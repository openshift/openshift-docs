...
** Optional: *Username* and *Password*: If you are configuring *Scanner + Registry*, enter your Quay.io account credentials or Quay robot account credentials to access the registry. Both credential types are valid. If you are using a robot account, enter the user name in the format `_<namespace>+<accountname>_`.
