...
= The {rh-openstack} Cloud Controller Manager (CCM) config map

A {rh-openstack-first} CCM config map defines how your cluster interacts with your {rh-openstack} cloud. By default, this configuration is stored under the `cloud.conf` key in the `cloud-conf` config map in the `openshift-cloud-controller-manager` namespace. For example:
...
[NOTE]
====
You can modify this config map like any other one, but the CCM Operator will override some options. 

For more information, see "The {rh-openstack} Cloud Controller Manager".
====
...
For the Amphora provider, to use the `LEAST_CONNECTIONS` or `SOURCE_IP` methods, you must configure the `"create-monitor=true` option in the `cloud-provider-config` namespace and `ETP:Local` on the load balancing service.
