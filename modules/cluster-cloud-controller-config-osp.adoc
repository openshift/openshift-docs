...
= OpenStack Cloud Controller Manager (CCM) configuration options
...
[id="ccm-config-lb-timeout-annotations"]
=== Load balancer timeout annotations

You can configure timeout settings for Octavia load balancer listeners by setting annotations on a load balancer service. These annotations have no corresponding config map option.

|===
| Annotation | Description

| `loadbalancer.openstack.org/timeout-client-data`
| The frontend client inactivity timeout in milliseconds. The default value is `50000`.

| `loadbalancer.openstack.org/timeout-member-connect`
| The backend member connection timeout in milliseconds. The default value is `5000`.

| `loadbalancer.openstack.org/timeout-member-data`
| The backend member inactivity timeout in milliseconds. The default value is `50000`.

| `loadbalancer.openstack.org/timeout-tcp-inspect`
| The time in milliseconds to wait for additional TCP packets for content inspection. The default value is `0`.
|===

For example, to set timeout values on the load balancer service of the Ingress Controller:

[source,terminal]
----
$ oc annotate service router-default -n openshift-ingress \
  loadbalancer.openstack.org/timeout-client-data=60000 \
  loadbalancer.openstack.org/timeout-member-data=60000
----
