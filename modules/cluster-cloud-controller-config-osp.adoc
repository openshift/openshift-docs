...
A {rh-openstack-first} CCM config file, called `cloud.conf` by convention, comprises several options in an INI format.
...
For the OVN provider, only the `SOURCE_IP` algorithm is supported.
...
The CCM Operator overrides the following options. Do not configure them yourself. They are included in this document for informational purposes only.
