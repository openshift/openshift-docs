...
[role="_abstract"]
You can use the following example pod specifications to create a pod with a label and a pod that uses an anti-affinity preferred rule to attempt to prevent scheduling with that pod.
...
`spec.affinity.podAffinity.preferredDuringSchedulingIgnoredDuringExecution`:: Specifies the parameters for a _preferred_ rule. Alternatively, you can configure a _required_ rule by using the `requiredDuringSchedulingIgnoredDuringExecution` parmeter. 
+
