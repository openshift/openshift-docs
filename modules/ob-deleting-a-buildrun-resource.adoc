...
----
Replace `<buildrun_resource_name>` with the name of the `buildrun` resource.
...
Replace `<buildrun_resource_name>` with the name of the `buildrun` resource.
+
If the `buildrun` resource is successfully deleted, the command will not return any output.
+
[source,terminal]
----
