...
* Use pre-stop hooks to allow the application workload to perform required actions before pod disruption, such as during an upgrade or node maintenance. These hooks allow the pod to save state to persistent storage, offload traffic/service, signal other pods, and so on.
