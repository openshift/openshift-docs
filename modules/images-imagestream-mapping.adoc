...
When the integrated registry receives a new image, it creates and sends an image
stream mapping to {product-title}, providing the image's project, name, tag, and
image metadata.
...
and to tag the image into the imagestream. {product-title} stores complete
...
The following imagestream mapping example results in an image being tagged as
