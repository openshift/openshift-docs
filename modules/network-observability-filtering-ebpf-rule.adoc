...
[role="_abstract"]
Configure the `FlowCollector` custom resource to filter `eBPF` flows using multiple rules to control the flow of packets cached in the eBPF flow table.
...
. Configure the `FlowCollector` custom resource. For more information, see "eBPF flow data filtering examples".

////
- moved to network-observability-ebpf-flow-data-filtering-examples.adoc.
- leaving to verify builds and for Dev/QE reviews.
- will remove when builds and Dev/QE reviews pass
...
//Note: follows slide 33. Passes check:"No remaining issues found for this rule!"
//Following Slide 33 causes build failure. Reverted back to existing format.
