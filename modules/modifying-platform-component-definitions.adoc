...
. Under the **Platform components configuration** section, click the **Red{nbsp}Hat layered products** tab. Components found in Red{nbsp}Hat layered and partner product namespaces are included in the platform definition by default.
+
To change the Red{nbsp}Hat layered products definition, enter one or more namespaces using regular expressions, separated by a pipe `|` symbol. For more information on the syntax structure, see the "RE2 syntax reference".
...
.. Enter the **Namespace rules (Regex)** for this custom component. Enter one or more namespaces using regular expressions, separated by a pipe `|` symbol. For more information on the syntax structure, see the "RE2 syntax reference".
. Click **Save**.