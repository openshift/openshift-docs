...
[NOTE]
====
For further information on feature gates, see xref:../../../nodes/clusters/nodes-cluster-enabling-features.adoc[Enabling features using feature gates].
====
...
* Failure domain: Specifies the relationships between regions/zones in {product-title} and clusters/host groups in vSphere. You define a failure domain by using vCenter objects, such as a `datastore` object. A failure domain defines the vCenter location for {product-title} cluster nodes.
