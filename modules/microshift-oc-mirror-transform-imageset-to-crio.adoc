...
* `imagedigestmirrorset.yaml` and `CatalogSource` YAML files are available in the `oc-mirror-workspace/results-*` directory.
...
[source,terminal]
----
$ cat oc-mirror-workspace/<results-directory>/imagedigestmirrorset.yaml.yaml <1>
----
...
. Transform the imagedigestmirrorset into a format ready for CRI-O configuration by running the following command:
+
[source,terminal]
----
