...
[source,yaml]
----
apiVersion: operator.openshift.io/v2alpha1
...
//So I think Patryk means https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/disconnected_environments/mirroring-in-disconnected-environments#oc-mirror-enclave-support_about-installing-oc-mirror-v2 to replace these steps, and possibly the mirror to mirror steps(?), but I am unclear on it because there is more in that doc than I think we need.
