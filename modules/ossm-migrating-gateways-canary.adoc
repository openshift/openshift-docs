...
The gateway canary migration scenario is recommend for a gradual rollout. It uses multiple gateway versions with maximum control over gateway rollout.

[NOTE]
====
The label for the gateway differs between multitenant and cluster-wide meshes. Return to the either the multitenant or cluster-wide migration guide to understand which labels to use in your specific migration case, and to check if you need to remove the `maistra.io/ignore-namespace: "true"` label.
====
...
----
+
.. If necessary, remove the `istio-injection=enabled` label.

. Deploy a canary gateway:
+
     ...
     template:
       ...
       spec:
         ...
         - name: istio-proxy
           image: auto
----
<1> Must be in in the same namespace as your existing gateway
<2> Must match your existing gateway service selector.
<3> Set to your {SMProduct} 3.0 control plane revision
...
. Repeat steps 5 and 6, incrementally adjusting replica counts until the new gateway handles all traffic to the gateway Service.
