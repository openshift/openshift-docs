...
[NOTE]
====
On the `baremetal` platform, the internal API endpoint, `api-int.<cluster_name>.<base_domain>`, is resolved by the cluster's on-host DNS, which points it at the API virtual IP address. You do not need to create an external `api-int` DNS record unless you configure the cluster to use external DNS records, in which case you must provide the `api-int` record externally as well.

On the `none` platform, the cluster provides no on-host DNS. You must also create a DNS record for `api-int.<cluster_name>.<base_domain>` that resolves to your load balancer.
====
...
. Configure the load balancer to forward traffic to both control plane nodes. The following example uses HAProxy; other load balancers use a different configuration format.
+
