...
[source,terminal]
----
$ curl http://<app>.<cluster_name>.<base_domain>
----
...
[NOTE]
====
In a two-node cluster with fencing, the control plane nodes also run worker workloads. If you add compute nodes later, you must update the load balancer configuration to include those nodes.
====
