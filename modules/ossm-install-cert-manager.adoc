...
== Upgrading `RevisionBased` control plane

If you chose the `RevisionBased` upgrade strategy, then you need to follow this order each time you upgrade your `RevisionBased` control plane:

. Update `istio-csr` deployment with new revision.
. Update `Istio.spec.version`.

This is because `istio-csr` requires you to pass all revisions each time.
