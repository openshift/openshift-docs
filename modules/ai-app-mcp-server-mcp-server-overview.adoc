...
[id="ai-app-mcp-server-mcp-server-overview-goals_{context}"]
...
[id="ai-app-mcp-server-mcp-server-overview-toolsets_{context}"]
== Toolsets

The MCP server for Red Hat OpenShift groups related capabilities into *toolsets*. Each toolset exposes a set of tools, and some toolsets also provide resources and prompts.

Enable toolsets in the MCP server `config.toml` file. The default `core` toolset provides Kubernetes primitives such as `pods_list` and `pods_exec`. Optional toolsets extend the server for specific workflows. For example, the `ovn-kubernetes` and `cni-diagnostics` toolsets expose read-only networking diagnostic tools for OVN-Kubernetes clusters.
