...
As a non-admin user, if you have created a `NonAdminBackupStorageLocation` (NABSL) custom resource (CR) in your authorized namespace, you cannot set the NABSL as the default backup storage location.

In such a scenario, the NABSL CR fails to validate and the `NonAdminController` (NAC) reports back an error message.
...
status:
  ...
  phase: BackingOff
----
1. In this example, the `default` field is set to `true`.
2. The error message reported by NAC.

.Solution

To successfully validate and reconcile the NABSL CR, delete the `default: true` field in your NABSL CR.