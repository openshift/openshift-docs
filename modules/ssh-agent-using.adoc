...
* You configured an {ibm-cloud-name} account to host the cluster. For more information, see "Configuring an {ibm-cloud-name} account".
* If you use a firewall, you configured it to allow the sites that your cluster requires access to. For more information, see "Configuring your firewall".
* You configured the `ccoctl` utility before you installed the cluster. For more information, see "Configuring IAM for {ibm-cloud-name}".
endif::ibm-cloud-vpc[]
...
----
+
Replace `<path>/<file_name>` with the path and file name, such as `~/.ssh/id_ed25519`, of the new SSH key. If you have an existing key pair, ensure your public key is in the your `~/.ssh` directory.
+
ifndef::ibm-power-vs[]
[NOTE]
====
...
----
+
Replace `<path>/<file_name>` with the path and file name of your SSH public key. For example, run the following to view the `~/.ssh/id_ed25519.pub` public key:
+
[source,terminal]
----
...
----
+
Replace `<path>/<file_name>` with the path and file name for your SSH private key, such as `~/.ssh/id_ed25519`.
+
.Example output
[source,terminal]
----
