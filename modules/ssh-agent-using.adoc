...
----
+
The `<path>/<file_name>` parameter specifies the path and file name, such as `~/.ssh/id_ed25519`, of the new SSH key. If you have an existing key pair, ensure your public key is in the your `~/.ssh` directory.
+
ifndef::ibm-power-vs[]
[NOTE]
====
...
----
+
The `<path>/<file_name>` parameter specifies the path and file name for your SSH private key, such as `~/.ssh/id_ed25519`
+
.Example output
[source,terminal]
----
