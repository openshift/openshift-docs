...
[role="_abstract"]
You can define an OpenAPI function in a calling workflow that starts an instance of a called workflow. You then set the address of the called workflow in the deployment configuration. The following procedure uses a service discovery URI, and also shows how to use the DNS name of the Kubernetes `Service` instead.
...
. In the user-defined properties `ConfigMap` of the calling workflow, set the endpoint URL for that key to a service discovery URI. Enclose the URI in `${}`:
+
.Example `order-processing-props` `ConfigMap`
[source,yaml]
----
apiVersion: v1
kind: ConfigMap
...
----
+
The URI omits the namespace segment, so the {ServerlessLogicOperatorName} looks for the `inventory-check` service in the namespace of the calling workflow.
+
[IMPORTANT]
====
The {ServerlessLogicOperatorName} scans the user-defined properties `ConfigMap` of the workflow for service discovery URIs. It does not scan the `application.properties` file of the workflow project.
====
...
. Optional: To use a fixed URL instead of service discovery, set the property to the DNS name of the Kubernetes `Service` that the {ServerlessLogicOperatorName} creates for the called workflow:
+
[source,properties]
----
