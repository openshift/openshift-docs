...
[IMPORTANT]
====
You cannot store images in the `/usr/lib/containers/storage/` directory when building containers. For example, if you update the additional container store setting in `/etc/containers/storage.conf` to point to the `/usr/lib/containers/storage` directory, updates fail.
====
...
. Build the container and share the pull secret with it to allow the pulling of images during the container build procedure by running the following command:
+
