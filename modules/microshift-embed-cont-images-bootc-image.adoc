...
First, you must add instructions to an existing Containerfile to copy the images you want and list them in an accessible place where you can keep track of the image names. Then, you must copy images locally from the `/usr/lib/containers/storage/` directory to the local container storage.
