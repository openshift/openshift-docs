...
If you are moving from {SMProductName} 2.6, you can run {SMProduct} 2.6 side-by-side with {SMProduct} 3.0, in one cluster, without them interfering with each other.
...
* You have installed {SMProduct} Operator 3.0.
* You have created an `IstioCNI` resource in the `istio-cni` namespace.

If you are not running {SMProduct} 2.6, you must upgrade to v2.6 before following this procedure.
