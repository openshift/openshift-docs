...
. View the `self-provisioners` cluster role binding usage by running the
following command:
+
[source,terminal]
----
...
. Remove the `self-provisioner` cluster role from the group
...
** If the `self-provisioners` cluster role binding binds the `self-provisioner`
...
`rbac.authorization.kubernetes.io/autoupdate` parameter value to `false`, as
...
----
+
Consider customizing this project request message to provide more helpful
