...
[IMPORTANT]
====
In each {microshift-short} deployment, you can use one AI model, but potentially more than one model runtime.
====
...
* If the {microshift-short} cluster is not running, or if you want to manually prepare a manifest, you can use the original definition on the disk.
