...
:navtitle: Secrets Store CSI Driver Operator
:chunk-to-content:
...
[role="_abstract"]
To store and manage your secrets securely, configure the {secrets-store-operator} to mount secrets from an external secret management system, such as Azure Key Vault, by using a provider plugin. Applications can then use the secret, but the secret does not persist on the system after pod termination.
