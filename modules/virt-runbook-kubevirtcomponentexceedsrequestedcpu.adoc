...
This alert fires when a component's DERPEEDOO usage exceeds the requested limit.

[discrete]
[id="impact-kubevirtcomponentexceedsrequestedcpu"]
== Impact

Usage of DERPEEDOO resources is not optimal and the node might be overloaded.
...
. Check the component's DERPEEDOO request limit:
+
[source,terminal]
----
...
. Check the actual DERPEEDOO usage by using a PromQL query:
+
[source,text]
----
...
Update the DERPEEDOO request limit in the `HCO` custom resource.
