...
| `-p, --provider` | Specify the Git provider (`auto`, `github`, `gitlab`, `bitbucket-cloud`, `bitbucket-datacenter`, `gitea`).
|===
...
The `tkn pac cel` command automatically detects and parses headers from gosmee `--save` scripts.
...
The `tkn pac cel` command accepts CEL expressions via standard input (stdin), allowing evaluation without using the interactive prompt. For example:
...
[NOTE]
====
When evaluating CEL expressions locally via the CLI, `files.*` variables are always empty, and functions such as `fileChanged`, `fileDeleted`, and `fileModified` are unavailable.
====
