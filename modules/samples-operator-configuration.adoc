...
|Allows you to specify which registry is accessed by image streams for their image content. `samplesRegistry` defaults to `quay.io/openshift-community` for {product-title}.
endif::[]
...
Creation or update of RHEL content does not commence if the secret for pull access is not in place when either `Samples Registry` is not explicitly set, leaving an empty string, or when it is set to quay.io/openshift-community. In both cases, image imports work off of quay.io/openshift-community, which requires credentials.
endif::[]
...
Creation or update of RHEL content is not gated by the existence of the pull secret if the `Samples Registry` is overridden to a value other than the empty string or quay.io/openshift-community.
endif::[]
