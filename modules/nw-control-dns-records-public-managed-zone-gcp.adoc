...
= Creating DNS records on a public managed zone for {gcp-full}

You can create DNS records on a public managed zone for {gcp-full} by using the External DNS Operator.
...
<8> Defines the `route` resource as the source for {gcp-full} DNS records.
