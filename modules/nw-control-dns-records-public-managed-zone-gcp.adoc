...
`domains.filterType`:: By default, all hosted zones are selected as potential targets. You can include your hosted zone.
`domains.matchType`:: Specifies the domain of the target that must match the string defined by the `name` key.
...
`openshiftRouteOptions`:: Specifies options for the source of DNS records.
