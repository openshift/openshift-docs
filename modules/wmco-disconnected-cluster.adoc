...
Using IDMS and ITMS objects to mirror WMCO container images results in the following behaviors that are different than with Linux containers:

* Windows images mirrored IDMS and ITMS objects have specific naming requirements. Mirroring on Windows nodes works on the registry level, where on Linux nodes mirroring acts on the image level. 

For example, if your cluster has an ITMS object that specifies the `quay.io/remote-org/image` should use the `quay.io/my-org/image` mirror registry, any container that uses `quay.io/remote-org/image:tag` would instead use the `quay.io/my-org/image:tag` image .

However, a Windows node takes the ITMS object and uses it to configure registry-wide mirrors. Following the previous example, by specifying a mirror of `quay.io/remote-org/image` to `quay.io/my-org/image`, the Windows node now uses that mirror for all images from `quay.io/remote-org`. This means that `quay.io/remote-org/image:tag` uses the `quay.io/my-org/image:tag` image, as expected, but another container using `quay.io/remote-org/different-image:tag`
...
For this reason, it is recommended that you specify container images using a digest, and use an IDMS instead of an ITMS object.
...
Additionally, Windows images mirrored through IDMS and ITMS objects have specific naming requirements. 
