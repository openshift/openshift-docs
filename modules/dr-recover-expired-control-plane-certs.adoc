...
[role="_abstract"]
Approve pending `node-bootstrapper` certificate signing requests, and kubelet serving CSRs on user-provisioned clusters, to complete recovery after control plane certificates expire.

The cluster can automatically recover from expired control plane certificates. However, you must manually approve the pending `node-bootstrapper` certificate signing requests (CSRs) to recover kubelet certificates. For user-provisioned installations, you might also need to approve pending kubelet serving CSRs.
...
where:
+
`csr-2s94x`:: A pending kubelet service CSR (for user-provisioned installations).
`csr-4hl85`:: A pending `node-bootstrapper` CSR.
