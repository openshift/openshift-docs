...
ifdef::migrating-3-4[]
* You cannot migrate from {product-title} 3.7 to {product-version} because certain API `GroupVersionKinds` (GVKs) that are used by the source cluster are deprecated. You must manually update the GVKs after migration. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1817251[*BZ#1817251*])

* Upgrading the {mtc-short} from {product-title} 3.7 requires a manual workaround to delete the Velero and Restic resources. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1861971[*BZ#1861971*])
endif::[]
