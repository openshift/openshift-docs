...
[role="_abstract"]
If a disruption prevents fencing from functioning correctly, you might need to perform manual recovery steps. In this situation, run commands directly on the control plane nodes to recover the cluster. Attempt the five main recovery scenarios in the following order:
...
----
+
Notice the PCSD status shows that my master-0 node in Online, and the other is offline.
...
----
+
The command shows following output
+
[source,terminal]
---- 
WARNING: If node(s) 'master-1.ostest.test.metalkube.org' are not powered off or they do have access to shared resources, data corruption and/or cluster failure may occur
...
----
+ 
Example output showing quorum restored:
+
.Example output
[source,terminal]
----
...
Flags:            2Node Quorate
----
.. Wait minimum 30 seconds for Pacemaker to process the fencing confirmation and begin recovery.
