...
You can create the default revision tag and relabel the namespaces after you have completed the {SMProduct} 2.6 to {SMProduct} 3 cluster-wide migration process by using the {istio} injection label.

The `bookinfo` application is used as an example.

.Prerequisites

* You have completed the {SMProduct} 2.6 to {SMProduct} 3 cluster-wide migration process by using {istio} injection label.
