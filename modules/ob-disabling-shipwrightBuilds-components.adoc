...
[role="_abstract"]
If you do not require `ShipwrightBuilds` custom resource (CR), you can disable it. This action removes the `shipwright-build-controller` and `shipwright-build-webhook` pods from the `openshift-builds` namespace. It optimizes your cluster's performance without affecting the Shared Resources Container Storage Interface (CSI) driver component or the `ShipwrightBuilds` custom resource definition (CRD).
