...
The Control Plane Migration Assistant (CPMA) is a CLI-based tool that assists you in migrating your control plane settings from {product-title} 3.7 (or later) to {product-title} {product-version}. The CPMA processes {product-title} 3 configuration files and generates Custom Resource (CR) manifest files, which are consumed by {product-title} {product-version} Operators.
