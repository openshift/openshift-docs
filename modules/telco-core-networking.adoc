...
[NOTE]
====
Figure 3.2 shows the recommended network topology, where bonding uses ports from the same dual-port NIC. Bonding ports from the same dual-port NIC is recommended. Cross-NIC port bonding is discouraged.
====
