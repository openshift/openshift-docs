...
[role="_abstract"]
You can upgrade your {product-title} cluster to the newest available version through {cluster-manager-first} or the {cluster-manager} command-line interface (CLI). Keeping your cluster up to date ensures you receive the latest features, fixes, and security enhancements.
...
[NOTE]
====
The update path to a brand new release of {product-title} is not available in the stable channel until 45 to 90 days after the initial general availability (GA) of a newer y-stream version.
====