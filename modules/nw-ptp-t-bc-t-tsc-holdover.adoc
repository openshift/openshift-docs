...
On nodes with multiple NICs, one NIC acts as the leading NIC. Other NICs on the node synchronize to the leading NIC through SMA cable connections managed by the `ts2phc` service.

When the upstream source degrades, disconnects, or fails, the leading NIC enters the holdover state, relying on its internal oscillator to maintain accurate time autonomously. In T-BC configurations, the time transmitter (TT) ports continue transmitting timing to downstream devices, signaling a degraded but usable clock quality. Timing remains stable on both the local node and downstream network until the upstream source recovers or the holdover timeout expires.
