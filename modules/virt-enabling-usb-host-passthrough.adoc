...
* The `HostDevices` feature gate is enabled. To ensure that this is the case, use the following command:
+
[source,terminal]
----
...
. Use the `oc patch` command to add the required USB device to the `permittedHostDevices` stanza of the HCO CR. The following example adds a device with vendor ID 045e and product ID 07a5: 
+
[source,terminal]
----
oc patch hyperconverged kubevirt-hyperconverged \
  ...
  -p '{
    "metadata": {
      "annotations": {
        "kubevirt.kubevirt.io/jsonpatch": "[{\"op\": \"add\", \"path\": \"/spec/configuration/permittedHostDevices/usbHostDevices/-\", \"value\": {\"resourceName\": \"kubevirt.io/peripherals\", \"selectors\": [{\"vendor\": \"045e\", \"product\": \"07a5\"}]}}]"
      }
    }
  }'
----
...
* xref:virt-configuring-vm-use-usb-device.adoc#virt-configuring-vm-use-usb-device[Connecting a USB device to a virtual machine]