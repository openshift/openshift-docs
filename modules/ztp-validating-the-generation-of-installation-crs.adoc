...
Apply the installation custom resources (CRs) to the hub cluster in a namespace with name
matching the site name using the following command:
...
If no object is returned, use the following procedure to troubleshoot the ArgoCD pipeline
flow from `SiteConfig` to the installation CRs.
...
. Check for `Status: Conditions:` to view the error logs. For example, setting invalid
