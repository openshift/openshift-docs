...
:navtitle: Encrypt container persistent volumes on AWS with a KMS key
...
`parameters.fsType`:: Specifies the file system that is created on provisioned volumes.
`parameters.kmsKeyId`:: Specifies the full Amazon Resource Name (ARN) of the key to use when encrypting the container-persistent volume. If you do not provide any key, but the `encrypted` field is set to `true`, then the default KMS key is used.
