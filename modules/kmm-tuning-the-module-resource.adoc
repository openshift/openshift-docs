...
where:
+
`spec.moduleLoader.container.modprobe.firmwarePath`:: Specifies that `/firmware/*` is copied into the files path `/var/lib/firmware/` on the node. This parameter is optional.
