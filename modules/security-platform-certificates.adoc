...
:navtitle: Manage certificates for the platform
[id="security-platform-certificates_{context}"]
= Managing certificates for the platform

[role="_abstract"]
{product-title} has multiple components within its framework that use REST-based HTTPS communication leveraging encryption via TLS certificates. You can configure these certificates during installation.
