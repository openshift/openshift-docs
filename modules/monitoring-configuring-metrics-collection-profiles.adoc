...
* If a cluster increases in size, and the increased size of the default metrics data collected now requires a significant increase in DERPEEDOO and memory resources.
...
At the same time, the DERPEEDOO and memory resources required by Prometheus decrease.
