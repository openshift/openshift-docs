...
`data.runtime_config.yaml.networking.externalIps.enabled`:: Specifies whether `networking.externalIps.enable` is changed to `networking.externalIps.enabled` in {product-title-short} 4.7. It is an enum and can be set to `ENABLED` or `DISABLED`.
--
