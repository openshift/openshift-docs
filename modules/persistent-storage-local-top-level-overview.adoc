...
:navtitle: Local storage options
:chunk-to-content:
[id="persistent-storage-local-top-level-overview_{context}"]
= Overview of local storage options

[role="_abstract"]
{product-title} provides three solutions for provisioning node-local storage. Each offers different capabilities for dynamic provisioning and topology awareness. Node-local storage binds workloads to specific nodes.
...
== Overview of {lvms} functionality
