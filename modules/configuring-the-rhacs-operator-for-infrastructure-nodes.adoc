...
+
--
`spec.config.nodeSelector.node-role.kubernetes.io/infra`:: Specifies the `nodeSelector` key and value instruct the Kubernetes scheduler to place the Operator pods on nodes that have the label `node-role.kubernetes.io/infra: ""`.
