...
`spec.config.tolerations.key`:: Specifies the `tolerations` entry, which allows the Operator pods to be scheduled on nodes that have a taint with the `node-role.kubernetes.io/infra` key. This is necessary as infrastructure nodes are usually provided with a taint to prevent non-infrastructure workloads from being scheduled on them.
--
