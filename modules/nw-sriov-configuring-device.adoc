...
====
<14> Optional: Specify whether to enable remote direct memory access (RDMA) mode. The default value is `false`.
<15> Optional: Specify the link type for the virtual functions. You can specify one of the following values: `eth` or `ib`. `eth` is ethernet and `ib` is infiniband. The default value is `eth` if not explicitly configured. When `linkType` is set to `ib`, `isRdma` will be set to `true` by SR-IOV Operator webhook automatically. When `linkType` is set to `ib`, `deviceType` should not be set to `vfio-pci`.
endif::ocp-sriov[]
ifdef::virt-sriov[]
