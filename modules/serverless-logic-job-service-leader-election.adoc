...
= Jobs Service leader election process

[role="_abstract"]
The Jobs Service operates as a singleton service, meaning only one active instance can schedule and execute jobs.

To prevent conflicts when the service is deployed in the cloud, where multiple instances might be running, the Jobs Service supports a leader election process. Only the instance that is elected as the leader manages external communication to receive and schedule jobs.
