...
[NOTE]
====
To specify a single IP address or a range of IP addresses for an address pool, add the `/32` `hostPrefix` to the CIDR notation.

.Example IPAddressPool resource that specifies a range of IP addresses in CIDR notation
[source,yaml]
----
# ...
spec:
...
Confirm that the address pool name, such as `doc-example`, and the IP address ranges shown in the output.
