...
To view only one pending workload starting from position 1 in ClusterQueue run the following command:

[source,terminal]
----
$ oc get --raw "/apis/visibility.kueue.x-k8s.io/v1beta2/clusterqueues/cluster-queue/pendingworkloads?limit=1&offset=1"
----