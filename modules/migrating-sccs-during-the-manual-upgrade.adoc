...
[role="_abstract"]
By migrating the security context constraints (SCCs) during the manual upgrade by using `roxctl` CLI, you can seamlessly migrate the {rh-rhacs-first} services to use the {osp} SCCs, ensuring compatibility and optimal security configurations across Central and all secured clusters.

.Procedure

ifndef::cloud-svc[]
. List all {product-title-short} services deployed on Central and all secured clusters:
endif::cloud-svc[]
ifdef::cloud-svc[]
...
`metadata.namespace`:: Specifies the namespace for the role.
