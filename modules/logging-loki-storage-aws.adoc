...
If your cluster has STS enabled, the Cloud Credential Operator (CCO) supports short term authentication using AWS tokens. Secrets created using CCO utility (`ccoctl`) use a different format to authenticate.

.Example AWS STS secret
[source,yaml]
----
apiVersion: v1
kind: Secret
