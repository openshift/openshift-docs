...
Enable the live migration feature gate in the {mtv-first} by modifying the `.spec` stanza in the forklift controller custom resource (CR).

.Procedure

. Run the following command to modify the CR:
+
[source,terminal]
----
...
. Verify that the feature gate has been enabled by reviewing the CR YAML, as shown in the following example:
+
[source,YAML]
----
# ...
