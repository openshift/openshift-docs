...
= Installing {loki-op} using the CLI
...
. Apply the `openshift-operators-redhat` namespace object YAML configuration that follows:
+
[source,terminal]
----
$ oc apply -f openshift_operators_redhat_namespace.yaml
----
+
...
<2> String. Enables metric service discovery.
--
...
. Apply the namespace object YAML configuration that follows:
+
[source,terminal]
----
$ oc apply -f openshift_logging_namespace.yaml
----
+
[source,yaml]
----
...
. Apply the instance object YAML configuration that follows:
[IMPORTANT]
====
