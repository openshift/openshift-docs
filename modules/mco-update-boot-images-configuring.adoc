...
[role="_abstract"]
For clusters on specific {product-title} platforms, the Machine Config Operator (MCO) manages and updates the boot image that is used to scale up new nodes. This means that the MCO updates the boot images whenever you upgrade your cluster.
...
Enabling the feature updates the boot image to the {op-system-first} boot image version for your cluster. If the cluster is again updated to a new {product-title} version in the future, the boot image is updated again. New nodes created after enabling the feature use the updated boot image. This feature has no effect on existing nodes.
