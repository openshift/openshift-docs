...
== Use case: Including `blockedImages`

The following `ImageSetConfiguration` file excludes the images that return `manifest unknown errors`.

This example also shows how to use `blockedImages` in the `imageSetConfiguration` to skip mirroring an image that fails to mirror due to `unknown manifest error`.
