...
The following `ImageSetConfiguration` file uses a local storage backend which contains images of specific version `4.12.1`, and mirrors {product-title} with specific version `4.13.1` without pruning version `4.12.1`.
...
The following `ImageSetConfiguration` file uses a local storage backend containing images of specific version `4.12.1`, and prunes the specific version of `4.12.1` and mirrors with the specific version of `4.13.1`.
...
The following `ImageSetConfiguration` file uses a local storage backend which includes only the Red Hat Advanced Cluster Security for Kubernetes Operator, version `4.0.1`  in the `stable` channel. It is modified to prune the existing version and update to the newer version.
