...
== Use case: Updating the mirror registry with images in new {product-title} update path

The following `ImageSetConfiguration` file uses a local storage backend and updates the mirror registry content with all {product-title} versions from the minimum version of `4.11.37` to the maximum version of `4.12.15`, and adds mirror registry content with all {product-title} versions from the minimum version of `4.13.4` to the maximum version of `4.13.6`.

.Example `ImageSetConfiguration` file before update
[source,yaml]
----
apiVersion: mirror.openshift.io/v1alpha2
kind: ImageSetConfiguration
...
[discrete]
[id="oc-mirror-image-set-examples-prune-images_{context}"]
== Use case: Pruning the mirror registry in {product-title} update path

The following `ImageSetConfiguration` file uses a local storage backend and prunes the mirror registry content with all {product-title} versions from the minimum version of `4.11.37` to the maximum version of `4.12.15`.
...
[discrete]
[id="oc-mirror-image-set-examples-prune-add-images_{context}"]
== Use case: Pruning the existing mirror registry and adding images in new {product-title} update path
...
[discrete]
[id="oc-mirror-image-set-examples-operator-multi-versions_{context}"]
== Use case: Updating mirror registry content for multiple Operator versions from a minimum to the latest

The following `ImageSetConfiguration` file uses a local storage backend and includes only the Red Hat Advanced Cluster Security for Kubernetes Operator, versions starting at 4.0.1 and later in the `stable` channel and `latest` channel.
...
== Use case: Pruning older mirror registry content and updating with Operator versions from a minimum to the latest
