...
== Mirror registry update

.Mirroring {product-title} with specific versions

Storage:: Local
Existing images version:: 4.12.1
Specific version to be updated:: 4.13.1
  ...
  platform:
    channels:
      ...
      - name: stable-4.13
        ...
        maxVersion: 4.13.1
----
<1> Existing version present in local storage. Since the updated imageset configuration file does not remove this value, the images are not pruned.

.Pruning the existing version and updating to a specific version

A local storage backend contains images of specific version `4.12.1`.
The following `ImageSetConfiguration` file prunes the specific version of `4.12.1` and mirrors with the specific version of `4.13.1`.

.Example updated `ImageSetConfiguration` file
[source,yaml]
----
apiVersion: mirror.openshift.io/v1alpha2
kind: ImageSetConfiguration
  ...
  platform:
    channels:
      - name: stable-4.13 #<1>
        ...
        maxVersion: 4.13.1
----
<1> `stable-4.12` replaced by `stable-4.13` prunes the specific version of `4.12.1`.

.Pruning previous operator version and updating to newer version

A local storage backend includes only the Red Hat Advanced Cluster Security for Kubernetes Operator, version `4.0.1` and later in the `stable` channel. 
The following `ImageSetConfiguration` file prunes the existing version and update to the newer version.
    ...
    - catalog: registry.redhat.io/redhat/redhat-operator-index:v{product-version}
      packages:
        - name: rhacs-operator
          ...
          - name: stable #<1>
----
<1> Removing `minVersion` updates the Operator to the newer version in the `stable` channel.

[id="oc-mirror-image-set-examples-operator-pruning-versions_{context}"]
.Pruning the existing Operator and mirroring a new Operator

A local storage backend includes only the Red Hat Advanced Cluster Security for Kubernetes Operator, versions starting at 4.0.1 and later in the `stable` channel. The following `ImageSetConfiguration` file prunes the Red Hat Advanced Cluster Security for Kubernetes Operator and mirrors a new operator of latest version in the `stable` channel.

.Example `ImageSetConfiguration` file for update
[source,yaml,subs=attributes+]
----
apiVersion: mirror.openshift.io/v1alpha2
kind: ImageSetConfiguration
...
mirror:
  operators:
    - catalog: registry.redhat.io/redhat/redhat-operator-index:v{product-version}
      packages:
        - name: <new-operator-name> #<1>
          channels:
          - name: stable
----
<1> Replacing `rhacs-operator` with `new-operator-name` prunes the Red Hat Advanced Cluster Security for Kubernetes Operator. To add new Operator without pruning the existing Operator, add a new Operator under `mirror.operators.packages` instead of replacing the existing Operator.