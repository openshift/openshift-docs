...
| `--concurrency` 
| Specify and integer value for the number of templates to process in parallel when comparing with resources from the live version. A larger number increases speed but also memory, I/O, and CPU usage during that period. The default value is `4`.

| `-c`, `--diff-config` 
| Specify the path to the user configuration file.

| `-f`, `--filename` 
| Specify a filename, directory, or URL for the configuration resources that you want to use for a comparison with a reference configuration.
...
| `-p`, `--overrides` 
| Specify a path to user overrides.
