...
[role="_abstract"]
{product-title} clusters use OpenID Connect (OIDC) and the AWS Security Token Service (STS) to authenticate Operator access to required AWS resources.

Each production cluster requires its own OIDC configuration. When creating a {product-title} cluster, you can create the OIDC configuration before creating your cluster.
...
[source,terminal]
----
? Would you like to create a Managed (Red{nbsp}Hat hosted) OIDC Configuration Yes
