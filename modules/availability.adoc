...
* Red{nbsp}Hat does not commit to a Recovery Point Objective (RPO) or Recovery Time Objective (RTO). For more information, see link:https://www.redhat.com/licenses/Appendix_4_Red_Hat_Online_Services_20221213.pdf[PRODUCT APPENDIX 4 RED HAT ONLINE SERVICES].
...
The {product-title-managed-short} includes Red{nbsp}Hat Standard and Premium support, which you can access using the link:https://access.redhat.com/support[Red{nbsp}Hat Customer Portal].
You can open support tickets for the product "Red{nbsp}Hat Advanced Cluster Security Cloud Service".

Red{nbsp}Hat support responds to support tickets submitted by Limited Availability customers, while Red{nbsp}Hat Site Reliability Engineers (SREs) proactively monitor the health of {rh-rhacscs-first}.

In addition, a Red{nbsp}Hat Business Unit Solution Architect (BU SA) acts as a hands-on technical liaison between customer experts, Red{nbsp}Hat support, and Red{nbsp}Hat SREs, supporting customers with limited availability.
...
You can delete {product-title-managed-short} using the default delete operations from the link:https://console.redhat.com/[Red{nbsp}Hat Hybrid Cloud Console].
