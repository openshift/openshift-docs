...
// OSDOCS-21049: Machine API row updated for Milestone-2 least-privilege changes.
// SME review needed: Permission changes derived from managed-cluster-config 4.22 vanilla.yaml
// and JIRA description, not from cluster-storage-operator codebase.
// Confirm: (1) compute.images.useReadOnly removal, (2) iam.serviceAccounts.actAs removal,
// (3) roles/iam.serviceAccountUser scoped to osd-worker and osd-control-plane.
...
This component also requires the following {gcp-short} predefined role:

* `roles/iam.serviceAccountUser` (scoped to `osd-worker` and `osd-control-plane` service accounts)
