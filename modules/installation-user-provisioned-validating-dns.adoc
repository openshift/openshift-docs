...
----
+
* `<cluster_name>.<base_domain>`: Replace `<nameserver_ip>` with the IP address of the name server, `<cluster_name>` with your cluster name, and `<base_domain>` with your base domain name.
+
.Example output
[source,terminal]
----
...
where:
+
`api-int.ocp4.example.com`:: Provides the record name for the Kubernetes internal API.
`api.ocp4.example.com.`:: Provides the record name for the Kubernetes API.
+
[NOTE]
====
