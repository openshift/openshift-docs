...
[role="_abstract"]
Apply the identity provider custom resource to {product-title} so users can authenticate through your configured provider.
...
====
ifdef::oidc[]
 . After the OIDC identity provider is configured in {product-title}, log in by using the following command, which prompts for your user name and password:
+
...
. Confirm that the user logged in successfully and the user name displays by running the following command:
+
[source,terminal]
----
$ oc whoami
