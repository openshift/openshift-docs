...
[role="_abstract"]
Apply the identity provider custom resource (CR) to the cluster and verify that users can log in so you can confirm that authentication works for your organization.
...
. Log in to the cluster by running the followinc command and pass in the token to authenticate.
+
[source,terminal]
----
...
. Confirm that the user logged in successfully and that the username is displayed by running the following command.
+
[source,terminal]
----
$ oc whoami
