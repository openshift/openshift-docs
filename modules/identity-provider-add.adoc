...
// GitHub, GitLab, and Google IDPs do not support username/password login commands
ifeval::["{context}" == "configuring-github-identity-provider"]
:no-username-password-login:
endif::[]
ifeval::["{context}" == "configuring-gitlab-identity-provider"]
:no-username-password-login:
endif::[]
...
+
ifdef::oidc[]
. After the OIDC identity provider is configured in {product-title}, log in by running the following command, which prompts for your user name and password:
+
