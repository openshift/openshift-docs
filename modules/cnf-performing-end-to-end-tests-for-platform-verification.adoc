...
* `SriovNetworkNodePolicy`.
* A node matching both the `SriovNetworkNodePolicy` and a machine config that enables SCTP.
...
XT_U32 tests just run pods on different nodes to check iptables rule that utilize xt_u32. The impacts on the cluster are related to running
simple pods on two nodes.
