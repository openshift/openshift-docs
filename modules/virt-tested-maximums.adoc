...
[role="_abstract"]
Maximums are based on a single cluster of the largest possible size for a {VirtProductName} 4.x environment. When you plan an environment, remember that multiple smaller clusters might be the best option for your use case. Approaching the maximum values can reduce performance and increase latency.

[id="vm-maximums_{context}"]
== Virtual machine maximums

Maximums apply to virtual machines (VMs) running on {VirtProductName} and are subject to the limits specified in "Virtualization limits for Red Hat{nbsp}Enterprise Linux with KVM".
...
|Attached disks (virtio) |160 disks |177 disks
