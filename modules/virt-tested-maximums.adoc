...
[role="_abstract"]
Maximums are based on a single cluster of the largest possible size for a {VirtProductName} 4.x environment. Approaching the maximum values can reduce performance and increase latency. Consider using multiple smaller clusters if possible to improve performance.
