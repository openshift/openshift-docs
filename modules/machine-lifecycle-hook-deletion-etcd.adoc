...
[role="_abstract"]
The etcd Operator uses lifecycle hooks to protect etcd quorum during control plane machine deletion, ensuring cluster stability.
...
`name: EtcdQuorumOperator`:: Specifies the name of the `preDrain` lifecycle hook.
`owner: clusteroperator/etcd`:: Specifies the hook-implementing controller that manages the `preDrain` lifecycle hook.