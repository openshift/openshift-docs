...
Registries contain a collection of one or more image repositories, which contain one or more tagged images. Red{nbsp}Hat provides a registry at link:https://catalog.redhat.com/en[quay.io/openshift-community] for subscribers. {product-title} can also supply its own {product-registry} for managing custom container images.
endif::[]
