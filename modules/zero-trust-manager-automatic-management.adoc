...
* You installed the {zero-trust-full} on all clusters that will participate in the federation.
* You installed the {oc-first}.
* You have `cluster-admin` privileges on all participating clusters.
tion endpoints must be publicly accessible for Let's Encrypt HTTP-01 challenge validation.
...
+
Create or update your `SpireServer` resource with the following federation configuration:
+
[source,yaml]
----
    ...
    bundleEndpoint:
      ...
      httpsWeb:
        acme:
          ...
          domainName: federation.<cluster-apps-domain>
...
----
+
Replace `<example.com>`with your endpoint URL or the default value.
+
