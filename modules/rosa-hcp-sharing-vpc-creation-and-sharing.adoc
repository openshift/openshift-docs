...
[role="_abstract"]
You can share subnets within a virtual private cloud (VPC) with another AWS account in your AWS organization.

image::522-shared-vpc-step-1.png[Step one of the shared VPC workflow showing VPC creation and sharing.]

.Procedure

. Create or modify a VPC to your specifications in the link:https://us-east-1.console.aws.amazon.com/vpc/[VPC section of the AWS console]. Ensure you have selected the correct region.
...
+
.. Create the Identity and Access Management (IAM) role for the AWS managed policy link:https://docs.aws.amazon.com/rosa/latest/userguide/security-iam-awsmanpol.html#security-iam-awsmanpol-rosasharedvpcroute53policy[`ROSASharedVPCRoute53Policy`].
+
[source,terminal]
----
...
----
+
. Provide the `Route 53 role` Amazon Resource Name (ARN) and the `VPC endpoint role` ARN to the *Cluster Creator* to continue configuration.