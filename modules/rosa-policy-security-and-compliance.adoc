...
- Configure the {product-title} service to encrypt control plane, infrastructure, and worker node volume data by default using the
AWS managed Key Management Service (KMS) key that Amazon EBS provides.

- Configure the {product-title} service to encrypt customer persistent volumes that use the default storage class with the AWS
...
- Manage Amazon EBS volume storage to ensure enough storage is available to mount as a volume in {product-title}.
...
**Compute:** Secure Amazon EC2, used for {product-title}
...
**Storage:** Provide AWS KMS, which {product-title} uses to
ifdef::openshift-rosa[]
...
**Storage:** Secure Amazon S3, used for the {product-title} service’s built-in container image registry. For more information, see link:https://docs.aws.amazon.com/AmazonS3/latest/userguide/security.html[Amazon S3 security] in the S3 User Guide.
...
- Provide the AWS global infrastructure that {product-title} uses to deliver service functionality. For more information regarding AWS security
...
validation for {product-title}] in the {product-title} User
Guide.
