...
[role="_abstract"]
You can dynamically provision VMware vSphere volumes by using the {product-title} web console to create persistent volume claims with the default `thin` storage class, so that your applications have on-demand access to vSphere storage without manual volume creation.
