...
[role="_abstract"]
In many production environments, you are likely to require a custom `HTTPRoute` object for your {mcpg}.

For example, if any of the following situations apply, use a custom HTTP route:

* An identity provider protects your {mcpg}.
...
* Replace the `spec.rules.backend.Refs.name:` value with the name of your {mcpg}oyment.
...
. Delete the automatically created `HTTPRoute` CRs by running the following command:
+
[source,terminal]
----
