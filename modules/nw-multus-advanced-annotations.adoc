...
name (net2):: The `name` key is the name of the secondary network to associate with the pod.
...
name:: Use the `<name>` as provided when creating the `rawCNIConfig` above.
ips:: Provide an IP address including the subnet mask.
mac:: Provide the MAC address.
+
[NOTE]
====
