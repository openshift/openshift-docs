...
[role="_abstract"]
{pac} supports private repositories by creating or updating a secret in the target namespace with the user token. The `git-clone` task from {artifact-hub} uses the user token to clone private repositories.
...
* link:https://artifacthub.io/packages/tekton-task/tekton-catalog-tasks/git-clone[git-clone task on {artifact-hub}]
