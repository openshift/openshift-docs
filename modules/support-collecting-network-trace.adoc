...
* Optionally, start a `tcpdump` capture for the node by using the following commands:
...
.. Start a Toolbx container by using the following command:
+
[source,terminal]
----
# toolbox create
...
[NOTE]
====
If an existing `toolbox` pod is already running, Toolbx outputs an error. Remove the running Toolbx container by using the `toolbox rm <name>` or `toolbox rm --force <name>` command, where `<name>` is the name of the container. Then spawn a new toolbox container.
====
...
.. Enter the Toolbx container to execute commands by using the following command:
+
[source,terminal]
----
# toolbox enter
----
+
.. Obtain the node's interface names:
+
[source,terminal]
----
# ip ad
...
`/host/var/tmp/my-cluster-node_$(date +%d_%m_%Y-%H_%M_%S-%Z).pcap`:: The `tcpdump` capture file's path is outside of the `chroot` environment.

* Optionally, start a `tcpdump` capture for a specific container on the node by using the following commands:

.. Determine the target container ID by running the following command:
+
[source,terminal]
----
...
`/host/var/tmp/my-cluster-node-my-container_$(date +%d_%m_%Y-%H_%M_%S-%Z).pcap`:: The `tcpdump` capture file's path is outside of the `chroot` environment.
