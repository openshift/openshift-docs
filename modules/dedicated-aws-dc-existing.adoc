...
. In the *Navigation* pane of the {product-title} AWS Account Dashboard, ??? select *Direct Connect*??? and choose *virtual private gateways* and select the virtual private gateway. Choose *View details*.
...
. Optional: Add prefixes to Allowed prefixes, separating them using commas ??or separate them on different lines??.
