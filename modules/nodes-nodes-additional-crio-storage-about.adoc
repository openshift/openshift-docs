...
Use the `additionalLayerStores` field to enable lazy pulling through a third-party storage plugin. External Bring Your Own Storage (BYOS) plugins, such as stargz-snapshotter and nydus-storage-plugin, serve container image layers on-demand through a FUSE file system. The `additionalLayerStores` parameter configures the FUSE mount paths for CRI-O.
+
