...
[NOTE]
====
If you are using the Linux native zstd:chunked format for partial pulling, you do not need to configure the `additionalLayerStores` object. However, you must use a machine config to set `enable_partial_images = "true"` in the `storage.conf` file. With partial pulling, CRI-O retrieves the chunk metadata, determines which chunks are needed, and fetches only those chunks using HTTP range requests rather than downloading the entire compressed layer.
====
+
