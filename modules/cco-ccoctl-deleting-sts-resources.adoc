...
<name>:: Specifies the name that was originally used to create and tag the cloud resources.
ifdef::aws-sts,azure-workload-id[<{cp-name}_region>:: Specifies the {cp} region in which to delete cloud resources.]
ifdef::gcp-workload-id[]
<{cp-name}_project_id>:: Specifies the {cp} project ID in which to delete cloud resources.
<path_to_credentials_requests_directory>:: Specifies the directory containing the credentials requests.
endif::gcp-workload-id[]
ifdef::azure-workload-id[<{cp-name}_subscription_id>:: Specifies the {cp} subscription ID for which to delete cloud resources.]
ifdef::aws-sts[]
+
.Example output
[source,text]
