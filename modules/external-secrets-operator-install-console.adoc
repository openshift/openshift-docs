...
. Enter *{external-secrets-operator-short}* into the filter box.

. Select the *{external-secrets-operator-short}*

. Select the {external-secrets-operator-short} version from *Version* drop-down list, and click *Install*.
//+
//[NOTE]
//====
...
+
* If the `external-secrets-operator` namespace does not exist, it is created for you.
+
.. Select an *Update approval* strategy.
+
* The *Automatic* strategy enables {olm-first} to automatically update the Operator when a new version is available.
+
...
----
+
You can use the {external-secrets-operator-short} only after `external-secrets` pods are up and running.
