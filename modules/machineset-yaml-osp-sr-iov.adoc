...
<radio_network_UUID>:: Enter a network UUID for each port.
<radio_subnet_UUID>:: Enter a subnet UUID for each port.
<uplink_subnet_UUID>:: Enter a subnet UUID for each port.
direct:: The value of the `vnicType` parameter must be `direct` for each port.
false:: The value of the `portSecurity` parameter must be `false` for each port.
+
...
true:: The value of the `configDrive` parameter must be `true`.
