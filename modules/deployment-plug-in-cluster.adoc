...
[NOTE]
====
If you are deploying on {rosa-first}, it is recommended to exclude configurations related to pod security by adding the parameter `--set plugin.securityContext.enabled=false`.
====
endif::openshift-rosa-hcp[]
ifdef::openshift-rosa[]
[NOTE]
====
If you are deploying on {rosa-classic-first} 4.10 and later, it is recommended to exclude configurations related to pod security by adding the parameter `--set plugin.securityContext.enabled=false`.
====
