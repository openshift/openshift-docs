...
[IMPORTANT]
====
RWO and RWX volumes using *mount option by default* is targeted for {product-title} 5.0.
====

To assist you with the upcoming move to the mount option default, {product-title} 4.20, and later, reports SELinux-related conflicts when creating pods, and on running pods, to make you aware of potential conflicts, and to help you resolve them. For more information about this reporting, see the Red{nbsp}Hat Knowledgebase article "OpenShift reports SELinux-related conflicts when creating Pods".

If you are unable to resolve the SELinux-related conflicts, you should proactively opt-out of the future move to mount option as default for selected pods or namespaces. To opt out, see "Opting out of the SELinux mount option default".