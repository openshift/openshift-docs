...
You can create filters to reduce the number of generated network flows. Filtering network flows can help reduce the resource usage of the Network Observability components.
...
<2> Optional. Applies a sampling ratio to limit the number of matching flows to be stored or exported. For example, a value of 10 means 1 matching flow every 10 is kept.