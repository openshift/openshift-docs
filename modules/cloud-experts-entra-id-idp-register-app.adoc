...
[role="_abstract"]
Register your application in Entra ID to establish the OAuth trust relationship that {product-title} requires for identity provider authentication.
...
----
+
The `AAD` (Azure Active Directory) directory at the end of the OAuth callback URL must match the OAuth identity provider name that you will set up later in this process.
