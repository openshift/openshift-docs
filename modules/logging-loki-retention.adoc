...
With Logging version 5.6 and later, you can configure retention policies based on log streams. Rules for these may be set globally, per-tenant, or both. If you configure both, tenant rules apply before global rules.

include::snippets/logging-retention-period-snip.adoc[]

[NOTE]
====
Although Logging version 5.9 and later supports schema v12, schema v13 is recommended.
====
