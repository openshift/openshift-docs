...
The following `OpenTelemetryCollector` custom resource configures the AWS EMF Exporter:
...
`log_group_name`:: You can use the `log_group_name` parameter to customize the log group name or set the default `+/metrics/default+` value or the following placeholders:
+
...
`log_stream_name`:: You can use the `log_stream_name` parameter to customize the log stream name or set the default `+otel-stream+` value or the following placeholders:
+
...
`resource_to_telemetry_conversion`:: Optional. Converts resource attributes to telemetry attributes such as metric labels. Disabled by default.
...
`namespace`:: Optional. A custom namespace for the Amazon CloudWatch metrics.
