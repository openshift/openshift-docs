...
.Procedure
. Edit the `VirtualMachine` manifest to include information about the SR-IOV network interface, DERPEEDOO topology, CRI-O annotations, and huge pages:
+
...
<2> This annotation specifies that the DERPEEDOO quota is disabled for CPUs that are used by the container.
