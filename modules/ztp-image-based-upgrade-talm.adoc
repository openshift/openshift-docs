...
. Create a `PolicyGenTemplate` CR that contains policies for the `Prep` and `Upgrade` stages. Each stage has two policies: the `<stage>-stage-policy` policies are used for stage transition, and the `<stage>-status-policy` policies are for monitoring the stage status.
+
[source,yaml]
----
...
<2> Defines the policy compliance type. The `mustonlyhave` compliant type ensures that the `Prep` stage begins with a clean `spec` section and includes only the fields defined in the policy. This requirement is specific to the `prep-stage-policy`.
<3> Define the seed image, {product-title} version, and pull secret for the upgrade in the `Prep` stage.
<4> Define the OADP `ConfigMap` resources required for backup and restore in the `Prep` stage.
<5> (Optional) Define the `ConfigMap` resource for your user-specific extra manifests in the `Prep` stage.
...
[source,terminal]
----
$ oc get policies -n spoke1 | grep -E "stage-policy|status-policy"
----

.Example output
[source,terminal]
----
ztp-group.group-ibu-prep-stage-policy          inform               NonCompliant          31h
ztp-group.group-ibu-prep-status-policy         inform               NonCompliant          31h
ztp-group.group-ibu-upgrade-stage-policy       inform               NonCompliant          31h
ztp-group.group-ibu-upgrade-status-policy      inform               NonCompliant          31h
----
--
...
.. Monitor the status and wait for the `cgu-ibu-prep` `ClusterGroupUpgrade` to report `Completed` and for the policies to report `Compliant`.
+
--
[source,terminal]
----
...
.. Monitor the status and wait for the `cgu-ibu-upgrade` `ClusterGroupUpgrade` to report `Completed` and for the policies to report `Compliant`.
+
--
[source,terminal]
----
...
spec:
  ...
  enable: true
  managedPolicies:
  - group-ibu-rollback-stage-policy
  - group-ibu-rollback-status-policy
  remediationStrategy:
    canaries:
      - spoke1
    maxConcurrency: 1
