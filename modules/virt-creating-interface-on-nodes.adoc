...
[NOTE]
====
If you set the `maxUnavailable` parameter to `50%` in two NNCP manifests that each apply to the same node, the policy configuration coverage is 100% for that node. To avoid this situation, ensure that only one NNCP manifest applies to a node. 
====
