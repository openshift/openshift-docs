...
where:
+
`name`:: Specifies the name of the cluster-scoped instance.
`namespace`:: Specifies the namespace where you want to run the cluster-scoped instance.
`aggregatedClusterRoles`:: Specifies the value set to `true` enables the creation of aggregated cluster roles. If you do not want to enable the creation of aggregated cluster roles, either do not include this line or set the value to `false`.
+
.Example output
[source,terminal]
----
...
where:
+
`name`:: Specifies the name of the aggregated cluster role.
...
`rules`:: Specifies the no predefined permissions are set. However, when the Operator immediately creates a `<argocd_name>-<argocd_namespace>-argocd-application-controller-view` cluster role, the corresponding predefined `view` permissions are added into the aggregated cluster role.
+
...
where:
+
`labels`:: Specifies the labels match the predefined list of an existing aggregated cluster role.
`name`:: Specifies the name of the `view` cluster role.
...
+
`labels`:: Specifies the labels match the predefined list of an existing aggregated cluster role.
`name`:: Specifies the name of the `admin` cluster role.
