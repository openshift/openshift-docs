...
. Optional: Retrieve the logging information for a task run from a record by using the log record name. To get the log record name, replace `records` with `logs` in the record name. Enter the following command:
+
[source,terminal]
----
