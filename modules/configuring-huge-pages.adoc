...
To minimize node reboots, follow the order of steps in this procedure.
...
where:
+
`name: hugepages`:: Set the `name` of the Tuned resource to `hugepages`.
`profile`:: Set the `profile` section to allocate huge pages.
`cmdline_openshift_node_hugepages`:: The order of parameters is important as some platforms support huge pages of various sizes.
`machineConfigLabels`:: Enable machine config pool based matching.
...
name:: Set the `name` of the Tuned resource to `hugepages`.
profile:: Set the `profile` section to allocate huge pages.
match:: Set the `match` section to associate the profile to nodes with the `hugepages` label.
