...
[role="_abstract"]
Complete these prerequisites to enable Bare Metal as a Service (BMaaS) for dynamic bare-metal host provisioning and lifecycle management, including configuring `BareMetalHost` resources with BMC Redfish credentials, DHCP networking, and Metal³ infrastructure integration.

:FeatureName: Bare Metal as a Service
