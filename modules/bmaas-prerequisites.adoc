...
Web server with images::
  BMaaS does not provide images for deployment on hardware. You must configure a web server with the images and checksums you want to use. The `image` field of the `BareMetalHost` (BMH) spec references these images during deployment. Ensure the bare-metal hosts can reach the web server's URL. Here is an example of an image and checksum you might include:
