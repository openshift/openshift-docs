...
. Searches the directory using the generated filter. If the search does not return exactly one entry, deny access.
...
When a client attempts to connect using username `bob`, the resulting
