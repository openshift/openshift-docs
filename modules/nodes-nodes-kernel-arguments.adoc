...
role (labels):: Applies the new kernel argument only to worker nodes.
name (metadata):: Named to identify where it fits among the machine configs (05) and what it does (adds a kernel argument to configure SELinux permissive mode).
