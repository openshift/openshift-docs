...
:navtitle: Configure the install-config.yaml file
[id="configuring-the-install-config-file_{context}"]
= Configuring the install-config.yaml file

[role="_abstract"]
To configure {product-title} for {ibm-cloud-bm} infrastructure, you can edit the `install-config.yaml` file to set the required IPMI privilege level and hardware parameters for your bare-metal nodes.

The `install-config.yaml` file requires some additional details. Most of the information is teaching the installation program and the resulting cluster enough about the available {ibm-cloud-bm} hardware so that it is able to fully manage it. The material difference between installing on bare metal and installing on {ibm-cloud-bm} is that you must explicitly set the privilege level for IPMI in the BMC section of the `install-config.yaml` file.
...
`bmc.address`::
Specifies the IPMI address with `privilegelevel=OPERATOR`. {ibm-cloud-bm} infrastructure requires this privilege level.
...
. Power off all bare-metal nodes before installing the {product-title} cluster:
+
[source,terminal]
----
