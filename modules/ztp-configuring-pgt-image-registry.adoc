...
. Configure the storage class, persistent volume claim, persistent volume, and image registry configuration in the appropriate `{policy-gen-cr}` CR.
ifeval::["{policy-gen-cr}" == "PolicyGenTemplate"]
For example, to configure settings that apply to multiple clusters, add the following YAML to the file `{group-sno-ranGen}`. Group or site-specific values are provided using `ConfigMap` resources in the `template-values/` directory:
endif::[]
