...
`spec.clusterImageSetNameRef`:: Specifies the cluster image set used for deployment.
`spec.ignitionConfigOverride`:: Configures the cluster-level ignition config override for the discovery stage.
`spec.nodes[].installerArgs`:: Specifies the installation program arguments to preserve the data partition.
`spec.nodes[].ignitionConfigOverride`:: Configures the node-level ignition config override for the installation stage.
