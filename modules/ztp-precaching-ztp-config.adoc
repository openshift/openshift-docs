...
The following example shows a `SiteConfig` with the additional fields:
...
** `spec.clusterImageSetNameRef` specifies the cluster image set used for deployment, unless you specify a different image set in the `spec.clusters.clusterImageSetNameRef` field.
** `spec.clusters.clusterImageSetNameRef` specifies the cluster image set used to deploy an individual cluster. If defined, it overrides the `spec.clusterImageSetNameRef` at the site level.
