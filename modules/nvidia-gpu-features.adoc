...
[role="_abstract"]
Learn about the NVIDIA GPU features, software components, and monitoring tools available to accelerate containerized workloads in {product-title}.
...
+
NVIDIA AI Enterprise includes support for Red Hat {product-title}. 
The following installation methods are supported:
+
* {product-title} on bare metal or {vmw-first} with GPU Passthrough.

* {product-title} on {vmw-first} with NVIDIA vGPU.
...
+
The Node Feature Discovery Operator (NFD) manages the discovery of hardware features and configurations in an {product-title} cluster by labeling nodes with hardware-specific information. NFD labels the host with node-specific attributes, such as PCI cards, kernel, OS version, and so on.
+
