...
----
`--from-file`:: Replace `<secret_name>` with any name. The following example uses a KMS secret called `kms-secrets`.
+
...
# ...
----
`value`:: In the `CONVERTER_DB_LIMIT` variable, set the number of records to convert at the same time in a single transaction.
...
# ...
----
`runAt`:: Specify, in the cron format, when to run the pruning job in the database. This example runs the job at 5:03 AM every Sunday.
`maxRetention`:: Specify the number of days to keep the data in the database. This example retains the data for 30 days.
