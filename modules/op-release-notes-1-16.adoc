...
* Before this update, in {pipelines-shortname} 1.16, you could not cancel a pipeline run by patching the `PipelineRun` object and setting the `spec.status` parameter to `Cancelled` if the first task run in the pipeline run has completed. Instead, a `PipelineRun was cancelled but had errors trying to cancel the TaskRuns and/or Runs` error message was logged. With this update, the pipeline run is cancelled successfully.
