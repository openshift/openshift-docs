...
where:
+
`<keyvault_name>`:: Specifies the name of the {azure-short} Key Vault to be created.
`<keyvault_key_name>`:: Specifies the name of the encryption key to be created.
`<disk_encryption_set_name>`:: Specifies the name of the disk encryption set to be created.
...
----
+
where `<service_principal_id>` specifies the ID of the service principal to be used for installation.
...
----
+
where `<reader_role>` specifies an {azure-short} role with read permissions to the disk encryption set. You can use the `Owner` role or a custom role with the necessary permissions.
