...
. Grant the `collect-infrastructure-logs` cluster roles to the service account:
+
...
* Verify that the collector is listening on the service that has a name in the format `<ClusterLogForwarder_CR_name>-<input_name>`. In this example, the service name is `collector-syslog-receiver`:
+
[source,terminal]
----
$ oc get svc
