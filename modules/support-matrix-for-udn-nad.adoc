...
[NOTE]
====
As of {product-title} 4.19, the Localnet topology is generally available for the `ClusterUserDefinedNetwork` CRs and is the preferred method for connecting your physical network to your virtual one. Alternatively, the `NetworkAttachmentDefinition` CR can leverage secondary networks to create Localnet topologies as well.
====
