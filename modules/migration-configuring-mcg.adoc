...
* You must deploy {rh-storage} by using the appropriate xref:..red_hat_openshift_data_foundation/4.13[OpenShift Data Foundation deployment guide].

.Procedure

. Create a bucket using MCG resources according to xref:..managing_hybrid_and_multicloud_resources/index#doc-wrapper[Managing hybrid and multicloud resources guide].

. Obtain the S3 endpoint, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `Bucket Name` by running the xref:..managing_hybrid_and_multicloud_resources/accessing-the-multicloud-object-gateway-with-your-applications_rhodf#accessing-the-Multicloud-object-gateway-from-the-terminal_rhodf[`describe` command] to the bucket resource created.
