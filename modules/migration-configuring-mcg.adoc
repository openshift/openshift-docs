...
You must retrieve the Multicloud Object Gateway (MCG) bucket credentials, which you need to create a `Secret` custom resource (CR) for the OpenShift API for Data Protection (OADP).
endif::[]
//ifdef::installing-oadp-mcg[]
//endif::[]
...
ifdef::installing-oadp-mcg[]
. Create a MCG bucket. For details, see link:https://docs.redhat.com/en/documentation/red_hat_openshift_data_foundation/4.17/html-single/managing_hybrid_and_multicloud_resources/index[Managing hybrid and multicloud resources].

. Obtain the S3 endpoint, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and bucket name by running the link:https://docs.redhat.com/en/documentation//red_hat_openshift_data_foundation/4.15/html/managing_hybrid_and_multicloud_resources/accessing-the-multicloud-object-gateway-with-your-applications_rhodf#accessing-the-Multicloud-object-gateway-from-the-terminal_rhodf[`describe` command] on the created bucket resource.
...
----
+
You can use the `credentials-velero` file to create a `Secret` object when you install the Data Protection Application. For more information, see xref:./../backup_and_restore/application_backup_and_restore/installing/installing-oadp-mcg.adoc#oadp-about-backup-snapshot-locations_installing-oadp-mcg[About backup and snapshot locations and their secrets].
endif::[]
