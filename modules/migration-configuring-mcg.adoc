...
Retrieve the Multicloud Object Gateway (MCG) credentials and S3 endpoint, which you need to configure MCG as a replication repository for the {mtc-first}.

Retrieve the Multicloud Object Gateway (MCG) credentials, which you need to create a `Secret` custom resource (CR) for {mtc-short}.
endif::[]

ifdef::installing-oadp-mcg[]
Retrieve the Multicloud Object Gateway (MCG) bucket credentials to create a `Secret` custom resource (CR) for {oadp-first}.
endif::[]
//ifdef::installing-oadp-mcg[]
//endif::[]
