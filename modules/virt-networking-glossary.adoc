...
UserDefinedNetwork (UDN):: A namespace-scoped CRD introduced by the user-defined network API that allows tenant owners to create a network that isolates their namespace from other namespaces.

ClusterUserDefinedNetwork (CUDN):: A cluster-scoped CRD introduced by the user-defined network API that allows cluster administrators to create a shared network across multiple namespaces.
