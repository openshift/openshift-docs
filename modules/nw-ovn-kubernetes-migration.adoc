...
[NOTE]
====
This step will not deploy the OVN-Kubernetes immediately. Instead, it will trigger MCO to apply new Machine Config to all the nodes in the cluster as a preparation for OVN-Kubernetes deployment.
====
...
.. To remove custom configuration for the OpenShiftSDN network provider, enter the following command:
+
[source,terminal]
----
$ oc patch Network.operator.openshift.io cluster --type='merge' \
