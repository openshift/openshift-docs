...
= Migrating to the OVN-Kubernetes network plugin (offline migration)

As a cluster administrator, you can change the network plugin for your cluster to OVN-Kubernetes.
During an offline migration, you must reboot every node in your cluster.

[IMPORTANT]
====
While performing the offline migration, your cluster is unavailable and workloads might be interrupted.
Perform the migration only when an interruption in service is acceptable; otherwise, see "Migrating to the OVN-Kubernetes network plugin (live migration)". 
====
