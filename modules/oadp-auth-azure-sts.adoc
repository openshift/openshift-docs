...
----
+
This step is supported only for an Azure cluster.
...
. Annotate the Velero and {oadp-short} service accounts to use Microsoft Entra Workload ID as shown in the following example:
+
[source, terminal]
----
...
. Verify that the Velero service accounts have the correct annotations:
+
[source, terminal]
----
...
. Additionally, create a backup of an application and verify the backup is stored successfully in Azure storage.