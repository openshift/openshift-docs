...
<10> If the specified type is `Network`, you can optionally provide a port. The `port` field must have a value between `1` and `65535`. In this example, the `port` field is set to `53`, the standard port for DNS. If you do not specify a port for the upstream, by default port 853 is tried.
+
[NOTE]
====
