...
[NOTE]
====
Kubernetes labels that contain dots (`.`) or slashes (`/`) are normalized by the collector before the prune filter processes them. Each special character is replaced with an underscore (`_`). When specifying such labels in `notIn`, you can use either the original form or the normalized form — the collector handles both.

For example, to preserve the label `example.com/my-label`, you can specify either `.kubernetes.labels."example.com/my-label"` or `.kubernetes.labels.example_com_my_label` in the `notIn` list.

Labels that contain hyphens (`-`) are also normalized to underscores, but must be specified in their normalized form. For example, to preserve `my-label`, specify `.kubernetes.labels.my_label`.
====
+
[IMPORTANT]
====
