...
To enable this functionality, you can configure the target namespaces in the Argo CD custom resource (CR). The {gitops-title} Operator reconciles the corresponding notification resources only for namespaces that are explicitly defined in the ArgoCD CR.
...
If a namespace is missing from these fields, notification configuration is not processed in that namespace.
====
