...
[role="_abstract"]
To capture authenticated user identities in access logs for all request types, you must create an `AuthPolicy` custom resource (CR) for each of the {mcpg} two listeners.

The {mcpg} uses the two following listeners:
