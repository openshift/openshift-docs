...
[id="restoring-the-3scale-operator-secrets-and-apimanager_{context}"]
= Restoring the 3scale Operator, secrets, and APIManager

You can restore the 3scale Operator resources, and `Secret` and APIManager custom resources (CR) by using the following procedure.
...
+
If you are restoring on a different cluster, install and configure {oadp-short} with `nodeAgent` enabled on the destination cluster. Ensure that the {oadp-short} configuration is same as it was on the source cluster.
