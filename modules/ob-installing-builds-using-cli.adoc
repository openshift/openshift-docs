...
[role="_abstract"]
Use the Command Line Interface (CLI) to install the necessary {builds-title} components in {ocp-product-title}. This method enables scripted and automated deployment, providing users with granular control over configurations required for compiling and containerizing applications within the cluster.
