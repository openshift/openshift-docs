...
[role="_abstract"]
Use the Command Line Interface (CLI) to install the necessary {builds-title} components in {ocp-product-title}. The CLI method provides you with granular control over configurations required for compiling and containerizing applications within the cluster.
