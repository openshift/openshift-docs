...
* You can use the `--help` flag to get more information about the commands.
====
...
To cause {product-title} to re-pull image metadata and image scan results from the associated registry and scanner, add the `--force` flag.

[NOTE]
====
To check specific image scan results, you need a token with both `read` and `write` permissions for the `Image` resource.
