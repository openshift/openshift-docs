...
When you check policy compliance by using the `deployment check`, `image check`, or `image scan` commands, you can specify the output format by using the `-o` option. This option determins how the output of a command is displayed in the terminal.

You can change the output format by adding the `-o` option to the command and specifying the format as either `json`, `table`, `csv`, or `junit`.
...
[NOTE]
====
When you do not specify the `-o` option for the output format:

* The default output format for the `deployment check` and the `image check` commands is `table`.
...
Depending on the format you select, there are different options available to configure the output. The following tabele lists the options for each format.
...
|`json`
|`--compact-output`
|Displays the JSON output in a compact format.
...
a|Use this option to specify link:https://github.com/tidwall/gjson[GJSON paths] to select specific items from the output. For example, to get the *Policy name* and *Severity* for a deplyment check, use the following command:
[source,terminal]
----
$ roxctl -e "$ROX_CENTRAL_ADDRESS" \
  deployment check --file=<yaml_filename> \
...
|`--row-jsonpath-expressions`
|Use this option to specify link:https://github.com/tidwall/gjson[GJSON paths] to select specific items from the output.
