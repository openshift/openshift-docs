...
For backing up volumes, OADP on GCP with Google workload identity federation authentication only supports CSI snapshots.

OADP on GCP with Google workload identity federation authentication does not support VSL backups. For more details, see xref:oadp-gcp-wif-known-issues[Google workload identity federation known issues].
...
* VSL backups finish with a `PartiallyFailed` phase when GCP workload identity federation is configured.
