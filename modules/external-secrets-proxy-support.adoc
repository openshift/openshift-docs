...
The Operator and the operand make use of the OpenShift supported trusted Certificate Authority (CA) bundle for the proxy validations. The Operator and the application it manages need to communicate through a proxy, and uses {product-title} built-in list of trusted authorities to verify that the connection is secure and authentic.
...
. Edit the `spec.appConfig.proxy` section to set the proxy as follows:
+
[source,yaml]
----
...
----
 ...
 Edit the `spec.globalConfig.proxy` section to set the proxy as follows:
+
[source,yaml]
----
apiVersion: operator.openshift.io/v1alpha1
