...
[role="_abstract"]
The OpenTelemetry Collector provides five types of components that handle telemetry:
...
The following example shows an `OpenTelemetryCollector` custom resource file:
...
|service:
  pipelines:
    metrics:
      processors:
|You enable processors for metrics by adding them under `service.pipelines.metrics`.
|
|None
