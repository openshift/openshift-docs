...
Red{nbsp}Hat does not suggest turning off TLS verification. Without TLS verification, an unintended recipient could intercept data.
====
...
For example, if your webhook receiver accepts objects from many sources, you can add `"source": "rhacs"` as an extra field and filter on this value to identify all alerts from {product-title}.
