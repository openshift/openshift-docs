...
[id="telco-upgrade-verifying-the-health-of-the-newly-upgraded-cluster_{context}"]
= Verifying the health of the newly upgraded cluster

Run the following commands after upgrading the cluster to verify that the cluster is back up and running:
...
[NOTE]
====
You might see a few pods still moving after the upgrade.
