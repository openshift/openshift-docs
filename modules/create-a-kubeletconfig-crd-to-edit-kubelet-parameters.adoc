...
[NOTE]
====
If you are applying a kubelet or container runtime config to a custom machine config pool, there is an additional requirement for the custom macine config pool definition. When creating the custom machine config pool, the role in the `machineConfigSelector` must match the name of the custom machine config pool. 
====
