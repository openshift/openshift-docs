...
:navtitle: Enable {aws-short} {sts-first} on an existing cluster
...
`--permissions-boundary-arn`:: Optional: Specify the Amazon Resource Name (ARN) of the {aws-short} IAM policy to use as the permissions boundary for the IAM roles created by the `ccoctl` utility.
