...
$ oc get configmap \
  ...
  --output json | \
  jq --raw-output '.data["service-account-001.pub"]' > serviceaccount-signer.public <1>
----
<1> This procedure uses a file named `serviceaccount-signer.public` as an example.

.. Create an output directory for `ccoctl` generated manifests, and move the public key file to this directory by running the following command:
+
[source,terminal]
----
...
.. Save/note the Amazon Resource Name (ARN) for the IAM identity provider. You can find this information in the final line of the output of the last command.

. Configure the service account issuer.

.. Create an `OIDC_ISSUER_URL` environment variable by running the following command:
+
[source,terminal]
----
...
.. Extract the OIDC issuer URL from the generated manifests in the output directory, and patch the cluster `authentication` config, setting `spec.serviceAccountIssuer` by running the following command:
+
[source,terminal]
----
...
$ ./ccoctl aws create-iam-roles \
  --output-dir ./newstscluster/ \
  --name newstscluster \
  --identity-provider-arn ARN_CREATED_FROM_CREATE_IDENTITY_PROVIDER_COMMAND \
  --region us-east-2 \
