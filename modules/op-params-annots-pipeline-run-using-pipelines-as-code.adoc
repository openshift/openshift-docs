...
[role="_abstract"]
To run pipelines using {pac}, you can create pipeline run definitions or templates as YAML files in the `.tekton/` directory of your Git repository. You can reference YAML files in other repositories using remote URLs, but pipeline runs are only triggered by events in the repository containing the `.tekton/` directory.
