...
* Suspend a cronjob by running this command in the `openshift-logging` project:
+
[source,bash]
----
$ mburke@mburke ~ $ oc patch cronjob curator -p '{"spec":{"suspend":true}}'

cronjob.batch/curator patched
----

* Resume a CronJob by running this command in the `openshift-logging` project:
+
[source,bash]
----
