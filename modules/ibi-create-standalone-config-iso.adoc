...
The `openshift-install` program consumes these resources to generate a configuration ISO that you can attach to your preinstalled target host and complete the deployment.

[NOTE]
====
For more information about the specifications for the `image-based-config.yaml` manifest, see the section "Reference specifications for the image-based-config.yaml manifest".
====
