...
`<service.name>.<service.namespace>.svc`, and is only valid for
internal communications.

If your service is a headless service (does not have a `clusterIP` value set), the generated certificate is also valid for `*.<service.name>.<service.namespace>.svc`.
====
