...
+
NOTE: The number of `linuxptp-daemon` pods depends on how many nodes match the PtpConfig profile. In this example, all 5 worker nodes have PTP-capable network interfaces. For Single Node OpenShift (SNO) clusters, you will see only one `linuxptp-daemon` pod.
