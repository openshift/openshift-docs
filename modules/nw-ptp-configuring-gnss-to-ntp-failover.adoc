...
----
+
IMPORTANT: This configuration uses wildcard patterns for automatic interface detection. The `(?<iface_timeTx>[[:alnum:]]+)` pattern will automatically match your E810 NIC interface names. For Single Node OpenShift (SNO) clusters, replace `$mcp` with `master` in the nodeLabel match. For multi-node clusters using worker nodes as T-GM, use `worker`.
+
...
* `-R 16`: Update rate
* `-s (?<iface_timeTx>[[:alnum:]]+)`: Source interface (wildcard pattern auto-detects E810 interface)
* `-n 24`: Domain number
+
**Failover Configuration:**
+
* `ts2phcOpts --ts2phc.holdover 14400`: 4-hour holdover before switching to NTP
* `chronydConf`: NTP server configuration for failover (clock1.rdu2.redhat.com)
...
+
* `ts2phc.nmea_serialport (?<gnss_serialport>[/\w\s/]+)`: Wildcard pattern auto-detects GNSS serial port (e.g., /dev/ttyGNSS_1700_0)
...
. Check the NodePtpDevice to see if the profile is applied by running the following command, replacing `<node-name>` with your node's hostname:
+
[source,terminal]
----
$ oc describe nodeptpdevice <node-name> -n openshift-ptp
----
+
For example, on a multi-node cluster with worker nodes: `worker-0.cluster.local`
+
For SNO clusters, use the master node name, which you can find by running:
+
[source,terminal]
----
$ oc get nodes
