...
The `buildpacks` cluster build strategy uses Buildpacks to build and push a container image from the source code, without requiring a `Dockerfile`. The configuration details are provided through parameters in the `Build` resource.

The `buildpacks-extender` cluster build strategy supports the image extension feature of Cloud Native Buildpacks(CNB), in addition to all the features offered by the `buildpacks` strategy. The `buildpacks-extender` allows you to modify the builder or run the image using a `Dockerfile`. This is useful for installing system dependencies or applying security configurations that the standard Buildpacks cannot handle. The `Dockerfile` is provided by a Buildpack that supports extensions. To use the extender, set the `cnb-extender-kind` parameter in the `Build` resource.
