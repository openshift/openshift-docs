...
Fine-grained network access control rules can be configured by customers per project by using `AdminNetworkPolicy`, `BaselineAdminNetworkPolicy` and `NetworkPolicy`, objects and the OVN-Kubernetes network plugin.
