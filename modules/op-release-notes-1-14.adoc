...
* With this update, you can use a `StepAction` custom resource (CR) to define a reusable scripted action that you can call from any number of tasks. To use this feature, you must set the `pipeline.options.configMaps.feature-flags.data.enable-step-actions` spec in the `TektonConfig` CR to `true`.
