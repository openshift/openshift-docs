...
|`Subnets is required with ipam.mode is Enabled or unset, and forbidden otherwise`.
|You must set the `spec.network.localnet.subnets` field when using the optional `spec.network.localnet.ipam.mode`.

|Setting two CIDR ranges for `spec.network.localnet.subnets` field requires that one be IPv4 and the other be a IPv6.
|`The ClusterUserDefinedNetwork "localnet-subnets-multiple-ipv4-cidrs-fail" is invalid: spec.network.localnet.subnets: Invalid value: "array": When 2 CIDRs are set, they must be from different IP families`.
