...
|You must set the `mtu` lower than `65536`.
...
.6+|`invalid vlan` or `invalid mode`
3+|One of the following messages is returned when the `spec.network.localnet.vlan` is set incorrect:
h|Reason
h|Message
h|Resolution

|The `spec.network.localnet.vlan.mode` parameter must be set.
|`spec.network.localnet.vlan.mode: Unsupported value: "Disabled": supported values: "Access`
|You must set the `spec.network.localnet.vlan.mode` parameter to `Access` mode.

|The `spec.network.localnet.vlan` parameter must be set when `spec.network.localnet.vlan.mode` is set to `Access` mode.
|`vlan access config is required when vlan mode is 'Access', and forbidden otherwise`.
|You must set `spec.network.localnet.vlan.mode.access` parameter when using `Access` mode.
...
|You must set a value of 1 or greater for `access.id` parameter.
...
|You must set a value of 4094 or less for `access.id` parameter.
|===
