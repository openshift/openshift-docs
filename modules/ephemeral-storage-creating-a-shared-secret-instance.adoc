...
----
`metadata`:: Specifies the name and namespace of the `RoleBinding` CR.
`roleRef`:: Specifies the apiGroup, kind and name of the `Role` CR.
`subjects`:: Specifies the kind, name and namespace of the `ServiceAccount` CR.
...
rules:
  - apiGroups:
    ...
    verbs:
      - use
----
`name`:: Name of the `ClusterRole` CR.
