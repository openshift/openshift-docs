...
You can create a `SharedSecret` Custom Resource (CR) to share the reference `Secret` object across namespaces.
...
. Create a `SharedSecret` CR in the cluster by using the following example configuration:
+
[source,yaml]
----
...
[id="additional-resources_creating-shared-secret-instance"]
