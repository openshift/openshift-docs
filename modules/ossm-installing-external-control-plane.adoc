...
. Install Istio in the Data Plane cluster:
...
. Set up the external {istio} control plane on the Control Plane cluster: CONFIRM IS IN OR ON SHOULD BE USED
...
.. Create a `ServiceAccount` resource in the Control Plane cluster by running the following command:
+
[source,terminal]
----
...
.. Create `Gateway` and `VirtualService` resources so that the sidecar proxies in the data plane cluster can access the control plane by running the following command:
+
[source,terminal]
----
...
.. Create a sample application namespace on the Data Plane cluster by running the following command:
+
[source,terminal]
----
...
.. Verify that the pods in the `sample` namespace have a sidecar injected by running the following command:
+
[source,terminal]
----
...
You should see `2/2` for each pod in the `sample` namespace:
+
[source,terminal]
----
