...
Install {istio} on a control plane cluster and a separate data plane cluster. This installation provides increased security.
...
* You have installed all of the clusters on the same network, and there is direct connectivity between the pods in all of the clusters. NEED TO CONFIRM

* You have installed the {SMProduct} Operator on the control plane cluster and the data plane cluster.

* You have created an {IstioCNI} resource. IS THIS ONLY FOR DATA PLANE CLUSTER?
...
.. Wait for the ingress gateway to be assigned an IP address by running the following command:
+
[source,terminal]
----
$ oc --context "${CTX_CONTROL_PLANE_CLUSTER}" wait --for='jsonpath={.status.loadBalancer.ingress[].ip}' --timeout=300s svc istio-ingressgateway -n istio-system
----
...
[source,terminal]
----
$ DATA_PLANE_API_SERVER=https://<hostname or IP address of the data plane cluster's API server>
----
...
. Install the ingress gateway to expose the sample application to external clients:
...
----
$ oc --context="${CTX_DATA_PLANE_CLUSTER}" apply \
  -f https://raw.githubusercontent.com/istio-ecosystem/sail-operator/main/samples/docs/multicluster/ingress-gateway.yaml -n external-istiod
----

.. Confirm that ingress gateway is running by running the following command:
+
[source,terminal]
----
$ oc get pod -l app=istio-ingressgateway -n external-istiod --context="${CTX_DATA_PLANE_CLUSTER}"
----
+
The command should return output confirming that the gateway is running:
+
.Example output
[source,terminal]
----
...
[source,terminal]
----
$ oc apply -f samples/helloworld/helloworld-gateway.yaml -n sample --context="${CTX_DATA_PLANE_CLUSTER}"
----
...
[source,terminal]
----
$ export INGRESS_HOST=$(kubectl -n external-istiod --context="${CTX_DATA_PLANE_CLUSTER}" get service istio-ingressgateway -o jsonpath='{.status.loadBalancer.ingress[0].ip}'); \
  export INGRESS_PORT=$(kubectl -n external-istiod --context="${CTX_DATA_PLANE_CLUSTER}" get service istio-ingressgateway -o jsonpath='{.spec.ports[?(@.name=="http2")].port}'); \
  export GATEWAY_URL=$INGRESS_HOST:$INGRESS_PORT
----

.. Confirm you can access the `helloworld` application through the gateway by running the following command: 
+
[source,terminal]
----
