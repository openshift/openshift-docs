...
= Select the AWS billing account during cluster deployment using the web console

[role="_abstract"]
When deploying your cluster using {cluster-manager}, you must select the correct AWS billing account for {product-title}.

.Procedure
. To start creating a cluster, select *Create with web interface* of the introductory *Set up {product-title}* page.
+
