...
[WARNING]
====
Do not elevate the permissions of Argo CD instances to be cluster-scoped unless you have rational reason(s) and distinct use case(s). Only an administrative team at all times must manage the instances you elevate. Anyone with access to the namespace of a cluster-scoped instance can elevate their privileges on the cluster to become a cluster administrator themselves.
====
