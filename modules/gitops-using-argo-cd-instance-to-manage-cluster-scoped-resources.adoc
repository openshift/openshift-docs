...
[WARNING]
====
Do not elevate the permissions of Argo CD instances to be cluster-scoped unless you have one or more rational reasons and distinct use cases. Only users with `cluster-admin` privileges must manage the instances you elevate. Anyone with access to the namespace of a cluster-scoped instance can elevate their privileges on the cluster to become a cluster administrator themselves.
====
