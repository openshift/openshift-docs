...
`<roleName>`:: Specifies the Vault Kubernetes authentication role used by the application Service Account.
...
where:
+
`serviceAccountName`:: Specifies the assigns the Kubernetes `ServiceAccount` name, for example, `demo-app-sa`, used by the application pod. This `ServiceAccount` is fundamental for authenticating with HashiCorp Vault, as it is linked to a Vault role that grants permissions to retrieve the necessary secrets.
`volumeMounts`:: Specifies the mounts the vault-secrets volume into the container at the `/mnt/secrets-store` directory.
`volumes`:: Specifies the defines the vault-secrets volume using the `secrets-store.csi.k8s.io` driver and references the `demo-app-creds` `SecretProviderClass`.
