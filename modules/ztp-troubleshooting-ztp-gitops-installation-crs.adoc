...
[source,terminal]
----
$ oc get applications.argoproj.io -n openshift-gitops clusters -oyaml
----

.. Check for the `status.operationState.syncResult.resources` field to view the error logs for the managed cluster. For example, setting an invalid value for `extraManifestPath` in the `SiteConfig` CR raises the following error::
+
[source,text]
----
syncResult:
...
.. To look for a more detailed `SiteConfig` error, use the ArgoCD Dashboard. Click the `siteconfig` resource that ArgoCD is trying to sync and then check the `DESIRED MANIFEST` tab for the `siteConfigError` field.
+
[source,text]
----
