...
Configuring any of the three power states requires a Git repository where you manage your custom site configuration data. The repository must be accessible from the hub cluster and be defined as a source repository for Argo CD. For more information about setting up the repository, see "Preparing the {ztp} site configuration repository".
