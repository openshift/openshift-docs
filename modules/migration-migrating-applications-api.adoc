...
spec:
  destMigClusterRef:
    name: <host_cluster> <1>
    namespace: openshift-migration
  indirectImageMigration: true <2>
  indirectVolumeMigration: true <3>
  migStorageRef:
    name: <migstorage> <4>
    namespace: openshift-migration
  namespaces:
    - <application_namespace> <5>
  srcMigClusterRef:
    name: <remote_cluster> <6>
    namespace: openshift-migration
EOF
----
<1> Specify the name of the source cluster `MigCluster` instance.
<2> Direct image migration is enabled if `false`.
<3> Direct volume migration is enabled if `false`.
<4> Specify the name of the `MigStorage` CR instance.
<5> Specify one or more source namespaces. By default, the destination namespace has the same name.
<6> Specify a destination namespace if it is different from the source namespace.
