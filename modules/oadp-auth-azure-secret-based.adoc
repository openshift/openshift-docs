...
= Authenticating OADP by using a service principal or a storage account access key
...
. The `Secret` is referenced in the `spec.backupLocations.velero.credential` block of the `DataProtectionApplication` CR when you install the Data Protection Application as shown in the following example:
+
