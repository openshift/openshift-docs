...
[IMPORTANT]
====
The {cluster-manager} and user IAM roles are required if you want to use {cluster-manager} to install and manage other {product-title} in the same AWS account. Only remove the roles if you no longer need to use the {cluster-manager} to install {product-title} clusters.
====
