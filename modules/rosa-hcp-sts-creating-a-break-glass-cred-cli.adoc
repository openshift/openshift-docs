...
[role="_abstract"]
You can create a break glass credential to generate temporary cluster-admin credentials for {product-title} clusters that use custom OIDC token issuers. The break glass credential provides a `kubeconfig` file that you can use to access the cluster.
...
----
+
This command starts an interactive command-line interface (CLI) process:
+
[source,terminal]
----
...
----
+
. List the break glass credential IDs, status, and associated users who are available for a cluster called `mycluster` by running the following command:
+
[source,terminal]
----
