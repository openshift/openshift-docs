...
Normally, the {lcao} cleans up the new stateroot when you stop at the `Prep` stage or the {lcao} cleans up the old stateroot when you finalize after a successful upgrade or a rollback.
If this step fails, it is recommended that you inspect the logs to determine why the failure occurred. 

Resolution::

. Check if there are any existing deployments in the stateroot by running the following command directly on the node being upgraded:
+
[source,terminal]
----
$ ostree admin status
