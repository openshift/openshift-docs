...
`<oadp_must_gather_image>`:: Optional: Add this option, for example `quay.io/konveyor/oadp-must-gather:latest`, if you need to gather more information from the {oadp-short} Operator.
`<origin_must_gather_image>`:: Optional: Add this option, for example `quay.io/openshift/origin-must-gather:latest`, if you need to gather more information from the SR-IOV Operator.
...
The condition message and log show which steps failed, as shown in the following example:
...
`restoreStatus`:: To preserve the persistent volumes for your application, you must configure this section as shown.
