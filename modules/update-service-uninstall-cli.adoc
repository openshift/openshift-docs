...
You can use the {product-title} CLI to uninstall the OpenShift Update Service Operator.
...
To uninstall the OpenShift Update Service Operator by using the {product-title} CLI:
...
. Get the name of the OpenShift Update Service Operator operatorgroup:
+
[source,terminal]
----
...
. Delete the operatorgroup (for example, `openshift-update-service-fprx2`):
+
[source,terminal]
----
