...
[role="_abstract"]
After you install the DPF Operator, create the required DPF custom resources, starting with a `DPFOperatorConfig` resource to configure the DPF Operator components, including the provisioning controller, static cluster manager, and SR-IOV device plugin controller. The full set of required resources is `DPFOperatorConfig`, `NodeSRIOVDevicePluginConfig`, `DPUFlavor`, `BFB`, and `DPUDeployment`.
