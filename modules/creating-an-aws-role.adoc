...
.. Create the IAM policy based on the policy definition above, by running the following command:
+
[source,terminal]
----
...
. Create IAM Role for your {product-title} service account to assume.
...
----
+
Replace `<OPENSHIFT_OIDC_PROVIDER_URL>` with the URL of your {product-title} OIDC URL.

. Create an IAM role based on the trust policy defined above, by running the following command:
+
[source,terminal]
----
...
.Verification
* Verify AssumeRole and permissions policy by running the following command:
+
[source,terminal]
----
