...
. Verify that the primary network interface uses the new MTU value by entering the following command. You can apply the command regardless of what configuration method you used.
+
[source,terminal]
----
$ oc debug node/<node_name> -- chroot /host ip link show <interface>
----
** Replace `<interface>` with the name of your primary network inteface name.
