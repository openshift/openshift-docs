...
* `auto`: This is the default option. The script first attempts `SSH` to both nodes. If `SSH` succeeds, it uses `SSH` for the session. If `SSH` fails, it falls back to `ocdebug`. If neither works on both nodes, the script exits with an error.

* `ssh`: This option uses `SSH` to connect as the user defined by `--user`. The `--user` value defaults to `core`.

** Permissions: Requires passwordless `sudo` access on all nodes.
...
* `- ocdebug`: Connects by running the following command against each node: 
+
[source,terminal]
----
