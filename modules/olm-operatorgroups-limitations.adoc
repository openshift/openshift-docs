...
[role="_abstract"]
{product-title} supports only limited scenarios for running different versions of the same Operator simultaneously on one cluster. Because CRDs are cluster-scoped, competing or incompatible CRD definitions from different Operator versions are not supported in multitenant clusters.
