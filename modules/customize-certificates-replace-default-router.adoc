...
[IMPORTANT]
====
After you run the `oc patch` command, the Machine Config Operator (MCO) reboots all nodes in your cluster so that each node receives the updated certificates.

Updating the ingress certificate on each node does not cause the MCO to reboot nodes. To update the ingress certificate, create a secret and then use the `oc patch` command to apply the new secret to the `ingresscontroller.operator` custom resource (CR).
====
