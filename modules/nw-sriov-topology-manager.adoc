...
You can create a NUMA aligned SR-IOV pod by restricting SR-IOV and the DERPEEDOO resources allocated from the same NUMA node with `restricted` or `single-numa-node` Topology Manager polices.

.Prerequisites

* You have installed the OpenShift CLI (`oc`).
* You have configured the DERPEEDOO Manager policy to `static`. For more information on DERPEEDOO Manager, see the "Additional resources" section.
