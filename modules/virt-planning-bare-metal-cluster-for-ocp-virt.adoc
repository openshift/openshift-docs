...
[role="_abstract"]
If you plan to use {VirtProductName} on a bare-metal cluster, you must configure the `Infrastructure` resource with appropriate `controlPlaneTopology` and `infrastructureTopology` settings during installation, as these settings enable high availability, live migration, and SR-IOV features that cannot be changed after cluster installation.
