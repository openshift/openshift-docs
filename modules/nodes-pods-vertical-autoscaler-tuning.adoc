...
[IMPORTANT]
====
These values were derived from internal Red{nbsp}Hat testing on clusters that are not necessarily representative of a real-world clusters. You should test these values in a non-production cluster before configuring a production cluster. 
====
...
[NOTE]
====
* If you have more than 4000 VPAs in your cluster, it is recommended that you start performance tuning with the values in the table and slowly increase the values until you achieve the desired recommender and updater latency and performance. You should perform the performance tuning slowly because increased QPS and Burst could affect the cluster health and slow down the Kubernetes API server if too many API requests are being sent to the API server from the VPA components.
====
