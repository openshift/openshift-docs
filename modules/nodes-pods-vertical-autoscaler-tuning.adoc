...
For guidelines on the resources and rate limits that you could set for each VPA component, the following tables provide baseline values, depending on the size of your cluster and other factors. These values were derived by internal Red{nbsp}Hat testing on clusters that are not necessarily representative of the loads a real-world cluster would experience. You should test these values in a non-production cluster before configuring a production cluster. 
...
** It is recommended that you start performance tuning with the values in the table and slowly increase the values until you achieve the desired recommender and updater latency and performance. 
...
<7> Specifies that the VPA Operator monitors only workloads with a VPA CR. The default is `false`.
<8> Specifies the tuning parameters for the VPA updater.
