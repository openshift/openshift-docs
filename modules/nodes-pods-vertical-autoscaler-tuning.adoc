...
For guidelines on the resources and rate limits that you could set for each VPA component, the following tables provide baseline values, depending on the size of your cluster and other factors. 

[IMPORTANT]
====
These values were derived by internal Red{nbsp}Hat testing on clusters that are not necessarily representative of the loads a real-world cluster would experience. You should test these values in a non-production cluster before configuring a production cluster. 
====

.Requests by containers in the cluster
[cols="1,1,1,1,1,1,1,1,1,3,3"]
|===
...
[NOTE]
====
It is recommended that you set the memory limit on your containers to double the recommended requests in the table. However, because CPU is a compressible resource, setting CPU limits for containers can throttle the VPA. As such, it is recommended that you do not set a CPU limit on your containers.
====
...
[NOTE]
====
* If you have more than 4000 VPAs in your cluster, note the following recommendations: 
** It is recommended that you start performance tuning with the values in the table and slowly increase the values until you achieve the desired recommender and updater latency and performance. You should perform the performance tuning slowly because increased QPS and Burst could affect the cluster health and slow down the Kubernetes API server if too many API requests are being sent to the API server from the VPA components.
====

////
Hiding these two NOTEs as not supported
...
.Admission Rate limits (by custom resource pod creation surge) ^[1]^
[options="header"]
|===
| Component 2+| 1-25 pod creation surge 2+| 26-50 pod creation surge 2+| 50+ pod creation surge
