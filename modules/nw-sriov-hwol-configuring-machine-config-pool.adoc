...
//
// * networking/sriov-performance-hardware-offload.adoc
...
* Verify that the `MachineConfigPool` was created and has the expected node by running the following command:
+
[source,terminal]
----
$ oc get mcp mcp-offloading
----
+
.Example output
[source,terminal]
...
mcp-offloading   rendered-mcp-offloading-a]b2c3d4e5f6a7b8c9d0e1f2a3   True      False      False      1              1                   1                     0                         5m
----

* Verify that the `SriovNetworkPoolConfig` was created by running the following command:
+
[source,terminal]
----
...
----
NAME                              AGE
sriovnetworkpoolconfig-offload    3m
----
