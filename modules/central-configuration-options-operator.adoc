...
//Do these settings still only apply to StackRox Scanner? I assume that we are leaving the Scanner V4 parameters named as they are in 4.7?
...
| Enables Scanner V4. The default value is `default`, which is enabled. To disable Scanner V4, set this parameter to `Disabled`.
//Assume this needs changes - But I'm not sure if this is what it was changed to, please confirm. Also should we point out the danger of disabling Scanner V4? Does disabling Scanner V4 mean that then only the StackRox Scanner will be used and they won't have capabilities of Scanner V4? I assume that StackRox Scanner will still work until it is removed, but is that a valid assumption?
