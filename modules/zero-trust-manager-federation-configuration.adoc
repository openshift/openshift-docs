...
* You installed the {zero-trust-full} on all clusters that participate in the federation.
...
. Check the status of the SPIRE Server by running the following command:
+
[source,terminal]
----
...
----
+
Wait for the `Ready` status to be returned.
...
[NOTE]
====
For `https_spiffe` profile, use the `-k` flag if the certificate is not trusted by your system CA bundle.
====
+
...
`spec.trustDomain`:: Specifies the trust domain of the external cluster you are federating with.
...
. Repeat steps 5-7 on each cluster for every remote cluster that Cluster 1 should federate with. For bidirectional federation, each cluster needs a `ClusterFederatedTrustDomain` resource for every other cluster.
