...
.. If a schema is not available, generate a schema from a running cluster by running the following command:
+
[source,bash]
----
...
+
Policy generator passes the OpenAPI schema to Kustomize to control how list fields are patched.
