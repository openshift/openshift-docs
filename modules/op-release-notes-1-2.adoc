...
== New features
{pipelines-title} Technology Preview (TP) 1.2 is now available on {OCP} 4.6.
{pipelines-title} TP 1.2 is updated to support:

* Tekton Pipelines 0.16.3
* Tekton `tkn` CLI 0.13.1
* Tekton Triggers 0.8.1
* cluster tasks based on Tekton Catalog 0.16
* {ibmpowerProductName} Systems on {OCP} 4.6
* {ibmzProductName} and LinuxONE on {OCP} 4.6
...
[NOTE]
====
Installations in restricted environments are currently not supported on {ibmpowerProductName} Systems, {ibmzProductName}, and LinuxONE.
====
...
* Annotations from the event listener can now be propagated to the deployment, services, and other pods. Note that custom annotations on services or deployment are overwritten, and therefore, must be added to the event listener annotations so that they are propagated.
...
* When you run a multi-arch container image task without command specification on an {ibmpowerProductName} Systems (ppc64le) or {ibmzProductName} (s390x) cluster, the `TaskRun` resource fails with the following error:
+
[source,terminal]
----
