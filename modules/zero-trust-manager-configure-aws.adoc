...
After the Entra ID configuraiton is complete, you can set up Entra ID to be used with {azure-short}.

.Prerequisites

* An Entra ID is configured.

== Configuring an {azure-short} account

. Log into Azure by running the following command:
+
[source,terminal]
----
$ az login
...
----
+
<1> Your unique subscription identifieer.
<2> The ID for your Azure Active Directory instance.
<3> The Azure region where your resource are created.

. Define resource variable names.
+
[source,terminal]
----
$ eexport NAME=ztwim <1>
...
. Set the applicatiion name and namespace.
+
[source,terminal]
----
...
.  Deploy the workload application. Copy the entire command block provided and paste it directory into your terminal. Press *Enter* to execute.
+
[source,terminal]
----
oc apply -f - << EOF
...
== Verifing that the application workload can access the content in the Azure Blob Storage
...
.Verification
Confirm the file was uploaded successfully by listing the files contained.
+
[source,terminal]
----
