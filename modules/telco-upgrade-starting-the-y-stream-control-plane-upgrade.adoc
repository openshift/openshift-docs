...
[id="telco-upgrade-starting-the-y-stream-control-plane-upgrade_{context}"]
= Starting the y-stream control plane upgrade

Once you have determined the full new release that you are moving to, you can run the `oc adm upgrade –to=x.y.z` command.

.Procedure
* Start the y-stream control plane upgrade.
...
The following example shows a potential problem for cluster upgrades on Microsoft Azure:
+
[source,console]
----
