...
<2> The `spec.priority` field supports a maximum of 100 ANP in the values of 0-99 in a cluster. The lower the value the higher the precedence. Because there is no guarantee which policy will take precedence when ANPs are created at the same priority, it is preferable to create ANPs at different priorities so that precedence is well defined.
