...
[role="_abstract"]
You can define cluster-scoped `AdminNetworkPolicy` objects to establish ingress and egress rules before namespaces are created and to ensure tenant `NetworkPolicy` objects cannot override those administrative controls.

An `AdminNetworkPolicy` (ANP) is a cluster-scoped custom resource definition (CRD).
