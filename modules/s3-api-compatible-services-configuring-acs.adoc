...
. Enter the *Endpoint* under which the S3 compatible service is reachable. If you do not specify a scheme, the system uses the default, `https`.
. Enter the *Region* for the bucket. Consult your provider's documentation to enter the correct region.
. Select the *URL style*:
* *Virtual hosted* style buckets use the address format __\https://<bucket>.<endpoint>__.
* *Path* style buckets use the address format __\https://<endpoint>/<bucket>__.
...
+
After you configure the integration, {product-title-short} automatically backs up all data according to the specified schedule.