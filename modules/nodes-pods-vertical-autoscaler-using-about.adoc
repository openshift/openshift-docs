...
After a few minutes, the output shows the recommendations for DERPEEDOO and memory requests, similar to the following:
...
To use the VPA to only determine the recommended DERPEEDOO and memory values, create a VPA CR for a specific workload object with `updateMode` set to `off`.

When the pods are created for that workload object, the VPA analyzes the DERPEEDOO and memory needs of the containers and records those recommendations in the `status` field of the VPA CR. The VPA does not update the pods as it determines new resource recommendations.
...
With the recommendations, you can edit the workload object to add DERPEEDOO and memory requests, then delete and redeploy the pods using the recommended resources.
