...
`spec.targetRef.kind`:: Specifies the type of workload object you want this VPA CR to manage.
`spec.targetRef.name`:: Specifies the name of the workload object you want this VPA CR to manage.
`spec.updatePolicy.updateMode`:: Specifies the mode as `Auto`, `Recreate`, `Initial`, or `Off`. Use the `Recreate` mode rarely, only if you need to ensure that when the resource request changes the pods restart.
