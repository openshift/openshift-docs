...
{SMProductName} 2.6 supported one form of multi-cluster, _federation_, which was introduced in {SMProduct} 2.1. Each cluster maintained its own independent control plane in this topology, with services only shared between those meshes on an as-needed basis.
...
{SMProduct} 3.2 introduces support for the following {istio} multi-cluster topologies as well:
