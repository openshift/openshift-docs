...
----
+
* When set to `true`, `AMBIENT_ENABLE_MULTI_NETWORK` and `AMBIENT_ENABLE_MULTI_NETWORK_INGRESS` enable ingress gateways to route requests to clusters on remote networks. Without this configuration, ingress gateways keep traffic local.
...
----
+
* With both `AMBIENT_ENABLE_MULTI_NETWORK` and `AMBIENT_ENABLE_MULTI_NETWORK_INGRESS` set to `true`, ingress gateways route requests to clusters on remote networks.
