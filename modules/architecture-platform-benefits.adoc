...
plane machines, but you can use {op-system-base-full} ({op-system-base}) as the operating
system for compute machines, which are also known as worker machines. If you choose to use {op-system-base} workers, you
...
{op-system-base} worker machines, the control plane benefits from the streamlined update
process, but you must perform more tasks to upgrade the {op-system-base} machines.
endif::openshift-dedicated,openshift-rosa[]
