...
[role="_abstract"]
You can configure alert retention settings by using the {product-title-short} portal or the {product-title} API.
...
. Under the *Data Retention Configuration* section, update the number of days for each type of data. To save a type of data forever, set the retention period to `0` days.
