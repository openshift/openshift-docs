...
* Optional: You have installed and configured the Cloud Credential Operator in your cluster.
...
. When the object storage secret is created, add the `credentialMode` parameter to the `TempoStack` custom resource:
+
[source,yaml]
----
...
////
The previous YAML block for the `TempoMonolithic` custom resource:
+
[source,yaml]
----
apiVersion: tempo.grafana.com/v1alpha1
