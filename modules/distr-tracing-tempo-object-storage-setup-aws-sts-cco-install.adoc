...
* Optional: You have installed and configured the CCO in your cluster.

.Procedure

. Create an AWS S3 bucket.

. Create the following `trust.json` file for the AWS Identity and Access Management (AWS IAM) policy for the purpose of setting up a trust relationship between the AWS IAM role, which is created in the next step, and the service account of either the `TempoStack` or `TempoMonolithic` instance:
+
[source,yaml]
----
{
...
. Optional: If you are not using the CCO, skip this step. If you are using the CCO, configure the cloud provider environment for the {TempoOperator}:
+
[source,terminal]
----
...
. When the object storage secret is created, update the custom resource of the {TempoShortName} instance:

** An example for the `TempoStack` custom resource:
+
[source,yaml]
----
...
spec:
  storage:
    secret:
      name: aws-sts
      type: s3
      credentialMode: token-cco # <1>
----
<1> Optional: If you are not using the CCO, omit this line. If you are using the CCO, add this parameter with the `token-cco` value.

** An example for the `TempoMonolithic` custom resource:
+
[source,yaml]
----
apiVersion: tempo.grafana.com/v1alpha1
