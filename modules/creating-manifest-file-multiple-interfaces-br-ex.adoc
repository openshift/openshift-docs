...
[id="creating-manifest-file-multiple-interfaces-br-ex_{context}"]
= Creating an additional OVN-Kubernetes `br-ex` bridge network

You can create an additional `br-ex` bridge network, `br-ex1`, that has a different configuration to the default `br-ex` bridge. You must define the additional bridge in an interface definition of a machine configuration manifest file. The Machine Config Operator uses the manifest to create a new file at `/etc/ovnk/extra_bridge` on the host. The new file includes the name of the bond interface that hosts the additional `br-ex` bridge network.
...
. Injects Ignition configuration files into each node, so that each node receives the additional `br-ex` bridge network configuration.
//. Restarts the node with the modified override file. Remove when content is SME approved.
...
. Uncordons the nodes. After all the nodes return to the `Ready` state, the Operator applies the `k8s.ovn.org/l3-gateway-config` annotation to each node.

For more information about useful situations for the additional `br-ex` bridge and a situation that always requires the default `br-ex` bridge, see "Configuration for a localnet topology".

.Procedure

. Optional: Create an interface connection that your secondary bridge, `br-ex1`, can use by completing the following steps. The example steps show the creation of a new bond and its dependent interfaces that are all defined in a machine configuration manifest file. The additional bridge uses the `MachineConfig` object to form a secondary bond interface.
+
[IMPORTANT]
====
Do not use the Kubernetes NMState Operator to define or a `NodeNetworkConfigurationPolicy` (NNCP) manifest file to define the secondary interface.

Also ensure that the secondary interface or sub-interfaces when defining a `bond` interface are not used by existing an `br-ex` OVN Kubernetes network deployment.
====
+
...
+
.Example configuration for the `extra_bridge` file, `/etc/ovnk/extra_bridge`, that references a secondary interface
[source,text]
----
bond1
----
...
. Observe that the secondary bridge exists on target nodes by reviewing the network interface names on the host node:
+
[source,terminal]
----
...
[source,terminal]
----
$ oc debug node/<node-name> -- chroot /host sh -c "ip a | grep -A1 -E 'br-ex|ens3'
----
+
.Example output that shows the primary interface for `br-ex` as `ens3`
[source,terminal]
----
Starting pod/worker-1-debug ...
To use host binaries, run `chroot /host`
# ...
sh-5.1# ip a | grep -A1 -E 'br-ex|ens3'
