...
----
+
Set the `spec.profile` field to `ambient` and `spec.values.cni.ambient.reconcileIptablesOnStartup` to `true`. `reconcileIptablesOnStartup` ensures that the iptables rules are correctly set up when the CNI starts after pod restarts.
