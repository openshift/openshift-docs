...
* Setting this value to `Container` specifies that logs should go to a sidecar container. When the destination type is set to `Container`, a container called `access-log` is automatically created. Using container logs means that logs might be dropped if the rate of logs exceeds the container runtime capacity or the custom logging solution capacity. You must have a custom logging solution that reads logs from this sidecar.
...
* If you use the `Prefix` setting, you must also specify a prefix in the `httpCaptureCookies.namePrefix` field. For example, the settings of `matchType: Prefix` when the `namePrefix` is "foo" captures a cookie named "foo" or "foobar" but not one named "bar". The first matching cookie is captured.
...
//Q: open QE question on this bullet; did not work during testing--Enabled had to be set manually
...
|`defaultHTTPVersion`
|Sets the HTTP version for the ingress controller. Default value is `1` for HTTP 1.1. Setting up a load balancer for HTTP 2 and 3 is recommended.
...
//Q: Does the admin need to configure Prometheus or Grafana to have incrementing metrics?
