...
. The use of {k8s} Gateway API requires custom resource definitions (CRDs). The CRDs are present by default and generally available on {product-title} 4.19 and later releases. {product-title} 4.18 and earlier releases does not include or provide support for these CRDs.