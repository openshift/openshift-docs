...
[role="_abstract"]
To install {lvms} on clusters using {rh-rhacm-first}, you must create a `Policy` custom resource (CR) and configure the criteria to select the target clusters.

You can also configure the criteria to select the clusters on which you want to install {lvms}.
...
* `spec.objectDefinition.objectDefinition.spec.objectDefinition`: Specifies the namespace configuration.
* `spec.complianceType.spec.object-templates.objectDefinition`: Specifies the `OperatorGroup` CR configuration.
* `spec.objectDefinition.spec.complianceType.complianceType.objectDefinition`:Specifies the `Subscription` CR configuration.
