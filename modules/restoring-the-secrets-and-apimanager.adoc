...
[IMPORTANT]
====
If you are restoring the database on a different cluster than you backed up, ensure that {oadp-short} is installed and configured with `nodeAgent` enabled on the destination cluster as it was on to the source cluster.
====

. Restore the Operator by using the following YAML file:
+
[source,yaml]
----
...
. Manually create the `s3-credentials` Secret by using the following YAML example:
+
[source,yaml]
----
apiVersion: v1
...
. Restore the API manager by using the following YAML:
+
[source,yaml]
----
apiVersion: velero.io/v1
