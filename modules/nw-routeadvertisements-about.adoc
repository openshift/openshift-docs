...
For example, routes are dynamically discovered. This eliminates the need to manually configure routes on each node. Routing for cluster user-defined (CUDN) networks can be announced to the provider network, avoiding manual configuration steps.
...
Services::
Work in concert with the MetalLB Operator to advertise services to the provider network.
