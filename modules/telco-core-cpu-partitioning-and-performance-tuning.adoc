...
Description::
CPU partitioning separates sensitive workloads from general-purpose tasks, interrupts, and driver work queues, improving performance and reducing latency.
The CPUs allocated to those auxiliary processes are referred to as _reserved_ in the following sections.
In Hyper-Threaded systems, a CPU is one hyper-thread.
...
* The reserved CPUs reduce the pod density, because the reserved CPUs are removed from the allocatable capacity of the OpenShift node.
...
[NOTE]
====
Support for cgroup v1 will be removed in {product-title} 4.19.
