...
New in this release::
* https://issues.redhat.com/browse/CNF-12861[CNF-12861: Sustainability: RDS updates for optional support of acpi_idle CPUIdle driver]
* https://issues.redhat.com/browse/CNF-18946[CNF-18946: RDS Expansion Work for 4.21 - System Test Required]
* https://issues.redhat.com/browse/CNF-16828[CNF-16828: Add systemReserved setting to Core and RAN RDS]
* https://issues.redhat.com/browse/CNF-18196[CNF-18196: Add panic_on_unrecovered_nmi to Core and RAN reference]
...
* The operating system requires a certain amount of CPU to perform all the support tasks, including kernel networking.
** A system with just user plane networking applications (DPDK) requires at least one core (2 hyper-threads when enabled) reserved for the operating system and the infrastructure components.
* In a system with Hyper-Threading enabled, core sibling threads must always be in the same poll of CPUs.
...
* To enable the acpi_idle CPUIdle driver (e.g. for Intel FlexRAN), add `intel_idle.max_cstate=0` to the `additionalKernelArgs` list in the PerformanceProfile.
* The TunedPerformancePatch configures the `kernel.panic_on_unrecovered_nmi` sysctl parameter to enable triggering a kernel panic through BMC Non-Maskable Interrupt (NMI) on x86_64 architures.
