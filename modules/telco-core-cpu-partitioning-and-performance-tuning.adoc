...
New in this release::
* link:https://issues.redhat.com/browse/CNF-12861[CNF-12861: Sustainability: RDS updates for optional support of acpi_idle CPUIdle driver]
* link:https://issues.redhat.com/browse/CNF-18946[CNF-18946: RDS Expansion Work for 4.21 - System Test Required]
* link:https://issues.redhat.com/browse/CNF-16828[CNF-16828: Add systemReserved setting to Core and RAN RDS]
* link:https://issues.redhat.com/browse/CNF-18196[CNF-18196: Add panic_on_unrecovered_nmi to Core and RAN reference]
...
* To enable the acpi_idle CPUIdle driver (for example, for Intel FlexRAN), add `intel_idle.max_cstate=0` to the `additionalKernelArgs` list in the `PerformanceProfile` resource.
* The `TunedPerformancePatch` resource configures the `kernel.panic_on_unrecovered_nmi` sysctl parameter to enable triggering a kernel panic through BMC Non-Maskable Interrupt (NMI) on x86_64 architectures.
