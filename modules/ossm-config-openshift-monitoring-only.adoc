...
spec:
  ...
  - path: /stats/prometheus
    ...
    - action: replace
      ...
      sourceLabels: ["__meta_kubernetes_pod_annotation_prometheus_io_port","__meta_kubernetes_pod_ip"]
      targetLabel: "__address__"
    - sourceLabels: ["__meta_kubernetes_pod_label_app_kubernetes_io_name","__meta_kubernetes_pod_label_app"] # <2>
      separator: ";"
      targetLabel: "app"
      action: replace
      regex: "(.+);.*|.*;(.+)"
      replacement: "${1}${2}" # <3>
    - sourceLabels: ["__meta_kubernetes_pod_label_app_kubernetes_io_version","__meta_kubernetes_pod_label_version"] # <4>
      separator: ";"
      targetLabel: "version"
      action: replace
      regex: "(.+);.*|.*;(.+)"
...
<2> Specify the `app` label from `app.kubernetes.io/name` or fallback to `app`.
<3> Specify the first non-empty value.
<4> Specify the `version` label from `app.kubernetes.io/version` or fallback to `version`.
