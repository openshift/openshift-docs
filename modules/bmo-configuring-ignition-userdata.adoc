...
The Fedora CoreOS image requires the `userData` field to use an Ignition configuration instead of a `cloud-init` script. Ignition is the utility Fedora CoreOS uses for initial system configuration.
...
[source,terminal]
----
$ oc create secret generic bmaas-ignition --from-file=userData=ignition-config.yaml -n bmaas
----
...
[source,terminal]
----
$ oc patch baremetalhost <hostname> -n bmaas --type=json -p='[{"op": "add", "path": "/spec/userData", "value": {"name": "bmaas-ignition"}}]'
----
+
<hostname>::
