...
[role="_abstract"]
{product-title} provides a method for setting a custom timeout period for a specific route or Ingress Controller. Additionally, an {aws-full} Classic Load Balancer (CLB) has its own timeout period with a default time of 60 seconds.
