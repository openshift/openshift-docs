...
:navtitle: Operator Catalogs in {product-title}
[id="olm-installing-operators-from-software-catalog-configure_{context}"]
= About Operator Catalogs in {product-title}

[role="_abstract"]
{product-title} includes a catalog source for community Operators, which are developed and distributed by the community rather than provided through the Red Hat Operators catalog.
