  ...
  defaultNetwork:
    ovnKubernetesConfig:
      hybridOverlayConfig:
        ...
        - cidr: <hybrid_cluster_cidr>
          hostPrefix: <host_prefix>
        hybridOverlayVXLANPort: <vxlan_port>
----
where:

<hybrid_cluster_cidr>:: Specifies the CIDR configuration used for nodes on the additional overlay network. The `hybridClusterNetwork` CIDR must not overlap with the `clusterNetwork` CIDR.
<host_prefix>:: Specifies the host prefix for the hybrid overlay network.
