...
.Example: Downgrade a task `mytask` in a namespace to it's older version
[source,terminal]
----
$ tkn hub downgrade task mytask --to olderversion -n mynamespace
----

== hub get
Get a resource manifest by its name, kind, catalog, and version.

.Example: Get manifest for a resource `myresource` from the `tekton` catalog
[source,terminal]
----
...
.Example: Display information about a task `mytask` from the `tekton` catalog
[source,terminal]
----
...
.Example: Install a task `mytask` from the `tekton` catalog
[source,terminal]
----
...
.Example: Reinstall a task `mytask` from the `tekton` catalog
[source,terminal]
----
...
.Example: Upgrade an installed task `mytask` to a new version
[source,terminal]
----
