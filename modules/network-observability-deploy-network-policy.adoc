...
The Network Observability Operator also deploys a network policy for the operator namespace (`openshift-netobserv-operator`) at runtime. This policy is controlled by the `DEPLOY_OPERATOR_NETWORK_POLICY` environment variable in the Cluster Service Version (CSV), which defaults to `true`. The operator network policy includes:

* Egress rules to allow the operator to contact the API server
* Ingress rules to allow webhooks and metrics endpoints
