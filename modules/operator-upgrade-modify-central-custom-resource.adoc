...
+
--
`spec.central.db.isEnabled`:: Specifies that you must not change the value of `IsEnabled` to `Enabled`.
`spec.central.db.persistence.persistentVolumeClaim`:: Specifies whether your cluster uses the existing claim; otherwise, it creates a new claim.
--
