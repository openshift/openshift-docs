...
Maximize power savings by updating the `sysfs` plugin fields, setting an appropriate value for `max_perf_pct` in the `TunedPerformancePatch` CR for the reference configuration. This example based on the `{group-sno-ranGen}` describes the procedure to follow to restrict the maximum CPU frequency.
...
. Update the `{policy-gen-cr}` entry for `TunedPerformancePatch` in the `{group-sno-ranGen}` reference file in `{argocd-folder}`. To maximize power savings, add `max_perf_pct` as shown in the following example:
+
--
