...
[role="_abstract"]
The following table lists the search attributes that you can use while searching and filtering in {product-title}.
...
| Add Capabilities
| Provides the container with additional Linux capabilities, for instance the ability to change files or perform network operations.

| Annotation
| Arbitrary non-identifying metadata attached to an orchestrator object.

| CPU Cores Limit
| Maximum number of cores that a resource can use.

| CPU Cores Request
| Minimum number of cores required for a given resource.
...
| Created Time
| Time and date of secret object creation.

| Deployment
| Name of the deployment.

| Deployment Type
| The type of Kubernetes controller that the deployment uses.
...
| Environment Key
| Key part of a label key-value string that is metadata for further identifying and organizing the environment of a container.

| Environment Value
| Value part of a label key-value string that is metadata for further identifying and organizing the environment of a container.
...
| Image Created Time
| The time and date of image creation.
...
| Image Remote
| Whether an image is remotely accessible.
...
| Image Users
| Name of the user or group that a container image uses when it runs.
...
| Label
| The key part of a label key-value string that serves as metadata for further identifying and organizing images, containers, daemons, volumes, networks, and other resources.

| Lifecycle Stage
| The type of lifecycle stage where this policy applies or where an alert triggers.

| Max Exposure Level
| For a deployment, the maximum level of network exposure for all given ports/services.

| Memory Limit (MB)
| Maximum amount of memory that a resource can use.

| Memory Request (MB)
| Minimum amount of memory required for a given resource.
...
| Role ID
| Role ID that a Kubernetes RBAC role binding references.
...
| Severity
| Level of importance of a violation: Critical, High, Medium, Low.
...
| Violation
| A notification displayed on the *Violations* page when a deployment does not meet the conditions that a policy specifies.
...
| Volume ReadOnly
| Use `true` to search for volumes mounted as read only.

| Volume Source
| The provisioning method for the volume, for example, `persistentVolumeClaim` or `hostPath`.
