...
----
+
The `jq` command adds a new `component_severity` metric descriptor with the labels `Component` and `Severity` to the existing `policyViolations` configuration, then wraps the result in a `config` object for the update request.