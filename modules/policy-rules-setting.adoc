...
With policy as code, {product-title-short} supports both internal and external sources for policies. When using external policies, {product-title-short} defines a Kubernetes native Custom Resource (CR) to streamline policy as code using tools like OpenShift Pipelines and Argo CD. 
