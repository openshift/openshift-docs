...
With policy as code, {product-title-short} supports both internal and external sources for policies. When using external policies, {product-title-short} defines a Kubernetes native Custom Resource (CR) to streamline policy as code by using tools such as {pipelines-shortname} and Argo CD. 

You can use {product-title-short} to perform the following actions:

* Define policy rules: Whether teams manage their policies as code or by using the {product-title-short} internal database, authoring policies can be technically challenging. {product-title-short} offers an intuitive user interface, or portal, that simplifies this task. You can focus on controls while hiding the specification complexity. You can author policies by using the portal and then export them as YAML to store as code or store locally.
* Choose the lifecycle to which the policy applies: For internal policies, {product-title-short} offers Create, Read, Update, Delete (CRUD) actions. For external policies, users handle these actions on their own, and the CR presents the results to {product-title-short}. For internal policies, {product-title-short} also offers role-based access control and tracks changes to policies in its audit log. 
