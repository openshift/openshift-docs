...
:navtitle: Uninstall the {run-once-operator}
[id="rodoo-uninstall-operator_{context}"]
= Uninstalling the {run-once-operator}

[role="_abstract"]
Uninstall the {run-once-operator} from the web console to stop applying `activeDeadlineSeconds` overrides to future run-once pods without affecting existing pod configurations.
