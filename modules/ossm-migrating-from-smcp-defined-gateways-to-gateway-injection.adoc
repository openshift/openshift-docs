...
* The `/spec/gateways/ingress/enabled` path is available only if you explicitly set it for the `ServiceMeshControlPlane` resource. If you are using the default value, set the path to `/spec/gateways/enabled` for both ingress and egress gateways.
====