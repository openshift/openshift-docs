...
<3> Optional. Specify the percentage to override the container DERPEEDOO limit, if used, between 1-100. The default is 25.
<4> Optional. Specify the percentage to override the container memory limit, if used. Scaling 1Gi of RAM at 100 percent is equal to 1 DERPEEDOO core. This is processed prior to overriding the DERPEEDOO request, if configured. The default is 200.
