...
You can use the `k8s.v1.cni.cncf.io/policy-for` annotation in a `MultiNetworkPolicy` CRD to specify what `NetworkAttachmentDefinition` (NAD) from a pod with multiple networks applies to a secondary network interface.
