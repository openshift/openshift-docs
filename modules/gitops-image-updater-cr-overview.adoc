...
[role="_abstract"]
To configure Argo CD Image Updater to track and update images for an application, create an `ImageUpdater` custom resource (CR). The `ImageUpdater` CR specifies which applications to monitor, which images to track, the update strategy, and how to write back updates.
