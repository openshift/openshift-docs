...
[WARNING]
====
For clusters without existing {SMProduct} instances, follow this procedure. {SMProduct} operates cluster-wide, requiring proper scope configuration to prevent conflicts between {istio} control planes. For clusters with {SMProduct} 3 or later, see how to deploy multiple service meshes on a single cluster.
====
