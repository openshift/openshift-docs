...
[role="_abstract"]
Before you install the DPF Operator, configure worker nodes in the management cluster by creating a `MachineConfig` resource and the DPF namespace.
You must create the `MachineConfig` resource for worker nodes with DPUs before adding those nodes to the management cluster.
