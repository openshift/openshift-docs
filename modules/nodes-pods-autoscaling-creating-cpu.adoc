...
You can create a horizontal pod autoscaler (HPA) for an existing DeploymentConfig, ReplicationController or Deployment object
...
* For a Deployment, use `apps/v1`.
<4> Specify the kind of object to scale, either `ReplicationController` , `DeploymentConfig` or `Deployment`.
