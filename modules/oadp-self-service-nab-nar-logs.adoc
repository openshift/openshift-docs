...
As a namespace admin user, you can review the logs for the NAB and NAR CRs by creating a `NonAdminDownloadRequest` (NADR) CR.
...
* The cluster administrator has configured the `DataProtectionApplication` CR to enable {oadp-short} Self-Service.
...
. To review NAB CR logs, create a `NonAdminDownloadRequest` CR and specify the NAB CR as shown in the following example:
+
...
. Verify that the NADR CR is processed by running the following command. Use the `downloadURL` to download and review the NAB logs.
+
[source,terminal]
----
...
status:
  ...
  velero:
    status:
      ...
      phase: Processed # <2>
----
<1> The `status.downloadURL` field has the download URL of the NAB logs.
<2> The `status.phase` is `Processed`.

. Download and analyze the backup information using the `status.downloadURL` URL.

. To review NAR CR logs, create a `NonAdminDownloadRequest` CR and specify the NAR CR as shown in the following example:
+
.Example `NonAdminDownloadRequest` CR
[source,yaml]
----
...
<2> Specify the name of the NAR CR.

. Verify that the NADR CR is processed by running the following command. Use the `downloadURL` to download and review the NAR logs.
+
[source,terminal]
----
...
----
<1> The `status.downloadURL` field has the download URL of the NAR logs.
<2> The `status.phase` is `Processed`.

. Download and analyze the restore information using the `status.downloadURL` URL.