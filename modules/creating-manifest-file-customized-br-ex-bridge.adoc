...
[WARNING]
====
The OpenShift SDN network plugin was removed in {product-title} 4.17 but the following reserved interface names were not removed because {product-title} uses the names for internal use only:
...
If you attempt to use one of these reserved names in situations such as the following examples, the `nmstate` API fails and outputs an error message that shows the `state` parameter for the bridge name set to `Ignore`:

* You want to deploy the interface bridge, such as `ext`, on a different interface than the available interface on a host or server IP address.
* You want to make advanced configurations to the interface bridge, `br-local`, by using the `configure-ovs.sh` shell script. Using this script for these configurations might cause the bridge interface to fail in connecting multiple network interfaces or forwarding data between the interfaces.
====
