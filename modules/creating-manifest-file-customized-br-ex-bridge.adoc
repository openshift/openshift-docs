...
- name: br-ex
  ...
  ipv4:
    enabled: true
    dhcp: true
    auto-route-metric: <route_metric>
  ipv6:
    enabled: true
    dhcp: true
...
<interface_type>:: Specifies the type of interface, such as ethernet.
<interface_state>:: Specifies the requested state for the interface after creation.
<ipv4_enabled>:: Specifies whether IPv4 is enabled. Set to `false` to disable IPv4 and IPv6 in this example.
<node_nic>:: Specifies the node NIC to which the bridge attaches.
<route_metric>:: Specifies the route metric. Set the parameter to `48` to ensure the `br-ex` default route always has the highest precedence (lowest metric). This configuration prevents routing conflicts with any other interfaces that are automatically configured by the `NetworkManager` service.
