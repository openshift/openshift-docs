...
<6> Set the parameter to `48` to ensure the `br-ex` default route always has the highest precedence (lowest metric). This configuration prevents routing conflicts with any other interfaces that are automatically configured by the Network Manager.
...
<7> Set to `48` so that the default Dynamic Host Configuration Protocol (DHCP) route, which is automatically enabled on new network interfaces, does not take precedence over the `br-ex` route.
endif::postinstall-bare-metal-ipi,postinstall-bare-metal-upi[]
