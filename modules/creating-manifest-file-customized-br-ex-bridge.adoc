...
. Use the command line to apply the `MachineConfig` file, which defines your network interfaces, to your cluster configuration:
+
[source,terminal]
----
...
. Apply the NNCP configuration to each node in your cluster by running the following command:
+
[source,yaml]
----
