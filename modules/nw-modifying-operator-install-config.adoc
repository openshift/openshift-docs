...
Advanced configuration customization lets you integrate your cluster into your existing network environment.
...
[source,terminal]
----
$ cat <<EOT > <installation_directory>/manifests/cluster-network-03-config.yml <1>
...
. Edit the `cluster-network-03-config.yml` to define the advanced network configuration for your installation by specifying the `defaultNetwork` field:

** If you configured OpenShift SDN as your cluster network provider you can configure the following settings:
...
... To specify an MTU for the cluster network, enter the following command:
+
[source,terminal]
---
$ cat <<EOT > <installation_directory>/manifests/cluster-network-03-config.yml <1>
...
** If you configured OVN-Kubernetes as your cluster network provider in `install-config.yaml` you can configure the following settings:
