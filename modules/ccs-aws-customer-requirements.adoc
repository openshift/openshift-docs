...
* The customer ensures that AWS service quotas are sufficient to support {product-title} provisioned within the customer-provided AWS account. For more information, see "AWS service quotas" in the __Additional resources__.
...
* The customer must not use the AWS account to elevate their permissions within the {product-title} cluster.
...
* Red Hat recommends that the customer have at least the Business Support plan from AWS. For more information, see the __Additional resources__.
...
* Red Hat must have ingress access to EC2 hosts and the API server through allowlisted Red{nbsp}Hat machines.
