...
* A {product-title} ROSA cluster with the required access and tokens. For instructions, see the procedure in "Preparing AWS credentials". If you plan to use two different clusters for backing up and restoring, you need to prepare AWS credentials, including `ROLE_ARN`, for each cluster.
...
.. Deploy a Data Protection Application for CSI only by entering the following command:
+
[source,terminal]
----
$ cat << EOF | oc create -f -
