...
[role="_abstract"]
Install {oadp-first} on clusters with {aws-short} {sts-short}. AWS Security Token Service (AWS STS) is a global web service that provides short-term credentials for IAM or federated users. {product-title} with {sts-short} is the recommended credential mode.
