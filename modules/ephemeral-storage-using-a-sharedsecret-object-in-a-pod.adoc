...
* You are logged in to the {ocp-product-title} cluster either:
** as a cluster administrator and have created a `SharedSecret` CR instance for the `Secret` object, or
** as a user with permissions to access the target namespaces and to create or manage the SharedSecret custom resource for the Secret to be shared.
