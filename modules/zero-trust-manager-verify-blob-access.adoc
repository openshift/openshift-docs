...
. Retrieve a JWT token from the SPIFFE Workload API.
+
[source,terminal]
----
...
. Create and export an environment variable named `TOKEN`.
+
[source,terminal]
----
...
. Log in to {azure-short} CLI included within the pod.
+
[source,terminal]
----
...
. Upload a file to the {azure-short} Blog Storage.
+
[source,terminal]
----
