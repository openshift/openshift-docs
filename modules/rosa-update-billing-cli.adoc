...
[role="_abstract"]
Change the AWS billing account a deployed cluster uses by running `rosa edit cluster` in interactive mode.

.Prerequisites

* You must have more than one AWS billing account.
* You must link the AWS billing account to the same Red{nbsp}Hat organization where you deployed the cluster.
...
. Select the required AWS billing account from the list of available options and press *Enter*.
+
