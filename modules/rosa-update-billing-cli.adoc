...
[role="_abstract"]
Use the following procedure to update the AWS billing account linked to your cluster.
...
----
where::
`<cluster_ID>`:: Replace `<cluster_ID>` with the ID of the cluster that you want to update the AWS billing account.
+
[NOTE]
====
