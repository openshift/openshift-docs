...
[NOTE]
====
The value of the `spec.template.spec.providerSpec.value.image` parameter specifies the image details of the boot image for your nodes. You should use the use the latest image when adding a new machine set. If you want to use an {azure-short} Marketplace image, see "Using the {azure-short} Marketplace offering".
