...
* This feature is not supported for {ms}, which does not use the MCO.

* If you need to downgrade your cluster to {product-title} version 4.21 or earlier, before you downgrade, delete any `ContainerRuntimeConfig` resource that includes the `additionalArtifactStores`, `additionalImageStores`, or `additionalLayerStores` fields.
