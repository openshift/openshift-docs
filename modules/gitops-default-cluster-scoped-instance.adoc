...
The default instance does not have full cluster-level privileges, though. However, the instance must and does have read access to all resources in the cluster to be able to function, but the set of resources it is allowed to deploy is limited.

[NOTE]
====
Use the default Argo CD instance in the `openshift-gitops` namespace for cluster configuration and delegating tenant use cases to one or many separate instances in other namespaces.
====
