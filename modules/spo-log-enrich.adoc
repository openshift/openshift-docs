...
[role="_abstract"]
The Audit JSON log enricher enables the SPO Advanced Audit Logging Framework so you can review container activity as JSON Lines records. Similar to the log enricher feature, it watches the `auditd` (`/var/log/audit/audit.log`) or the `syslog` (`/var/log/syslog`) daemons and generates a new audit log in JSON lines format.
