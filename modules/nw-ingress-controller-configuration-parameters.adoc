...
Valid values for {product-title} {product-version} include the following:
...
If `haproxyVersion` is set to a value that a target cluster update does not support, a preflight check blocks the update until you unset the field or change it to a supported version.
