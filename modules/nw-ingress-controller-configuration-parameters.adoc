...
|`haproxyVersion`
|`haproxyVersion` specifies the HAProxy version for this Ingress Controller.

{product-title} {product-version} uses HAProxy 3.2 as the default version and supports HAProxy 2.8 from {product-title} 4.22 for migration. When a later {product-title} release introduces a new default HAProxy version, that version becomes available as a pinnable value in subsequent releases.

Valid values for {product-title} {product-version} are:

* An empty value (default): Uses HAProxy 3.2.
...
If `haproxyVersion` is set to a value that a target cluster update does not support, a preflight check blocks the update until you change the field to an empty value or to a supported version.

The Ingress Operator reports the resolved version in `status.effectiveHAProxyVersion`.
