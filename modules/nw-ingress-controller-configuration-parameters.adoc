...
These adjustments are only applied to plain text, edge-terminated, and re-encrypt routes, and only when using HTTP/1.
...
== Unsupported Ingress Controller configuration overrides

[IMPORTANT]
====
Using the `unsupportedConfigOverrides` section to modify the configuration of an Operator is unsupported and might block cluster upgrades. You must generally remove these settings before you can update your cluster. These overrides are intended only for specific workarounds.
====

The `IngressController` custom resource (CR) includes an `unsupportedConfigOverrides` parameter that allows you to bypass standard configurations. 

[cols="3a,8a",options="header"]
|===
|Parameter |Description

|`mutualTLSHeaderFilter`
|By default, the {product-title} router strips `X-SSL-Client-*` headers on HTTP frontends to prevent unauthenticated spoofing of mutual TLS (mTLS) client identities (CVE-2026-46579). 

However, if you have a Load Balancer in front of your {product-title} router that handles TLS termination, you might need these headers to pass through. You can set the `mutualTLSHeaderFilter` override to `"false"` to prevent the router from stripping these headers. 

[WARNING]
====
If you set `mutualTLSHeaderFilter` to `"false"`, you are exposing the router to mTLS spoofing vulnerabilities. You **must** configure a `NetworkPolicy` between your Load Balancer and the router to disallow all untrusted, direct access to the router. 

If your router has `hostNetwork` enabled, standard network policies might not apply. In this scenario, you must secure the connection using infrastructure-level firewalls or other external protections to restrict access strictly to the trusted Load Balancer.
====
