...
When a pull request (PR) with a YAML error triggers the event, {pac} creates a comment on the PR that describes the error. If the PR is updated and new YAML errors are detected, the comment is updated with the latest information. If no new errors are found, the existing comment remains unchanged and is not deleted.
