...
[IMPORTANT]
====
If the cluster storage capability is disabled, the cluster does not have a default `storageclass` or any CSI drivers. Users with administrator privileges can create a default `storageclass` and manually install CSI drivers if the cluster storage capability is disabled.
====
endif::cluster-caps[]
