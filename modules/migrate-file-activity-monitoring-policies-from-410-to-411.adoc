...
. Map 4.10 fixed paths to 4.11 wildcard patterns. Review each monitored path and decide whether you can expand coverage using wildcard patterns:
...
.. Verify that you have selected the correct severity.

.. In the *Categories* section, select *File Activity Monitoring*.

.. Verify that you have selected the correct *lifecycle stage*.
...
.. After verifying that 4.11 policies offer equal or better security coverage, disable your 4.10 file activity policies by setting them to *Disabled*.
...
. Verify that wildcard patterns detect file operations across many files as intended.
