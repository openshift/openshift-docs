...
You can also configure the profiles and settings for the descheduler later using the {oc-first}. If you did not adjust the profiles when creating the descheduler instance from the web console, the `AffinityAndTaints` profile is enabled by default.
endif::nodes[]
