...
where:
+
`02_lb_ext.py` and `cluster-lb-ext`:: Required only when deploying an external cluster.
...
`<control_subnet>`:: Specifies the URI to the control subnet.
`<zones>`:: Specifies the zones to deploy the control plane instances into, like `us-east1-b`, `us-east1-c`, and `us-east1-d`.
