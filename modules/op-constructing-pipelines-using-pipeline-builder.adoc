...
[NOTE]
====
The search list has tasks from {artifact-hub} and tasks available in the cluster. Also, if a task is already installed, it will show *Add* to add the task, whereas it will show *Install and add* to install and add the task. It will show *Update and add* when you add the same task with an updated version.
====
