...
| `--quayRoot`, `-r` | The directory where container image layer and configuration data is saved, including `rootCA.key`, `rootCA.pem`, and `rootCA.srl` certificates. Requires about 7.7 GB for {product-title} 4.7 Release images, or about 713 GB for {product-title} 4.7 Release images and {product-title} 4.7 Red Hat Operator images. Defaults to `/etc/quay-install` if left unspecified.
