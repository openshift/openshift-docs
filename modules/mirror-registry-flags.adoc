...
| `--quayRoot`, `-r` | The directory where container image layer and configuration data is saved, including `rootCA.key`, `rootCA.pem`, and `rootCA.srl` certificates. Requires about 6.8 GB for {product-title} 4.6 Release images, or about 696 GB for {product-title} 4.6 Release images and {product-title} 4.6 Red Hat Operator images. Defaults to `/etc/quay-install` if left unspecified.
