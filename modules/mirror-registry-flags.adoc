...
| `--quayRoot`, `-r` | The directory where container image layer and configuration data is saved, including `rootCA.key`, `rootCA.pem`, and `rootCA.srl` certificates. Requires about 9.6 GB for {product-title} 4.9 Release images, or about 444 GB for {product-title} 4.9 Release images and {product-title} 4.9 Red Hat Operator images. Defaults to `/etc/quay-install` if left unspecified.
