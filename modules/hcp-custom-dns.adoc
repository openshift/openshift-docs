...
If you remove the `kubeAPIServerDNSName` parameter from the specification for the `HostedCluster` object, removal of all newly generated secrets and the `CustomKubeconfig` reference happen from the cluster and from the `status` parameter.
