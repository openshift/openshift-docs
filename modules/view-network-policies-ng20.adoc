...
[role="_abstract"]
Network policies specify how groups of pods communicate with each other and with other network endpoints.
Kubernetes `NetworkPolicy` resources use labels to select pods and define rules that specify what traffic can flow to or from the selected pods.
...
. To view the YAML file for the network policies, you can click the policy rule, or click the *Network policies* tab.
