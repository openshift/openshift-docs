  ...
  config:
    ...
    systemd:
      units:
        - contents: |
            [Unit]
            Description=Provision file-backed swap
            ...
            ExecStart=/bin/sh -c "sudo fallocate -l ${SWAP_SIZE_MB}M /var/ocpswap.file && \
            sudo chmod 600 /var/ocpswap.file && \
            sudo mkswap /var/ocpswap.file"
        ...
        - name: ocpswap-file-enable.service
          dropins:
            - name: 10-wait-for-provisioning.conf
              contents: |
                [Unit]
                # Reset the inherited first-boot condition so the file can be
                # activated on a newly provisioned worker.
                ConditionFirstBoot=
                ConditionPathExists=
                ConditionPathExists=/var/ocpswap.file
...
The `ocpswap-file-enable.service` drop-in does not run `swapon` itself. It
orders the existing swap activation service after `swap-provision.service`, so
the file is completely allocated and formatted before it is activated. The
drop-in also resets the inherited first-boot condition so that a newly
provisioned worker can activate file-backed swap on its first boot.

`swap-provision.service` is wanted, rather than required, by kubelet
dependencies. If provisioning fails because the filesystem lacks capacity, the
failure is recorded in the journal but does not prevent the node from starting.
