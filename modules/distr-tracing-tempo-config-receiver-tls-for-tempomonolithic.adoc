...
= Configuring the receiver TLS for a TempoMonolithic instance

* You can provide a TLS certificate in a secret and configure it in the `TempoMonolithic` custom resource.
+
[NOTE]
====
...
* Alternatively, you can use OpenShift's service serving certificates.
+
[NOTE]
====
...
====
+
.TLS for receivers and using OpenShift's service serving certificates
[source,yaml]
----
apiVersion: tempo.grafana.com/v1alpha1
kind:  TempoMonolithic
