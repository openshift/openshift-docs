...
[role="_abstract"]
When the Compliance Operator creates or updates a `ComplianceCheckResult` object for an OpenSCAP-backed check, it can copy custom labels and annotations from the corresponding `Rule` object in the same namespace.

For checks produced by the CEL-based scanner from a `CustomRule`, the Operator copies custom metadata from that `CustomRule` instead.
