...
`acme.privateKeySecretRef.name`:: Specifies the name of the secret to store the ACME account private key in.
`acme.server`:: Specifies the URL to access the ACME server's `directory` endpoint. This example uses the _Let's Encrypt_ staging environment.
`acme.solvers.dns01.cloudDNS.project`:: Specifies the name of the {gcp-short} project that contains the Cloud DNS zone.
