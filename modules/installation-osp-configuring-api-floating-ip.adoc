...
:navtitle: Configure application access with floating IP addresses
:chunk-to-content:
[id="installation-osp-configuring-api-floating-ip_{context}"]
= Configuring application access with floating IP addresses

[role="_abstract"]
After you install {product-title}, configure {rh-openstack-first} to allow application network traffic by attaching a floating IP address to the ingress port.
...
. Attach a floating IP address to the ingress port by completing the following commands:
+
.. Show the port by entering the following command:
+
[source,terminal]
----
...
----
+
.. Attach the port to the IP address by entering the following command:
+
[source,terminal]
----
