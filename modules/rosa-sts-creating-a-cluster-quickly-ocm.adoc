...
+
The following example creates an admin {cluster-manager} user using the default options and `auto` mode for immediate STS resource creation. The example also links the {cluster-manager} role to a Red Hat organization:
+
[source,terminal]
----
...
<3> Selects the role creation mode. You can use `auto` mode to automatically create the {cluster-manager} role and link it to your Red Hat organization account.
<4> Links the {cluster-manager} role to your Red Hat organization account.
