...
* Before this update, `Results` configuration options were not being applied to `TektonConfig` Custom Resource (CR). With this update, `Results` configuration options from the `TektonConfig` CR are now correctly applied to the TektonResults object.

* Before this update, if `PipelineRun` without the `cancel-in-progress` annotation was executed due to a Pull Request (PR) and that PR was closed, the `PipelineRun` would be automatically cancelled. With this update, `Pipelines as Code` can cancel `PipelineRun` if the `cancel-in-progress` annotation is explicitly specified.

* Before this update, the `Pipelines as Code` controller experienced crashes due to an index out of range error. With this update, this issue is resolved and the `Pipelines as Code` controller does not crash.

* Before this update, the {OCP} Web UI for `PipelineRun` displayed duplicate `TaskRuns` and showed incorrect step completion status based on archived data. With this update, you must list `TaskRuns` that belong to the current `PipelineRun` and use the status of the current `TaskRun`.

* Before this update, when using the `cancel-in-progress` annotation to cancel `PipelineRun` on PR updates or pushes, `PipelineRun` created with the `generateName` field could not be filtered by PAC. This was because the `generateName` field includes a hyphen (-) suffix, which does not comply with Kubernetes label requirements; labels must start and end with an alphanumeric character. This mismatch led to errors. With this update, you can use the `originalPRName` label instead of the annotation to filter `PipelineRun`.