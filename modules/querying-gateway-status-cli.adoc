...
+
--
** To list all `GatewayClass` objects in your cluster, run the following command:
+
[source,terminal]
----
$ oc get gatewayclass
...
** To check if a specific `GatewayClass` object has been accepted by the controller, run the following command:
+
[source,terminal]
----
...
** To check if a parent gateway accepted a specific `HTTPRoute` CR, run the following command:
+
[source,terminal]
----
...
** To check if all references on a specific `HTTPRoute` CR resolved, run the following command:
+
[source,terminal]
----
$ oc get httproute <httproute_name> -n <namespace> -o jsonpath='{.status.parents[*].conditions[?(@.type=="ResolvedRefs")].status}'
----
*** `<httproute_name>`: Specify the name of your HTTPRoute.
*** `<namespace>`: Specify the namespace where the HTTPRoute resides.
