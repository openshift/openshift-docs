...
* With this update, the new `set-security-context` feature flag is set to `true` by default, to enable task runs and pipeline runs to run in namespaces with restricted pod security admission policies.
...
* This update fixes an error in processing the Common Expression Language (CEL) NOT expression when querying {tekton-results}.
