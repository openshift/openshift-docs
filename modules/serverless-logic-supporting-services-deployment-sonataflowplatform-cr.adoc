...
<3> Jobs Service configuration field.
<4> Setting `enabled: true` deploys the Jobs Service. If set to `false` or omitted, the deployment will be disabled. The default value is `false`.
