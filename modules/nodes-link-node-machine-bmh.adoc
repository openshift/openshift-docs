...
. Link the new control plane node and `Machine` object to the `BareMetalHost` by performing the following steps in a single bash shell session:
...
.. Define the `BMH_UID` by running the following command to extract it from the new node's `BareMetalHost` object:
+
[source,terminal]
----
...
. Set the machine provisioned state by running the following command:
+
[source,terminal]
----
