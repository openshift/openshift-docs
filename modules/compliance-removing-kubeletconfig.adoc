...
`compliance.openshift.io/scan-name: one-rule-tp-node-master`:: Specifies the scan name of the remediation.
`imagefs.available: 10%`:: Specifies the remediation that was added to the `KubeletConfig` objects.
+
...
. Remove the remediation by completing the following steps:
