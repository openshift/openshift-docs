...
[role="_abstract"]
Deploy an operating system image to a bare-metal host by updating the `image` field with the image URL and checksum in the `BareMetalHost` resource, automatically provisioning the host with your specified OS configuration.
