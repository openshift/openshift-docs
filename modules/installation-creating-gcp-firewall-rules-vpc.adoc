...
allowed_external_cidr:: Specifies the CIDR range that can access the cluster API and SSH to the bootstrap host. For an internal cluster, set this value to `${NETWORK_CIDR}`.
infra_id:: Specifies the `INFRA_ID` infrastructure name from the extraction step.
cluster_network:: Specifies the `selfLink` URL to the cluster network.
network_cidr:: Specifies the CIDR of the VPC network, for example `10.0.0.0/16`.
