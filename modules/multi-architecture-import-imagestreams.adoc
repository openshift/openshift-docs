...
:navtitle: Import manifest lists in image streams on your multi-architecture compute machines
:chunk-to-content:
...
[role="_abstract"]
On an {product-title} {product-version} cluster with multi-architecture compute machines, the image streams in the cluster do not import manifest lists automatically. You must manually change the default `importMode` option to the `PreserveOriginal` option to import the manifest list.
