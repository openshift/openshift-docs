...
. Add the `group-du-clo5-cleanup-policy.yaml` in the git repository that contains your other `PolicyGenTemplate` CRs.
...
. Add the cleanup `PolicyGenTemplate` CR to to the `resources` section in the `kustomization.yaml` file.
