...
* A credential secret has been configured.
...
.. Create a file, for example openshift-logging-sts-secret-credentials.yaml, to configure a credentials secret:
+
[source,yaml]
----
apiVersion: v1
...
type: Opaque
----
<1> Specify the `Arn` of the {aws-short} IAM role.

. Create a credentials secret based on the configuration above, by running the following command: 
+
[source,terminal]
----
