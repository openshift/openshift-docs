      ...
      exporters:
        otlp/dev:
          ...
          headers:
            X-Scope-OrgID: "dev"
        otlphttp/dev: <1>
          endpoint: tempo-simplest-gateway.tempo.svc.cluster.local:8080
          tls:
            insecure: false
            ca_file: "/var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt"
          auth:
      ...
      service:
        ...
        pipelines:
          traces:
            exporters: [otlp/dev]
----
<1> The OTLP HTTP exporter. It needs to be specified in `service.pipelines.traces.exporters` to enable it. If it is enabled disable OTLP gRPC (`otlp/dev`).
