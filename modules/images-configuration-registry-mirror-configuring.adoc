...
ifdef::winc[]
After the object is created, the Windows Machine Config Operator (WMCO) drains the nodes for `ImageTagMirrorSet` objects only. The MCO does not drain the nodes for `ImageDigestMirrorSet` objects.
endif::winc[]
...
ifdef::winc[]
.. Check the WMCO generated a for each registry on each Windows instance. For the prebious example IDMS object, there should be three files in the following file structure:
+
[source,terminal]
----
$ tree $config_path
...
----
+
The following output represents a `hosts.toml` containerd configuration file where the prebious example IDMS object was applied.
+
[source,terminal]
.Example output
----
