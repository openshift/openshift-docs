...
* `host-firmware-settings.yaml` specifies the name of the `HostFirmwareSettings` CR.
--
...
+
--
* `host-firmware-settings-cm` specifies the `ConfigMap` reference that applies the firmware profile to all hosts in the cluster.
--
...
+
--
* `host-firmware-node1-cm` specifies the `ConfigMap` reference that applies the firmware profile only to the `node1.example.com` host.
