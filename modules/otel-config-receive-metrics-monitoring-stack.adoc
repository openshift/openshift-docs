...
You can use the OpenTelemetry Collector custom resource (CR) to configure the Prometheus receiver to scrape metrics from the in-cluster monitoring stack.

.Example CR for the OpenTelemetry Collector for scraping metrics from the in-cluster monitoring stack
[source,yaml]
----
