...
.Procedure
To create an Ingress Controller with manually specified load balancer subnets in Red Hat OpenShift Container Platform 4.17, you can follow these steps:
        ...
        aws:
          ...
          classicLoadBalancer: (3)
              subnets:
                ...
                - <updated-subnet>
+
2. Examine the `Progressing` condition on the IngressController for instructions on how to apply the subnet updates:
...
type: Progressing
+
3. Delete the service associated with the IngressController to apply the update:
