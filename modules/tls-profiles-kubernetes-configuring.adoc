...
type:: Specify the TLS security profile type (`Old`, `Intermediate`, or `Custom`). The default is `Intermediate`.
custom:: Specify the appropriate field for the selected type:
* `old: {}`
* `intermediate: {}`
* `modern: {}`
* `custom:`
ciphers:: For the `custom` type, specify a list of TLS ciphers and minimum accepted TLS version.
