...
In the following example, Ingress Controllers managed by the Ingress Operator run in IPv4-only mode because only one cluster network exists and the network is an IPv4 `cidr`:
