...
[NOTE]
====
Memory hot plugging for virtual machines requires that guest operating systems support the `virtio-mem` driver. Support is determined by whether the driver is included and enabled in the guest OS, rather than by specific upstream kernel versions, see link:https://access.redhat.com/articles/certified-hypervisors?extIdCarryOver=true&sc_cid=701f2000001Css5AAC#ocpvirt[RHEL guests supported kernel versions]. 
====