...
[role="_abstract"]
The following hardware specifications are the minimum requirements for deploying DPF v26.4 on {product-title} 4.22.

== Workstation

A workstation with the following command-line interface (CLI) tools installed:

* OpenShift CLI (`oc`). Download from the link:https://mirror.openshift.com/pub/openshift-v4/clients/ocp/[OpenShift mirror].
* Helm CLI (`helm`). See the link:https://helm.sh/docs/intro/install/[Helm installation guide].
