...
You can deploy waypoint proxies by using {k8s} Gateway resource.
...
----
+
Starting with {ocp-product-title} 4.19, the Gateway API CRDs are installed by default.

[NOTE]
====
The CRDs are community maintained and not supported by Red Hat. Upgrading to {ocp-product-title} 4.19 or later, which includes supported Gateway API CRDs, might disrupt applications.
====