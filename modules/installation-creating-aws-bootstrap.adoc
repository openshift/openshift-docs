...
<cluster-name>-infra:: `<cluster-name>-infra` is the bucket name. When creating the `install-config.yaml` file, replace `<cluster-name>` with the name specified for the cluster.
+
...
<installation_directory>:: For `<installation_directory>`, specify the path to the directory that you stored the installation files in.
