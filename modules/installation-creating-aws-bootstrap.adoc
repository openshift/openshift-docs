...
config file to it.
ifdef::restricted[]
For a restricted network installation, if you upload the Ignition config file to an S3 bucket, you must access it by using a pre-signed URL. If you want to use HTTP protocol to access the file, upload it to a location within your VPC.
endif::restricted[]
+
[IMPORTANT]
====
...
ifdef::restricted[]
.. Configure your AWS profile for AWS Signature Version 4:
+
----
...
ifdef::restricted[]
.. Generate a pre-signed URL to access the image that you uploaded by using HTTPS protocol:
+
----
$ aws s3 presign s3://<cluster-name>-infra/bootstrap.ign --region <s3_bucket_region> --expires-in <time_in_seconds> <1>
----
<1> for `<time_in_seconds>`, specify the amount of time, in seconds, that the pre-signed URL is valid.
+
...
<14> Specify location of the bootstrap file. Enter
ifdef::restricted[]
either the the pre-signed image URL that you generated or
endif::restricted[]
