...
* You must specify an annotation with the name of the network attachment definition that defines the secondary network. You can use the `k8s.v1.cni.cncf.io/policy-for` annotation in a `MultiNetworkPolicy` CRD to specify what `NetworkAttachmentDefinition` (NAD) from a pod with multiple networks applies to a secondary network interface.
+
[source,yaml]
----
apiVersion: k8s.cni.cncf.io/v1beta1
