...
* You must specify an annotation with the name of the network attachment definition that defines the secondary network. The `k8s.v1.cni.cncf.io/policy-for` annotation can be used in a `MultiNetworkPolicy` CRD to specify what network policy from a pod with multiple networks applies to a secondary network interface.
+
[source,yaml]
----
apiVersion: k8s.cni.cncf.io/v1beta1
