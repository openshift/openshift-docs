...
Configure your service mesh by adding your own traffic configuration to {SMProductName} with a custom resource definitions in a YAML file.
...
The following example mesh-external service entry adds the `ext-resource` external dependency to the {SMProductName} service registry:
