...
The following example routes requests to different versions of a the Bookinfo sample app service depending on which user connects to the application. Use this command to apply this example YAML file, or use this example and the explanation of the example that follows to create your own `VirtualService` resource.
