...
You apply tolerations to the Kibana pods through the `ClusterLogging` custom resource (CR)
...
. Edit the `visualization` section of the `ClusterLogging` custom resource (CR) to configure a toleration for the Kibana pod:
+
[source,yaml]
----
  visualization:
