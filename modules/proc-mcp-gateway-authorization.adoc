...
= Configure {mcpg} authorization with an AuthPolicy

[role="_abstract"]
The following example demonstrates using a {prodname} `AuthPolicy` custom resource (CR) with Common Expression Language (CEL) to implement role-based access control. You can use the following general pattern for applying the authorization specific to your use case.
...
* The identity provider client IDs match the namespaced `MCPServerRegistration` CR name in the format `_<namespace>_/_<mcpserverregistration_name>_`.
