...
[IMPORTANT]
====
There are many ways to configure the hardware MTU for your cluster nodes. This procedure describes only the most common examples. You must verify that your infrastructure MTU is correct and that your preferred method for configuring your hardware MTU in the cluster nodes is in effect before proceeding with this procedure.
====
