...
[role="_abstract"]
ifdef::outposts[]
You might need to decrease the maximum transmission unit (MTU) value for the cluster network to support an AWS Outposts subnet.

During installation, the MTU for the cluster network is detected automatically based on the MTU of the primary network interface of nodes in the cluster.
endif::outposts[]
