...
[role="_abstract"]
You can install {sno} on {ibm-z-name} and {ibm-linuxone-name} in an LPAR by generating the installation configuration and booting the server.
...
`replicas: 0` (compute):: Set the `compute` replicas to `0`. This makes the control plane node schedulable.
`replicas: 1` (controlPlane):: Set the `controlPlane` replicas to `1`. In conjunction with the previous `compute` setting, this setting ensures the cluster runs on a single node.
`<cluster_name>`:: Specifies the cluster name for the `metadata` field.
`networking`:: Set the `networking` details. OVN-Kubernetes is the only allowed network plugin type for single-node clusters.
`<machine_network_cidr>`:: Specifies the `cidr` value to match the subnet of the {sno} cluster.
`<pull_secret>`:: Copy the {cluster-manager-url-pull} and add the contents to this configuration setting.
...
+
--
`<block_device>`:: Specifies the block device on the system to install to. For installations on DASD-type disk use `dasda`, for installations on FCP-type disks use `sda`.
...
`rd.dasd`:: For installations on DASD-type disks, use `rd.dasd=` to specify the DASD where {op-system} is to be installed. Omit this entry for FCP-type disks.
`rd.zfcp`:: For installations on FCP-type disks, use `rd.zfcp=<adapter>,<wwpn>,<lun>` to specify the FCP disk where {op-system} is to be installed. Omit this entry for DASD-type disks.
--
+
