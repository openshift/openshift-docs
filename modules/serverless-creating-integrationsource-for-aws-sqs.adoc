...
You can create an `IntegrationSource` to receive messages from an Amazon SQS queue by applying a YAML manifest. A manifest is a Kubernetes resource definition file in YAML format that acts as a blueprint for the resource and its configuration.
