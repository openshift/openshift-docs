...
[role="_abstract"]
Use the following required and optional fields in your `Build` custom resource (CR):
...
| `spec.output`
| Specifies the location where the generated image will be pushed.

| `spec.output.pushSecret`
| Specifies an existing secret to get access to the container registry.
|===
...
| `spec.paramValues`
| Specifies a name-value list to specify values for parameters defined in the build strategy.
