...
= Configurable fields in a build
...
| Specifies an existing secret to gain access to the container registry. Required only when pushing to private registries that require authentication.
...
| Specifies the RuntimeClass to use for build pod isolation, enabling builds to run in sandboxed or isolated environments such as Kata Containers.

| `spec.volumes`
| Specifies additional volumes to mount in the build container.

| `spec.trigger`
| Specifies trigger configurations for automatically initiating builds.
|===
