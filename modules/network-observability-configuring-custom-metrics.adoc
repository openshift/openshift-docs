...
. Configure the `FlowMetric` resource to to show RTT latency for cluster external ingress traffic, similar to the following sample configuration:
+
[source,yaml]
----
apiVersion: flows.netobserv.io/v1alpha1
...
.. You can also enter an expression to verify the `FlowMetric` resource is generating a metric that tracks ingress bytes received from cluster externa resources. For example: `topk(5, sum(rate(netobserv_cluster_external_ingress_bytes_total{DstK8S_Namespace="my-namespace"}[2m])) by (DstK8S_HostName, DstK8S_OwnerName, DstK8S_OwnerType))`.