...
[IMPORTANT]
====
If you want to use custom rules, do not set the top-level profile field to `None`. If you do set  the top-level profile field to `None`, the OVN-Kubernetes plug-in ignores custom rules because the plug-in applies its most restrictive policy that ignores all traffic types.
====
