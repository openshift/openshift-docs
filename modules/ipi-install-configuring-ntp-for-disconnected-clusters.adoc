...
[role="_abstract"]
Configure Network Time Protocol (NTP) servers on control plane nodes using `chrony` and Butane-generated `MachineConfig` resources to provide time synchronization for disconnected clusters without external NTP access, ensuring cluster operations function correctly.
endif::[]
...
+
--
`server openshift-master-0.<cluster-name>.<domain> iburst`:: You must replace `<cluster-name>` with the name of the cluster and replace `<domain>` with the fully qualified domain name.
--
