...
metadata:
  ...
  annotations:
    ...
    service.beta.kubernetes.io/aws-load-balancer-target-node-labels: <key_value_pair>
spec:
  ports:
  - name: http
    port: 80
...
<key_value_pair>:: Specifies the key-value pair that matches the pair in the node label. For example, `<key_name>=<value>`.
