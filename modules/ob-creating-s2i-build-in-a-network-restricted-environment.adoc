...
[role="_abstract"]
You can create a `source-to-image` build in a network-restricted environment by mirroring the images required by the `source-to-image` build strategy. By mirroring the images required by the `source-to-image` build strategy, you eliminate dependency on public registries and ensuring your clusters utilize container images that strictly adhere to all organizational external content policies.
