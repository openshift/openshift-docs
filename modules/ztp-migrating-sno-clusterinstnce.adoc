...
Migrate a {sno} cluster from using a `SiteConfig` CR to a `ClusterInstance` CR removing the `SiteConfig` CR from the old pipeline, and adding a corresponding `ClusterInstance` CR to the new pipeline.
...
Moved 1 extraManifest files from /resources/output/extra-manifests/sno1 to /resources/output/extra-manifests
...
[NOTE]
====
By default, the `siteconfig-converter` tool generates `ConfigMap` `kustomization` files. These files package cluster-specific extra manifests into a `ConfigMap` resource, which is referenced by the generated `ClusterInstance` CR. You can merge the generated `kustomization` file with your original one to generate the `ConfigMap` and the `ClusterInstance` CR.
====
