...
spec:
  ...
  - name: "profile1" <3>
    ...
    ptp4lConf: | <6>
      ...
      timeSource              0xA0
    phc2sysOpts: "-a -r" <9>
    ptpSchedulingPolicy: SCHED_FIFO <10>
...
<10> Scheduling policy for ptp4l and phc2sys processes. Either SCHED_FIFO or default of SCHED_OTHER. SCHED_FIFO should only be used on systems that support fifo scheduling.
<11> Required if using SCHED_FIFO. Integer from 1-65 to use as fifo priority for ptp4l and phc2sys processes.
<12> Specify an array of one or more `recommend` objects which define rules on how the `profile` should be applied to nodes.
