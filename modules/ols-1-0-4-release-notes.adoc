...
* This release introduces support for Model Context Protocol (MCP) server as a Technology Preview feature. MCP is an open protocol that standardizes how applications provide context to large language models (LLMs). Using the protocol, an MCP server offers a standardized way for an LLM to increase context by requesting and receiving real-time information from external resources. 
+
--
