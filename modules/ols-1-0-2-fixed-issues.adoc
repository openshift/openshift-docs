...
[role="_abstract"]
{ols-official} 1.0.2 resolves a networking issue where large language model (LLM) provider connections ignored the `no_proxy` environment variable. This fix ensures that {ols-long} correctly adheres to cluster-wide proxy settings, providing reliable connectivity for proxied environments.
