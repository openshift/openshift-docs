...
[IMPORTANT]
====
Restic and Kopia are not supported in the OADP on ROSA with AWS STS environment. Make sure that the Restic service is disabled in the `DataProtectionApplication` resource.

For backing up volumes, OADP on ROSA with AWS STS only supports native snapshots and CSI snapshots. See _Known Issues_ for more information.
====
