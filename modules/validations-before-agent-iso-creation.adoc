...
* Each fencing credential must include `address`, `username`, and `password`, and at least one of `hostName` or `macAddress`.
...
* All `hostName` values must be unique. All `macAddress` values must be unique.
