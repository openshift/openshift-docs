...
= RHSA-2026:34769 - {product-title} {product-version}.23 fixed issues and security update
...
* Before this update, during parallel deployments of single-node {product-title} clusters, specifically with hypervisor-based single-node clusters, some systems would hang in the middle of the deployment due to a race condition involving the BareMetalHost (BMH) custom resource. The cluster node was never powered on by metal3 after virtual media was attached. As a consequence, parallel deployments at scale (10+ nodes) had approximately 80% success rate, with the remaining nodes requiring manual intervention to patch the `online` field to `true` for the impacted BMH custom resources. With this release, the race condition in the BMH power-on process has been resolved. As a result, parallel single-node {product-title} deployments successfully power on all nodes without manual intervention, even at scale. (link:https://issues.redhat.com/browse/OCPBUGS-90554[OCPBUGS-90554])
