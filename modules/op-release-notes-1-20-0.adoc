...
* {pipelines-shortname} no longer emits deprecated metrics, such as `tekton_pipelines_controller_pipelinerun_count` or `tekton_pipelines_controller_running_taskruns_count`. You must update your dashboards and alerts with the new metrics, such as `tekton_pipelines_controller_pipelinerun_total` or `tekton_pipelines_controller_running_pipelineruns`.
+
[options="header"]
|===
|Deprecated |New
...
|`tekton_pipelines_controller_running_pipelineruns_waiting_on_pipeline_resolution_count`
|`tekton_pipelines_controller_running_pipelineruns_waiting_on_pipeline_resolution`


|`tekton_pipelines_controller_running_pipelineruns_waiting_on_task_resolution_count`
...
* Before this update, the `defaultRetention` configuration option was introduced with a default value of 30 days. The `defaultRetention` value took precedence over the existing `maxRetention` configuration during upgrades. As a result, previously configured retention settings defined using `maxRetention` were not applied, which could lead to unintended retention behavior. With this update, the retention agent restores backward compatibility by reading and applying the existing `maxRetention` value during upgrades.
...
* Before this update, querying logs for a running `PipelineRun` or `TaskRun` in OpenShift Pipelines CLI (OPC) Results resulted in an unclear error message. With this update, the message clearly indicates that logs cannot be retrieved while the run is still in progress.
...
* The `maxRetention` parameter in the {tekton-results} retention agent is deprecated. Use `defaultRetention` instead. This change affects only the parameter name; the functionality remains unchanged.
