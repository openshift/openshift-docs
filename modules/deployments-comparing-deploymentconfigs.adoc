...
[role="_abstract"]
You can use both Kubernetes `Deployment` objects and {product-title} `DeploymentConfig` objects to manage application rollouts. Before deciding which to use, understand the differences between the two objects in design and supported features.
