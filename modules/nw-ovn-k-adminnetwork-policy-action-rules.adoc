...
[role="_abstract"]
You can set the `action` field to `Allow`, `Deny`, or `Pass` on each `AdminNetworkPolicy` rule so OVN-Kubernetes tiered ACLs apply administrative decisions before lower-tier policies can take effect.

Because OVN-Kubernetes uses tiered ACLs to evaluate network traffic rules, an ANP lets you define strong policy rules that can only be changed by an administrator modifying them, deleting the rule, or overriding them by setting a higher priority rule.
