...
[role="_abstract"]
The default security policies in {product-title} provide broad coverage to identify security issues and ensure best practices for security in your environment. When you configure those policies, you can automatically prevent high-risk service deployments in your environment and respond to runtime security incidents.
...
While a policy's severity level and the Red{nbsp}Hat Product Security severity levels can interact, it is important to distinguish between them. For more information about the Red{nbsp}Hat Product Security severity levels, see "Severity Ratings".
====