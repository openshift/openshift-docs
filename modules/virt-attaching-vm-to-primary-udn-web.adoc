...
= Attaching a virtual machine to the primary UserDefinedNetwork (UDN) by using the web console

You can connect a virtual machine (VM) to the primary UserDefinedNetwork (UDN) by using the {product-title} web console. VMs that are created in a namespace where the primary UDN is configured are automatically attached to the UDN with the Layer 2 bridge network binding plugin.
...
.. Click *Preview features* and set *Enable Passt binding for primary UserDefinedNetwork (UDN)* to on.
