...
When {microshift-short} starts, an internal {microshift-short} cluster certificate authority (CA) issues the default API server certificate. By default, clients outside of the cluster cannot verify the {microshift-short}-issued API server certificate. To grant access and encrypt connections between the {microshift-short} API server and external clients, you can replace the default certificate with a custom server certificate that is issued externally by a CA that clients trust.

//Q: are these steps still valid?
