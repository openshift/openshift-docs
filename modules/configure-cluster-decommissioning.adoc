...
[role="_abstract"]
You can configure {product-title-short} to automatically remove nonactive clusters from {product-title-short}. Nonactive clusters are those that installed and performed a handshake with Central at least one time but have been unreachable by Sensor for a specified period of time. You can also label clusters so that {product-title-short} does not remove them when they are unreachable.
...
* *Decommissioned cluster age*: The number of days that a cluster is unreachable before it becomes eligible for removal from {product-title-short}. If Central cannot reach Sensor on the cluster for this number of days, {product-title-short} removes the cluster and all of its resources. To leave this feature disabled, which is the default behavior, enter `0` in this field. To enable this feature, enter a nonzero number, such as `90`, to configure the number of unreachable days.
* *Ignore clusters which have labels*: To prevent {product-title-short} from removing a cluster, you can configure a label by entering a key and value. {product-title-short} does not remove clusters with this label, even if they have been unreachable for the number of days set in the *Decommissioned cluster age* field.
...
[NOTE]
====
In the *Cluster deletion* section, click *Clusters which have Sensor Status: Unhealthy* to go to the *Clusters* list page. This page shows nonactive clusters that are subject to removal and a time frame for the removal from {product-title-short}.
====
. Click *Save*.
+
[NOTE]
