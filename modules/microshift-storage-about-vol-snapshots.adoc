...
You can use volume snapshots with logical volume manager (LVM) thin volumes to help protect against data loss from applications running in a {product-title} cluster. {product-title} only supports the logical volume manager storage (LVMS) Container Storage Interface (CSI) provider.
