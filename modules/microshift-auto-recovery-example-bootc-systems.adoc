...
[role="_abstract"]
To use automatic recovery for {product-title} on {op-system-base} image-based systems, you can embed the `10-auto-recovery.conf` and `microshift-auto-recovery.service` files in your Containerfile and rebuild the bootc image.
...
* The podman `--authfile` argument is required to pull the base `rhel-bootc:9.4` image from the `quay.io/openshift-community` registry.
endif::[]
