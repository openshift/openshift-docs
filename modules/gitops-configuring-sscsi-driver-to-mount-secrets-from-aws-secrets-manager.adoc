...
where:
+
`namespace`:: Specifies the namespace for the secret reference. Update the value of this `namespace` field according to your project deployment setup.
`<aws_region>`:: Specifies the aRN of your secret in the region where your cluster is on. The `<aws_region>` of `<aws_secret_arn>` has to match the cluster region. If it does not match, create a replication of your secret in the region where your cluster is on.
+
[TIP]
====
...
where:
+
`name`:: Specifies the name of the secret provider class.
`namespace`:: Specifies the namespace for the secret provider class. The namespace must match the namespace of the resource which will use the secret.
`provider`:: Specifies the name of the secret store provider.
`parameters`:: Specifies the provider-specific configuration parameters.
`objects`:: Specifies the secret name you created in AWS.
