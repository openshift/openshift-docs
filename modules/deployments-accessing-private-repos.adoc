...
[role="_abstract"]
To pull container images from a private repository into a `DeploymentConfig` object in {product-title}, you can add a pull secret to the object. Create the secret in the web console, then set it as the pull secret in the `DeploymentConfig` object.
