...
* link:https://raw.githubusercontent.com/openshift-kni/commatrix/release-4.19/docs/stable/raw/none-sno.csv[{sno-caps} with Platform=None]
...
== Additional network flows for {sno} with Platform=None

In addition to the base network flows, the following matrix describes the ingress flows to {product-title} services that are specific to {sno} configured with `Platform=None`.
