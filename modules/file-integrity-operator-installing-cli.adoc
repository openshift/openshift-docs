...
:navtitle: Install the File Integrity Operator using the CLI
[id="installing-file-integrity-operator-using-cli_{context}"]
= Installing the File Integrity Operator using the CLI

[role="_abstract"]
Install the File Integrity Operator from the {oc-first} by creating `Namespace`, `OperatorGroup`, and `Subscription` objects.
