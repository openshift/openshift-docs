...
{product-title} supports Container Storage Interface (CSI) volume group snapshots by default. However, the only storage type in {product-title} that supports volume group snapshots is {rh-storage}.
