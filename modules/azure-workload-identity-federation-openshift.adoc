...
+
For more information, see link:https://docs.openshift.com/container-platform/{ocp-latest-version}/authentication/managing_cloud_provider_credentials/cco-short-term-creds.html["Manual mode with short-term credentials for components"] in _{ocp}_ documentation.
...
`--issuer`:: Specifies the issuer. The issuer must match the service account token issuer of the {ocp} cluster.
--