...
. Install the earlier version of the {product-title-short} Operator:
.. In the OpenShift web console, click *Ecosystem* -> *Software Catalog*.
.. Search for *Advanced Cluster Security for Kubernetes*.
.. Select and install the earlier version of the Operator.
+
[NOTE]
====
Installing the earlier Operator version rolls back {product-title-short} to the same version as the Operator.
====
endif::cloud-svc[]
