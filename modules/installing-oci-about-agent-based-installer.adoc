...
* Compute flexible shapes, where you can customize the number of Oracle(R) CPUs (OCPUs) and memory resources for your VM. With access to this capability, a cluster's workload can perform operations in a resource-balanced environment. You can find all {op-system-base}-certified {oci} shapes by going to the Oracle page on the Red Hat Ecosystem Catalog portal.
