...
You can install an {product-title} cluster on {oci-first} by using the Agent-based Installer. Both Red{nbsp}Hat and Oracle test, validate, and support running {oci} workloads in an {product-title} cluster.
...
// Based on the actual workflow throughout this assembly and Oracle docs, some of the things in these images seem to be out of order. These images can probably be updated later.
