...
[role="_abstract"]
Enable the built-in Data Mover by configuring the CSI plugin and node agent in the `DataProtectionApplication` custom resource (CR). This enables volume-level backup and restore operations using the Kopia uploader.
