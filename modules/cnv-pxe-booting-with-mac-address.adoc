...
[id="cnv-pxe-booting-with-mac-address-{context}"]
...
file before you start the VMI. You can also specify a MAC address in the VMI
configuration file, if required by the PXE server.
...
[NOTE]
====
The VMI will be attached to the bridge `br1` through an access port with the
requested VLAN.
====
