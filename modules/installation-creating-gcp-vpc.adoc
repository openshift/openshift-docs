...
ifdef::shared-vpc[]
`<infra_id>`:: Specifies the prefix of the network name.
endif::shared-vpc[]
`<region>`:: Specifies the region to deploy the cluster into, for example `us-central1`.
`<master_subnet_cidr>`:: Specifies the CIDR for the master subnet, for example `10.0.0.0/17`.
`<worker_subnet_cidr>`:: Specifies the CIDR for the worker subnet, for example `10.0.128.0/17`.
...
where:
+
`<vpc_deployment_name>`:: Specifies the name of the VPC to deploy.
