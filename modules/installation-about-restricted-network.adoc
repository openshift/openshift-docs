...
For more information on mirroring images for a disconnected installation, see "Mirroring images for a disconnected installation".
...
Before deploying the cluster, you can update the installation configuration file (`install-config.yaml`) to specify the URI of an alternate service endpoint. For more information on usage, see "Additional {ibm-cloud-title} configuration parameters".
endif::ibm-cloud[]
