...
You can disable a user by setting the `enabled` field to `false`.
...
You can remove a user completely by deleting the entry from the `localUsers` list in the Argo CD CR.
