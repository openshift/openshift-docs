...
If you define the `lca.openshift.io/apply-wave` annotation in the `Backup` or `Restore` CRs, they will be applied in increasing order based on the annotation value.
If you do not define the annotation, they will be applied together.

You must restore the platform artifacts before restoring your application, so the `lca.openshift.io/apply-wave` annotation must be numerically lower in your platform `Restore` CRs, for example {rh-rhacm} and {lvms} artifacts, than that of the application so that the platform artifacts are restored before your applications.
