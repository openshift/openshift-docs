...
:navtitle: Set up a management cluster for {hcp} in a disconnected environment
[id="hcp-dc-mgmt-cluster_{context}"]
= Setting up a management cluster for {hcp} in a disconnected environment

[role="_abstract"]
An important part of a {hcp} deployment is the {product-title} management cluster. To set up an management cluster for a disconnected environment, you must install {mce} on it. The {mce-short} plays a crucial role in deploying clusters across providers.
