...
. Create a `ServiceAccount`, `Role`, and `RoleBinding` manifest for the DPDK checkup. For example:
+
[%collapsible]
[source,yaml]
----
...
. Create a `ConfigMap` manifest that contains the input parameters for the checkup. For example:
+
[source,yaml]
----
apiVersion: v1
