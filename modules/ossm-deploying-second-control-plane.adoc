...
= Deploy the second control plane

[role="_abstract"]

After deploying the first control plane, deploy the second control plane by creating its assigned namespace.
...
. Label the second namespace, which the {istio} `discoverySelectors` field uses by running the following command:
+
[source,terminal]
----
...
. Create a YAML file named `istio-2.yaml` similar to the following example:
+
