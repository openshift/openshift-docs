...
.Example user with expiring token
[source,yaml]
----
apiVersion: argoproj.io/v1beta1
kind: ArgoCD
...
.Example user with long-lived token
[source,yaml]
----
apiVersion: argoproj.io/v1beta1
kind: ArgoCD
...
.Example user with login capability
[source,yaml]
----
apiVersion: argoproj.io/v1beta1
kind: ArgoCD
...
| Disabled user | No (disabled) | Yes | Retains configuration | No
