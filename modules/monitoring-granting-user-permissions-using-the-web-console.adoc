...
[IMPORTANT]
====
The monitoring role or cluster role is be bound to the project that you apply in the *Namespace* field. The permissions that you grant to a user by using this procedure apply only to the selected project.
====

. Select the wanted monitoring role or cluster role from the *Role Name* list.
