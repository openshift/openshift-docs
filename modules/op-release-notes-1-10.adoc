...
With this update, {pipelines-title} General Availability (GA) 1.10 is available on {OCP} 4.11, 4.12, and 4.13.
...
[NOTE]
====
You need to update the custom task controller to listen for the `*v1beta1.CustomRun` API version instead of `*v1alpha1.Run` to respond to `v1beta1.CustomRun` requests.
====
...
* This update changes the `MinVersion` of TLS to `tls.VersionTLS12` so that triggers run on {OCP} when the Federal Information Processing Standards (FIPS) mode is enabled.
...
* This update enhances incoming webhook support. For a GitHub application installed on the {OCP} cluster, you do not need to provide the `git_provider` specification for an incoming webhook. Instead, {pac} detects the secret and use it for the incoming webhook.
...
* Before this update, OpenShift console UI would use a hard-coded pipeline run template as a fallback template when a runtime template was not found in the OpenShift namespace. This update in the `pipelines-as-code` config map provides a new default pipeline run template named, `pipelines-as-code-template-default` for the console to use.
...
With this update, {pipelines-title} General Availability (GA) 1.10.1 is available on {OCP} 4.11, 4.12, and 4.13.
...
* With this update, {pac} does not echo a secret to the GitHub checks interface when it failed to create the secret on the {OCP} cluster because of an infrastructure issue.
...
With this update, {pipelines-title} General Availability (GA) 1.10.2 is available on {OCP} 4.11, 4.12, and 4.13.
...
With this update, {pipelines-title} General Availability (GA) 1.10.3 is available on {OCP} 4.11, 4.12, and 4.13.
...
With this update, {pipelines-title} General Availability (GA) 1.10.4 is available on {OCP} 4.11, 4.12, and 4.13.
...
With this update, {pipelines-title} General Availability (GA) 1.10.5 is available on {OCP} 4.10 in addition to 4.11, 4.12, and 4.13.

[IMPORTANT]
====
{pipelines-title} 1.10.5 is only available in the `pipelines-1.10` channel on {OCP} 4.10, 4.11, 4.12, and 4.13. It is not available in the `latest` channel for any {OCP} version.
====
...
[id="release-notes-1-10-6_{context}"]
== Release notes for {pipelines-title} General Availability 1.10.6

With this update, {pipelines-title} General Availability (GA) 1.10.6 is available on {OCP} 4.10, 4.11, 4.12, and 4.13.
...
* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-linux-amd64.tar.gz[Linux (x86_64, amd64)]

* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-linux-s390x.tar.gz[Linux on {ibmzProductName} and {linuxoneProductName} (s390x)]

* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-linux-ppc64le.tar.gz[Linux on {ibmpowerProductName} (ppc64le)]

* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-linux-arm64.tar.gz[Linux on ARM (aarch64, arm64)]

* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-windows-amd64.zip[Windows]

* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-macos-amd64.tar.gz[macOS]

* link:https://mirror.openshift.com/pub/openshift-v4/clients/pipelines/1.10.6/tkn-macos-arm64.tar.gz[macOS on ARM]

If you installed the `tkn` command line tool using RPM on {op-system-base-full}, use the `yum update` command to install the updated version.

[id="known-issues-1-10-6_{context}"]
=== Known issues

* If you enter the `tkn task start` or `tkn clustertask start` command, the `tkn` command line utility displays an error message. As a workaround, to start tasks or cluster tasks using the command line, use the version of the `tkn` utility shipped with {pipelines-shortname} 1.11 or a later version.
