...
[role="_abstract"]
If you are using {VirtProductName} on a {product-title} cluster running Windows VMs you need to be license-compliant with Microsoft Windows in AWS. The hosts (metal EC2 instances) running these VMs must be enabled with AWS EC2 Windows License Included (LI).
...
* You identified a bare-metal EC2 instance type to use {VirtProductName}. For more, see link:https://aws.amazon.com/ec2/instance-types/#Bare_metal[Amazon EC2 Instance Types].
...
`--instance-type=<instance_type>`:: Specifies the instance type. You can only select a bare-metal instance type to enable Windows LI. For example, you can use `m5zn.metal` or `i3.metal`. You cannot change the instance type for a machine pool after the pool is created.
...
Message:                    Minimum availability requires 1 replica, current 1 available
----
