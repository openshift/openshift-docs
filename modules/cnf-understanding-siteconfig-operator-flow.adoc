...
The {sco} dynamically generates installation manifests based on user-defined cluster templates that use the data that is encapsulated in the `ClusterInstance` CR.
...
. Create a `ClusterInstance` CR that references those cluster templates.
. After the resources are created, the {sco} reconciles them by populating the templated fields that are referenced in the CR.
...
. The {sco} continuously monitors for changes in the `ClusterDeployment` CR.
