...
However, these scoped permissions cannot exceed the authority of the `dedicated-admin` role provided by your platform.
endif::[]
...
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
You cannot elevate a service account to interact with restricted namespaces maintained by Red{nbsp}Hat SRE or core control plane components.
endif::[]
...
ifdef::openshift-rosa-hcp,openshift-rosa,openshift-dedicated[]
This can only occur if the updated permissions remain within the delegated administrative boundaries of the cluster.
endif::[]
