...
prune job will accept the same sync configuration file and allows or denies
...
[source,terminal]
----
$ oc adm prune groups --allow=/path/to/allow.txt --sync-config=/path/to/ldap-sync-config.yaml --confirm
----
