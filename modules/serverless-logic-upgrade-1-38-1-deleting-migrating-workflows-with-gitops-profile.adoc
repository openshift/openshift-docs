...
[role="_abstract"]
For workflows that use the `gitops` profile, it is strongly recommended to rebuild the workflow image that you generated for version `1.38` before upgrading the Operator. Rebuilding the image ensures compatibility with {ServerlessLogicProductName} `1.39.0`.
