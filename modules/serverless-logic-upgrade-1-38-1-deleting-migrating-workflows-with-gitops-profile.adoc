...
[id="serverless-logic-upgrade-1-38-1-deleting-migrating-workflows-with-gitops-profile_{context}"]
= Deleting workflows with the gitops profile

[role="_abstract"]
For workflows that use the `gitops` profile, it is strongly recommended to rebuild the workflow image that you generated for version `1.38.1` before upgrading the Operator. Rebuilding the image ensures compatibility with {ServerlessLogicProductName} `1.39.0`.

For each workflow, for example `my-workflow`, complete the following steps.

.Procedure

. Rebuild the workflow image by using the {ServerlessLogicProductName} `1.39.0` workflow builder image in your build pipeline or build configuration:
+
----
registry.redhat.io/openshift-serverless-1/logic-swf-builder-rhel9:1.39.0
----
