...
You can configure the `secretPassthrough` field to prevent BGP passwords from being exposed in cleartext in the generated `FRRConfiguration` custom resources. When enabled, the MetalLB Operator passes BGP password Secret references to the FRR-K8s daemon instead of resolving them to cleartext.
...
spec:
  frrk8sConfig:
    secretPassthrough: true <1>
# ...
----
<1> When set to `true`, the MetalLB Operator passes BGP password Secrets as references to FRR-K8s. BGP peer Secrets must reside in the `openshift-frr-k8s` namespace.
