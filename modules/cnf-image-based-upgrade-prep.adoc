...
+
--
`version: 4.15.2`:: Target platform version. The value must match the version of the seed image.
`image: <seed_container_image>`:: Repository where the target cluster can pull the seed image from.
`pullSecretRef: <seed_pull_secret>`:: Reference to a secret with credentials to pull container images if the images are in a private registry.
`#    initMonitorTimeoutSeconds: 1800`:: Optional: Time frame in seconds to roll back if the upgrade does not complete within that time frame after the first reboot. If not defined or set to `0`, the default value of `1800` seconds (30 minutes) is used.
`extraManifests:`:: Optional: List of `ConfigMap` resources that contain your custom catalog sources to retain after the upgrade and your extra manifests to apply to the target cluster that are not part of the seed image.
