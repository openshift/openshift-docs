...
* You have installed the `oc` and the `kubectl` command-line interfaces (CLIs).
...
[NOTE]
====
After applying the `ClusterRoleBinding`, the {product-title} cluster is configured and the `rosa` CLI and {hybrid-console-url} authenticate through the external OIDC provider. You can now start assigning roles and deploying applications on the cluster.
====