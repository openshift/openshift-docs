...
[id="bonding-multiple-network-interfaces-with-mtu_{context}"]
[discrete]
=== Bonding multiple network interfaces to a single interface with an MTU
Optional: You can configure the maximum transmission unit (MTU) on a bonded interface by adding `bond0:none:9000` to the `ip=` parameter to specify the MTU, for example: 

[source,terminal]
----
bond=bond0:em1,em2:mode=active-backup,fail_over_mac=1 
ip=10.10.10.2::10.10.10.254:255.255.255.0:core0.example.com:bond0:none:9000
----
