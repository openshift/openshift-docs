...
The table below provides examples for configuring networking of your {op-system-first} nodes.  These are networking options that are passed to `dracut` during the boot of the system.  For more information about the networking options supported by `dracut`, please see the manpage for `dracut.cmdline`.
...
To configure a VLAN on a network interface and use DHCP:
