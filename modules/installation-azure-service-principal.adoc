...
:navtitle: Create a service principal
[id="installation-azure-service-principal_{context}"]
= Creating a service principal

[role="_abstract"]
To enable {product-title} to create Azure resources, you must create a service principal that represents the installation program in Azure Resource Manager.
...
`<years>`:: Specifies the number of years. By default, a service principal expires in one year. By using the `--years` option you can extend the validity of your service principal.
+
.Example output
[source,terminal]
----
...
`<role_name>`:: Specifies the role name. You can use the `Contributor` role, or you can specify a custom role which contains the necessary permissions.
`<service_principal>`:: Specifies the service principal name.
`<subscription_id>`:: Specifies the subscription ID.
+
.Example output
