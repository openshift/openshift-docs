...
= Creating a storage class for the HPP CSI driver
...
To solve this problem, use the Kubernetes pod scheduler to bind the persistent volume claim (PVC) to a PV on the correct node. By setting `volumeBindingMode` to `WaitForFirstConsumer`, the binding and provisioning of the PV are delayed until a pod is created using the PVC.
====
