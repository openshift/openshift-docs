...
[role="_abstract"]
You can switch the Ingress Controller that is using a Network Load Balancer (NLB) to one that uses a Classic Load Balancer (CLB) on {aws-full}. Switching between these load balancers does not delete the `IngressController` object.
