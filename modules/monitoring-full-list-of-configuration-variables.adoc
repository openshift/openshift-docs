...
This table contains the full list of the inventory file variables for configuring the {cmo-first}:

.{cmo-full} Ansible variables
[options="header"]
|===
...
| Deploy the {cmo-full} if `true`. Otherwise, undeploy. This variable is set to `true` by default.
