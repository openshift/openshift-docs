...
[role="_abstract"]
You can store the size and digest or the content hash of the image that is created by your build strategy to a set of result files. You can also store error details for debugging purposes when a `BuildRun` resource fails. You can define the following result parameters in your `BuildStrategy` or `ClusterBuildStrategy` CR:
...
| `$(results.shp-image-digest.path)`  | Denotes the path to the file that stores the digest or the content hash of the image.
