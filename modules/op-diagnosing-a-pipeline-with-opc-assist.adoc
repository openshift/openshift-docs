...
[role="_abstract"]
Use the `opc assist` CLI command to diagnose failed `PipelineRun` executions by analyzing logs, error messages, and execution context with {ols-official}.
...
[IMPORTANT]
====
To use the `opc assist` CLI command, you must install and configure {ols-official} on your cluster.
====
...
* link:https://docs.redhat.com/en/documentation/red_hat_openshift_lightspeed/latest/html/install/ols-installing-lightspeed[Installing {ols-official}]
