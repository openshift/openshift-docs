...
You can create an `IntegrationSource` to produce periodic messages with a custom payload by applying a YAML manifest.
...
----
+
This manifest configures the `IntegrationSource` to emit a message `"Hello, Eventing Core"` every 2000 milliseconds (2 seconds)The generated messages are sent to the `default` Knative broker defined in the sink section.