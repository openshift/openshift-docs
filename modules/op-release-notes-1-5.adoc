...
{pipelines-title} General Availability (GA) 1.5 is now available on {OCP} 4.8.
...
* In {OCP}, you can customize the installation of the Tekton Add-ons component by modifying the values of the new parameters `clusterTasks` and `pipelinesTemplates` in the `TektonConfig` custom resource:
+
[source,yaml,subs="attributes+"]
----
...
* The `enableMetrics` parameter is added to the `TektonConfig` custom resource. You can use it to disable the service monitor, which is part of Tekton Pipelines for {OCP}.
+
[source,yaml,subs="attributes+"]
----
apiVersion: operator.tekton.dev/v1alpha1
...
* On {ibmpowerProductName} Systems, {ibmzProductName}, and LinuxONE, the `s2i-dotnet` cluster task and the `tkn hub` command are unsupported.
