...
[NOTE]
====
To use IMDSv2 on AWS clusters that were created with {product-title} version 4.7 or earlier, you must update your boot image. For more information, see "Updated boot images".

Enabling IMDSv2 on clusters installed with {product-title} versions earlier than 4.7 without updating the boot image can cause machines to fail to boot. Ensure that you update the boot image as per the instructions in "updating AWS boot images for OpenShift before enabling IMDSv2".
====
