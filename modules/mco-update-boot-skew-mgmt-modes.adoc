...
Automatic:: 
When set to `Automatic`, the Machine Config Operator (MCO) examines the boot image for version skew. If skew is detected, the MCO blocks cluster updates until you manually update the boot image or disable boot image skew enforcement by setting the `None` mode. Automatic updates of the boot image is not currently supported in {product-title}.
endif::openshift-rosa[]
