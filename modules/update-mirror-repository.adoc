...
:navtitle: Mirrore images to a mirror registry
[id="update-mirroring-images-to-registry_{context}"]
= Mirroring images to a mirror registry

[role="_abstract"]
Before you can update a cluster in a disconnected environment, you must mirror the required {product-title} release images onto a local registry mirror.
