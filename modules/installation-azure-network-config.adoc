...
:navtitle: Configure a public DNS zone in {azure-short}
[id="installation-azure-network-config_{context}"]
= Configuring a public DNS zone in {azure-short}

[role="_abstract"]
To install {product-title}, the {azure-first} account you use must have a dedicated public hosted DNS zone in your account that is authoritative for the domain. This zone provides cluster DNS resolution and name lookup for external connections to the cluster.
