...
{product-title} uses the `MachineHealthCheck` controller to detect faulty bare metal nodes by initiating power-cycling. Power-based remediation is fast and reboots unhealthy hosts instead of removing them from the cluster.
...
. The MHC notifies the bare metal machine controller requests to power-off the unhealthy node.
...
[NOTE]
====
If the power operations did not complete, the bare metal machine controller triggers the reprovisioning of the unhealthy node unless this is a master node or a node that is reprovisioned externally.
====
