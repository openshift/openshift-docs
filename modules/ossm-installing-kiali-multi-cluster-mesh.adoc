...
= Install Kiali in a multi-cluster mesh
...
.. Create a `kubeconfig` file and save it as a secret in the namespace on the East cluster where the Kiali deployment is present.
+
...
.. Enter the script so that it passes the East and West cluster contexts to the `kubeconfig` file by running the following command: 
+
[source,terminal]
----
...
.. When you first access Kiali, log in to the cluster that has the Kiali deployment. In this example, access the `East` cluster.
...
.. Select *Login to West*. The {ocp-short-name} login page appears and requires your West cluster credentials to continue.
