...
<16> Optional: The link type for the VFs. The default value is `eth` for Ethernet. Change this value to `ib` for InfiniBand.
+
...
//Commented out as there doesn't seem to be a corresponding callout 9/19/2022.
//<17> Optional: To enable hardware offloading, the 'eSwitchMode' field must be set to `switchdev`.
