...
* You have configured a `DataProtectionApplication` CR.
* A `PriorityClass` object must exist in the cluster.

.Procedure

. Edit the DPA manifest and configure the `priorityClassName` field in the `podConfig` object of the `DataProtectionApplication` CR for the node agent, Velero, or both, as shown in the following example:
+
...
where:
+
`<dpa_file_name>`:: Specifies the file name of the `DataProtectionApplication` CR.
...
[source,terminal]
----
node-agent-xxxxx system-cluster-critical
node-agent-yyyyy system-cluster-critical
----
...
[source,terminal]
----
velero-xxxxx system-cluster-critical
----