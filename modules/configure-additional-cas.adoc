...
[role="_abstract"]
You can add custom certificate authorities (CAs) to {product-title} by using the `ca-setup.sh` script to configure single or multiple CA certificates.

.Procedure

. Download the `ca-setup.sh` script.
+
[NOTE]
====
...
* You can also use the `-u` (update) option along with the `-f` option to update any certificate added before.
====
.. Many certificates at once, move all certificates in a directory, and then use the `-d` (directory) option:
+
[source,terminal]
----
...
* You can also use the `-u` (update) option along with the `-d` option to update any certificates added before.
====