...
You can copy and modify a default compute machine set configuration to create a GPU-enabled machine set and machines for the {gcp-full} cloud provider.
...
. View the existing nodes, machines, and machine sets by running the following command. Note that each node is an instance of a machine definition with a specific {gcp-full} region and {product-title} role.
+
[source,terminal]
----
$ oc get nodes
