...
`metadata.labels.security.openshift.io/scc.podSecurityLabelSync`:: Specifies turning off pod security label synchronization by adding the `security.openshift.io/scc.podSecurityLabelSync=false` label to the namespace.
`metadata.labels.pod-security.kubernetes.io/enforce`:: Specifies the pod security admission `pod-security.kubernetes.io/enforce` label. Set the label to `baseline` or `privileged`. Use the `baseline` pod security profile unless other workloads in the namespace require a `privileged` profile.
