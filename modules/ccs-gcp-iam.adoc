...
Red Hat is responsible for creating and managing the following IAM {gcp-full} resources.
...
The `osd-managed-admin` IAM service account is created immediately after taking control of the customer-provided {gcp-full} account. This is the user that will perform the {product-title} cluster installation.
...
The `sd-sre-platform-gcp-access` Google group is granted access to the {gcp-full} project to allow Red Hat Site Reliability Engineering (SRE) access to the console for emergency troubleshooting purposes.
