...
[id="oadp-using-must-gather_{context}"]
...
* You must use {op-system-base-full} {op-system-version-9} with {oadp-short} 1.5.
...
[source,terminal,subs="attributes+"]
----
$ oc adm must-gather --image={must-gather-v1-5} -- /usr/bin/gather
----
+
...
. You can upload this output file to a support case on the link:https://access.redhat.com/[Red{nbsp}Hat Customer Portal].
