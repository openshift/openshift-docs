...
[source,terminal,subs="attributes+"]
----
$ oc adm must-gather --image={must-gather-v1-4} -- /usr/bin/gather_without_tls <true/false> # <1>
----
<1> By default, the flag value is set to false. Set the value to true to allow insecure TLS connections.
