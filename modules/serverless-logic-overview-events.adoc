...
You can define an event state with separate `onEvents` items for `noisy` and `silent` `CloudEvent` types, and set the exclusive property to `false` to run different actions when both events occur.
