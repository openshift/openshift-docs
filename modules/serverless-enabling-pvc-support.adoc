...
. To enable Knative Serving to use PVCs and write to them, change the `KnativeServing` custom resource (CR) to include the following YAML:
+
...
----
+
* The `kubernetes.podspec-persistent-volume-claim` extension controls whether Knative Serving can use persistent volumes (PVs).
...
The `ocs-storagecluster-cephfs` storage class is supported and comes from link:https://docs.openshift.com/container-platform/latest/storage/persistent_storage/persistent-storage-ocs.html[{rh-storage-first}].
====
+
...
----
+
In this case, to claim a PV with write access, change your service as follows:
+
