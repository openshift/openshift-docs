...
:navtitle: Configure how pods behave after restart
[id="nodes-pods-configuring-restart_{context}"]
= Configuring how pods behave after restart

[role="_abstract"]
You can configure a pod restart policy to determine how {product-title} responds when containers in that pod exit. Having a proper restart policy helps you keep your cluster running efficiently.
