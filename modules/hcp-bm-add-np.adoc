...
[NOTE]
====
Only a single agent namespace is supported per hosted cluster. As a result, when you add a node pool to a hosted cluster, the node pool must be either from a single `InfraEnv` resource or from an `InfraEnv` resource that is in the same agent namespace.
====
