...
* `gnss-sync-status` notifications describe the GPS synchronization state with regard to the external GNSS clock signal. Can be in `SYNCHRONIZED`, `ANTENNA_DISCONNECTED`, or `ACQUIRING_SYNC` state.
