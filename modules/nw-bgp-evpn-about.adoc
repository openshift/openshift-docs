...
[role="_abstract"]
Border Gateway Protocol (BGP) Ethernet Virtual Private Networking (EVPN) is a standards-based control-plane that exchanges layer 2 and layer 3 overlay network reachability within the data center. Enabling this feature on {product-title} allows a `ClusterUserDefinedNetwork` (CUDN) overlay network to use the EVPN control plane for deeper integration with the data center network.
...
EVPN defines two VPN types that you can deploy with primary CUDNs when you attach the overlay to the fabric:
...
* BGP EVPN with primary CUDNs is supported only on bare-metal clusters.
* BGP EVPN with CUDNs is only supported in local gateway mode.
