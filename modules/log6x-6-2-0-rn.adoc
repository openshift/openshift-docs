...
[id="logging-release-notes-6-2-0-technology-preview-features"]
== Technology Preview
...
* With this update, Loki can now drop attributes of type resource, scope or log from a log entry in the OpenTelemetry data format before storing the log. Attributes that are not explicitly set as stream labels or dropped from the entry are saved as structured metadata by default.
