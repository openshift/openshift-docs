...
= Remove a multi-cluster topology from a development environment
...
. Remove {istio} and the sample applications from the West cluster of the development environment by running the following command:
+
[source,terminal]
----
...
. If you used {SMProductShortName} in ambient mode, remove the ztunnel resources: 

.. Remove the resources from the East cluster with the following command:
+
[source,terminal]
----
$ oc --context="${CTX_CLUSTER1}" delete ns/ztunnel
----

.. Remove them from the West cluster with the following command:
+
[source,terminal]
----
$ oc --context="${CTX_CLUSTER2}" delete ns/ztunnel
----