...
The guidance provided in this documentation focuses on general aspects of cluster deployment that spans data centers. Some caveats to remember:

* While the designs for deployments that span data centers are not bound by any special support requirements, these clusters do have additional inherent complexities that can require additional consideration or support involvement (time to identify, remediate and resolve issues) when compared to a standard single-site cluster.
