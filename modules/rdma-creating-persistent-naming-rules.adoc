...
In some cases, device names won't persist following a reboot. For example, on R760xa systems 
Mellanox devices might be renamed after a reboot. You can avoid this problem by using a `MachineConfig` to set persistence.
...
. Create a machine configuration and set the base64 encoding in the custom resource file:
+
[source,terminal]
----
...
. Create the machine configuration on the cluster:
+
[source,terminal]
----
