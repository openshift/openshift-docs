...
* Maximum number of PIDs per node.
+
The default value depends on node resources. In {product-title}, this value is controlled by the `--system-reserved` parameter, which reserves PIDs on each node based on the total resources of the node. For more information, see _Additional resources_.
...
* link:https://access.redhat.com/documentation/en-us/openshift_container_platform/latest/html-single/nodes/index#nodes-nodes-resources-configuring[Configuring node resources]
* link:https://kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/#system-reserved[Kubernetes system-reserved parameter]
* link:https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#eviction-signals-and-thresholds[Kubernetes eviction signals and thresholds]
