...
Cluster administrators use a custom Operator catalog image based on the Package
Manifest Format to create a CatalogSource that loads the Operator content onto
their cluster. For this example, the procedure uses a custom `redhat-operators`
...
You can now install the Operators from your customer catalog by using the the
*OperatorHub* page on your {product-title} cluster web console.
