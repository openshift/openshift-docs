...
= Configure the collector to receive audit logs as an HTTP server

[role="_abstract"]
Configure a log collector to listen for HTTP connections and receive audit logs.
...
inputs.receiver.type::
  Specify the input receiver type as `http`.

inputs.receiver.port::
