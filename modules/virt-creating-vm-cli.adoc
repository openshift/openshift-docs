...
. Create a `VirtualMachine` manifest for your VM and save it as a YAML file:
+
[source,terminal]
----
$ virtctl create vm --name <vm_name> --volume-import type:ds,src:<namespace>/<datasource>
----
+
For example, to create a minimal {op-system-base-full} VM, run the following command:
+
.Example `virtctl` command for creating a {op-system-base} VM
[source,terminal]
----
...
----
<1> The VM name.
<2> The `rhel9` golden image is used to install {op-system-base} 9 as the guest operating system.
<3> Golden images are stored in the `openshift-virtualization-os-images` namespace.
