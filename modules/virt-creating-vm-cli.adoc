...
name (metadata):: The VM name.
name (sourceRef):: The boot source for the guest operating system.
namespace:: The namespace for the boot source. Golden images are stored in the `openshift-virtualization-os-images` namespace.
inferFromVolume (instancetype):: The instance type is inferred from the selected `DataSource` object.
inferFromVolume (preference):: The preference is inferred from the selected `DataSource` object.
