...
= Setting scrape and evaluation intervals and scrape limits for user-defined projects
...
. Add the interval and enforced limit configurations to `data/config.yaml`:
+
[source,yaml]
----
apiVersion: v1
...
<3> Specifies the maximum length in characters of a label name.
The default value is `0`, which specifies no limit.
<4> Specifies the maximum length in characters of a label value.
The default value is `0`, which specifies no limit.
