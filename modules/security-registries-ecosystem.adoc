...
Red Hat images are actually stored in what is referred to as the _Red Hat Registry_, which is represented by a public container registry (`registry.access.redhat.com`) and an authenticated registry (`quay.io/openshift-community`). Both include basically the same set of container images, with
endif::[]
...
`quay.io/openshift-community` including some additional images that require authentication with Red Hat subscription credentials.
endif::[]
