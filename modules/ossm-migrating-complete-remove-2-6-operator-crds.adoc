...
After you remove the {SMProductName} 2 `ServiceMeshControlplane` resource, and all other {SMProduct} 2.6 resources, you can remove the {SMproduct} 2.6 Operator and custom resource definitions (CRDs).
...
.. Delete the `clusterserviceversion` CSV by running the following command:
+
[source,terminal]
----
