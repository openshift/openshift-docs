...
:navtitle: Mirrore on a remote host with mirror registry for Red Hat OpenShift
[id="mirror-registry-remote_{context}"]
= Mirroring on a remote host with mirror registry for Red Hat OpenShift

[role="_abstract"]
You can install the _mirror registry for Red{nbsp}Hat OpenShift_ on a remote host by using the `mirror-registry` tool. By doing so, users can create a registry to hold a mirror of {product-title} images.
...
----
+
You can avoid running `--tls-verify=false` by configuring your system to trust the generated rootCA certificates. See "Securing {quay}" and "Configuring the system to trust the certificate authority" for more information.
+
[NOTE]
====
