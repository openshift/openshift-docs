...
[role="_abstract"]
The edge compute pool configuration is common between {aws-first} {zone-type} locations. You can use the edge compute pool to create new labels to deploy applications onto {aws-first} {zone-type} nodes.

Edge compute nodes are tainted compute nodes that run in {aws-short} {zone-type} locations.
