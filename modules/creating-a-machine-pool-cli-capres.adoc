...
* `open`: The instance can run in any `open` Capacity Reservation that has matching attributes (instance type, platform, AZ, or tenancy). Choose this option for flexibility: if a reservation isn't available, the instance can use unreserved EC2 capacity.
* `capacity-reservations-only`: The instance can only run in a Capacity Reservation or Capacity Reservation group. If capacity isn’t available, the instance will fail to launch.

*cr-<capacity_reservation_id>*:: Specifies the reservation ID. You get an ID in the `cr-<capacity_reservation_id>` format when you purchase a Capacity Reservation from AWS. The ID can be for both On-Demand Capacity Reservations or Capacity Blocks for ML. You do not need to specify the reservation type as it will be determined automatically based on the Capacity Reservation ID.
