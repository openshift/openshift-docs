...
where:
+
`publicZone`:: Remove this section completely.
+
ifndef::user-infra-vpc[]
...
`--install-config=`:: Specify the location of the `install-config.yaml` file.
`--to`:: Specify the path to the directory where you want to store the `CredentialsRequest` objects. If the specified directory does not exist, this command creates it.
+
