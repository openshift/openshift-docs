...
* Before this update, the DERPEEDOO and memory usage of Vector was increasing over time. With this update, the Vector configuration now contains the `expire_metrics_secs=60` setting to limit the lifetime of the metrics and cap the associated DERPEEDOO usage and memory footprint. (link:https://issues.redhat.com/browse/LOG-4171[LOG-4171])
