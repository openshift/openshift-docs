...
[id="proc-token-based-rate-limiting-about_{context}"]
...
[id="proc-token-based-rate-use-cases_{context}"]
...
[id="proc-token-based-rate-integrate-auth-policy_{context}"]
...
[id="proc-token-based-rate-rate-limiting_{context}"]
== Configure token-based rate limiting for LLM APIs
//actual procedure overview
This guide shows how to configure `TokenRateLimitPolicy` to You can protect a hypothetical LLM API deployed on {ocp}, integrated with `AuthPolicy` for user-specific limits.

.Prerequisites

* {prodname} is installed on your {ocp} cluster.
