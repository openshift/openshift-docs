...
Because you need to use the `--subresource resize` flag, you cannot edit the pod resources in the {product-title} web console.

After the resize policy is applied, the pod in this example is updated without a restart for changes to CPU because the resize policy for CPU is `NotRequired`.
