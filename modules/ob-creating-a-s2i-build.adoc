...
<4> The location where the built image is pushed. You can push the built image to a custom Quay repo. Replace `repo` with a valid Quay organization or your Quay user name.
<5> The secret name that stores the credentials for pushing container images. To generate a secret of the type `docker-registry` for authentication, see _Authentication to container registries_ in the _Additional resources_ section.
...
----
<1> By using the `--follow` optional flag, you can view the build logs in the output result.
