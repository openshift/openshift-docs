...
Workload partitioning (`cpuPartitioning: AllNodes`) must be enabled at install time, or the `kubelet` CPU Manager `strict-cpu-reservation` policy option must be configured in your OVS-DPDK performance profile using the `kubeletconfig.experimental` annotation. The performance profile populates the workload partitioning CPU sets when applied. If neither prerequisite is enabled, the Node Tuning Operator marks the performance profile as `Degraded` with the reason `OvsDpdkCPUsPrerequisiteNotMet` and does not complete the configuration, because `Burstable` and `BestEffort` pods could still be scheduled on the OVS-DPDK CPUs.

[IMPORTANT]
====
If workload partitioning is not enabled, you must configure `strict-cpu-reservation` using the `kubeletconfig.experimental` annotation in your OVS-DPDK performance profile. Do not use a standalone `KubeletConfig` custom resource, as the Node Tuning Operator validates prerequisites by checking the performance profile-generated `KubeletConfig`, not standalone `KubeletConfig` resources.
====
