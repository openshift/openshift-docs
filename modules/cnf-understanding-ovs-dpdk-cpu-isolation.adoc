...
* Enable workload partitioning at cluster install time by setting `cpuPartitioning: AllNodes`. The performance profile populates the workload partitioning CPU sets when applied.

* Configure `strict-cpu-reservation` using the `kubeletconfig.experimental` annotation in your OVS-DPDK performance profile. Do not use a standalone `KubeletConfig` custom resource. The Node Tuning Operator validates prerequisites by checking the performance profile-generated `KubeletConfig`, not standalone resources.
