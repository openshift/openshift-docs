...
:navtitle: Set the cluster node hostnames through DHCP
[id="ipi-install-setting-cluster-node-hostnames-dhcp_{context}"]
= Setting the cluster node hostnames through DHCP

[role="_abstract"]
On {op-system-first} machines, `NetworkManager` sets the hostnames. By default, DHCP provides the hostnames to `NetworkManager`, which is the recommended method. 
