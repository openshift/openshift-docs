...
On {op-system-first} machines, `NetworkManager` sets the hostnames. By default, DHCP provides the hostnames to `NetworkManager`, which is the recommended method. If DHCP does not provide the hostnames, if you use kernel arguments to set the hostnames, or if you use another method to set the hostnames, `NetworkManager` gets the hostnames through a reverse DNS lookup. 
