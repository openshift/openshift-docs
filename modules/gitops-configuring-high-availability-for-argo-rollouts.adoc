...
//
// * extend/enabling-ha-support-for-argo-rollouts.adoc
...
[role="_abstract"]
Enable high availability for Argo Rollouts by configuring the `ha` specification in the `RolloutManager` custom resource (CR). When HA is enabled, the Operator sets the controller pod count to 2 and activates leader election.
