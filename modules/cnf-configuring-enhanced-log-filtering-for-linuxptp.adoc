...
----
+
<1>  By default, the `linuxptp-daemon` is configured to generate summary logs every 30 seconds if no value is specified. In the example configuration, the daemon generates summary logs every 60 seconds and a threshold of 100 nanoseconds for the master offset logs is set. This means the daemon only produces summary logs at the specified interval. However, if your clock's offset from the master exceeds plus or minus 100 nanoseconds, that specific log entry is recorded. You can set the interval independently of the threshold. For example, setting the value of the `logReduce` field to `enhanced 60s` configures a 60-second interval without a master offset threshold.  
