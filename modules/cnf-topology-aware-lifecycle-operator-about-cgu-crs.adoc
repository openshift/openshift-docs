...
// * scalability_and_performance/cnf-{cgu-operator}-for-cluster-upgrades.adoc
...
As the {cgu-operator} works through remediation of the policies to the specified clusters, the `ClusterGroupUpgrade` CR can have the following states:
...
[NOTE]
====
After the {cgu-operator} completes a cluster update, the cluster does not update again under the control of the same `ClusterGroupUpgrade` CR. You must create a new `ClusterGroupUpgrade` CR in the following cases:
...
The {cgu-operator} builds a remediation plan based on the following fields:
...
The `ClusterGroupUpgrade` CR transitions to the `UpgradeNotCompleted` state after the remediation plan is successfully created and after the `enable` field is set to `true`. At this point, the {cgu-operator} starts to update the non-compliant clusters with the specified managed policies.
...
In the `UpgradeNotCompleted` state, the {cgu-operator} enforces the policies following the remediation plan defined in the `UpgradeNotStarted` state.

Enforcing the policies for subsequent batches starts immediately after all the clusters of the current batch are compliant with all the managed policies. If the batch times out, the {cgu-operator} moves on to the next batch. The timeout value of a batch  is the `spec.timeout` field divided by the number of batches in the remediation plan.
...
In the `UpgradeTimedOut` state, the {cgu-operator} checks every hour if all the policies for the `ClusterGroupUpgrade` CR are compliant. The checks continue until the `ClusterGroupUpgrade` CR is deleted or the updates are completed.
The periodic checks allow the updates to complete if they get prolonged due to network, CPU, or other issues. 

The {cgu-operator} transitions to the `UpgradeTimedOut` state in two cases:
...
If the policies are compliant, the {cgu-operator} transitions to the `UpgradeCompleted` state.
