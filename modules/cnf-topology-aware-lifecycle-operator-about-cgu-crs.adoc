...
TALO builds the remediation plan from the `ClusterGroupUpgrade` CR for a group of clusters. You can define the following specifications in a `ClusterGroupUpgrade` CR:
...
As TALO works through remediation of the policies to the specified clusters, the `ClusterGroupUpgrade` CR can be in the following states:
...
* If the current batch is of canary type and the cluster in the batch does not become compliant with all the managed policies within the batch timeout.
