...
:navtitle: Create an {product-title} service account for {gcp-short}
...
`metadata.annotations.cloud.google.com/workload-identity-provider`:: Specifies the workload identity provider for the service account of the cluster.
`metadata.annotations.cloud.google.com/audience`:: Specifies the allowed audience for the workload identity provider.
`metadata.annotations.cloud.google.com/token-expiration`:: Specifies the token expiration time period in seconds.
`metadata.annotations.cloud.google.com/injection-mode`:: Specifies the `direct` external credentials configuration injection mode.
