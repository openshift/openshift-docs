...
= Upgrading the OpenShift Serverless Logic Operator to 1.37.0
...
. In every namespace where a `SonataFlowPlatform` resource is installed and configured with `dbMigrationStrategy: job` for either Data Index or Jobs Service, remove the associated database migration job by running the following command:
+
[source,terminal]
----
