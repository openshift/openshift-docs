...
<<<<<<< Updated upstream
Each Jaeger instance is associated with a deployment strategy. You define the strategy in the custom resource file.  When you create a Jaeger instance the Operator uses the deployment strategy and configuration options defined in the CR file to create the objects necessary for the deployment.
=======
You can deploy Jaeger following different deployment strategies. You define the strategy in the custom resource file. When you create a Jaeger instance the Operator uses this configuration file to create the objects necessary for the deployment.
>>>>>>> Stashed changes
...
====
<<<<<<< Updated upstream
In-memory storage is not persistent, which means that if the Jaeger instance shuts down, restarts, or is replaced, that your trace data will be lost.  For persistent storage, use the production or streaming strategies, which use Elasticsearch as the default storage.
=======
