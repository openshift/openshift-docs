...
:gitops-ver: 1.16
// for the :upstream-ver: field, update the value with whatever release number you are updating in the compatibility matrix field in the gitops X.Y RN under the Argo CD CLI column. Later when GitOps X.Y will become GA, the gitops-docs-main branch must reflect the same details as 1the existing Y relesase, as mentioned above. Likewise, remember to update the _attributes/common-attributes.adoc file for every release in the future.
:upstream-ver: 2.13
