...
* When creating the *KubeVirt HyperConverged Cluster Operator Deployment*
custom resource during {CNVProductName} installation, a YAML file is displayed
with an incorrect value. The file resembles the following example:
+
[source,yaml]
----
...
----
<1> The single quotation marks around the word `'false'` are incorrect.
You must edit the file so that the line reads `BareMetalPlatform: false` before
you click *Create*. If the quotation marks are not removed, deployment is not successful.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1767167[*BZ#1767167*])
...
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1754920[*BZ#1754920*])
+
