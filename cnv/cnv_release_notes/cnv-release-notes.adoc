...
* Enhancements to the single virtual machine import from VMware workflow.

* Health monitoring of Virtual Machine-backed services that is equivalent to that for pod-based ones.

* A persistent local storage option for VM images.

* A new virtual machine > dashboard screen.

* An option to view and edit the CD-ROMs attached to a VM as well as attaching new ones.

* Exposing the network attachment definitions as a list and a graphical creation editor.

* The virtual machine creation wizard has been completely redesigned. It is now using an in-page style and has a much better review page.

* Template details can now be edited (until now only the VM details had this ability.
