...
function collapseExpandAll() {
  const collapsibleContentBlocks = document.getElementsByTagName("details");
  const collapseExpandAllButton = document.getElementById("button-collapse-expand-all");
  ...
  if (collapsibleTooltip[0].innerHTML == "Collapse all") {
    for (var i=0; i < collapsibleContentBlocks.length; i++) {
        collapsibleContentBlocks[i].removeAttribute("open");
    }
    ...
    for (var i=0; i < collapsibleContentBlocks.length; i++) {
        collapsibleContentBlocks[i].setAttribute("open", "");
    }
    for (var j=0; j < collapsibleTooltip.length; j++) {
