...
name.
.. If required,
xref:../install_config/configuring_authentication.adoc#identity-providers_parameters[set `*mappingMethod*`] 
...
xref:../install_config/sssd_for_ldap_failover.adoc#setting-up-for-ldap-failover[configuring LDAP failover]
...
With the
xref:../install_config/sssd_for_ldap_failover.adoc#setting-up-for-ldap-failover[SSSD failover setup],
FreeIPA and Active Directory can also set rules to specifically restrict which
users can and cannot access {product-title}.

The following advanced topic begin where this basic LDAP authentication
topic ends and describe
xref:../install_config/sssd_for_ldap_failover.adoc#setting-up-for-ldap-failover[configuring LDAP failover].
====
...
identity provider *name* (defined in the `*identityProviders*` stanza of the xref:../install_config/master_node_configuration.adoc#master-configuration-files[*_master configuration file_*], which you configure in the next section of this topic):
+
----
