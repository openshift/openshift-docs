...
Red Hat also recommends creating an {product-title} cluster deployed on {gcp-short} in Private cluster mode with Private Service Connect (PSC) to manage and monitor a cluster to avoid all public ingress network traffic.

////
[id="osd-getting-started-prerequisites"]
== Prerequisites

* You reviewed the introduction to {product-title} and the documentation on architecture concepts.
* You reviewed the {product-title} cloud deployment options.
////
