...
# openshift_portal_net is the range that services and Octavia load balancer
# VIPs use. Amphora VMs use Neutron ports in the range that is defined by
# [openshift_openstack_kuryr_service_pool_start, openshift_openstack_kuryr_service_pool_end].
#
# The value of openshift_portal_net in OSEv3.yml must be within the range that is
# defined by openshift_openstack_kuryr_service_subnet_cidr. Typically, this range is half of
# openshift_openstack_kuryr_service_subnet_cidr's range.
# This practice ensures that openshift_portal_net does not overlap with the range that is defined
# by [openshift_openstack_kuryr_service_pool_start, openshift_openstack_kuryr_service_pool_end].
#
