...
# The value of openshift_portal_net in OSEv3.yml must be within the range given
# for openshift_openstack_kuryr_service_subnet_cidr (usually half of it),
# ensuring it does not overlap with the range provided by
# [openshift_openstack_kuryr_service_pool_start, openshift_openstack_kuryr_service_pool_end].
# openshift_portal_net is the range used by services and Octavia Load Balancers
# VIPs, while amphora VMs get Neutron ports on the range specified by
# [openshift_openstack_kuryr_service_pool_start, openshift_openstack_kuryr_service_pool_end]
# (service VRRP ports):
# [OSEv3.yml]
