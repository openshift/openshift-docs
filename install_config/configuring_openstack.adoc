...
the following subsections we will configure Ansible and modify certain yaml files
...
== Registering with Subscription Manager the {rhocp} Instances
...
required packages for a successful {rhocp} installation. For simplicity,
...
playbooks and roles provided by the OpenShift RPM packages. Review the <<osev3>>
that was previous configured to ensure all the options have been properly set.

include::install_config/topics/ocp_osp_node_install.adoc[leveloffset=+2]
