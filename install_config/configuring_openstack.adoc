...
[[configuring-opestack-permissions]]
== Permissions
Configuring OpenStack for {product-title} requires following permissions:

* *`_member_`* - For creating assets, you need the member role on the project.
* *`heat_stack_owner`* _(optional)_ - If you are using heat templates the `heat_stack_owner` role is required.
