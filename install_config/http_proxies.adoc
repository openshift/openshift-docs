...
The proxy configuration must be the same on each host in the cluster. So when
setting up the proxy or modifying it the files on each {product-title} host
must be updated to the same values and then {product-title} services on each
host in the cluster must be restarted.
...
`*NO_PROXY*` accepts a comma-separated list of hosts, IP addreses, or IP ranges in CIDR format:
...
. Edit the proxy environment variables in the {product-title} control files. Make sure all
...
available in {product-title} by default do not include settings for HTTP
...
set up in your {product-title} instance, the changes happen automatically. Otherwise,
