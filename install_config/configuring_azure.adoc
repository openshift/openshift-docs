...
Azure disk as persistent storage] for application data. After Microsoft Azure is
configured properly, some additional configurations need to be completed on the
{product-title} hosts.

[[configuring-azure-permissions]]
== Permissions
Configuring Microsoft Azure for {product-title} requires the following role:
...
|===
For more information about adding administrator roles, see link:https://docs.microsoft.com/en-us/azure/billing/billing-add-change-azure-subscription-administrator[Add or change Azure subscription administrators].

[[configuring-azure-prerequisites]]
== Prerequisites

* If you are using Microsoft Azure Disk as a persistent volume on the {product-title} version 3.5 or later, you must enable Azure Cloud Provider.
* All {product-title} node virtual machines (VMs) running in Microsoft Azure must belong to a single resource group.
* Microsoft Azure VMs must be named the same as {product-title} nodes and this
cannot include capital letters.
* If you plan to use  Azure Managed Disks:
  ** {product-title} version 3.7 or later is required.
  ** You must create VMs with Azure Managed Disks.
* If you plan to use unmanaged disks:
  ** You must create VMs with unmanaged disks.
* If you are using a custom DNS configuration for your {product-title} cluster or
your cluster nodes are in different Microsoft Azure Virtual Networks (VNet), you
must configure DNS so that each node in the cluster can resolve IP addresses for
other nodes.
