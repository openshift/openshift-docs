...
Alternatively you can use a xref:../../dev_guide/configmaps.adoc#[configMap].
...
A xref:../../dev_guide/configmaps.adoc#[configMap] permits you to customize
the router instance without rebuilding the router image.
You can modify the *_haproxy-config.template_*, *_reload-haproxy_*, and
other scripts, as well as create and modify router environment variables.

The following example shows how to use
a custom *_haproxy-config.template_* file.

. Get a copy of the *_haproxy-config.template_* that you want to modify as
xref:deploying-customized-router[described above].
...
or alternatively by editing the router dc.
+
Using `oc` commands::
+
[source,bash]
