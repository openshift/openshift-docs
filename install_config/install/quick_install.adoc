...
variant_version: 3.1 <2>
ansible_ssh_user: root <3>
ansible_log_path: /tmp/ansible.log <4>
hosts: <5>
- ip: 10.0.0.1 <6>
  hostname: master-private.example.com <6>
  public_ip: 24.222.0.1 <7>
  public_hostname: master.example.com <7>
  master: true <8>
  node: true <8>
  containerized: true <9>
- ip: 10.0.0.2
...
- ip: 10.0.0.3
  ...
  node: true
----
<1> The OpenShift variant to install. For OpenShift Enterprise, set this to
`openshift-enterprise`.
<2> A valid version your selected variant. If not specified, this defaults to
the newest version for the specified variant. For example: `3.1` or `3.0`.
<3> Defines which user Ansible uses to SSH in to remote systems for gathering
...
<5> Defines a list of the hosts onto which you want to install the OpenShift
master and node components.
<6> Required. Allows the installer to connect to the system and gather facts
before proceeding with the install.
<7> Required for unattended installations. If these details are not
...
<8> Determines the type of services that are installed. At least one of these
must be set to *true* for the configuration file to be considered valid.
<9> If set to *true*, OpenShift services are run in a container on this host
