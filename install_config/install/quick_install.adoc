...
install OpenShift across a set of hosts. The installation utility can deploy
OpenShift components on targeted hosts by installing RPMs.
...
usage on a Red Hat Enterprise Linux (RHEL) 7 system.
...
- xref:adding-nodes-or-reinstalling[add nodes] to an existing cluster,
- xref:../../install_config/upgrades.adoc#install-config-upgrades[upgrade your cluster], or
- xref:adding-nodes-or-reinstalling[reinstall] the OpenShift cluster completely.
endif::[]
...
SDN]. The node component on the masters, however, are marked unschedulable, which
...
xref:../../install_config/install/prerequisites.adoc#install-config-install-prerequisites[Prerequisites] topic, you
can continue to running an
...
xref:../../install_config/upgrades.adoc#install-config-upgrades[upgrade your cluster]. Then, see
xref:quick-install-whats-next[What's Next] for the next steps on configuring
...
cluster. When running an xref:running-an-interactive-installation[interactive
...
<10> The IP address that Ansible attempts to connect to when installing,
...
through all of the xref:running-an-interactive-installation[interactive
...
xref:running-an-interactive-installation[interactive] or
xref:running-an-unattended-installation[unattended] installation, you can
