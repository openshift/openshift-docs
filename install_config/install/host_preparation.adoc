...
A base installation of 7.3 or later (with the latest packages from the Extras
...
Edit the `json-file` log driver to use the options you require. When supplying additional options, ensure that you
maintain the single quotation mark formatting.

The following example indicates that Docker rotates between three 1 MB log files, 
always keeps the last three log files, and does not require signature verification:
