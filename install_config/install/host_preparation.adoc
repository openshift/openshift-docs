...
channel) or RHEL Atomic Host 7.3.6 or later is required for master and node
...
. Enable only the repositories required by {product-title} 3.6:
+
----
...
xref:../../install_config/install/advanced_install.adoc#cluster-variables-table[`openshift_portal_net`] parameter in your inventory file.
