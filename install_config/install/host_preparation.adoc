...
For example, to set the maximum file size to 1MB and always keep the last three log files, edit the
`/etc/sysconfig/docker` file to configure `max-size=1M` and `max-file=3`, ensuring that the values maintain the single quotation mark formatting:
