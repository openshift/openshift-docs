...
link:https://bugzilla.redhat.com/show_bug.cgi?id=1417723[(*BZ#1417723*)] with
the *atomic-openshift-docker-excluder* script in the {product-title} 3.3 channel
where the `docker*1.12*` does not get added to the host's *_/etc/yum.conf_*
file. This can cause Docker 1.12 to be installed on {product-title} 3.3 hosts,
which is not supported.

A fix will be provided in an upcoming release, but as a workaround, manually add
`docker*1.12*` in the list on each host:

----
exclude=docker*1.12* [...]
----

This ensures that hosts will not accidentally upgrade past Docker 1.10.
====
