...
{product-title} is capable of cryptographically verifying images are from trusted
sources. The xref:../../security/deployment.adoc#security-deployment-from-where-images-deployed[security guide]
provides a high-level description of how image signing works.

Configuration is performed by the `atomic` command line interface (CLI), version 1.12.5 or greater.
ifdef::openshift-enterprise[]
...
ifdef::openshift-origin[]
Install package **atomic** if it is not installed on the host system.
endif::[]
...
namespace, blacklist (reject) untrusted registries and and require signature
...
The following files and directories comprise the trust
...
link:https://access.redhat.com/articles/2750891#automating-cluster-configuration[knowledge base article]
on the Red Hat Portal for an example of automating file distribution with
Ansible.
