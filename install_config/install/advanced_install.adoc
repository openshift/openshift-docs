...
when using an interface that is not configured with the default route. This
...
[NOTE]
====
The firewalld default configuration cannot be overridden. This means that more
advanced options, such as using multi-zones, should not be configured.
====
...
namespace(s) for loading templates and image streams into the service catalog. You
can also set the nodeselector.
...
[NOTE]
====
When specifying multiple *etcd* hosts, stand-alone *etcd* (non-embedded) is
installed and configured. Clustering of {product-title}'s embedded *etcd* is not
supported. Stand-alone *etcd* can also be collocated on master hosts, if
desired.
====
