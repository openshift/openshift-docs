...
by the `fact_caching_connection` value in the *_/etc/ansible/ansible.cfg_* file. 
====
...
. Verify that the master is started and nodes are registered and reporting in
*Ready* status. *On the master host*, run the following as root:
+
----
# oc get nodes
