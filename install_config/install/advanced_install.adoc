...
To configure the log file, edit the `/etc/sysconfig/docker` file. For example, to set the maximum file size to 1MB and always
keep the last three log files, append `max-size=1M` and `max-file=3` to the `OPTIONS=` line, ensuring that the values
maintain the single quotation mark formatting:

----
OPTIONS='--log-driver json-file --insecure-registry=172.30.0.0/16 --selinux-enabled --log-opt max-size=1M --log-opt max-file=3'
----
Do not use when
...
If the registry required to access the registry is other than `80` include the port number required in the form of
`<address>:<port>`

|`openshift_cockpit_deployer_prefix`
|Specify the URL and path to namespace where the registry-console image is located.
ifdef::openshift-origin[]
Note that the value defaults to `kubernetes`, so the value would be `cockpit/kubernetes:latest`. This differs from several other images that have the `origin-` as the name.
endif::[]
ifdef::openshift-enterprise[]
Note that the value for this must end in `/openshift3` rather than `ose-`, which is the standard for other images.
endif::[]
...
|A setting for if you are using CRI-O and if you are using an alternative CRI-O
system container image from another registry.
|===
...
ifdef::openshift-origin[]
openshift_cockpit_deployer_prefix='registry.example.com/cockpit/'
openshift_web_console_prefix='registry.example.com/openshift/origin-
openshift_service_catalog_image_prefix='registry.example.com/openshift/origin-'
ansible_service_broker_image_prefix='registry.example.com/openshift/origin-'
template_service_broker_prefix="registry.example.com/openshift/origin-"
endif::[]
ifdef::openshift-enterprise[]
...
you can
xref:installer-known-issues[retry the installation].
