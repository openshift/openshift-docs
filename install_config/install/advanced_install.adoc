...
The vSphere Cloud Provider must be configured with a datastore accessible by the OCP nodes.
...
[IMPORTANT]
====
When using vSphere volume for the registry the storage access mode is ReadWriteOnce, the replica count must be set to 1.
====
