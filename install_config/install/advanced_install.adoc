...
must be defined. By default, the whitelist is empty so that a user cannot add
APB images to the broker without a cluster administrator configuring the broker.

To whitelist all images that end in `-apb`:
...
either a single or multiple external *etcd* hosts.
