...
master and node hosts during the Ansible install by configuring the *_/etc/ansible/hosts_*
...
To assign labels to a host during an Ansible install, use the
`openshift_node_labels` variable with the desired labels added to the desired
host entry in the `[master]` or `[nodes]` section. In the following example,
labels are set for a region called `primary` and a zone called `east`:
...
[masters]
master.example.com openshift_node_labels="{'region': 'primary', 'zone': 'east'}"
...
# host group for masters
[masters]
master1.example.com openshift_node_labels="{'region': 'primary', 'zone': 'east'}"
master2.example.com openshift_node_labels="{'region': 'primary', 'zone': 'east'}"
master3.example.com openshift_node_labels="{'region': 'primary', 'zone': 'east'}"
...
at least 430MiB of memory for each unique host that you have defined in your
inventory before installing or upgrading to 3.9. This will be fixed in a future
release. link:https://bugzilla.redhat.com/show_bug.cgi?id=1558672[(*BZ#1558672*)]
