...
link:../../install_config/certificate_customization.html[Custom serving certificates]
for the public hostnames of the OpenShift API and web console may be deployed during
installation and are configurable in the inventory file.

[NOTE]
====
Custom certificates should only be configured for the hostname associated with the
`publicMasterURL` which can be set using `*openshift_master_cluster_public_hostname*`.
Using a custom serving certificate for the hostname associated with the `masterURL`
(*`openshift_master_cluster_hostname`*) will result in TLS errors as infrastructure
components will attempt to contact the master API using the internal `masterURL` host.
====

Certificate and key file paths can be configured with `*openshift_master_named_certificates*`.
File paths must be local to the system where ansible will be ran. Certificates are copied to
master hosts and are deployed within the *_/etc/origin/master/named_certificates/_* directory.
...
Ansible will detect a certificate's `Common Name` and `Subject Alternative Names`. Detected
names may be overridden by providing the `*"names"*` key when setting
`*openshift_master_named_certificates*`.
...
Certificates configured using `*openshift_master_named_certificates*` will be cached on masters,
meaning that each additional ansible run with a different set of certificates will result in all
previosuly deployed certificates remaining in place on master hosts and within the master
configuration file.

If you would like `*openshift_master_named_certificates*` to be overwritten with the provided
value (or no value), specify `*openshift_master_overwrite_named_certificates*`.
