...
ifdef::openshift-enterprise[]
- Base OS: Red Hat Enterprise Linux (RHEL) 7.2  with "Minimal" installation
option
endif::[]
- 2 vCPU
- Minimum 8 GB RAM
...
ifdef::openshift-enterprise[]
Currently link:../../install_config/persistent_storage/persistent_storage_nfs.html[NFS is fully
supported], however other options are available as
link:../../release_notes/ose.3.0/ose_3.0.1_release_notes.html[Technology
Preview].
endif::[]
ifdef::openshift-origin[]
The Administrator Guide provides instructions on provisioning an OpenShift
cluster with link:../../install_config/persistent_storage/persistent_storage_nfs.html[persistent
storage using NFS].
endif::[]
...
A base installation of Red Hat Enterprise Linux (RHEL) 7.2 is required for
