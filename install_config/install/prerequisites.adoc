...
firewall rules on each host using `firewalld`. However, if your network
...
* Port *1936* can still be inaccessible due to your firewalld rules. Run the following command to configure firewalld to open port *1936*:
+
----
# firewall-cmd --permanent --add-port=1936/tcp --zone=OS_FIREWALL_ALLOW
----
