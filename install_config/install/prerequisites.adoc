...
[IMPORTANT]
====
Without SELinux enabled, the installer will fail and there is an increased risk
of a container accessing the contents of another, ensuring you will not have a
true multi-tenant system.
====

Also, configure `*SELINUXTYPE=targeted*` in the *_/etc/selinux/config_* file:
...
[IMPORTANT]
====
If your Red Hat Enterprise Linux environment is using OverlayFS, your
environment will need manual configurations before it is ready to use with
{product-title}.

See the
link:https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/7.2_Release_Notes/technology-preview-file_systems.html[Red
Hat Enterprise Linux 7.2 documentation] for the requirements to enable OverlayFS
support with {product-title}.
====
