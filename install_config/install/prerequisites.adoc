...
enough resources so that a node host failure does not affect your maximum
capacity.
...
context constraints] that control the actions that pods can perform and what
they can access.
...
change) only apply to new installations of {product-title}
ifdef::openshift-enterprise[]
3.2.
endif::[]
ifdef::openshift-origin[]
...
Clusters upgraded to {product-title}
ifdef::openshift-enterprise[]
3.2
endif::[]
ifdef::openshift-origin[]
