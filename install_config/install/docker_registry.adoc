...
a service account named *registry*, but it can be named anything:
+
----
$ echo \
...
If you wish to expose your internal registry externally, it is recommended that
you run a link:#securing-the-registry[secure registry].  To expose the registry,
you must first have link:deploy_router.html[deployed a router].
...
[NOTE]
====
In this example, `test` in the URL `<host>/test/busybox` refers to the project
name.
====
