...
By default the registry is created with no settings for compute resource requests or limits.
For production it is highly recommended that the deployment config be updated to set
resource requests and limits for the registry pod.  Otherwise, the registry pod will
be considered a link:../../dev_guide/compute_resources.html#quality-of-service-tiers[best effort pod].

For more information on configuring requests and limits please see
the link:../../dev_guide/compute_resources.html[Compute Resources Guide].
...
link:../../admin_guide/quota.html#create-a-quota[Create a quota object] on a
...
This will happen if the pulled image is pushed to an image stream different
from the one it's being pulled from. It's a result of
link:../../architecture/core_concepts/builds_and_image_streams.html#adding-tag[re-tagging]
of a built image into arbitrary image stream like this:
...
And subsequently pulling from it, using image reference such as:
...
1. avoid tagging of internally managed images completely
2. or re-push the built image to the desired namespace
