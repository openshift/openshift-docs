...
xref:../../admin_guide/persistent_storage_nfs.adoc#admin-guide-persistent-storage-nfs[define and use persistent
storage using NFS].
...
xref:prerequisites.adoc#security-warning[privileged containers] by default. If
...
. Ensure you are logged in to OpenShift as a xref:../../architecture/core_concepts/projects_and_users.adoc#users[regular user]:
+
----
$ oc login
----
