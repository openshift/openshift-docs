...
. link:#deploy-registry[Deploy the registry].
. Edit the configuration file locally as needed. See details on
https://github.com/docker/distribution/blob/master/docs/configuration.md#override-configuration-options[overriding registry options]. The initial file may be found in the link:https://github.com/openshift/origin/blob/master/images/dockerregistry/config.yml[Docker image registry source repository].
. Create a secret from the local configuration file edited in the previous step:
+
----
$ oc secrets new registry-config config.yml=/path/to/custom/registry/config.yml
----
+
. Add secret volume *_registry-config_* to the registry deployment configuration to mount the configuration file at *_/etc/docker/registry/_*:
+
----
$ oc volume dc/docker-registry --add --type=secret \
...
----
+
. Update the registry to reference the configuration path from the previous step by adding the following environment variable to the registry deployment configuration:
+
----
$ oc env dc/docker-registry \
    REGISTRY_CONFIGURATION_PATH=/etc/docker/registry/config.yml
----
. Re-deploy the registry:
+
----
$ oc deploy docker-registry --latest
----

This may be performed as an iterative process to achieve the desired configuration. For example, during troubleshooting the configuration may be temporarily updated to put it in *_debug_* mode.

To update an existing configuration:

[WARNING]
====
This procedure will overwrite the deployed registry configuration.
====

. Edit the local registry configuration file, *_config.yml_*.
. Delete the *_registry-config_* secret:
+
----
$ oc delete secret registry-config
----
+
. Re-create the secret to reference the updated configuration file from step 1:
+
----
$ oc secrets new registry-config config.yml=/path/to/custom/registry/config.yml
----
