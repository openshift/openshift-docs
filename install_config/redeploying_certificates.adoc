...
{product-title} automatically rotates node certificates when they get close to expiring. If you need to redeploy certificates because the CA certificate was changed, you can use the *_playbooks/redeploy-certificates.yml_* playbook with the `-e openshift_redeploy_openshift_ca=true` flag. See xref:redeploying-all-certificates-current-ca[Redeploying All Certificates Using the Current OpenShift Container Platform and etcd CA] for details.
