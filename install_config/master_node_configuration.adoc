...
[[master-node-config-kubernetes-infrastructure]]
== Configuring Kubernetes Infrastructure

Master API pods are defined as static pods by the xref:../architecture/infrastructure-components/kubernetes_infrastructure#ensuring-host-access[Kubernetes Infrastructure]. In order to make changes to the `api`, `conrollers`, or `etcd` pods, the static pod definitions will need to be changed. These are located in `/etc/origin/node/pods/` on each of the _master hosts_.

[CAUTION]
====
Kubernetes will detect changes in ALL files in this directory and create static pods from them. When working with these files, designate all backups with a leading ".", so they are skipped by Kubernetes.
====

=== Configuring Enterprise Certificate Authority Trust
By default, the _master API_ will not trust and Certificate Authorities that are set on the _master host_ operating systems. This can be corrected by exposing the `/etc/pki/` directory through to the _api_ pod.

. Open the */etc/origin/node/pods/apiserver.yaml*

. Add the following section to the *volumeMounts* section:
+
----
- mountPath: /etc/pki
  name: pki-data
...
. Add the following section to the *volumes* section:
----
- name: pki-data
  hostPath:
    path: /etc/pki
