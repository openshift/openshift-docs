...
----
+
For HTPasswd authentication the `openshift_master_identity_providers` variable will enable the authentication type.
There are three choices to configure this authentication type.

* Only `openshift_master_identity_providers` assumes `/etc/origin/master/htpasswd` is already configured and present on the host
* Both `openshift_master_identity_providers` and `openshift_master_htpasswd_file` to copy a local htpasswd file to the host
* Both `openshift_master_identity_providers` and `openshift_master_htpasswd_users` to generate a new htpasswd file on the host
