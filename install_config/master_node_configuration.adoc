...
To setup the advanced audit feature please add the following entries at the `openshift_master_audit_config` variable

----
openshift_master_audit_config={"enabled": true, "auditFilePath": "/var/log/oscp-audit/-oscp-audit.log", "maximumFileRetentionDays": 14, "maximumFileSizeMegabytes": 500, "maximumRetainedFiles": 5, "policyFile": "/etc/security/adv-audit.yaml", "policyConfiguration": "true"}
----
