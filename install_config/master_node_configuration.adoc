...
Advanced Audit feature provides several improvements over the basic audit
functionality described in xref:master-node-config-audit-config[here]. These
include: fine-grained events filtering, multiple output backends.

The below table contains the additional options you can tweak the advanced
audit feature. See xref:master-node-config-audit-config[basic audit] for the description of those.
...
|`logFormat`
|Specifies format of the saved audit logs.  Allowed values are `legacy` (the
format used in basic audit), and `json`.

|`webHookKubeConfig`
|Path to a `.kubeconfig` formatted file that defines the audit webhook configuration, where the events will be send to.

|`webHookMode`
|Specifies the strategy for sending audit events.  Allowed values are `block`
(blocks processing another event until previous has fully processed), `batch` (buffer events and deliver in batches).
|===

To enable the advanced audit feature you need to provide either `policyFile` or
`policyConfiguration` describing the audit policy rules.  Here is a sample
...
----
<1> There are 4 possible levels every event can be logged at.  `None` - don't
log events that match this rule.  `Metadata` - log request metadata
(requesting user, timestamp, resource, verb, etc.) but not request or response body (this is the same level as the one use in basic audit.  `Request` - log
event metadata and request body but not response body.  `RequestResponse` - log event metadata, request and response bodies.
<2> A list of users the rule applies to.  Empty list implies every user.
<3> A list of verbs this rule applies to.  Empty list implies every verb.  This is kube verb associated with API requests (this includes get, list, watch, create, update, patch, delete, deletecollection, and proxy),
<4> A list of resources the rule applies to.  Empty list implies every resource.  Each resource is specified as a group it is assigned to (eg. empty for kubernetes core API, batch, build.openshift.io, etc.), and a resource list from that group.
<5> A list of groups the rule applies to.  Empty list implies every group.
<6> A list of non-resources URLs the rule applies to.
<7> A list of namespaces the rule applies to. Empty list implies every namespace.
====

The link:https://kubernetes.io/docs/tasks/debug-application-cluster/audit[Kubernetes documentation]
has more information on advanced audit.
