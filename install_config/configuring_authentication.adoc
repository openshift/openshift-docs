...
http://doc.gitlab.com/ce/integration/oauth_provider.adoc[OAuth integration].
...
<2> When *true*, unauthenticated token requests from non-web clients (like
the CLI) are sent a `WWW-Authenticate` challenge header for this provider.
This uses the http://doc.gitlab.com/ce/api/oauth2.adoc#resource-owner-password-credentials[Resource Owner Password Credentials]
...
link:http://openid.net/specs/openid-connect-core-1_0.adoc#CodeFlowAuth[Authorization
Code Flow].
...
link:http://openid.net/specs/openid-connect-core-1_0.adoc#StandardClaims[standard
...
identity.
<8> link:http://openid.net/specs/openid-connect-core-1_0.adoc#AuthorizationEndpoint[Authorization Endpoint]
described in the OpenID spec. Must use `https`.
<9> link:http://openid.net/specs/openid-connect-core-1_0.adoc#TokenEndpoint[Token Endpoint]
described in the OpenID spec. Must use `https`.
====
...
<8> link:http://openid.net/specs/openid-connect-core-1_0.adoc#UserInfo[UserInfo
