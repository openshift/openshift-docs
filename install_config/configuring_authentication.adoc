...
header in the new `*userAgentMatching*` configuration setting.

For example, you can deny clients using particular libraries or binaries. This
example denies the Kubernetes 1.2 client binary, OpenShift Origin 1.1.3
binary, and the POST and PUT *httpVerbs*:
...
Administrators can also deny clients that do not exactly match the expect
clients:
