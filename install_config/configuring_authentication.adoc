...
<10> Optional: When *true*, the provider will use Keystone IDs instead of user
names for newly created OpenShift users. Read more about this option in a
xref:use-keystone-identity[special section].
...
[[use-keystone-identity]]
==== useKeystoneIdentity option

By default all new users are created with the name equal to the user name in
Keystone. That could lead to situations when, after removing a user
account in Keystone, a new user with the same name can see all the resources
available to the previous user.

To prevent such situations a new boolean option `useKeystoneIdentity` was
added to the provider.

When enabled, all new OpenShift users will be created with names equal to
their unique Keystone IDs, instead of Keystone user names. So, if the
identifier of the new user does not match what is stored in the identity,
then the user's request to login to the system is rejected, even if
he entered the correct password.
