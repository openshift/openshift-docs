...
=== Enabling Features in Tech Preview

Sometimes features will be available in tech preview, by default these features may be hidden
from end users.  

The current list of features in tech preview is:

* pipelines - Enabling this will add the Pipelines navigation item underneath the Builds menu
...
Save this script to a file (for example, *_tech-preview.js_*) and add it to the
master configuration file:
