...
. As a cluster administrator, run the following command to give users the
permission to use the feature in their build configuration:
+
----
$ oadm policy add-cluster-role-to-group system:build-strategy-<feature_name>:authenticated
----
+
For example, to give users the ability to use Jenkins pipelines with
{product-title}:
+
====
----
...
. Test the functionality. If testing the use of Jenkins pipelines, refer to link:https://github.com/openshift/origin/tree/master/examples/jenkins/pipeline[this
example].
