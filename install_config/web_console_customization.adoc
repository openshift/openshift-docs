...
=== Customizing the About Page
To provide a custom About page for the console, write
an extension that looks like:
...
Save this script to a file (for example, *_about/about.js_*). Then write a customized template.
Start from the version of https://github.com/openshift/origin-web-console/blob/master/app/views/about.html[about.html]
from the OpenShift Container Platform https://github.com/openshift/origin-web-console/branches[release] you are using.
Within the template there will be two angular scope variables available, these are `version.master.openshift` and
`version.master.kubernetes`.  Save the custom template to a file (for example, *_about/about.html_*). Then modify
