...
ifdef::openshift-enterprise[]
`openshift3/openshift-metrics-cassandra:3.5.0`, set prefix `openshift/openshift-`.
endif::[]
...
ifdef::openshift-enterprise[]
`openshift3/openshift-metrics-cassandra:3.5.0`, set version as `3.5.0`.
endif::[]
...
Hawkular Alerts must be connected to the Aggregated Logging's Elasticsearch to react on log events. By default,
Hawkular will try to find Elasticsearch on its default place (namespace `logging`, pod `logging-es`) at every boot.
If the Aggregated Logging is installed after Hawkular, the Hawkular Metrics pod might need to be restarted in order
to recognize the new Elasticsearch server. The Hawkular boot log provides a clear indication if the integration could
not be properly configured, with messages like:
