...
[NOTE]
====
Heapster was deprecated in {product-title} 3.11 and will be removed in
{product-title} 4.0. Its functionality is now provided by
xref:prometheus_cluster_monitoring.adoc#prometheus-cluster-monitoring[Prometheus
Cluster Monitoring].
====
...
[NOTE]
====
Hawkular was deprecated in {product-title} 3.11 and will be removed in
{product-title} 4.0. Use
xref:prometheus_cluster_monitoring.adoc#prometheus-cluster-monitoring[Prometheus
Cluster Monitoring] instead.
====
...
== Prometheus on {product-title} (technology preview)
...
[NOTE]
====
Prometheus is a technology preview. In this form, it will be removed in
{product-title} 4.0. Its functionality is now provided by
xref:prometheus_cluster_monitoring.adoc#prometheus-cluster-monitoring[Prometheus
Cluster Monitoring].
====
...
Prometheus deployment is enabled by default, uninstall it by setting
`openshift_prometheus_state` to `absent`. For example:
...
|The default value is `present`, which results in the installation or update of
Prometheus. To uninstall Prometheus, set to `absent`.
