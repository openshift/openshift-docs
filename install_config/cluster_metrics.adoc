...
the instructions for node certificates outlined in xref:upgrading/manual_upgrades.adoc#manual-updating-master-and-node-certificates[Updating
...
certificate *must* contain the hostname used by the route (eg HAWKULAR_METRICS_HOSTNAME). The default
router's certificate will be used for the route if this option is unspecified.
