...
[Note]
====
Support for Hawkular Metrics is not included as part of the link:https://www.redhat.com/en/technologies/cloud-computing/openshift/migrating[OpenShift 3 to OpenShift 4 migration offerings].
====

The xref:../architecture/infrastructure_components/kubernetes_infrastructure.adoc#kubelet[kubelet] exposes metrics that can be collected and stored in back-ends by link:https://github.com/kubernetes/heapster[Heapster].
...
The following describes the various areas and the parameters that can be added to
...
|Specify the storage class to use. If you want to explicitly set the storage class,
also set `openshift_metrics_cassandra_storage_type=pv`.
...
persistent volumes. If you want to explicitly set the storage class,
specify `pv` and set a `openshift_metrics_cassandra_pvc_storage_class_name`.
...
`{"node-role.kubernetes.io/infra":"true"}`. If not specified, the Cassandra pod is deployed on any schedulable node.
...
xref:../admin_guide/scheduling/node_selector.adoc#admin-guide-sched-selector[node selector] to ensure that
pods are placed onto nodes with specific labels. For example,
`{"node-role.kubernetes.io/infra":"true"}`.  If not specified, the Hawkular pod is deployed on any schedulable node.
...
xref:../admin_guide/scheduling/node_selector.adoc#admin-guide-sched-selector[node selector] to ensure that
pods are placed onto nodes with specific labels. For example,
`{"node-role.kubernetes.io/infra":"true"}`.  If not specified, the Heapster pod is deployed on any schedulable node.
