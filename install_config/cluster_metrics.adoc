...
For example, access to the master server is configured by the `*MASTER_URL*`
parameter. The default value is `https://kubernetes.default.svc:443`, however
this likely needs to be overridden and configured for your environment. Failing
to set this parameter properly may result in timeout errors in the deployer
logs. You may also want to override the value for `*IMAGE_PREFIX*` to suit your
deployment.

However, the only required parameter is `*HAWKULAR_METRICS_HOSTNAME*`. This
value is required when creating the deployer because it specifies the hostname
for the Hawkular Metrics
link:../architecture/core_concepts/routes.html[route]. You will need to know the
