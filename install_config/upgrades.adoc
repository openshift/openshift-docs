...
method, you can use the installation utility to perform the automated upgrade.
endif::[]
...
*_/etc/ansible/hosts_*. If your *_hosts_* file is located somewhere else, add
...
advanced installation method, if you have an inventory file. If your *_hosts_*
file is located somewhere other than the default *_/etc/ansible/hosts_*, add the
`-i` flag to specify the location.

For OpenShift Enterprise 3.1, the only upgrade playbook currently available is
for upgrading from OpenShift Enterprise 3.0:
...
If you are upgrading from OpenShift Enterprise 3.0, you must also create an
*etcd* backup on each master:
...
*Create and Add Master Proxy Client Certificates*

Create *_/etc/origin/master/master.proxy-client.{crt,key}_* certificates:
...
Add the master proxy client certificates to the
...
Enable the following renamed service on node hosts:
