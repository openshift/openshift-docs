...
method and a *_~/.config/installer.cfg.yml_* file is available, you can use the
...
Enterprise 3.0 to 3.1. See link:#upgrading-using-ansible-directly[Using Ansible
Directly] for applying asynchronous releases.
====
...
Then, run the following upgrade playbook. If your inventory file is located
...
cluster, run the following upgrade playbook. If your inventory file is located
somewhere other than the default *_/etc/ansible/hosts_*, add the `-i` flag to
specify the location.
...
==== OpenShift Enterprise 3.1.0 and 3.1.1
