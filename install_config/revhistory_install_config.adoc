...
|Added a xref:../install_config/install/docker_registry.adoc#known-issue-push-failed-due-to-eof[new section] about the `error: build error: Failed to push image: EOF` known error.

|xref:../install_config/registry/index.adoc#install-config-registry-overview[Deploying a Docker Registry]
|Extended the registry configuration file example within the xref:../install_config/install/docker_registry.adoc#docker-registry-deploying-updated-configuration[Deploying Updated Configuration] section to include the `*blobrepositorycachettl*` option.
