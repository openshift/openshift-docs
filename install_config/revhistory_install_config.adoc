...
// tag::install_config_mon_sep_25_2017[]
...
|Added information about `fsType`.

|xref:../install_config/aggregate_logging.adoc#install-config-aggregate-logging[Aggregating Container Logs]
|Added `openshift_logging_elasticsearch_kibana_index_mode` parameter to the
...
|Added a new xref:../install_config/cluster_metrics.adoc#metrics-logging[Integration with Aggregated Logging] section

|xref:../install_config/persistent_storage/dynamically_provisioning_pvs.adoc#install-config-persistent-storage-dynamically-provisioning-pvs[Configuring Persistent Storage -> Dynamic Provisioning and Creating Storage Classes]
|Added details of how administrators can configure zones in storage classes.



|===

// end::install_config_mon_sep_25_2017[]
== Fri Sep 22 2017
...
|Added xref:../install_config/install/advanced_install.adoc#enabling-service-catalog[Enabling the Service Catalog], Configuring the Ansible Service Broker, xref:../install_config/install/advanced_install.adoc#configuring-template-service-broker[Configuring the Template Service Broker], and Deploying the Template Service Broker sections.
