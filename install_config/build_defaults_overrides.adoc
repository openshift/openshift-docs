...
on clusters where the `BuildDefaults` or `BuildOverrides` nodeselector is not set. 
...
which is set by the `BuildDefaults` or `BuildOverrides` admission plug-ins. These settings mean that the build will be scheduled only 
...
[NOTE]
====
You must use a `BuildOverrides` node selector in order to override tolerations 
using the `BuildOverrides` plug-in.
====
