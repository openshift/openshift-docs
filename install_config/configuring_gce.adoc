...
== Permissions
Configuring GCE for {product-title} requires following role:

* *roles/owner* - To create service accounts, cloud storage, instances, images, templates, Cloud
...
[NOTE]
====
The account running the deployment needs to have `owner` role for the project,
because the deployment creates the Service Account, which is later used by
Ansible to create resources in the GCE. It is also possible to create the
Service Account beforehand, therefore the account running the deployment only
