...
You can do this with a long complex command using the values from the
...
. Update the value of the `*peerURLs*` using the `etcdctl member update` command
by passing the member ID obtained from the previous step:
+
====
----
# etcdctl --cert-file=/etc/etcd/peer.crt \
