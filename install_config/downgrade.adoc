...
link:#downgrade-bringing-openshift-services-back-online[Bringing {product-title}
Services Back Online].
...
On each {product-title} master, restore your *openshift-master* configuration
from backup and restart relevant services:
...
On each {product-title} node, enable and restart the *openshift-node* service:
