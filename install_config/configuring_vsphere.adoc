...
You can configure {product-title} to use link:https://www.vmware.com/au/products/vsphere.html[VMware vSphere] VMDKs as backing for PersistentVolumes. This includes
...
The vSphere Cloud Provider allows using vSphere managed storage within {product-title} and supports every storage primitive exposed by Kubernetes:
...
The latest updates to the vSphere Cloud Provider are located link:https://vmware.github.io/vsphere-storage-for-kubernetes/documentation/index.html[here].
...
{product-title} provisions new volumes as *independent persistent disks* to freely attach and detach the volume on any node in the cluster. As a consequence, link:https://docs.vmware.com/en/VMware-vSphere/6.7/com.vmware.vsphere.vm_admin.doc/GUID-53F65726-A23B-4CF0-A7D5-48E584B88613.html[it is not possible to back up volumes that use snapshots].
