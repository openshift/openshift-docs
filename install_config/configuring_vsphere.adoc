...
toc::[]
You can configure {product-title} to access an existing 19 link:https://www.vmware.com/au/products/vsphere.html[VMware vSphere] infrastucture, including xref:../install_config/persistent_storage/persistent_storage_vsphere.adoc#install-config-persistent-storage-persistent-storage-vsphere[using VMware vSphere VMDK Volumes as persistent storage] for application data.

== Before you begin
Deploying virtual machines (VMs)
Configuring authorization for {product-title}
