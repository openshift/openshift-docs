...
xref:../../architecture/additional_concepts/storage.adoc#persistent-volumes[persistent volumes] (Persistent Volumes, PVs), by defining labels in the PV and matching selectors in the PVC.
This feature is available for all xref:../persistent_storage/index.adoc#[storage
options]. It is assumed that a {product-title} cluster contains persistent storage
resources which are available for binding by PVCs.
