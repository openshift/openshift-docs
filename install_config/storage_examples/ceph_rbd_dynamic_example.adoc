...
an {product-title} dynamic persistent store. It is assumed that a working Ceph cluster
...
It is recommended to create a pool for your dynamic volumes to live.
...
From a Ceph admin or Mon node, create a new pool for dynamic volumes
----
...
...
----
<1> Place the key from ceph-user-secret here in base64 format. See xref:../install_config/storage_examples/ceph_example.adoc#using-ceph-rbd-creating-the-ceph-secret[Creating the Ceph Secret].
====
