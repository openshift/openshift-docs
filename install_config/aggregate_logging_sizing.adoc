...
As of {product-title} 3.7, with 1KB sized log messages, the logging
architecture tests have achieved a stable logging rate of 800-900 messages per
second per node.

Scaling the logging infrastructure to a higher throughput is currently limited
by the current versions and configurations of the OpenShift logging components.
