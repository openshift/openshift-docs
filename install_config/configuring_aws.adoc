...
A cluster uses the value of the `kubernetes.io/cluster/<clusterid>,Value=(owned|shared)` tag to determine which
resources belong to the cluster. Therefore, you must tag all resources with the
key `kubernetes.io/cluster/<clusterid>,Value=(owned|shared)` and have the same value for that key.

. Tag all instances with `kubernetes.io/cluster/<clusterid>,Value=(owned|shared)` and a value to be used as the cluster ID.
. Tag any security groups with `kubernetes.io/cluster/<clusterid>,Value=(owned|shared)` and the same value used for the instances.
. Tag any load balancers with `kubernetes.io/cluster/<clusterid>,Value=(owned|shared)` and the same value used for the instances.
. Tag all EBS volumes with `kubernetes.io/cluster/<clusterid>,Value=(owned|shared)` and the same value used for the instances. The EBS Volumes that need to be tagged can be found with:
+
[source,bash]
----
