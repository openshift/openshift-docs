...
====
----
$ haproxy_exporter_ip="<router-ip-address-or-hostname>"
...
----
prometheus -config.file=haproxy-scraper.yml
----

Or you can start it as a container on {product-title}.

Once the prometheus server is up, view the {product-title} HAProxy router
metrics at: `\http://<prometheus-address>:9090/consoles/haproxy.html`.
