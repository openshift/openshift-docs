...
+
---- 
$ oc secrets new router-certs tls.crt=custom-router.crt tls.key=router.key -o json --type='kubernetes.io/tls' --confirm | oc replace -f -
----
+
To create a new secret, run the following commands:
+
---- 
$ oc secrets new router-certs tls.crt=custom-router.crt tls.key=router.key --type='kubernetes.io/tls' --confirm

$ oc volume dc/router --add --mount-path=/etc/pki/tls/private --secret-name='router-certs' --name router-certs
----

. Deploy latest router. 
+
----
