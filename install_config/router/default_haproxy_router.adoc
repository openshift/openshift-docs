...
xref:../install/quick_install.adoc#install-config-install-quick-install[quick installation], then
a default router was automatically created for you. The `oadm router` command
creates the service and deployment configuration objects.  Use the `--service-account` option
to specify the service account the router will use to contact the master.

The
xref:index.adoc#creating-the-router-service-account[router service account]
can be created in advance or created by the `oadm router --service-account` command.

Just about every form of communication between {product-title} components is secured by TLS and uses
various certificates and authentication methods. The `--default-certificate` .pem format file can 
be supplied or one will be created as a by the `oadm router` command. When
xref:../../architecture/core_concepts/routes.adoc#[routes] are created the user can provide route
certificates that the router will use when handling the route.

When the router is deleted, make sure the deployment config, service, and secret are all deleted. 

Routers will usually be deployed on specific nodes. This makes it easier for the cluster administrator
and external network manager to coordinate which IP address will run a router and which traffic
the router will handle. The routers are deployed on specific nodes by using
xref:adding-nodeselector-to-a-deployment[node selectors].

[IMPORTANT]
====
Routers, by default, use host networking and they directly attach to port 80 and 443 on all
interfaces on a host. Restrict routers to hosts where ports 80/443 are available and not being
consumed by another service, and set this using xref:adding-nodeselector-to-a-deployment[node selectors] and the
...
[IMPORTANT]
====
It is recommended to use separate distinct *openshift-router*
xref:index.adoc#creating-the-router-service-account[service account]
...
--replicas is ususally 1 unless a
xref:../../admin_guide/high_availability.adoc#[high availability] configuration is 
being created.

To find the host IP address of the router:

----
$ oc get po <routeri-pod>  --template={{.status.hostIP}}
----
...
environment variables] after router creation. In this case create a router for each 
shard.
...
ifdef::openshift-origin[]
First, ensure you have xref:/index.adoc#creating-the-router-service-account[created the
...
ifdef::openshift-enterprise[]
----
$ oadm router --dry-run -o yaml --service-account=router
----
endif::[]
ifdef::openshift-origin[]
----
...
Using the
xref:../../architecture/core_concepts/router.adoc#env-variables[`ROUTE_LABELS`]
environment variable, you can filter routes so that
...
Edit the
xref:../../architecture/core_concepts/router.adoc#env-variables[`ROUTER_MAX_CONNECTIONS`]
environment variable in the router's deployment configuration to change the value.
The router pods are restarted with the new value. If `ROUTER_MAX_CONNECTIONS` is not
present, the default value of 20000, is used.
...
router] on your {product-title} cluster using IP failover. This setup has multiple replicas on differrent
nodes so the failover software can scitch to another replica if the current one fails.

[[customizing-the-router-service-ports]]
== Customizing the Router Service Ports
You can customize the service ports that a template router binds to by setting
the environment variables
xref:../../architecture/core_concepts/router.adoc#env-variables[`*ROUTER_SERVICE_HTTP_PORT*`] and
xref:../../architecture/core_concepts/router.adoc#env-variables[`*ROUTER_SERVICE_HTTPS_PORT*`].
...
to serve the same set of xref:routes.adoc#[routes]. Each router will be on a different
xref:../../install_config/router/default_haproxy_router.adoc#adding-nodeselector-to-a-deployment[node]
and will have a different IP address. The network administrator will need to get the 
desired traffic to each node.
...
Each router or shard in the group admits routes based on the selectors in the router.
...
xref:../../architecture/core_concepts/router.adoc#env-variables[`NAMESPACE_LABELS`].
...
The access controls are based on the
xref:index.adoc#creating-the-router-service-account[service account] that the router is run
with.

Using
xref:../../architecture/core_concepts/router.adoc#env-variables[`NAMESPACE_LABELS`] and/or
xref:../../architecture/core_concepts/router.adoc#env-variables[`ROUTE_LABELS`], a router can filter out the
namespaces and/or routes that it should admit. This enables you to partition
...
Router sharding is
xref:../../architecture/core_concepts/routes.adoc#route-labels[based on labels];
...
xref:index.adoc#creating-the-router-service-account[service account]
associated with the router has
xref:index.adoc#router-use-of-labels[`cluster reader`] permission.
...
First, ensure that
xref:index.adoc#creating-the-router-service-account[service account] associated
with the router has
xref:index.adoc#router-use-of-labels[`cluster reader`] permission.
