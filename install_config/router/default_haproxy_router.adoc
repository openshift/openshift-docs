...
[IMPORTANT]
====
To change a route's wildcard policy, you must remove the route and recreate it with the updated wildcard policy. Editing only the route's wildcard policy in route's *_.yaml_* file does not work.
====
