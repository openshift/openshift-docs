...
Use of labels (e.g., to define xref:default_haproxy_router.adoc#using-router-shards[router shards])
...
xref:default_haproxy_router.adoc#install-config-router-default-haproxy[a default HAProxy Router],
xref:customized_haproxy_router.adoc#install-config-router-customized-haproxy[a customized HAProxy Router]
or xref:f5_router.adoc#install-config-router-f5[F5 Router].
endif::[]
