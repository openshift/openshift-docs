...
[[3par]]
=== HPE 3PAR Storage Object Definition
...
parameters: <2>
  ...
  qos_name: "vvset_db"
----
<1> To specify this provisioner, you must first install and configure the HPE Docker Volume Plugin for HPE 3PAR StoreServ for {product-title}. See link: https://github.com/hpe-storage/python-hpedockerplugin/tree/master/quick-start[HPE 3PAR Volume Plug-in for Kubernetes/OpenShift environments].
<2> For a complete list of parameters the dynamic provisioner and the FlexVolume driver support, see link:https://github.com/hpe-storage/python-hpedockerplugin/blob/master/docs/usage.md[Supported StorageClass parameters].

[NOTE]
====
The HPE 3PAR Volume Plug-in for Kubernetes/OpenShift environments relies on a
xref:../../install_config/persistent_storage/persistent_storage_flex_volume.adoc#flex-volume-drivers[FlexVolume driver]
that is included in the HPE Docker Volume Plugin for HPE 3PAR StoreServ, which HPE 3PAR
Storage customers and partners can obtain at link:https://github.com/hpe-storage/python-hpedockerplugin/tree/master/quick-start[HPE 3PAR Docker Volume Plugin].

For a brief overview, see link:https://access.redhat.com/openshift-primed/#/hpe-3par-storage[HPE 3PAR Storage]
on the Primed Partners page.
====
