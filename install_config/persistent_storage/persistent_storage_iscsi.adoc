...
You can configure custom initiator IQN. This is helpful if the iSCSI targets are restricted to certain IQNs but the nodes that the iSCSI PVs are attached 
are not guaranteed to have these IQNs.

To specify custom initiator IQN, use `initiatorName` field. For example:
...
spec:
  ...
  iscsi:
    ...
    readOnly: false
----
<1> Add additional custom initiator IQN, use `initiatorName` field.
====
