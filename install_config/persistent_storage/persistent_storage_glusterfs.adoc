...
As an example, assume that the target gluster volume, "_HadoopVol_", is mounted
...
access to persistent storage. Supplemental groups are covered further in
...
A custom SCC can be created to define minimum and maximum group IDs, enforce
group ID range checking, and allow a group ID of *590*.
