...
Define the authorization key in a secret configuration, which is then converted
to base64 for use by {product-title}.

[NOTE]
====
In order to use Ceph storage to back a persistent volume, the secret must be
created in the same project as the PVC and pod. The secret cannot simply be in
the default project.
====
...
.  Copy the keyring for your client to every {product-title} node.
+
----
# ceph auth get-or-create client.openshift-dynamic mon 'allow r' \
 osd 'allow class-read object_prefix rbd_children, \
 allow rwx pool=openshift-joe' -o ceph.client.dynamic.keyring
----
