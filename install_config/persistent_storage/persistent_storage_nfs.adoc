...
To enable writing in SELinux on each node, run:
...
The NFS export and directory must be set up so that it is accessible
by your pods. Either set the export to be owned by the container's
main UID, or give your pod group based access using
`SuppplementalGroups`. See link:pod_security_context.html[Volume
