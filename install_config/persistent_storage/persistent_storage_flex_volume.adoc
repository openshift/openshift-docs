...
In {product-title} {product-version} since controller-manager runs as a static pod, the
Flexvolume binary that performs attach/detach MUST be a self-contained executable with no external
dependencies.

On Atomic hosts the default location of Flexvolume plugin directory is `/etc/origin/kubelet-plugins`.
Thus the Flexvolume executable must be placed under *_/etc/origin/kubelet-plugins/volume/exec/<vendor>~<driver>/<driver>_* on
all master and nodes in the cluster.
