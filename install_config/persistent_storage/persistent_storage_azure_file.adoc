...
[NOTE]
====
Mount options are not available in {product-title} 3.7.43. You cannot use Azure
File in {product-title} 3.7.43. To use Azure file, upgrade to {product-title}
3.7.46.  
====
...
xref:../../architecture/additional_concepts/storage.adoc#architecture-additional-concepts-storage[PV 
claims] or from pods.
...
. Save the definition to a file, for example *_azure-file-pv.yaml_*, and create the
PV:
+
[source, bash]
----
