...
Additionally, the owner UID of the Azure File mounted directory is different
from the process UID of the container.

You may experience instability in your environment if you use any container
images that utilize unsupported file system features. Containers for PostgreSQL
and MySQL are known to have issues when used with Azure File.

For MySQL containers, the container changes the file owner permission in the
mounted directory. Because of the mismatch between the owner UID and container
process UID, this operation fails. The following workaround is provided:
