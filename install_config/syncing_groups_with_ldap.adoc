...
operation. This value may also be provided in an
link:../install_config/master_node_configuration.html#passwords[environment 
...
By default, if groups being synced contain members whose entries are outside of the
scope defined in the member query, the group sync fails with an error like:

----
Error determining LDAP group membership for "<group>": membership lookup for user "<user>" in group "<group>" failed because of "search for entry with dn="<user-dn>" would search outside of the base dn specified (dn="<base-dn>")".
----

This often indicates a mis-configured `baseDN` in the `usersQuery`. However,
in cases where the `baseDN` intentionally does not contain some of the members
of the group, setting `tolerateMemberOutOfScopeErrors: true` will allow the group
sync to continue. Out of scope members will be ignored.

Similarly, when the group sync process fails to locate a member for a group, it fails
outright with errors like:
...
This often indicates a mis-configured `usersQuery`. However, in cases where the
group contains member entries that are known to be missing, setting 
`tolerateMemberNotFoundErrors: true` will allow the group sync to continue. Problematic
members will be ignored.

[WARNING]
====
Enabling error tolerances for the LDAP group sync will cause the sync process to ignore
problematic member entries. If the LDAP group sync is not configured correctly, this could
result in synced OpenShift groups missing members.
====
...
----
<1> This member does not exist on the LDAP server.
<2> This member may exist, but is not under the base DN in the
...
----
<1> When `true`, the sync job will tolerate groups for which some members were not found.
If this is `true`, members whose LDAP entries are not found will be ignored. The default
behavior for the sync job is to fail if a member of a group is not found.
<2> When `true`, the sync job will tolerate groups for which some members are outside
the user scope given in the `usersQuery` base DN. If this is `true`, members outside the
member query scope will be ignored. The default behavior for the sync job is to fail if a
...
- jim.adams@example.com
----
<1> The users that are members of the Group, named as specified by the sync file,
absent any members for which lookup encountered errors that were tolerated.
====
