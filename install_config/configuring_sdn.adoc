...
The key to making the right decision rests between mutli-tenancy isolation vs
performance. Depending on the priority of each of these, should determine the
appropriate choice when deciding between OpenShift SDN and flannel internal
networks.
...
NOTE: Flannel gather information from etcd to configure and assign
...
NOTE: In order to provide custom networking CIDR for pods and services using
flannel, there is a bugzilla being invested by Red Hat Bugzilla ID 1473858 at
the time of writing this paper.

To isolate container traffic from the rest of the {product-title} traffic, it is
recommended to create an isolated tenant network and attach all the nodes to it.
If using a different network interface (eth1), remember to configure the
interface to start at boot time as:
...
In order to enable flannel, a set of iptables rules should be added as well
after the {product-title} installation has been completed.

On every {product-title} node:
...
NOTE: iptables-save saves all the current "in memory" iptables rules, but, as
both Docker, Kubernetes and {product-title} creates a lot of iptables rules
(services, etc.) not designed to be persisted, you will end up with a messy
