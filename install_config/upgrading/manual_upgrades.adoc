...
[IMPORTANT]
====
The manual upgrade steps in this topic are currently only applicable for
...
xref:manual-step-ocp-3-5-5-2-petsets-statefulsets[Migrating PetSets to
...
*atomic-openshift-utils* package on each host, which provides files that will be
...
# etcdctl backup --data-dir /var/lib/origin/openshift.local.etcd \
    --backup-dir /var/lib/origin/openshift.local.etcd.bak.<date> <2>
----
<1> The *etcd* package is required, even if using embedded etcd, for access to the
`etcdctl` command.
<2> Set the date of the backup in `<date>`, or some unique identifier. The command
will not make a backup if the `--backup-dir` location already exists.

. For any upgrade path, always ensure that you are running the latest kernel on
each host:
+
----
# yum update kernel
...
. On each master host, upgrade the *atomic-openshift-master* package:
+
----
# yum upgrade atomic-openshift-master
----

. Restart the *atomic-openshift-master* service and review its logs to ensure
...
Upgrade any external etcd hosts:

. On each external etcd host, upgrade the *etcd* package:
+
----
# yum update etcd
----

. Restart the *etcd* service and review its logs to ensure services have been
...
[IMPORTANT]
====
Masters technically also have node components running on them (in order to be
configured as part of the OpenShift SDN), but are set by default to
unschedulable status during initial installation. However, it is possible to set
them schedulable during the initial installation or manually thereafter. If any
of your masters are also configured as a schedulable node, skip the following
Docker upgrade steps for those masters and instead run all steps described in
xref:upgrading-nodes[Upgrading Nodes] when you get to that section for those
hosts as well.
====
...
. Restart the *docker* service and review its logs to ensure services have been
restarted successfully:
+
----
...
ifdef::openshift-enterprise[]
. On the node host, upgrade all *atomic-openshift* packages:
+
----
...
xref:../../install_config/registry/deploy_registry_existing_clusters.adoc#storage-for-the-registry[Deploying
...
You may safely skip this part if you do not use a custom configuration file.
...
the EFK Stack]. After the deployer completes, as a cluster administrator,
re-attach the persistent volumes you were previously using, then deploy a
...
[[manual-step-ocp-3-5-5-2]]
=== {product-title} 3.5.5.2

[discrete]
[[manual-step-ocp-3-5-5-2-petsets-statefulsets]]
