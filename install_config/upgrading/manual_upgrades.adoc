...
liveness probe. Ensure you have added `iptables` rules and cloud network
security rules allowing access to your node's public IP on port 1936:
...
An errata has been released which ensures that the router liveness probe uses
localhost rather than the public interface. This means you no longer need to open
the firewall rule above. Please see https://access.redhat.com/errata/product/290/ver=3.1/rhel---7/x86_64/RHBA-2016:0293[RHBA-2016:0293] for details
of that errata.
endif::[]
