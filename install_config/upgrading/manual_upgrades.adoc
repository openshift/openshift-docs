...
channel and enable the 3.6 channel on each host:
+
----
...
resources in  {product-title} 3.6 that are not supported by RBAC in
{product-title} 3.7. Such configurations require manual migration based on your
use case.
+
If you are upgrading from {product-title} 3.6 to 3.7, to guarantee that all
...
[IMPORTANT]
====
During a rolling upgrade, avoid actions that require changes to {product-title}
...
longer present due to a {product-title} 3.7 controller process being the leader,
...
>>>>>>> c5c3b2e... changed master service to master-api and master-controllers
[[upgrading-masters]]
...
+
In this example,
*_/usr/share/ansible/openshift-ansible/roles/openshift_examples/files/examples/v3.6/image-streams/image-streams-rhel7.json_*
...
into
*_/usr/share/ansible/openshift-ansible/roles/openshift_examples/files/examples/v3.6/_*.:
+
----
...
for {product-title} 3.6.

See the xref:../../release_notes/ocp_3_6_release_notes.adoc#release-notes-ocp-3-6-release-notes[{product-title} 3.6 Release Notes] to review the latest release notes.
