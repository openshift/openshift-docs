 ...
 template:
    spec:
      ...
      - env:
        ...
ifdef::openshift-enterprise[]
        image: registry.access.redhat.com/openshift3/ose-haproxy-router:v3.2.1.13 <1>
endif::[]
ifdef::openshift-origin[]
 ...
 template:
    spec:
      ...
      - env:
        ...
ifdef::openshift-enterprise[]
        image: registry.access.redhat.com/openshift3/ose-docker-registry:v3.2.1.13 <1>
endif::[]
ifdef::openshift-origin[]
...
# oc get -n default dc/docker-registry -o json | grep \"image\"
    "image": "openshift3/ose-docker-registry:v3.2.1.13",
# oc get -n default dc/router -o json | grep \"image\"
    "image": "openshift3/ose-haproxy-router:v3.2.1.13",
endif::[]
ifdef::openshift-origin[]
# oc get -n default dc/docker-registry -o json | grep \"image\"
...
uptime requirements or the resources to properly perform blue-green
deployments.  In an {product-title} environment the most suitable candidate for
blue-green deployments are the Nodes.  Uptime is most important for these
workloads and the additional complexity of blue-green deployments can be
justified.  Often the main hurdle to overcome is having the excess capacity to
...
[IMPORTANT]
====
When the deployment has been declared successful, it is important that the old
host be unregistered with `*subscription-manager clean*` to prevent the
environment from being out of compliance.
====
...
you will need to ensure that your current production nodes are labeled either
blue or green.  In this example the current installation will be blue and the
new environment will be green.

----
oc label --all nodes color=blue
----

There is quite a bit of flexibility here.  Perhaps there was only some subset of
nodes that required the uptime guarantees of a blue-green deployment.  In that
case, the `-l` flag can be used to match a subset of the environment using a
selector.

Next, xref:../install/advanced_install.adoc#adding-nodes-advanced[the green
environment] should be created for any nodes that are to be replaced.  The
`*color=green*` label can be applied by Ansible using the `*openshift_node_labels*`
variable for each node.  The most critical attribute of the node at this step
is that the `*openshift_schedulable=false*` variable be set.  The goal is for no
workloads to be scheduled until these nodes are determined to be healthy.

Once the green nodes are in `*Ready*` state, they can be made scheduleable. The
blue nodes are then disabled so that no new pods are run on them.
...
migrated to the green nodes.  For these pods, a canary deployment approach is
commonly used.  Scaling them up will make them immediately active on the new
nodes.  Pointing the deployment configuration to the new image causes a rolling
update to happen.  However, because of node anti-affinity, and the fact that the
...
images must be pulled.  Network latency and and load on the registry can cause
delays if there is not sufficient capacity built in to the environment. Often
the best way to minimize impact to the running system is trigger new pod
deployments that will land on the new nodes.  This can be accomplished by
importing new ImageStreams.

Quite often, a major release of {product-title} is the motivation for a blue-green
deployment.  When this occurs, there will be new ImageStreams available for users of
Source-to-Image.  Upon import, any builds or deployments configured with
ImageChangeTriggers will be automatically created.
...
In large environments, builds rarely completely stop.  You should, however, see
a large increase and descrease caused by the administrative import.
...
build images, the pod infrastructure image, and deployers.  Everything else can be
moved over using node evacuation and will proceed more quickly as a result.

=== Node Evacuation

For larger deployments, it is quite possible to have other labels that help
...
In OSE 3.2 and later, a node evacuation is triggered whenever the service is
stopped.  Manual evacuation and deletion of all blue nodes at once can be
achieved as follows:
...
critical pieces of {product-title} infrastructure are self-hosted there.  Uptime is
most important for these workloads and the additional complexity of blue-green
deployments can be justified.  The exact implementation of this approach
varies based on your requirements.
