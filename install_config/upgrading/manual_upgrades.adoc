...
[[manual-step-ocp-3-5-0-0]]

With deprecation of `extensions/v1beta1.Job` resource it is required to migrate
all the `Job` resources to use `batch/v1.Job` instead.  To verify which objects
will be migrated use following command:
...
If needed you can increase the log level using `--loglevel` flag.  When you are
ready to perform actual migration add `--confirm` to the above command:
