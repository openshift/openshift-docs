...
are joined to the
xref:../../architecture/additional_concepts/networking.adoc#network-plugins[pod network]).
To improve cluster management, add a label to each host that describes its type,
such as `type=master` or `type=node`.
+
