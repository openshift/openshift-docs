...
xref:../../install_config/http_proxies.adoc#install-config-http-proxies[Working with HTTP Proxies] for more about `NO_PROXY`). The installer will be updated in
a future release to handle this scenario automatically during installation and
upgrades
(link:https://bugzilla.redhat.com/show_bug.cgi?id=1466783[*BZ#1466783*]).
+
If you are not using a proxy, you can skip this step. Otherwise:
...
(clustered) or `4001` for embedded etcd (single master, non-clustered etcd).
