...
<3> Replace with the desired image version, or with `{latest-int-tag}` for the latest version.

. Add any other `openshift_logging_*` or `openshift_hosted_logging_*` variables
that you want to specify to override the defaults, as described in
...
. Add any other `openshift_metrics_*` or `openshift_hosted_metrics_*` variables that you want to specify to
