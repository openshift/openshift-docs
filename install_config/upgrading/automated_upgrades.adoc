...
xref:../../release_notes/ocp_3_5_release_notes.adoc#ocp-35-asynchronous-errata-updates[asynchronous
errata updates]
...
[NOTE]
====
See the link:https://github.com/openshift/openshift-ansible/blob/release-1.5/inventory/byo/hosts.origin.example[{product-title} example Ansible host file] in the *_openshift/openshift-ansible_* repo for the full list of variables.
====
endif::[]
+
ifdef::openshift-enterprise[]
[NOTE]
====
See the link:https://github.com/openshift/openshift-ansible/blob/release-1.5/inventory/byo/hosts.ose.example[{product-title} example Ansible host file] in the *_openshift/openshift-ansible_* repo for the full list of variables.
====
endif::[]
...
openshift_hosted_metrics_deployer_version=3.5.0 <3>
...
<3> Replace `3.5.0` with `{latest-int-tag}` for the latest version if desired.
...
----
+
Use the IP address or host name that the master uses to contact the etcd cluster as the
