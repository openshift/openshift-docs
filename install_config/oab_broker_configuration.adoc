...
[[install-config-oab-authenticating]]
== Authenticating on Red Hat Partner Connect Registry

Before configuring the Automation Broker, you must run the following command on all nodes of an {product-title} cluster to use the Red Hat Partner Connect:
...
=== Red Hat Partner Connect Registry

Third-party images in the Red Hat Container Catalog are served from the Red Hat
Partner Connect Registry at link:https://registry.connect.redhat.com[]. The
