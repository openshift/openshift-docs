...
There is variety of production-ready storage back-ends to choose from. They fall into two categories:

1. Supported directly by the Docker registry.
2. Supported by the Kubernetes
xref:../../architecture/additional_concepts/storage.adoc#architecture-additional-concepts-storage[persistent
volume] framework.

The former category is generally preferred because the storage back-ends are
object stores with an excellent read-after-write consistency which is a hard
requirement of the registry. See
xref:../registry/extended_registry_configuration.adoc#docker-registry-configuration-reference-storage[the
list of the supported back-ends and their configuration].

The back-end falling into the later category needs to be attached as a
persistent remote volume into the registry pod. The registry must be configured
to use
xref:extended_registry_configuration.adoc#docker-registry-configuration-reference-storage[`filesystem
driver`] in order to utilize it. Read more about
xref:../persistent_storage/index.adoc#install-config-persistent-storage-index[defining
and using the persistent storage method of your choice]. It comprises of less
consistent storage options like
xref:../persistent_storage/persistent_storage_nfs.adoc#install-config-persistent-storage-nfs[NFS]
which should be avoided if possible.

[NOTE]
====
See xref:registry_known_issues.adoc#install-config-registry-known-issues[Known
Issues] if using a scaled registry with a shared NFS volume.
====

Following is an example how to use an existing persistent volume claim:
...
==== Amazon S3 as a Storage Back-end

Is one of the mature options in the category of back-ends directly supported by
the Docker Registry. It is a secure cloud storage manageable through
