...
To deploy the integrated container registry, use the `oc adm registry` command from
...
To deploy the integrated container registry, use the `oc adm registry` command as a
...
the container registry. This includes the OpenShift Container Registry and Quay.
...
internal container registry. It is a secure cloud storage manageable through
...
[IMPORTANT]
====
The container registry pod runs as user *1001*. This user must be able to write to
...
----
+
Use the following command to get the container registry service IP address:
+
----
...
----
+
Use the following command to get the container registry public hostname:
+
----
