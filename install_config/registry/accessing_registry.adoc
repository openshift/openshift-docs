...
To view the logs for the container registry, use the `oc logs` command with the
...
. List the current pods to find the pod name of your container registry:
+
----
# oc get pods
----
...
. Log in to the container registry by using your access token:
+
----
