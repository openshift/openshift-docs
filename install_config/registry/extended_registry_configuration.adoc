...
You are advised to configure the hostname and port the registry is known by for both internal and
external references. By doing this, image streams will provide hostname based push and pull
specifications for images, allowing consumers of the images to be isolated from changes to the
registry service ip and potentially allowing image streams and their references to be portable
between clusters. Starting with {product-title} 3.7, this is done by default during the
xref:../install/advanced_install.adoc[advanced installation].
...
----
imagePolicyConfig:
  internalRegistryHostname: docker-registry.default.svc.cluster.local:5000 <1>
  externalRegistryHostname: docker-registry.mycompany.com
----
<1> The default value used in xref:../install/advanced_install.adoc[advanced installation] is
`docker-registry.default.svc:5000`.
====

The registry itself must be configured with the same internal hostname value.  This can be
accomplished by setting the `*REGISTRY_OPENSHIFT_SERVER_ADDR*` environment variable on the
registry deployment configuration, or by setting the value in the
xref:extended_registry_configuration#docker-registry-configuration-reference-openshift[OpenShift
section] of the registry configuration. This is again set by the
xref:../install/advanced_install.adoc[advanced installer] to the same default value as the
`*imagePolicyConfig:internalRegistryHostname*` above.
...
This section lists the supported registry storage drivers. See link:https://docs.docker.com/registry/configuration/#storage[the Docker registry documentation for more information].
...
*  link:https://docs.docker.com/registry/storage-drivers/s3/[S3]. See the https://github.com/docker/docker.github.io/blob/master/registry/storage-drivers/s3.md#cloudfront-as-middleware-with-s3-backend[CloudFront configuration] documentation for more information.
...
General registry storage configuration options are supported. See link:https://docs.docker.com/registry/configuration/#maintenance[the Docker registry documentation for more information].
...
in the *_/etc/pki/tls/certs_* directory on the pod. You can mount the certifiates 
