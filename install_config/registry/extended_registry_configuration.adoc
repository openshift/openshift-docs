...
[[s3-driver]]
==== S3
The main documentation about S3 driver configuration is
link:https://docs.docker.com/registry/storage-drivers/s3/[here].
...
For more information about the regions and endpoints for Amazon S3, see
link:https://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region[Regions
and Endpoints].
...
[NOTE]
====
It is very important in this case that the `region` and `regionendpoint` fields
are consistent between themselves. Otherwise the integrated registry will start,
but it can not read or write anything to the S3 storage.
====

The `regionendpoint` can also be useful if you use a S3 storage different from the
Amazon S3 like a link:minio.io[Minio] or
link:ceph.com/ceph-storage/object-storage[Redhat Ceph].
