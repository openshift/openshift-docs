...
* link:https://docs.docker.com/registry/storage-drivers/filesystem[Filesystem] which is not recommended for production use
...
[NOTE]
====
The list only includes the filesystems that need to be configured in the
registry's configuration file with an exception of filesystem, which is the default.
There are other storage options including:
...
Rados Block Device]
* xref:../../install_config/persistent_storage/index.adoc#install-config-persistent-storage-persistent-storage-ceph-rbd[And
more]. See
xref:../../install_config/storage_examples/index.adoc#install-config-storage-examples-index[examples]
as well.

You must configure the
link:https://docs.docker.com/registry/storage-drivers/filesystem[filesystem
driver] to use these storage options.
====
