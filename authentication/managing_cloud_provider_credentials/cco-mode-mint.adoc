...
Mint mode is the default Cloud Credential Operator (CCO) credentials mode for {product-title} on platforms that support it. Mint mode supports Amazon Web Services (AWS) and {gcp-full} clusters.
...
The credential you provide for mint mode in {gcp-full} must have the following permissions:

.Required {gcp-full} permissions
[%collapsible]
====
