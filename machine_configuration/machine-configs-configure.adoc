...
You can create a machine config that applies changes to all control plane or all worker nodes, or you can use xref:../machine_configuration/machine-config-about.html#machine-config-pool-custom_machine-configs-custom[custom machine config pools] that apply changes to a specific set of worker nodes.
