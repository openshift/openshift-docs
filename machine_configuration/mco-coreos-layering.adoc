...
content: |-
  ...
  RUN dnf update https://example.com/files/systemd-252-46.el9_4.x86_64.rpm \
...
----
+
If you are using out-of-cluster layering, because libreswan requires additional {op-system-base} packages, the image must be built on an entitled {op-system-base} host. For RHEL entitlements to work, you must copy the `etc-pki-entitlement` secret into the `openshift-machine-config-operator` namespace.
