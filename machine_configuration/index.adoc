...
[role="_abstract"]
You can configure operating system on {product-title} nodes by modifying network time services, adding kernel arguments, and setting up journaling. You can make changes to the operating systems on {product-title} nodes by creating `MachineConfig` objects, which are managed by the Machine Config Operator.
...
include::modules/machine-config-overview-changes.adoc[leveloffset=+2]
...
include::modules/machine-config-node-update-phases.adoc[leveloffset=+2]

include::modules/machine-config-viewing-node-status.adoc[leveloffset=+2]
...
* link:https://access.redhat.com/solutions/5414371[How to skip validation of failing / stuck MachineConfig in OCP 4 (Red Hat Knowledgebase)]
...
* link:https://github.com/openshift/runbooks/blob/master/alerts/machine-config-operator/MachineConfigControllerDrainError.md[MCCDrainError runbook]
