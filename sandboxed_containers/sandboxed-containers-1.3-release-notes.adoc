...
----
+
MCS labels set at the container level aren't applied to virtiofsd. The `kata` runtime doesn't have access to the container's security context when creating the VM.
+
This means that virtiofsd won't run with the appropriate SELinux label and won't be able to access host files on behalf of the containers in the VM; all containers can access all files in the VM.
+
