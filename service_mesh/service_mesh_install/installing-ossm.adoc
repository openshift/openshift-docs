...
[NOTE]
====
Mixer’s policy enforcement is disabled by default. You must enable it to run policy tasks. See xref:../../service_mesh/service_mesh_day_two/prepare-to-deploy-applications-ossm.adoc#ossm-mixer-policy[Update Mixer policy enforcement] for instructions on enabling Mixer policy enforcement.
====
