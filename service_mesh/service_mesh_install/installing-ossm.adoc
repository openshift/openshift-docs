...
[NOTE]
====
Mixer’s policy enforcement is disabled by default. You must enable it to run policy tasks. See xref:../../prepare-to-deploy-applications-ossm.adoc#ossm-mixer-policy_deploying-applications-ossm[Update Mixer policy enforcement] for instructions on enabling Mixer policy enforcement.
====
...
.Prerequisites
* Follow the xref:../../preparing-ossm-installation.adoc#preparing-ossm-installation[Preparing to install {ProductName}] process.
...
* xref:../../customizing-installation-ossm.adoc#customize-installation-ossm[Customize the {ProductName} installation].
