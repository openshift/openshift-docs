...
Installing the {ProductShortName} involves installing the {ProductShortName} Operator, creating a `ServiceMeshControlPlane` resource to deploy the control plane, and creating a `ServiceMeshMemberRoll` resource to specify the namespaces associated with the {ProductShortName}.
