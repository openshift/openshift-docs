...
For information about mTLS, see xref:../../service_mesh/v2x/ossm-config.adoc#ossm-security-mtls_ossm-config-v2x[Enabling mutual Transport Layer Security (mTLS)]
...
Istiod manages client certificates and private keys used by service proxies.  By default, Istiod uses a self-signed certificate for signing, but you can configure a custom certificate and private key. For more information about how to configure signing keys, see xref:../../service_mesh/v2x/ossm-config.adoc#ossm-cert-manage_ossm-config-v2x[Adding an external certificate authority key and certificate]
