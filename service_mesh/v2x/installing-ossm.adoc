...
To run the Service Mesh control plane on an infrastructure node, move the operation from the worker node it was installed on to an infrastructure node. For more information, see ____________________________________________________.

include::modules/ossm-installing-service-mesh-operator-to-infrastructure-node.adoc[leveloffset=+1]
