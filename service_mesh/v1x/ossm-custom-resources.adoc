...
ifdef::openshift-enterprise[]
// this content is not supported on this branch. For more information about configuring Elasticsearch with {product-title}, see  xrf:../../observability/logging/log_storage/logging-config-es-store.adoc#logging-config-es-store[Configuring the Elasticsearch log store].
endif::[]
