...
[NOTE]
====
Mixer’s policy enforcement is disabled by default. You must enable it to run policy tasks. See xref:../../service_mesh/service_mesh_day_two/prepare-to-deploy-applications-ossm.adoc#ossm-mixer-policy_deploying-applications-ossm[Update Mixer policy enforcement] for instructions on enabling Mixer policy enforcement.
====
...
== Prerequisites
* Follow the xref:../../service_mesh/service_mesh_install_v1x/preparing-ossm-installation-v1x.adoc#preparing-ossm-installation-v1x[Preparing to install {ProductName}] process.
...
* xref:../../service_mesh/service_mesh_install_v1x/customizing-installation-ossm-v1x.adoc#customize-installation-ossm-v1x[Customize the {ProductName} installation].
