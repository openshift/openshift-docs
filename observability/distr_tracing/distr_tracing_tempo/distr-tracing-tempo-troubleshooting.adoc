...
The `must-gather` tool helps you gather diagnostic data for various resources, such as `TempoStack`, `TempoMonolithic`, and all related resources like `Deployments`, `Pods`, or `ConfigMaps`.
...
[source,terminal]
----
$ oc adm must-gather --image=ghcr.io/grafana/tempo-operator/must-gather -- /usr/bin/must-gather --operator-namespace openshift-tempo-operator
----
