...
The {JaegerOperator} Operator will be removed from the `redhat-operators` catalog in November 2025. For more information, see link:https://access.redhat.com/solutions/7083722[Jaeger Deprecation and Removal in OpenShift].

The {TempoOperator} and the {OTELName} are the preferred Operators for distributed tracing collection and storage.
Users must adopt the OpenTelemetry and Tempo distributed tracing stack because it is the stack to be enhanced going forward. For more information, see xref:../otel/otel-migrating.adoc#dist-tracing-otel-migrating[Migrating] in the {OTELName} documentation.
