...
|`endpoint_params`
|Further metadata, in key-value pairs, transferred during authentication (e.g., `audience`).
|N/A
...
|`tls.insecure`
|When set to `true`, configures the Collector to use an insecure or non-verified TLS connection to the token endpoint.
|`false`

|`tls.ca_file`
|The path to a Certificate Authority (CA) file used to verify the server's certificate during the TLS handshake.
|N/A

|`tls.cert_file`
|The path to the client certificate file used for authentication with the OAuth2 server if required.
|N/A

|`tls.key_file`
|The path to the client's private key file used with the client certificate for authentication.
|N/A
...
|`authenticator`
|Assigns the authenticator configuration to an OTLP exporter.
|N/A

|`client_id_file`
|The file path to retrieve the client identifier issued to the client. The extension reads this file and updates the client ID.
|N/A

|`client_secret_file`
|The file path to retrieve the secret string associated with the client identifier. The extension reads this file and updates the client secret.
|N/A
|===
...
- Verify the `client_id` and `client_secret`: ensure the `client_id` and `client_secret` values match those provided by the identity provider.
+
- Check the file paths: if using `client_id_file` or `client_secret_file`, confirm the paths are correct and accessible by the OpenTelemetry Collector.
+
- Inspect the `token_url`: verify that the `token_url` is correct and reachable.
+
- Check the certificate paths: confirm that `tls.ca_file`, `tls.cert_file`, and `tls.key_file` paths are correctly set and accessible.
+
- Review identity provider rate limits: ensure that the identity provider is not rate-limiting token requests, which could delay or fail requests.
+
- Check the `token_url`: ensure the token endpoint URL is correct and accessible by the OpenTelemetry Collector.
+
- Verify `scopes`: ensure the requested `scopes` align with the permissions provided to the client by the identity provider.
