...
This processor reassigns spans, log records, and metric datapoints to a Resource that matches the specified attributes. Consequently, all spans, log records, and metric datapoints sharing the same values for these attributes are grouped under the same Resource.

For configuring this processor you only need to specify an array of attribute keys that will be used to "group" spans, log records or metric data points together, as in the below example:
...
The keys property specifies which attribute keys will be used for grouping:

- If a processed span, log record, or metric data point contains at least one of the specified attribute keys, it will be reassigned to a Resource that shares the same attribute values. If no such Resource exists, a new one will be created.
- If none of the specified attribute keys are present in the processed span, log record, or metric data point, it will remain associated with its current Resource. Multiple instances of the same Resource will still be consolidated.