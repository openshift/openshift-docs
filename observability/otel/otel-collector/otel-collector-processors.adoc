...
== WIP Tail Sampling Processor

The Tail Sampling Processor samples traces with all completed spans based on a set of defined policies. Tail-based sampling enables you to filter traces of interest and reduce your data ingestiion and storage costs.
...
[WARNING]
====
In pipelines, this processor must follow any processors that rely on context: for example, after the Kubernetes Attributes Processor.
====
...
<4> Optional: The expected number of new traces (helps in allocating data structures). Defaults to `0`.
<5> Definitions of the policies against which the processor evaluates each trace. each and all of the added policies to make a final decision: If there is a "drop" decision, the trace is not sampled. If there is a "sample" decision, the trace is sampled. In all other cases, the trace is not sampled.

You can choose and add a combination of policies from the following list:
...
# ...
----
<1> You can determine the desired latency values by looking at the earliest start time and latest end time. If you omit `upper_threshold_ms`, the policy samples all latencies greater than the `threshold_ms` value.

* The following policy samples traces by ?????? number attributes (resource and record):
+
----
# ...
      policies:
...
* The following policy samples traces by string attributes (resource and record) value matches:
+
----
# ...
      policies:
...
# ...
----
<1> This policy definition supports both exact and regular-expression value matches.
...
* The following policy samples traces by the minimum and/or maximum number of spans, inclusive:
+
----
# ...
      policies:
...
# ...
----
<1> If the sum of all spans in the trace is outside the range threshold, the trace will not be sampled.
...
* The following policy samples traces by a given boolean OTTL condition (span and span event):
+
----
# ...
      policies:
...
* The following is an `AND` policy that samples traces by combining multiple policies:
+
----
# ...
      policies:
...
# ...
----
<1> Rate allocation allocates percentages of spans according to the order of applied policies. For example, if the `max_total_spans_per_second` field is set to the `100` value, you can set `rate_allocation` as follows: `test-composite-policy-1` = 50 % of `max_total_spans_per_second` = 50 spans_per_second, `test-composite-policy-2` = 25 % of `max_total_spans_per_second` = 25 `spans_per_second`, To ensure that the remaining capacity is filled, you can add the `always_sample` policy.

=== Scaling Collectors with the tail sampling processor
For effective sampling decisions, the same instance of the Collector must be receiving all spans of the same trace.
...
Use the Load Balancing Exporter
ADD source to Additional resources: https://opentelemetry.io/docs/collector/deployment/gateway/
"you will need to use the load-balancing exporter when using the Tail Sampling processor so that all spans for a given trace reach the same collector instance where the tail sampling policy is applied."
----
