...
This processor reassembles spans into new batches and strips spans of their original context. So in pipelines, you must position this processor after any processors that rely on context, for example after the Kubernetes Attributes Processor.
...
<9> Policy to sample based on string attributes (resource and record) value matches, both exact and regular-expression value matches are supported.
