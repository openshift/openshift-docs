...
The AWS CloudWatch Logs Exporter sends logs data to the AWS CloudWatch Logs service with signing the requests by the AWS SDK for Go using the default credential provider chain.
...
# ...
----
<1> Required. If the log group does not exist yet, it will be created automatically.
<2> Required. If the log stream does not exist yet, it will be created automatically.
<3> Optional. If the AWS region is not already set in the default credential chain, you must specify it.
<4> Optional. The Amazon CloudWatch Logs service endpoint to which the requests are forwarded. link:https://docs.aws.amazon.com/general/latest/gr/cwl_region.html[See the CloudWatch Logs endpoints] for a list.
<5> Optional. With this parameter, you can set the log retention policy for new CloudWatch log groups. If this parameter is omitted or set to `0`, the logs never expire by default. Supported values for retention in days are `1`, `3`, `5`, `7`, `14`, `30`, `60`, `90`, `120`, `150`, `180`, `365`, `400`, `545`, `731`, `1827`, `2192`, `2557`, `2922`, `3288`, or `3653`.

[role="_additional-resources"]
.Additional resources
* link:https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/WhatIsCloudWatchLogs.html[What is Amazon CloudWatch Logs?] (AWS CloudWatch Logs User Guide)
...
* link:https://docs.aws.amazon.com/general/latest/gr/cwl_region.html[Service endpoints] (AWS General Reference)
...
The EMF metrics are then sent directly to the AWS CloudWatch Logs service by using the `PutLogEvents` API.

One of the benefits of using this exporter is the possibility to view logs and metrics in the CloudWatch console at `+https://console.aws.amazon.com/cloudwatch/+`.
...
# ...
  config:
    exporters:
      awsemf:
        log_group_name: "testing-logs" # <1>
        log_stream_name: "testing-integrations-stream" # <2>
...
<3> The AWS region of the log stream. If a region is not already set in the default credential provider chain, you must specify the region.
<4> The CloudWatch Logs service endpoint to which the requests are forwarded. This must be an endpoint from the list of the link:https://docs.aws.amazon.com/general/latest/gr/cwl_region.html[Service endpoints] (AWS General Reference).

The `log_group_name` configuration parameter allows you to customize the log group name and supports the default `+/metrics/default+` value or the following placeholders:
...
The `log_stream_name` configuration parameter allows you to customize the log stream name and supports the default `+otel-stream+` value or the following placeholders:
...
The AWS X-Ray Exporter converts OpenTelemetry spans to AWS X-Ray Segment Documents and then sends them directly to the AWS X-Ray service by using the `PutTraceSegments` API and signing the requests by the AWS SDK for Go using the default credential provider chain.
...
# ...
  config:
    exporters:
      awsxray:
        region: "eu-west-1" # <1>
        resource_arn: "arn:aws:ec2:us-east1:123456789:instance/i-293hiuhe0u" # <2>
        role_arn: "arn:aws:iam::123456789:role/monitoring-EKS-NodeInstanceRole" # <3>
        indexed_attributes: [ "indexed_attr_0", "indexed_attr_1" ] # <4>
        aws_log_groups: ["group1", "group2"] # <5>
        request_timeout_seconds: 120 # <6>
# ...
----
<1> The destination region for the X-Ray segments sent to the AWS X-Ray service.
<2> The Amazon Resource Name (ARN) of the AWS resource that is running the Collector.
<3> The AWS Identity and Access Management (IAM) role for uploading the X-Ray segments to a different account.
<4> The list of attribute names to be converted to X-Ray annotations.
<5> The list of log group names for AWS CloudWatch Logs.
<6> Time duration in seconds before timing out a request. If omitted, the default value is `30`.
