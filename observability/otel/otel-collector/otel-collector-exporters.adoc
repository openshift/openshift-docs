...
# ...
----
<1> In this example, the metrics are sent to a Prometheus instance exposed in a `Service` called `my-prometheus`.
...
.Parameters used by the Prometheus Remote Write Exporter
[options="header"]
[cols="a,a,a"]
|===
|Parameter |Description |Default
...
|`tls`
|Server-side TLS configuration, defining paths to TLS certificates.
|N/A
...
|When set to `true`, exports a `_created` metric for Summary, Histogram, and Monotonic Sum metric points.
|`false`

|`max_batch_size_bytes`
|Maximum size of the batch of samples sent to the remote write endpoint. If exceeded, the batch is split.
|`3000000` (2.861 MB)
...
|`retry_on_failure.max_elapsed_time`
|Maximum time spent trying to send a batch.
|`300s`

|`remote_write_queue.enabled`
|Settings for the remote write sending queue.
|`true`

|`remote_write_queue.queue_size`
|Number of OTLP metrics that can be queued.
|`1000`
...
=== Troubleshooting the Prometheus Remote Write Exporter

If you encounter issues with the Prometheus Remote Write Exporter, the following troubleshooting steps can help diagnose and resolve common problems.

==== Metrics not reaching the Prometheus instance

If metrics are not reaching the Prometheus instance, consider the following:

.Procedure

- Verify the `endpoint`: ensure that the `endpoint` is correctly configured, including the appropriate URL and port. Confirm that the Prometheus instance is accessible and that the `--web.enable-remote-write-receiver` flag is enabled on the remote Prometheus server.
+
- Check the pipeline configuration: ensure that the Prometheus Remote Write Exporter is properly configured within the OpenTelemetry Collector's pipeline. Metrics should be flowing through the correct pipeline to the exporter.

==== TLS or certificate issues

If you experience TLS-related errors or connection failures, check the following:

.Procedure

- Verify the TLS configuration: ensure that the `ca_file`, `cert_file`, and `key_file` paths are correctly specified and that the certificates are valid and accessible by the OpenTelemetry Collector.
+
- Endpoint security: ensure that the Prometheus endpoint is correctly configured for TLS, and that any intermediate certificates are correctly set up for secure communication.

==== Exporter dropping specific metrics

If certain metrics (e.g., non-cumulative monotonic, histogram, or summary metrics) are missing, consider the following:

.Procedure

- Check for unsupported metrics: the Prometheus Remote Write Exporter drops non-cumulative monotonic, histogram, and summary OTLP metrics by design. Ensure that the exported metrics are compatible with Prometheus Remote Write.
+
- Review configuration flags on the Prometheus server: ensure that the `--web.enable-remote-write-receiver` flag is enabled. Without this flag, pushing metrics to Prometheus will fail.
