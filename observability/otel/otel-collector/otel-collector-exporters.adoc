...
# ...
----
<1> The file-system path where the data is to be written. No default.
...
<4> The `max_days` setting is for how many days a file is to be retained, counting from the timestamp in the file name. No default.
...
<8> File compression is optional and not set by default. This setting defines the compression algorithm for the data that is exported to a file. Currently, only the `zstd` compression algorithm is supported. No default.
