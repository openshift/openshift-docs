...
This exporter writes telemetry data to files on disk, supporting rotation, compression, and writing to multiple files. The filename can be determined by a resource attribute. The only mandatory option is path, which specified the path to write data in the filesystem.
...
# ...
----
<1> path - this is the path in the file system where the data will be written.
<2> rotation - under this node all otions for rotate file can be found. rotation options determine when the file should be rotated and include maximun size in megabytes, max_days, max_backups also allow to specify if local timestam or UTC will be used.
<3> format - Format in which the data will be written to disk, possible values json/proto
<4> compression - compresison to be applied when exporting data, default=None, possible values are: zstd
<5> flush_interval ime.Duration interval between flushes
