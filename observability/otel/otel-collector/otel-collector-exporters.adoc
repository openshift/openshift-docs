...
The Prometheus Remote Write Exporter exports metrics to Prometheus remote write compatible backends. Non-cumulative monotonic, histogram, and summary OTLP metrics are dropped by this exporter.

:FeatureName: The Prometheus Remote Write
include::snippets/technology-preview.adoc[]
...
# ...
----
<1> The endpoint where the metrics are send.
<2> The server-side TLS configuration. Defines paths to TLS certificates.
<3> If enabled, a `target_info` metric will be created for each resource metric.
<4> If enabled, a `_created` metric is exported for Summary, Histogram, and Monotonic Sum metric points.
<5> Maximum number of batch of samples.
