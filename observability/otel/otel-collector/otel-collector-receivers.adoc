...
<2> Server-side TLS configuration. Defines paths to TLS certificates. If omitted, the TLS is disabled. For more information, see the xref:otel-collector-tls.adoc#otel-collector-tls[TLS configuration for the OpenTelemetry Collector].
...
<3> OTLP HTTP endpoint. The default value is `+0.0.0.0:4318+`.
...
<2> The server-side TLS configuration. For more information, see the xref:otel-collector-tls.adoc#otel-collector-tls[TLS configuration for the OpenTelemetry Collector].
