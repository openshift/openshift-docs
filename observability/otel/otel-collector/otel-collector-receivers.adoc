...
# ...
----
<1> OTLP gRPC endpoint. If omitted, the default `+0.0.0.0:4317+` is used.
<2> Server-side TLS configuration. Defines paths to TLS certificates. If omitted, the TLS is disabled.
// For more information, see the xref:otel-collector-tls.adoc#otel-collector-tls[TLS configuration for the OpenTelemetry Collector].
<3> Path to the TLS certificate at which the server verifies a client certificate. This sets the value of `ClientCAs` and `ClientAuth` to `RequireAndVerifyClientCert` in the `TLSConfig`. For more information, see the link:https://godoc.org/crypto/tls#Config[`Config` of the Golang TLS package].
<4> Reloads the `client_ca_file` file when it is modified. Disabled by default.
<5> Specifies the time interval after which the certificate is reloaded. If the value is not set, the certificate is never reloaded. The `reload_interval` accepts a string containing valid units of time such as `ns`, `us` or `µs`, `ms`, `s`, `m`, `h`.
<6> OTLP HTTP endpoint. The default value is `+0.0.0.0:4318+`.
<7> Server-side TLS configuration. For more information, see the `grpc` protocol configuration section.
...
<5> The client-side TLS configuration. Defines paths to the TLS certificates. If omitted, TLS authentication is disabled. For more information, see the xref:otel-collector-tls.adoc#otel-collector-tls[TLS configuration for the OpenTelemetry Collector].
