...
[id="prometheus-remote-write-receiver_{context}"]
== Prometheus Remote Write Receiver

The Prometheus Remote Write Receiver receives metrics from Prometheus via the Remote Write protocol and converts them to OpenTelemetry format. This receiver supports only the Prometheus Remote Write v2 protocol.
...
[IMPORTANT]
====
Prometheus must be started with the metadata WAL records feature flag enabled, which is required to translate metric type, unit and description into OTLP format.

Additionally, the Prometheus remote write configuration must specify the v2 protobuf message format and enable conversion of classic histograms into native histograms which also requires enabling native histograms feature flag.
====

.OpenTelemetry Collector custom resource with an enabled Prometheus Remote Write Receiver
[source,yaml]
----
# ...
  config:
