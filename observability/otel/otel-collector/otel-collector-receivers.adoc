...
The OTLP JSON File Receiver extracts pipeline information from JSON-formatted files. These files contain data encoded in Protobuf JSON format, adhering to the OpenTelemetry protocol specifications.

This component monitors the specified folder for changes. When a new file appears or an existing one is modified, it processes the entire contents of that file once more.
...
# ...
----
<1> The list of file glob patterns that match the file paths to be read.
<2> The list of file glob patterns that match the file paths to be excluded.
