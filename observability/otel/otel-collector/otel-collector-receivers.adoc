...
The AWS X-Ray exporter converts OpenTelemetry spans to AWS X-Ray Segment Documents and then sends them directly to X-Ray using the PutTraceSegments API.
...
# ...
----
<1> Send segments to AWS X-Ray service in a specific region.
<2> Amazon Resource Name (ARN) of the AWS resource running the collector.
<3> IAM role to upload segments to a different account.
<4> List of attribute names to be converted to X-Ray annotations.
<5> List of log group names for CloudWatch.
<6> Number of seconds before timing out a request. If omitted, the default value is `+30+`.
...
.OpenTelemetry Collector custom resource with the enabled AWS CloudWatch exporter
[source,yaml]
----
# ...
  config:
...
# ...
----
<1> The group name of the CloudWatch Logs. If it does not exist it will be created automatically.
<2> The stream name of the CloudWatch Logs. If it does not exist it will be created automatically.
<3> The AWS region where the log stream is in. Region must be specified if it is not already set in the default credential chain.
<4> The CloudWatch Logs service endpoint which the requests are forwarded to. link:https://docs.aws.amazon.com/general/latest/gr/cwl_region.html[See the CloudWatch Logs endpoints] for a list.
<5> LogRetention is the option to set the log retention policy for only newly created CloudWatch Log Groups. Defaults to Never Expire if not specified or set to 0. Possible values for retention in days are 1, 3, 5, 7, 14, 30, 60, 90, 120, 150, 180, 365, 400, 545, 731, 1827, 2192, 2557, 2922, 3288, or 3653.

[id="aws-emf-exporter_{context}"]
== AWS EMF Exporter

This exporter converts OpenTelemetry metrics to
link:https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Embedded_Metric_Format_Specification.html[AWS CloudWatch Embedded Metric Format(EMF)]
and then sends them directly to CloudWatch Logs using the link:https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_PutLogEvents.html[PutLogEvents API].
