...
The {OTELName} 3.6 is provided through the link:https://catalog.redhat.com/software/containers/rhosdt/opentelemetry-operator-bundle/615618406feffc5384e84400/history[{OTELOperator} 0.???.0].

[NOTE]
====
The {OTELName} 3.6 is based on the open source link:https://opentelemetry.io/docs/collector/[OpenTelemetry] release 0.???.0.
====
...
* WIP With this update, the OpenTelemetry Collector now is able to read TLS certificates in TTS2 format, neccesary when TLS certificates are stored in TPM.

* WIP With this update, the Operator have a more informative error message when creating/upgrading collector instance and using API version opentelemetry.io/v1beta1 fails.

* WIP With this update, the Operator will re-try failed upgrades.
