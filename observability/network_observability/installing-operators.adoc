...
[NOTE]
====
The {loki-op} can also be used for xref:../../observability/logging/logging-6.1/log6x-loki-6.1#log6x-loki-6.1[Configuring LokiStack storage]. The Network Observability Operator requires a dedicated LokiStack separate from the {logging}.
====
...
// This link seems to be obsolete now
