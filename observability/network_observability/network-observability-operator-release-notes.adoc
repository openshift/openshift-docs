...
//==Features to be documented. Based on past releases, it looks like each feature gets its own === heading
...
* Previously it was not obvious to figure out there was an issue when referencing a LokiStack from the FlowCollector resource, such as in case of typo. Now, the FlowCollector status clearly states that the referenced LokiStack is not found in that case. (link:https://issues.redhat.com/browse/NETOBSERV-2174[*NETOBSERV-2174*])
