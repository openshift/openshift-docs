...
* With this update, {product-title} 4.19 has a new version of the Kernel, so only Network Observability 1.9 version of OVN Observability works in {product-title} 4.19 If you are using an older version of Network Observability, Network Events will not work. For example, OpenShift 4.18 is compatible with tech preview Network Events in Network Observability 1.8. (to be re-tooled with eng team at time of release notes creation)
