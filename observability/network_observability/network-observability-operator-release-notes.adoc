...
With this release, the user-defined networks (UDN) feature is generally available with Network Observability. When the `UDNMapping` feature is enabled in Network Observability, the *Traffic* flow table has a `UDN labels` column. You can filter on *Source Network Name* and *Destination Network Name*.
...
* This update includes a new Kernel version in {product-title} 4.19. Consequently, only Network Observability 1.9 is compatible with {product-title} 4.19's OVN Observability. Older versions of Network Observability are not supported and will cause Network Events to fail. For example, OpenShift 4.18 is compatible with the Technology Preview of Network Events in Network Observability 1.8.
...
For more information, see
//xref:../../network_observability/observing-network-traffic.adoc#network-observability-ebpf-manager_nw-observe-network-traffic[Working with eBPF Manager Operator]
