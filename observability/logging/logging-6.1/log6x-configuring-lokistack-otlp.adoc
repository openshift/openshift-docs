...
When the {loki-op} is set to `openshift-logging` mode, it automatically applies a default set of xref:../../../observability/logging/logging-6.1/log6x-opentelemetry-data-model.adoc#attributes_log6x-opentelemetry-data-model[attribute mappings]. These mappings align specific OTLP attributes with Loki's stream labels and structured metadata.
...
For more infomation on configuring custom attribute mappings, see xref:../../../observability/logging/logging-6.1/log6x-configuring-lokistack-otlp.adoc#custom-attribute-mapping-for-openshift_log6x-configuring-lokistack-otlp[Custom attribute mapping for OpenShift].
