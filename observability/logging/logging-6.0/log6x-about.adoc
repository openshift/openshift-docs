...
* OTEL (Technology Preview)

You can select either of these data models based on your requirements. The ViaQ data model is the default and fully supported option, while OTEL uses the OpenTelemetry standards and is not fully supported. For more information, see xref:../../../observability/logging/logging-6.0/log6x-otel-data-model.adoc#log6x-otel-data-model[OTEL data model].
...
[NOTE]
====
The ViaQ data model is now deprecated and is planned to be removed in future logging releases.
====
...
=== Quick Start with OTEL

To configure OTLP ingestion and enable the OTEL data model, follow these steps:
...
spec:
  ...
  serviceAccount:
    name: logger-admin
----
<1> Use the annotation to enable the `Otel` output, which is a Technology Preview feature.
<2> Define the `OTLP` output type as `lokistack`.
<3> DataModel is `Otel`.
<4> `tls` spec identifies the keys and secret to the respective certificates that they represent.
+
[NOTE]
====
You cannot use `lokistack` `labelKeys` when `dataModel` is `Otel`.
====
