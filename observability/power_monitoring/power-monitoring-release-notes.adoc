...
[id="power-monitoring-release-notes-0-4"]
...
[id="power-monitoring-0-4-features"]
=== Features

* With this release, FIPS mode is now enabled for {PM-title}. When installed on an {product-title} cluster in FIPS mode, {PM-operator} ensures compatibility without affecting the cluster’s FIPS support status.

[id="power-monitoring-release-notes-0-4-CVEs"]
=== CVEs
