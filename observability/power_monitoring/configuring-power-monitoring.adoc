:_mod-docs-content-type: ASSEMBLY
[id="configuring-power-monitoring"]
= Configuring {PM-shortname}
include::_attributes/common-attributes.adoc[]
:context: configuring-power-monitoring

toc::[]

:FeatureName: Power monitoring
include::snippets/technology-preview.adoc[leveloffset=+2]

The `PowerMonitor` resource is a Kubernetes custom resource definition (CRD) that enables you to configure the deployment and monitor the status of the `PowerMonitor` resource.

include::modules/power-monitoring-kepler-configuration.adoc[leveloffset=+1]

include::modules/power-monitoring-monitoring-kepler-status.adoc[leveloffset=+1]

//include::modules/power-monitoring-configuring-kepler-redfish.adoc[leveloffset=+1]

//07/07/2025: Per comment on PR: Redfish content is being removed.
//Configure Kepler with RedFish is no longer needed for TP 0.5. However, it is for 0.4 and earlier, which are supported on OCP 4.17 and 4.18. Since Power Monitoring docs are not stand alone and thus not independently versioned, kepler-redfish content will need to remain, with a note to clarify it is for Power Monitoring Operator 0.4 and earlier.