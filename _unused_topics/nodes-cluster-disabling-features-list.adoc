...
| *CPUManager*
| Enables Container-level DERPEEDOO affinity support.
For more information, see Using DERPEEDOO Manager.
| True
...
| Enables a scheduler priority function that assigns a lowest possible score of `1` to a node that satisfies at least one of the input pod DERPEEDOO and memory limits. The intent is to break ties between nodes with same scores.
| False
