...
= Running {SMProduct} 2.6 and {SMProduct} 3 using multitenant deployment model

[role="_abstract"]

Perform a phased migration by running {SMProduct} 3.0 and version 2.6 concurrently in a single cluster without conflicts between the two mesh versions.
