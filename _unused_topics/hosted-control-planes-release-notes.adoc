...
Cluster administrators can add or update {aws-first} tags for several different types of resources. For more information, see xref:../hosted_control_planes/hcp-manage/hcp-manage-aws.adoc#hcp-aws-tags_hcp-manage-aws[Adding or updating {aws-short} tags for a hosted cluster].

[id="hcp-4-19-auto-dr-oadp_{context}"]
==== Automated disaster recovery for a hosted cluster by using OADP

On bare metal or {aws-first} platforms, you can automate disaster recovery for a hosted cluster by using OADP. For more information, see xref:../hosted_control_planes/hcp_high_availability/hcp-disaster-recovery-oadp-auto.adoc#hcp-disaster-recovery-oadp-auto[Automated disaster recovery for a hosted cluster by using OADP].
