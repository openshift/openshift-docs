...
def scrub_file(info, book_src_dir, src_file, tag=None, cwd=None):
    ...
    if base_src_file in TITLE_IDS:
        title_ids = TITLE_IDS[base_src_file]
    else:
        title_ids = {}
        ...
        if include_line(line):
            if not header_found and line.strip() != "" and line.startswith("="):
                ...
                if info['all_in_one'] and base_src_file in ALL_IN_ONE_SCRAP_TITLE and line.startswith("= "):
                    ...
                    # Add the section id, so we have something to link to
                    if src_file in info['file_to_id_map']:
                        file_id = info['file_to_id_map'][src_file]
                        content += "[[" + file_id + "]]\n"
            # Add the title id, if one is needed
            ...
            # Add the line if we've found the header or it's not a blank line
            if header_found or line.strip() != "":
                content += line
