...
A `CustomResource` called your `MeteringConfig` specifies all the configuration details for your metering installation. When you first install the metering stack, a default `MeteringConfig` is generated. Use the examples in the documentaion to modify this default file. Keep in mind the following key points:
