...
The NFS mount path is not unique when there is more than one volume while restoring by using `Restic` or `Kopia`. Consequently, the `velero` lock files use the same file on the NFS server during the restore.
Due to this, `PodVolumeRestore` either fails with the following error or keeps trying to restore before it fails:
...
include::modules/oadp-operator-fails-silently.adoc[leveloffset=+1]
include::modules/premature-termination-issues.adoc[leveloffset=+1]
