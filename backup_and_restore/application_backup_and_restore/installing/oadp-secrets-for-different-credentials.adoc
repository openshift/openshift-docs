...
[id="oadp-secrets-for-different-credentials"]
= Creating profiles for different credentials
:context: oadp-different-credentials

toc::[]

// need to come back to this section and rework
...
----
$ oc create secret generic {credentials} -n openshift-adp
----
...
[id="different-credentials-gcp-mcg-ocs"]
== Creating secrets for different credentials for GCP, MCG, and OCS

For Google Cloud Platform (GCP), OpenShift Data Foundation (OCS), and Multicloud Object Gateway (MCG), if your backup and snapshot locations use different credentials, you must create two `Secret` objects:
...
[id="different-credentials-azure-dpa"]
=== Azure DPA
...
[id="different-credentials-gcp-dpa"]
=== GCP DPA
...
[id="different-credentials-mcg-dpa"]
=== MCG DPA
...
[id="different-credentials-ocs-dpa"]
=== OCS DPA
+
[source,yaml,subs="attributes+"]
----
