...
`enable`:: The flag to enable the node agent.
`uploaderType`:: The type of uploader. The possible values are `restic` or `kopia`. The built-in Data Mover uses Kopia as the default uploader mechanism regardless of the value of the `uploaderType` field.
`csi`:: The CSI plugin included in the list of default plugins.
`defaultVolumesToFSBackup`:: In {oadp-short} 1.3.1 and later, set to `true` if you use Data Mover only for volumes that opt out of `fs-backup`. Set to `false` if you use Data Mover by default for volumes.
