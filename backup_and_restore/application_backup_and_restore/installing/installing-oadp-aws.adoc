...
[role="_abstract"]
When you need to back up your {product-title} applications, you want to store your data in a secure, reliable location. To accomplish this, you can configure {oadp-short} to use Amazon Web Services (AWS) S3 compatible storage.

This documentation guides you through the process of setting up {oadp-short} to use AWS S3 storage, ensuring you can successfully back up and restore your applications as needed. You will accomplish this by first installing the {oadp-short} Operator, then configuring AWS for Velero, creating a default Secret, and finally installing the Data Protection Application.

You install the {oadp-short} with Amazon Web Services (AWS) S3 compatible storage by installing the {oadp-short} Operator. The Operator installs link:https://{velero-domain}/docs/v{velero-version}/[Velero {velero-version}].

include::snippets/oadp-mtc-operator.adoc[]

[id="oadp-configure-aws-storage_{context}"]
== Configure AWS S3 compatible storage
When you need to back up your {product-title} applications, you want to store your data in a secure, reliable location. To accomplish this, you can configure {oadp-short} to use Amazon Web Services (AWS) S3 compatible storage.
...
* To install the {oadp-short} Operator in a restricted network environment, you must first disable the default OperatorHub sources and mirror the Operator catalog. See xref:../../../disconnected/using-olm.adoc#olm-restricted-networks[Using Operator Lifecycle Manager in disconnected environments] for details.
