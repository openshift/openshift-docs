...
You configure AWS for Velero, create a default `Secret`, and then install the Data Protection Application. For more details, see xref:../../oadp/installing/oadp-installing-operator.adoc#oadp-installing-operator-doc[Installing the OADP Operator].
...
You can also download the file with the additional encryption key backed up with Velcro by running a different command. See xref:../../oadp/installing/installing-oadp-aws.adoc#oadp-ssec-encrypted-backups-velero_installing-oadp-aws[Downloading a file with an SSE-C encryption key for files backed up by Velero].
...
For more details, see xref:../../oadp/installing/installing-oadp-aws.adoc#oadp-configuring-node-agents_installing-oadp-aws[Configuring node agents and node labels].
