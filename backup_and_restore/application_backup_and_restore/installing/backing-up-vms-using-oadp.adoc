...
* Back up virtual machines by creating a Backup custom resource (CR). For more details, see xref:../../../backup_and_restore/application_backup_and_restore/installing/backing-up-vms-using-oadp.adoc#oadp-creating-backup-cr_oadp-backing-up-vms[Creating a `Backup` custom resource (CR)].
