...
. Create a DPA CR similar to the following example. The default plugins include `CSI` and `VSM` as defaultPlugins, and also the `dataMover.enable` flag. Add the restic secret name from the previous step to your DPA CR in `spec.features.dataMover.credentialName`. If this step is not completed, then it will default to the secret name `dm-credential`.
+
...
<2> Use the Restic Secret that you created when you prepared your environment for working with OADP 1.2 Data Mover. If you do not, then the CR will use the default value `dm-credential` for this parameter.
<3> OADP 1.2 only.
+
