...
[id="backingup-three-scale-oadp-intro"]
== Backing up and restoring 3scale

This document should describes how a Cluster Admin (kubeadmin) could use {oadp-first} Operator could by a Cluster Admin (kubeadmin) be used to backup and restore 3scale with on cluster storage. 

The backup scenario covers a situation where no custom domains are used, and we need to recover to a cluster with a different domain than the source cluster.

The flow of the backup and restore is divided into three phases:
...
* Operator backup and restore:
    ...
    * Restoring secrets and APIM

* 3scale data backup and restoration:

    * Backing up data using write locks: 

        * Backing up system database (MySQL)
        * Restoring system database
        * Backing up backend Redis
        * Restoring backend redis

    * Backing up data using database tooling:
...
* There are two approaches for backing up the system database, recommended by {oadp-short}: the service-affecting approach and the non-service-affecting approach.
+
[NOTE]
====
This would need to be approved by the {oadp-short} team because of the potential impact of using this approach.
====
...
* Installation and configuration of {oadp-short} follow the setup for {oadp-short} with AWS as the S3 provider. The important bit is to enable Restic (when creating the DataProtectionApplication CR), since it is what we are going to use to perform the backup and restoration of the PVs. These steps need to be performed for **both** the source cluster and the destination cluster.
