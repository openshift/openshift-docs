...
<5> The `ttl` field defines the retention time of the created backup and the backed up data. For example, if you are using Restic as the backup tool, the backed up data items and data contents of the persistent volumes (PVs) is stored until the backup expires. But storing this data consumes more space in the target backup locations. An additional storage will be consumed with frequent backups, which are created even before other in-progress backups might have timed out.
