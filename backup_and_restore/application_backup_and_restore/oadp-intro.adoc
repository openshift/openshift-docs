...
= OpenShift API for Data Protection (OADP): Backing up and Restoring Your Applications
:context: oadp-api
...
[role="_abstract"]
// adding JTBD
As a cluster administrator, your job is to secure your applications and data by performing backups and restores. This guide provides the conceptual and procedural content you need to plan your data protection strategy and ensure application resilience and data integrity in your {product-title} environment.

The {oadp-full} product safeguards customer applications on {product-title}. It offers comprehensive disaster recovery protection, covering {product-title} applications, application-related cluster resources, persistent volumes, and internal images. {oadp-short} is also capable of backing up both containerized applications and virtual machines (VMs).

{oadp-short} is not a full cluster backup solution; its explicit purpose is application-level data protection. It allows you to protect Kubernetes resources, internal images, and persistent volumes. It is crucial to understand that OADP is built on the open-source Velero project, and there are versioning constraints between OADP, Velero, and OpenShift Container Platform (OCP).

[id="oadp-how-it-works_{context}"]
== How it works

To successfully complete the job of protecting your application data, you will perform the following key tasks:
