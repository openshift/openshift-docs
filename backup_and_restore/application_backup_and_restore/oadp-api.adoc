...
Default and custom plugins are described in xref:../oadp/oadp-features-plugins#oadp-features-plugins[OADP plugins]
...
For more details, see xref:../oadp/oadp-api.adoc#oadp-configuring-node-agents_oadp-api[Configuring node agents and node labels].
...
|Set specific resource `limits` and `requests` for a `Velero` pod or a `Restic` pod as described in xref:../oadp/installing/installing-oadp-aws.adoc#oadp-setting-resource-limits-and-requests_installing-oadp-aws[Setting Velero CPU and memory resource allocations].
