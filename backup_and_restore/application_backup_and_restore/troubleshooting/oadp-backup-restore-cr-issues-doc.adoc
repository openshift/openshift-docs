...
[id="oadp-backup-restore-cr-issues-doc"]
...
[id="backup-cannot-retrieve-volume-doc"]
...
The persistent volume (PV) and the VolumeSnapshotLocation (VSL) are in different regions.
...
[id="backup-cr-remains-in-progress-doc"]
...
[source,terminal]
----
$ oc delete backup -n openshift-adp ==> oc delete backup -n {namespace}
----
+
...
[id="backup-cr-remains-partiallyfailed-doc"]
...
If the backup is created based on the CSI VolumeSnapshotClass CR, but the label is missing, CSI snapshot plugin fails to create a snapshot. As a result, the `Velero` pod logs an error similar to the following:
+
[source,text]
----
