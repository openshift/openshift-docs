...
You can configure automatic backups for the Central database by integrating with Amazon S3, {rh-storage-first}, S3 API compatible services, or Google Cloud Storage.
...
* xref:../integration/integrate-with-odf-s3.adoc#perform-on-demand-backups-odf-s3_integrate-with-odf-s3[Perform on-demand backups with {rh-storage-first}]
