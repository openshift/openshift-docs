...
{rh-storage-first} 3, which is available for use with {product-title} 3.11, uses Red Hat Gluster Storage as the backing storage.

{rh-storage-first} 4, which is available for use with {product-title} 4, uses Red Hat Ceph Storage as the backing storage.

For more information, see xref:../../storage/persistent_storage/persistent-storage-ocs.adoc#red-hat-openshift-container-storage[Persistent storage using {rh-storage-first}] and the link:https://access.redhat.com/articles/4731161[interoperability matrix] article.
