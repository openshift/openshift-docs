...
* Installing the CAM Operator xref:installing-migration-operator-manually_{context}[manually on the source cluster]

:targetcluster:
* Installing the CAM Operator xref:installing-migration-operator-with-olm_{context}[with OLM on the target cluster]
:targetcluster!:

* Configuring xref:migration-configuring-cors-3_{context}[cross-origin resource sharing on the source cluster] to enable communication with the CAM tool
