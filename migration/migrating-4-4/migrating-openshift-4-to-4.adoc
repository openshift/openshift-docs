...
:sourcecluster:
* Installing the CAM Operator xref:installing-migration-operator-with-olm_{context}[with OLM on the {product-title} 4.2 source cluster]
:sourcecluster!:

:targetcluster:
* Installing the CAM Operator xref:installing-migration-operator-with-olm_{context}[with OLM on the {product-title} 4.2 target cluster]
:targetcluster!:

* Configuring xref:migration-configuring-cors-4_{context}[cross-origin resource sharing on the source cluster] to enable communication with the CAM tool
