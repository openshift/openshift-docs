...
=== cluster
Start or stop a {product-title} cluster:
----
$ oc cluster [--options]
----
...
=== rollout
Manage a Kubernetes deployment or an OpenShift deployment configuration. Start a
...
=== import
Commands that import applications into {product-title}.
----
$ oc import [--options]
----
...
The debug pod is deleted when the the remote command completes or you interrupt
the shell.
