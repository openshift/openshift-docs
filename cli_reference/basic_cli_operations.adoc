...
----
$ osc <action> <object_type> <object_name_or_id>
----
...
|`scale`
|`osc scale _<object_type>_ _<object_id>_ --replicas=_<#_of_replicas>_`
...
----
$ osc new-project <project_name> --display-name=<display_name>
...
Management Console as "OpenShift 3 Sample". `test-admin` is the project
administrator and, when launched onto nodes, pods receive the matching label of
`environment : test`:

====
----
$ osc new-project test --display-name="OpenShift 3 Sample" --description="This
...
----
$ osc rollback <deployment> [<options>]
----
