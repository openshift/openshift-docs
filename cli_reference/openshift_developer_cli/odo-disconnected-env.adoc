[id='odo-disconnected-cluster']
= Running odo in cluster disconnected from the Internet
...
== Configuring the client OS (`odo` machine)
...
. Copy and encode the cluster host's `root` CA certificate.
//CA certificate from the root user of the OpenShift host machine
+
----
$ cat <CA_certificate_content> | base64 -d > disconnect-ca.crt
----

. Copy and trust the encoded certificate on the machine where you want to install `odo`.
...
[NOTE]
====
CA certificate target paths may vary between operating systems.
====
