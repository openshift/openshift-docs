...
. Overwrite init image using environment variable ODO_BOOTSTRAPPER_IMAGE
...
To deploy a Node.js component using odo in a restricted environment follow these steps:
...
. Check `nodejs` image stream into OpenShift namespace
...
. Create a Node.js git component in `odo`
...
[NOTE]
====
A mirror of `npm` registry should be created and configured within the cluster, so that the npm packages will be sourced form the cluster's registry. To overwrite the mirror registry you can use odo config set --env NPM_MIRROR=<npm_mirror_registry> or odo component create nodejs --env NPM_MIRROR=<npm_mirror_registry>.
====

. Push the nodejs component
