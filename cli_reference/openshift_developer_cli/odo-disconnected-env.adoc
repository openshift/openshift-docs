...
* `odo` installed 
// link?
* A mirror registry created for the disconnected cluster
* Certificate Authority (CA) certificate from the OpenShift cluster's host machine (`root` user)
//link?
...
== Configuring the `odo` machine (machine that has `odo`)
...
[NOTE]
====
Complete these steps on the client machine (where you want )
====

. Encode the mirror registry's certificate on the machine with `odo` 
//CA certificate from the root user of the OpenShift host machine
+
----
$ cat <certificate> | base64 -d > disconnect-ca.crt
----
