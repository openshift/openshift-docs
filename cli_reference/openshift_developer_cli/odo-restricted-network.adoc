...
[NOTE]
====
A mirror Maven registry has to be created and configured in the cluster. Override the default mirror Maven registry with `odo config set --env MAVEN_MIRROR_URL=<mvn_mirror_registry>` or `odo component create java --env MAVEN_MIRROR_URL=<mvn_mirror_registry>`.
====
////
...
.Prerequisites
- Configured cluster as per (link)
...
== Deploying a Node.js app on cluster in restricted network
