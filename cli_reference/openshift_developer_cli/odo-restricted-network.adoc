////
Yana:
- find out if all images are mirrored as per https://docs.openshift.com/container-platform/4.3/installing/install_config/installing-restricted-networks-preparations.html (don't ask why it is under "FIPS", I don't know.) If yes, we're good-> all images are accessible and we basically don't need "Configuring the cluster in the restricted network" except:
- nodejs -> in cluster, we need to mirror the npm repo
...
- java -> in cluster, we need to mirror the maven repo


Use cases:
  - this config has to be done when provisioning the cluster. Question is, whether we want to enable user's to do it to an existing cluster in restricted env.
  - windows/macos commands are missing
  - do we want the examples how to deploy the apps? (seems to be a copy of what is in https://docs.openshift.com/container-platform/4.3/cli_reference/openshift_developer_cli/creating-a-single-component-application-with-odo.html). Look at the procedures in this doc, though, there are some extra info that can be implemented into the "creating-a-single-component-application-with-odo" (e.g. what --git does)
Notes:
  - @kalexand The bastion is connected to the internet and the restricted network. And you can destroy it after the provisioning/turn it into a registry.
////
////
== Configuring the cluster in the restricted network

=== Mirroring the `odo-init-image` into the cluster's local registry from the Red Hat registry
//on the machine with cluster

On the cluster's host OS:
. Log in to the mirror registry with Docker.
+
----
$ docker login <mirror-registry>:<port> -u <mirror_registry_username> -p <mirror_registry_password>
----
// host has to be conected to the internet
// not an issue if "setting up the cluster for usage in restricted network" requires an internet connection as well
. Mirror the `odo-init-image` image into the local mirror registry.
+
----
...
// can you connect the (openshift) host machines to the internet? (not the cluster itself)
// tkadel nope, that defeats the whole purpose of the disconnected cluster.

=== Configuring the mirror registry to contain a `java` builder image


. Check that the `java` builder image is available in the cluster's mirror registry. This example did not find the `java` builder image.
...
=== Importing a `nodejs` builder image into the cluster
...
[id='odo-restricted-network']
= Using `odo` with cluster in restricted network
include::modules/openshift-developer-cli-attributes.adoc[]
include::modules/common-attributes.adoc[]
:context: odo-restricted-network
toc::[]
...
.Prerequisites
* An {product-title} cluster in a restricted network
* `oc` installed on the client OS
// link?
* `odo` installed on the client OS
// link?
* A mirror registry created for the disconnected cluster on the cluster's host OS
* Certificate Authority (CA) certificate from the OpenShift cluster's host OS (`root` user)
//link?
...
// How to setup a mirror registry? = is in ocp docs
.Prerequisites
- Configured cluster in a restricted environment
- CA certificate of the `root` user of the cluster's host OS
// ^^do the image mirroring first so you can set `ODO_BOOTSTRAPPER_IMAGE`

.Procedure
. Encode the cluster host's `root` CA certificate.
//CA certificate from the root user of the OpenShift host machine
+
----
$ echo <CA_certificate_content> | base64 -d > disconnect-ca.crt
----

. Copy the encoded `root` CA certificate to the following folder.
...
[IMPORTANT]
====
CA certificate folders vary between operating systems.
====
// is the windows/macos commands the same?
// TODO: paths for macos/windows

//not documented anywhere unfort vvvvvvv
. Enable the dynamic CA configuration in Linux.
+
----
...
////
. Override the default `odo-init-image` image path by setting the environment variable `ODO_BOOTSTRAPPER_IMAGE`.
+
----
...
== EXAMPLE Deploying a Node.js app on cluster in restricted network

.Prerequisites
* Cluster and client OS configured as 

=== Creating a Node.js component:
...
.Prerequisites
* Cluster and client OS configured
