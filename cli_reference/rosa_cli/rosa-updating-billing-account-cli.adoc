...
= Manage billing accounts for {product-title} clusters
:context: rosa-updating-account-cli

toc::[]

[role="_abstract"]
You can use the {rosa-cli-first} to point an existing cluster at a different AWS billing account after deployment. You can correct a billing account linked at install time or change the account at a later date.
...
[id="additional-resources_billing-account-cli"]
