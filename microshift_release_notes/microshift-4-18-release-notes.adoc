...
=== Updating
Updating two minor versions in a single step is supported in 4.18. Updates for both single-version minor releases and patch releases are also supported.
...
With this release, make configuring your {microshift-short} instances even easier by using drop-in configuration snippets. See
//See xref:../microshift_configuring/microshift-using-config-yaml.adoc#microshift-config-snippets_microshift-configuring[Using configuration snippets]
