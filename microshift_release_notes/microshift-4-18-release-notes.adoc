...
* A failed greenboot health check flag does not clear when an RPM-install-based {microshift-short} service is updated without rebooting the system. As a consequence, greenboot health checks for optional components continue to fail. With this release, the primary {microshift-short} greenboot health check clears the condition that would cause optional components health checks to fail.(link:https://issues.redhat.com/browse/OCPBUGS-51198[*OCPBUGS-51198*])
...
* Previously, a failed greenboot health check flag did not clear when an RPM-install-based {microshift-short} service was updated without rebooting the system. As a consequence, greenboot health checks for optional components continued to fail. With this release, the primary {microshift-short} greenboot health check clears the condition that would cause optional components health checks to fail.(link:https://issues.redhat.com/browse/OCPBUGS-51198[*OCPBUGS-51198*])
