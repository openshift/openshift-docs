...
You can now deploy and configure your {microshift-short} cluster to run with low-latency rates. Using low latency configurations and tuning, combined with workload partitioning can result in reduced application response times and performance latency.
//See (crossref here)
