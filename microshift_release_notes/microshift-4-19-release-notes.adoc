...
==== Collecting metrics using {microshift-short} Observability service

 With this release, you can configure {microshift-short} Observability to collect performance and usage metrics for monitoring. The amount of collected data depends on one of three predefined configurations that differ in the amount of collected data. The configuration defaults to the maximum amount of data collected. For more information, see xref:../microshift_running_apps/microshift-observability-service.adoc#microshift-observability-service-checks[{microshift-short} Observability service].
