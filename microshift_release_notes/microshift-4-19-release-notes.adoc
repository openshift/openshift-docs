...
With this release, {microshift-short} adds the Telemeter API. Lightweight attributes can be sent from a connected cluster to Red{nbsp}Hat to monitor the health of the cluster. To opt-out of Telemetry, see
//xref:../microshift_support/microshift-remote-cluster-monitoring.adpc#microshift-remote-cluster-monitoring[Remote health monitoring with a connected cluster].
