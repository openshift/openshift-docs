...
With this release, prerequisites and instructions for installing a RHEL image mode for {microshift-short} are now included. For more information, see xref:../microshift_install_bootc/microshift-install-bootc-image.adoc#microshift-install-bootc-image[Installing a bootc image and publishing to a registry].
