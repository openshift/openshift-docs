...
ifdef::openshift-aro[]
The cluster runs on xref:azure/resources.adoc#architecture-azure-resources[Microsoft Azure cloud resources].
It is managed by {product-title} site reliability engineers who, among other things, follow
xref:azure/processes.adoc#architecture-azure-processes[cluster maintenance processes].
endif::[]
