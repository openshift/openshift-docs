...
link:kubernetes_infrastructure.html#master[master]. All static assets required to run the web
...
configuration file parameter `masterPublicURL`]. The web console uses WebSockets to maintain a
persistent connection with the API server and receive updated information as
...
== Extended Console for Integration Components

For pods based on Java images, the web console also exposes access to an
extended, http://hawt.io/[hawt.io]-based console for viewing and managing any
relevant integration components. A *Connect* link is displayed in the pod's
details on the _Browse -> Pods_ page, provided the container has a port named
*jolokia*.

====
.Pod with a Link to the Extended Console
image::extended_connect.png["Pod with a Console Connection"]
====

After connecting to the extended console, different pages are displayed
depending on which components are relevant to the connected pod.

====
.Extended Console for Integration Components
image::extended_threads.png["Java Components Console"]
====
