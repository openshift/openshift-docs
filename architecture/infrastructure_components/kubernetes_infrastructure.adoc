...
xref:high-availability-masters[highly-available masters].
...
method] provides specific examples using xref:master-components[Pacemaker] as
...
HA for the API Server load balancer. One HA option is to run multiple HAProxy
load balancers and use Keepalived to provide a floating virtual IP address for
them.
====
...
xref:../../admin_guide/master_node_configuration.adoc#admin-guide-master-node-configuration[dedicated node
configuration files].
