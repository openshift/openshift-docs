...
ifdef::openshift-aro[]
On the master nodes *etcd*, *openvswitch* and *openshift-sdn* are run as static pods.
endif::[]
...
ifdef::openshift-aro[]
{product-title} creates two types of nodes *infrastructure nodes* and
*compute nodes*. *Infrastructure nodes* are used exclusively to run pods needed
for cluster management while *compute nodes* run customers' applications and
builds.
endif::[]
