...
There is one VIP, and it is managed by Pacemaker when using the `pacemaker` HA
method or another load balancer such as HAProxy when using the `native` method.

|HAProxy
a|Optional, used when configuring
link:#high-availability-masters[highly-available masters] with the `native`
method. Balances load between API master endpoints and manages the VIP.
