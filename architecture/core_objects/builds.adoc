...
than having to download them each time the build is run.
...
system to root privilege escalation by a malicious user because the entire
docker build process is run as a user with docker privileges. STI restricts the
operations performed as a root user, and can run the scripts as an individual
user.
...
[#custom-build]
== Custom Build
Custom builds allow developers to define the builder image that is responsible
for the entire process of the build. The custom builder image is a plain Docker
image were the author embeds the logic of the desired build process, such as
...
builder image from the private Docker Registry that requires authentication or
Docker Hub, supply the `.dockercfg` file with valid Docker
...
resource, which you can use to store your configuration and passwords. You must
...
To create the `*Secret*` from a *_secret.json_* file:
...
To build your application from a private repository you must supply valid
credentials. Currently only SSH key based authentication is supported. The keys
used to access that repository are in the `$HOME/.ssh/` directory, and are
usually named `id_dsa.pub`, `id_ecdsa.pub`, `id_ed25519.pub` or `id_rsa.pub`.
You can generate SSH key credentials with the following command:
...
Kubernetes provides the
https://github.com/GoogleCloudPlatform/kubernetes/blob/master/docs/design/secrets.md[Secret]
resource, which you can use to store your keys. You must create the `*Secret*`
first before builds can use your ssh key for accessing your private repository.
The `*data*` field for the `*Secret*` object must contain your private key with
the value set to the base64-encoded content of that file. For example:
...
Copy the value returned from the above command and paste it into the
...
You can then create the `*Secret*` from the *_secret.json_* file using the
following command:
...
Once you have created the `*Secret*`, you can add a `SourceSecret` field into
the `Source` section inside the `BuildConfig` and set it to the name of the
