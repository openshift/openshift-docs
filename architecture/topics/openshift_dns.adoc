...
For this reason, {product-title} has a built-in DNS so that the services can be reached by the service DNS as well as the service IP/port. {product-title} supports split DNS by running link:https://github.com/skynetservices/skydns[SkyDNS] on the master and on the nodes that answers DNS queries for services. SkyDNS listens on both port `53` and port `8053` on the masters and on port `8053` on the nodes.
