...
== Deployment
A deployment is a specially annotated replication controller, specifying the desired configuration of that controller. See the https://github.com/openshift/origin/blob/master/docs/deployments.md[deployment documentation] for more information.

== DeploymentConfig
A DeploymentConfig specifies an existing deployment, the triggers that can result in replacing the deployment with a new one, the strategy for doing so, and the history of such changes. See the https://github.com/openshift/origin/blob/master/docs/deployments.md[deployment documentation] for more information.
