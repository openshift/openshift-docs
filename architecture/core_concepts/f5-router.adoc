...
F5 iControl REST API. The F5 router supports xref:router.adoc#route-types[unsecured],
xref:router.adoc#edge-termination[edge terminated],
xref:router.adoc#re-encryption-termination[re-encryption terminated], and
xref:router.adoc#passthrough-termination[passthrough terminated] routes matching on HTTP
vhost and request path.

The F5 router has feature parity with the
xref:router.adoc#haproxy-template-router[HAProxy template router],
...
xref:../../admin_guide/manage_noder.adoc#marking-nodes-as-unschedulable-or-schedulable[unschedulable]
