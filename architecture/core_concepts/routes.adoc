...
ifdef::openshift-enterprise,openshift-origin[]
See
xref:../../admin_guide/high_availability.adoc#admin-guide-high-availability[High Availibility]
...
A better user experience is achieved by routing all traffic from a user's session
to the same pod. While satisfying the user's requests, the pod caches data which can be used
in subsequent requests. The session becomes sticky, in that requests go to the same pod.
For example, for a cluster with five back-end pods, and two load-balanced routers,
...
While routing returning traffic to the same pod is desired it cannot be guaranteed.
You can use HTTP headers to set a cookie determining the pod used by the last connection.
When the user sends another request to the application the browser re-sends the cookie
and the router knows where to send the traffic.
...
By default, sticky sessions for pass through routes are implemented using
the `source` xref:load-balancing[load balancing strategy]. The default can be
changed for all pass through routes by using the `*ROUTER_TCP_BALANCE_SCHEME*`
xref:env-variables[environment variable]. It can be changed for individual routes
by using the `*haproxy.router.openshift.io/balance*` xref:route-specific-annotations[route specific annotation].

Other types of routes use the `leastconn` xref:load-balancing[load balancing strategy] by
default.  The default can be changed by using the `*ROUTER_LOAD_BALANCE_ALGORITHM*`
xref:env-variables[environment variable]. It can be changed for individual routes
by using the `*haproxy.router.openshift.io/balance*` xref:route-specific-annotations[route specific annotation].
...
variables in the *deployment config* for the router to alter its configuration, or use the `oc set env` command:
...
among the endpoints based on the selected load-balancing strategy. This
applies when no xref:routes-sticky-sessions[persistence information] is available,
such as on the first request in a session.
...
expected, such as LDAP, SQL, TSE, etc... It is not intended to be used
