...
== Route Status
====
The route status field is only set by routers, the routers themselves do not clear it. If changes have been made to a route so that a router no longer 
serves a specific route the status becomes stale. The way to remove the stale entries in the route status is to use the clear-route-status script avalible
link:https://github.com/openshift/origin/blob/master/images/router/clear-route-status.sh[here].
====
