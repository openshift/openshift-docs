...
An {product-title} administrator can deploy _routers_ in an OpenShift cluster, which
enable routes xref:../../dev_guide/routes.adoc#creating-routes[created by
developers] to be used by external clients. The routing layer in OpenShift is
pluggable, and two available xref:available-router-plug-ins[router plug-ins]
...
xref:../../architecture/additional_concepts/sdn.adoc#architecture-additional-concepts-sdn[{product-title} SDN], a cluster
