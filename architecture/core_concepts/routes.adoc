...
|`*ROUTER_DEFAULT_CLIENT_TIMEOUT*`| 30s | Lenght of time within which a client has to acknowledge or send data.
|`*ROUTER_DEFAULT_CONNECT_TIMEOUT*`| 5s | The maximum connect time.
|`*ROUTER_DEFAULT_SERVER_TIMEOUT*`| 30s | Lenght of time within which a server has to acknowledge or send data.
|`*ROUTER_DEFAULT_TUNNEL_TIMEOUT*` | 1h | Time till which TCP or WebSocket connections will remain open.
|`*ROUTER_ENABLE_COMPRESSION*`| false | If `ture`, compress responses when possible.
...
|`*ROUTER_SLOWLORIS_TIMEOUT*` | 10s | Lenght of time the transmission of an HTTP request can take.
