...
* If your image stream tag points to a Docker image tag, you need to understand how that Docker image tag is updated. For example, a Docker image tag `docker.io/ruby:2.5` will probably always point to a v2.5 ruby image. But, a Docker image tag `docker.io/ruby:latest` will probably change with major versions. So, the Docker image tag that a image stream tag points to can tell you how stable the image stream tag will be, if you choose to reference it.
