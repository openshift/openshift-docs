...
[[crossnamespace-image-import]]

=== Importing Images from internal registry

An image stream can be configured to import tag and image metadata from internal
registry but from a different project. The recommended approach for this, is to use
link:#adding-tag[`*oc tag*`] command:

====
----
$ oc tag originalproject/imagestream:tag newimagestream:newtag
----
====

Other option, to import image from internal registry, from other project manually,
using full pull spec, is to add necessary link:../../dev_guide/service_accounts.html[policy]
to access the other project:

====
----
$ oc policy add-role-to-group system:image-puller system:serviceaccounts:newproject -n originalproject
----
====

This allows the *newproject* to pull images from *originalproject*. With the policy
in place, we can safely import the image:

====
----
$ oc import-image newimagestream --confirm --from=docker_registry/originalproject/imagestream
----
====

WARNING: The latter method is strongly discouraged and should be used only if
the former using `*oc tag*` isn't sufficient.
