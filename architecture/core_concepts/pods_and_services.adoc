...
maintain state when recreated. Therefore manage pods with higher-level xref:deployments.adoc#replication-controllers[controllers],
rather than directly by users.

The following example definition of a pod provides a long-running service, which is actually a part of the {product-title} infrastructure: the
...
<9> Pods making requests against the {product-title} API is a common enough pattern that there is a `*serviceAccount*` field for specifying which
...
link:https://github.com/kubernetes/kubernetes/blob/release-1.0/docs/user-guide/pods.md[Kubernetes pod documentation]
...
A Kubernetes `service` serves as an internal load balancer. It identifies a set
of replicated xref:pods[pods] in order to proxy the connections it receives to
them. Backing pods can be added to or removed from a service arbitrarily while
the service remains consistently available, enabling anything that depends on
the service to refer to it at a consistent internal address.
...
Like pods, services are REST objects. The following example shows the definition
of a service for the pod defined above:
...
The link:https://github.com/kubernetes/kubernetes/blob/release-1.0/docs/user-guide/services.md[Kubernetes service documentation] has more information about services.
...
Most objects can include labels in their metadata. Labels can
...
The link:https://github.com/kubernetes/kubernetes/blob/release-1.0/docs/user-guide/labels.md[Kubernetes
labels documentation] has more information about labels.
