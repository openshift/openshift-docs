...
xref:../../dev_guide/getting_traffic_into_cluster.adoc#using-externalIP[external] to cluster, can be assigned to the service. These externalIP addresses can also be virtual IP addresses that provide xref:../../admin_guide/high_availability.adoc#admin-guide-high-availability[highly available] access to the service.
...
xref:../admin_guide/tcp_ingress_external_ports.adoc#unique-external-ips-ingress-traffic-restrictions[*_master-config.yaml_*] file. When *_master-config.yaml_* is changed, the master service must be restarted.
...
The pool is configured in *_/etc/origin/master/master-config.yaml_* file. After changing this file, restart the master service.

The `ingressIPNetworkCIDR` is set to `172.46.0.0/16` by default. Use the default range if the cluster environment is not already using this private range; or set a custom range.
