...
xref:deployments.adoc#replication-controllers[replication controller]
...
The default service clusterIP addresses are from the {product-title} internal network and they are used to permit pods to access each other. To permit external access to the service, additional externalIP and ingressIP addresses that are
xref:getting_traffic_into_cluster.adoc#using-externalIP[external] to cluster, can be assigned to the service. These externalIP addresses can also be virtual IP addresses that provide
...
that are
xref:getting_traffic_into_cluster.adoc#traffic-overview[external to the cluster].
...
In non-cloud clusters, externalIP addresses can be
xref:getting_traffic_into_cluster.adoc#traffic-overview[automatically assigned]
from a pool of addresses. This eliminates the need for the administrator manually assigning them.
...
.Sample xref:../../install_config/master_node_configuration.adoc#master-node-config-kubernetes-master-config[*servicesNodePortRange*] xref:../../install_config/master_node_configuration.adoc#[/etc/origin/master/master-config.yaml]
====
----
