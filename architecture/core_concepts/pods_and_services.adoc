...
{product-title} leverages the Kubernetes concept of a pod, which is one or more xref:containers_and_images.adoc#containers[containers] deployed
...
range configured in *_master-config.yaml_* file. When *_master-config.yaml_* is changed, the master service must be restarted.
endif::[]
