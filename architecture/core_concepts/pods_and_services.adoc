...
In addition to the cluster's internal IP addresses, the user can configure IP addresses that are
ifdef::openshift-enterprise,openshift-origin[]
xref:../../dev_guide/getting_traffic_into_cluster.adoc#getting-traffic-into-cluster[external to the cluster].
endif::[]
ifndef::openshift-enterprise,openshift-origin[]
external to the cluster.
endif::[]
The administrator is responsible for ensuring that traffic arrives at a node
with this IP.
