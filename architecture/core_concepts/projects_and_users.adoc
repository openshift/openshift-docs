...
ifdef::openshift-aro[]
{product-title} is a managed service and therefore the cluster administrator role is not available to customers.  The customer admin role is delegated to the customer for cluster managed operations.

The customer administrator can xref:../../dev_guide/projects.adoc#dev-guide-projects[create projects] and manage any
project created by the customer, they can also
xref:../../admin_guide/manage_rbac.adoc#managing-role-bindings[delegate administrative rights].

Developers can also create their own projects.
endif::openshift-aro[]
