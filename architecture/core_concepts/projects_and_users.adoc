...
ifdef::openshift-aro[]
{product-title} is a managed service. The cluster administrator role is not available to customers; it is delegated to the customer for cluster-managed operations.
