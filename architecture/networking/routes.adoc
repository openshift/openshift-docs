...
at a host name, such as _www.example.com_, so that external clients can reach it
by name.
...
the endpoints over the internal pod network are not encrypted.
...
is encrypted, even over the internal pod network. The router uses health
