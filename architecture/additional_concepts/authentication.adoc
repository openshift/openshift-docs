...
Having a specific directory allows for faster lookup times for a user. As a result, there must be enough information in the object's name to determine the user specific etcd key.
...
The name of the object is only used to determine the user when the cluster admin is doing a lookup.
User lookups use information from the context and enforce UID matching.

For example (note the mix of old and new objects), as a `system:admin` user:
...
Non-administrator users can only see current OAuth Client Authorization objects.

[NOTE]
====
Non-administrator users need to use the `selfoauthclientauthorization`flag and administrator users need to use the `oauthclientauthorization` flag.
====

[[authentication-manage-oauth-view]]
=== Viewing OAuth Client Authorization Files

As an administrator, you can view the contents of all OAuth Client Authorization object file.
...
It is not possible to change an exsiting . Once the API changes for this card are approved, I will write the instructions for cluster administrators to delete the legacy objects.
