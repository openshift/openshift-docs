...
The naming formant for OAuth Client Authorizations has also changed. The new format is `<userName>::<clientName>`, 
...
[NOTE]
====
Non-admin users need to use the `selfoauthclientauthorization`flag and admin ussers need to use the `oauthclientauthorization` flag.
====
...
As an admin, you can view the contents of all OAuth Client Authorization object file.
...
As a non-admin user, you can view the contents of your own OAuth Client Authorization object files.
