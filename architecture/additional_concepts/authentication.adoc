...
Starting in the current version of {product-title} the Oauth Client Authorizations use a different naming format. Because of this change,
previous authorizations are no longer considered valid. 

It is not possible to change an exsiting Oauth Client Authorizations. The administrator must delete and recreate these authorizations.
