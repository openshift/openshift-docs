...
ifdef::openshift-enterprise,openshift-origin[]
Uses the same values seen in xref:../../install_config/configuring_authentication.adoc#grant-options[Grant Options].
endif::openshift-enterprise,openshift-origin[]
ifdef::openshift-enterprise,openshift-origin[]
Uses the same values seen in Grant Options.
endif::openshift-enterprise,openshift-origin[]
...
<3> Token is returned in the `Location` header of the `302` response as
`access_token=VO4dAgNGLnX5MGYu_wXau8au2Rw0QAqnwq8AtrLkMfU`.
