...
[[authentication-manage-oauth]]
== Managing OAuth Objects

Each user has an individual directory where OAuth Client Authorizations for that user are stored:
*_/openshift.io/oauth/clientauthorizations/<userName>/<clientName>_*.

Having a specific directory allows for faster lookup times for a user. 
Previously, {product-title} maintained OAuth Client Authorizations in etcd.

The naming format for OAuth Client Authorizations has also changed. The new format is `<userName>::<clientName>`,
where the previous format is `<userName>:<clientName>`.

[NOTE]
====
Previous authorizations are no longer considered valid and need to be xref:authentication-manage-oauth-view-cleanup[deleted] and recreated.
====

You can access information about OAuth Client Authorizations using the {product-title} CLI. The information
you receive depends on whether you are an admin or non-administrator user. 

For example, as a `system:admin` user, listing the authorizations includes old and new objects:

----
oc get oauthclientauthorization

NAME                                                  USER NAME    CLIENT NAME                               SCOPES
developer::system:serviceaccount:myproject:jenkins    developer    system:serviceaccount:myproject:jenkins   user:info <1>
developer1::system:serviceaccount:myproject:jenkins   developer1   system:serviceaccount:myproject:jenkins   user:info
developer1:system:serviceaccount:myproject:jenkins    developer1   system:serviceaccount:myproject:jenkins   user:info <2>
developer:system:serviceaccount:myproject:jenkins     developer    system:serviceaccount:myproject:jenkins   user:info
----

<1> Current OAuth Client Authorization object
<2> Previous OAuth Client Authorization object

However, non-administrator users can only see current OAuth Client Authorization objects associated with their account.

----
oc get selfoauthclientauthorization

NAME                                      CLIENT NAME                               SCOPES
system:serviceaccount:myproject:jenkins   system:serviceaccount:myproject:jenkins   user:info
----

[NOTE]
====
Non-administrator users need to use the `selfoauthclientauthorization` flag. Administrator users need to use the `oauthclientauthorization` flag.
====

[[authentication-manage-oauth-view]]
=== Viewing OAuth Client Authorization Files

As an administrator, you can view the contents of all OAuth Client Authorization object files.

----
# oc get oauthclientauthorization <file-client-name> -o yaml
----

For example:

----
# oc get oauthclientauthorization system:serviceaccount:myproject:jenkins -o yaml
----

The output is similar to the following:

.Sample OAuth Client Authorization object file for an admin
----
apiVersion: v1
items:
- apiVersion: v1 <1>
  clientName: system:serviceaccount:myproject:jenkins
  kind: OAuthClientAuthorization
  metadata: <2>
    creationTimestamp: 2016-11-22T16:08:23Z
    name: developer::system:serviceaccount:myproject:jenkins
    resourceVersion: "636"
    selfLink: /oapi/v1/oauthclientauthorizations/developer::system:serviceaccount:myproject:jenkins
    uid: e5163dca-b0cd-11e6-b245-507b9dac97ff
  scopes: <3>
  - user:info
  userName: developer
  userUID: cf29b7cb-b0ca-11e6-ac48-507b9dac97ff
- apiVersion: v1
  clientName: system:serviceaccount:myproject:jenkins
  kind: OAuthClientAuthorization
  metadata:
    creationTimestamp: 2016-11-22T16:02:44Z
    name: developer1::system:serviceaccount:myproject:jenkins
    resourceVersion: "599"
    selfLink: /oapi/v1/oauthclientauthorizations/developer1::system:serviceaccount:myproject:jenkins
    uid: 1afc194d-b0cd-11e6-b245-507b9dac97ff
  scopes:
  - user:info
  userName: developer1
  userUID: fe3f3a4e-b0ca-11e6-ac48-507b9dac97ff
- apiVersion: v1
  clientName: system:serviceaccount:myproject:jenkins
  kind: OAuthClientAuthorization
  metadata:
    creationTimestamp: 2016-11-22T15:48:39Z
    name: developer1:system:serviceaccount:myproject:jenkins
    resourceVersion: "508"
    selfLink: /oapi/v1/oauthclientauthorizations/developer1:system:serviceaccount:myproject:jenkins
    uid: 235cbdff-b0cb-11e6-ac48-507b9dac97ff
  scopes:
  - user:info
  userName: developer1
  userUID: fe3f3a4e-b0ca-11e6-ac48-507b9dac97ff
- apiVersion: v1
  ...
  kind: OAuthClientAuthorization
  metadata:
    creationTimestamp: 2016-11-22T15:47:12Z
    name: developer:system:serviceaccount:myproject:jenkins
    resourceVersion: "494"
    selfLink: /oapi/v1/oauthclientauthorizations/developer:system:serviceaccount:myproject:jenkins
    uid: ef5c24f2-b0ca-11e6-ac48-507b9dac97ff
  scopes:
  - user:info
  userName: developer
  userUID: cf29b7cb-b0ca-11e6-ac48-507b9dac97ff
...
metadata: {}
----
<1> Each OAuth Client Authorization object is listed, starting with `apiVersion`.
<2> Metadata added to identify the object.
<3> The scope of the access request.

As a non-administrator user, you can view the contents of your own OAuth Client Authorization object files.

----
# oc get selfoauthclientauthorization <file-client-name> -o yaml
----

For example:

----
# oc get selfoauthclientauthorization system:serviceaccount:myproject:jenkins -o yaml
----

The output is similar to the following:

.Sample OAuth Client Authorization object file for a user
----
apiVersion: v1
clientName: system:serviceaccount:myproject:jenkins
kind: SelfOAuthClientAuthorization
metadata:
  creationTimestamp: 2016-11-22T16:02:44Z
  name: system:serviceaccount:myproject:jenkins
  resourceVersion: "599"
  selfLink: /oapi/v1/selfoauthclientauthorizations/system:serviceaccount:myproject:jenkins
  uid: 1afc194d-b0cd-11e6-b245-507b9dac97ff
scopes:
- user:info
userUID: fe3f3a4e-b0ca-11e6-ac48-507b9dac97ff
...
[[authentication-manage-oauth-cleanup]]
=== Cleanup Old Objects

It is not possible to change an exsiting Oauth Client Authorizations.

Use the following command to delete all invalid Oauth Client Authorizations:
...
Use the following command to delete specific invalid Oauth Client Authorizations:

----
oc delete oauthclientauthorization <file-client-name>
----
