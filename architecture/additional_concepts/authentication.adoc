...
Users can see these authorizations in the *_clientauthorizations_* directory. 

[NOTE]
====
In previous versions of {product-title} The Oauth Client Authorizations were maintained in etcd. These authorizations are no longer 
considered valid and need to be recreated. An administrator make these authorization valid by changing the naming scheme of new objects. 
The previous format is `<userName>:<clientName>`. The new format is `<userName>::<clientName>`. The new naming scheme makes it easy 
to distinguish where the object is stored. The name of the object is only used to determine the user 
when the cluster admin is doing a lookup. User lookups use information from the context and enforce UID matching.
====

For example (note the mix of old and new objects):

----
oc get oauthclientauthorization as system:admin
...
NAME                                       USER NAME  CLIENT NAME                            SCOPES
pm4:system:serviceaccount:test:testclient  pm4        system:serviceaccount:test:testclient  user:info
----	

As user

----	
oc get oauthclientauthorizaion
	
NAME                                  CLIENT NAME                            SCOPES
system:serviceaccount:test:testlient  system:serviceaccount:test:testclient  user:info
​​----
	
As a user:
	
----
oc describe oauthclientauthorization
----

	
Delete the oauthclientauthorization

----
oc delete oauthclientauthorization
----
​​
