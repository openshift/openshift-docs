...
<9> A PEM-encoded CA certificate that signs the server certificate used by the webhook server.
<10> Rules that define when the API server should use this controller.
<11> The operation(s) that triggers the API server to call this controller: 
* create
* update
* delete
* connect
<10> Specifies how the policy should proceed if the webhook admission server is unavailable. 
