...
You can configure a custom recycler pod template using the controller manager command line arguments. The custom recycler pod template must contain a `volumes` specification, as shown in the example below:
