...
[IMPORTANT]
====
 * PVs are provisioned either as azure-disk (default) or azure-file.
 * There are no guarantees regarding PVs, they can be wiped at any time during
 normal operation. It's recommended to use external services to store
 application state.
====
endif::[]
