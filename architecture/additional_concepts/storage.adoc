...
Both of these storage technologies rely on Azure storage infrastructure. The Azure Red Hat OpenShift service does not backup persistent volumes and therefore recommends using external services for storing sensitive data or application state.
====
endif::[]
