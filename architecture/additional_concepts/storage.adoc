...
iSCSI and Fibre Channel volumes don't have any fencing mechanisms yet. The user must 
ensure the volumes are only used by one node at a time. In certain situtations,
such as draining a node, the volumes may be used simultaniously by two nodes. 
Users should first ensure the Pods that use these volumes are deleted before draining the node.
====
