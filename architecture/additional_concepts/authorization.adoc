...
The usage of specific volume types can be controlled by setting the `*volumes*` field
of the SCC.  The allowable values of this field correspond to the volume sources that
are defined when creating a volume:
...
The recommended minimum set of allowed volumes for new SCCs are configMap, downwardAPI, emptyDir,
persistentVolumeClaim, and secret.
...
[NOTE]
====
For backwards compatibility, the usage of `*allowHostDirVolumePlugin*` will override settings
in the `*volumes*` field.  For example, if `*allowHostDirVolumePlugin*` is set to false but allowed
in the `*volumes*` field the `*hostPath*` value will be removed from `*volumes*`.
====
...
For more information on configuring seccomp please refer to the link:http://kubernetes.io/docs/admin/kubelet/[Kubelet
configuration documentation] as well as the
link:https://github.com/kubernetes/kubernetes/blob/release-1.4/docs/design/seccomp.md[Kubernetes seccomp design document]
