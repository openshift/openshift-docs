...
In addition to policy which controls what a user may do OpenShift provides security context
constraints which controls actions that a pod can perform and what it has the ability to access.

A security context constraint is an object that defines a set of conditions that a pod must
run with in order to be accepted into the system.  Security context constraints allow an administrator
...
In order to provide these capabilities a security context constraint allows you to configure
values to validate the pod.  Values in a security context constraint fall into three categories.


|===

|*Governed by a boolean* | Fields of this type will be defaulted to the most restrictive value.
For instance, `AllowPrivilegedContainer` will always be set to false if unspecified.

|*Governed by an allowable set* | Fields of this type will be checked against the set to ensure their value is allowed.

|*Governed by a strategy* | Items that have a strategy to generate a value will provide a
mechanism to generate the value as well as a mechanism to ensure that a specified value falls
into the set of allowable values.

|===

=== Admission

Admission control with security context constraints allows the ability to control the creation
of resources based on capabilities granted to a user. In terms of the security context
constraints it means that an admission controller may inspect the user info made available
in the context to retrieve an appropriate set of security context constraints to ensure the pod
is authorized to make requests about its operating environment or to generate a set of constraints
to apply to the pod.

The set of security context constraints that admission uses to authorize a pod are determined
by the user identity and groups that the user belongs to.  Additionally, if the pod specifies
a service account, the set of allowable security context constraints includes any constraints
accessible to the serivice account.

Admission ensures uses the following approach to create the final security context for the pod:

1.  Retrieve all security context constraints available for use
1.  Generate field values for any security context setting that was not specified on the request
1.  Validate the final settings against the available constraints

If a matching set of constraints is found then the pod will be accepted.  If the request cannot
be matched to a security context constraint then the pod will be rejected.
