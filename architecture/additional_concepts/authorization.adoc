...
<5> A list of capabilities that will be droped from a pod.
...
list of capabilities] that are allowed to each container of
a pod. The containers use the capabilities from this default list but pod
manifest authors can alter it by requesting additional capabilities or droping
some of defaulting. The `*allowedCapabilities*`, `*defaultAddCapabilities*`,
and `*requiredDropCapabilities*` fields are used to control such requests from
the pods and to dictate which capabilities can be requested, which ones must be
