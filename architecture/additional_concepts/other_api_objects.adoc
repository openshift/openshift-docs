...
See the link:../../dev_guide/quota.html[Developer's Guide] and
...
link:../../admin_guide/configuring_authentication.html#identity-providers[identity provider].
This determines the user's identity, and provides that information to OpenShift.

OpenShift then looks for a `*UserIdentityMapping*` for that `*Identity*`:

- If the `*Identity*` already exists, but is not mapped to a `*User*`, then login fails.
- If the `*Identity*` already exists, and is mapped to a `*User*`, then OpenShift gives the user an `*OAuthAccessToken*` for the mapped `*User*`.
- If the `*Identity*` does not exist, then OpenShift creates an `*Identity*`, `*User*`, and `*UserIdentityMapping*`. OpenShift also gives the user an
...
<3> `providerUserName` is the name that uniquely represents this identity in the
scope of the identity provider.
<4> The `name` in the `user` parameter is the name of the user that this
identity maps to.
...
link:../../admin_guide/manage_authorization_policy.html#managing-role-bindings[adding roles to users or to their groups].

User objects are created automatically on first login, or can be created via the
API.

Administrators can add group support for a user by listing groups in the user object. You could grant project access to a group, and manage membership to that group via user objects.
...
<3> The `fullName` value is an optional display name for the user.
...
An `*Identity*` can only map to a single `*User*`. Therefore, logging in as a
particular identity unambiguously determines the `*User*`.
