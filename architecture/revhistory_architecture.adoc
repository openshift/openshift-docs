...
// tag::architecture_mon_sep_25_2017[]
...
|Added information about the `AllowedFlexVolumes` field.

|xref:../architecture/networking/routes.adoc#architecture-core-concepts-routes[Networking -> Routes]
|Added the `DROP_SYN_DURING_RESTART`, `ROUTER_CLIENT_FIN_TIMEOUT`, and `ROUTER_DEFAULT_SERVER_FIN_TIMEOUT` environment variables to the table in xref:../architecture/networking/router_environment_variables.adoc#env-variables[Router Environment Variables].



|===

// end::architecture_mon_sep_25_2017[]
== Sun Sep 24 2017
