...
In OLM v1, a _plain bundle_ is a static collection of arbitrary Kubernetes manifests in the YAML format. The experimental `olm.bundle.mediatype` property of the `olm.bundle` schema object differentiates a plain bundle (`plain+v0`) from a regular (`registry+v1`) bundle.
...
include::modules/olmv1-adding-a-catalog-to-a-cluster.adoc[leveloffset=+1]
