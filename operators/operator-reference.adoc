...
`operator-lifecycle-manager-packageserver`:: Provides an API server responsible for collecting metadata from all catalogs on the cluster and serves the `PackageManifest` API.
