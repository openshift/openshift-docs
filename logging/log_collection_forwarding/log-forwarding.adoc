...
[id="log-forwarding"]
= Log collection and forwarding overview
:context: log-forwarding

toc::[]

By default, the {logging} sends container and infrastructure logs to the default internal log store defined in the `ClusterLogging` CR. However, it does not send audit logs to the internal store because it does not provide secure storage. If this default configuration meets your needs, you do not need to configure a log forwarder.

If a `ClusterLogForwarder` CR object exists, logs are not forwarded to the internal Elasticsearch instance unless there is a pipeline with the `default` output. To send audit logs to an Elasticsearch log store, configure the log forwarder as described in xref:../../logging/log_storage/logging-config-es-store.adoc#cluster-logging-elasticsearch-audit_logging-config-es-store[Forwarding audit logs to the log store].

[id="log-forwarding-about-forwarders"]
== About log forwarders
...
You can specify a combination of _outputs_ and _pipelines_ in a `ClusterLogForwarder` custom resource (CR) to forward application, infrastructure, and audit logs to specific endpoints within or outside of a cluster. You can also use _inputs_ to forward the application logs associated with a specific project to an endpoint.

Kubernetes secrets are used for authentication. Secrets are `key:value` maps that contains confidential data, such as user credentials. Transport Layer Security (TLS) is supported so that log forwarders can be configured to send logs securely. Administrators can also authorize RBAC permissions that define which service accounts and users can access and forward which types of logs.

include::modules/log-forwarding-example.adoc[leveloffset=+2]
...
include::modules/log-forwarding-pipelines.adoc[leveloffset=+2]

[id="log-forwarding-about-collectors"]
...
include::modules/about-log-collection.adoc[leveloffset=+2]
include::modules/logging-vector-fluentd-feature-comparison.adoc[leveloffset=+3]
include::modules/log-forwarding-collector-outputs.adoc[leveloffset=+3]
