...
* Before this update, the image included builder tools that are unnecessary at runtime. This change removes those tools. (link:https://issues.redhat.com/browse/LOG-1998[LOG-1998])

* Before this update, the Elasticsearch Prometheus exporter plugin was compiling index-level metrics using a high-cost query which had a negative performance impact on the processing Elasticsearch node. With this update, this query has been replaced with a low-impact query that will not impact performance. (link:https://issues.redhat.com/browse/LOG-1897[LOG-1897])

* Before this update, the Logging dashboard displayed an empty CPU graph because of a reference to an invalid metric. (link:https://issues.redhat.com/browse/LOG-1925[LOG-1925])

* Before this change, the OpenShift Logging Dashboard displayed the wrong pod namespace in the table which displays top producing and collected containers over the last 24 hours. (link:https://issues.redhat.com/browse/LOG-2051[LOG-2051])

* Before this change, any situation where Elasticsearch OutputDefault was defined for cluster logforwarding and Elasticsearch outputs did not have structured keys, the configuration was replacing the output secret with the default secret used to communicate to the default log store. (link:https://issues.redhat.com/browse/LOG-2046[LOG-2046])
...
* Before this update, a regression introduced in a prior release intentionally disabled JSON message parsing. With this update, a log entry's "level" value is set based on: a parsed JSON message that has a "level" field or by applying a regex against the message field to extract a match. (link:https://issues.redhat.com/browse/LOG-1759[LOG-1759])

* Before this update, few alerts were found to not include a namespace label and this was not compliance with the OpenShift Monitoring Team's guidelines for writing alerting rules in OpenShift. With this update, all the alerts in Elasticsearch Operator includes a namespace label and follows all the guidelines for writing alerting rules in OpenShift. (link:https://issues.redhat.com/browse/LOG-1857[LOG-1857])

*Before this update, the Elasticsearch Prometheus exporter plugin was compiling index-level metrics using a high-cost query which had a negative performance impact on the processing Elasticsearch node. With this update, this query has been replaced with a low-impact query that will not impact performance. (link:https://issues.redhat.com/browse/LOG-1970[LOG-1970])

* Before this update records shipped via syslog would serialize a ruby hash encoding key/value pairs to contain a '=>' character, as well as replace tabs with "#11". This change serializes the message correctly as proper JSON. (link:https://issues.redhat.com/browse/LOG-1775[LOG-1775])

* Before this update it was possible for Elasticsearch to reject messages because LogForwarding was configured with multiple outputs. Configuration for one of the outputs modified message content to be a single message. This update duplicates the message so output specific processing does not affect other outputs. (link:https://issues.redhat.com/browse/LOG-1824[LOG-1824])
