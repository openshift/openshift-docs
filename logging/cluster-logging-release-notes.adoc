...
* If you forward logs to an external Elasticsearch server and change a configured value in the pipeline secret, such as the username and password, you must restart the Fluentd Pods using `oc delete pod -l component=fluentd` for the new credentials to take effect.  (link:https://issues.redhat.com/browse/LOG-1652[LOG-1652])
