...
{product-title} clusters can perform logging tasks using the {es-op}.

The {logging} is configurable using a `ClusterLogging` custom resource (CR) deployed in the `openshift-logging` project namespace.

The {clo} watches for changes to `ClusterLogging` CR, creates any missing logging components, and adjusts the logging environment accordingly.
