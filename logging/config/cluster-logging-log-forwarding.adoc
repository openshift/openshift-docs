...
Log Forwarding provides an easier way to forward logs to specific endpoints inside or outside your {product-title} cluster than using the xref:../../logging/config/cluster-logging-external.adoc#cluster-logging-external[Fluentd protocols or syslog].
...
include::modules/cluster-logging-log-forwarding-disable.adoc[leveloffset=+1]
