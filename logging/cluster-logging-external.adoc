...
You can use the {clf-api} to send container, infrastructure, and audit logs to internal or external endpoints, including third party log aggregators.

You can configure the {clf-api} by creating and modifying `ClusterLogForwarder` custom resources (CRs). `ClusterLogForwarder` CRs can be used to specify different input and output types that determine where logs are forwarded from and where they are forwarded to. Logs can be forwarded to different outputs such as third party aggregators as well as, or instead of, the default internal log store. The internal log store is provided by the Loki Operator.
// check information about default internal log store, requirements, etc.

[NOTE]
====
In earlier implementations of {ocp-logging}, the default log store was provided by Elasticsearch. This functionality has since been deprecated. See the documentation "X(migrating your log store)".
====

You can also set RBAC permissions for `ClusterLogForwarder` CRs that determine which users have permissions to forward which types of logs, or enable Transport Layer Security (TLS) support to send logs securely.

// REQUIRED MODULES / sections
// creating CLF CRs + samples + reference to API docs
// Setting RBAC permissions (check which cluster types this is for)
// Security topics, e.g. TLS
// inputs (forwarding logs from x)
// outputs (forwarding logs to y)
// NOTE: forwarding may need its own bucket in topicmap rather than an assembly

////
// fluentd - what replaces this for new implementations? Is the configmap bit still true?
...
// ELASTICSEARCH
To send audit logs to an internal Elasticsearch log store, use the Cluster Log Forwarder as described in xref:../logging/config/cluster-logging-log-store.adoc#cluster-logging-elasticsearch-audit_cluster-logging-store[Forward audit logs to the log store].
////

// Forwarding logs from specific projects (INPUTS)
...
// Forwarding logs to different applications
// Splunk
...
// move to audit logs part
[NOTE]
====
By default, the {logging} sends container and infrastructure logs to the default internal log store defined in the `ClusterLogging` custom resource. However, it does not send audit logs to the internal store because it does not provide secure storage.
====
...
// needs to move to its own module
...
[id="cluster-logging-log-forwarding-reference"]
== Reference
