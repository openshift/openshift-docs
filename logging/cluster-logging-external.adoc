...
By default, the {logging-title} sends container and infrastructure logs to the default internal log store defined in the `ClusterLogging` custom resource (CR).
The {logging} does not send audit logs to external or third-party log stores by default, because these log stores do not provide secure storage.

If the default, internal log store configuration meets your needs, you do not need to configure the `ClusterLogForwarder` CR. However, to send logs to other log aggregators, including third-party logging systems, you must configure the `ClusterLogForwarder` CR.

The `ClusterLogForwarder` CR enables you to send container, infrastructure, and audit logs to specific endpoints within or outside of your cluster. In addition, you can send different types of logs to various systems so that different users can access each type of log. You can also enable Transport Layer Security (TLS) support to send logs securely, as required by your organization.

[NOTE]
====
To send audit logs to the default internal Elasticsearch log store, you must configure the `ClusterLogForwarder` CR as described in xref:../logging/config/cluster-logging-log-store.adoc#cluster-logging-elasticsearch-audit_cluster-logging-store[Forward audit logs to the log store].
====
...
[IMPORTANT]
====
You cannot use both config maps and the `ClusterLogForwarder` CR to configure log forwarding in the same cluster. You must use either config maps or the `ClusterLogForwarder` CR exclusively.
====

include::modules/cluster-logging-collector-log-forwarding-about.adoc[leveloffset=+1]

// Filtering audit log information
include::modules/cluster-logging-audit-log-filtering.adoc[leveloffset=+1]
