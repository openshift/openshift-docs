...
The logging system can forward JSON-formatted log entries to external systems such as syslog. These log entries are formatted as a fluentd message with extra fields such as `kubernetes`. The fields exported by the logging system and available for searching from Elasticsearch and Kibana are documented at the end of this document.
// TBD - add an xref?
...
include::modules/cluster-logging-json-log-entry-format.adoc[leveloffset=+1]
